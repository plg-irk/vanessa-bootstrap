...
//
// Параметры:
//  ДокументТабель - ДокументСсылка.Табель
...
Процедура ИнициализироватьДанныеДокумента(ДокументТабель, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументТабель, "СпособВводаДанных")
		= Перечисления.СпособыВводаДанныхОВремени.ВЦеломЗаПериод Тогда
...
// Параметры:
//	ВидВремени - СправочникСсылка.ВидыРабочегоВремени
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|	,Табель.Организация.НаименованиеПолное КАК ОрганизацияНаименованиеПолное
		|	,Табель.Организация КАК Организация
		|	,Табель.СпособВводаДанных КАК СпособВводаДанных
		...
		НомерДокумента = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(
			Шапка.ДатаДокумента, Шапка.Номер, Шапка.Префикс);
		...
		Если Шапка.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ПоДням Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Для Счетчик = 1 По 15 Цикл
					...
					ДанныеДня = ВидыВремениЧасыПоДню(Выборка, Счетчик);
					
					СтрокаВидВремени = "" + ДанныеДня.ПервыйВидВремени
						+ ?(ЗначениеЗаполнено(ДанныеДня.ВторойВидВремени), "/" + ДанныеДня.ВторойВидВремени, "")
						+ ?(ЗначениеЗаполнено(ДанныеДня.ТретийВидВремени), "/" + ДанныеДня.ТретийВидВремени, "");
					СтрокаЧасы = "" + ?(ДанныеДня.ПервыйЧасы = 0, "", ДанныеДня.ПервыйЧасы)
						+ ?(ЗначениеЗаполнено(ДанныеДня.ВторойВидВремени), "/" + ДанныеДня.ВторойЧасы, "")
						+ ?(ЗначениеЗаполнено(ДанныеДня.ТретийВидВремени), "/" + ДанныеДня.ТретийЧасы, "");
					
					ОбластьДетали.Параметры["Символ" + Счетчик] = СтрокаВидВремени;
					ОбластьДетали.Параметры["ДополнительноеЗначение" + Счетчик] = СтрокаЧасы;
					
					Часы = ?(ПрибавитьКОтработанномуВремени(ДанныеДня.ПервыйВидВремени), ДанныеДня.ПервыйЧасы, 0)
						+ ?(ПрибавитьКОтработанномуВремени(ДанныеДня.ВторойВидВремени), ДанныеДня.ВторойЧасы, 0)
						+ ?(ПрибавитьКОтработанномуВремени(ДанныеДня.ТретийВидВремени), ДанныеДня.ТретийЧасы, 0);
					ЧасыПерваяПоловина = ЧасыПерваяПоловина +  Часы;
					ДниПерваяПоловина = ДниПерваяПоловина + ?(Часы > 0, 1, 0);
					
				КонецЦикла;
				
				Для Счетчик = 16 По День(КонецМесяца(Шапка.ПериодРегистрации)) Цикл
				...
				Для Счетчик = День(КонецМесяца(Шапка.ПериодРегистрации)) + 1 По 31 Цикл
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				СтрокаВидВремени = "" + Выборка.ВидВремени1
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени2), "/" + Выборка.ВидВремени2, "")
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени3), "/" + Выборка.ВидВремени3, "")
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени4), "/" + Выборка.ВидВремени4, "")
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени5), "/" + Выборка.ВидВремени5, "")
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени6), "/" + Выборка.ВидВремени6, "");
				СтрокаЧасы = "" + ?(Выборка.ВидВремени1 = 0, "", Выборка.Часов1)
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени2), "/" + Выборка.Часов2, "")
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени3), "/" + Выборка.Часов3, "")
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени4), "/" + Выборка.Часов4, "")
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени5), "/" + Выборка.Часов5, "")
					+ ?(ЗначениеЗаполнено(Выборка.ВидВремени6), "/" + Выборка.Часов6, "");
				
				ОбластьДетали.Параметры.Символ1 = СтрокаВидВремени;			 
				ОбластьДетали.Параметры.ДополнительноеЗначение1 = СтрокаЧасы;					 
				
				ОбластьДетали.Параметры.ЧасыЗаМесяц
							 = ?(ПрибавитьКОтработанномуВремени(Выборка.ВидВремени1), Выборка.Часов1, 0) 
				...
				ОбластьДетали.Параметры.ДниЗаМесяц
							= ?(ПрибавитьКОтработанномуВремени(Выборка.ВидВремени1), Выборка.Дней1, 0) 
		...
		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(
			ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, ТекущийДокумент);
...
//
// Параметры:
//   МассивОбъектов  - Массив из ДокументСсылка.Табель - Массив ссылок на объекты которые нужно распечатать
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Табель") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Табель",
			 НСтр("ru = 'Табель учета рабочего времени'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати));
...
Функция ВидыВремениЧасыПоДню(Выборка, НомерДня)

	Результат = Новый Структура;
	Результат.Вставить("ПервыйВидВремени", Выборка["ПервыйВидВремени" + НомерДня]);
	Результат.Вставить("ВторойВидВремени", Выборка["ВторойВидВремени" + НомерДня]);
	Результат.Вставить("ТретийВидВремени", Выборка["ТретийВидВремени" + НомерДня]);
	Результат.Вставить("ПервыйЧасы", Выборка["ПервыйЧасы" + НомерДня]);
	Результат.Вставить("ВторойЧасы", Выборка["ВторойЧасы" + НомерДня]);
	Результат.Вставить("ТретийЧасы", Выборка["ТретийЧасы" + НомерДня]);
