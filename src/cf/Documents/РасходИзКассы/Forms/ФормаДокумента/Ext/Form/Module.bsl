...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
		СтавкаНДСПоУмолчанию = Справочники.СтавкиНДС.СтавкаНДС(
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Организация, "ВидСтавкиНДСПоУмолчанию"),
	...
	РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Объект.Контрагент, "ВестиРасчетыПоДокументам, ВестиУчетОплатыПоСчетам");
	...
	Если ТипЗнч(Параметры.ЗначенияЗаполнения) = Тип("Структура")
	   ...
	   И Параметры.ЗначенияЗаполнения.ЗаполнитьРасшифровкаПлатежа Тогда
		...
		СтрокаТабличнойЧасти.СуммаНДС = СтрокаТабличнойЧасти.СуммаПлатежа - (СтрокаТабличнойЧасти.СуммаПлатежа)
			/ ((ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТабличнойЧасти.СтавкаНДС, "Ставка") + 100) / 100);
...
Процедура СуммаДокументаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СуммаДокументаПриИзмененииОбработчик();
...
Процедура РасшифровкаПлатежаСуммаПлатежаПриИзменении(Элемент)
	...
	РасшифровкаПлатежаСуммаПлатежаПриИзмененииОбработчик();
...
Процедура РасшифровкаПлатежаПрочиеРасчетыКурсПриИзменении(Элемент)
	...
	РасшифровкаПлатежаПрочиеРасчетыКурсПриИзмененииОбработчик();
...
&НаКлиенте
Процедура РасшифровкаПлатежаПрочиеРасчетыКурсПриИзмененииОбработчик()
...
&НаКлиенте
Процедура РасшифровкаПлатежаСуммаПлатежаПриИзмененииОбработчик()
...
&НаКлиенте
Процедура СуммаДокументаПриИзмененииОбработчик()
...
КонецПроцедуры // СуммаДокументаПриИзмененииОбработчик()
...
Процедура КурсРасчетовОбработкаВыбораЗавершение(ДатаКурса, ДополнительныеПараметры) Экспорт
	...
	Если ДатаКурса <> Неопределено Тогда
		КурсНаДату = ПолучитьКурсВалютыНаСервере(Элементы.РасшифровкаПлатежаПрочиеРасчеты.ТекущиеДанные.Договор, ДатаКурса);
		Элементы.РасшифровкаПлатежаПрочиеРасчеты.ТекущиеДанные.Курс = КурсНаДату.Курс;
		Элементы.РасшифровкаПлатежаПрочиеРасчеты.ТекущиеДанные.Кратность = КурсНаДату.Кратность;
		РасшифровкаПлатежаПрочиеРасчетыКурсПриИзмененииОбработчик();
	КонецЕсли;
...
Функция ПолучитьКурсВалютыНаСервере(Договор, ДатаКурса)
	
	Возврат РаботаСКурсамиВалют.ПолучитьКурсВалюты(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор,
		"ВалютаРасчетов"), ДатаКурса);
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Контрагент, "ВестиРасчетыПоЗаказам") Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.СчетНаОплату) Тогда
			...
			Если НЕ ЗначениеЗаполнено(ТекущиеДанные.Заказ) И
				ТипЗнч(ТекущиеДанные.СчетНаОплату) = Тип("ДокументСсылка.СчетНаОплатуПоставщика") Тогда
				
				ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущиеДанные.СчетНаОплату,
					"ДокументОснование");
				Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
					ТекущиеДанные.Заказ = ДокументОснование;
				КонецЕсли;
...
Процедура ПересчитатьСуммыДокумента(Курс, Кратность, ПересчитатьСуммуПлатежа, ДополнительныеПараметры)
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.РасшифровкаПлатежа Цикл
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТабличнойЧасти.Договор, "ВалютаРасчетов")
			= Объект.ВалютаДенежныхСредств Тогда
...
Функция ПолучитьДанныеПодотчетникПриИзменении(Подотчетник, Дата)
	...
	РеквизитыПодотчетника = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Подотчетник, "Наименование, ФизЛицо");
	Запрос.УстановитьПараметр("Подотчетник", РеквизитыПодотчетника.ФизЛицо);
	Запрос.УстановитьПараметр("Дата", Дата);
	
	ВыборкаРезультатаЗапроса = Запрос.Выполнить().Выбрать();
	
	СтруктураДанные.ПодотчетникНаименование = РеквизитыПодотчетника.Наименование;
...
Функция ПолучитьДанныеВыплатаЗаработнойПлатыВедомостьПриИзменении(Ведомость)
	...
	РеквизитыВедомости = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ведомость, "СтруктурнаяЕдиница, Сотрудники");
	ПараметрыВедомости = Новый Структура;
	ПараметрыВедомости.Вставить("Подразделение", РеквизитыВедомости.СтруктурнаяЕдиница);
	ПараметрыВедомости.Вставить("СуммаПлатежа", РеквизитыВедомости.Сотрудники.Выгрузить().Итог("СуммаПлатежа"));
...
Процедура ВидОперацииПриИзмененииНаСервере(ЭтоЗаполнениеПоОснованию = Ложь)
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ЛичныеСредстваПредпринимателя Тогда
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Организация, "ЮридическоеФизическоеЛицо")
...
Функция ДокументУтвержден(ДокументОснование)
	...
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "СтатусУтвержденияПлатежа")
		= Перечисления.СтатусыУтвержденияПлатежей.Утвержден;
...
Функция ДанныеКонтрагентаДляИзменения(Знач Контрагент, Знач КонтрагентПередИзменением, Знач Организация, Знач Дата)
	...
	Результат.Вставить("КонтрагентНаименованиеПолное",
	...
	Результат.Вставить("КонтрагентПередИзменениемНаименованиеПолное",
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтрагентПередИзменением, "НаименованиеПолное"));
	...
	Результат.Вставить(
		...
		РегистрыСведений.КурсыВалют.ПолучитьПоследнее(
			Дата,
			Новый Структура("Валюта", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорПоУмолчанию, "ВалютаРасчетов"))));
	
	РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Объект.Контрагент, "ВестиРасчетыПоДокументам, ВестиРасчетыПоДоговорам, ВестиРасчетыПоЗаказам,
		| ВестиУчетОплатыПоСчетам, СтатьяДДСПоУмолчанию");

	Результат.Вставить("ВестиРасчетыПоДокументам", РеквизитыКонтрагента.ВестиРасчетыПоДокументам = Истина);
	Результат.Вставить("ВестиРасчетыПоДоговорам", РеквизитыКонтрагента.ВестиРасчетыПоДоговорам = Истина);
	Результат.Вставить("ВестиРасчетыПоЗаказам", РеквизитыКонтрагента.ВестиРасчетыПоЗаказам = Истина);
	Результат.Вставить("ВестиУчетОплатыПоСчетам", РеквизитыКонтрагента.ВестиУчетОплатыПоСчетам = Истина);
	...
	Результат.Вставить("СтатьяДДСПоУмолчанию", РеквизитыКонтрагента.СтатьяДДСПоУмолчанию);
	Если ЗначениеЗаполнено(РеквизитыКонтрагента.СтатьяДДСПоУмолчанию) Тогда
		Результат.Вставить("УчитыватьВНУ", РегламентированнаяОтчетностьУСН.НужноУчитыватьВНУ(
			РеквизитыКонтрагента.СтатьяДДСПоУмолчанию, Объект.ВидОперации, Организация, Дата));
	Иначе
...
Процедура ОчиститьРеквизитыНеОтносящиесяКОперации()
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику") Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Подразделение) Тогда
			Объект.Подразделение = ПерсональныеНастройкиРУНФВызовСервера.Прочитать("ОсновноеПодразделение", ,
				ТекущийПользовательСистемы, СтруктурнаяЕдиницаПодразделение);
		КонецЕсли;
		Для каждого СтрокаТаблицы Из Объект.РасшифровкаПлатежа Цикл
			СтрокаТаблицы.Договор = Неопределено;
			СтрокаТаблицы.ПризнакАванса = Ложь;
...
Функция ДоговорКредитаЗаймаПриИзмененииНаСервере(ДоговорКредитаЗайма, Дата)
	...
	РеквизитыДоговораКредитаЗайма = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДоговорКредитаЗайма,
		"ВалютаРасчетов, Контрагент, Сотрудник, ВидДоговора");
			
	СтруктураДанные = Новый Структура;
	
	СтруктураДанные.Вставить("ВалютаРасчетов", РеквизитыДоговораКредитаЗайма.ВалютаРасчетов);
	СтруктураДанные.Вставить("Контрагент", РеквизитыДоговораКредитаЗайма.Контрагент);
	СтруктураДанные.Вставить("Сотрудник", РеквизитыДоговораКредитаЗайма.Сотрудник);
	СтруктураДанные.Вставить("ЭтоДоговорЗайма",
		РеквизитыДоговораКредитаЗайма.ВидДоговора = Перечисления.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику);
...
Процедура ЗаполнитьПоДоговоруЗаймаНаСервере()
	...
	Если Выборка.Следующий() Тогда
		...
		КонецЕсли;
	Иначе
		РеквизитыДоговораКредитаЗайма = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДоговорКредитаЗайма,
			"ВалютаРасчетов, СуммаДокумента");
		Объект.СуммаДокумента = РеквизитыДоговораКредитаЗайма.СуммаДокумента;
		Объект.ВалютаДенежныхСредств = РеквизитыДоговораКредитаЗайма.ВалютаРасчетов;
	КонецЕсли;
...
//
// Параметры:
//  СтруктураРезультата - Структура - содержит код возврата диалога и признак изменений настроек налогообложения.
...
Процедура ЗаполнитьТаблицуПросмотраНаСервере(ПоДвижениям = Ложь, ПроверилиНаКлиенте = Ложь)
	...
	РасчетыРаботаСФормамиВызовСервера.ЗаполнитьТаблицуПросмотраВДенежномДокументеНаСервере(
		ЭтотОбъект, ПоДвижениям, ПроверилиНаКлиенте);
