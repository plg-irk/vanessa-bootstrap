...
// Заполняет ограничение доступа для документа "Расходный кассовый ордер".
//
// Параметры:
...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОстатокСуммы", СтруктураДополнительныеСвойства.ДляПроведения.ОстатокСуммыДляПлатежногоКалендаря);
	...
	// БЗКБ
	Если Справочники.Организации.НужноПоказыватьЭлементыБЗКБДляОрганизации(СтруктураДополнительныеСвойства.ДляПроведения.ОрганизацияДляРегОтчетности) Тогда
		Если СтруктураДополнительныеСвойства.ДляПроведения.ТЧВыплатаРегламентированнойЗаработнойПлатыКоличествоСтрок > 0
			И СтруктураДополнительныеСвойства.ДляПроведения.ТЧВыплатаЗаработнойПлатыКоличествоСтрок = 0 Тогда
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику
		ИЛИ СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.НашейОрганизации Тогда
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Покупателю Тогда
		РасчетыПроведениеДокументов.СформироватьДвиженияПоВзаиморасчетам(СтруктураДополнительныеСвойства, ДокументСсылкаРасходИзКассы, , "Касса, ВалютаДенежныхСредств");
		ПолучатьРеквизитыРасчетов = Истина;
	Иначе
		ЭтоРасчетыСПоставщиком = СтруктураДополнительныеСвойства.ПараметрыДокумента.ЭтоРасчетыСПоставщиком;
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации =
		Перечисления.ВидыОперацийРасходИзКассы.Покупателю Тогда
...
Процедура СформироватьТаблицаРасчетыСПерсоналомБЗКБ(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ТЧВыплатаРегламентированнойЗаработнойПлатыКоличествоСтрок > 0
		И СтруктураДополнительныеСвойства.ДляПроведения.ТЧВыплатаЗаработнойПлатыКоличествоСтрок = 0
		И Справочники.Организации.НужноПоказыватьЭлементыБЗКБДляОрганизации(СтруктураДополнительныеСвойства.ДляПроведения.ОрганизацияДляРегОтчетности) Тогда
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
	Запрос.УстановитьПараметр("ПроектВШапке", СтруктураДополнительныеСвойства.ДляПроведения.ПроектВШапке);
	Запрос.УстановитьПараметр("ПодразделениеВШапке", СтруктураДополнительныеСвойства.ДляПроведения.ПодразделениеВШапке);
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаРасходИзКассы);
	Запрос.УстановитьПараметр("ВидОперации", СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
...
Процедура СформироватьНДФЛРасчетыНалоговыхАгентовСБюджетом(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Если ВыборкаПоНДФЛ.Следующий() Тогда
		...
		И СтруктураДополнительныеСвойства.ДляПроведения.ПериодРегистрации > УчетНДФЛПредпринимателяКлиентСервер.ДатаНачалаПрогрессивнойШкалы()
		И СтруктураДополнительныеСвойства.ДляПроведения.ВидНалога = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины; 
		...
		Если РасчетПоПрогрессивнойШкале Тогда
			СтрокиРаспределения = СотрудникиУНФ.РаспределитьПеречисленныйНалогПоФизическимЛицамСПревышения(
				ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства.ДляПроведения.ОрганизацияДляРегОтчетности,
				ВыборкаПоНДФЛ.Период , ВыборкаПоНДФЛ.Сумма, ВыборкаПоНДФЛ.ПериодРегистрации);
		Иначе
...
Процедура СформироватьРасчетыСФондамиПоСтраховымВзносам(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Если ВыборкаПоВзносам.Следующий() Тогда
		СтрокиРаспределения = СотрудникиУНФ.РаспределитьПеречисленныйВзносПоФизическимЛицам(ДокументСсылкаРасходИзКассы,
			СтруктураДополнительныеСвойства.ДляПроведения.ОрганизацияДляРегОтчетности, ВыборкаПоВзносам.Период,
			ВыборкаПоВзносам.Сумма, ВыборкаПоВзносам.ВидОбязательногоСтрахованияСотрудников);
		Для Каждого СтрокаТЗ Из СтрокиРаспределения Цикл
...
//
// Параметры:
//  ДокументСсылкаРасходИзКассы - ДокументСсылка.РасходИзКассы - документ-источник движений
//  СтруктураДополнительныеСвойства	 - Структура - свойство объекта ДополнительныеСвойства
//
Процедура СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства) Экспорт
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасчетыСПоставщиками.Дата, ДЕНЬ) КАК ДатаПервичногоДокумента,
		               |	ТаблицаДокумента.Номер КАК НомерПервичногоДокумента,
		               ...
		               |ИЗ
		               |	ВременнаяТаблицаРасчетыСПоставщиками КАК ВременнаяТаблицаРасчетыСПоставщиками
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходИзКассы КАК ТаблицаДокумента
		...
		УчетПартийУСН.ЗаполнитьДвиженияОплатаПартий(ДокументСсылкаРасходИзКассы, Запасы, ТаблицаОплаты, ТаблицаПартииТоваров);
		...
		Если ТаблицаПартииТоваров.Найти(Истина, "ОтражатьВКУДиР") <> Неопределено Тогда
			...
			Если ТолькоТовары Тогда
				СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеОплатаПоставщикуЗаТовары(Истина, ТекстПоставщик);
			Иначе
				СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеОплатаПоставщикуЗаУслуги(Истина, ТекстПоставщик);
			КонецЕсли;
		КонецЕсли;		
		
	ИначеЕсли СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Покупателю Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ТаблицаДокумента.Контрагент КАК Контрагент,
		               |	НАЧАЛОПЕРИОДА(ВременнаяТаблицаРасчетыСПокупателями.Дата, ДЕНЬ) КАК ДатаПервичногоДокумента,
		               |	ТаблицаДокумента.Номер КАК НомерПервичногоДокумента,
		               |	ТаблицаДокумента.Контрагент.НаименованиеПолное КАК ЮридическоеНаименованиеКонтрагента,
		               |	ТаблицаДокумента.Контрагент.Наименование КАК НаименованиеКонтрагента,
		               |	ТаблицаДокумента.Дата КАК Период
		               ...
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходИзКассы.РасшифровкаПлатежа КАК РасходСоСчетаРасшифровкаПлатежа
		               ...
		               |	ВТДокументыВозврата.Контрагент КАК Контрагент,
		               ...
		               |	ВТДокументыВозврата.ЮридическоеНаименованиеКонтрагента КАК ЮридическоеНаименованиеКонтрагента,
		               |	ВТДокументыВозврата.НаименованиеКонтрагента КАК НаименованиеКонтрагента,
...
//
// Параметры:
//  ДокументСсылкаРасходИзКассы - ДокументСсылка.РасходИзКассы - Ссылка на документ "Расход из кассы".
//  СтруктураДополнительныеСвойства - Структура - Структура дополнительных свойств для проведения документа:
//  * ДляПроведения - Структура - Структура данных для проведения документа:
//    ** ОрганизацияДляРегОтчетности - СправочникСсылка.Организации - Организация для регламентированной отчетности.
//
Процедура СформироватьДвиженияНаЕдиномНалоговомСчете(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаРасходИзКассы);
	Запрос.УстановитьПараметр("ОрганизацияДляРегОтчетности", СтруктураДополнительныеСвойства.ДляПроведения.ОрганизацияДляРегОтчетности);   
	Запрос.УстановитьПараметр("ГоловнаяОрганизация", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		СтруктураДополнительныеСвойства.ДляПроведения.ОрганизацияДляРегОтчетности, "ГоловнаяОрганизация"));
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
...
Процедура СформироватьТаблицаВыполнениеДоговораОбслуживания(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураДополнительныеСвойства.ДляПроведения.НаправлениеДеятельности)
...
// Инициализирует данные документа РасходИзКассы для проведения.
// Инициализирует таблицы значений, содержащие данные табличных частей документа.
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
// Параметры:
//  ДокументСсылкаРасходИзКассы - ДокументСсылка.РасходИзКассы - Ссылка на документ.
//  СтруктураДополнительныеСвойства - Структура - Дополнительные свойства для проведения.
//
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("ВалютаУчета", СтруктураДополнительныеСвойства.ВалютаУчета);
	Запрос.УстановитьПараметр("ВалютаДС", СтруктураДополнительныеСвойства.ВалютаДокумента);
	Запрос.УстановитьПараметр("ВалютаДоговораКредитаЗайма", СтруктураДополнительныеСвойства.ДляПроведения.ВалютаДоговораКредитаЗайма);
	Если НЕ ЗначениеЗаполнено(СтруктураДополнительныеСвойства.ДляПроведения.НаправлениеДеятельности)
		ИЛИ НЕ ПолучитьФункциональнуюОпцию("ИспользоватьБиллинг")
		ИЛИ НЕ Константы.БиллингВестиУчетРасходовПоДоговорамОбслуживания.Получить() Тогда
		Запрос.УстановитьПараметр("ВалютаДоговораОбслуживания", Справочники.Валюты.ПустаяСсылка());
	Иначе
		ДоговорОбслуживания = Справочники.ДоговорыКонтрагентов.НайтиПоРеквизиту("ДоговорОбслуживанияНаправлениеДеятельности",
			СтруктураДополнительныеСвойства.ДляПроведения.НаправлениеДеятельности);
		Запрос.УстановитьПараметр("ВалютаДоговораОбслуживания", ?(ЗначениеЗаполнено(ДоговорОбслуживания),
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорОбслуживания, "ВалютаРасчетов"),
			Справочники.Валюты.ПустаяСсылка()));
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ОбъединятьНакопительнуюИСтраховуюЧасть",
		СтруктураДополнительныеСвойства.ДляПроведения.ДатаДокументаОснования >= УчетСтраховыхВзносовУНФ.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР());
	Запрос.УстановитьПараметр("ПустаяДата", Дата('00010101'));
	Запрос.УстановитьПараметр("ПериодРегистрацииНДФЛ",
		НачалоМесяца(ДобавитьМесяц(СтруктураДополнительныеСвойства.ДляПроведения.СрокУплаты, -1)));
	...
	СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства);
...
// Выполнить контроль возникновения отрицательных остатков.
// 
// Параметры:
//  ДокументСсылкаРасходИзКассы - ДокументСсылка.РасходИзКассы - Документ ссылка расход из кассы
	...
	Если СтруктураВременныеТаблицы.ДвиженияДенежныеСредстваИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияДенежныеСредстваВРезервеИзменение Тогда
		...
		Если НЕ МассивРезультатов[7].Пустой() Тогда
			...
			КонтрольОстатковУНФ.РасчетыСПокупателями(ДокументОбъектРасходИзКассы, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура СформироватьРКО(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати)
	...
	|	МАКСИМУМ(ПлатежнаяВедомостьСотрудники.Сотрудник.СчетРасчетовСПерсоналом.Код) КАК ДебетСубСчет
	...
	|ГДЕ
	|	РасходИзКассыВыплатаЗаработнойПлаты.Ссылка В (&Документы)
	|	И РасходИзКассыВыплатаЗаработнойПлаты.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Зарплата)
	|СГРУППИРОВАТЬ ПО
	|	РасходИзКассыВыплатаЗаработнойПлаты.Ссылка
	|;
	|
	...
	|	РасходИзКассы.Подразделение.НаименованиеПолное КАК ПредставлениеПодразделения,
	...
	|	ВЫБОР
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Поставщику)
	|		ИЛИ РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.НашейОрганизации)
	|			ТОГДА РасходИзКассы.Контрагент.СчетУчетаРасчетовСПоставщиком.Код
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Покупателю)
	|			ТОГДА РасходИзКассы.Контрагент.СчетУчетаРасчетовСПокупателем.Код
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Подотчетнику)
	|			ТОГДА РасходИзКассы.Подотчетник.СчетРасчетовСПодотчетниками.Код
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Налоги)
	|			ТОГДА РасходИзКассы.ВидНалога.СчетУчета.Код
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Прочее)
	|		ИЛИ РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.НаРасходы)
	|		ИЛИ РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.ПрочиеРасчеты)
	|		ИЛИ РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.РасчетыПоКредитам)
	|		ИЛИ РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.ВыдачаЗаймаСотруднику)
	|		ИЛИ РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.ВзносНаличнымиВБанк)
	|			ТОГДА РасходИзКассы.Корреспонденция.Код
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.ПеремещениеВКассуККМ)
	|			ТОГДА РасходИзКассы.КассаККМ.СчетУчета.Код
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику)
	|			ТОГДА РасходИзКассы.Подотчетник.СчетРасчетовСПерсоналом.Код
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Зарплата)
	|			ТОГДА ЕСТЬNULL(ВременнаяТаблицаЗарплата.ДебетСубСчет, НЕОПРЕДЕЛЕНО)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДебетСубСчет,
	|	ВЫБОР
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Поставщику)
	|		ИЛИ РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.НашейОрганизации)
	|			ТОГДА РасходИзКассы.Контрагент.СчетУчетаАвансовПоставщику.Код
	|		КОГДА РасходИзКассы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Покупателю)
	|			ТОГДА РасходИзКассы.Контрагент.СчетУчетаАвансовПокупателя.Код
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДебетСубСчетАвансов
	|ИЗ
	|	Документ.РасходИзКассы КАК РасходИзКассы
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗарплата КАК ВременнаяТаблицаЗарплата
	|		ПО РасходИзКассы.Ссылка = ВременнаяТаблицаЗарплата.Ссылка
	|ГДЕ
	|	РасходИзКассы.Ссылка В (&Документы)";
...
Функция СформироватьПечатнуюФормуКвитанцияКСумке(МассивОбъектов, ОбъектыПечати)
	...
	ТаблицаРасшифровкаПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МассивОбъектов[0], "РасшифровкаПлатежа");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если НЕ ЕстьОшибкиПечатиИнкассации(ДанныеПечати) Тогда
			...
			СведенияООрганизации.Вставить("ЮрФизЛицо", ДанныеПечати.ЮрФизЛицо);
...
Функция СформироватьПечатнуюФормуПрепроводительнаяВедомостьНакладнаяКСумке(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Если НЕ МассивОбъектов.Количество() = 0 Тогда
		...
		РеквизитыОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектПечати,
			"ОписьСдаваемыхНаличныхДенег, РасшифровкаПлатежа");		
		ТаблицаОписьСдаваемыхНаличныхДенег = РеквизитыОбъекта.ОписьСдаваемыхНаличныхДенег;
		ТаблицаРасшифровкаПлатежа          = РеквизитыОбъекта.РасшифровкаПлатежа;
...
//
// Параметры:
//  МассивОбъектов - Массив из ДокументСсылка.РасходИзКассы - массив объектов для печати.
//  ПараметрыПечати - Структура - параметры печати.
//  КоллекцияПечатныхФорм - ТаблицаЗначений - коллекция печатных форм.
//  ОбъектыПечати - СписокЗначений из ДокументСсылка.РасходИзКассы - массив объектов печати.
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Использование", Метаданные.Отчеты.РеестрДокументов) Тогда
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "РеестрДокументовРасходИзКассы";
...
//                                                         форме документа.
//           ТаблицаРасшифровкиПлатежей - ТаблицаЗначений - Таблица расшифровки платежей
//           
// Возвращаемое значение:
//           Булево - Если ошибки ИСТИНА, необходимо не выполнять печать.
...
// Параметры:
//           ОбластьМакетаСекцияОсновнаяЧасть - ТабличныйДокумент - Область табличного документа для которой необходимо
//                                                             заполнить параметры.
//           ОбластьМакетаСекцияТаблицаОсновнаяЧасть - ТабличныйДокумент - Область табличного документа для которой необходимо
//                                                             заполнить параметры.
...
Процедура ЗаполнитьПараметрыМакетаДляПечатиЛицевойСтороныИнкассации(ОбластьМакетаСекцияОсновнаяЧасть, ОбластьМакетаСекцияТаблицаОсновнаяЧасть, СведенияООрганизации, ДанныеПечати)
	...
	ОбластьМакетаСекцияТаблицаОсновнаяЧасть.Параметры.НомерСумки = Сред(ДанныеПечати.НомерСумки, СтрНайти(ДанныеПечати.НомерСумки, "/") + 1.00);
...
Процедура ЗаполнитьQRКодДляПечатиЛицевойСтороныИнкассации(ОбластьМакетаСекцияОсновнаяЧасть, ТаблицаОписьСдаваемыхНаличныхДенег, ТаблицаРасшифровкаПлатежа, ДанныеПечати, СведенияООрганизации)
	...
	Если XMLШтрихКода <> Неопределено Тогда
		ОбластьМакетаСекцияОсновнаяЧасть.Рисунки.Рисунок.Картинка = СформироватьШтрихкодПечатнойФормы(XMLШтрихКода,
			ОбластьМакетаСекцияОсновнаяЧасть.Рисунки.Рисунок);
	Иначе
		ОбластьМакетаСекцияОсновнаяЧасть.Рисунки.Удалить(ОбластьМакетаСекцияОсновнаяЧасть.Рисунки.Рисунок);
	КонецЕсли;
...
Функция СоздатьXMLШтрихкода(ТаблицаОписьСдаваемыхНаличныхДенег, ДанныеПечати, СведенияООрганизации, ТаблицаРасшифровкаПлатежа)
	...
	ЗаписьXML.ЗаписатьТекст(XMLСтрока(Сред(ДанныеПечати.НомерСумки, СтрНайти(ДанныеПечати.НомерСумки, "/") + 1.00)));
