...
Процедура НалогиСчетУчетаПриИзменении(Элемент)
    ...
    Если СписокСчетовАгентскогоНДС.НайтиПоЗначению(ТекущиеДанные.СчетУчета) = Неопределено Или ТекущийСчетУчета
        <> ТекущиеДанные.СчетУчета Тогда
...
Процедура НалогиСуммаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
    ...
    Если Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеНалогов") Тогда
        Если ТекущиеДанные <> Неопределено И СписокСчетовАгентскогоНДС.НайтиПоЗначению(ТекущиеДанные.СчетУчета)
            <> Неопределено Тогда
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСанкции
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыЕдиныйСчет
...
Процедура Заполнить(Команда)
    ...
    ОткрытьФорму("Документ.ОперацияПоЕдиномуНалоговомуСчету.Форма.ФормаСпискаВыбораНалогов", ПараметрыФормы, ЭтотОбъект,
...
Функция ОписаниеСтрокиДокумента()
    ...
    ОписаниеСтроки.Вставить("НомерСтроки");
    ОписаниеСтроки.Вставить("Налог");
    ОписаниеСтроки.Вставить("КодБК");
    ОписаниеСтроки.Вставить("СчетУчета");
    ОписаниеСтроки.Вставить("КлючСтроки");
    ОписаниеСтроки.Вставить("ВидПлатежа");
    ОписаниеСтроки.Вставить("РегистрацияВНалоговомОргане");
    ОписаниеСтроки.Вставить("КодПоОКТМО");
    ОписаниеСтроки.Вставить("Сумма");
    ОписаниеСтроки.Вставить("СрокУплаты");
    ОписаниеСтроки.Вставить("СчетЗатрат");
    ОписаниеСтроки.Вставить("РедактироватьВидПлатежа");
    ОписаниеСтроки.Вставить("УчитыватьВНУ");
    ОписаниеСтроки.Вставить("Подразделение");
    ОписаниеСтроки.Вставить("АналитикаУчета");
    ОписаниеСтроки.Вставить("АналитикаУчетаЗаполнена");
    ОписаниеСтроки.Вставить("ПоказыватьСчетУчета");
    ОписаниеСтроки.Вставить("ВидНалогаНДС");
...
Процедура ПолучитьРеквизитыОрганизации()
    ...
    Если ЗначениеЗаполнено(Объект.Организация) Тогда
        РегистрацияВНалоговомОргане = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Организация,
...
Процедура НалогПриИзмененииНаСервере(ПараметрыСтроки)
    ...
    Документы.ОперацияПоЕдиномуНалоговомуСчету.ЗаполнитьДанныеНалогаВСтрокеТабличнойЧасти(ПараметрыСтроки,
        Объект.Организация, Объект.Дата);

    Если СписокСчетовАгентскогоНДС.НайтиПоЗначению(ПараметрыСтроки.СчетУчета) = Неопределено Или ТекущийСчетУчета
        <> ПараметрыСтроки.СчетУчета Тогда
...
Процедура РегистрацияВНалоговомОрганеПриИзмененииНаСервере(ПараметрыСтроки)
    ...
    Если ЗначениеЗаполнено(ПараметрыСтроки.РегистрацияВНалоговомОргане) Тогда
        ПараметрыСтроки.КодПоОКТМО = РегламентированнаяОтчетностьУСН.КодТерритории(
            ПараметрыСтроки.РегистрацияВНалоговомОргане);
    КонецЕсли;
...
Процедура ОткрытьФормуРедактированияАналитикиУчета(ВыбранноеЗначение, ИмяТаблицы)
    ...
    ОткрытьФорму("Документ.ОперацияПоЕдиномуНалоговомуСчету.Форма.ФормаРедактированияСтроки", ПараметрыФормы,
        ЭтотОбъект);
...
Процедура ОткрытьФормуРедактированияАгентскогоНДС(ВыбранноеЗначение, ИмяТаблицы)
    ...
    Если (ЗначениеЗаполнено(ДанныеСтроки.СрокУплаты) И ДанныеСтроки.СрокУплаты < Объект.Дата) Или Не ЗначениеЗаполнено(
        ДанныеСтроки.СрокУплаты) Тогда

        ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                НСтр("ru='Некорректно заполнена колонка ""Срок уплаты"" в строке %1 списка ""Налоги"":
                     | - срок уплаты должен быть указан и быть больше даты документа'"), ДанныеСтроки.НомерСтроки);

        ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
            "Налоги", ДанныеСтроки.НомерСтроки, "СрокУплаты"), "Объект");
    ...
    ПараметрыФормы.Вставить("АдресТаблицыНалоговыйАгентНДСВХранилище", АдресТаблицыНалоговыйАгентНДС(
...
Процедура ОбработкаВыбораАналитикиУчетаНаКлиенте(ЗначенияВыбранныеВФормеАналитикиУчета)
    ...
    Если Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеНалогов") Или Объект.ВидОперации
        = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.УплатаНалогов") Тогда
        ИмяТаблицы = "Налоги";
    ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеПенейШтрафов") Или Объект.ВидОперации
        = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.ПогашениеПенейШтрафов") Тогда
        ИмяТаблицы = "Санкции";
    Иначе
        ИмяТаблицы = "ЕдиныйСчет";
    КонецЕсли;
...
Процедура ОбновитьИтоги()
    ...
    Объект.СуммаДокумента = Объект.Налоги.Итог("Сумма") + Объект.Санкции.Итог("Сумма") + Объект.ЕдиныйСчет.Итог("Сумма");
...
Процедура УправлениеФормой(Форма)
    ...
    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ГруппаНалоги", "Видимость",
        Форма.Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеНалогов") Или Форма.Объект.ВидОперации
        = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.УплатаНалогов"));

    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "НалогиПлатежныйДокумент", "Видимость",
        Форма.Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.УплатаНалогов"));

    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "НалогиСчетУчета", "Видимость",
        Форма.ПоказыватьСчетаУчетаВДокументах);

    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "НалогиВидНалогаНДС", "Видимость",
        Не Форма.ПоказыватьСчетаУчетаВДокументах);

    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ГруппаСанкции", "Видимость",
        Форма.Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеПенейШтрафов") Или Форма.Объект.ВидОперации
        = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.ПогашениеПенейШтрафов"));

    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "СанкцииПлатежныйДокумент", "Видимость",
        Форма.Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.ПогашениеПенейШтрафов"));

    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "НалогиАналитикаУчета", "Видимость",
        Форма.Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.УплатаНалогов"));

    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ГруппаЕдиныйСчет", "Видимость",
        Форма.Объект.ВидОперации = ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.КорректировкаСчета"));

    ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ГруппаИтоги", "Видимость",
        Форма.Объект.ВидОперации <> ПредопределенноеЗначение(
        "Перечисление.ВидыОперацийПоЕдиномуНалоговомуСчету.КорректировкаСчета"));
...
Процедура УстановитьУсловноеОформление()
    ...
    Если Объект.ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеНалогов Тогда
        ...
        ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "Объект.Налоги.СчетУчета",
            ВидСравненияКомпоновкиДанных.ВСписке, СписокСчетовАгентскогоНДС);
    ...
    Если Объект.ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеНалогов
        ...
        Если Не ПоказыватьСчетаУчетаВДокументах Тогда
            ...
            ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "Объект.Налоги.Налог",
                ВидСравненияКомпоновкиДанных.НеРавно, Налог_НДС);
            ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);

            Для Каждого ОписаниеСчета Из СписокОписанийСчетовНДС Цикл
                ЭлементУО = УсловноеОформление.Элементы.Добавить();
                КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НалогиВидНалогаНДС");
                ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "Объект.Налоги.ВидНалогаНДС",
                    ВидСравненияКомпоновкиДанных.Равно, ОписаниеСчета.Значение);
    ...
    Если Объект.ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.КорректировкаСчета Тогда
        ...
        ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1, "Объект.ЕдиныйСчет.ВидДвижения",
            ВидСравненияКомпоновкиДанных.Равно, Ложь);
        ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1, "Объект.ЕдиныйСчет.ПлатежныйДокумент",
        ...
        ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "Объект.ЕдиныйСчет.ПлатежныйДокумент",
            ВидСравненияКомпоновкиДанных.НеЗаполнено);
        ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "Объект.ЕдиныйСчет.ВидДвижения",
...
Процедура ВопросЗаполнитьДокумент()
    ...
    ЕстьДанные = Объект.Налоги.Количество() > 0 Или Объект.Санкции.Количество() > 0
        Или Объект.НалоговыйАгентНДС.Количество() > 0 Или Объект.ЕдиныйСчет.Количество() > 0;
...
// Заполняет добавленные колонки "АналитикаУчета" и "АналитикаУчетаЗаполнена" в строке таблицы.
//
// Параметры:
//  ДанныеСтроки - Структура - Строка таблицы, содержащая следующие ключи:
//   * СчетЗатрат - ПланСчетовСсылка - Счет затрат, по которому заполняется аналитика учета.
//   * Налог - СправочникСсылка.ВидыНалогов - Налог, который может быть включен в аналитику учета.
//   * Подразделение - СправочникСсылка.СтруктурныеЕдиницы - Подразделение, которое может быть включено в аналитику учета.
//
&НаСервере
