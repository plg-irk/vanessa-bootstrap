...
// Заполнить по заказу покупателя.
// 
// Параметры:
//  ДанныеЗаполнения - ДокументСсылка.ЗаказПокупателя - Данные заполнения
...
Процедура ЗаполнитьПоЗаказуНаПродажуИлиНаПереработку(ДанныеЗаполнения)
	...
	Если Выборка.Следующий() Тогда
		...
		ПроверяемыеДанные = Новый Структура("ОжидаетсяВыборВариантаКП,ВидОперации");
		ЗаполнитьЗначенияСвойств(ПроверяемыеДанные, Выборка);
		Документы.ЗаказПокупателя.ПроверитьВозможностьВводаНаОснованииЗаказаПокупателя(ДанныеЗаполнения, ПроверяемыеДанные);
...
Процедура ДозаполнитьПоУмолчанию()
	...
	Если ЗначениеЗаполнено(ВидЦен) И ЗначениеЗаполнено(ВидРабот) Тогда
		...
		СтруктураДанных.Вставить("ВалютаДокумента",	ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидЦен, "ВалютаЦены"));
...
Процедура ЗаполнитьКолонкуЗаписиКалендаря(ЗаписьКалендаря)
	...
	ЗаписьКалендаряКолонкаКалендаряВладелец = Справочники.КалендариСотрудников.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(ЗаписьКалендаря.КолонкаКалендаря) Тогда
		ЗаписьКалендаряКолонкаКалендаряВладелец = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			ЗаписьКалендаря.КолонкаКалендаря, "Владелец");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ЗаписьКалендаря.КолонкаКалендаря)
		Или ЗаписьКалендаряКолонкаКалендаряВладелец <> ЗаписьКалендаря.Календарь Тогда
		ЗаписьКалендаря.КолонкаКалендаря = Справочники.КолонкиКалендарейСотрудников.КолонкаНеобработанное(
			ЗаписьКалендаря.Календарь);
	КонецЕсли;
