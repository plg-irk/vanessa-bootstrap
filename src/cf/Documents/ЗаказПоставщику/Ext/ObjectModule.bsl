...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Авторегистрация цен
	Документы.ЗаказПоставщику.ЗарегистрироватьЦеныПоставщика(Ссылка, Отказ);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	// Авторегистрация цен
	Документы.ЗаказПоставщику.УдалитьЦеныПоставщика(Ссылка);
...
// Заполнить по заказу на производство.
// 
// Параметры:
...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДокументСсылкаЗаказНаПроизводство) Экспорт
	...
	Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
		...
		СтавкаНДС = СоответствиеСтавок.Получить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация,
	...
	Для каждого СтрокаЗапасы Из Запасы Цикл
		Если СтавкаНДСИзНоменклатуры Тогда
			Если Не ЗначениеЗаполнено(СтрокаЗапасы.СтавкаНДС) И ЗначениеЗаполнено(СтрокаЗапасы.Номенклатура) Тогда
				СтрокаЗапасы.СтавкаНДС = СоответствиеСтавок.Получить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					СтрокаЗапасы.Номенклатура, "ВидСтавкиНДС"));
			КонецЕсли;
		Иначе
...
Процедура ЗаполнитьКолонкуРезервПоОстаткам() Экспорт
	...
	|ГДЕ
	|	НЕ ТаблицаЗапасы.ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)";
	...
	|		ЗапасыОстатки.ЗаказПокупателя
	|	ИЗ
	|		РегистрНакопления.Запасы.Остатки(, (Организация, СтруктурнаяЕдиница, СчетУчета, Номенклатура, Характеристика,
	|			Партия, ЗаказПокупателя) В
	|			(ВЫБРАТЬ
	|				&Организация,
	|				&СтруктурнаяЕдиница,
...
// Дозаполнить по умолчанию.
// 
// Параметры:
//  ДанныеЗаполнения - Структура из КлючИЗначение
	...
	Если ТаблицаУзлы = Неопределено Тогда
		...
		Для каждого СтрокаПродукция Из ТаблицаПродукция Цикл
			Если ЗначениеЗаполнено(СтрокаПродукция.ЕдиницаИзмерения)
				...
				СтрокаПродукция.Коэффициент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					СтрокаПродукция.ЕдиницаИзмерения, "Коэффициент");
			Иначе
