...
//Заполняет ограничение доступа
//
//Параметры:
// Ограничение - См. УправлениеДоступомПереопределяемый.ПриЗаполненииОграниченияДоступа
...
// Проверить возможность ввода на основании заказа поставщику.
// 
// Параметры:
...
Процедура ПроверитьВозможностьВводаНаОснованииЗаказаПоставщику(ДанныеЗаполнения, ЗначенияРеквизитов) Экспорт
	...
	Если ЗначенияРеквизитов.Свойство("СостояниеЗаказа") И ЗначенияРеквизитов.СостояниеЗаказа
		= Справочники.СостоянияЗаказовПоставщикам.Завершен Тогда
...
// Параметры:
//  Заказ - ДокументСсылка.ЗаказПоставщику
//  МоментВремени - Неопределено, Граница - Момент времени
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СтатусОтгрузки - Статус отгрузки
...
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СтатусОплаты, Неопределено - Статус оплаты
...
// Параметры:
//  Заказ - ДокументСсылка.ЗаказПоставщику
//  МоментВремени - Неопределено - Момент времени
// 
// Возвращаемое значение:
//  Число, Неопределено - Процент оплаты
...
// 
// Возвращаемое значение:
//  Массив из ДокументСсылка.ЗаказПоставщику
//
...
// Обеспечиваемые заказы покупателя.
// 
// Параметры:
...
//
// Параметры:
// ТекущийДокумент - ДокументСсылка.ЗаказПоставщику - Документ, ставку по которому надо проверить
...
Функция НаличиеСтавкиУСН(ТекущийДокумент, ВидСтавкиУСН) Экспорт
	...
	|		ПО ЗаказПоставщику.СтавкаНДС = СтавкиНДС.Ссылка
	|			И (ЗаказПоставщику.Ссылка = &СсылкаНаДокумент)
...
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ЗаказПоставщику
//  Отказ - Булево
Процедура ЗарегистрироватьЦеныПоставщика(ДокументСсылка, Отказ) Экспорт
	...
	|ИЗ
	|	Документ.ЗаказПоставщику.Запасы КАК ТаблицаЦены
	|ГДЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПоставщику.Запасы КАК ТаблицаЦены
...
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ЗаказПоставщику - Документ ссылка
...
//  СписокНеоднозначностей    - Массив из СправочникСсылка - Список для заполнения с неоднозначными данными:
...
Функция ВыполненыУсловия(Предмет, Условия) Экспорт
	...
	Для Каждого Условие Из Условия Цикл
		...
		Если Условие.Параметр = "СтатусОтгрузки" Тогда
			Если Не АссистентУправления.ВыполненоУсловиеСравнения(СтатусОтгрузки(Предмет),
		...
		Если Условие.Параметр = "ПроцентОплаты" Тогда
			Если Не АссистентУправления.ВыполненоУсловиеСравнения(ПроцентОплаты(Предмет),
				Условие.ВидСравнения, Условие.Значение) Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция СформироватьЗаказПоставщику(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	|	ЗаказПоставщикуЗапасы.НомерСтроки КАК НомерСтроки,
	|	ЗаказПоставщикуЗапасы.Ссылка КАК Ссылка,
	|	ЕСТЬNULL(ШтрихкодыНоменклатуры.Штрихкод, ЗаказПоставщикуЗапасы.Номенклатура.Штрихкод) КАК Штрихкод
	|ИЗ
	|	Документ.ЗаказПоставщику.Запасы КАК ЗаказПоставщикуЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	|		ПО ЗаказПоставщикуЗапасы.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	|		И ЗаказПоставщикуЗапасы.Характеристика = ШтрихкодыНоменклатуры.Характеристика
	|ГДЕ
...
//  ОбъектыПечати - СписокЗначений из ДокументСсылка 
//   *значение - ДокументСсылка
