...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РаботаСОтборами.НастроитьПанельОтборовМобильныйКлиент(ЭтотОбъект, , ,
		"ОтборКонтрагент,ОтборСостояние,ОтборЗавершениеЗаказа,ОтборОплата,ОтборОтгрузка,ОтборОтветственный,ОтборОрганизация",
		, Истина);
...
Процедура УстановитьСостояниеЗаказа(Команда)
	...
	УстановитьСостояниеЗаказаНаКлиенте(Команда)
...
&НаКлиенте
Процедура УстановитьСостояниеЗаказаОтменен(Команда)
...
&НаКлиенте
Процедура УстановитьСостояниеЗаказаНаКлиенте(Команда)
...
Функция СформироватьЗаказыПоставщикуИЗаписать(МассивЗаказов)
	...
	Для каждого СтрокаЗаказПокупателя Из МассивЗаказов Цикл
		...
		НовыйДокументЗНП.Дата = ОбщегоНазначенияБПО.ДатаСеанса();
...
Процедура УстановитьСостояниеЗаказаСервер(ИмяКоманды, Заказы, ВариантЗавершения = Неопределено)
	...
	Для Каждого Заказ Из Заказы Цикл
		...
		Исключение
			...
			ОбщегоНазначения.СообщитьПользователю(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), Заказ);
			НеизмененныеЗаказы.Добавить(Заказ);
			Продолжить;
