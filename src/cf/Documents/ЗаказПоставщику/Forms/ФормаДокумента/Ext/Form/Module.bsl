...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказыватьКолонкуСодержаниеВЗаказеПоставщику = ПерсональныеНастройкиРУНФ.Прочитать(
		"ПоказыватьКолонкуСодержаниеВЗаказеПоставщику");
	ПоказыватьКолонкуВесВЗаказеПоставщику = ПерсональныеНастройкиРУНФ.Прочитать("ПоказыватьКолонкуВесВЗаказеПоставщику");
	ПоказыватьКолонкуОбъемВЗаказеПоставщику = ПерсональныеНастройкиРУНФ.Прочитать(
		"ПоказыватьКолонкуОбъемВЗаказеПоставщику");
	ПоказыватьКолонкуНоменклатураПоставщикаВЗаказеПоставщику = ПерсональныеНастройкиРУНФ.Прочитать(
		"ПоказыватьКолонкуНоменклатураПоставщикаВЗаказеПоставщику");
			
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИтогОбъем", "Видимость",
		ПоказыватьКолонкуОбъемВЗаказеПоставщику);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИтогВес", "Видимость",
...
Процедура ЗаказПокупателяПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ЗаказПокупателя) Тогда
		Объект.Проект = ПроектИзЗаказа(Объект.ЗаказПокупателя);
	КонецЕсли;
...
Процедура ЗапасыНоменклатураПоставщикаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("НоменклатураДляНоменклатурыПоставщика");
	ПараметрыОткрытия.Вставить("РежимВыбора");
	ПараметрыОткрытия.Вставить("ЗакрыватьПриВыборе");
	ПараметрыОткрытия.Вставить("ВидЦенКонтрагента");
	ПараметрыОткрытия.Вставить("Склад");
	ПараметрыОткрытия.Вставить("ДополнительныеПараметры");
	ПараметрыОткрытия.Вставить("ТекущаяСтрока");
...
&НаКлиенте
Процедура ЗапасыПараметрыСпецификацииНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
...
&НаКлиенте
Процедура МатериалыСтруктурнаяЕдиницаРезервОстаткиПриИзменении(Элемент)
	ЗаполнитьЗначенияРезервовИОстатокНоменклатуры()
КонецПроцедуры
...
&НаКлиенте
Процедура ЗаполнитьЗначенияРезервовИОстатокНоменклатуры()
...
Процедура ЗаполнитьПризнакРегистрацииЦенПоставщика() 
	...
	АвтоРегистрацияЦенПоставщика = Ложь;
...
&НаСервере
Функция ПолучитьСкладРезерва( СтруктураДанные)
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураДанные.Склад, "ТипСтруктурнойЕдиницы") = 
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	КонецЕсли;
	// ПараметрическиеСпецификации
	СтруктураДанные.Вставить("ВключаяШаблоны",СтруктураДанные.Свойство("ВидОперации") И СтруктураДанные.ВидОперации = Перечисления.ВидыОперацийЗаказПоставщику.ЗаказНаПереработку);
	Если СтруктураДанные.Свойство("Характеристика") Тогда
		СтруктураДанные.Вставить("Спецификация", Справочники.Спецификации.СпецификацияПоУмолчанию(
			СтруктураДанные.Номенклатура, СтруктураДанные.Характеристика, СтруктураДанные.ВключаяШаблоны));
	Иначе
		СтруктураДанные.Вставить("Спецификация", Справочники.Спецификации.СпецификацияПоУмолчанию(
			СтруктураДанные.Номенклатура, , СтруктураДанные.ВключаяШаблоны));
	КонецЕсли; 
	СтруктураДанные = ПолучитьДанныеСпецификацияПриИзменении(СтруктураДанные);
	// Конец ПараметрическиеСпецификации
	
    //Вес и объем
	СтруктураДанные.Вставить("ВесЕдиницыТовара", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураДанные.Номенклатура, "Вес"));
	СтруктураДанные.Вставить("ОбъемЕдиницыТовара", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураДанные.Номенклатура, "Объем"));
		...
		Если МассивДанныхШтрихкода <> Неопределено
			...
			Для Каждого ДанныеШтрихкода Из МассивДанныхШтрихкода Цикл
				...
				Если ЗначениеЗаполнено(СтруктураДанные.ВидЦенКонтрагента) Тогда
					...
					Если ЗначениеЗаполнено(ДанныеШтрихкода.ЕдиницаИзмерения) И ТипЗнч(ДанныеШтрихкода.ЕдиницаИзмерения)
						= Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
...
Процедура ПослеВыбораИзВыпадающегоСпискаТабличнойЧасти(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ВыбранноеПоле = "МатериалыОстатокОбщий" Тогда
...
Процедура ОбработатьИзменениеКонтрагентаНаСервере()
	...
	ДанныеКонтрагента = ПолучитьДанныеКонтрагентПриИзменении(Объект.Дата, Объект.ВалютаДокумента, Объект.Контрагент,
		Объект.Организация);
...
&НаСервере
Процедура ОбработатьИзменениеДоговораНаСервере(ДанныеДоговора = Неопределено)
...
Процедура ЗаполнитьПоСпецификацииНаСервере()
	...
	Отказ = Ложь;
	ПроизводствоФормулыСервер.РассчитатьПараметрическиеСпецификации(Документ, "Запасы", Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли; 
...
Коэффициент = ?(ТипЗнч(ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения"),
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЕдиницаИзмерения, "Коэффициент"), 1);
...
Процедура МатериалыНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество)
	...
	Если СтруктураДанные.ИспользоватьХарактеристики Тогда
		...
		КонецЕсли;
	КонецЕсли;
	СтруктураДанные = ПолучитьСкладРезерва(СтруктураДанные);
	...
	Для каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда
			...
			НоваяСтрока.Спецификация = Справочники.Спецификации.СпецификацияПоУмолчанию(НоваяСтрока.Номенклатура,
				Характеристика, СтруктураДанные.ВключаяШаблоны);
...
Процедура ПроверитьТаблицуЗапасыНаНаличиеУслуг()
	...
	Для Каждого СтрокаЗапасы Из Объект.Запасы Цикл
		Если СтрокаЗапасы.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга") Тогда
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) И ПоказыватьКолонкуВесВЗаказеПоставщику Тогда
