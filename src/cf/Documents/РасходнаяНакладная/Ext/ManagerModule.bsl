...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(
			КомандыСозданияНаОсновании, Метаданные.Документы.РасходнаяНакладная);
...
//
// Параметры:
// ТекущийДокумент - ДокументСсылка.РасходнаяНакладная - Документ, ставку по которому надо проверить
...
Функция НаличиеСтавкиУСН(ТекущийДокумент, ВидСтавкиУСН) Экспорт
	...
	|		ПО РасходнаяНакладная.СтавкаНДС = СтавкиНДС.Ссылка
	|			И (РасходнаяНакладная.Ссылка = &СсылкаНаДокумент)
...
// Возвращает документы, для которых выписка счетов-фактур не требуется.
//
// Параметры:
//  МассивДокументов - Массив из ДокументСсылка.РасходнаяНакладная
//
// Возвращаемое значение:
//  Массив из ДокументСсылка.РасходнаяНакладная - документы, для которых выписка счетов-фактур не требуется.
//
Функция СчетаФактурыНеТребуются(МассивДокументов) Экспорт
	...
	|    И (РасходнаяНакладная.СуммаНДС > 0
	|    ИЛИ РасходнаяНакладная.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|    Ссылка
	|;
	...
	|        ПО ВТ_ДокументыСНДС.Ссылка = РасходнаяНакладная.Ссылка
	|ГДЕ
	|    РасходнаяНакладная.Ссылка В (&МассивДокументов)
	|    И РасходнаяНакладная.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаНаОтветХранение)
	|    И НЕ РасходнаяНакладная.Договор.УчетАгентскогоНДСПокупателем
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ГДЕ
	|    РасходнаяНакладная.Ссылка В (&МассивДокументов)
	|    И РасходнаяНакладная.Договор.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоров.СКомиссионером)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Пока ВыборкаБезПереходаПраваСобственности.Следующий() Цикл
		...
		Если Не ВыборкаБезПереходаПраваСобственности.ЕстьНДС И (Не УчетнаяПолитика.ПлательщикНДС(
			ВыборкаБезПереходаПраваСобственности.Организация, ВыборкаБезПереходаПраваСобственности.Дата)
			Или УчетНДСПереопределяемый.ВерсияПостановленияНДС1137(ВыборкаБезПереходаПраваСобственности.Дата) >= 3
			И Не УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(ВыборкаБезПереходаПраваСобственности.Организация,
			ВыборкаБезПереходаПраваСобственности.Дата)) Тогда
	...
	Пока ВыборкаРеализация.Следующий() Цикл
		...
		Если Не ВыборкаРеализация.ЕстьНДС И (Не УчетнаяПолитика.ПлательщикНДС(ВыборкаРеализация.Организация,
			ВыборкаРеализация.Дата) Или УчетНДСПереопределяемый.ВерсияПостановленияНДС1137(ВыборкаРеализация.Дата) >= 3
			И Не УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(ВыборкаРеализация.Организация,
			ВыборкаРеализация.Дата)) Тогда
...
// 
// Параметры:
//  Операция - ПеречислениеСсылка.ВидыОперацийРасходнаяНакладная
// 
// Возвращаемое значение:
//  Булево - Прослеживаемая операция
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	РасходнаяНакладнаяВидОперации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная,
		"ВидОперации");
	Если РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю
		Или РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию
		Или РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером
		Или РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаВПереработку
		Или РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаНаОтветХранение Тогда

		СформироватьТаблицаЗапасыПродажа(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства);

	ИначеЕсли РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику Тогда
...
Процедура СформироватьТаблицаЗапасыВРазрезеГТД(СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПереданныеВРазрезеГТД",
		РезультатЗапроса[1].Выгрузить());
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПринятыеВРазрезеГТД",
...
Процедура СформироватьТаблицаЗапасыПродажа(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|	КОНЕЦ КАК ЗаказПокупателя,
	|	СУММА(ТаблицаЗапасы.Количество) КАК КоличествоОстаток,
	|	СУММА(ВЫБОР
	|		КОГДА ТаблицаЗапасы.СуммаВключаетНДС
	|			ТОГДА ТаблицаЗапасы.СуммаПередачи - ТаблицаЗапасы.СуммаНДСПередачи
	|		ИНАЧЕ ТаблицаЗапасы.СуммаПередачи
	|	КОНЕЦ) КАК СуммаБезНДСОстаток,
	|	СУММА(ТаблицаЗапасы.СуммаПередачи) КАК СуммаОстаток
	...
	|		ИЛИ ЕСТЬNULL(ЗапасыОстатки.КоличествоОстаток, 0) < ТаблицаДокумента.КоличествоОстаток
	|			ТОГДА ТаблицаДокумента.КоличествоОстаток
	|		ИНАЧЕ ЗапасыОстатки.КоличествоОстаток
	|	КОНЕЦ) КАК КоличествоОстаток,
	|	СУММА(ВЫБОР
	|		КОГДА ЕСТЬNULL(ЗапасыОстатки.СуммаБезНДСОстаток, 0) = 0
	|		ИЛИ ЕСТЬNULL(ЗапасыОстатки.КоличествоОстаток, 0) < ТаблицаДокумента.КоличествоОстаток
	|			ТОГДА ТаблицаДокумента.СуммаБезНДСОстаток
	|		ИНАЧЕ ЗапасыОстатки.СуммаБезНДСОстаток
	|	КОНЕЦ) КАК СуммаБезНДСОстаток,
	|	СУММА(ВЫБОР
	|		КОГДА ЕСТЬNULL(ЗапасыОстатки.СуммаОстаток, 0) = 0
	|		ИЛИ ЕСТЬNULL(ЗапасыОстатки.КоличествоОстаток, 0) < ТаблицаДокумента.КоличествоОстаток
	|			ТОГДА ТаблицаДокумента.СуммаОстаток
	|		ИНАЧЕ ЗапасыОстатки.СуммаОстаток
	...
	|			И ДвиженияДокументаЗапасы.Количество > 0
	|			И &Выкуп) КАК ЗапасыОстатки
	|		ПО ТаблицаДокумента.Номенклатура = ЗапасыОстатки.Номенклатура
	|		И ТаблицаДокумента.Характеристика = ЗапасыОстатки.Характеристика
	|		И ТаблицаДокумента.Партия = ЗапасыОстатки.Партия
	|		И ТаблицаДокумента.ЗаказПокупателя = ЗапасыОстатки.ЗаказПокупателя
	|		И ТаблицаДокумента.Организация = ЗапасыОстатки.Организация
	|		И ТаблицаДокумента.СтруктурнаяЕдиница = ЗапасыОстатки.СтруктурнаяЕдиница
	|		И ТаблицаДокумента.СчетУчета = ЗапасыОстатки.СчетУчета
	|ГДЕ
	|	&Выкуп
	|СГРУППИРОВАТЬ ПО
	...
	РасходнаяНакладнаяВидОперации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная,
		"ВидОперации");
	Выкуп = РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;
	Запрос.УстановитьПараметр("Выкуп", Выкуп);
	...
	Передача = (РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию
		Или РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаВПереработку
		Или РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаНаОтветХранение);
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		КоличествоТребуетсяСвободныйОстаток = ?(ЗначениеЗаполнено(СтрокаТаблицаЗапасы.Количество),
		...
		Если КоличествоТребуетсяРезерв > 0 Тогда
			...
			Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяРезерв Тогда
				...
				МассивСтрокОстатков[0].КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток
					- КоличествоТребуетсяРезерв;
				МассивСтрокОстатков[0].СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток - СуммаКСписанию;
				МассивСтрокОстатков[0].СуммаБезНДСОстаток = МассивСтрокОстатков[0].СуммаБезНДСОстаток
					- СуммаБезНДСКСписанию;
			...
			Иначе
				...
				Если Окр(СуммаКСписанию, 2, 1) <> 0 Или Окр(СуммаБезНДСКСписанию, 2, 1) <> 0 Тогда
					...
					Если Не СтрокаТаблицаЗапасы.ТипНоменклатуры = ПредопределенноеЗначение(
						"Перечисление.ТипыНоменклатуры.ПодарочныйСертификат") Тогда
				...
				Если Выкуп Тогда 
					...
					СтрокаТаблицыРасход.КоррСтруктурнаяЕдиница = СтрокаТаблицаЗапасы.Контрагент;
		...
		Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
			...
			Если Выкуп И ЗначениеЗаполнено(СтрокаТаблицаЗапасы.ЗаказПокупателя) Тогда
				СтруктураДляПоиска.Вставить("ЗаказПокупателя", СтрокаТаблицаЗапасы.ЗаказПокупателя);
			Иначе
			...
			Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуетсяСвободныйОстаток Тогда
				...
				СуммаБезНДСКСписанию = Окр(СуммаБезНДСОстаток * КоличествоТребуетсяСвободныйОстаток
					/ КоличествоОстаток, 2, 1);

				МассивСтрокОстатков[0].КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток
					- КоличествоТребуетсяСвободныйОстаток;
				МассивСтрокОстатков[0].СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток - СуммаКСписанию;
				МассивСтрокОстатков[0].СуммаБезНДСОстаток = МассивСтрокОстатков[0].СуммаБезНДСОстаток
					- СуммаБезНДСКСписанию;
			...
			Иначе
				...
				ИначеЕсли Окр(СуммаКСписанию, 2, 1) <> 0 Тогда
					...
					СтрокаДоходыИРасходы.СтруктурнаяЕдиница = СтрокаТаблицаЗапасы.ПодразделениеПродажи;
					СтрокаДоходыИРасходы.ЗаказПокупателя = СтрокаТаблицаЗапасы.ЗаказПродажи;
					Если СтрокаТаблицаЗапасы.ТипНоменклатуры = ПредопределенноеЗначение(
						"Перечисление.ТипыНоменклатуры.ПодарочныйСертификат") Тогда
						СтрокаДоходыИРасходы.СчетУчета = СтрокаТаблицаЗапасы.СчетДт;
					Иначе
						СтрокаДоходыИРасходы.СчетУчета = СтрокаТаблицаЗапасы.СчетУчетаСебестоимость;
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		...
		= Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;
	ТекстЗапросаСертификаты = СтрЗаменить(ТекстЗапросаСертификаты, "ВложенныйЗапросСертификаты", "("
		+ РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийЗапасыОприходованиеДляПовторнойПродажи(
		ВыполнятьДвиженияПоСертификатам) + ")");
	...
	Если ТаблицаОприходованиеПодарочныхСертификатов.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаОприходованиеПодарочныхСертификатов,
...
Процедура СформироватьТаблицаЗапасыВозврат(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Для Каждого СтрокаТаблицаЗапасы Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.ЗаказПокупателя) И СтрокаТаблицаЗапасы.Резерв
			< -СтрокаТаблицаЗапасы.Количество Тогда
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяСвободныйОстаток > 0 Тогда
			...
			// Запасы.
			Если Окр((СтрокаТаблицаЗапасы.Сумма + СуммаКСписанию), 2, 1) <> 0 Или Окр((СтрокаТаблицаЗапасы.СуммаБезНДС
				+ СуммаБезНДСКСписанию), 2, 1) <> 0 Тогда
...
Процедура СформироватьТаблицаЗапасыКРасходуСоСкладов(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыКРасходуСоСкладов",
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаЗапасыИАгентскиеУслугиПринятые(ДокументСсылкаРасходнаяНакладная,
	...
	РезультатЗапроса = Запрос.Выполнить();
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыИАгентскиеУслугиПринятые",
		РезультатЗапроса.Выгрузить());
...
Процедура СформироватьТаблицаЗаказыПокупателей(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	РегистрыНакопления.ЗаказыПокупателей.ДобавитьДвиженияСУчетомСкладов(ДокументСсылкаРасходнаяНакладная,
		ТаблицаЗаказыПокупателей);
...
Процедура СформироватьТаблицаЗаказыПоставщикам(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	РегистрыНакопления.ЗаказыПоставщикам.ДобавитьДвиженияСУчетомСкладов(ДокументСсылкаРасходнаяНакладная,
		ТаблицаЗаказыПоставщикам);
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ТекстЗапроса = ТекстЗапроса + "
								  ...
								  |ИЗ
								  |	(" + РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийДоходыИРасходы(3,
		ВыполнятьДвиженияПоСертификатам) + ") КАК ДанныеСертификатов
										   |";
	...
	Запрос.УстановитьПараметр("МоментКонтроля",
		Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Исключая));
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПокупателяПоДоставке", НСтр(
		"ru='Возникновение обязательств покупателя по доставке'"));
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		...
		Или СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		= Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;
	Запрос.Текст = РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийРасчетыСПокупателями(
		ВыполнятьДвиженияПоСертификатам);
	...
	РасходнаяНакладнаяВидОперации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная,
		"ВидОперации");
	Если РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю
		Или РасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером Тогда

		РасчетыПроведениеДокументов.СформироватьДвиженияПоВзаиморасчетам(СтруктураДополнительныеСвойства,
			ДокументСсылкаРасходнаяНакладная, Запрос.Параметры);
		РасчетыПроведениеДокументов.ПересчитатьТаблицыПоКурсамВалют(ДокументСсылкаРасходнаяНакладная,
			СтруктураДополнительныеСвойства);
	...
	ТаблицаРасчеты = МассивРезультатов[НомерЗапроса].Выгрузить();
	  
	// Объединение таблиц
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПодарочныеСертификаты, ТаблицаРасчеты);

	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРасчетыСПокупателями", ТаблицаРасчеты);
...
Процедура СформироватьТаблицаБонусныеБаллы(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Если ТаблицаОплатаБонусами.Количество() > 0 Тогда
		...
		РаботаСБонусами.ОбъединитьТаблицыБонусов(
			ТаблицаБонусныеБаллы, ТаблицаОплатаБонусами, ДокументСсылкаРасходнаяНакладная);
...
Процедура СформироватьТаблицаНачисленияБонусныхБаллов(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаНачисленияБонусныхБаллов",
		ТаблицаНачисленияБонусныхБаллов);
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ВосстановлениеПредоплаты", СодержанияПроводок.ВосстановлениеАванса());
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница'"));
...
Процедура СформироватьТаблицаДоходыИРасходыОтложенные(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	РасчетыПроведениеДокументов.ОбновитьДатыВТаблицеДоходыРасходыОтложенные(СтруктураДополнительныеСвойства,
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаДоходыИРасходыНераспределенные(ДокументСсылкаРасходнаяНакладная,
	СтруктураДополнительныеСвойства)
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаДоходыИРасходыКассовыйМетод(ДокументСсылкаРасходнаяНакладная,
	СтруктураДополнительныеСвойства)
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|			ИЛИ ТаблицаУправленческий.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером))
	|	И НЕ ТаблицаУправленческий.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПодарочныйСертификат)
	|	И НЕ (ТаблицаУправленческий.ТоварыНаКомиссии ИЛИ ТаблицаУправленческий.ЭтоАгентскаяУслуга)
	...
	|			И НЕ ТаблицаДокумента.ОплатаСертификатом = ИСТИНА
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		...
		= Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;
	ТекстЗапросаСертификаты = СтрЗаменить(ТекстЗапросаСертификаты, "ВложенныйЗапросСертификаты", "("
		+ РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийУправлченческий(12, ВыполнятьДвиженияПоСертификатам)
		+ ")");
	...
	РасходнаяНакладнаяВидОперации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная,
		"ВидОперации");
	Запрос.УстановитьПараметр("ВидОперации", РасходнаяНакладнаяВидОперации);
	...
	ТаблицаДокумента.Свернуть(
		"Период, Организация, СценарийПланирования, СчетДт, СчетКт, ВалютаДт, ВалютаКт, Содержание",
	...
	ТаблицаРасчетыСПоставщиками = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПоставщиками.Скопировать();

	СтрокаВосстановленияАванса = ТаблицаРасчетыСПоставщиками.Найти(Перечисления.ТипыРасчетов.Аванс, "ТипРасчетов");
	Если СтрокаВосстановленияАванса <> Неопределено Тогда
		Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная, "Контрагент");
		СчетУчетаАвансовПокупателя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "СчетУчетаАвансовПоставщику");

		НоваяПроводка = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяПроводка, СтрокаДокумента);
		НоваяПроводка.СчетДт = СчетУчетаАвансовПокупателя;
		НоваяПроводка.СчетКт = СтрокаДокумента.СчетДт;
		НоваяПроводка.Содержание = СодержанияПроводок.ВосстановлениеАванса();
		НоваяПроводка.Сумма = СтрокаВосстановленияАванса.Сумма;
		Если НоваяПроводка.СчетДт.Валютный Тогда
			НоваяПроводка.СуммаВалДт = СтрокаВосстановленияАванса.Сумма;
		КонецЕсли;
		Если НоваяПроводка.СчетКт.Валютный Тогда
			НоваяПроводка.СуммаВалКт = СтрокаВосстановленияАванса.Сумма;
		КонецЕсли;
	КонецЕсли;
		...
		Для Каждого СтрокаОпаченногоТовара Из СтрокиОплаченногоТовара Цикл
			...
			Иначе
				...
				Если ТекущаяСтрокаТоварыКВозврату.Сумма = 0 Тогда
					НовСтр.Количество = ТекущаяСтрокаТоварыКВозврату.Количество;
				Иначе
					НовСтр.Количество = ТекущаяСтрокаТоварыКВозврату.Количество * СтрокаОпаченногоТовара.Сумма
						/ ТекущаяСтрокаТоварыКВозврату.Сумма;
				КонецЕсли;
			КонецЕсли;
...
Функция ТекстЗапросаЗакупкиДляКУДиР(ДокументСсылкаРасходнаяНакладная)
	...
	РасходнаяНакладнаяДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная,
		"ДокументОснование");
	Если ЗначениеЗаполнено(РасходнаяНакладнаяДокументОснование)
		И ТипЗнч(РасходнаяНакладнаяДокументОснование) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаЗапасы.Период,
...
//
Процедура СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации 
		= Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю
		ИЛИ СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации 
		= Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером Тогда

		МенеджерВТ = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |ПОМЕСТИТЬ ВТЗапасыОтразитьКУДиР
					   |ИЗ
					   |	Документ.РасходнаяНакладная КАК РасходнаяНакладная
					   ...
					   |	ВТЗапасы.Количество * ЕСТЬNULL(ЕдиницыИзмерения.Коэффициент, 1) КАК Количество,
					   |	ВТЗапасы.Контрагент КАК Контрагент,
					   |	ВТЗапасы.УчитыватьВНУ КАК УчитыватьВНУ,
					   |	ВТЗапасы.Контрагент.Наименование КАК НаименованиеКонтрагента,
					   |	ВТЗапасы.Контрагент.НаименованиеПолное КАК ЮридическоеНаименованиеКонтрагента
					   |ИЗ
					   |	ВТЗапасыОтразитьКУДиР КАК ВТЗапасы
					   |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииНоменклатуры КАК ПартииНоменклатуры
					   |		ПО ВТЗапасы.Партия = ПартииНоменклатуры.Ссылка
					   |			И (ПартииНоменклатуры.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии))
					   |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЕдиницыИзмерения КАК ЕдиницыИзмерения
					   |		ПО ВТЗапасы.ЕдиницаИзмерения = ЕдиницыИзмерения.Ссылка
					   |ГДЕ
		...
		УчетПартийУСН.ЗаполнитьДвиженияСписаниеПартий(ДокументСсылкаРасходнаяНакладная, ПартииСписать, ТаблицаЗапасы,
			ТаблицаПартииТоваров);
		...
		Если ТаблицаПартииТоваров.Найти(Истина, "ОтражатьВКУДиР") <> Неопределено Тогда
			...
			Если ТаблицаЗапасы.Количество() > 0 Тогда
				ТекстПокупатель = ?(ПустаяСтрока(ТаблицаЗапасы[0].ЮридическоеНаименованиеКонтрагента),
					ТаблицаЗапасы[0].НаименованиеКонтрагента, ТаблицаЗапасы[0].ЮридическоеНаименованиеКонтрагента);
			КонецЕсли;
			СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеПродажаТоваров(ТекстПокупатель);
		КонецЕсли;

	ИначеЕсли СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		= Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику Тогда

		ИмяПоляНоменклатура = "Номенклатура";
		
		// Возврат товаров поставщику не учитывается в КУДиР, уменьшаем остаток партий
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаРасходнаяНакладная);
		Запрос.УстановитьПараметр("ДатаДокумента", СтруктураДополнительныеСвойства.ДляПроведения.Дата);
		Запрос.УстановитьПараметр("ГраницаДвижения",
			Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Исключая));
		Запрос.УстановитьПараметр("Организация", ОрганизацияПартий);
		Запрос.Текст = "ВЫБРАТЬ
					   |	РасходнаяНакладная.Ссылка КАК Возврат,
					   |	ЕСТЬNULL(ПриходнаяНакладная.Ссылка, ЗНАЧЕНИЕ(Документ.ПриходнаяНакладная.ПустаяСсылка)) КАК ПриходнаяНакладная,
					   |	ВЫБОР
					   |		КОГДА РасходнаяНакладная.ПоложениеЗаказаПокупателя = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
					   |			ТОГДА РасходнаяНакладнаяЗапасы.Заказ
					   |		ИНАЧЕ РасходнаяНакладная.Заказ
					   |	КОНЕЦ КАК ЗаказПоставщику,
					   |	РасходнаяНакладная.Контрагент КАК Контрагент,
					   |	РасходнаяНакладнаяЗапасы.Номенклатура КАК Номенклатура,
					   |	РасходнаяНакладнаяЗапасы.Количество * ЕСТЬNULL(ЕдиницыИзмерения.Коэффициент, 1) КАК Количество
					   |ПОМЕСТИТЬ ВТЗапасыКВозврату
					   |ИЗ
					   ...
					   |			ПО РасходнаяНакладная.ДокументОснование = ПриходнаяНакладная.Ссылка
					   ...
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   |	ВТЗапасыКВозврату.Возврат КАК Возврат,
					   |	ВТЗапасыКВозврату.ПриходнаяНакладная КАК ПриходнаяНакладная,
					   |	ВТЗапасыКВозврату.ЗаказПоставщику КАК ЗаказПоставщику,
					   |	ВТЗапасыКВозврату.Контрагент КАК Контрагент,
					   |	ВТЗапасыКВозврату.Номенклатура КАК Номенклатура,
					   |	ВТЗапасыКВозврату.Количество КАК Количество
					   |ИЗ
					   |	ВТЗапасыКВозврату КАК ВТЗапасыКВозврату";

		Результат = Запрос.Выполнить();
		ТаблицаЗапасы = Результат.Выгрузить();
		Если ТаблицаЗапасы.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;

		Запрос.УстановитьПараметр("Контрагент", ТаблицаЗапасы[0].Контрагент);
		Запрос.УстановитьПараметр(ИмяПоляНоменклатура, ТаблицаЗапасы.ВыгрузитьКолонку(ИмяПоляНоменклатура));
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |			И (ПриходнаяНакладнаяЗапасы.Номенклатура В (&Номенклатура))
					   ...
					   |	ВТЗапасыКВозврату.ЗаказПоставщику КАК ЗаказПоставщику,
					   |	ВТЗапасыКВозврату.Номенклатура КАК Номенклатура,
					   |	ВТДокументыПрихода.Дата КАК Дата,
					   |	0 КАК Приоритет
					   |ПОМЕСТИТЬ ВТВозможныеПартии
					   |ИЗ
					   |	ВТЗапасыКВозврату КАК ВТЗапасыКВозврату
					   |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыПрихода КАК ВТДокументыПрихода
					   |		ПО ВТЗапасыКВозврату.ПриходнаяНакладная = ВТДокументыПрихода.ПриходнаяНакладная
					   |			И ВТЗапасыКВозврату.ЗаказПоставщику = ВТДокументыПрихода.ЗаказПоставщику
					   |			И ВТЗапасыКВозврату.Номенклатура = ВТДокументыПрихода.Номенклатура
					   |			И (ВТЗапасыКВозврату.ПриходнаяНакладная <> ЗНАЧЕНИЕ(Документ.ПриходнаяНакладная.ПустаяССылка))
					   |
					   |ОБЪЕДИНИТЬ ВСЕ
					   |
					   |ВЫБРАТЬ
					   |	ВТДокументыПрихода.ПриходнаяНакладная,
					   |	ВТЗапасыКВозврату.ЗаказПоставщику,
					   |	ВТЗапасыКВозврату.Номенклатура,
					   |	ВТДокументыПрихода.Дата,
					   |	1
					   |ИЗ
					   |	ВТЗапасыКВозврату КАК ВТЗапасыКВозврату
					   |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыПрихода КАК ВТДокументыПрихода
					   |		ПО ВТЗапасыКВозврату.ЗаказПоставщику = ВТДокументыПрихода.ЗаказПоставщику
					   |			И ВТЗапасыКВозврату.Номенклатура = ВТДокументыПрихода.Номенклатура
					   |			И (ВТЗапасыКВозврату.ПриходнаяНакладная = ЗНАЧЕНИЕ(Документ.ПриходнаяНакладная.ПустаяССылка))
					   |
					   |ОБЪЕДИНИТЬ ВСЕ
					   ...
					   |ВЫБРАТЬ
					   |	ВТДокументыПрихода.ПриходнаяНакладная,
					   |	ВТДокументыПрихода.ЗаказПоставщику,
					   |	ВТДокументыПрихода.Номенклатура,
					   |	ВТДокументыПрихода.Дата,
					   |	2
					   |ИЗ
					   |	ВТДокументыПрихода КАК ВТДокументыПрихода
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   |	ВТВозможныеПартии.ПриходнаяНакладная КАК ПриходнаяНакладная,
					   |	ВТВозможныеПартии.Номенклатура КАК Номенклатура,
					   |	ВТВозможныеПартии.Дата КАК Дата,
					   |	МАКСИМУМ(ВТВозможныеПартии.Приоритет) КАК Приоритет
					   |ПОМЕСТИТЬ ВТПриходныеНакладныеТоваров
					   |ИЗ
					   |	ВТВозможныеПартии КАК ВТВозможныеПартии
					   |
					   |СГРУППИРОВАТЬ ПО
					   |	ВТВозможныеПартии.ПриходнаяНакладная,
					   |	ВТВозможныеПартии.Номенклатура,
					   |	ВТВозможныеПартии.Дата
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   |	ПартииТоваровУСНОстатки.Организация КАК Организация,
					   |	ПартииТоваровУСНОстатки.Номенклатура КАК Номенклатура,
					   |	ПартииТоваровУСНОстатки.ДокументПартии КАК ДокументПартии,
					   |	ПартииТоваровУСНОстатки.ДокументРегистрацииВКУДиР КАК ДокументРегистрацииВКУДиР,
					   |	ПартииТоваровУСНОстатки.СтатусРасхода КАК СтатусРасхода,
					   |	ПартииТоваровУСНОстатки.ЭтоТоварыКРеализации КАК ЭтоТоварыКРеализации,
					   |	ПартииТоваровУСНОстатки.НомерСтрокиДокумента КАК НомерСтрокиДокумента,
					   |	ПартииТоваровУСНОстатки.УчитыватьВНУ КАК УчитыватьВНУ,
					   |	ПартииТоваровУСНОстатки.КоличествоОстаток КАК Количество,
					   |	ПартииТоваровУСНОстатки.СуммаОстаток КАК Сумма,
					   |	ПартииТоваровУСНОстатки.СуммаНДСОстаток КАК СуммаНДС
					   |ПОМЕСТИТЬ ВТОстаткиПартий
					   |ИЗ
					   |	РегистрНакопления.ПартииТоваровУСН.Остатки(
					   |			&ГраницаДвижения,
					   |			Организация = &Организация
					   |				И Номенклатура В (&Номенклатура)
					   |				И СтатусРасхода В (ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеСписано), ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеСписаноНеОплачено))
					   |				И ДокументПартии В
					   |					(ВЫБРАТЬ
					   |						ВТПриходныеНакладныеТоваров.ПриходнаяНакладная КАК ПриходнаяНакладная
					   |					ИЗ
					   |						ВТПриходныеНакладныеТоваров)) КАК ПартииТоваровУСНОстатки
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   |	&ДатаДокумента КАК Период,
					   |	ВТОстаткиПартий.Организация КАК Организация,
					   |	ВТОстаткиПартий.Номенклатура КАК Номенклатура,
					   |	ВТОстаткиПартий.ДокументПартии КАК ДокументПартии,
					   |	ВТОстаткиПартий.ДокументРегистрацииВКУДиР КАК ДокументРегистрацииВКУДиР,
					   |	ВТОстаткиПартий.СтатусРасхода КАК СтатусРасхода,
					   |	ВТОстаткиПартий.ЭтоТоварыКРеализации КАК ЭтоТоварыКРеализации,
					   |	ВТОстаткиПартий.НомерСтрокиДокумента КАК НомерСтрокиДокумента,
					   |	ВТОстаткиПартий.УчитыватьВНУ КАК УчитыватьВНУ,
					   |	ВТОстаткиПартий.Количество КАК Количество,
					   |	ВТОстаткиПартий.Сумма КАК Сумма,
					   |	ВТОстаткиПартий.СуммаНДС КАК СуммаНДС,
					   |	ВТПриходныеНакладныеТоваров.Дата КАК Дата,
					   |	ВТПриходныеНакладныеТоваров.Приоритет КАК Приоритет
					   |ИЗ
					   |	ВТПриходныеНакладныеТоваров КАК ВТПриходныеНакладныеТоваров
					   |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОстаткиПартий КАК ВТОстаткиПартий
					   |		ПО ВТПриходныеНакладныеТоваров.ПриходнаяНакладная = ВТОстаткиПартий.ДокументПартии
					   |			И ВТПриходныеНакладныеТоваров.Номенклатура = ВТОстаткиПартий.Номенклатура
					   |
					   |УПОРЯДОЧИТЬ ПО
					   |	Номенклатура,
					   |	Приоритет,
		...
		Пока ВыборкаПартии.СледующийПоЗначениюПоля(ИмяПоляНоменклатура) Цикл

			СтрокиЗапаса = ТаблицаЗапасы.НайтиСтроки(Новый Структура(ИмяПоляНоменклатура, ВыборкаПартии.Номенклатура));
			Пока ВыборкаПартии.Следующий() Цикл

				РесурсыСписать = Новый Структура("Количество, Сумма, СуммаНДС");
				ЗаполнитьЗначенияСвойств(РесурсыСписать, ВыборкаПартии);

				Для Каждого СтрокаЗапас Из СтрокиЗапаса Цикл
					Если СтрокаЗапас.Количество = 0 Или РесурсыСписать.Количество = 0 Тогда
						Продолжить;
					КонецЕсли;
					...
					Если СтрокаЗапас.Количество < РесурсыСписать.Количество Тогда
						Коэффициент = ?(ВыборкаПартии.Количество = 0, 1, СтрокаЗапас.Количество
							/ РесурсыСписать.Количество);
						СтрокаРасход.Количество = СтрокаЗапас.Количество;
						СтрокаРасход.СуммаНДС = ВыборкаПартии.СуммаНДС * Коэффициент;
						СтрокаРасход.Сумма = ВыборкаПартии.Сумма * Коэффициент;
					КонецЕсли;
					СтрокаЗапас.Количество = СтрокаЗапас.Количество - СтрокаРасход.Количество;
					РесурсыСписать.Количество = РесурсыСписать.Количество - СтрокаРасход.Количество;
					РесурсыСписать.Сумма = РесурсыСписать.Сумма - СтрокаРасход.Сумма;
					РесурсыСписать.СуммаНДС = РесурсыСписать.СуммаНДС - СтрокаРасход.СуммаНДС;
				КонецЦикла;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Процедура СформироватьТаблицаРезервыТоваровОрганизаций(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
		...
		Если СерииНоменклатуры.Количество() <> 0 Тогда
			...
			Для Каждого СтрокаСерии Из СерииНоменклатуры Цикл
				...
				Пока СтрокаСерии.Количество <> 0 Цикл
					...
					Отбор = Новый Структура("КлючСвязи, Серия", СтрокаСерии.КлючСвязи,
	...
	ИнтеркампаниСервер.СформироватьТаблицаРезервыТоваровОрганизаций(ДокументСсылкаРасходнаяНакладная,
		СтруктураДополнительныеСвойства);
...
Процедура СформироватьТаблицаПродажиПоДисконтнойКарте(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	РасходнаяНакладнаяДисконтнаяКарта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная,
		"ДисконтнаяКарта");
	Если РасходнаяНакладнаяДисконтнаяКарта.Пустая() Тогда
	...
	Запрос.УстановитьПараметр("ПроцентСкидки", ДисконтныеКартыУНФСервер.ВычислитьПроцентСкидкиПоДисконтнойКарте(
		СтруктураДополнительныеСвойства.ДляПроведения.Дата, РасходнаяНакладнаяДисконтнаяКарта));
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаПродажиПоПредоставленнымСкидкам(ДокументСсылкаРасходнаяНакладная,
	СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПредоставленныеСкидки",
		РезультатЗапроса.Выгрузить());
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаВыполнениеДоговораОбслуживания(ДокументСсылкаРасходнаяНакладная,
	СтруктураДополнительныеСвойства)

	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьБиллинг") Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаВыполнениеДоговоровОбслуживания",
			Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
...
Процедура ЗарегистрироватьЦеныПродажи(ДокументСсылка, Отказ) Экспорт
	...
	Если ЗначенияРеквизитов.ВидОперации <> Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю Тогда
		Возврат;
	КонецЕсли;
...
// 
// Возвращаемое значение:
//  Строка - Строка - строка с необходимым текстом запроса.
Функция ТекстЗапросаДанныеДляОбновленияЦенДокументов() Экспорт
	...
	|	И РасходнаяНакладнаяЗапасы.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПодарочныйСертификат))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика";
...
//  ТЧИсточникДанных - ТабличнаяЧасть - источник данных
...
Процедура УдалитьЦеныПродажи(ДокументСсылка, ТЧИсточникДанных, ВидЦен, ДатаДокумента, Отказ) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки.УстановитьЗначение("Период", ДатаДокумента);   // проверить док регистратор
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Сумма", НСтр(
		"ru = 'Сумма выкупа'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_2, ,
		, Ложь);
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ПараметрыСопоставления, АдресРезультата) Экспорт
	...
	Для Каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицыФормы,
			"ЭтоУслуга_ВходящиеДанные") Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартии") Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура) Тогда
				...
				ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьПартию(СтрокаТаблицыФормы.Партия,
					СтрокаТаблицыФормы.Номенклатура, СтрокаТаблицыФормы.Штрихкод,
					СтрокаТаблицыФормы.Партия_ВходящиеДанные);
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСерииНоменклатуры") Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура) И ЗначениеЗаполнено(
				СтрокаТаблицыФормы.Серия_ВходящиеДанные) Тогда
		...
		// Заказ
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьЗаказ(СтрокаТаблицыФормы.Заказ,
			СтрокаТаблицыФормы.Заказ_ВходящиеДанные);
		
		// Склад
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСтруктурнуюЕдиницу(
			СтрокаТаблицыФормы.СтруктурнаяЕдиница, СтрокаТаблицыФормы.СтруктурнаяЕдиница_ВходящиеДанные,
		...
		Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
			...
			ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСтрануПроисхождения(
				СтрокаТаблицыФормы.СтранаПроисхождения, СтрокаТаблицыФормы.СтранаПроисхождения_ВходящиеДанные,
				ЗначениеПоУмолчанию);

			Если ЗначениеЗаполнено(СтрокаТаблицыФормы.СтранаПроисхождения) И СтрокаТаблицыФормы.СтранаПроисхождения
				<> Справочники.СтраныМира.Россия Тогда
...
Процедура ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения = "") Экспорт
	...
	Если НоменклатураЗаполнена Тогда
		...
		СтрокаТаблицыФормы[ИмяСлужебногоПоля] = Не СтрокаТаблицыФормы.Номенклатура.ЭтоНабор // Исключая наборы 
			...
			И СтрокаТаблицыФормы.Количество <> 0 И Не СтрокаТаблицыФормы.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы
...
Процедура СформироватьТаблицаСерииНоменклатуры(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ТаблицаДвиженияСерий = МассивРезультатов[0].Выгрузить();
	ТаблицаОстаткиСерий = МассивРезультатов[1].Выгрузить();
	...
	ТаблицаОприходованиеПодарочныхСертификатов = Запрос.Выполнить().Выгрузить();
	Если ТаблицаОприходованиеПодарочныхСертификатов.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаОприходованиеПодарочныхСертификатов, ТаблицаДвиженияСерий);
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаОприходованиеПодарочныхСертификатов, ТаблицаОстаткиСерий);
	КонецЕсли;	
	// Конец Оприходование подарочных сертификатов

	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаДвиженияСерииНоменклатуры",
		ТаблицаДвиженияСерий);
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерииНоменклатурыГарантии",
		Новый ТаблицаЗначений);

	ПараметрыОтбора = Новый Структура("ОстаткиСерийНоменклатуры", Истина);
	ОстаткиСерийНоменклатурыСтроки = ТаблицаОстаткиСерий.НайтиСтроки(ПараметрыОтбора);
	ТаблицаСерииНоменклатуры = ТаблицаОстаткиСерий.Скопировать(ОстаткиСерийНоменклатурыСтроки);
...
Процедура СформироватьТаблицаСерииНоменклатурыКРасходу(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	ЗапросСерииНоменклатуры = Новый Запрос;
	ЗапросСерииНоменклатуры.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	*
	|ИЗ
	...
	ЗапросСерииНоменклатуры.УстановитьПараметр("ДокументСсылка", ДокументСсылка);
	РезультатЗапроса = ЗапросСерииНоменклатуры.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерииНоменклатурыКРасходу",
			Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерииНоменклатурыКРасходу",
		РезультатЗапроса.Скопировать(ОстаткиСерийНоменклатурыСтроки));
...
Процедура СформироватьТаблицаПрименениеПромокодов(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПрименениеПромокодов",
		РезультатЗапроса.Выгрузить());
...
Процедура СформироватьТаблицаДоступностьПромокодов(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаДоступностьПромокодов",
		РезультатЗапроса.Выгрузить());
...
// Инициализирует таблицы значений, содержащие данные табличных частей документа РасходнаяНакладная.
//
// Параметры:
//  ДокументСсылкаРасходнаяНакладная - ДокументСсылка.РасходнаяНакладная - ссылка на документ.
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства для проведения документа.
//
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("ОстаткиСерийНоменклатуры",
		СтруктураДополнительныеСвойства.УчетнаяПолитика.ОстаткиСерийНоменклатуры);
	// Прослеживаемость
	...
	РасходнаяНакладнаяВалютаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная,
		"ВалютаДокумента");
	Запрос.УстановитьПараметр("ВалютаНакладной", РасходнаяНакладнаяВалютаДокумента);
	...
	РасчетыПроведениеДокументов.МодифицироватьЗапросДляПересчетаВВалюту(ДокументСсылкаРасходнаяНакладная, Запрос,
		СтруктураДополнительныеСвойства);
	...
	Если Не СтруктураДополнительныеСвойства.УчетВалютныхОпераций
		...
		И РасходнаяНакладнаяВалютаДокумента = СтруктураДополнительныеСвойства.НациональнаяВалюта
		И СтруктураДополнительныеСвойства.Договор.ВалютаРасчетов = СтруктураДополнительныеСвойства.НациональнаяВалюта) Тогда
		ИмяВременнойТаблицы = "ВременнаяТаблицаЗапасы";
	Иначе
		ИмяВременнойТаблицы = "ВременнаяТаблицаЗапасыВВалютеДокумента";
	КонецЕсли;
	КонтрольОстатковУНФ.КонтрольПересчетаКоличества(Запрос.МенеджерВременныхТаблиц,
		СтруктураДополнительныеСвойства.Отказ, ИмяВременнойТаблицы);
	...
	ПроведениеДокументовУНФ.СформироватьТаблицуПроводок(ДокументСсылкаРасходнаяНакладная,
		СтруктураДополнительныеСвойства);
	...
	// Прослеживаемость
	ПрослеживаемостьУНФ.СформироватьДвиженияРеализацияТоваров(ДокументСсылкаРасходнаяНакладная,
		СтруктураДополнительныеСвойства);
	...
	СформироватьТаблицаПродажиПоПредоставленнымСкидкам(ДокументСсылкаРасходнаяНакладная,
		СтруктураДополнительныеСвойства);
	...
	РасчетыПроведениеДокументов.СформироватьВременнаяТаблицаПредоплатаДляКассовогоМетода(
		СтруктураДополнительныеСвойства, Метаданные.РегистрыНакопления.РасчетыСПокупателями.Имя);
	...
	// Эквайринг
	ЭквайринговыеОперацииСервер.СформироватьТаблицаДоходыИРасходыКассовыйМетодЭквайринг(
		ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства);
	...
	СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства);
	...
	СформироватьТаблицаСуммыДокументовРегламентированныйУчет(ДокументСсылкаРасходнаяНакладная,
		СтруктураДополнительныеСвойства);
	СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства);

	Если СтруктураДополнительныеСвойства.ДляПроведения.ОстаткиЗапасовНеИспользуются Тогда
		ПроведениеДокументовУНФ.ДополнитьТаблицуОбратнымиДвижениями(
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы);
		ПроведениеДокументовУНФ.ДополнитьТаблицуОбратнымиДвижениями(
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах);
	КонецЕсли;
	
	// НДС
	УчетНДСУНФ.СформироватьДвиженияНДСЗаписиКнигиПродажРеализация(ДокументСсылкаРасходнаяНакладная,
		СтруктураДополнительныеСвойства);
    УчетНДСУНФ.СформироватьДвиженияНДСРеализация0(ДокументСсылкаРасходнаяНакладная,
		СтруктураДополнительныеСвойства);
...
Функция ТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|        И
	|            РасходнаяНакладная.Договор.ДоговорОбслуживанияНаправлениеДеятельности <> ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|    ВЫБОР
	|        КОГДА
	|            РасходнаяНакладная.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию)
	...
	|        &ВалютаНакладной, &ВалютаРасчетов)) КАК КурсыВалютСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|    ВЫБОР
	|        КОГДА
	|            РасходнаяНакладнаяЗапасы.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|            ТОГДА РасходнаяНакладнаяЗапасы.ДокументПоступления
	|        КОГДА
	|            РасходнаяНакладнаяЗапасы.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	...
	|    ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВидОперации В
	|        (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаВПереработку),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаНаОтветХранение))
	|            ТОГДА &Организация
	|        ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|    ВЫБОР
	|        КОГДА
	|            ВТРасходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|    ВЫБОР
	|        КОГДА
	|            ВТРасходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|        ИНАЧЕ НЕОПРЕДЕЛЕНО
	|    КОНЕЦ КАК Ячейка,
	|    ЕСТЬNULL(ВТРасходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗапасов,
	...
	|    ЕСТЬNULL(ВТРасходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаЗатрат,
	|        ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)) КАК СчетУчетаЗатрат,
	|    ВЫБОР
	|        КОГДА &ИспользоватьПартии
	|        И ВТРасходнаяНакладнаяЗапасы.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
	|        И (ВременнаяТаблицаШапка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю)
	|        ИЛИ
	|            ВременнаяТаблицаШапка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером))
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|            ТОГДА ВЫРАЗИТЬ(ВТРасходнаяНакладнаяЗапасы.ЗаказПокупателя КАК
	|                Документ.ЗаказПокупателя).УчетПотребностиПоЗаказам
	|        ИНАЧЕ ЛОЖЬ
	...
	|                ТОГДА ВТРасходнаяНакладнаяЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ВТРасходнаяНакладнаяЗапасы.СуммаНДС * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|        КОНЕЦ
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ВТРасходнаяНакладнаяЗапасы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ВТРасходнаяНакладнаяЗапасы.Всего * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|                ТОГДА ВТРасходнаяНакладнаяЗапасы.СуммаНДС * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	|                    (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ВТРасходнаяНакладнаяЗапасы.Всего * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	|                (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	...
	|    ВЫБОР
	|        КОГДА
	|            ВременнаяТаблицаШапка.ПоложениеПроекта <> ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	...
	|        КОГДА ВТРасходнаяНакладнаяЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|            ТОГДА ВТРасходнаяНакладнаяЗапасы.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|        ИНАЧЕ ВТРасходнаяНакладнаяЗапасы.Номенклатура.ПолитикаУчетаСерий
	...
	|            ИНАЧЕ 1 / ВТРасходнаяНакладнаяЗапасы.Количество
	|        КОНЕЦ
	|    КОНЕЦ / КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность / (ВременнаяТаблицаШапка.Курс *
	|        КурсыРегВалюты.Кратность) КАК ЧИСЛО(15, 2)) КАК НоминалСертификатаВал,
	...
	|    ВременнаяТаблицаШапка.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ВТРасходнаяНакладнаяЗапасы.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ВТРасходнаяНакладнаяЗапасы.Сумма * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРуб,
	|    ВТРасходнаяНакладнаяЗапасы.Ссылка
	...
	|        ПО ВЫБОР
	|            КОГДА ВТРасходнаяНакладнаяЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|                ТОГДА ВТРасходнаяНакладнаяЗапасы.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|            ИНАЧЕ ВТРасходнаяНакладнаяЗапасы.Номенклатура = ПолитикиУчетаСерий.Владелец
	|        КОНЕЦ
	...
	|    ВЫРАЗИТЬ(ТаблицаДокумента.СуммаРасчетов * ТаблицаДокумента.Курс * КурсыВалютУчетаСрезПоследних.Кратность /
	|        (КурсыВалютУчетаСрезПоследних.Курс * ТаблицаДокумента.Кратность) КАК ЧИСЛО(15, 2)) КАК СуммаАванса,
	|    ВЫБОР
	...
	|            ТОГДА ВЫРАЗИТЬ(ТаблицаДокумента.СуммаРасчетов * ТаблицаДокумента.Курс / ТаблицаДокумента.Кратность КАК
	|                ЧИСЛО(15, 2))
	...
	|            ТОГДА ТаблицаДокумента.СуммаРасчетов
	|        ИНАЧЕ ВЫРАЗИТЬ(ТаблицаДокумента.СуммаРасчетов * ТаблицаДокумента.Курс / ТаблицаДокумента.Кратность КАК
	|            ЧИСЛО(15, 2))
	|    КОНЕЦ КАК СуммаРег,
	|    ВЫБОР
	...
	|    Выбор
	|        Когда ТаблицаДокумента.ОплатаСертификатом
	|            Тогда ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.ПодарочныйСертификат)
	...
	|        КОГДА КонстантаВалютаУчета.Значение = КонстантаНациональнаяВалюта.Значение
	|            ТОГДА ВЫРАЗИТЬ(ТаблицаДокумента.СуммаПлатежа * ТаблицаДокумента.Курс / ТаблицаДокумента.Кратность КАК
	|                ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ТаблицаДокумента.СуммаПлатежа * ТаблицаДокумента.Курс * КурсыВалютУчетаСрезПоследних.Кратность
	|            / (КурсыВалютУчетаСрезПоследних.Курс * ТаблицаДокумента.Кратность) КАК ЧИСЛО(15, 2))
	...
	|            ТОГДА РасходнаяНакладнаяСкидкиНаценки.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ РасходнаяНакладнаяСкидкиНаценки.Сумма * РасходнаяНакладнаяСкидкиНаценки.Ссылка.Курс *
	|            КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * РасходнаяНакладнаяСкидкиНаценки.Ссылка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	...
	|    И РасходнаяНакладнаяПредоплата.Заказ В (ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка),
	|        ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА
	|                ВременнаяТаблицаЗапасы.ПолитикаУчетаСерий.ТипПолитики = ЗНАЧЕНИЕ(Перечисление.ТипыПолитикУказанияСерий.СправочноеУказаниеСерий)
	|                ТОГДА ЛОЖЬ
	|            ИНАЧЕ ИСТИНА
	|        КОНЕЦ
	...
	|        ИНАЧЕ ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ВременнаяТаблицаШапка.Дата, ДЕНЬ),
	|            ДЕНЬ, ПараметрыГенерацииПромокода.АктивироватьЧерезДней), ДЕНЬ), ДЕНЬ,
	...
	ТекстЗапроса = ТекстЗапроса + ОбщегоНазначения.РазделительПакетаЗапросов() + РаботаСПодарочнымисертификатами.ТекстЗапросаФормированияВременныхТаблицДляПроведения(
...
// 
// Параметры:
//  ДокументСсылкаРасходнаяНакладная - ДокументСсылка.РасходнаяНакладная - документ, для которого выполняется контроль
//  ДополнительныеСвойства - Структура - дополнительные свойства
//  Отказ - Булево - признак отказа
//  УдалениеПроведения - Булево - Удаление проведения
Процедура ВыполнитьКонтроль(ДокументСсылкаРасходнаяНакладная, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение Или СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
		Или СтруктураВременныеТаблицы.ДвиженияЗапасыКРасходуСоСкладовИзменение Или СтруктураВременныеТаблицы.ДвиженияЗапасыПереданныеИзменение
		Или СтруктураВременныеТаблицы.ДвиженияЗапасыПринятыеИзменение Или СтруктураВременныеТаблицы.ДвиженияЗаказыПокупателейИзменение
		Или СтруктураВременныеТаблицы.ДвиженияЗаказыПоставщикамИзменение Или СтруктураВременныеТаблицы.ДвиженияПотребностьВЗапасахИзменение
		...
		Или СтруктураВременныеТаблицы.ДвиженияСерииНоменклатурыИзменение Или СтруктураВременныеТаблицы.ДвиженияЗапасыВРазрезеГТДИзменение
		Или СтруктураВременныеТаблицы.ДвиженияЗапасыПринятыеВРазрезеГТДИзменение Или СтруктураВременныеТаблицы.ДвиженияПодарочныеСертификатыИзменение
		Или СтруктураВременныеТаблицы.ДвиженияПрослеживаемыеТоварыИзменение Или СтруктураВременныеТаблицы.ДвиженияРезервыТоваровОрганизацийИзменение
		...
		Запрос.УстановитьПараметр("МассивКонтроляПоЗапасамКОтгрузкеПоСкладскимОрдерам",
			ДополнительныеСвойства.МассивКонтроляПоЗапасамКОтгрузкеПоСкладскимОрдерам);
		...
		Если Не МассивРезультатов[0].Пустой() Или Не МассивРезультатов[1].Пустой() Или Не МассивРезультатов[2].Пустой()
			Или Не МассивРезультатов[3].Пустой() Или Не МассивРезультатов[4].Пустой()
			Или Не МассивРезультатов[5].Пустой() Или Не МассивРезультатов[6].Пустой()
			Или Не МассивРезультатов[7].Пустой() Или Не МассивРезультатов[8].Пустой()
			Или Не МассивРезультатов[9].Пустой() Или Не МассивРезультатов[10].Пустой()
			Или Не МассивРезультатов[11].Пустой() Или Не МассивРезультатов[12].Пустой()
			Или Не МассивРезультатов[13].Пустой() Или Не МассивРезультатов[14].Пустой()
		...
		Если ДополнительныеСвойства.УчетнаяПолитика.ПередачаТоваровМеждуОрганизациями И ДополнительныеСвойства.Свойство(
			"ЕстьОшибкиПередачиТоваров") И ДополнительныеСвойства.ЕстьОшибкиПередачиТоваров Тогда
			Отказ = Истина;
		КонецЕсли; 
		...
		Если Не МассивРезультатов[0].Пустой() Тогда
			...
			КонтрольОстатковУНФ.ЗапасыНаСкладахСписком(ДокументОбъектРасходнаяНакладная, ВыборкаИзРезультатаЗапроса,
				Отказ);
		КонецЕсли;
		...
		Если Не МассивРезультатов[7].Пустой() Тогда
			...
			КонтрольОстатковУНФ.РасчетыСПокупателями(ДокументОбъектРасходнаяНакладная, ВыборкаИзРезультатаЗапроса,
				Отказ);
		КонецЕсли;
		...
		Если Константы.КонтролироватьОстаткиПоНомерамГТД.Получить() Тогда
			...
			Если Не МассивРезультатов[9].Пустой() Тогда
				...
				КонтрольОстатковУНФ.ЗапасыВРазрезеГТД(ДокументОбъектРасходнаяНакладная, ВыборкаИзРезультатаЗапроса,
					Отказ);
			...
			Если Не МассивРезультатов[10].Пустой() Тогда
				...
				КонтрольОстатковУНФ.ЗапасыПринятыеВРазрезеГТД(ДокументОбъектРасходнаяНакладная,
		...
		Если Не МассивРезультатов[11].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[10].Выбрать();
			КонтрольОстатковУНФ.ПодарочныеСертификаты(ДокументОбъектРасходнаяНакладная, ВыборкаИзРезультатаЗапроса,
				Отказ);
		КонецЕсли;
		
		// Отрицательный остаток учета прослеживаемых товаров.
		Если Константы.ВестиУчетПрослеживаемыхТоваров.Получить() И Не МассивРезультатов[12].Пустой() Тогда
		...
		Если Не МассивРезультатов[14].Пустой() Тогда
			...
			КонтрольОстатковУНФ.СерииНоменклатурыКРасходуПоДокументам(ДокументОбъектРасходнаяНакладная,
				ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если Не МассивРезультатов[15].Пустой() И Не Отказ Тогда
			...
			КонтрольОстатковУНФ.РезервыКПередачиСУчетомОстатковПоЗапасамНаСкладах(ДокументОбъектРасходнаяНакладная,
				ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Функция ОбновитьСтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция) Экспорт
	...
	ИспользоватьАвтоматическийОбмен = ПолучитьФункциональнуюОпцию(
	...
	РегистрыСведений.СтатусыИнформированияГИСМ.ОбновитьСтатус(
		ДокументСсылка, НовыйСтатус, ДальнейшееДействие);
...
Функция ОбновитьСтатусПослеПередачиДанных(ДокументСсылка, Операция, СтатусОбработки) Экспорт
	...
	Если Операция = Перечисления.ОперацииОбменаГИСМ.ПередачаДанных Тогда
		...
		ИначеЕсли СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Отклонено Или СтатусОбработки
			= Перечисления.СтатусыОбработкиСообщенийГИСМ.Ошибка Тогда
...
Функция ОбработатьДанныеШтрихкода(Форма, ДанныеШтрихкода, ПараметрыСканирования, ВложенныеШтрихкоды = Неопределено) Экспорт
	...
	Если ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
		...
		ИначеЕсли ДанныеШтрихкода.Свойство("АдресДереваУпаковок") И Не ПустаяСтрока(
			ДанныеШтрихкода.АдресДереваУпаковок) Тогда
		...
		Если Не ДеревоУпаковок = Неопределено Тогда
			ИнтеграцияИСУНФ.ОбработатьДанныеШтрихкодовДереваУпаковок(ДеревоУпаковок, ПараметрыШтрихкодаДляОбработки,
				Форма);
		КонецЕсли;
	...
	Если НЕ ДанныеШтрихкода.ОбработатьБезМаркировки Тогда
...
Процедура ЗаполнитьДанныеДобавленныхИзмененныхСтрок(Форма, Результат)
	...
	Для Каждого СтрокаДляОбработки Из СтрокиДляОбработки Цикл
		...
		СтруктураДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаДляОбработки.Номенклатура,
			"Вес, Объем");

		СтрокаДляОбработки.ВесЕдиницыТовара = СтруктураДанныеНоменклатуры.Вес;
		СтрокаДляОбработки.Вес = СтрокаДляОбработки.ВесЕдиницыТовара * СтрокаДляОбработки.Количество;
		СтрокаДляОбработки.ОбъемЕдиницыТовара = СтруктураДанныеНоменклатуры.Объем;
		СтрокаДляОбработки.Объем = СтрокаДляОбработки.ОбъемЕдиницыТовара * СтрокаДляОбработки.Количество;
		...
		ИнтеграцияИСУНФКлиентСервер.ЗаполнитьСлужебныеРеквизитыСтроки(Форма, СтрокаДляОбработки, ПараметрыСканирования,
			СлужебныеРеквизиты);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.РасходнаяНакладная";
...
//
// Параметры:
//  РасходнаяНакладная - Массив из ДокументСсылка.РасходнаяНакладная - документы для выборки
// 
// Возвращаемое значение:
	...
	Для Каждого ОбъектПечати Из ДанныеОбъектовПечати Цикл
		...
		Если ОбъектПечати.НомерСчетаНаОплату = Перечисления.НомераСчетаВРасходнойНакладной.ЗаказПокупателя Тогда
			...
			Если ОбъектПечати.ПоложениеЗаказаПокупателя = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке Тогда
				...
				Если ЗначениеЗаполнено(ОбъектПечати.Заказ) И ТипЗнч(ОбъектПечати.Заказ) = Тип(
					"ДокументСсылка.ЗаказПокупателя") Тогда
			...
			Иначе
				...
				Для Каждого СтрокаТабличногоДокумента Из ОбъектПечати.ТаблицаЗапасы Цикл
					...
					Если ЗначениеЗаполнено(СтрокаТабличногоДокумента.Заказ) И ТипЗнч(СтрокаТабличногоДокумента.Заказ)
		...
		ИначеЕсли ОбъектПечати.НомерСчетаНаОплату = Перечисления.НомераСчетаВРасходнойНакладной.СчетНаОплату И ТипЗнч(
			ОбъектПечати.ДокументОснование) = Тип("ДокументСсылка.СчетНаОплату") Тогда
...
Функция УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов, ИспользоватьФаксимиле, ИспользоватьНаборы)
	...
	|	РасходнаяНакладнаяЗапасы.НомерСтроки КАК НомерСтроки,
	|	РасходнаяНакладнаяЗапасы.Ссылка КАК Ссылка,
	|	ЕСТЬNULL(ШтрихкодыНоменклатуры.Штрихкод, РасходнаяНакладнаяЗапасы.Номенклатура.Штрихкод) КАК Штрихкод
	|ИЗ
	|	Документ.РасходнаяНакладная.Запасы КАК РасходнаяНакладнаяЗапасы
	...
	|		И РасходнаяНакладнаяЗапасы.Партия = ШтрихкодыНоменклатуры.Партия
	|ГДЕ
	...
	ДанныеДокументов = РезультатыЗапроса[0].Выгрузить();
	ДоставкаСервер.ДобавитьСтрокуДоставкиУниверсальныеДанные(ДанныеДокументов);

	ШтрихкодыНоменклатуры = РезультатыЗапроса[1].Выгрузить();
	ШтрихкодыНоменклатуры.Индексы.Добавить("Ссылка");
	ШтрихкодыНоменклатуры.Индексы.Добавить("НомерСтроки");
	ПараметрыПоискаШтрихкодыНоменклатуры = Новый Структура("Ссылка, НомерСтроки");

	Для Каждого СтрокаДанных Из ДанныеДокументов Цикл

		Для Каждого СтрокаТаблицы Из СтрокаДанных.ТаблицаЗапасы Цикл

			ПараметрыПоискаШтрихкодыНоменклатуры.Ссылка = СтрокаДанных.Ссылка;
			ПараметрыПоискаШтрихкодыНоменклатуры.НомерСтроки = СтрокаТаблицы.НомерСтроки;
			НайденныеСтрокиШтрихкоды = ШтрихкодыНоменклатуры.НайтиСтроки(ПараметрыПоискаШтрихкодыНоменклатуры);
			ПечатьДокументовУНФ.ПодобратьШтрихкодНоменклатуры(СтрокаТаблицы, НайденныеСтрокиШтрихкоды);
...
Процедура АдаптироватьДанныеУниверсальногоЗапросаДляМХ1(ДанныеОбъектовПечати)
	...
	ДанныеОбъектовПечати.Колонки.Добавить("ВидОперации", Новый ОписаниеТипов("Строка", , , ,
		Новый КвалификаторыСтроки(1)));
	
	// (4)
	ДанныеОбъектовПечати.Колонки.Удалить("ПредставлениеПодразделения");
	ДанныеОбъектовПечати.Колонки.Добавить("ПредставлениеПодразделения", Новый ОписаниеТипов("Строка", , , ,
		Новый КвалификаторыСтроки(1)));
	
	// (5)
	ДанныеОбъектовПечати.Колонки.Удалить("ПредставлениеСклада");
	ДанныеОбъектовПечати.Колонки.Добавить("ПредставлениеСклада", Новый ОписаниеТипов("Строка", , , ,
		Новый КвалификаторыСтроки(1)));
	
	// (6)
	ДанныеОбъектовПечати.Колонки.Удалить("ДолжностьРуководителя");
	ДанныеОбъектовПечати.Колонки.Добавить("ДолжностьРуководителя", Новый ОписаниеТипов("Строка", , , ,
		Новый КвалификаторыСтроки(1)));

	ДанныеОбъектовПечати.Колонки.Удалить("ДолжностьКладовщика");
	ДанныеОбъектовПечати.Колонки.Добавить("ДолжностьКладовщика", Новый ОписаниеТипов("Строка", , , ,
		Новый КвалификаторыСтроки(1)));
	...
	ДанныеОбъектовПечати.Колонки.Добавить("РасшифровкаПодписиРуководителя", Новый ОписаниеТипов("Строка", , , ,
		Новый КвалификаторыСтроки(1)));
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина,
	...
	РеквизитыШтуки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Справочники.КлассификаторЕдиницИзмерения.шт,
		"Код, Наименование");
	Запрос.УстановитьПараметр("ШтукаНаименование", РеквизитыШтуки.Наименование);
	Запрос.УстановитьПараметр("ШтукаКод", РеквизитыШтуки.Код);
	Запрос.УстановитьПараметр("ИспользоватьФаксимиле", ИспользоватьФаксимиле);
	...
	|	ВЫБОР
	|		КОГДА
	|			РасходнаяНакладная.Организация.ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|			И
	|				РасходнаяНакладная.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	|				ТОГДА РасходнаяНакладная.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору.Наименование
	|			ИНАЧЕ РасходнаяНакладная.Запасы.Номенклатура.ЕдиницаИзмерения.Наименование
	|		КОНЕЦ КАК ЕдиницаИзмеренияПоОКЕИ_Наименование,
	|		ВЫБОР
	...
	|		СтавкаНДС КАК СтавкаНДС,
	|		Вес КАК МассаБрутто,
	|		ВЫРАЗИТЬ((РасходнаяНакладная.Запасы.СуммаАвтоматическойСкидки + РасходнаяНакладная.Запасы.СуммаСкидкиНаценки) *
	|			РасходнаяНакладная.Курс / РасходнаяНакладная.Кратность КАК ЧИСЛО(15, 2)) КАК СуммаСкидкиПоСтроке,
	|		ВЫРАЗИТЬ(РасходнаяНакладная.Запасы.Цена * РасходнаяНакладная.Курс / РасходнаяНакладная.Кратность КАК ЧИСЛО(15, 2))
	|			КАК Цена,
	|		ВЫРАЗИТЬ(РасходнаяНакладная.Запасы.Сумма * РасходнаяНакладная.Курс / РасходнаяНакладная.Кратность КАК ЧИСЛО(15, 2))
	|			КАК Сумма,
	|		ВЫРАЗИТЬ(РасходнаяНакладная.Запасы.СуммаНДС * РасходнаяНакладная.Курс / РасходнаяНакладная.Кратность КАК ЧИСЛО(15,
	|			2)) КАК СуммаНДС,
	|		ВЫРАЗИТЬ(РасходнаяНакладная.Запасы.Всего * РасходнаяНакладная.Курс / РасходнаяНакладная.Кратность КАК ЧИСЛО(15, 2))
	|			КАК Всего,
	|		СуммаАкциза КАК Акциз) КАК ТаблицаЗапасы,
	...
	|		ВЫБОР
	|			КОГДА
	|				РасходнаяНакладная.ДобавленныеНаборы.НоменклатураНабора.ВариантПечатиНабора = ЗНАЧЕНИЕ(Перечисление.ВариантыПечатиНаборов.ТолькоНабор)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ КАК ВыводитьИтоги) КАК ТаблицаДобавленныеНаборы,
	|	РасходнаяНакладная.СведенияПрослеживаемости.(
	|		НомерСтроки КАК НомерСтроки,
	|		РНПТ КАК РНПТ,
	|		Количество КАК Количество,
	|		КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|		ИдентификаторСтроки КАК ИдентификаторСтроки) КАК СведенияПрослеживаемости,
	|	РасходнаяНакладная.НоменклатураДоставки КАК НоменклатураДоставки,
	...
	|	ВЫБОР
	|		КОГДА РасходнаяНакладнаяЗапасы.Ссылка.ОставлятьПустымКоличествоВУПД
	|		И РасходнаяНакладнаяЗапасы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|			ТОГДА """"
	|		ИНАЧЕ ВЫБОР
	...
	|				ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|			И
	|				РасходнаяНакладнаяЗапасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	...
	|	РасходнаяНакладнаяЗапасы.СуммаАвтоматическойСкидки + РасходнаяНакладнаяЗапасы.СуммаСкидкиНаценки КАК
	|		СуммаСкидкиПоСтроке,
	|	ВЫБОР
	|		КОГДА РасходнаяНакладнаяЗапасы.Ссылка.ОставлятьПустымКоличествоВУПД
	|		И РасходнаяНакладнаяЗапасы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	...
	|		И
	|			РасходнаяНакладнаяЗапасы.НоменклатураНабора.ВариантПечатиНабора = ЗНАЧЕНИЕ(Перечисление.ВариантыПечатиНаборов.НаборИКомплектующие)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НеобходимоВыделитьКакСоставНабора,
	...
	|	ВЫБОР
	|		КОГДА РасходнаяНакладнаяЗапасы.СуммаАкциза = 0
	|			ТОГДА ""Без акциза""
	|		ИНАЧЕ РасходнаяНакладнаяЗапасы.СуммаАкциза
	|	КОНЕЦ КАК Акциз,
	|	РасходнаяНакладнаяЗапасы.Цена КАК ЦенаДляПрослеживаемости,
	|	РасходнаяНакладнаяЗапасы.Количество КАК КоличествоДляПрослеживаемости
	...
	|	ВЫБОР
	|		КОГДА РасходнаяНакладнаяЗапасы.Ссылка.ОставлятьПустымКоличествоВУПД
	|		И РасходнаяНакладнаяЗапасы.НоменклатураДоставки.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|			ТОГДА """"
	|		ИНАЧЕ &ШтукаНаименование
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА РасходнаяНакладнаяЗапасы.Ссылка.ОставлятьПустымКоличествоВУПД
	|		И РасходнаяНакладнаяЗапасы.НоменклатураДоставки.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	...
	|		И РасходнаяНакладнаяЗапасы.НоменклатураДоставки.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|			ТОГДА """"
	|		ИНАЧЕ РасходнаяНакладнаяЗапасы.СтоимостьДоставки
	|	КОНЕЦ,
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.СуммаСкидкиПоСтроке * РасходнаяНакладнаяЗапасы.Ссылка.Курс /
	|			РасходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|	КОНЕЦ КАК СуммаСкидкиПоСтроке,
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях
	|		И НЕ (РасходнаяНакладнаяЗапасы.Ссылка.ОставлятьПустымКоличествоВУПД
	|		И РасходнаяНакладнаяЗапасы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
	|			ТОГДА ВЫБОР
	|				КОГДА РасходнаяНакладнаяЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|					ТОГДА РасходнаяНакладнаяЗапасы.Цена
	|				КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	|					ТОГДА ВЫРАЗИТЬ(ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.Цена КАК ЧИСЛО(15, 2)) * РасходнаяНакладнаяЗапасы.Ссылка.Курс /
	|						РасходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|				ИНАЧЕ ВЫБОР
	...
	|					ИНАЧЕ ВЫРАЗИТЬ((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС) /
	|						ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.Количество КАК ЧИСЛО(15, 2)) КАК ЧИСЛО(15, 2))
	|				КОНЕЦ
	|			КОНЕЦ
	|		ИНАЧЕ РасходнаяНакладнаяЗапасы.Цена
	|	КОНЕЦ КАК Цена,
	...
	|				КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	|					ТОГДА ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.Сумма * РасходнаяНакладнаяЗапасы.Ссылка.Курс /
	|						РасходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|				ИНАЧЕ ВЫБОР
	|					КОГДА РасходнаяНакладнаяЗапасы.Ссылка.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ(СуммыДокументовРегламентированныйУчет.Всего КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ВЫРАЗИТЬ(СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС КАК
	|						ЧИСЛО(15, 2))
	|				КОНЕЦ
	|			КОНЕЦ
	...
	|				КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	|					ТОГДА ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.СуммаНДС * РасходнаяНакладнаяЗапасы.Ссылка.Курс /
	|						РасходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|				КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	|					ТОГДА ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.Всего * РасходнаяНакладнаяЗапасы.Ссылка.Курс /
	|						РасходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|				ИЛИ
	|					РасходнаяНакладнаяЗапасы.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	...
	|	РасходнаяНакладнаяЗапасы.Акциз КАК Акциз,
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях
	|			ТОГДА ВЫБОР
	|				КОГДА РасходнаяНакладнаяЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|			КОНЕЦ
	|		ИНАЧЕ РасходнаяНакладнаяЗапасы.Сумма
	|	КОНЕЦ КАК ЦенаДляПрослеживаемости,
	|	РасходнаяНакладнаяЗапасы.КоличествоДляПрослеживаемости КАК КоличествоДляПрослеживаемости,
	|	РасходнаяНакладнаяЗапасы.СуммаВключаетНДС КАК СуммаВключаетНДС
	|ПОМЕСТИТЬ ИтоговаяТаблицаЗапасы
	|ИЗ
	...
	|	ИтоговаяТаблицаЗапасы.ДатаОтгрузочногоДокумента КАК ДатаОтгрузочногоДокумента,
	|	ИтоговаяТаблицаЗапасы.НомерОтгрузочногоДокумента КАК НомерОтгрузочногоДокумента,
	|	ИтоговаяТаблицаЗапасы.Номенклатура КАК Номенклатура,
	|	ИтоговаяТаблицаЗапасы.Содержание КАК Содержание,
	|	ИтоговаяТаблицаЗапасы.ПредставлениеНоменклатуры КАК ПредставлениеНоменклатуры,
	|	ИтоговаяТаблицаЗапасы.ТипНоменклатуры КАК ТипНоменклатуры,
	|	ИтоговаяТаблицаЗапасы.ЗапасКод КАК ЗапасКод,
	|	ИтоговаяТаблицаЗапасы.Код КАК Код,
	|	ИтоговаяТаблицаЗапасы.Артикул КАК Артикул,
	|	ИтоговаяТаблицаЗапасы.ЕдиницаИзмеренияПоОКЕИ_Наименование КАК ЕдиницаИзмеренияПоОКЕИ_Наименование,
	|	ИтоговаяТаблицаЗапасы.ЕдиницаИзмеренияПоОКЕИ_Код КАК ЕдиницаИзмеренияПоОКЕИ_Код,
	|	ИтоговаяТаблицаЗапасы.Характеристика КАК Характеристика,
	|	ИтоговаяТаблицаЗапасы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ИтоговаяТаблицаЗапасы.ВидУпаковки КАК ВидУпаковки,
	|	ИтоговаяТаблицаЗапасы.Количество КАК Количество,
	|	ИтоговаяТаблицаЗапасы.КоэффициентЕдиницыИзмерения КАК КоэффициентЕдиницыИзмерения,
	|	ИтоговаяТаблицаЗапасы.КоличествоВОдномМесте КАК КоличествоВОдномМесте,
	|	ИтоговаяТаблицаЗапасы.КоличествоМест КАК КоличествоМест,
	|	ИтоговаяТаблицаЗапасы.СтавкаНДС КАК СтавкаНДС,
	|	ИтоговаяТаблицаЗапасы.МассаБрутто КАК МассаБрутто,
	|	ИтоговаяТаблицаЗапасы.НомерСтроки КАК НомерСтроки,
	|	ИтоговаяТаблицаЗапасы.СуммаСкидкиПоСтроке КАК СуммаСкидкиПоСтроке,
	|	ИтоговаяТаблицаЗапасы.Цена КАК Цена,
	|	ИтоговаяТаблицаЗапасы.Сумма КАК Сумма,
	|	ИтоговаяТаблицаЗапасы.СуммаНДС КАК СуммаНДС,
	|	ИтоговаяТаблицаЗапасы.Всего КАК Всего,
	|	ИтоговаяТаблицаЗапасы.Ссылка КАК Ссылка,
	|	ИтоговаяТаблицаЗапасы.НоменклатураНабора КАК НоменклатураНабора,
	|	ИтоговаяТаблицаЗапасы.ХарактеристикаНабора КАК ХарактеристикаНабора,
	|	ИтоговаяТаблицаЗапасы.ЭтоНабор КАК ЭтоНабор,
	|	ИтоговаяТаблицаЗапасы.НеобходимоВыделитьКакСоставНабора КАК НеобходимоВыделитьКакСоставНабора,
	|	ИтоговаяТаблицаЗапасы.СтранаСсылка КАК СтранаСсылка,
	|	ИтоговаяТаблицаЗапасы.СтранаПредставление КАК СтранаПредставление,
	|	ИтоговаяТаблицаЗапасы.СтранаКод КАК СтранаКод,
	|	ИтоговаяТаблицаЗапасы.ПредставлениеГТД КАК ПредставлениеГТД,
	|	ИтоговаяТаблицаЗапасы.КодТНВЭД КАК КодТНВЭД,
	|	ИтоговаяТаблицаЗапасы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ИтоговаяТаблицаЗапасы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ИтоговаяТаблицаЗапасы.ПрослеживаемыйТоварЧисло КАК ПрослеживаемыйТоварЧисло,
	|	ИтоговаяТаблицаЗапасы.КодТНВЭДЕдиницаИзмеренияКод КАК КодТНВЭДЕдиницаИзмеренияКод,
	|	ИтоговаяТаблицаЗапасы.КодТНВЭДЕдиницаИзмеренияНаименование КАК КодТНВЭДЕдиницаИзмеренияНаименование,
	|	ИтоговаяТаблицаЗапасы.ШтрихКод КАК ШтрихКод,
	|	ИтоговаяТаблицаЗапасы.СтоимостьБезНалога КАК СтоимостьБезНалога,
	|	ИтоговаяТаблицаЗапасы.Акциз КАК Акциз
	|ИЗ
	|	ИтоговаяТаблицаЗапасы КАК ИтоговаяТаблицаЗапасы
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	НомерСтроки
	...
	|				КОГДА ИтоговаяТаблицаЗапасы.СуммаВключаетНДС
	|					ТОГДА ВЫРАЗИТЬ((ИтоговаяТаблицаЗапасы.Сумма - ИтоговаяТаблицаЗапасы.СуммаНДС) /
	|						ИтоговаяТаблицаЗапасы.КоличествоДляПрослеживаемости КАК ЧИСЛО(15, 2))
	|				ИНАЧЕ ВЫРАЗИТЬ(ИтоговаяТаблицаЗапасы.Сумма / ИтоговаяТаблицаЗапасы.КоличествоДляПрослеживаемости КАК ЧИСЛО(15, 2))
	|			КОНЕЦ
	...
	|		ПО ИтоговаяТаблицаЗапасы.Ссылка = РасходнаяНакладнаяСведенияПрослеживаемости.Ссылка
	|		И ИтоговаяТаблицаЗапасы.ИдентификаторСтроки = РасходнаяНакладнаяСведенияПрослеживаемости.ИдентификаторСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ТЗСтрок = РезультатыЗапроса[4].Выгрузить();
	ТзСтрокСведенияПрослеживаемости = РезультатыЗапроса[5].Выгрузить();
	
	ШтрихкодыНоменклатуры = РезультатыЗапроса[6].Выгрузить();
	ШтрихкодыНоменклатуры.Индексы.Добавить("Ссылка");
	ШтрихкодыНоменклатуры.Индексы.Добавить("НомерСтроки");
	ПараметрыПоискаШтрихкодыНоменклатуры = Новый Структура("Ссылка, НомерСтроки");
	...
	Для Каждого ТекущаяСтрока Из Результат Цикл
		...
		Для Каждого СтрокаТаблицы Из ТекущаяСтрока.ТаблицаЗапасы Цикл

			ПараметрыПоискаШтрихкодыНоменклатуры.Ссылка = СтрокаТаблицы.Ссылка;
			ПараметрыПоискаШтрихкодыНоменклатуры.НомерСтроки = СтрокаТаблицы.НомерСтроки;
			НайденныеСтрокиШтрихкоды = ШтрихкодыНоменклатуры.НайтиСтроки(ПараметрыПоискаШтрихкодыНоменклатуры);
			ПечатьДокументовУНФ.ПодобратьШтрихкодНоменклатуры(СтрокаТаблицы, НайденныеСтрокиШтрихкоды);
...
Процедура СформироватьТоварныйЧек(ТабличныйДокумент, ТекущийДокумент)
	...
	ШтрихкодыНоменклатуры = РезультатыЗапроса[1].Выгрузить();
	ШтрихкодыНоменклатуры.Индексы.Добавить("НомерСтроки");
	ПараметрыПоискаШтрихкодыНоменклатуры = Новый Структура("НомерСтроки");
	...
	Для Каждого Шапка Из ДанныеДокументов Цикл
		...
		ТекстЗаголовка = СтрШаблон(НСтр("ru ='Товарный чек № %1 от %2'"), НомерДокумента, Формат(Шапка.ДатаДокумента,
			"ДЛФ=DD"));
		...
		Если КонтактнаяИнформацияТоварногоЧека <> Перечисления.КонтактнаяИнформацияТоварногоЧека.Организация Тогда
			...
			Если ОбластьМакета <> Неопределено Тогда
				...
				ФактАдресСтруктурнойЕдиницы = ПечатьДокументовУНФ.КонтактнаяИнформация(Шапка.СтруктурнаяЕдиницаСсылка,
					Справочники.ВидыКонтактнойИнформации.ФактАдресСтруктурнойЕдиницы);
				Если Не ПустаяСтрока(ФактАдресСтруктурнойЕдиницы) Тогда

					ПредставлениеСтруктурнойЕдиницы = ПредставлениеСтруктурнойЕдиницы + ", "
						+ ФактАдресСтруктурнойЕдиницы;

				КонецЕсли;

				ТелефонСтруктурнойЕдиницы = ПечатьДокументовУНФ.КонтактнаяИнформация(Шапка.СтруктурнаяЕдиницаСсылка,
				...
				Если КонтактнаяИнформацияТоварногоЧека
					= Перечисления.КонтактнаяИнформацияТоварногоЧека.СтруктурнаяЕдиница Тогда
		...
		Если Не ЕстьСкидки Тогда
			...
			ОбластьКолонкаЗапас.ШиринаКолонки = ОбластьКолонкаЗапас.ШиринаКолонки + Макет.Область(
				"СуммаБезСкидки").ШиринаКолонки + Макет.Область("СуммаСкидки").ШиринаКолонки;
		...
		Для Каждого СтрокаЗапасы Из Шапка.ТаблицаЗапасы Цикл
			...
			ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиТоварныйЧек(СтрокаЗапасы, СтруктураЗаполненияСекции,
				ПараметрыНоменклатуры, Итоги, Шапка, ПараметрыПоискаШтрихкодыНоменклатуры, ШтрихкодыНоменклатуры);
		...
		Иначе
			...
			СтруктураЗаполненияСекции.Вставить("ЗаголовокНДС", ?(Шапка.СуммаВключаетНДС, "В том числе НДС:",
				"Сумма НДС:"));
...
Процедура СформироватьМ15(ТабличныйДокумент, ТекущийДокумент)
	...
	Если ИспользоватьПересчет Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Цена_Параметр", "ВЫБОР КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
																   ...
																   |			 КОНЕЦ
																   |КОНЕЦ");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Сумма_Параметр", "ВЫБОР КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
																	...
																	|		  КОНЕЦ
																	|КОНЕЦ");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СуммаНДС_Параметр", "ВЫБОР КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL 
																	   |	ТОГДА Выразить(РасходнаяНакладнаяЗапасы.СуммаНДС * &Курс / &Кратность КАК Число(15,2))
																	   |	ИНАЧЕ СуммыДокументовРегламентированныйУчет.НДС
																	   |КОНЕЦ");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Всего_Параметр", "ВЫБОР КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	...
	ВыборкаСтрокЗапасы = РезультатыЗапроса[0].Выбрать();
	...
	СведенияОГрузополучателе = ПечатьДокументовУНФ.СведенияОЮрФизЛице(Шапка.Контрагент, Шапка.ДатаДокумента, ,
		Шапка.БанковскийСчетКонтрагента);
	...
	Пока ВыборкаСтрокЗапасы.Следующий() Цикл
		...
		Иначе
			...
			ОбластьМакетаСтрока.Параметры.ЗапасНаименование = ПечатьДокументовУНФ.ПредставлениеНоменклатуры(
		...
		ПараметрыПоискаШтрихкодыНоменклатуры.НомерСтроки = ВыборкаСтрокЗапасы.НомерСтроки;
		НайденныеСтрокиШтрихкоды = ШтрихкодыНоменклатуры.НайтиСтроки(ПараметрыПоискаШтрихкодыНоменклатуры);
		ПечатьДокументовУНФ.ПодобратьШтрихкодНоменклатуры(ПараметрыНоменклатуры, НайденныеСтрокиШтрихкоды);
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета, ПараметрыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ИмяМакета = "ГарантийныйТалон" Тогда
			...
			НоменклатураВДокументахСервер.СформироватьГарантийныеТалоныДокумента(ТабличныйДокумент, ТекущийДокумент,
				Ошибки);
...
// Процедура формирует печатные формы объектов, используя переданные параметры и заполняет коллекцию печатных форм.
// 
// Параметры:
//  МассивОбъектов - Массив из ДокументСсылка.РасходнаяНакладная -документы, для которых сформировать печатные формы
//  ПараметрыПечати - Структура - дополнительные параметры печати
//  КоллекцияПечатныхФорм - ТаблицаЗначений - сформированные табличные документы
//  ОбъектыПечати - Структура - параметры сформированных табличных документов
//  ПараметрыВывода - Структура - параметры вывода сформированных табличных документов
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	Перем Ошибки;

	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм,
		Обработки.ПечатьАктаОбОказанииУслуг.ИдентификаторПечатнойФормы(Ложь, Ложь));
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = Обработки.ПечатьАктаОбОказанииУслуг.СинонимМакета(Ложь);
	...
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм,
		Обработки.ПечатьАктаОбОказанииУслуг.ИдентификаторПечатнойФормы(Ложь, Истина));
	Если ПечатнаяФорма <> Неопределено Тогда
	...
	Для Каждого СтрокаТаблицы Из ВозможныеВарианты Цикл
		...
		ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм,
			Обработки.ПечатьНакладная.ИдентификаторПечатнойФормы(СтрокаТаблицы.ВключаяУслуги,
		...
		Если ПечатнаяФорма <> Неопределено Тогда
			...
			ПечатнаяФорма.СинонимМакета = Обработки.ПечатьНакладная.СинонимМакета(СтрокаТаблицы.ВключаяУслуги);

			//@skip-check query-in-loop
			ДанныеОбъектовПечати = УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов,
				СтрокаТаблицы.ИспользоватьФаксимиле, Истина);
			Обработки.ПечатьНакладная.СформироватьПФ(ПечатнаяФорма, ДанныеОбъектовПечати, ОбъектыПечати,
	...
	ОписаниеПечатнойФормы = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм,
		Обработки.ПечатьМХ1.ИдентификаторПечатнойФормы());
	...
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм,
		Обработки.ПечатьМХ3.ИдентификаторПечатнойФормы());
	Если ПечатнаяФорма <> Неопределено Тогда
	...
	Для Каждого СтрокаТаблицы Из ВозможныеВарианты Цикл
		...
		Если ПечатнаяФорма <> Неопределено Тогда
			...
			Обработки.ПечатьТОРГ12.СформироватьПФ(ПечатнаяФорма, ДанныеОбъектовПечати, ОбъектыПечати,
				СтрокаТаблицы.ВключаяУслуги, ВыводитьФаксимиле);
	...
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм,
		Обработки.ПечатьАктПередачиПрав.ИдентификаторПечатнойФормы());
	Если ПечатнаяФорма <> Неопределено Тогда
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "М15") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "М15", НСтр(
			"ru = 'М15 (Накладная на отпуск материалов)'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "М15",
			ПараметрыПечати));
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТТН") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ТТН", НСтр(
			"ru = '1-Т (Товарно-транспортная накладная)'"), Обработки.ПечатьТТН.ПечатнаяФорма(МассивОбъектов,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТоварныйЧек") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ТоварныйЧек", НСтр(
			"ru = 'Товарный чек'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ТоварныйЧек", ПараметрыПечати));

	КонецЕсли;

	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм,
		Обработки.ПечатьСчетНаОплату.ИдентификаторПечатнойФормы(Ложь, Ложь));
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = Обработки.ПечатьСчетНаОплату.СинонимМакета(Ложь);
	...
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм,
		Обработки.ПечатьСчетНаОплату.ИдентификаторПечатнойФормы(Ложь, Истина));
	Если ПечатнаяФорма <> Неопределено Тогда
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ГарантийныйТалон") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ГарантийныйТалон", НСтр(
			"ru = 'Гарантийный талон'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ГарантийныйТалон",
			ПараметрыПечати));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	ЗначениеИдентификатора = СтрШаблон(ЗначениеИдентификатора,
		Обработки.ПечатьАктПередачиПрав.ИдентификаторПечатнойФормы());
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуСклад") Тогда
		ЗначениеИдентификатора = СтрЗаменить(ЗначениеИдентификатора, ",БланкТоварногоНаполнения", "");
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ВозможныеВарианты Цикл
		...
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = Обработки.ПечатьТОРГ12.ИдентификаторПечатнойФормы(СтрокаТаблицы.ВключаяУслуги,
			СтрокаТаблицы.ИспользоватьФаксимиле);
		КомандаПечати.Представление = Обработки.ПечатьТОРГ12.ПредставлениеПФ(СтрокаТаблицы.ВключаяУслуги,
			СтрокаТаблицы.ИспользоватьФаксимиле);
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидОперации", ЗначениеВидимости,
	...
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидОперации", ЗначениеВидимостиУПД,
		ВидСравнения.ВСписке);
	...
	Для Каждого СтрокаТаблицы Из ВозможныеВарианты Цикл
		...
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = Обработки.ПечатьНакладная.ИдентификаторПечатнойФормы(СтрокаТаблицы.ВключаяУслуги,
			СтрокаТаблицы.ИспользоватьФаксимиле);
		КомандаПечати.Представление = Обработки.ПечатьНакладная.ПредставлениеПФ(СтрокаТаблицы.ВключаяУслуги,
			СтрокаТаблицы.ИспользоватьФаксимиле);
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.ФункциональныеОпции = "ИспользоватьПодсистемуСклад";
	КомандаПечати.Порядок = 47;

	Если ПравоДоступа("Просмотр", Метаданные.Обработки.ПечатьЭтикетокИЦенников) И ПолучитьФункциональнуюОпцию(
		"ПечатьЭтикетокИЦенников") И Не УправлениеДоступомУНФ.ЕстьПрофильРабочееМестоКассира() Тогда
	...
	Если ПравоДоступа("Использование", Метаданные.Отчеты.РеестрДокументов) Тогда
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "РеестрДокументовРасходнаяНакладная";
...
Процедура ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиТоварныйЧек(СтрокаТабличнойЧасти, ДанныеПечати,
	ПараметрыНоменклатуры, СтруктураИтогов, Шапка, ПараметрыПоискаШтрихкодыНоменклатуры, ШтрихкодыНоменклатуры)
	...
	ПараметрыНоменклатуры.Вставить("ПредставлениеСерииНоменклатуры", СерииНоменклатурыУНФ.СтрокаСерииНоменклатуры(
		Шапка.ТаблицаСерииНоменклатуры, СтрокаТабличнойЧасти.КлючСвязи));
	// Наборы
	...
	ПараметрыПоискаШтрихкодыНоменклатуры.НомерСтроки = НомерСтроки;
	НайденныеСтрокиШтрихкоды = ШтрихкодыНоменклатуры.НайтиСтроки(ПараметрыПоискаШтрихкодыНоменклатуры);
	ПечатьДокументовУНФ.ПодобратьШтрихкодНоменклатуры(СтрокаТабличнойЧасти, НайденныеСтрокиШтрихкоды);
...
Процедура СформироватьТаблицаПодарочныеСертификаты(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		...
		Или СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		= Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;
	Запрос.Текст = РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийПодарочныхСертификатов(
		ВыполнятьДвиженияПоСертификатам);
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаОплатаПодарочнымиСертификатами(ДокументСсылкаРасходнаяНакладная,
	СтруктураДополнительныеСвойства)
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		...
		Или СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации
		= Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;
	Запрос.Текст = РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийОплатыПодарочнымиСертификатами(
		ВыполнятьДвиженияПоСертификатам);
	Результат = Запрос.Выполнить().Выгрузить();
...
//    * ЗначенияОбщихРеквизитов - Соответствие из КлючИЗначение:
//      ** Ключ     - Строка - имя реквизита в шаблоне;
//      ** Значение - Строка - значение заполнения в шаблоне.
...
//  ПредметСообщения - ЛюбаяСсылка, Структура - ссылка на объект являющийся источником данных, либо структура с полями:
