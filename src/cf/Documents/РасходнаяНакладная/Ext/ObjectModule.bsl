...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции() И ЗначениеЗаполнено(Контрагент)
		...
		Если (СтранаРегистрации = Справочники.СтраныМира.Россия Или Не ЗначениеЗаполнено(СтранаРегистрации))
			...
			И ВидыПродукцииВЗапасах.НайтиПоЗначению(Перечисления.ВидыПродукцииИС.АлкогольнаяПродукцияДо9Процентов)
			= Неопределено Тогда
	...
	Если ПоложениеДокументаПоступления = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке И УчетОстатковПоСкладскимОрдерам
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю Или ВидОперации
		= Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию Или ВидОперации
		= Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером Тогда
...
Процедура ЗаполнитьПоПриходнойНакладной(ДанныеЗаполнения, Операция = "") Экспорт
	...
	Если Операция = "Продажа" Тогда
		...
		Иначе
			...
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
		Если НесколькоЗаказовПокупателей(ДанныеЗаполнения) Или Не ПерсональныеНастройкиРУНФ.РеквизитВШапке(
...
Процедура ЗаполнитьПоЗаказуПоставщику(ЗаказПоставщику) Экспорт
	...
	Если ПерсональныеНастройкиРУНФ.РеквизитВШапке("ПоложениеЗаказаПокупателяВДокументахОтгрузки") Тогда
...
Процедура ЗаполнитьПоСчетуНаОплату(ДанныеЗаполнения) Экспорт
	...
	Пока ВыборкаИсключаемыеНаборы.Следующий() Цикл
		ДобавитьОписаниеНабора(
		ПропускаемыеНаборы, ВыборкаИсключаемыеНаборы.НоменклатураНабора, ВыборкаИсключаемыеНаборы.ХарактеристикаНабора);
	КонецЦикла; 
	// Конец Наборы
...
Процедура ЗаполнитьПоПеремещениюЗапасов(ДанныеЗаполнения) Экспорт
	...
	Иначе
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ЗначенияПроверяемыхРеквизитов = Новый Структура("СостояниеЗаказа, Проведен, ВидОперации",
				Выборка.СостояниеЗаказа, Выборка.ОснованиеПроведен, Выборка.ВидОперации);
...
Процедура ЗаполнитьПоСборкеЗапасов(ДанныеЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ЗначенияРеквизитов.ПоложениеЗаказаПокупателя) Тогда
		...
		КонецЕсли;
	ИначеЕсли ПерсональныеНастройкиРУНФ.РеквизитВШапке("ПоложениеЗаказаПокупателяВДокументахОтгрузки") Тогда
		Заказ = ЗначенияРеквизитов.ЗаказПокупателя;
	Иначе
		Заказ = Неопределено;
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходномуОрдеру(ДанныеЗаполнения) Экспорт
	...
	Если ПоложениеСклада = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке И ВидУчета
		= Перечисления.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаЗапасы Из МассивСтрокЗапасы Цикл
			...
			Если СтрокаЗапасы.Количество >= ВсегоОстаток Тогда
				СтрокаЗапасы.Резерв = СтрокаЗапасы.Резерв + ВсегоОстаток;
				Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
...
Процедура ЗаполнитьРеквизитыПечати()
	...
	ОрганизацияОплачиваетПеревозку = ПерсональныеНастройкиРУНФ.Прочитать(
		"ОрганизацияОплачиваетПеревозку", Организация);

	Если Не ЗначениеЗаполнено(УсловияСчетаЗаказа) Тогда
		УсловияСчетаЗаказа = ПерсональныеНастройкиРУНФ.Прочитать("УсловияСчетаЗаказа", Организация);
	КонецЕсли;

	Если Не ЗначениеЗаполнено(УсловияГарантийногоТалона) Тогда
		УсловияГарантийногоТалона = ПерсональныеНастройкиРУНФ.Прочитать("УсловияГарантии", Организация);
	КонецЕсли;
...
Процедура ВыполнитьПроверкуОграниченийСертификатов(Отказ)
	...
	Сертификаты.Колонки.Добавить("Регистратор", Новый ОписаниеТипов("ДокументСсылка.РасходнаяНакладная"));
...
// Наборы
Процедура ДобавитьОписаниеНабора(ПропускаемыеНаборы, НоменклатураНабора, ХарактеристикаНабора, Заказ = Неопределено)
