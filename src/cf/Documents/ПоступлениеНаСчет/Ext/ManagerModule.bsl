...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации =
		Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика Тогда
		Для Каждого СтрокаТаблицы Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПоставщиками Цикл
			ЭтоДенежныйДокументАванса =  ЗначениеЗаполнено(СтрокаТаблицы.Документ)
				И (ТипЗнч(СтрокаТаблицы.Документ) = Тип("ДокументСсылка.РасходИзКассы")
				Или ТипЗнч(СтрокаТаблицы.Документ) = Тип("ДокументСсылка.РасходСоСчета"));
...
Функция ТекстЗапросаТаблицаУправленческий()
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ТаблицаДокумента.СуммаРасчетов - ТаблицаДокумента.СуммаРасчетовКомиссии,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СчетЗатратЭквайринг.Валютный
	|			ТОГДА ТаблицаДокумента.СуммаУчетаКомиссии
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СчетУчетаЭквайринг.Валютный
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ТаблицаДокумента.СуммаРасчетовКомиссии,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам)
...
//
// Параметры:
//  ДокументСсылкаПоступлениеНаСчет - ДокументСсылка.ПоступлениеНаСчет - документ-источник движений
//  СтруктураДополнительныеСвойства	 - Структура - свойство объекта ДополнительныеСвойства
//
Процедура СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства) Экспорт
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ПоступлениеНаСчетЭквайринговыеОперации.СуммаРасчетовВозврата КАК Сумма,
		               |	НАЧАЛОПЕРИОДА(ТаблицаДокумента.Дата, ДЕНЬ) КАК ДатаПервичногоДокумента,
		               ...
		               |		ПО (ПоступлениеНаСчетЭквайринговыеОперации.Ссылка = ТаблицаДокумента.Ссылка)
		               ...
		               |	ПоступлениеНаСчетЭквайринговыеОперации.СуммаРасчетовВозврата,
		               |	НАЧАЛОПЕРИОДА(ТаблицаДокумента.Дата, ДЕНЬ),
		               ...
		               |				И (ОперацияПоПлатежнымКартам.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю))
		               |		ПО (ПоступлениеНаСчетЭквайринговыеОперации.Ссылка = ТаблицаДокумента.Ссылка)
		               |			И (ПоступлениеНаСчетЭквайринговыеОперации.УчитыватьВНУ)
		               |			И (ТаблицаДокумента.ПоложениеНастроекНалоговогоУчета = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти))
		               |			И (ТаблицаДокумента.Ссылка = &Ссылка)
		               |;
		               |
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура ОбработатьОплатуСчетаЧерезСБП(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства) Экспорт

	Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаОплатаСчетовИЗаказов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДатаВходящегоДокумента =
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПоступлениеНаСчет, "ДатаВходящегоДокумента");
		
	Дата = ?(ЗначениеЗаполнено(ДатаВходящегоДокумента), ДатаВходящегоДокумента, СтруктураДополнительныеСвойства.ДляПроведения.Дата);
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	Запрос.УстановитьПараметр("ТаблицаОплатаСчетовИЗаказов", СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаОплатаСчетовИЗаказов);
	Запрос.УстановитьПараметр("ДокументСсылкаПоступлениеНаСчет", ДокументСсылкаПоступлениеНаСчет);
	Запрос.УстановитьПараметр("Дата", Дата);
	
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТаблицаОплатаСчетовИЗаказов.СчетНаОплату КАК СчетНаОплату,
	               |	ТаблицаОплатаСчетовИЗаказов.Организация КАК Организация,
	               |	ТаблицаОплатаСчетовИЗаказов.СуммаАванса КАК СуммаАванса,
	               |	ТаблицаОплатаСчетовИЗаказов.СуммаОплаты КАК СуммаОплаты
	               |ПОМЕСТИТЬ ТаблицаОплатаСчетовИЗаказов
	               |ИЗ
	               |	&ТаблицаОплатаСчетовИЗаказов КАК ТаблицаОплатаСчетовИЗаказов
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |ПОМЕСТИТЬ ОплатаСчетовИЗаказовРегистр
	               |ИЗ
	               |	РегистрНакопления.ОплатаСчетовИЗаказов.Обороты(НАЧАЛОПЕРИОДА(&Дата, ДЕНЬ), КОНЕЦПЕРИОДА(&Дата, ДЕНЬ), Регистратор, ) КАК ОплатаСчетовИЗаказовОбороты
	               |ГДЕ
	               |	ВЫРАЗИТЬ(ОплатаСчетовИЗаказовОбороты.Регистратор КАК Документ.ОперацияПоПлатежнымКартам).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП)
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	ОплатаСчетовИЗаказовРегистр.Регистратор КАК ОперацияПоПлатежнымКартам
	               |ИЗ
	               |	ТаблицаОплатаСчетовИЗаказов КАК ТаблицаОплатаСчетовИЗаказов
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОплатаСчетовИЗаказовРегистр КАК ОплатаСчетовИЗаказовРегистр
	               |		ПО ТаблицаОплатаСчетовИЗаказов.СчетНаОплату = ОплатаСчетовИЗаказовРегистр.СчетНаОплату
	               |			И ТаблицаОплатаСчетовИЗаказов.Организация = ОплатаСчетовИЗаказовРегистр.Организация
	               |			И ТаблицаОплатаСчетовИЗаказов.СуммаАванса = ОплатаСчетовИЗаказовРегистр.СуммаАванса
	               |			И ТаблицаОплатаСчетовИЗаказов.СуммаОплаты = ОплатаСчетовИЗаказовРегистр.СуммаОплаты";
	...
	Пока Выборка.Следующий() Цикл
		ОперацияПоПлатежнымКартамОбъект = Выборка.ОперацияПоПлатежнымКартам.ПолучитьОбъект();
		ОперацияПоПлатежнымКартамОбъект.ПоступлениеНаСчет = ДокументСсылкаПоступлениеНаСчет;
		ОперацияПоПлатежнымКартамОбъект.Записать(РежимЗаписиДокумента.Проведение);
	КонецЦикла;	
	
	УстановитьПривилегированныйРежим(Ложь);
	
КонецПроцедуры // ОбработатьОплатуСчетаЧерезСБП()
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства) Экспорт
	...
	СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства);
...
Процедура ВыполнитьКонтроль(ДокументСсылкаПоступлениеНаСчет, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияДенежныеСредстваИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияРасчетыСПокупателямиИзменение Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщиками.Остатки(&МоментКонтроля, Организация = &Организация) КАК РасчетыСПоставщикамиОстатки
		...
		Если НЕ МассивРезультатов[4].Пустой() Тогда
			...
			КонтрольОстатковУНФ.РасчетыСПоставщиками(ДокументОбъектПоступлениеНаСчет, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Использование", Метаданные.Отчеты.РеестрДокументов) Тогда
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "РеестрДокументовПоступлениеНаСчет";
