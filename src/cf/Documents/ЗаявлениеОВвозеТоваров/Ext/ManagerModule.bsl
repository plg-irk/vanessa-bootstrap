...
Функция ТоварыПоДаннымПоступления(ДанныеОбъекта, ДокументыПоступления) Экспорт
	...
	ВалютаРеглУчета = УправлениеНебольшойФирмойПовтИсп.ПолучитьНациональнуюВалюту();
	
	ВидыОпераций = Новый Массив;
	ВидыОпераций.Добавить(Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика);
	ВидыОпераций.Добавить(Перечисления.ВидыОперацийПриходнаяНакладная.ПриемНаКомиссию);
	...
	|	НЕОПРЕДЕЛЕНО КАК КодВидаТранспорта,
	...
	|	ЕСТЬNULL(СправочникНоменклатура.ТоварнаяНоменклатураВЭД, """") КАК КодТНВЭД,
	...
	|		ПО (Товары.Ссылка = (ВЫРАЗИТЬ(СчетФактура.ДокументОснование КАК Документ.ПриходнаяНакладная)))
	|ГДЕ
	...
	Если ЕстьУчетПодакцизныхТоваров Тогда
		...
		УчетАкцизовУНФ.ЗаполнитьДанныеПоАкцизам(ТаблицаЗначенийТовары, ДанныеОбъекта.Организация, ДанныеОбъекта.Дата, ПараметрыЗаполнения);
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Реквизиты.Вставить("НДСВключенВСтоимость",
		Не ПлательщикНДС Или (ОрганизацияОсвобожденаОтУплатыНДС));
	Реквизиты.Вставить("ПрименяетсяУСНДоходыМинусРасходы", Ложь);
	...
	//СчетаТМЦ.Добавить(ПланыСчетов.Управленческий.СырьеИМатериалы); // 10 счет
	//Субсчета10 = БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(СчетаТМЦ, УсловияОтбораСубсчетов);
	//
	//СчетаТМЦ.Добавить(ПланыСчетов.Управленческий.ТоварыПродукция);   // дополняем счетом 41
	//СубсчетаТМЦ = БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(СчетаТМЦ, УсловияОтбораСубсчетов);
	
	Счет10 = Новый Массив();
	Счет10.Добавить(ПланыСчетов.Управленческий.СырьеИМатериалы); // 10 счет 
	
	Счета41 = Новый Массив();
	Счета41.Добавить(ПланыСчетов.Управленческий.ТоварыПродукция);   // дополняем счетом 41
	
	Запрос.УстановитьПараметр("Субсчета10",  Счет10);
	Запрос.УстановитьПараметр("СубсчетаТМЦ", Счета41);
	...
	Если ВедетсяПростойУчетЕНП Тогда
		...
		Если КоличествоСтрок > 0 
			...
			МассивСумм = ОбщегоНазначенияУНФКлиентСервер.РаспределитьПропорционально(
...
Функция ПодготовитьТаблицыЗаявленияОВвозеТоваровДляВыгрузкиПечати(МассивДокументов) Экспорт
	...
	|	Реквизиты.Контрагент.НаименованиеПолное КАК ПродавецНаимОрг,
	...
	|	Реквизиты.ДоговорКонтрагента.НомерДоговора КАК НомерДог,
	|	Реквизиты.ДоговорКонтрагента.ДатаДоговора КАК ДатаДог,
	...
	|	Товары.Номенклатура.НаименованиеПолное КАК П000010000102,
	...
	Для каждого СтрокаШапки Из Шапка Цикл
		...
		Руководители = ОтветственныеЛицаОрганизации(СтрокаШапки.Организация);
		...
		ПоляПредставления = "ЮридическийАдрес,ЮридическоеФизическоеЛицо,ПолноеНаименование,ФизическоеЛицо";
		СведенияОПокупателе = ПечатьДокументовУНФ.ОписаниеОрганизации(СтрокаШапки.Организация, ПоляПредставления);
		...
		Если СведенияОПокупателе.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
			ДанныеФизЛица = Справочники.ФизическиеЛица.ДанныеФизЛица(СтрокаШапки.Организация, СведенияОПокупателе.ФизическоеЛицо);
			РеквизитыРаздел1.ПокупательНаимОрг = ДанныеФизЛица.Представление;
		Иначе
		...
		Если НайденныеСтрокиУчастникиСделки.Количество() > 0 Тогда
			...
			Если СтрокаУчастникиСделки <> Неопределено Тогда
				...
				СведенияОПродавце = ПечатьДокументовУНФ.СведенияОЮрФизЛице(
				...
				Если НайденныеСтрокиУчастникиСделки.Количество() = 1 Тогда
					...
					СведенияОПокупателе = ПечатьДокументовУНФ.СведенияОЮрФизЛице(
		...
		СведенияОКонтрагенте = ПечатьДокументовУНФ.СведенияОЮрФизЛице(СтрокаШапки.Контрагент, СтрокаШапки.Дата);
		МестонахождениеКонтрагента = СформироватьПредставлениеМестонахожденияКонтрагента(
			СтрокаШапки.КодСтраныПродавец, 
			ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОКонтрагенте, "ЮридическийАдрес,"));
...
Функция ОтветственныеЛицаОрганизации(Организация)
	
	СтруктураОтветственныхЛиц = Новый Структура;
	
	СтруктураОтветственныхЛиц.Вставить("РуководительПредставление", "");
	СтруктураОтветственныхЛиц.Вставить("РуководительДолжностьПредставление", "");
	СтруктураОтветственныхЛиц.Вставить("ГлавныйБухгалтерПредставление", "");
	СтруктураОтветственныхЛиц.Вставить("ДолжностьГлавногоБухгалтера", "");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Организации.ПодписьРуководителя.РасшифровкаПодписи КАК РуководительПредставление,
		|	Организации.ПодписьРуководителя.Должность КАК РуководительДолжностьПредставление,
		|	Организации.ПодписьГлавногоБухгалтера.РасшифровкаПодписи КАК ГлавныйБухгалтерПредставление,
...
Функция ПодготовитьТаблицыСтатистическойФормыУчетаПеремещенияТоваровДляВыгрузкиПечати(МассивДокументов) Экспорт
	...
	Для каждого СтрокаШапки Из Шапка Цикл
		...
		Если ЗначениеЗаполнено(РеквизитыШапка.Организация) И ЗначениеЗаполнено(РеквизитыШапка.Контрагент) Тогда
			...
			СведенияОПродавце = ПечатьДокументовУНФ.СведенияОЮрФизЛице(РеквизитыШапка.Контрагент, РеквизитыШапка.Дата);
			СведенияОПокупателе = ПечатьДокументовУНФ.СведенияОЮрФизЛице(РеквизитыШапка.Организация, РеквизитыШапка.Дата);
			
			РеквизитыШапка.ПолучательРеквизиты = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОПокупателе, "ПолноеНаименование,СтранаРегистрации,ЮридическийАдрес,");
			...
			РеквизитыШапка.ОтправительРеквизиты = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОПродавце, "ПолноеНаименование,СтранаРегистрации,ЮридическийАдрес,");
			РеквизитыШапка.ОтправительНаименование = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОПродавце, "ПолноеНаименование,");
			...
			ПолучательНаименование = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОПокупателе, "ПолноеНаименование,");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаявлениеОВвозеТоваров") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			УчетНДСУНФ.ПечатьЗаявленияОВвозеТоваров(МассивОбъектов, ОбъектыПечати));
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СтатистическаяФормаУчетаПеремещенияТоваров") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			УчетНДСУНФ.ПечатьСтатистическойФормыУчетаПеремещенияТоваров(МассивОбъектов, ОбъектыПечати));
...
Функция ПолучитьАдресВВидеСоответствия(СсылкаНаОбъект, ВидКонтактнойИнформации, Дата = '00010101')
	...
	КонтактнаяИнформация = ПечатьДокументовУНФ.КонтактнаяИнформация(СсылкаНаОбъект, ВидКонтактнойИнформации, Дата);
...
Функция ТекстЗапросаТаблицаТовары()
	...
	|	ЗаявлениеТовары.ДокументПартии.СтруктурнаяЕдиница КАК Склад,
	|	ЗаявлениеТовары.ДокументПартии.Подразделение КАК Подразделение,
	|	ВЫБОР
