...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КэшЗначений.Вставить("ЭтоАванс", (КэшЗначений.ВидОперации = Перечисления.ВидыОперацийСчетФактура.Аванс));
	
	Если КэшЗначений.ЭтоАванс Тогда
	    ...
	    Запрос.УстановитьПараметр("Ссылка", КэшЗначений.Ссылка);
	    ...
	    Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	         ...
	         КэшЗначений.ДокументАванса = ВыборкаДетальныеЗаписи.ДокументОснование;
	        // Вставить обработку выборки ВыборкаДетальныеЗаписи
	    КонецЦикла;

   КонецЕсли;
	
	КЭШОрганизация = КэшЗначений.Организация; // см. СправочникСсылка.Организации
	ВидСтавкиНДСПоУмолчанию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КЭШОрганизация,
	...
	КэшЗначений.Вставить("СтавкаНДСПоУмолчанию", УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСРасчетная(
		Справочники.СтавкиНДС.СтавкаНДС(ВидСтавкиНДСПоУмолчанию, КэшЗначений.Дата)));
...
Процедура АвансыКорректируемыйСчетФактураПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.КорректируемыйСчетФактура) Тогда
		...
		ЗаполнитьЗначенияСвойств(ДанныеСтроки, ВладелецФормы.Объект, "Дата");
...
Процедура ОбъектАвансыНоменклатураПриИзменении(Элемент)
	...
	РассчитатьСуммуНДСТабЧасти(ТекущиеДанные, КэшЗначений.СуммаВключаетНДС);
...
Процедура АвансыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока 
	    ...
	    Если ЗначениеЗаполнено(КэшЗначений.ДокументАванса) Тогда
	        Элемент.ТекущиеДанные.ДокументОснование = КэшЗначений.ДокументАванса;   
	    КонецЕсли;
...
Процедура РассчитатьСуммуНДСТабЧасти(СтрокаТабличнойЧасти, СуммаВключаетНДС) Экспорт
	...
	СтрокаТабличнойЧасти.СуммаНДС = ?(СуммаВключаетНДС, СтрокаТабличнойЧасти.Сумма - (СтрокаТабличнойЧасти.Сумма)
		/ ((СтавкаНДС + 100) / 100), СтрокаТабличнойЧасти.Сумма * СтавкаНДС / 100);

	СтавкаНДСАвансыПеречисление = УчетНДСВызовСервера.РасчетнаяСтавкаНДСОрганизации(
		КэшЗначений.Организация, КэшЗначений.Дата);

	УчетНДСКлиентСервер.РассчитатьСуммуНДС(
        СтрокаТабличнойЧасти.Сумма, СуммаВключаетНДС, УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(
...
Функция ПолучитьДанныеНоменклатураПриИзменении(НоменклатураСсылка)
	...
	Если ТипЗнч(СтавкаНДСРасчетная) = Тип("СправочникСсылка.СтавкиНДС")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтавкаНДСРасчетная, "Расчетная") Тогда
...
Процедура ПеренестиЗапасыВТабличнуюЧастьАвансов(ДокументСсылка, ИмяТабличнойЧасти, ЭтоЗаказПокупателя)
	...
	Если ЭтоЗаказПокупателя Тогда
		...
		Если НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ОжидаетсяВыборВариантаКП") Тогда
			
			МассивСтрок = ДокументСсылка[ИмяТабличнойЧасти].НайтиСтроки(Новый Структура("НомерВариантаКП",
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ОсновнойВариантКП")));
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗапасов Цикл
		...
		НоваяСтрока.Сумма						= СтрокаТаблицы.Сумма + ?(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			ДокументСсылка, "СуммаВключаетНДС"), 0, СтрокаТаблицы.СуммаНДС);
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "СуммаНДСДоставки") <> 0 Тогда
		
		НоваяСтрока = Авансы.Добавить();
		НоваяСтрока.Содержание					= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка,
			"НоменклатураДоставки");
		НоваяСтрока.Сумма						= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка,
			"СтоимостьДоставки") + ?(
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "СуммаВключаетНДС"), 0,
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "СуммаНДСДоставки"));
		НоваяСтрока.СуммаНДС					= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка,
			"СуммаНДСДоставки");
		НоваяСтрока.СтавкаНДС					= ПолучитьРасчетнуюСтавкуНДС(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			ДокументСсылка, "СтавкаНДСДоставки"));
...
Процедура УстановитьПараметрыВыбора()
	...
	Элементы["АвансыКорректируемыйСчетФактура"].ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров);
