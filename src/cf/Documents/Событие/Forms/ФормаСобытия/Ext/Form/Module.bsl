...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ТипСобытия = Перечисления.ТипыСобытий.ЛичнаяВстреча Тогда
		ОбсужденияДоступны = Обсуждения.ОбсужденияДоступны();
	КонецЕсли;
	
	ВидеовстречиДоступны = ОбсужденияДоступны И Видеовстречи.ИспользованиеФункционалаДоступно();
	Элементы.СоздатьОбсуждение.Видимость = ВидеовстречиДоступны;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ОбновитьДанныеВидеовстречиНаСервере();
...
&НаКлиенте
Процедура АдресОбсужденияВидеовстречиПриИзменении(Элемент)

	ОбновитьДанныеВидеовстречиНаКлиенте();
...
Процедура СоздатьКонтрагент(Команда)
	...
	ЭлектроннаяПочтаУНФКлиент.СоздатьКонтрагентНаОснованииСобытия(, ЭтотОбъект);
...
Процедура СоздатьКонтакт(Команда)
	...
	ЭлектроннаяПочтаУНФКлиент.СоздатьКонтактНаОснованииСобытия(, ЭтотОбъект);
...
Процедура СоздатьЛид(Команда)
	...
	ЭлектроннаяПочтаУНФКлиент.СоздатьЛидНаОснованииСобытия(, ЭтотОбъект);
...
&НаКлиенте
Процедура ОткрытьОбсуждениеВидеовстречи(Команда)
...
&НаКлиенте
Асинх Процедура СоздатьОбсуждение(Команда)

	Если Параметры.Ключ.Пустая() Или Модифицированность Тогда
		Ответ = Ждать ВопросАсинх(НСтр("ru='Документ будет записан. Продолжить?'"), РежимДиалогаВопрос.ДаНет, ,
			КодВозвратаДиалога.Да, НСтр("ru='Обсуждение'"));
	...
	Если ЗначениеЗаполнено(Объект.Тема) Тогда
		ОписаниеВидеовстречи.Тема = Объект.Тема;
	Иначе 
		ШаблонТемы = НСтр("ru='Встреча от %1'");
		ОписаниеВидеовстречи.Тема = СтрШаблон(ШаблонТемы, Формат(Объект.НачалоСобытия, "ДФ=dd.MM.yyyy"));
	КонецЕсли;
	...
	Объект.АдресОбсуждения = Результат.АдресОбсуждения;
...
Процедура СформироватьПредставлениеПродолжительности(Форма)
	...
	СобытияУНФКлиентСервер.СформироватьПредставлениеПродолжительности(Форма, "ПредставлениеПродолжительности",
		Форма.Объект.НачалоСобытия, Форма.Объект.ОкончаниеСобытия);
...
Процедура ОбновитьИнтерфейсКомандПоЗаявкам()
	...
	ПервыйКонтактЭтоЛид = (ТипЗнч(Контрагент) = Тип("СправочникСсылка.Лиды"));
...
&НаКлиенте
Процедура НабратьНомерОбработчик()
...
&НаСервере
Процедура ОбновитьДанныеВидеовстречиНаСервере()
	
	ЭтоЛичнаяВстреча = Объект.ТипСобытия = ПредопределенноеЗначение("Перечисление.ТипыСобытий.ЛичнаяВстреча");
	
	Элементы.АдресОбсуждения.Видимость = ЭтоЛичнаяВстреча;
	Элементы.СоздатьОбсуждение.Видимость = ЭтоЛичнаяВстреча И ВидеовстречиДоступны 
		И ПустаяСтрока(Объект.АдресОбсуждения);;
	Элементы.ПерейтиВОбсуждение.Видимость = ЭтоЛичнаяВстреча И Не ПустаяСтрока(Объект.АдресОбсуждения);

	Элементы.АдресОбсуждения.РастягиватьПоГоризонтали = Не Элементы.СоздатьОбсуждение.Видимость 
		И Не Элементы.ПерейтиВОбсуждение.Видимость;
		
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьДанныеВидеовстречиНаКлиенте()
