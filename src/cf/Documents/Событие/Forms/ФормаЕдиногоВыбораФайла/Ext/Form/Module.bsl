...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ВыборФайла Тогда
		...
		Заголовок = НСтр("ru = 'Сохранить файлы в основании'");
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыПапкиСортировки
...
&НаСервереБезКонтекста
Функция ФайлыПоВладельцам(Владельцы)
	
	Результат = Новый Соответствие;
	Для Каждого Владелец Из Владельцы Цикл
		Результат[Владелец] = Новый Массив;
	КонецЦикла;
	
	ИменаСправочниковПоВладельцам = Новый Соответствие;
	Для Каждого Владелец Из Владельцы Цикл
		ПолноеИмяСправочника = "Справочник." + РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(Владелец);
		ИменаСправочниковПоВладельцам[Владелец] = ПолноеИмяСправочника;
	КонецЦикла;
	
	Запрос = Новый Запрос("");
	Для Индекс = 0 По Владельцы.ВГраница() Цикл
		
		Владелец = Владельцы[Индекс];
		ПолноеИмяСправочника = ИменаСправочниковПоВладельцам[Владелец];
		...
		|	%2 КАК Файлы
		|ГДЕ
		|	Файлы.ВладелецФайла = &ВладелецФайлов%3",
		Запрос.Текст,
		ПолноеИмяСправочника,
		Индекс);
		
		Запрос.УстановитьПараметр("ВладелецФайлов" + Индекс, Владелец);
		
	КонецЦикла;
	
	Выборка = Запрос.Выполнить().Выбрать();
	ДанныеФайлов = ОбработатьПолученныеФайлы(Выборка);
	
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		
		Если ТипЗнч(Результат[ДанныеФайла.ВладелецФайла]) = Тип("Массив") Тогда
			Результат[ДанныеФайла.ВладелецФайла].Добавить(ДанныеФайла);
		КонецЕсли;
...
Функция ОбработатьПолученныеФайлы(Выборка)
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеФайла.Вставить("Автор" , Выборка.Автор);
		ДанныеФайла.Вставить("ДатаСоздания", Выборка.ДатаСоздания);
		ДанныеФайла.Вставить("ИндексКартинки", Выборка.ИндексКартинки);
		...
		ДанныеФайла.Вставить("ВладелецФайла", Выборка.ВладелецФайла);
...
Процедура ЗаполнитьСвязанныеПисьма(ДеревоПапок, МассивВладельцевФайлов)
	...
	ВременноеДерево = ДеревоСтруктурыПодчиненности;
	
	Если ЗначениеЗаполнено(Документ) Тогда
		ДокументыУНФ.ЗаполнитьДеревоСтруктуройПодчинения(Документ, ВременноеДерево, УжеВСписке, Истина);
	КонецЕсли;
...
Функция АдресВыбранныхФайловВХранилище(ВыделенныеДокументы)
	...
	ПодходящиеВладельцыФайлов = Новый Массив;
	Для Каждого ВладелецФайла Из ВыделенныеДокументы Цикл
		...
		ПодходящиеВладельцыФайлов.Добавить(ВладелецФайла);
		
	КонецЦикла;
	
	ФайлыПоВладельцам = ФайлыПоВладельцам(ПодходящиеВладельцыФайлов);
	
	МассивФайлов = Новый Массив;
	Для Каждого ВладелецФайла Из ВыделенныеДокументы Цикл
		
		ФайлыПоВладельцу = ФайлыПоВладельцам[ВладелецФайла];
	...
	Для Каждого Ссылка Из Результат Цикл
		...
		ОписаниеВложения.Вставить("Расширение", ДанныеФайла.Расширение);
		ОписаниеВложения.Вставить("Ссылка", ДанныеФайла.Ссылка);
		ОписаниеВложения.Вставить("Имя", ДанныеФайла.ИмяФайла);
		ОписаниеВложения.Вставить("Идентификатор", "");
		ОписаниеВложения.Вставить("Данные", ПолучитьИзВременногоХранилища(ДанныеФайла.СсылкаНаДвоичныеДанныеФайла));
