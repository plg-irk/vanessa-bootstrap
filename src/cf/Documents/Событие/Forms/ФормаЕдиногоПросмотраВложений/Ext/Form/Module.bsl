...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл" И Параметр.Свойство("Событие")
		...
		Если ЗначениеЗаполнено(НужнаяСтрока) Тогда
			...
			ДействияПриАктивизацииСтрокиВложений(Элементы.Вложения);
		КонецЕсли;
...
Процедура ВложенияПриАктивизацииСтроки(Элемент)
	...
	ДействияПриАктивизацииСтрокиВложений(Элемент);
...
Процедура СохранитьФайл(Команда)
	...
	ДанныеФайла = ДанныеФайла(Элементы.Вложения.ТекущиеДанные.Ссылка, УникальныйИдентификатор);
...
Процедура СохранитьВсеФайлы(Команда)
	...
	Ссылки = Новый Массив;
	Для каждого Идентификатор Из Элементы.Вложения.ВыделенныеСтроки Цикл
		Ссылки.Добавить(Вложения.НайтиПоИдентификатору(Идентификатор).Ссылка);
	КонецЦикла;
	
	ДанныеФайлов = ДанныеФайлов(Ссылки, УникальныйИдентификатор);
	Если ДанныеФайлов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаписьФайлаЗавершение", ЭтотОбъект);
	ФайловаяСистемаКлиент.СохранитьФайлы(ОписаниеОповещения, ДанныеФайлов);
...
Процедура Печать(Команда)
	...
	Если РасширениеТекущегоФайла = "pdf" Тогда
		ДокументПДФ.Напечатать(РежимИспользованияДиалогаПечати.НеИспользовать);
...
&НаКлиенте
Процедура ПоказатьФайл(УФайлаЕстьДанные)
	...
	Если Не УФайлаЕстьДанные Тогда
		Элементы.СтраницыПредпросмотра.ТекущаяСтраница = Элементы.СтраницаПредупреждение;
		ТипФайла = "Неподдерживаемый";
	ИначеЕсли Не (СписокРасширенийИзображений.НайтиПоЗначению(РасширениеТекущегоФайла) = Неопределено) Тогда
...
&НаКлиенте
Процедура ДействияПриАктивизацииСтрокиВложений(Элемент)
...
Процедура ЗаполнитьРеквизитыФормыПоСобытию(Событие)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(СвязиАдресатКонтактCRM.КонтактCRM, СобытиеУчастники.Контакт) ССЫЛКА Справочник.Лиды
		|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(СвязиАдресатКонтактCRM.КонтактCRM, СобытиеУчастники.Контакт) КАК Справочник.Лиды).Вид
		|		КОГДА ЕСТЬNULL(СвязиАдресатКонтактCRM.КонтактCRM, СобытиеУчастники.Контакт) ССЫЛКА Справочник.Контрагенты
		|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(СвязиАдресатКонтактCRM.КонтактCRM, СобытиеУчастники.Контакт) КАК Справочник.Контрагенты).ВидКонтрагента
		|		ИНАЧЕ NULL
		|	КОНЕЦ,
		|	NULL,
		|	NULL
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.КонтактCRM) Тогда
			...
			ИндексКартинки = ЭлектроннаяПочтаУНФ.НомерКартинкиУчастникаПоТипуКонтакта(
			Выборка.КонтактCRM, Выборка.ВидКонтакта, 0);
			Значение.Вставить("ИндексКартинки", ИндексКартинки);
			
			УчастникиИДубли.Добавить(Значение);
...
&НаСервереБезКонтекста
Функция ДанныеФайлов(Знач Ссылки, Знач УникальныйИдентификатор)
	
	Результат = Новый Массив;
	
	Для каждого Файл Из Ссылки Цикл
		ДанныеФайла = РаботаСФайлами.ДанныеФайла(Файл, УникальныйИдентификатор);
