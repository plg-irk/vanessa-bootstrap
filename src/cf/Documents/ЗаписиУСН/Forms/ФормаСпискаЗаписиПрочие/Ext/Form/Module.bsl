...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьОтборПоПериоду(ЭтотОбъект, ОтборПериодЗначение.ДатаНачала, ОтборПериодЗначение.ДатаОкончания,
		ОтборПериодИспользование);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененыНастройкиФормированияКУДиР" Тогда
...
Процедура ДекорацияУстановитьСдаватьОтчетностьНажатие(Элемент)
	ОткрытьОрганизациюИспользуетсяОтчетность();
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияИспользоватьОтчетностьНажатие(Элемент)
	ОткрытьОрганизациюИспользуетсяОтчетность();
КонецПроцедуры
...
Процедура ДобавитьРучнуюЗаписьУСН(Команда)
	ОткрытьФорму("Документ.ЗаписиУСН.Форма.ФормаЗаписиКУДИР",
		Новый Структура("ДокументыЗаПериод, Организация", ДокументыЗаПериод, Организация), Элементы.КУДиР);
КонецПроцедуры
...
Процедура ПересчитатьСДаты(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПересчитатьСДатыЗавершение", ЭтотОбъект);
...
Процедура НастроитьВидимостьИДоступность()
	...
	Если МесяцыДоходыБезНДС.Количество() > 0 Тогда	
		Элементы.КУДиРГруппаПлательщикНДС.Видимость = Истина;
		Элементы.КУДиРГруппаБезНДС.Видимость = Ложь;
	Иначе
		Элементы.КУДиРГруппаПлательщикНДС.Видимость = Ложь;
		Элементы.КУДиРГруппаБезНДС.Видимость = Истина;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьОрганизациюИспользуетсяОтчетность()
...
Процедура УстановитьУсловноеОформление()
	...
	КУДиР.УсловноеОформление.Элементы.Очистить();
	УОРасходНДС = КУДиР.УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = УОРасходНДС.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("НДС");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(УОРасходНДС.Отбор, "ЕстьОбщаяСтавкаНДС",
		ВидСравненияКомпоновкиДанных.Равно, Ложь);
	УОРасходНДС.Оформление.УстановитьЗначениеПараметра("ЦветФона", ЦветаСтиля.РозоваяПодсветкаВажныхСтрок);
...
Процедура ПоказатьПредложениеСменыУчетаПартий(ЭтоОткрытиеФормы)
	...
	ОткрытьФорму("Документ.ЗаписиУСН.Форма.ФормаПереключениеПартий", ДанныеПредложения, ЭтотОбъект, , , ,
...
Процедура УстановитьОтборПоПериоду(Форма, НачалоПериода, КонецПериода, Использование)
	...
	ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбора(ОтборКомпоновкиДанных, "ГодДокумента");
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ОтборКомпоновкиДанных, "ГодДокумента", 
		Год(НачалоПериода), ВидСравненияКомпоновкиДанных.Равно, "ДатаПервичногоДокументаНачало", Использование);
...
Процедура УстановитьНастройкиПоОрганизации()
	...
	МесяцыИсключенияНДС = Документы.ЗаписиУСН.МесяцыИсключенияНДСИзДоходовРасходов(Организация,
		ОтборПериодЗначение.ДатаНачала, ОтборПериодЗначение.ДатаОкончания);
	...
	// НДС в течение года не применяется, МесяцыРасходыБезНДС - все месяцы года, чтобы убрать выделение колонки НДС:
	// сохранение внешнего вида до 2025 года для ИП без НДС.
	Если МесяцыДоходыБезНДС.Количество() = 0 Тогда
		Месяц = НачалоГода(ОтборПериодЗначение.ДатаНачала);
		Пока Месяц < КонецГода(ОтборПериодЗначение.ДатаОкончания) Цикл
			МесяцыРасходыБезНДС.Добавить(Месяц);
			Месяц = ДобавитьМесяц(Месяц, 1);
		КонецЦикла;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(КУДиР, "МесяцыОбщаяСтавкаНДС", МесяцыРасходыБезНДС);
...
Процедура ВыборПериодаИзВыпадающегоСписка(Результат, ПараметрыВыбора) Экспорт
	...
	Если ПараметрыВыбора.Вариант = Неопределено
		Или ПараметрыВыбора.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		
		СписокПериодов = ОтчетыУНФКлиентСервер.СписокФиксированныхПериодов(
			ПараметрыВыбора.НачалоПериода, ПараметрыВыбора.ВидПериода);
		...
		Если Не ПараметрыВыбора.Свойство("ВариантСтандартногоПериодаПоВиду") Тогда
			ПараметрыВыбора.Вставить("ВариантСтандартногоПериодаПоВиду",
				ОтчетыУНФКлиентСервер.ПривестиВидПериодаКСтандартному(ПараметрыВыбора.ВидПериода));
		КонецЕсли;
...
Процедура НачатьВыборПериодаИзВыпадающегоСписка()
	...
	Если ДанныеВыбораПериодаИзВыпадающегоСписка.Свойство("ПараметрыВыбора") Тогда
		...
		ПоказатьВыборИзСписка(Обработчик, Контекст.СписокПериодов, Контекст.ПараметрыВыбора.Элемент,
			Контекст.ИндексНачальногоЗначения);
	КонецЕсли;
...
Процедура ЗагрузитьРезультатВыбораПериодаИзВыпадающегоСписка(Результат, ПараметрыВыбора)
	...
	Иначе
		ОтборПериодЗначение.ДатаНачала = ОтчетыУНФКлиентСервер.НачалоПериодаОтчета(
			ПараметрыВыбора.ВидПериода, Результат.Значение);
		ОтборПериодЗначение.ДатаОкончания  = ОтчетыУНФКлиентСервер.КонецПериодаОтчета(
			ПараметрыВыбора.ВидПериода, Результат.Значение);
...
Функция ВключенУчетПоПартиямУСН()
	...
	Возврат УчетПартийУСН.ТекущийРегистрУчетаПартийТоваров()
