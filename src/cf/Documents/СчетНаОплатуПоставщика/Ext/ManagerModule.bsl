...
// Процедура регистрирует цены поставщика в регистре сведений "ЦеныНоменклатурыКонтрагентов".
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.СчетНаОплатуПоставщика - документ, для которого необходимо зарегистрировать цены.
//  Отказ - Булево - если установлен в Истина, то процедура завершается без регистрации цен.
//
...
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.СчетНаОплатуПоставщика - ссылка на документ
...
//
// Параметры:
//	ДокументСсылка - ДокументСсылка.СчетНаОплатуПоставщика - ссылка на текущий документ
//	СтруктураДополнительныеСвойства - Структура - дополнительные свойства документа
//
Процедура СформироватьТаблицаДенежныеСредстваВРезерве(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|				КОГДА КурсыВалютРасчетов.Курс <> 0
	|				И КурсыВалютДокумента.Кратность <> 0
	|					ТОГДА КурсыВалютДокумента.Курс * КурсыВалютРасчетов.Кратность / (ЕСТЬNULL(КурсыВалютРасчетов.Курс, 1) *
...
// Выполняет контроль возникновения отрицательных остатков по денежным средствам в резерве и с учетом резервов.
//
// Параметры:
//  ДокументСсылкаСчетНаОплатуПоставщика - ДокументСсылка.СчетНаОплатуПоставщика - документ, для которого выполняется контроль
...
//
Процедура ВыполнитьКонтроль(ДокументСсылкаСчетНаОплатуПоставщика, ДополнительныеСвойства, Отказ,
	...
	|ИЗ
	|	РегистрНакопления.ДенежныеСредстваВРезерве.Остатки(&МоментКонтроля, Документ = &СсылкаНаДокумент) КАК
	|		ДенежныеСредстваВРезервеОстатки
	|ГДЕ
	...
	|	ЕСТЬNULL(РезервыПоДокументам.СуммаОстаток, 0) +
	|		ЕСТЬNULL(НеснижаемыеОстаткиДенежныхСредствСрезПоследних.СуммаНеснижаемогоОстатка, 0) -
	|		ДвиженияДенежныеСредстваВРезервеИзменение.СуммаПриЗаписи КАК ВРезерве,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НеснижаемыеОстаткиДенежныхСредств.СрезПоследних(&МоментКонтроля,) КАК
	|			НеснижаемыеОстаткиДенежныхСредствСрезПоследних
	...
	|		И
	|			ДвиженияДенежныеСредстваВРезервеИзменение.БанковскийСчетКасса = НеснижаемыеОстаткиДенежныхСредствСрезПоследних.БанковскийСчетКасса
	...
	|ГДЕ
	|	ЕСТЬNULL(ДенежныеСредстваОстатки.СуммаВалОстаток, 0) -
	|		(ЕСТЬNULL(НеснижаемыеОстаткиДенежныхСредствСрезПоследних.СуммаНеснижаемогоОстатка, 0) +
	|		ЕСТЬNULL(РезервыПоДокументам.СуммаОстаток, 0)) < 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СчетНаОплатуПоставщика.РезервироватьДенежныеСредства КАК РезервироватьДенежныеСредства
	|ИЗ
	|	Документ.СчетНаОплатуПоставщика КАК СчетНаОплатуПоставщика
	|ГДЕ
	|	СчетНаОплатуПоставщика.Ссылка = &СсылкаНаДокумент");
	...
	Если Не МассивРезультатов[0].Пустой() Или Не МассивРезультатов[1].Пустой() Тогда
	...
	Если Не МассивРезультатов[0].Пустой() Тогда
		...
		КонтрольОстатковУНФ.ДенежныеСредстваВРезерве(ДокументОбъектСчетНаОплатуПоставщика, ВыборкаИзРезультатаЗапроса,
			Отказ);
	КонецЕсли;
	...
	Если Не МассивРезультатов[1].Пустой() Тогда //Если остатка денежных средств не хватает, то выводить ошибку по резервам нет смысла
		ВыборкаРезервироватьДенежныеСредства = МассивРезультатов[2].Выбрать();
		ВыборкаРезервироватьДенежныеСредства.Следующий();
		Если ВыборкаРезервироватьДенежныеСредства.РезервироватьДенежныеСредства Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[1].Выбрать();
			КонтрольОстатковУНФ.ДенежныеСредстваСУчетомРезервов(ДокументСсылкаСчетНаОплатуПоставщика,
...
//
// Параметры:
// ТекущийДокумент - ДокументСсылка.СчетНаОплатуПоставщика - документ, сумму НДС которого нужно вычислить
//
// Возвращаемое значение:
...
Функция НаличиеСуммыНДС(ТекущийДокумент) Экспорт
	...
	|ИЗ
	|	Документ.СчетНаОплатуПоставщика.Запасы КАК СчетНаОплатуПоставщикаТовары
	|ГДЕ
...
// Параметры:
// ТекущийДокумент - ДокументСсылка.СчетНаОплатуПоставщика - документ, ставку по которому надо проверить
// СтавкаНДСПоУСН - СправочникСсылка.СтавкиНДС - искомая ставка НДС по УСН
//
// Возвращаемое значение:
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.СчетНаОплатуПоставщика";
