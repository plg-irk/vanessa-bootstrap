...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
		Документы.ВводНачальныхОстатков.ПараметрыУказанияСерий(Объект));
...
Процедура ДополнитьДанныеПоШтрихКодам(ДополняемыеДанные, ИмяТабличнойЧасти = "Запасы")
	...
	Для Каждого ТекШтрихкод Из ДополняемыеДанные.МассивШтрихкодов Цикл
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекШтрихкод, "ЭтоКодМаркировки")
			...
			ДополнитьДанныеПоШтрихкодамГОСИС(ДополняемыеДанные.ПараметрыСканированияМаркировки,
				ДополняемыеДанные.ДанныеФормы, ТекШтрихкод, ДанныеПоШтрихКодамГОСИС);
		Иначе
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрихкодов)
	...
	ШтрихКодыНоменклатуры.Вставить("МассивШтрихкодов", ПодготовленныеШтрихкоды);
	...
	Если КодМаркировки Тогда
		...
		ШтрихКодыНоменклатуры.Вставить("ПараметрыСканированияМаркировки", ПараметрыСканированияМаркировки);
		ШтрихКодыНоменклатуры.Вставить("ДанныеФормы", ДанныеФормы);
	КонецЕсли;
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				Если ЗначениеЗаполнено(СтрокаТаблицы.Цена_ВходящиеДанные) 
				И НЕ ЗначениеЗаполнено(СтрокаТаблицы.Сумма_ВходящиеДанные) Тогда
					
					НоваяСтрока.Сумма = СтрокаТаблицы.Цена * СтрокаТаблицы.Количество;
					
				ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаТаблицы.Цена_ВходящиеДанные) 
				
				И ЗначениеЗаполнено(СтрокаТаблицы.Сумма_ВходящиеДанные) Тогда 
