...
Процедура ПриОткрытии(Отказ)
	...
	ОбновитьОтображениеКолонокОстаткиПланФакт();
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если РежимОстаткиПланФакт Тогда
		ЗаполнитьЗначенияКолонокОстатковПланФакт();
	КонецЕсли; 
...
&НаКлиенте
Процедура ОстаткиПланФакт(Команда)
	
	РежимОстаткиПланФакт = Не РежимОстаткиПланФакт;
	Элементы.ОперацииОстаткиПланФакт.Пометка = РежимОстаткиПланФакт;
...
&НаКлиенте
Процедура ОбновитьОтображениеКолонокОстаткиПланФакт()
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КоличествоОстатки", "Видимость", РежимОстаткиПланФакт);
	
КонецПроцедуры

// Процедура заполняет значения колонок "КоличествоОстаткиПлан", "КоличествоОстаткиФакт" и "КоличествоПланПоЗаказу" в табличной части "Операции" документа.
//
&НаСервере
Процедура ЗаполнитьЗначенияКолонокОстатковПланФакт()
	
	ТаблицаОпераций = Объект.Операции.Выгрузить();
	ТаблицаОпераций.Свернуть("Номенклатура, Характеристика, Партия, Спецификация, Операция, Этап, ЗаказНаПроизводство, ЗаказПокупателя", "КоличествоПлан, КоличествоФакт");
	
	Если ЗначениеЗаполнено(Объект.ДокументОснование) И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.СборкаЗапасов") Тогда
		СсылкаСборкаЗапасов = Объект.ДокументОснование;
	Иначе
		СсылкаСборкаЗапасов = Документы.СборкаЗапасов.ПустаяСсылка();;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Операции", ТаблицаОпераций);
	Запрос.УстановитьПараметр("СборкаЗапасов", СсылкаСборкаЗапасов);
	...
	|	Операции.ЗаказПокупателя,
	|	&СборкаЗапасов КАК СборкаЗапасов
	|ПОМЕСТИТЬ Операции
	|ИЗ
	...
	|	Операции.ЗаказНаПроизводство,
	|	Операции.ЗаказПокупателя,
	|	Операции.СборкаЗапасов,
	|	ЕСТЬNULL(СпецификацииОперации.Количество / СпецификацииОперации.КоличествоПродукции *
	|		ЗаказПокупателяЗапасы.Количество * ЕСТЬNULL(ЕдиницыИзмерения.Коэффициент, 1), 0) КАК КоличествоПланПоЗаказу,
	...
	|			ПО ЗаказПокупателяЗапасы.ЕдиницаИзмерения = ЕдиницыИзмерения.Ссылка
	|		ПО Операции.ЗаказПокупателя = ЗаказПокупателяЗапасы.Ссылка
	|		И Операции.Номенклатура = ЗаказПокупателяЗапасы.Номенклатура
	|		И Операции.Характеристика = ЗаказПокупателяЗапасы.Характеристика
	|		И Операции.Спецификация = ЗаказПокупателяЗапасы.Спецификация
	|		И Операции.Партия = ЗаказПокупателяЗапасы.Партия
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Спецификации.Операции КАК СпецификацииОперации
	|		ПО Операции.Спецификация = СпецификацииОперации.Ссылка
	|		И Операции.Операция = СпецификацииОперации.Операция
	|		И Операции.Этап = СпецификацииОперации.Этап
	|ГДЕ
	|	Операции.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|	И Операции.ЗаказНаПроизводство = ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Операции.СборкаЗапасов,
	|	ЕСТЬNULL(СпецификацииОперации.Количество / СпецификацииОперации.КоличествоПродукции *
	|		СборкаЗапасовПродукция.Количество * ЕСТЬNULL(ЕдиницыИзмерения.Коэффициент, 1), 0),
	|	0,
	|	0
	|ИЗ
	|	Операции КАК Операции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборкаЗапасов.Продукция КАК СборкаЗапасовПродукция
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЕдиницыИзмерения КАК ЕдиницыИзмерения
	|			ПО СборкаЗапасовПродукция.ЕдиницаИзмерения = ЕдиницыИзмерения.Ссылка
	|		ПО Операции.СборкаЗапасов = СборкаЗапасовПродукция.Ссылка
	|		И Операции.Номенклатура = СборкаЗапасовПродукция.Номенклатура
	|		И Операции.Характеристика = СборкаЗапасовПродукция.Характеристика
	|		И Операции.Партия = СборкаЗапасовПродукция.Партия
	|		И Операции.Спецификация = СборкаЗапасовПродукция.Спецификация
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Спецификации.Операции КАК СпецификацииОперации
	|		ПО Операции.Спецификация = СпецификацииОперации.Ссылка
	|		И Операции.Операция = СпецификацииОперации.Операция
	|		И Операции.Этап = СпецификацииОперации.Этап
	|ГДЕ
	|	Операции.ЗаказНаПроизводство = ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
	|	И Операции.СборкаЗапасов <> ЗНАЧЕНИЕ(Документ.СборкаЗапасов.ПустаяСсылка)
	|	И Операции.ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Операции.ЗаказПокупателя,
	|	Операции.СборкаЗапасов,
	|	ЕСТЬNULL(ЗаказНаПроизводствоОперации.КоличествоПлан, 0),
	|	0,
	|	0
	|ИЗ
	|	Операции КАК Операции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство.Операции КАК ЗаказНаПроизводствоОперации
	...
	|		ПО Операции.ЗаказНаПроизводство = ЗаказНаПроизводствоПродукция.Ссылка
	|		И Операции.Номенклатура = ЗаказНаПроизводствоПродукция.Номенклатура
	|		И Операции.Характеристика = ЗаказНаПроизводствоПродукция.Характеристика
	|		И Операции.Партия = ЗаказНаПроизводствоПродукция.Партия
	|		И Операции.Спецификация = ЗаказНаПроизводствоПродукция.Спецификация
	|		И Операции.Операция = ЗаказНаПроизводствоОперации.Операция
	|		И Операции.Этап = ЗаказНаПроизводствоОперации.Этап
	|ГДЕ
	|	Операции.ЗаказНаПроизводство <> ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Операции.СборкаЗапасов,
	|	0,
	|	СдельныеНарядыОбороты.КоличествоПланОборот,
	|	СдельныеНарядыОбороты.КоличествоФактОборот
	|ИЗ
	|	Операции КАК Операции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СдельныеНаряды.Обороты(,, Регистратор,
	|		НЕ ЗаказПокупателя ЕСТЬ NULL
	|		И НЕ ДокументПроизводства ЕСТЬ NULL) КАК СдельныеНарядыОбороты
	|		ПО Операции.Номенклатура = СдельныеНарядыОбороты.Номенклатура
	|		И Операции.Характеристика = СдельныеНарядыОбороты.Характеристика
	|		И Операции.Операция = СдельныеНарядыОбороты.Операция
	|		И Операции.Этап = СдельныеНарядыОбороты.Этап
	|		И Операции.ЗаказПокупателя = СдельныеНарядыОбороты.ЗаказПокупателя
	|		И ВЫБОР
	|			КОГДА Операции.ЗаказНаПроизводство <> ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
	|				ТОГДА Операции.ЗаказНаПроизводство = СдельныеНарядыОбороты.ДокументПроизводства
	|			КОГДА Операции.СборкаЗапасов <> ЗНАЧЕНИЕ(Документ.СборкаЗапасов.ПустаяСсылка)
	|				ТОГДА Операции.СборкаЗапасов = СдельныеНарядыОбороты.ДокументПроизводства
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|ГДЕ
	|	СдельныеНарядыОбороты.Регистратор <> Операции.ТекущийДокумент";
	...
	Для Каждого СтрокаОперации Из ТаблицаОпераций Цикл
		...
		СтруктураДляПоиска.Вставить("Номенклатура", СтрокаОперации.Номенклатура);
		СтруктураДляПоиска.Вставить("Характеристика", СтрокаОперации.Характеристика);
		СтруктураДляПоиска.Вставить("Партия", СтрокаОперации.Партия);
		СтруктураДляПоиска.Вставить("Спецификация", СтрокаОперации.Спецификация);
		СтруктураДляПоиска.Вставить("Операция", СтрокаОперации.Операция);
		СтруктураДляПоиска.Вставить("Этап", СтрокаОперации.Этап);
		СтруктураДляПоиска.Вставить("ЗаказНаПроизводство", СтрокаОперации.ЗаказНаПроизводство);
		СтруктураДляПоиска.Вставить("ЗаказПокупателя", СтрокаОперации.ЗаказПокупателя);
		
		СтрокиОстатков = ТаблицаОстатков.НайтиСтроки(СтруктураДляПоиска);
		
		КоличествоФактОстатки = СтрокаОперации.КоличествоФакт;
		КоличествоПланОстатки = СтрокаОперации.КоличествоПлан;
		КоличествоПланПоЗаказу = 0;
		Для Каждого СтрокаОстатка Из СтрокиОстатков Цикл
			КоличествоФактОстатки = КоличествоФактОстатки + СтрокаОстатка.КоличествоФакт;
			КоличествоПланОстатки = КоличествоПланОстатки + СтрокаОстатка.КоличествоПлан;
			КоличествоПланПоЗаказу = КоличествоПланПоЗаказу + СтрокаОстатка.КоличествоПланПоЗаказу;
		КонецЦикла;
		
		СтрокиТабличнойЧасти = Объект.Операции.НайтиСтроки(СтруктураДляПоиска);
		Для Каждого СтрокаТабличнойЧасти Из СтрокиТабличнойЧасти Цикл
			СтрокаТабличнойЧасти.КоличествоОстаткиФакт = КоличествоФактОстатки;
			СтрокаТабличнойЧасти.КоличествоОстаткиПлан = КоличествоПланОстатки;
			СтрокаТабличнойЧасти.КоличествоПланПоЗаказу = КоличествоПланПоЗаказу;;
			СтрокаТабличнойЧасти.ПредставлениеКоличествоПлан = СтрШаблон("%1 / %2", Формат(КоличествоПланПоЗаказу, "ЧДЦ=3"), Формат(КоличествоПланОстатки, "ЧДЦ=3"));
		КонецЦикла;
...
Процедура ОткрытьФормуНастройки(ПередаватьВидДокумента)
	...
	Если ПередаватьВидДокумента Тогда
		СтруктураПараметров.Вставить("ВидДокумента", НСтр("ru = 'сдельный наряд'"));
	КонецЕсли;
	
	СтруктураПараметров.Вставить("ПоложениеИсполнителяВСдельномНаряде", Объект.ПоложениеИсполнителя);
	СтруктураПараметров.Вставить("ПоложениеСтруктурнойЕдиницыВСдельномНаряде", Объект.ПоложениеСтруктурнойЕдиницы);
	...
	Если Объект.ПоложениеСтруктурнойЕдиницы<>Результат.ПоложениеСтруктурнойЕдиницыВСдельномНаряде Тогда
		Если Результат.ПоложениеСтруктурнойЕдиницыВСдельномНаряде<>Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
			...
			Если Не ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) Тогда
				Объект.СтруктурнаяЕдиница = ПерсональныеНастройкиРУНФ.Прочитать("ОсновноеПодразделение");
			КонецЕсли; 
		КонецЕсли; 
		Для каждого СтрокаТабличнойЧасти Из Объект.Операции Цикл
			СтрокаТабличнойЧасти.СтруктурнаяЕдиница = Объект.СтруктурнаяЕдиница;
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Операции.КоличествоПланПоЗаказу", Новый ПолеКомпоновкиДанных("Объект.Операции.КоличествоОстаткиПлан"), ВидСравненияКомпоновкиДанных.Равно);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ОперацииПредставлениеКоличествоПлан");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", ЦветаСтиля.ВыполненноеЗадание);
	
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Операции.КоличествоПланПоЗаказу", Новый ПолеКомпоновкиДанных("Объект.Операции.КоличествоОстаткиФакт"), ВидСравненияКомпоновкиДанных.Равно);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ОперацииКоличествоФактВсего");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", ЦветаСтиля.ВыполненноеЗадание);
	
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Операции.КоличествоПланПоЗаказу", 0, ВидСравненияКомпоновкиДанных.Равно);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ОперацииПредставлениеКоличествоПлан");
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ОперацииКоличествоФактВсего");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", ЦветаСтиля.ЦветТекстаПодсказкиВвода);
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", "<Нет данных>");
