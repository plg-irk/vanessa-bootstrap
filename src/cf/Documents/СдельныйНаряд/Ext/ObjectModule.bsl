...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	ВидОперацииЗаказаПокупателя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
	ДокументСсылкаЗаказПокупателя, "ВидОперации");
	
	Если ВидОперацииЗаказаПокупателя = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу 
		ИЛИ ВидОперацииЗаказаПокупателя = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПереработку Тогда
		
		ЗаполнитьПоЗаказуНаПродажуПереработку(ДокументСсылкаЗаказПокупателя);
		
	ИначеЕсли ВидОперацииЗаказаПокупателя = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		
		ЗаполнитьПоЗаказНаряду(ДокументСсылкаЗаказПокупателя);
		
	ИначеЕсли РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(ВидОперацииЗаказаПокупателя) Тогда
...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДокументСсылкаЗаказНаПроизводство) Экспорт
	...
	Иначе
		...
		ПоложениеЗаказаНаПроизводство = ПерсональныеНастройкиРУНФ.Прочитать(ИмяНастройки);
...
Процедура ЗаполнитьПоЗаказуНаПродажуПереработку(ДокументСсылкаЗаказПокупателя)
	...
	ЗаполнитьЗначенияСвойств(ПроверяемыеРеквизиты, ЗначенияРеквизитов);
	Документы.ЗаказПокупателя.ПроверитьВозможностьВводаНаОснованииЗаказаПокупателя(
	ДокументСсылкаЗаказПокупателя, ПроверяемыеРеквизиты);
...
Процедура ПроверитьЗаполнениеЭтаповПроизводства(Отказ, ОтменаПроведения = Ложь)
	...
	ТаблицаПродукции.Колонки.Добавить("КлючСвязи", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10)));
	Если ПоложениеЗаказаНаПроизводство<>Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
