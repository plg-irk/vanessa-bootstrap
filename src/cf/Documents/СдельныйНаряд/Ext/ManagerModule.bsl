...
Функция ТекстЗапросаИнициализации()
	...
	|	ВЫБОР
	|		КОГДА
	|			СдельныйНарядСоставБригады.Ссылка.ПоложениеСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВШапке)
	...
	|	ВЫБОР
	|		КОГДА
	|			СдельныйНарядОперации.Ссылка.ПоложениеСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВШапке)
	...
	|	И
	|		СдельныйНарядОперации.Ссылка.ПоложениеИсполнителя = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА
	|			СдельныйНарядОперации.Ссылка.ПоложениеСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	...
	|		И
	|			СдельныйНарядОперации.Ссылка.ДокументОснование.ЗаказНаПроизводство <> ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
	...
	|	СдельныйНарядОперации.Стоимость * КурсыВалютРасчетов.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс *
	|		КурсыВалютРасчетов.Кратность) КАК Стоимость,
	...
	|	СдельныйНарядОперации.Ссылка = &Ссылка
	|	И
	|		(СдельныйНарядОперации.Номенклатура.СчетУчетаЗатрат.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.НезавершенноеПроизводство)
	...
	|	ТаблицаОперации.КоличествоПлан
	|ИЗ
	|	ТаблицаОперации КАК ТаблицаОперации
	|ГДЕ
	|	&ИспользоватьЭтапыПроизводства
	...
	|ГДЕ
	|	(ЭтапыПроизводства.Заказ, ЭтапыПроизводства.Номенклатура, ЭтапыПроизводства.Характеристика,
	|		ЭтапыПроизводства.Спецификация, ЭтапыПроизводства.Партия, ЭтапыПроизводства.Этап) В
	|		(ВЫБРАТЬ
	...
	|	ЭтапыПредварительно.КоличествоФакт КАК КоличествоПлан,
	|	ЭтапыПредварительно.КоличествоФакт КАК КоличествоФакт,
