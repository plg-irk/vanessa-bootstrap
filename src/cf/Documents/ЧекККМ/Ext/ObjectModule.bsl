...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ЗначенияПроверяемыхРеквизитов = Новый Структура("СостояниеЗаказа, Проведен, ОжидаетсяВыборВариантаКП, ВидОперации",
		Выборка.СостояниеЗаказа, Выборка.ОснованиеПроведен, Выборка.ОжидаетсяВыборВариантаКП, Выборка.ВидОперации);
	...
	НалогообложениеДоИзменения = НалогообложениеНДС;
	...
	ИначеЕсли ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") И ЗначениеЗаполнено(СтруктурнаяЕдиница) И НЕ ЗначениеЗаполнено(ТекущаяКассаККМ) Тогда
		...
		Выборка = ПолучитьВыборкуКассККМ(Истина);
		ОбойтиВыборкуКассККМ(Выборка, ЗаказОснование);
		
	ИначеЕсли НЕ ЗначениеЗаполнено(ТекущаяКассаККМ) Тогда
		
		Выборка = ПолучитьВыборкуКассККМ();
		ОбойтиВыборкуКассККМ(Выборка, ЗаказОснование);
		
	КонецЕсли;
	
	Ответственный = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойОтветственный", , ТекущийПользователь);
	...
	Если НалогообложениеНДС <> НалогообложениеДоИзменения Тогда
		НоменклатураВДокументахСервер.ОбновитьСтавкуНДСТабличнойЧасти(ЭтотОбъект, "Запасы", Истина);
	КонецЕсли;
...
Процедура ЗаполнитьДокументПоОтбору(ДанныеЗаполнения) Экспорт
	...
	ИначеЕсли ДанныеЗаполнения.Свойство("ИзКорзины") Тогда
		...
		КассаККМ = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнаяКассаККМ");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты) 
	...
	ЗагрузкаИз1СРМК = ДополнительныеСвойства.Свойство("ОтложенноеПроведение") 
		И ТипКассыККМ = Перечисления.ТипыКассККМ.ККМED;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ТипКассыККМ = Перечисления.ТипыКассККМ.ККМED И Константы.ИспользоватьСерверЛояльностиПоставщикДанных.Получить() Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ТипКассыККМ = Перечисления.ТипыКассККМ.ККМED Или ТипКассыККМ = Перечисления.ТипыКассККМ.ККМOffline Тогда
		...
		Если ЗначениеЗаполнено(ОрганизацияЕГАИС) Тогда
			...
			Документы.ЧекККМ.ПриИзмененииСтатусаДокумента(Ссылка, Перечисления.СтатусыИнформированияЕГАИС.КПередаче,
...
Процедура ВыполнитьПроверкуОграниченийСертификатов(Отказ)
	...
	Сертификаты = БезналичнаяОплата.Выгрузить(
	...
	Сертификаты.Колонки.Добавить("Регистратор", Новый ОписаниеТипов("ДокументСсылка.ЧекККМ"));
...
Процедура ЗаполнитьРеквизитыПечати()
	...
	Подпись = ПерсональныеНастройкиРУНФ.Прочитать("ПодписьКассира", Организация);
	УсловияГТ = ПерсональныеНастройкиРУНФ.Прочитать("УсловияГарантии", Организация);
...
Процедура ПроверитьНачиличеПодарочныхСертификатов()
	...
	Для Каждого СтрокаТаблицы Из Запасы Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если СтрокиОплатыПодарочнымиСертификатами.Количество() > 0
		Или ЕстьПодарочныеСертификаты Тогда
...
Функция ПолучитьВыборкуКассККМ(ИскатьПоСтруктурнойЕдинице = Ложь)
	...
	ОсновнаяКассаККМ = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнаяКассаККМ");
