...
// 
&НаСервере
Процедура ЗаполнитьСтавкуНДСПоОрганизацииНалогообложениеНДС(НалогообложениеПередИзменением = Неопределено, ТолькоИнициализация = Ложь)
...
Процедура ОткрытьКассовуюСмену()
	...
	Если ИспользоватьПодключаемоеОборудование И НЕ ИспользоватьБезПодключенияОборудования И ЗначениеЗаполнено(ИдентификаторУстройства) Тогда
		...
		РеквизитыКассира = ОбщегоНазначенияРМКУНФВызовСервера.ПолучитьРеквизитыКассира(Объект.Автор);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.ЗапускСтарогоРМК.Получить() Тогда
		Константы.ЗапускСтарогоРМК.Установить(Истина);
	КонецЕсли;
	...
	СформироватьПредупреждениеОНовомРМК();
...
Процедура ИнициализацияФормы(пПриСозданииНаСервере = Истина)
	...
	Объект.Ответственный = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойОтветственный", , Пользователь);
...
Процедура ДекорацияПредупреждениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.НастройкаПрограммы.Форма", Новый Структура("СтрокаПоиска", НСтр("ru = 'рмк'")));
...
Процедура СсылкаПомощьНажатие(Элемент)
	...
	Если ОбщегоНазначенияКлиент.РазделениеВключено() Тогда
		...
		ПараметрыОбращения.Тема = НСтр("ru = 'Переход на новое РМК'");
		ПараметрыОбращения.Текст = НСтр("ru = 'Просьба оказать содействие в переходе на новое рабочее место кассира'");
...
&НаКлиенте
Процедура ДекорацияНовоеРМКЗакрытьНажатие(Элемент)
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПереходНаНовоеРМК", "Видимость", Ложь);
...
Процедура ПослеОбработкиШтрихкодов(РезультатОбработкиШтрихкода, КэшированныеЗначения) Экспорт
	...
	Иначе
		...
		Для Каждого СтрокаТЧ Из РезультатОбработкиШтрихкода.ДобавленныеСтроки Цикл
			ИнтеграцияИСМПУНФ.ЗаполнитьСлужебныеРеквизитыИСМПВСтрокеТЧ(СтрокаТЧ, ПараметрыСканирования, Объект.Дата);
...
Процедура СформироватьПредупреждениеОНовомРМК()
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "СсылкаПомощь", "Видимость", ОбщегоНазначения.РазделениеВключено());
	
	Элементы.СтраницыЖурналИБыстрыеТовары.ТекущаяСтраница = Элементы.СтраницаПредупреждение;
	ПереключательЖурналБыстрыеТовары = 0;
...
&НаСервереБезКонтекста
Функция ЕстьОплатаБонусами(Знач БезналичнаяОплата)
