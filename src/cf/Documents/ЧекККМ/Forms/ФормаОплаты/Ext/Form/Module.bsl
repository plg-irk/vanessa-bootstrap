...
Процедура ПолеНомерСертификатаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ДанныеВыбора = СформироватьСписокНомеровСертификатовНаСервере(Текст, Владелец);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСоставЧеков
&НаКлиенте
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыВременнаяТаблицаКарт
&НаКлиенте
...
Процедура ОК(Команда)
	...
	ПроверитьПодарочныйСертификатНаСервере(Отмена);
...
Процедура СчитатьДисконтнуюКартуИДобавитьОплатуЗавершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Параметры.СтруктураПараметров.Вставить("БонуснаяКарта", Результат.ДисконтнаяКарта);
...
Функция СформироватьСписокНомеровСертификатовНаСервере(Знач Текст, Знач Владелец)
	
	Результат = РаботаСПодарочнымиСертификатами.СформироватьСписокНомеровСертификатов(Текст, Владелец);
	Возврат Результат;
...
&НаСервере
Процедура ПроверитьПодарочныйСертификатНаСервере(Отмена)
	
	ПараметрыПроверки = Новый Структура("Запасы, БезналичнаяОплата, Организация", 
		Объект.Запасы, ВременнаяТаблицаКарт, Объект.Организация);

	РаботаСПодарочнымиСертификатами.ПроверитьОграниченийСертификатовВРМК(ПараметрыПроверки, Отмена);
...
Процедура ДобавитьОплатуКартойЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ОборудованиеДополнительныеПараметры = МенеджерОборудованияКлиентСервер.ДополнительныеПараметрыОперации(Истина);
		
		Оповещение = Новый ОписаниеОповещения("ОперацияПоПлатежнойКартеЗавершение", ЭтотОбъект, ДополнительныеПараметры);

		Если ЭтаФорма.ВладелецФормы <> Неопределено Тогда 
			УникальныйИдентификаторЭТ = ЭтаФорма.ВладелецФормы.УникальныйИдентификатор;
		Иначе
			УникальныйИдентификаторЭТ = УникальныйИдентификатор;
		КонецЕсли;
		
		ОборудованиеПлатежныеСистемыКлиент.НачатьВыполнениеОперацииНаЭквайринговомТерминале(
			Оповещение,
			УникальныйИдентификаторЭТ,
			ИдентификаторУстройстваЭТ,
			ПараметрыОперации,
			ОборудованиеДополнительныеПараметры,
			ИдентификаторУстройстваФР);
