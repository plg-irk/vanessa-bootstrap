...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСписаниеЗапасов, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ПересортицаЗапасовЗапасы.Ссылка.ПриходоватьТоварыПоСебестоимостиСписания КАК
	|		ПриходоватьТоварыПоСебестоимостиСписания,
	|	ЕСТЬNULL(ПолитикиУчетаСерийРасход.ПолитикаУчетаСерий, ВЫБОР
	|		КОГДА ПересортицаЗапасовЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|			ТОГДА ПересортицаЗапасовЗапасы.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|		ИНАЧЕ ПересортицаЗапасовЗапасы.Номенклатура.ПолитикаУчетаСерий
	...
	|		КОГДА
	|			ПересортицаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	...
	|	ПересортицаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом) КАК
	|		УчетПоСкладуВЦелом,
	|	ПересортицаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам) КАК
	...
	|		ПО ВЫБОР
	|			КОГДА ПересортицаЗапасовЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|				ТОГДА ПересортицаЗапасовЗапасы.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерийРасход.Владелец
	|			ИНАЧЕ ПересортицаЗапасовЗапасы.Номенклатура = ПолитикиУчетаСерийРасход.Владелец
	|		КОНЕЦ
	...
	|		КОГДА ПересортицаЗапасовЗапасы.НоменклатураОприходование.ИспользоватьСерииИзКатегории
	|			ТОГДА ПересортицаЗапасовЗапасы.НоменклатураОприходование.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|		ИНАЧЕ ПересортицаЗапасовЗапасы.НоменклатураОприходование.ПолитикаУчетаСерий
	|	КОНЕЦ),
	...
	|		ПО ВЫБОР
	|			КОГДА ПересортицаЗапасовЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|				ТОГДА ПересортицаЗапасовЗапасы.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерийПриход.Владелец
	|			ИНАЧЕ ПересортицаЗапасовЗапасы.Номенклатура = ПолитикиУчетаСерийПриход.Владелец
	|		КОНЕЦ
	...
	|	СУММА(ТаблицаЗапасы.СуммаОприходование) КАК СуммаОприходование,
	...
	|		ИНАЧЕ ВЫБОР
	|			КОГДА
	|				ТаблицаЗапасы.ПолитикаУчетаСерий.ТипПолитики = ЗНАЧЕНИЕ(Перечисление.ТипыПолитикУказанияСерий.УправлениеОстаткамиСерий)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ
	...
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПересортицаЗапасов.СерииНоменклатурыОприходование КАК
	|			ПересортицаЗапасовСерииНоменклатурыОприходование
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ПересортицаЗапасов";
