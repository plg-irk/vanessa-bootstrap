...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("БезОплаты", НСтр("ru = 'Без оплаты'"));
	Список.Параметры.УстановитьЗначениеПараметра("ОплаченПолностью", НСтр("ru = 'Оплачен полностью'"));
	Список.Параметры.УстановитьЗначениеПараметра("ОплаченЧастично", НСтр("ru = 'Оплачен частично'"));
	...
	ДвижениеДенежныхСредствСервер.СформироватьСписокКомандСозданияДокументов(ЭтотОбъект,
		"ПодменюСоздатьРасходДСПлан", "РасходДСПлан");
	...
	УстановитьУсловноеОформлениеВидаОперации();
...
Процедура ОтборВидОперацииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	// Возможность выбрать пустое значение?   
	// 	Элементы.ОтборВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийРасходДСПлан.ПустаяСсылка(), "Не выбрано");
...
Процедура СвернутьОтборыНажатие(Элемент)
	СвернутьРазвернутьПанельОтборов(Элемент);
КонецПроцедуры
...
&НаКлиенте
Процедура СвернутьОтборыРегулярныеПлатежиНажатие(Элемент)
	СвернутьРазвернутьПанельОтборовРегулярныеПлатежи(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияРазвернутьОтборыРегулярныеПлатежиНажатие(Элемент)
	СвернутьРазвернутьПанельОтборовРегулярныеПлатежи(Элемент);
КонецПроцедуры
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокРегулярныеПлатежи

&НаКлиенте
Процедура РегулярныеПлатежиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СоздатьЗаявкуНаКлиенте();
КонецПроцедуры
...
Процедура СброситьВсеМеткиОтбораНаСервере(ИмяРеквизитаСписка, ИмяТЧДанныеМеток, СтруктураИменЭлементов,
		ИмяТЧДанныеОтборов)
	РаботаСОтборами.УдалитьМеткиОтбораСервер(ЭтотОбъект, ЭтотОбъект[ИмяРеквизитаСписка], , ИмяРеквизитаСписка,
...
Процедура СохранитьНастройкиОтборовРегулярныеПлатежи()
	...
	СтруктураИменЭлементов.Вставить("ПраваяПанель", "ПраваяПанельРегулярныеПлатежи");
	СтруктураИменЭлементов.Вставить("ОтборПериод", "ОтборПериодРегулярныеПлатежи");
	СтруктураИменЭлементов.Вставить("ИмяПоляОтбора", "");
...
Процедура ЗаполнитьНастройкиДинамическогоСписка(Знач ИмяТаблицы, Знач ИмяКоманды)
	...
	Если ПустаяСтрока(Заголовок) Тогда
		ЗаголовокФормы = НСтр("ru = 'Заявка на расход денег'");
	Иначе
	...
	ПараметрыОтчета.ДополнительныеПоля = СписокДополнительныхПолей();
...
&НаСервере
Функция СписокДополнительныхПолей()
	
	ДополнительныеПоля = Новый Массив;
	
	ДополнительныеПоля.Добавить("ВидОперации");
	ДополнительныеПоля.Добавить("Комментарий");
...
&НаКлиенте
Процедура СоздатьЗаявкуНаКлиенте()
	Если Элементы.СписокРегулярныеПлатежи.ТекущиеДанные = Неопределено Тогда
	...
	Если МассивИсходныхДокументов.Количество() = 1 Тогда
		...
		ОткрытьФорму("Документ.РасходДСПлан.ФормаОбъекта", ПараметрыОткрытия, ЭтотОбъект, , , , ОповещениеОЗакрытии);
...
&НаСервере
Процедура УстановитьУсловноеОформлениеВидаОперации()
	...
	Оформление.Значение 		= НСтр("ru = '<Не выбрана>'");
	Оформление.Использование 	= Истина;
	
	Оформление = НовоеУсловноеОформление.Оформление.Элементы.Найти("ЦветТекста");
	Оформление.Значение 		= ЦветаСтиля.ЦветНедоступногоТекста;
	Оформление.Использование 	= Истина;
	
	Отбор = НовоеУсловноеОформление.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ВидСравнения 		= ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Отбор.Использование 	= Истина; 
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("ВидОперации");
