...
Процедура ЗаполнитьПоСчетуПоЗаказу(ДанныеЗаполнения) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти Из ДанныеЗаполнения.Запасы Цикл
		...
		ИначеЕсли СтрокаТабличнойЧасти.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
			...
			СчетУчетаЗатрат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрока.Номенклатура, "СчетУчетаЗатрат");
			ТипСчетаРасходы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетУчетаЗатрат, "ТипСчета");
			...
			Иначе
				...
				Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
					НоваяСтрока.СтруктурнаяЕдиница = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ДанныеЗаполнения, "СтруктурнаяЕдиницаПродажи");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ВключатьРасходыВСебестоимость Тогда
		...
		Для каждого СтрокаРасходы Из Расходы Цикл
			...
			СчетУчетаЗатрат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаРасходы.Номенклатура, "СчетУчетаЗатрат");
			...
			Если Константы.ФункциональнаяОпцияУчетПоНесколькимПодразделениям.Получить()
					...
					ИЛИ ТипСчета = Перечисления.ТипыСчетов.Доходы
