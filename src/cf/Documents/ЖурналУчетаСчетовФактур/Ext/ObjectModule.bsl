...
//  - Массив из Структура - Пустой если не удалось сформировать файл выгрузки. Ключи структуры:
...
Функция СведенияОРНПТДляЗаполненияСтрокиЖурнала()
	...
	СвойстваПрослеживаемости = "РегНомПросл, КолТовПросл, СтоимТовПросл, ОКЕИ";
		...
		Пока ВыборкаПоДокументам.Следующий() Цикл
			...
			Пока ВыборкаПоРНПТ.Следующий() Цикл 
				...
				Если ЗначениеЗаполнено(ВыборкаПоРНПТ.РегНомПросл) Тогда
					...
					СтруктураСвРегНом = Новый Структура(СвойстваПрослеживаемости);
...
Функция ЭлектронноеПредставлениеЖурналаСчетовФактур_505(ВыгружаемыйДокумент)
	...
	Если СведенияДокумента <> Неопределено Тогда
		...
		Исключение
			ИмяПроцедуры = НСтр("ru = 'ЖурналУчетаСчетовФактур.ЭлектронноеПредставлениеЖурналаСчетовФактур_505'");
			ЗаписьЖурналаРегистрации(ИмяПроцедуры, УровеньЖурналаРегистрации.Ошибка, , ,
...
Функция ОсновныеСведенияЭлектронногоПредставления(ВыгружаемыйДокумент, КНД)
	...
	Если ЭтоПБОЮЛ Тогда
		...
		ФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыгружаемыйДокумент.Организация, "ФизическоеЛицо");
		ФИО = ФИОФизЛица(ФизЛицо, ВыгружаемыйДокумент.Дата);
	...
	Если ЗначениеЗаполнено(ВыгружаемыйДокумент.ИФНС) Тогда
		КодНО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыгружаемыйДокумент.ИФНС, "Код");
...
Процедура ДобавитьСведенияОПодписанте(ОсновныеСведения, ВыгружаемыйДокумент)
	...
	Иначе
		ПодписьРуководителя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыгружаемыйДокумент.Организация,
			"ПодписьРуководителя");
	КонецЕсли;
	
	Подписант = Неопределено;
	Если ЗначениеЗаполнено(ПодписьРуководителя) Тогда
		Подписант = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПодписьРуководителя, "ФизическоеЛицо");
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Подписант) Тогда
		...
		Если ДанныеОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
			Подписант = ДанныеОрганизации.ФизическоеЛицо;
		КонецЕсли;
	КонецЕсли;
