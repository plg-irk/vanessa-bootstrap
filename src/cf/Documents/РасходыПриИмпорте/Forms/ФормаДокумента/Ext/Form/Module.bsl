...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Установка видимости реквизитов НДС.
	УстановитьВидимостьРеквизитовНДС();	
	...
	Если ИмпортПодакцизныхТоваров Тогда
		ВидыПроизведенныхПодакцизныхТоваров.ЗагрузитьЗначения(УчетАкцизовУНФ.СписокИмпортируемыхПодакцизныхТоваров(Объект.Организация));
...
Процедура КонтрагентАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ПараметрыПолученияДанных.Вставить("ВидОперации", "РасходыПриИмпорте");
...
Процедура КонтрагентОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	ПараметрыОбъекта = Новый Структура("ВидОперации", "РасходыПриИмпорте");
...
Процедура ДоговорПриИзменении(Элемент)
	...
	Иначе
		...
		Объект.ЗаказПоставщику = Заказ;	  // Восстанавливаем автоматически очищенный Заказ.
...
Процедура ЗапасыНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество)
	...
	Если ЕстьУчетПодакцизныхТоваров Тогда
		СтрокаЗапасы.ПодакцизныйТовар       = СтруктураДанные.ПодакцизныйТовар;
		СтрокаЗапасы.СуммаАкциза            = СтруктураДанные.СуммаАкциза;
		СтрокаЗапасы.КоэффициентПересчета   = СтруктураДанные.КоэффициентПересчета;
		СтрокаЗапасы.СтавкаАкциза           = СтруктураДанные.СтавкаАкциза;
		СтрокаЗапасы.ОкруглятьДоЦелого      = СтруктураДанные.ОкруглятьДоЦелого;
		СтрокаЗапасы.ЕдиницаИзмеренияАкциза = СтруктураДанные.ЕдиницаИзмеренияАкциза;
		УчетАкцизовУНФКлиентСервер.РассчитатьСуммуАкцизаВТабЧасти(СтрокаЗапасы,, Истина);
	Иначе
		СтрокаЗапасы.ПодакцизныйТовар       = Ложь;
		СтрокаЗапасы.СуммаАкциза            = 0;
		СтрокаЗапасы.КоэффициентПересчета   = 0;
		СтрокаЗапасы.СтавкаАкциза           = 0;
		СтрокаЗапасы.ОкруглятьДоЦелого      = 0;
		СтрокаЗапасы.ЕдиницаИзмеренияАкциза = "";
	КонецЕсли;
	// КонецАкцизы
...
Процедура ЗапасыКоличествоПриИзменении(Элемент)
	...
	Если ЕстьУчетПодакцизныхТоваров Тогда
		УчетАкцизовУНФКлиентСервер.РассчитатьСуммуАкцизаВТабЧасти(СтрокаТабличнойЧасти,, Истина);
	КонецЕсли;
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	Если СтруктураДанные.Свойство("ЕстьУчетПодакцизныхТоваров") И СтруктураДанные.ЕстьУчетПодакцизныхТоваров Тогда
		...
		Если ЗначениеЗаполнено(КодВидаПодакцизногоТовара) Тогда
			...
			Если СтруктураДанные.ПодакцизныйТовар Тогда
...
&НаСервере
Процедура УстановитьВидимостьРеквизитовНДС()
	...
	ПлательщикНДС = УчетнаяПолитика.ПлательщикНДС(
        Объект.Организация,
		Объект.Дата) Или ПлательщикНДСНаУСНСВычетами;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НДС", "Видимость",
		ПлательщикНДС);
...
Процедура ОбработатьВставленныеСтрокиНаСервере(НомерСтрокиРаздела, ИмяТЧ, КоличествоВставленных)
	...
	Для Итератор = 1 По КоличествоВставленных Цикл
		...
		Если ЕстьУчетПодакцизныхТоваров Тогда
			...
			УчетАкцизовУНФКлиентСервер.РассчитатьСуммуАкцизаВТабЧасти(Строка,, Истина);
		Иначе
...
Процедура РазделыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование Тогда
		СтрокаРодитель = Элементы.Разделы.ТекущиеДанные;
