...
Функция ПолучитьВходНомер(ДокументСсылка, РасшифровкаАО = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(НомерВходящегоДокумента) Тогда
		Номер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Номер");
		НомерВходящегоДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Номер, Истина, Истина);
	КонецЕсли;
...
Функция ОписаниеРасчетногоДокументаКонтрагента(ДокументСсылка, Знач ОписаниеДокументаОрганизации, Знач НомерДокумента,
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПриходнаяНакладная")
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратКомитенту
	...
	Если ОрганизациюВПредставление Тогда
		...
		УказатьПредставлениеОрганизации = НСтр("ru = ' ('") + НаименованиеПолное + НСтр("ru = ')'");
...
//
Процедура НачальноеСальдоПоДоговорам(Организация, Знач НачалоПериода, ДоговорыКонтрагентов, ВалютаДокумента,
	СальдоНачалоПериода, ДанныеПоДоговорам = Неопределено) Экспорт
	...
	Если ДанныеПоДоговорам = Неопределено Тогда
		СтрокиПоДоговорам = ДанныеПоДоговорам(Организация, НачалоПериода, МассивДоговоров);
	Иначе
		// Проектное решение: при формировании ПФ одного документа запрос в цикле исключен
		//@skip-check bsl-legacy-check-expression-type
		Если ТипЗнч(ДоговорыКонтрагентов) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
			СтрокиПоДоговорам = ДанныеПоДоговорам.НайтиСтроки(Новый Структура("Договор", ДоговорыКонтрагентов));
		Иначе
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого Выборка Из СтрокиПоДоговорам Цикл 
...
Функция СформироватьПечатнуюФормуАктБезРасхождений(ОписаниеПечатнойФормы, ДокументПечати, ОбъектыПечати, ИспользоватьФаксимиле)
	...
	ТекстСообщенияМакетИзменен = ПечатьДокументовУНФ.ТекстСообщенияМакетИзменен(НСтр(
		"ru = '""Акт сверки взаиморасчетов (сверено)""'"));
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		ДанныеПоДоговорам = ДанныеПоДоговорам(Шапка.Организация, Шапка.НачалоПериода, МассивДоговоров);
		...
		ПараметрыМакета.Вставить("ПредставлениеОрганизации", НСтр("ru ='1. '") + ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечати, ИНН, ЮридическийАдрес, Телефоны, Факс"));
		...
		Если Макет.Области.Найти("Договор") = Неопределено Тогда
			...
			ТабличныйДокумент.Область("R1C1").Текст = ТекстСообщенияМакетИзменен;
		...
		ПередаваемыеПараметры.Вставить("ДанныеПоДоговорам", ДанныеПоДоговорам);
		...
		Если Шапка.СортироватьПоДоговорам Тогда
			...
			Для Каждого СтрокаДанных Из ТаблицаДоговоров Цикл
				...
				НачальноеСальдоПоДоговорам(Шапка.Организация, Шапка.НачалоПериода, Договор, Шапка.ВалютаДокумента,
					НачальноеСальдоПоДоговору, ДанныеПоДоговорам);
		...
		ОбластьПодписиБезФаксимиле = ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "Подписи", "", Ошибки);
		
		Если ОбластьПодписиСФаксимиле <> Неопределено Тогда
			
			ПараметрыМакета.Очистить();
			
			ПараметрыМакета.Вставить("ПредставлениеОрганизации", ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечати, ИНН, ЮридическийАдрес"));
			...
			ЕстьФаксимиле = Ложь;
			Если ЗначениеЗаполнено(Шапка.ФаксимилеПодписиответственного)
				Или ЗначениеЗаполнено(Шапка.ФаксимилеПечати) Тогда
			...
			Если Не ЕстьФаксимиле И МакетИзменен Тогда
				ОбластьПодписиБезФаксимиле.Параметры.Заполнить(ПараметрыМакета);
			...
			Иначе
				ОбластьПодписиСФаксимиле.Параметры.Заполнить(ПараметрыМакета);
...
Процедура СформироватьПечатнуюФормуАктБезРасхожденийФрагмент(ПередаваемыеПараметры)
	...
	ДанныеПоДоговорам = ПередаваемыеПараметры.ДанныеПоДоговорам;
...
Функция СформироватьПечатнуюФормуАктСДаннымиКонтрагента(ОписаниеПечатнойФормы, ДокументПечати, ОбъектыПечати, ИспользоватьФаксимиле)
	...
	ТекстСообщенияМакетИзменен = ПечатьДокументовУНФ.ТекстСообщенияМакетИзменен(НСтр(
		"ru = '""Акт сверки взаиморасчетов (на сверке)""'"));
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		ЗаполнениеСтороныКонтрагентаВручную = ДанныеКонтрагента.Количество() = 0;
			...
			Если Шапка.СортироватьПоДоговорам Тогда
				...
				Если СтрокаДанных.Договор <> Договор Тогда
					...
					Если НЕ Договор.Пустая() Тогда
						...
						ОбластьМакетаДоговорИтоги.Параметры.ДолгОрганизацииДоговорКонец = Макс(ОстатокНаКонец, 0);
						ОбластьМакетаДоговорИтоги.Параметры.ДолгКонтрагентаДоговорКонец = Макс(-ОстатокНаКонец, 0);
						Если Не ЗаполнениеСтороныКонтрагентаВручную Тогда
							ОбластьМакетаДоговорИтоги.Параметры.ДолгОрганизацииПоДаннымКонтрагентаДоговорКонец =
								Макс(ОстатокНаКонецК, 0);
							ОбластьМакетаДоговорИтоги.Параметры.ДолгКонтрагентаПоДаннымКонтрагентаДоговорКонец =
								Макс(-ОстатокНаКонецК, 0);
						КонецЕсли;
		...
		Если Шапка.СортироватьПоДоговорам Тогда
			...
			Для Каждого СтрокаДанных Из ТаблицаДоговоров Цикл
				...
				ОбластьМакетаДоговорИтоги.Параметры.СуммаДолгОрганизации = 0;
				ОбластьМакетаДоговорИтоги.Параметры.СуммаДолгКонтрагента = 0;
				ОбластьМакетаДоговорИтоги.Параметры.КонтрагентСуммаДолгОрганизации = 0;
				ОбластьМакетаДоговорИтоги.Параметры.КонтрагентСуммаДолгКонтрагента = 0;
				ОбластьМакетаДоговорИтоги.Параметры.ДолгОрганизацииДоговорКонец = Макс(ОстатокНаКонец, 0);
				ОбластьМакетаДоговорИтоги.Параметры.ДолгКонтрагентаДоговорКонец = Макс(-ОстатокНаКонец, 0);
				ОбластьМакетаДоговорИтоги.Параметры.ДолгОрганизацииПоДаннымКонтрагентаДоговорКонец = Макс(-ОстатокНаКонец, 0);
				ОбластьМакетаДоговорИтоги.Параметры.ДолгКонтрагентаПоДаннымКонтрагентаДоговорКонец = Макс(ОстатокНаКонец, 0);
		...
		Если ОстатокНаКонец = 0 Тогда			
			ЗаключениеОрганизация = НСтр("ru='отсутствует'");			
		Иначе
			СуммаПоМодулю = Макс(ОстатокНаКонец, -ОстатокНаКонец);
	
			ШаблонЗаключениеОрганизация =
				НСтр("ru='в пользу %1 составила %2 %3 (%4).'");

			ЗаключениеОрганизация = СтрШаблон(ШаблонЗаключениеОрганизация,
				ПредставлениеДебитора,
				Формат(СуммаПоМодулю, "ЧЦ=15; ЧДЦ=2; ЧГ=3,0"),
				Строка(Шапка.ВалютаДокумента),
				РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаПоМодулю, Шапка.ВалютаДокумента));
		КонецЕсли;
		
		ПараметрыМакета.Вставить("ЗаключениеОрганизация", ЗаключениеОрганизация);
		...
		Если ЗаполнениеСтороныКонтрагентаВручную Тогда
			ПоДаннымКонтрагентаПодвал = "";			
		Иначе
			
			ШаблонПоДаннымКонтрагентаПодвал = НСтр("ru='на %1 задолженность %2'");
				
			Если ОстатокНаКонецК = 0 Тогда			
				ЗаключениеКонтрагент = НСтр("ru='отсутствует.'");			
			Иначе
				СуммаПоМодулю = Макс(ОстатокНаКонецК, -ОстатокНаКонецК);
		
				ШаблонЗаключениеКонтрагент =
					НСтр("ru='в пользу %1 составила %2 %3 (%4).'");
	
				ЗаключениеКонтрагент = СтрШаблон(ШаблонЗаключениеКонтрагент,
					ПредставлениеДебитора,
					Формат(СуммаПоМодулю, "ЧЦ=15; ЧДЦ=2; ЧГ=3,0"),
					Строка(Шапка.ВалютаДокумента),
					РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаПоМодулю, Шапка.ВалютаДокумента));
			...
			ПоДаннымКонтрагентаПодвал = СтрШаблон(ШаблонПоДаннымКонтрагентаПодвал,
				Формат(Шапка.КонецПериода, "ДЛФ=DD"),
				ЗаключениеКонтрагент);
		КонецЕсли;
		
		ПараметрыМакета.Вставить("ПоДаннымКонтрагентаПодвал", ПоДаннымКонтрагентаПодвал);
		
		Если Найти(ЗначениеВСтрокуВнутр(ОбластьМакета), "ПоДаннымКонтрагентаПодвал") = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщенияМакетИзменен);	
		КонецЕсли;	
...
Функция ДанныеПоДоговорам(Организация, НачалоПериода, МассивДоговоров)
