...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ИнвентаризацияЗапасов";
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "КоличествоУчет", НСтр(
		"ru = 'Количество учет'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_3, , ,
		Истина);

	ОписаниеТиповКолонка = Новый ОписаниеТипов(
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СуммаУчет", 
		НСтр("ru = 'СуммаУчет'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, 
		ОписанияТиповПолей.ОписаниеТиповЧисло15_2, , , Ложь); 
		
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Излишки", 
		НСтр("ru = 'Излишки'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, 
		ОписанияТиповПолей.ОписаниеТиповЧисло15_2, , , Ложь);

	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Недостача", 
		НСтр("ru = 'Недостача'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, 
		ОписанияТиповПолей.ОписаниеТиповЧисло15_2, , , Ложь);
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ПараметрыСопоставления, АдресРезультата) Экспорт
	...
	ЕстьКолонкаСтранаПроисхождения = Ложь;
	
	Если ТаблицаСопоставленияДанных.Количество() > 0 Тогда
		
		ПерваяСтрокаТаблицы = ТаблицаСопоставленияДанных[0]; 
				
		ЕстьКолонкаСуммаУчет = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ПерваяСтрокаТаблицы, "СуммаУчет");  
		
		ЕстьКолонкаКоличествоУчет = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ПерваяСтрокаТаблицы, "КоличествоУчет"); 
					
		ЕстьКолонкаСтранаПроисхождения = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ПерваяСтрокаТаблицы, "СтранаПроисхождения");
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		Если ЕстьКолонкаКоличествоУчет Тогда
			ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.КоличествоУчет, 
				СтрокаТаблицыФормы.КоличествоУчет_ВходящиеДанные, 1);    
		КонецЕсли;
		...
		Если ЕстьКолонкаСуммаУчет Тогда
			ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.СуммаУчет, 
				СтрокаТаблицыФормы.СуммаУчет_ВходящиеДанные, 0); 
		КонецЕсли;
		...
		Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
			...
			Если ЕстьКолонкаСтранаПроисхождения Тогда
...
Процедура ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения = "") Экспорт
	...
	Если ПолноеИмяОбъектаЗаполнения = "Документ.ИнвентаризацияЗапасов.ТабличнаяЧасть.Запасы" Тогда
		...
		Если НоменклатураЗаполнена И СтрокаТаблицыФормы.Номенклатура.ТипНоменклатуры
			...
			СтрокаТаблицыФормы[ИмяСлужебногоПоля] = СтрокаТаблицыФормы.Номенклатура.ТипНоменклатуры
				= Перечисления.ТипыНоменклатуры.Запас И Не СтрокаТаблицыФормы.Номенклатура.ЭтоНабор // Исключая наборы
...
Функция ТекстЗапросаУниверсальный()
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА &ВалютаНациональная = &ВалютаУчета
	|			ТОГДА Запасы.Цена
	|		ИНАЧЕ Запасы.Цена * КурсВалютыУправленческогоУчета.Курс / КурсВалютыУправленческогоУчета.Кратность
	|	КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Цена,
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА &ВалютаНациональная = &ВалютаУчета
	|			ТОГДА Запасы.Сумма
	|		ИНАЧЕ Запасы.Сумма * КурсВалютыУправленческогоУчета.Курс / КурсВалютыУправленческогоУчета.Кратность
	|	КОНЕЦ КАК ЧИСЛО(15, 2)) КАК ФактСумма,
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА &ВалютаНациональная = &ВалютаУчета
	|			ТОГДА Запасы.СуммаУчет
	|		ИНАЧЕ Запасы.СуммаУчет * КурсВалютыУправленческогоУчета.Курс / КурсВалютыУправленческогоУчета.Кратность
	|	КОНЕЦ КАК ЧИСЛО(15, 2)) КАК БухСумма,
	|	Цена КАК ЦенаБезПересчета,
	|	Сумма КАК СуммаБезПересчета,
	|	СуммаУчет КАК СуммаПоУчету
	|ИЗ
	|	Документ.ИнвентаризацияЗапасов.Запасы КАК Запасы,
	|	РегистрСведений.КурсыВалют.СрезПоследних(&Дата, Валюта = &ВалютаУчета) КАК КурсВалютыУправленческогоУчета
	|ГДЕ
	|	Запасы.Ссылка = &ТекущийДокумент
	|
	|УПОРЯДОЧИТЬ ПО
	|	Запасы.НомерСтроки
	|;
	...
	|ИЗ
	|	Справочник.Комиссии.ПодписиКомиссии КАК ПодписиКомиссии
	|ГДЕ
	|	ПодписиКомиссии.Ссылка = &Комиссия
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПодписиКомиссии.ЭтоПодписьПредседателяКомиссии УБЫВ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	Документ.ИнвентаризацияЗапасов.СерииНоменклатуры КАК СерииНоменклатуры
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИнвентаризацияЗапасовЗапасы.НомерСтроки КАК НомерСтроки,
	|	ЕСТЬNULL(ШтрихкодыНоменклатуры.Штрихкод, ИнвентаризацияЗапасовЗапасы.Номенклатура.Штрихкод) КАК Штрихкод
	|ИЗ
	|	Документ.ИнвентаризацияЗапасов.Запасы КАК ИнвентаризацияЗапасовЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	|		ПО ИнвентаризацияЗапасовЗапасы.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	|		И ИнвентаризацияЗапасовЗапасы.Характеристика = ШтрихкодыНоменклатуры.Характеристика
	|		И ИнвентаризацияЗапасовЗапасы.Партия = ШтрихкодыНоменклатуры.Партия
	|ГДЕ
	|	ИнвентаризацияЗапасовЗапасы.Ссылка = &ТекущийДокумент
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки,
	|	Штрихкод";
...
Функция ДанныеБланкаТоварногоНаполнения(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.УстановитьПараметр("ПредставлениеРегистратора", НСтр("ru = 'Инвентаризация запасов'"));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ИнвентаризацияЗапасов.Ссылка КАК Ссылка,
	|	ИнвентаризацияЗапасов.Дата КАК ДатаДокумента,
	...
	|ВЫБРАТЬ
	|	ИнвентаризацияЗапасовЗапасы.НомерСтроки КАК НомерСтроки,
	|	ИнвентаризацияЗапасовЗапасы.Ссылка КАК Ссылка,
	|	ЕСТЬNULL(ШтрихкодыНоменклатуры.Штрихкод, ИнвентаризацияЗапасовЗапасы.Номенклатура.Штрихкод) КАК Штрихкод
	|ИЗ
	|	Документ.ИнвентаризацияЗапасов.Запасы КАК ИнвентаризацияЗапасовЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	...
	|		И ИнвентаризацияЗапасовЗапасы.Партия = ШтрихкодыНоменклатуры.Партия
	|ГДЕ
	|	ИнвентаризацияЗапасовЗапасы.Ссылка В (&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	НомерСтроки,
...
Процедура СформироватьИНВ3(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати, ЗаполнятьФактическиеДанные)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ВыборкаКомиссия		= РезультатыЗапроса[2].Выбрать();
		
		ШтрихкодыНоменклатуры = РезультатыЗапроса[4].Выгрузить();
		ШтрихкодыНоменклатуры.Индексы.Добавить("НомерСтроки");
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			ПараметрыПоискаШтрихкодыНоменклатуры.НомерСтроки = ВыборкаСтрокТовары.Номер;
			НайденныеСтроки = ШтрихкодыНоменклатуры.НайтиСтроки(ПараметрыПоискаШтрихкодыНоменклатуры);
