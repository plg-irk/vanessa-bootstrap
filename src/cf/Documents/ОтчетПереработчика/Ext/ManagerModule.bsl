...
//
// Параметры:
// 	ТекущийДокумент - ДокументСсылка.ОтчетПереработчика - Документ, ставку по которому надо проверить
...
Функция НаличиеСтавкиУСН(ТекущийДокумент, ВидСтавкиУСН) Экспорт
	...
	|		ПО ОтчетПереработчика.СтавкаНДС = СтавкиНДС.Ссылка
	|			И (ОтчетПереработчика.Ссылка = &СсылкаНаДокумент)
...
Функция НаличиеСуммыНДС(ТекущийДокумент) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|    СУММА(ОтчетПереработчикаУслуги.СуммаНДС) КАК СуммаНДС,
	|    ОтчетПереработчикаУслуги.Ссылка КАК Ссылка
	|ИЗ
	|    Документ.ОтчетПереработчика.Расходы КАК ОтчетПереработчикаУслуги
	|ГДЕ
	|    ОтчетПереработчикаУслуги.Ссылка = &СсылкаНаДокумент
	|СГРУППИРОВАТЬ ПО
	|    ОтчетПереработчикаУслуги.Ссылка";
...
//
// Параметры:
//  ДокументСсылкаОтчетПереработчика - ДокументСсылка.ОтчетПереработчика - документ-источник движений
//  СтруктураДополнительныеСвойства	 - Структура - свойство объекта ДополнительныеСвойства
//
Процедура СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства) Экспорт
	...
	УчетПартийУСН.СоздатьВТДанныеПервичныхДокументов(Запрос.МенеджерВременныхТаблиц,
		Метаданные.Документы.ДополнительныеРасходы.ТабличныеЧасти.Предоплата.Реквизиты.Документ.Тип.Типы(), "ВТДокументыКУДиР");
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВременнаяТаблицаРасчетыСПоставщиками.Контрагент.НаименованиеПолное КАК ЮридическоеНазваниеКонтрагента,
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	1 КАК Количество,
	               |	ОтчетПереработчика.Всего КАК Сумма,
	               |	ОтчетПереработчика.СуммаНДС КАК СуммаНДС,
	               |	ОтчетПереработчика.ВалютаДокумента КАК ВалютаДокумента,
	               |	ВЫБОР
	               |		КОГДА ОтчетПереработчика.Кратность = 0
	               |			ТОГДА ОтчетПереработчика.Курс
	               |		ИНАЧЕ ОтчетПереработчика.Курс / ОтчетПереработчика.Кратность
	               |	КОНЕЦ КАК Курс
	               ...
	               |ВЫБРАТЬ
	               |	&Организация КАК Организация,
	               |	ШапкаДокумента.Ссылка КАК ДокументПартии,
	               |	ШапкаДокумента.Ссылка КАК ДокументРегистрацииВКУДиР,
	               |	&ДатаДокумента КАК Период,
	               |	ШапкаДокумента.Номенклатура КАК Номенклатура,
	               |	1 КАК Количество,
	               |	ШапкаДокумента.СуммаНДС * ДанныеДокументаДляНУ.РасчетКурса КАК СуммаНДС,
	               |	ШапкаДокумента.Сумма * ДанныеДокументаДляНУ.РасчетКурса КАК Сумма,
	               |	1 КАК НомерСтрокиДокумента
	               |ИЗ
	               |	ШапкаДокумента КАК ШапкаДокумента
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументаДляНУ КАК ДанныеДокументаДляНУ
	               |		ПО ШапкаДокумента.Ссылка = ДанныеДокументаДляНУ.Ссылка";
	...
	Если ТаблицаПартииТоваров.Найти(Истина, "ОтражатьВКУДиР") <> Неопределено Тогда
		...
		СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеРасходыНаУслуги();
	КонецЕсли;
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ЕСТЬNULL(ОтчетПереработчика.СтруктурнаяЕдиница, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)) КАК
	|		СтруктурнаяЕдиница,
	|	ЕСТЬNULL(ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)) КАК
	...
	|	ВЫБОР
	|		КОГДА
	|			ОтчетПереработчика.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|			ТОГДА ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗапасов
	|		ИНАЧЕ ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗатрат
	|	КОНЕЦ КАК СчетУчета,
	|	ВЫБОР
	...
	|		КОГДА ОтчетПереработчикаПродукция.Номенклатура.ИспользоватьСерииИзКатегории
	|			ТОГДА ОтчетПереработчикаПродукция.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|		ИНАЧЕ ОтчетПереработчикаПродукция.Номенклатура.ПолитикаУчетаСерий
	|	КОНЕЦ) КАК ПолитикаУчетаСерий,
	|	ВЫБОР
	|		КОГДА
	|			ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА ОтчетПереработчикаПродукция.Ссылка
	|		КОГДА
	|			ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	...
	|	ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом) КАК
	|		УчетПоСкладуВЦелом,
	|	ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам) КАК
	...
	|		ПО ВЫБОР
	|			КОГДА ОтчетПереработчикаПродукция.Номенклатура.ИспользоватьСерииИзКатегории
	|				ТОГДА ОтчетПереработчикаПродукция.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|			ИНАЧЕ ОтчетПереработчикаПродукция.Номенклатура = ПолитикиУчетаСерий.Владелец
	|		КОНЕЦ
	...
	СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ОтчетПереработчика";
