	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Если МассивДанныхШтрихкода <> Неопределено
			...
			Для Каждого ДанныеШтрихкода Из МассивДанныхШтрихкода Цикл 
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтруктураДанные, "СтатусПартии") Тогда
					СтруктураДанныеНоменклатуры.Вставить("СтатусПартии", СтруктураДанные.СтатусПартии);
				КонецЕсли;
...
Процедура ВопросОбОчисткеПредоплатыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Объект.ДокументОснование = Заказ;	  // Восстанавливаем автоматически очищенный Заказ.
	КонецЕсли;
...
Процедура ОбработатьИзменениеДоговораНаСервере(ДанныеКонтрагента = Неопределено)
	...
	Если ДанныеКонтрагента = Неопределено Тогда
		ДанныеДоговора = ДанныеДоговораДляИзменения(Объект.Дата, Объект.Договор);
	Иначе
