...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// Для работы универсальных механизмов, которые требуют наличия реквизита Контрагент. Например, 1С:Сверка 2.0.
	Контрагент = КонтрагентПолучатель;
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказ) Экспорт
	...
	|	ТабличнаяЧастьЗапасы.НоменклатураНабора КАК НоменклатураНабора,
	|	ТабличнаяЧастьЗапасы.ХарактеристикаНабора КАК ХарактеристикаНабора
	|ИЗ
	...
	// Наборы
	ВыборкаНаборы = РезультатыЗапроса[3].Выбрать();
	Пока ВыборкаНаборы.Следующий() Цикл
		НоваяСтрока = ДобавленныеНаборы.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаНаборы);
	КонецЦикла;
