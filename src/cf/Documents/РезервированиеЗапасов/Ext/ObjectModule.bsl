...
Процедура ЗаполнитьПоЗаказПокупателяНовоеМесто(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	ВидОперации = Перечисления.ВидыОперацийРезервированиеЗапасов.СкладРезервирование;
...
Процедура ЗаполнитьПоЗаказПокупателяИсходноеМесто(ДанныеЗаполнения)
	...
	ВидОперации = Перечисления.ВидыОперацийРезервированиеЗапасов.СкладСнятиеСРезерва;
...
Процедура ЗаполнитьПоВидуОперации()
	
	Если Не ЗначениеЗаполнено(ВидОперации) Тогда
		ВидОперации = Перечисления.ВидыОперацийРезервированиеЗапасов.СкладРезервирование;;
	КонецЕсли;
	
	СоответствиеПоВидамОпераций = Документы.РезервированиеЗапасов.МестаРезерваПоВидуОперации();
	СтруктураМестоРезерва = СоответствиеПоВидамОпераций.Получить(ВидОперации);
	
	МестаРезерва = Запасы.Выгрузить(,"ИсходноеМестоРезерва, НовоеМестоРезерва");
	МестаРезерва.Свернуть("ИсходноеМестоРезерва, НовоеМестоРезерва");
	
	Если МестаРезерва.Количество() > 1 Тогда
		ПоложениеМестаРезерва = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти;
	КонецЕсли;
	
	Если ПоложениеМестаРезерва <> Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
		
		ИсходноеМестоРезерва = ЗаполнениеОбъектовУНФ.ЗначениеДляШапки(Запасы, "ИсходноеМестоРезерва");
		НовоеМестоРезерва = ЗаполнениеОбъектовУНФ.ЗначениеДляШапки(Запасы, "НовоеМестоРезерва");
		ИсходноеМестоРезерва = ?(ЗначениеЗаполнено(ИсходноеМестоРезерва), ИсходноеМестоРезерва, СтруктураМестоРезерва.ЗначениеПоУмолчанию);
		НовоеМестоРезерва = ?(ЗначениеЗаполнено(НовоеМестоРезерва), НовоеМестоРезерва, СтруктураМестоРезерва.ЗначениеПоУмолчанию);
	
	Иначе
		
		Для Каждого СтрокаТаблицыЗапасов Из Запасы Цикл
			
			Если Не ЗначениеЗаполнено(СтрокаТаблицыЗапасов.ИсходноеМестоРезерва) Тогда
				СтрокаТаблицыЗапасов.ИсходноеМестоРезерва = СтруктураМестоРезерва.ЗначениеПоУмолчанию;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СтрокаТаблицыЗапасов.НовоеМестоРезерва) Тогда
				СтрокаТаблицыЗапасов.НовоеМестоРезерва = СтруктураМестоРезерва.ЗначениеПоУмолчанию;
			КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПоложениеМестаРезерва = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке Тогда
		
		Если ЗначениеЗаполнено(СтруктураМестоРезерва.ИмяЭлемента) И Не ЗначениеЗаполнено(ЭтотОбъект[СтруктураМестоРезерва.ИмяЭлемента]) Тогда
			
			НеУказаноМестоРезерва = НСтр("ru='Не заполнен %1.'");
			НеУказаноМестоРезерва = СтрШаблон(НеУказаноМестоРезерва, СтруктураМестоРезерва.Представление);
			ОбщегоНазначения.СообщитьПользователю(НеУказаноМестоРезерва, ЭтотОбъект, СтруктураМестоРезерва.ИмяЭлемента, , Отказ);
			
		ИначеЕсли Не (ЗначениеЗаполнено(ИсходноеМестоРезерва) ИЛИ ЗначениеЗаполнено(НовоеМестоРезерва)) Тогда
			
			НеУказаноМестоРезерва = НСтр("ru='Не указано исходное место резервиования'");
			ОбщегоНазначения.СообщитьПользователю(НеУказаноМестоРезерва, ЭтотОбъект, "ИсходноеМестоРезерва", , Отказ);
			НеУказаноМестоРезерва = НСтр("ru='Не указано новое место резервирования'");
			ОбщегоНазначения.СообщитьПользователю(НеУказаноМестоРезерва, ЭтотОбъект, "НовоеМестоРезерва", , Отказ);	
	...
	Иначе
		...
		Для каждого СтрокаТаблицыЗапасов Из Запасы Цикл
			...
			Если ЗначениеЗаполнено(СтруктураМестоРезерва.ИмяЭлемента) И Не ЗначениеЗаполнено(СтрокаТаблицыЗапасов[СтруктураМестоРезерва.ИмяЭлемента]) Тогда
			 	
			 	НеУказаноМестоРезерва = НСтр("ru='Не заполнен %1 в строке %2 списка ""Запасы"".'");
				НеУказаноМестоРезерва = СтрШаблон(НеУказаноМестоРезерва, СтруктураМестоРезерва.Представление, СтрокаТаблицыЗапасов.НомерСтроки);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Запасы", СтрокаТаблицыЗапасов.НомерСтроки, СтруктураМестоРезерва.ИмяЭлемента);
				ОбщегоНазначения.СообщитьПользователю(НеУказаноМестоРезерва, ЭтотОбъект, КонтекстноеПоле, , Отказ);
				
			ИначеЕсли Не (ЗначениеЗаполнено(СтрокаТаблицыЗапасов.ИсходноеМестоРезерва) ИЛИ ЗначениеЗаполнено(СтрокаТаблицыЗапасов.НовоеМестоРезерва)) Тогда
				
				НеУказаноМестоРезерва = НСтр("ru='Не указано исходное место резервирования в строке %1 списка ""Запасы"".'");
				НеУказаноМестоРезерва = СтрШаблон(НеУказаноМестоРезерва, СтрокаТаблицыЗапасов.НомерСтроки);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Запасы", СтрокаТаблицыЗапасов.НомерСтроки, "ИсходноеМестоРезерва");
				ОбщегоНазначения.СообщитьПользователю(НеУказаноМестоРезерва, ЭтотОбъект, КонтекстноеПоле, , Отказ);
				
				НеУказаноМестоРезерва = НСтр("ru='Не указано новое место резервирования в строке %1 списка ""Запасы"".'");
				НеУказаноМестоРезерва = СтрШаблон(НеУказаноМестоРезерва, СтрокаТаблицыЗапасов.НомерСтроки);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Запасы", СтрокаТаблицыЗапасов.НомерСтроки, "НовоеМестоРезерва");
				ОбщегоНазначения.СообщитьПользователю(НеУказаноМестоРезерва, ЭтотОбъект, КонтекстноеПоле, , Отказ);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ПоложениеМестаРезерва <> Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
		ЗаполнениеОбъектовУНФ.ЗаполнитьТаблицуПоШапке(ЭтотОбъект, "Запасы", "ИсходноеМестоРезерва", "ПоложениеМестаРезерва");
		ЗаполнениеОбъектовУНФ.ЗаполнитьТаблицуПоШапке(ЭтотОбъект, "Запасы", "НовоеМестоРезерва", "ПоложениеМестаРезерва");
	КонецЕсли;
