...
&НаКлиенте
Перем МестаРезерваПоВидуОперации;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
		Документы.РезервированиеЗапасов.ПараметрыУказанияСерий(Объект));
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ПоложениеМестаРезерва = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке") Тогда
		Для Каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
			СтрокаТабличнойЧасти.ИсходноеМестоРезерва = Объект.ИсходноеМестоРезерва;
			СтрокаТабличнойЧасти.НовоеМестоРезерва = Объект.НовоеМестоРезерва;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ИсходноеМестоРезерваНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	УстановитьТипЗначенияМестоРезерва(Объект.ИсходноеМестоРезерва);
КонецПроцедуры

&НаКлиенте
Процедура НовоеМестоРезерваНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	УстановитьТипЗначенияМестоРезерва(Объект.НовоеМестоРезерва);
КонецПроцедуры
...
&НаКлиенте
Процедура ЗапасыИсходноеМестоРезерваНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	УстановитьТипЗначенияМестоРезерва(ТекущиеДанные.ИсходноеМестоРезерва);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗапасыНовоеМестоРезерваНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	УстановитьТипЗначенияМестоРезерва(ТекущиеДанные.НовоеМестоРезерва);
...
&НаКлиенте
Процедура УстановитьТипЗначенияМестоРезерва(ТекущееЗначениеМестоРезерва, ЗначениеПоУмолчанию = Неопределено)
	
	Если МестоРезерваПоУмолчанию <> Неопределено И ТипЗнч(ТекущееЗначениеМестоРезерва) <> ТипЗнч(МестоРезерваПоУмолчанию) Тогда
		
		Если МестоРезерваПоУмолчанию <> Неопределено И ТипЗнч(ЗначениеПоУмолчанию) <> ТипЗнч(МестоРезерваПоУмолчанию) Тогда
			
			ТекущееЗначениеМестоРезерва = МестоРезерваПоУмолчанию;
			
		Иначе
			
			ТекущееЗначениеМестоРезерва = ЗначениеПоУмолчанию;
...
Процедура ОткрытьФормуНастройки(ПередаватьВидДокумента)
	...
	Если НЕ ТолькоПросмотр И Не ЗначениеЗаполнено(Объект.ПоложениеМестаРезерва) Тогда
		Объект.ПоложениеМестаРезерва = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти");
	КонецЕсли;
	...
	Если ПередаватьВидДокумента Тогда
		ПараметрыФормы.Вставить("ВидДокумента", НСтр("ru = 'резервирование запасов'"));
	КонецЕсли;
	
	ПараметрыФормы.Вставить("ПоложениеМестаРезерва", Объект.ПоложениеМестаРезерва);
	ПараметрыФормы.Вставить("Доступность", Новый Структура("ПоложениеМестаРезерва", Истина));
	ПараметрыФормы.Вставить("ТолькоПросмотр", ТолькоПросмотр);
	
	ОткрытьФорму("ОбщаяФорма.ШапкаТабличнаяЧасть", ПараметрыФормы,,,,,Новый ОписаниеОповещения("ШапкаТабличнаяЧастьЗавершение", ЭтотОбъект));
...
Процедура ШапкаТабличнаяЧастьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.БылиВнесеныИзменения Тогда
		...
		Если Результат.Свойство("ПоложениеМестаРезерва") И Объект.ПоложениеМестаРезерва <> Результат.ПоложениеМестаРезерва Тогда
			
			Объект.ПоложениеМестаРезерва = Результат.ПоложениеМестаРезерва;
			ЗаполнениеОбъектовУНФКлиент.ОбработатьИзменениеПоложения(Объект, "Запасы", "ИсходноеМестоРезерва", "ПоложениеМестаРезерва");
			ЗаполнениеОбъектовУНФКлиент.ОбработатьИзменениеПоложения(Объект, "Запасы", "НовоеМестоРезерва", "ПоложениеМестаРезерва");
...
&НаСервереБезКонтекста
Функция МестаРезерваПоВидуОперации()
	Возврат Документы.РезервированиеЗапасов.МестаРезерваПоВидуОперации();
КонецФункции
...
Процедура УправлениеФормой()
	...
	Если Не ЗначениеЗаполнено(МестаРезерваПоВидуОперации) Тогда
		МестаРезерваПоВидуОперации = МестаРезерваПоВидуОперации();
	КонецЕсли;
	
	МестоРезерва = МестаРезерваПоВидуОперации.Получить(Объект.ВидОперации);
	МестоРезерваПоУмолчанию = МестоРезерва.ЗначениеПоУмолчанию;
	ВыбиратьТип = МестоРезерваПоУмолчанию = Неопределено;
	ЭтоСнятиеСРезерва = МестоРезерва.ЭтоСнятиеСРезерва;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НовоеМестоРезерва", "Заголовок", МестоРезерва.Представление);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыНовоеМестоРезерва", "Заголовок", МестоРезерва.Представление);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИсходноеМестоРезерва", "Заголовок", МестоРезерва.Представление);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыИсходноеМестоРезерва", "Заголовок", МестоРезерва.Представление);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НовоеМестоРезерва", "ВыбиратьТип", ВыбиратьТип);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыНовоеМестоРезерва", "ВыбиратьТип", ВыбиратьТип);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИсходноеМестоРезерва", "ВыбиратьТип", ВыбиратьТип);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыИсходноеМестоРезерва", "ВыбиратьТип", ВыбиратьТип);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИсходноеМестоРезерва", "Видимость", 
		МестоРезерва.ИмяЭлемента <> "НовоеМестоРезерва" И Объект.ПоложениеМестаРезерва = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке"));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НовоеМестоРезерва", "Видимость", 
		МестоРезерва.ИмяЭлемента <> "ИсходноеМестоРезерва" И Объект.ПоложениеМестаРезерва = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке"));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыИсходноеМестоРезерва", "Видимость", 
		МестоРезерва.ИмяЭлемента <> "НовоеМестоРезерва" И Объект.ПоложениеМестаРезерва <> ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке"));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыНовоеМестоРезерва", "Видимость", 
		МестоРезерва.ИмяЭлемента <> "ИсходноеМестоРезерва" И Объект.ПоложениеМестаРезерва <> ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке"));
...
Процедура ЗаполнитьСписокВыбораВидОперации()
	...
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийРезервированиеЗапасов.ПустаяСсылка(), Нстр("ru = 'Произвольно'"));
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийРезервированиеЗапасов.СкладРезервирование);
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийРезервированиеЗапасов.СкладСнятиеСРезерва);
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуЗакупки") Тогда
		Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийРезервированиеЗапасов.ЗаказПоставщикуРезервирование);
		Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийРезервированиеЗапасов.ЗаказПоставщикуСнятиеСРезерва);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуПроизводство") Тогда
		Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийРезервированиеЗапасов.ЗаказНаПроизводствоРезервирование);
		Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийРезервированиеЗапасов.ЗаказНаПроизводствоСнятиеСРезерва);
	КонецЕсли;
...
Процедура ОбработатьИзменениеВидаОперации()
	...
	УстановитьТипЗначенияМестоРезерва(Объект.ИсходноеМестоРезерва, Объект.НовоеМестоРезерва);
	УстановитьТипЗначенияМестоРезерва(Объект.НовоеМестоРезерва, Объект.ИсходноеМестоРезерва);
		
	Для Каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
		УстановитьТипЗначенияМестоРезерва(СтрокаТабличнойЧасти.ИсходноеМестоРезерва, СтрокаТабличнойЧасти.НовоеМестоРезерва);
		УстановитьТипЗначенияМестоРезерва(СтрокаТабличнойЧасти.НовоеМестоРезерва, СтрокаТабличнойЧасти.ИсходноеМестоРезерва);
	КонецЦикла;
	
	Если ЭтоСнятиеСРезерва И МестоРезерваПоУмолчанию <> Неопределено Тогда
		
		Объект.НовоеМестоРезерва = Неопределено;
		
		Для Каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
			СтрокаТабличнойЧасти.НовоеМестоРезерва = Неопределено;
		КонецЦикла;
		
	ИначеЕсли Не ЭтоСнятиеСРезерва И МестоРезерваПоУмолчанию <> Неопределено Тогда
		
		Объект.ИсходноеМестоРезерва = Неопределено;
		
		Для Каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
			СтрокаТабличнойЧасти.ИсходноеМестоРезерва = Неопределено;
		КонецЦикла;
