...
//  Структура - набор сумм - итогов по документу:
// * СуммаНачислено - Число
// * СуммаУдержано - Число
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	|ГДЕ
	|	Управленческий.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Расходы)";	
	СчетаРасходы = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	ОсновноеНаправлениеДеятельности = Справочники.НаправленияДеятельности.ОсновноеНаправление;
	
	Для Каждого СтрокаНачисленияУдержания Из НачисленияУдержания Цикл
		
		Если СчетаРасходы.Найти(СтрокаНачисленияУдержания.СчетЗатрат) <> Неопределено Тогда
			СтрокаНачисленияУдержания.НаправлениеДеятельности = ОсновноеНаправлениеДеятельности;
		КонецЕсли;	
	...
	Для Каждого СтрокаВзносы Из Взносы Цикл
		...
		Если СчетаРасходы.Найти(СтрокаВзносы.СчетЗатрат) <> Неопределено Тогда
			СтрокаВзносы.НаправлениеДеятельности = ОсновноеНаправлениеДеятельности;
		КонецЕсли;	
