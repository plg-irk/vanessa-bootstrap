...
//  ПеречислениеСсылка.ТипыРасчетаДенежнымиСредствами
// 
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Договор) Тогда
		РегистрироватьЦеныПоставщика = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор,
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя
		...
		И ЗначениеЗаполнено(ДокументОснование) И ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РасходнаяНакладная") Тогда
		ЗаполнитьНомерСтрокиДокументаОснования(Отказ);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ВключатьРасходыВСебестоимость Тогда
		...
		Для каждого СтрокаРасходы Из Расходы Цикл
            ...
            СчетУчетаЗатратТипСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаРасходы.Номенклатура,
				"СчетУчетаЗатрат.ТипСчета");
			...
			Если Константы.ФункциональнаяОпцияУчетПоНесколькимПодразделениям.Получить()
					...
					ИЛИ СчетУчетаЗатратТипСчета = Перечисления.ТипыСчетов.КосвенныеЗатраты
					ИЛИ СчетУчетаЗатратТипСчета = Перечисления.ТипыСчетов.Доходы
					ИЛИ СчетУчетаЗатратТипСчета = Перечисления.ТипыСчетов.Расходы)
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		ВестиРасчетыПоЗаказам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ВестиРасчетыПоЗаказам");
	КонецЕсли;
	...
	Для каждого Строка Из ТаблицаПредоплата Цикл
		...
		Если ВестиРасчетыПоЗаказам И Строка.Заказ <> Неопределено
		...
		ИначеЕсли ВестиРасчетыПоЗаказам Тогда
			...
			НайденнаяСтрокаРасходы = ?(НайденнаяСтрокаРасходы = Неопределено, ТаблицаРасходы.Найти(
				Документы.ЗаказПокупателя.ПустаяСсылка(), "ЗаказПоставщику"), НайденнаяСтрокаРасходы);
			НайденнаяСтрокаРасходы = ?(НайденнаяСтрокаРасходы = Неопределено, ТаблицаРасходы.Найти(
				Документы.ЗаказПоставщику.ПустаяСсылка(), "ЗаказПоставщику"), НайденнаяСтрокаРасходы);
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// НДС
	ПроведениеДокументовУНФ.ОтразитьДвижения("НДСПредъявленный", ТаблицыДляДвижений, Движения, Отказ);
	ПроведениеДокументовУНФ.ОтразитьДвижения("НДСЗаписиКнигиПокупок", ТаблицыДляДвижений, Движения, Отказ);
...
//
// Параметры:
//	ТТНВходящаяЕГАИС - ДокументСсылка.ТТНВходящаяЕГАИС - ТТН ЕГАИС
//	
Процедура ЗаполнитьПоНакладнойЕГАИС(ТТНВходящаяЕГАИС) Экспорт
	...
	|	ТТНВходящаяЕГАИС.ЕстьРасхождения КАК ЕстьРасхождения,
	|	ТТНВходящаяЕГАИС.ЕстьРасхождения КАК ПроверятьРасхожденияСВходящимДокументом,
	...
	|	ТаблицаВЗапросе.ЕстьРасхождения КАК ЕстьРасхождения,
	|	ТаблицаВЗапросе.ПроверятьРасхожденияСВходящимДокументом КАК ПроверятьРасхожденияСВходящимДокументом
	|ИЗ
	...
	НДСВключатьВСтоимость    = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "НДСВключатьВСтоимость");
	...
	Если ЕстьРасхождения Тогда
		ПринятьПоКолонке = ПредопределенноеЗначение("Перечисление.ПринятьПоКолонке.КоличествоПоФакту");
	КонецЕсли;
...
Процедура ЗаполнитьТоварыПоступленияПоТТН(ТТНВходящаяЕГАИС) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТТНВходящаяЕГАИСТовары.Ссылка.ЕстьРасхождения
	|			ТОГДА ТТНВходящаяЕГАИСТовары.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоСправочно,
	...
	|	КОНЕЦ КАК Количество,
	|	ТаблицаЕГАИС.КоличествоСправочно * ВЫБОР
	|		КОГДА ТаблицаЕГАИС.АлкогольнаяПродукция.ТипПродукции <> ЗНАЧЕНИЕ(Перечисление.ТипыПродукцииЕГАИС.Неупакованная)
	|			ТОГДА 1
	|		КОГДА ЕСТЬNULL(СпрНоменклатура.ОбъемДАЛ, 1) = 0
	|			ТОГДА 0
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
			...
			Если ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Номенклатура,
				"ВидСтавкиНДС")) Тогда
				СтрокаТаблицы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					СтрокаТаблицы.Номенклатура, "ВидСтавкиНДС"));
			Иначе
				СтрокаТаблицы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					Организация, "ВидСтавкиНДСПоУмолчанию"));
			КонецЕсли;
			
			СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтрокаТаблицы.СтавкаНДС);
			СтрокаТаблицы.СуммаНДС = ?(СуммаВключаетНДС, СтрокаТаблицы.Сумма - (СтрокаТаблицы.Сумма) / ((СтавкаНДС
				+ 100) / 100), СтрокаТаблицы.Сумма * СтавкаНДС / 100);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(НоваяСтрока.ЕдиницаИзмерения) Тогда
			...
			Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда
				НоваяСтрока.ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрока.Номенклатура,
					"ЕдиницаИзмерения");
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) И ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			НоваяСтрока.Номенклатура, "ВидСтавкиНДС")) Тогда
			НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				НоваяСтрока.Номенклатура, "ВидСтавкиНДС"), ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
		ИначеЕсли ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			Организация, "ВидСтавкиНДСПоУмолчанию")) Тогда
			НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				Организация, "ВидСтавкиНДСПоУмолчанию"), ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
		КонецЕсли;
...
Процедура ЗаполнитьПредоплату() Экспорт
	...
	Для каждого ТекСтрока Из ТаблицаЗаказов Цикл
		Если НЕ ВестиРасчетыПоЗаказам Тогда
...
Процедура ЗаполнитьПоРасходнойНакладной(ДанныеЗаполнения) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера Тогда
		...
		НеУчитыватьЗаказы = Ложь;
		Если ЗначениеЗаполнено(Договор) Тогда
			НеУчитыватьЗаказы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор,
	...
	РаботаСПодарочнымиСертификатами.УстановитьЦенуВозвращаемыхСертификатов(Запасы, СерииНоменклатуры, СуммаВключаетНДС, Ссылка);
	...
	Если ПолучитьФункциональнуюОпцию("ПроизводствоПодакцизныхТоваров")
		И ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя Тогда
		...
		Если ЕстьУчетПодакцизныхТоваров Тогда
			...
			Для Каждого СтрокаТабличнойЧасти Из Запасы Цикл
				...
				УчетАкцизовУНФ.ЗаполнитьСтрокуДаннымиАкцизов(СтрокаТабличнойЧасти, ВидыПроизведенныхПодакцизныхТоваров, ЭкспортПриРеализации, Ложь, ТекущаяДатаСеанса());
				СтрокаТабличнойЧасти.СуммаАкциза = СуммаАкциза;
			КонецЦикла;
...
//
// Параметры:
//	ДанныеЗаполнения - ДокументСсылка.ЗаказПокупателя
//	
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	
	Если ПерсональныеНастройкиРУНФ.РеквизитВШапке("ПоложениеЗаказаПоставщикуВДокументахПоступления") Тогда
	...
	ИменаРеквизитов.Добавить(
		"Организация, Ссылка, ВидОперации, Контрагент, Договор, ВалютаДокумента, НалогообложениеНДС");
	ИменаРеквизитов.Добавить(
		"СуммаВключаетНДС, НДСВключатьВСтоимость, Курс, Кратность, СостояниеЗаказа, ВариантЗавершения");
	ИменаРеквизитов.Добавить(
		"Проведен, ОжидаетсяВыборВариантаКП, ПоложениеСклада, СтруктурнаяЕдиницаРезерв, УчетПотребностиПоСкладам");
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, Новый Структура(СтрСоединить(
		ИменаРеквизитов, ", ")));
	...
	Если ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") Или ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДанныеЗаполнения, "УчетПотребностиПоСкладам") Тогда
		СтруктурнаяЕдиница = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "СтруктурнаяЕдиницаРезерв");
	КонецЕсли;
	Если Не ЗначениеЗаполнено(СтруктурнаяЕдиница) Тогда
		СтруктурнаяЕдиница = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойСклад");
		Если Не ЗначениеЗаполнено(СтруктурнаяЕдиница) Тогда
	...
	Если Не ВалютаДокумента = Константы.НациональнаяВалюта.Получить() Тогда
		...
		СтруктураПоВалюте = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(ДатаСреза, Новый Структура("Валюта",
			ВалютаРасчетов));
	...
	Если ЗначениеЗаполнено(Договор) Тогда
		ВидЦенКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ВидЦенКонтрагента");
	КонецЕсли;
	...
	ИначеЕсли ЗначенияРеквизитов.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		...
		ДисконтнаяКарта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "ДисконтнаяКарта");
	Иначе
		ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя;
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСерииНоменклатуры") Тогда
			...
			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("СсылкаНаДокумент", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения,
				"Ссылка"));
			Запрос.Текст =
			"ВЫБРАТЬ
			...
			|ГДЕ
			|	ЗаказПокупателяСерииНоменклатуры.Ссылка = &СсылкаНаДокумент";
	
			ТаблицаСерииНоменклатуры = Запрос.Выполнить().Выгрузить();
...
Процедура ЗаполнитьПоЗаказуПоставщику(ДанныеЗаполнения) Экспорт
	...
	Иначе
		МассивЗаказов.Добавить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "Ссылка"));
		ПоложениеЗаказаПоставщику = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеЗаказаПоставщикуВДокументахПоступления");
	...
	Если РазрешитьСкладыВТабличныхЧастях Тогда
		ПоложениеСклада = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеСкладаВДокументахПоступления");
	КонецЕсли;
		...
		Пока ВыборкаЗапасы.Следующий() Цикл
			...
			Иначе
				...
				КоличествоСтрокиВЕдиницахХранения = НоваяСтрокаЗапасы.Количество
					* ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаЗапасы.ЕдиницаИзмерения, "Коэффициент");
				...
				НоваяСтрокаЗапасы.Количество = КоличествоВСтрокеЗапасов / ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			...
			Если ОблагаетсяНДС Тогда
				Если ЗначениеЗаполнено(НоваяСтрокаЗапасы.Партия) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаЗапасы.Партия, "Статус")
					= Перечисления.СтатусыПартий.ТоварыНаКомиссии 
					И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаЗапасы.Партия, "НалогообложениеНДС")
					= Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
					
					НоваяСтрокаЗапасы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(Перечисления.ВидыСтавокНДС.БезНДС);;
				Иначе
					
					Если ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаЗапасы.Номенклатура,
						"ВидСтавкиНДС")) Тогда
						НоваяСтрокаЗапасы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(
							ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаЗапасы.Номенклатура, "ВидСтавкиНДС"),
							?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
					Иначе
						НоваяСтрокаЗапасы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(
							ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Компания, "ВидСтавкиНДСПоУмолчанию"), ?(
							ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
					КонецЕсли;
			...
			Если УчетГТД Тогда
				НоваяСтрокаЗапасы.СтранаПроисхождения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					НоваяСтрокаЗапасы.Номенклатура, "СтранаПроисхождения");
			КонецЕсли;
...
Процедура ЗаполнитьПоСчетуНаОплатуПоставщика(ДанныеЗаполнения) Экспорт
	...
	ВалютаРасчетов = Константы.НациональнаяВалюта.Получить();
	Если НЕ ЗначениеЗаполнено(ВидЦенКонтрагента) и ЗначениеЗаполнено(Договор) Тогда
	...
	Если ВалютаДокумента = Константы.НациональнаяВалюта.Получить() Тогда
		...
		Кратность = ДанныеЗаполнения.Кратность;
	Иначе
		ВалютаКурсКратность = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаРасчетов, Дата);
	...
	Для каждого СтрокаТабличнойЧасти Из ДанныеЗаполнения.Запасы Цикл
		...
		ИначеЕсли СтрокаТабличнойЧасти.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
			...
			ТипСчетаРасходы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрока.Номенклатура,
				"СчетУчетаЗатрат.ТипСчета");
...
Процедура ЗаполнитьПоСкладскомуОрдеруБезУчетаОстатков(ДанныеЗаполнения, Компания) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти Из ДанныеЗаполнения.Запасы Цикл
		...
		Если ОблагаетсяНДС Тогда
			...
			Если ЗначениеЗаполнено(Партия) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Партия, "Статус")
				= Перечисления.СтатусыПартий.ТоварыНаКомиссии И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Партия,
				"НалогообложениеНДС") = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
...
Процедура ЗаполнитьПоЧекуККМ(ДанныеЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(Заказ) Тогда
		Договор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Заказ, "Договор");
	КонецЕсли;
...
Процедура ЗаполнитьПоПоступлениюНаСчет(ДанныеЗаполнения) Экспорт
	...
	УслугаИзДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорОснования, "УслугаДоставки");
	...
	Если ЗначениеЗаполнено(ДоговорОснования) Тогда
		...
		Если ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорОснования, "ДоговорСоСлужбойДоставки")) Тогда
			Договор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорОснования, "ДоговорСоСлужбойДоставки");
		КонецЕсли;
	...
	Если Не НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
		...
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УслугаИзДоговора, "ВидСтавкиНДС")) Тогда
		СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УслугаИзДоговора,
			"ВидСтавкиНДС"), ТекущаяДатаСеанса());
	Иначе
		СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация,
			"ВидСтавкиНДСПоУмолчанию"), ТекущаяДатаСеанса());
	КонецЕсли;
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДокументСсылка", ДанныеЗаполнения.Ссылка);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПоступлениеНаСчетРасшифровкаПлатежа.УдержаноАгентом,
	|	ПоступлениеНаСчетРасшифровкаПлатежа.Договор,
	|	ПоступлениеНаСчетРасшифровкаПлатежа.Документ,
	|	ПоступлениеНаСчетРасшифровкаПлатежа.Заказ,
	|	ПоступлениеНаСчетРасшифровкаПлатежа.СуммаРасчетов,
	|	ПоступлениеНаСчетРасшифровкаПлатежа.Курс,
	|	ПоступлениеНаСчетРасшифровкаПлатежа.Кратность,
	...
	|ГДЕ
	|	ПоступлениеНаСчетРасшифровкаПлатежа.Ссылка = &ДокументСсылка";
	
	ТаблицаРасшифировкаПлатежа = Запрос.Выполнить().Выгрузить();
	
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ВидОперации") = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты 
		И ТаблицаРасшифировкаПлатежа.Количество() 
		И ТаблицаРасшифировкаПлатежа[0].УдержаноАгентом > 0 Тогда
		
		СтрокаРасшифровки = ТаблицаРасшифировкаПлатежа[0];
		...
		Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
			ВалютаРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорКонтрагента, "ВалютаРасчетов");
		КонецЕсли;
		КурсИКратностьВалютыРасчетов = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Дата, Новый Структура("Валюта", ВалютаРасчетов));
		
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ВалютаДенежныхСредств") = ВалютаРасчетов Тогда
			НоваяСтрока.СуммаРасчетов = НоваяСтрока.СуммаПлатежа;
		Иначе
			КурсИКратностьВалютыДС = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Дата, Новый Структура("Валюта", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ВалютаДенежныхСредств")));
...
Процедура ЗаполнитьНаправлениеДеятельностиВТЧРасходы()
	...
	Для Каждого СтрокаРасходы Из Расходы Цикл
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаРасходы.Номенклатура, "СчетУчетаЗатрат.ТипСчета")
...
Процедура ЗаполнитьНомерСтрокиДокументаОснования(Отказ)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СсылкаНаДокумент", ДокументОснование);
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РасходнаяНакладнаяСкидкиНаценки.НомерСтроки
		|ИЗ
		|	Документ.РасходнаяНакладная.СкидкиНаценки КАК РасходнаяНакладнаяСкидкиНаценки
		|ГДЕ
		|	РасходнаяНакладнаяСкидкиНаценки.Ссылка = &СсылкаНаДокумент
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	РезультатыЗапросовРасходнаяНакладная = Запрос.ВыполнитьПакет();	
	ТаблицаСкидкиНаценки = РезультатыЗапросовРасходнаяНакладная[0].Выгрузить(); 
	
	ИспользоватьЦену = Не ТаблицаСкидкиНаценки.Количество()
		Или ТаблицаСкидкиНаценки.Количество()
		И ПолучитьФункциональнуюОпцию("ИспользоватьРучныеСкидкиНаценкиЗакупки");
	
	ДополнениеЦена = ?(ИспользоватьЦену, ", Цена", "");
		
	ТаблицаНоменклатурыОснования = РезультатыЗапросовРасходнаяНакладная[1].Выгрузить();
	ПроверитьКоэффициентыЕдиницИзмерения(ТаблицаНоменклатурыОснования);
	ТаблицаНоменклатурыОснования.Свернуть("Номенклатура, Характеристика, Партия" + ДополнениеЦена, "Количество");

	ТаблицаНоменклатурыВозврата = Запасы.Выгрузить( ,
		"Номенклатура, Характеристика, Партия, Количество, ЕдиницаИзмерения" + ДополнениеЦена);
	ПроверитьКоэффициентыЕдиницИзмерения(ТаблицаНоменклатурыВозврата);
	ТаблицаНоменклатурыВозврата.Свернуть("Номенклатура, Характеристика, Партия" + ДополнениеЦена, "Количество");

	НоменклатураБезКонтроляЦены = Новый Массив;
	Для Каждого СтрокаСертификата Из ТаблицаНоменклатурыВозврата Цикл
		ДанныеСертификата = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаСертификата.Номенклатура, "ТипНоменклатуры, ЧастичноеПогашение");
		Если ДанныеСертификата.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПодарочныйСертификат")
			И ДанныеСертификата.ЧастичноеПогашение Тогда
			// Сертификаты с частичным погашением при возврате имеют цену остатка номинала
			НоменклатураБезКонтроляЦены.Добавить(СтрокаСертификата.Номенклатура)
		КонецЕсли;
	КонецЦикла;
	
	СтруктураПоискаБезЦены = Новый Структура("Номенклатура, Характеристика, Партия");
	СтруктураПоискаСЦеной = Новый Структура("Номенклатура, Характеристика, Партия"+ ДополнениеЦена);

	Для Каждого СтрокаТабличнойЧасти Из ТаблицаНоменклатурыВозврата Цикл

		СтруктураПоиска = СтруктураПоискаСЦеной;
		Если НоменклатураБезКонтроляЦены.Найти(СтрокаТабличнойЧасти.Номенклатура) <> Неопределено Тогда
			СтруктураПоиска = СтруктураПоискаБезЦены;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаТабличнойЧасти);
		НайденныеСтроки = ТаблицаНоменклатурыОснования.НайтиСтроки(СтруктураПоиска);

		Если НайденныеСтроки.Количество() И НайденныеСтроки[0].Количество < СтрокаТабличнойЧасти.Количество Тогда

			ТекстСообщения = СтрШаблон(НСтр(
				"ru = 'Количество номенклатуры %1 (%2) превышает количество в документе основания (%3).'"),
				СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.Количество, НайденныеСтроки[0].Количество);
	...
	ТаблицаОстатков = ОстаткиНоменклатурыДокументаОснования();
	
	СтруктураПоискаБезЦены = Новый Структура();
	СтруктураПоискаБезЦены.Вставить("Номенклатура");
	СтруктураПоискаБезЦены.Вставить("Характеристика");
	СтруктураПоискаБезЦены.Вставить("Партия");
	СтруктураПоискаБезЦены.Вставить("НоменклатураНабора");
	СтруктураПоискаБезЦены.Вставить("ХарактеристикаНабора");
	СтруктураПоискаБезЦены.Вставить("ЗаказПокупателя");

	СтруктураПоискаСЦеной = Новый Структура("Номенклатура, Характеристика, Партия, НоменклатураНабора, ХарактеристикаНабора, ЗаказПокупателя"+ ДополнениеЦена);
	...
	Для Каждого СтрокаТабличнойЧасти Из Запасы Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			СтрокаТабличнойЧасти.НомерСтрокиДокументаОснования = 0;
			
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Для данных в строке %1 списка ""Запасы"" не найдено соответствие (номенклатура, характеристика, партия%2) в документе основания.
											|При вводе корректировочной счет-фактуры данные в ней будут отображены некорректно'"),  СтрокаТабличнойЧасти.НомерСтроки, ?(ИспользоватьЦену, ", цена", ""));
		...
		Иначе
			...
			Если СтрокаОстатки.КоличествоОстаток <= 0 Тогда
				
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Для номенклатуры ""%1"" в строке %2 списка ""Запасы"" в строке %3 документа основания нехватает остатка'"),
				СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.НомерСтроки, СтрокаОстатки.НомерСтроки);
			...
			Иначе
				...
				Если ТипЗнч(СтрокаТабличнойЧасти.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") 
					И ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЕдиницаИзмерения) Тогда
					КоличествоСтроки = СтрокаТабличнойЧасти.Количество * ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТабличнойЧасти.ЕдиницаИзмерения, "Коэффициент");
				Иначе
					КоличествоСтроки = СтрокаТабличнойЧасти.Количество;
				КонецЕсли;
				
				Если КоличествоСтроки > СтрокаОстатки.КоличествоОстаток Или КоличествоСтроки > СтрокаОстатки.Количество Тогда
				...
				Иначе
					...
					Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
						НайденнаяСтрока.КоличествоОстаток = НайденнаяСтрока.КоличествоОстаток - КоличествоСтроки;
					КонецЦикла;
					
				КонецЕсли;
				
				СтрокаТабличнойЧасти.НомерСтрокиДокументаОснования = СтрокаОстатки.НомерСтроки;
...
Процедура ПроверитьКоэффициентыЕдиницИзмерения(ТаблицаНоменклатуры);
	
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаНоменклатуры Цикл
		
		Если ТипЗнч(СтрокаТабличнойЧасти.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") 
			И ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЕдиницаИзмерения) Тогда
			СтрокаТабличнойЧасти.Количество = СтрокаТабличнойЧасти.Количество * ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТабличнойЧасти.ЕдиницаИзмерения, "Коэффициент");
		КонецЕсли;
...
Функция ОстаткиНоменклатурыДокументаОснования()
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ДатаОснования", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Дата"));
	Запрос.УстановитьПараметр("ИсправляемыйДокументРеализации", ДокументОснование);
		
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОстаткиНевозвращенныхСертификатов.ПодарочныйСертификат КАК ПодарочныйСертификат,
	|	СУММА(ОстаткиНевозвращенныхСертификатов.Количество) КАК Количество,
	|	СУММА(ОстаткиНевозвращенныхСертификатов.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ВТ_ПодарочныеСертификатыОбороты
	|ИЗ
	|	(ВЫБРАТЬ
	|		ПодарочныеСертификатыПродажа.ПодарочныйСертификат КАК ПодарочныйСертификат,
	|		КОЛИЧЕСТВО(ПодарочныеСертификатыПродажа.НомерСертификата) КАК Количество,
	|		СУММА(ПодарочныеСертификатыПродажа.Сумма) КАК Сумма
	|	ИЗ
	|		РегистрНакопления.ПодарочныеСертификаты КАК ПодарочныеСертификатыПродажа
	|	ГДЕ
	|		ПодарочныеСертификатыПродажа.Регистратор = &ИсправляемыйДокументРеализации
	|		И ПодарочныеСертификатыПродажа.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	СГРУППИРОВАТЬ ПО
	|		ПодарочныеСертификатыПродажа.ПодарочныйСертификат
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|	ИЗ
	|		РегистрНакопления.ПодарочныеСертификаты КАК ПодарочныеСертификатыПродажа
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриходнаяНакладная КАК Возврат
	|			ПО Возврат.Ссылка = ПодарочныеСертификатыПродажа.Регистратор
	|	ГДЕ
	|		Возврат.ДокументОснование = &ИсправляемыйДокументРеализации
	|		И ПодарочныеСертификатыПродажа.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И НЕ Возврат.Ссылка = &Регистратор
	|	СГРУППИРОВАТЬ ПО
	|		ПодарочныеСертификатыПродажа.ПодарочныйСертификат) КАК ОстаткиНевозвращенныхСертификатов
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиНевозвращенныхСертификатов.ПодарочныйСертификат
	|Имеющие СУММА(ОстаткиНевозвращенныхСертификатов.Количество) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ПродажиОбороты.КоличествоОборот КАК КоличествоОстаток
	|ИЗ
	|	РегистрНакопления.Продажи.Обороты(, &ДатаОснования, Запись, Документ = &ИсправляемыйДокументРеализации) КАК
	|		ПродажиОбороты
	...
	|ВЫБРАТЬ
	|	РасходнаяНакладнаяЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ПодарочныеСертификатыОбороты.Сумма / ПодарочныеСертификатыОбороты.Количество КАК Цена,
	|	РасходнаяНакладнаяЗапасы.Номенклатура КАК Номенклатура,
	|	РасходнаяНакладнаяЗапасы.Характеристика КАК Характеристика,
	|	РасходнаяНакладнаяЗапасы.Партия КАК Партия,
	|	РасходнаяНакладнаяЗапасы.Количество КАК Количество,
	...
	|	РасходнаяНакладнаяЗапасы.НоменклатураНабора КАК НоменклатураНабора,
	|	РасходнаяНакладнаяЗапасы.ХарактеристикаНабора КАК ХарактеристикаНабора,
	|	ПодарочныеСертификатыОбороты.Количество КАК КоличествоОстаток
	|ИЗ
	|	ВТ_ПодарочныеСертификатыОбороты КАК ПодарочныеСертификатыОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РасходнаяНакладная.Запасы КАК РасходнаяНакладнаяЗапасы
	|		ПО ПодарочныеСертификатыОбороты.ПодарочныйСертификат = РасходнаяНакладнаяЗапасы.Номенклатура
	|ГДЕ
	|	РасходнаяНакладнаяЗапасы.Ссылка = &ИсправляемыйДокументРеализации
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И НЕ ПродажиОбороты.Регистратор = &Регистратор", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И НЕ Возврат.Ссылка = &Регистратор", "");
	КонецЕсли;
