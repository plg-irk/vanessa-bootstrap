...
// Отказ - Булево
// ДоговорДляОтложенногоПроведения - СправочникСсылка.ДоговорыКонтрагентов
//
//Возвращаемое значение:
// Структура
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера
			  Или ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПереработчика
			  Или ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратСОтветХранения
		...
		Если КоличествоТребуется > 0 Тогда
			...
			Если ЭтоВозвратОтПокупателя Тогда
				...
				Если ЗначениеЗаполнено(ОснованиеНакладная)
					...
					СтрокаДоходыРасходы.Проект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
						ДокументСсылкаПриходнаяНакладная, "Проект");
...
Процедура СформироватьТаблицаРазмещениеЗаказов(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ВидОперации")
		= Перечисления.ВидыОперацийПриходнаяНакладная.ПриемВПереработку Тогда
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ВидОперации")
		= Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика Тогда
...
Процедура СформироватьТаблицаДоходыИРасходыНераспределенные(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "СпособЗачетаПредоплаты")
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|			ИЛИ ТаблицаУправленческий.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары))
	|	И НЕ ТаблицаУправленческий.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПодарочныйСертификат)
	|	И ТаблицаУправленческий.ТоварыНаКомиссии
	|	И &АвтоматическийЗачетПредоплаты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ПоложительнаяКурсоваяРазницаСчетУчета", ПланыСчетов.Управленческий.ПрочиеДоходы);
	Запрос.УстановитьПараметр("ОтрицательнаяКурсоваяРазницаСчетУчета", ПланыСчетов.Управленческий.ПрочиеРасходы);
	Запрос.УстановитьПараметр("ВидОперации", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
	...
	АвтоматическийЗачетПредоплаты =
		СтруктураДополнительныеСвойства.СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто;
	Запрос.УстановитьПараметр("АвтоматическийЗачетПредоплаты", АвтоматическийЗачетПредоплаты);
	...
	Для Каждого СтрокаТЗ Из ТаблицаУправленческийРасчетыСПокупателями Цикл  
		Если СтрокаТЗ.ЭтоВозвратВзаиморасчеты И АвтоматическийЗачетПредоплаты Тогда
...
Процедура СформироватьТаблицаЗакупкиПоставщиковДляКУДиР(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ВалютаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ВалютаДокумента");
	
	Если УчетВалюты И ВалютаДокумента <> НацВалюта Тогда
		ТекущийКурс = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаДокумента, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			ДокументСсылкаПриходнаяНакладная, "Дата"));
	КонецЕсли;
...
Процедура СформироватьТаблицаСуммыДокументовРегламентированныйУчет(ДокументСсылкаРасходнаяНакладная,
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная, "ВидОперации")
		= Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика
		 И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная, "Договор.ВалютаРасчетов")
		<> СтруктураДополнительныеСвойства.НациональнаяВалюта Тогда
...
Функция ПолучитьТекстЗапросаИнициализироватьДанные()
	...
	|        ИЛИ
	|            ПриходнаяНакладная.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары))
	...
	|    ПриходнаяНакладная.ДоговорКомиссии.НеУчитыватьЗаказыПриПередачеНаКомиссию КАК
	...
	|        (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПриемНаКомиссию),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПриемВПереработку),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПриемНаОтветХранение))
	...
	|    ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВидОперации В
	|        (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПереработчика),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветХранения))
	|            ТОГДА &Организация
	|        ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|    ВЫБОР
	|        КОГДА
	|            ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|    ВЫБОР
	|        КОГДА
	|            ПриходнаяНакладнаяЗапасы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|    ВЫБОР
	|        КОГДА ПриходнаяНакладнаяЗапасы.Заказ ССЫЛКА Документ.ЗаказПокупателя
	|        И ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Заказ КАК
	|            Документ.ЗаказПокупателя).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд)
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|        И
	|            (ВременнаяТаблицаШапка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя)
	|        ИЛИ
	|            ВременнаяТаблицаШапка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары))
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|    ВЫБОР
	|        КОГДА ТИПЗНАЧЕНИЯ(ПриходнаяНакладнаяЗапасы.ЕдиницаИзмерения) = ТИП(Справочник.ЕдиницыИзмерения)
	|            ТОГДА ПриходнаяНакладнаяЗапасы.Количество * ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.ЕдиницаИзмерения КАК
	|                Справочник.ЕдиницыИзмерения).Коэффициент
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.Количество
	|    КОНЕЦ КАК Количество,
	...
	|            ТОГДА ПриходнаяНакладнаяЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.СуммаНДС * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ПриходнаяНакладнаяЗапасы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.Всего * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	...
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.СуммаРасходов * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасходов,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ПриходнаяНакладнаяЗапасы.СуммаРасходов * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ПриходнаяНакладнаяЗапасы.СуммаРасходовБезНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.СуммаРасходовБезНДС * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасходовБезНДС,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ПриходнаяНакладнаяЗапасы.СуммаРасходовБезНДС * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность
	|                / (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	...
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.Себестоимость * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	...
	|    ВЫБОР
	|		КОГДА ВременнаяТаблицаШапка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя)
	|				И (ВЫРАЗИТЬ(ВременнаяТаблицаШапка.ДокументОснование КАК Документ.РасходнаяНакладная)) <> ЗНАЧЕНИЕ(Документ.РасходнаяНакладная.ПустаяСсылка)
	|			ТОГДА ИСТИНА
	...
	|        КОГДА ВременнаяТаблицаШапка.ВидОперации В
	|        (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПереработчика),
	|            ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветХранения))
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ПриходнаяНакладнаяЗапасы.Всего * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	|                (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.Всего
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|                ТОГДА ПриходнаяНакладнаяЗапасы.СуммаНДС * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	|                    (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	...
	|        КОГДА ПриходнаяНакладнаяЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|            ТОГДА ПриходнаяНакладнаяЗапасы.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.Номенклатура.ПолитикаУчетаСерий
	...
	|    ВЫБОР
	|        КОГДА
	|            ВременнаяТаблицаШапка.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|            ТОГДА ПриходнаяНакладнаяЗапасы.Ссылка
	|        КОГДА
	|            ПриходнаяНакладнаяЗапасы.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	...
	|    ВременнаяТаблицаШапка.ДоговорРасчетыВУсловныхЕдиницах,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ПриходнаяНакладнаяЗапасы.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяЗапасы.Сумма * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	...
	|        ПО ВЫБОР
	|            КОГДА ПриходнаяНакладнаяЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|                ТОГДА ПриходнаяНакладнаяЗапасы.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|            ИНАЧЕ ПриходнаяНакладнаяЗапасы.Номенклатура = ПолитикиУчетаСерий.Владелец
	|        КОНЕЦ
	...
	|            ТОГДА ПриходнаяНакладнаяРасходы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяРасходы.Всего * ПриходнаяНакладнаяРасходы.Ссылка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВЫБОР
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА ПриходнаяНакладнаяРасходы.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|                ТОГДА ПриходнаяНакладнаяРасходы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ПриходнаяНакладнаяРасходы.СуммаНДС * ПриходнаяНакладнаяРасходы.Ссылка.Курс *
	|                КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ВЫБОР
	|                КОГДА ПриходнаяНакладнаяРасходы.Ссылка.Кратность = 0
	|                    ТОГДА 1
	|                ИНАЧЕ ПриходнаяНакладнаяРасходы.Ссылка.Кратность
	|            КОНЕЦ)
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|                ТОГДА ПриходнаяНакладнаяРасходы.СуммаНДС * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	|                    (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	...
	|    ВЫБОР
	|        КОГДА
	|            ПриходнаяНакладнаяРасходы.Номенклатура.СчетУчетаЗатрат.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Расходы)
	...
	|    ВЫБОР
	|        КОГДА
	|            ПриходнаяНакладнаяРасходы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|            ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.НДСПоПриобретеннымЦенностям)
	...
	|    ВЫБОР
	|        КОГДА
	|            ПриходнаяНакладнаяРасходы.Ссылка.ПоложениеПроекта <> ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ПриходнаяНакладнаяРасходы.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ПриходнаяНакладнаяРасходы.Сумма * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРуб
	...
	|            / (КурсыВалютУчетаСрезПоследних.Курс * ВЫБОР
	|            КОГДА ТаблицаДокумента.Кратность = 0
	|                ТОГДА 1
	|            ИНАЧЕ ТаблицаДокумента.Кратность
	|        КОНЕЦ) КАК ЧИСЛО(15, 2))
	...
	|    И ПриходнаяНакладнаяПредоплата.Заказ В (ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка),
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	Если  НЕ СтруктураДополнительныеСвойства.УчетВалютныхОпераций
		...
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ВалютаДокумента")
	...
	// НДС
	УчетНДСУНФ.СформироватьДвиженияНДСПредъявленныйПоступление(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства);
...
// 
// Параметры:
//  ДокументСсылкаПриходнаяНакладная - ДокументСсылка.ПриходнаяНакладная - Документ ссылка приходная накладная
...
Процедура СформироватьТаблицаПродажиПоДисконтнойКарте(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ВидОперации")
		<> Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДокументСсылкаПриходнаяНакладная, "ВидОперации")
		<> Перечисления.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары Тогда //Только эта строка
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПродажиПоДисконтнымКартам",
			Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ДисконтнаяКарта").Пустая() Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПродажиПоДисконтнымКартам",
			Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ПроцентСкидки", ДисконтныеКартыУНФСервер.ВычислитьПроцентСкидкиПоДисконтнойКарте(
		СтруктураДополнительныеСвойства.ДляПроведения.Дата, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ПараметрыСопоставления, АдресРезультата) Экспорт
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		ДопустимыеТипыЗаказа = ДопустимыеТипыЗаказа(НастройкиЗагрузкиДанных.ВидОперации);
...
Функция ДанныеПолученныхДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина,
	...
	|			И
	|				ПриходнаяНакладная.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	...
	|				ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладная.Запасы.Цена * ПриходнаяНакладная.Курс / ПриходнаяНакладная.Кратность КАК
	|					ЧИСЛО(15, 2))
	...
	|			ИЛИ ПриходнаяНакладная.Запасы.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|			И ПриходнаяНакладная.Запасы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладная.Запасы.Сумма * ПриходнаяНакладная.Курс / ПриходнаяНакладная.Кратность КАК
	|					ЧИСЛО(15, 2))
	...
	|			ИЛИ ПриходнаяНакладная.Запасы.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|			И ПриходнаяНакладная.Запасы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладная.Запасы.СуммаНДС * ПриходнаяНакладная.Курс / ПриходнаяНакладная.Кратность КАК
	|					ЧИСЛО(15, 2))
	...
	|			ИЛИ ПриходнаяНакладная.Запасы.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|			И ПриходнаяНакладная.Запасы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладная.Запасы.Всего * ПриходнаяНакладная.Курс / ПриходнаяНакладная.Кратность КАК
	|					ЧИСЛО(15, 2))
	...
	|			И
	|				ПриходнаяНакладная.Запасы.НоменклатураНабора.ВариантПечатиНабора = ЗНАЧЕНИЕ(Перечисление.ВариантыПечатиНаборов.НаборИКомплектующие)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	...
	|		ВЫБОР
	|			КОГДА ПриходнаяНакладная.СуммаВключаетНДС
	|				ТОГДА ВЫРАЗИТЬ((ПриходнаяНакладная.Запасы.Сумма - ПриходнаяНакладная.Запасы.СуммаНДС) /
	|					ПриходнаяНакладная.Запасы.Количество КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ ПриходнаяНакладная.Запасы.Цена
	|		КОНЕЦ КАК СтоимостьБезНалога) КАК ТаблицаЗапасы,
	...
	|				ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|			И
	|				ПриходнаяНакладная.Расходы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	...
	|				ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладная.Расходы.Цена * ПриходнаяНакладная.Ссылка.Курс /
	|					ПриходнаяНакладная.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|			ИЛИ ПриходнаяНакладная.Расходы.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|			И ПриходнаяНакладная.Расходы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладная.Расходы.Сумма * ПриходнаяНакладная.Курс / ПриходнаяНакладная.Кратность КАК
	|					ЧИСЛО(15, 2))
	...
	|			ИЛИ ПриходнаяНакладная.Расходы.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|			И ПриходнаяНакладная.Расходы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладная.Расходы.СуммаНДС * ПриходнаяНакладная.Ссылка.Курс /
	|					ПриходнаяНакладная.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|			ИЛИ ПриходнаяНакладная.Расходы.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|			И ПриходнаяНакладная.Расходы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладная.Расходы.Всего * ПриходнаяНакладная.Ссылка.Курс /
	|					ПриходнаяНакладная.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|		НЕОПРЕДЕЛЕНО КАК ПредставлениеГТД) КАК ТаблицаРасходы,
	...
	|		ВЫБОР
	|			КОГДА
	|				ПриходнаяНакладная.ДобавленныеНаборы.НоменклатураНабора.ВариантПечатиНабора = ЗНАЧЕНИЕ(Перечисление.ВариантыПечатиНаборов.ТолькоНабор)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	...
	|			ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|		И
	|			ПриходнаяНакладнаяЗапасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	...
	|		И
	|			ПриходнаяНакладнаяЗапасы.НоменклатураНабора.ВариантПечатиНабора = ЗНАЧЕНИЕ(Перечисление.ВариантыПечатиНаборов.НаборИКомплектующие)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НеобходимоВыделитьКакСоставНабора,
	...
	|	ПриходнаяНакладнаяЗапасы.Штрихкод КАК Штрихкод,
	...
	|					ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Цена * ПриходнаяНакладнаяЗапасы.Ссылка.Курс /
	|						ПриходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|				ИНАЧЕ ВЫБОР
	...
	|						ПриходнаяНакладнаяЗапасы.Количество КАК ЧИСЛО(15, 2))
	|				КОНЕЦ
	|			КОНЕЦ
	...
	|				КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	|					ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Сумма * ПриходнаяНакладнаяЗапасы.Ссылка.Курс /
	|						ПриходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|				ИНАЧЕ ВЫБОР
	...
	|					ТОГДА ПриходнаяНакладнаяЗапасы.СуммаНДС
	|				КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	|					ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.СуммаНДС * ПриходнаяНакладнаяЗапасы.Ссылка.Курс /
	|						ПриходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|					ТОГДА ПриходнаяНакладнаяЗапасы.Всего
	|				КОГДА СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	|					ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладнаяЗапасы.Всего * ПриходнаяНакладнаяЗапасы.Ссылка.Курс /
	|						ПриходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|	ПриходнаяНакладнаяЗапасы.КодТНВЭД.ЕдиницаИзмерения.МеждународноеСокращение КАК
	|		КодТНВЭДЕдиницаИзмеренияМеждународноеСокращение,
	|	ВЫБОР
	|		КОГДА ПриходнаяНакладнаяЗапасы.Ссылка.СуммаВключаетНДС
	|			ТОГДА ВЫРАЗИТЬ((ПриходнаяНакладнаяЗапасы.Сумма - ПриходнаяНакладнаяЗапасы.СуммаНДС) /
	|				ПриходнаяНакладнаяЗапасы.Количество КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ ПриходнаяНакладнаяЗапасы.Цена
	...
	|		И
	|			СуммыДокументовРегламентированныйУчет.ТабличнаяЧастьДокумента = ЗНАЧЕНИЕ(Перечисление.ТабличныеЧастиДокументов.Запасы)
	|ГДЕ
	|	ПриходнаяНакладнаяЗапасы.Ссылка В (&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|			ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|		И
	|			ПриходнаяНакладнаяРасходы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	...
	|					ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.Цена * ПриходнаяНакладнаяРасходы.Ссылка.Курс /
	|						ПриходнаяНакладнаяРасходы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|				КОГДА ПриходнаяНакладнаяРасходы.Ссылка.Договор.РасчетыВУсловныхЕдиницах
	|				И ПриходнаяНакладнаяРасходы.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.Сумма * ПриходнаяНакладнаяРасходы.Ссылка.Курс /
	|						ПриходнаяНакладнаяРасходы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|				КОГДА ПриходнаяНакладнаяРасходы.Ссылка.Договор.РасчетыВУсловныхЕдиницах
	|				И ПриходнаяНакладнаяРасходы.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.СуммаНДС * ПриходнаяНакладнаяРасходы.Ссылка.Курс /
	|						ПриходнаяНакладнаяРасходы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|				ИНАЧЕ ПриходнаяНакладнаяРасходы.СуммаНДС
	|			КОНЕЦ
	|		ИНАЧЕ ПриходнаяНакладнаяРасходы.СуммаНДС
	...
	|				КОГДА ПриходнаяНакладнаяРасходы.Ссылка.Договор.РасчетыВУсловныхЕдиницах
	|				И ПриходнаяНакладнаяРасходы.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					ТОГДА ВЫРАЗИТЬ(ПриходнаяНакладнаяРасходы.Всего * ПриходнаяНакладнаяРасходы.Ссылка.Курс /
	|						ПриходнаяНакладнаяРасходы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|				ИНАЧЕ ПриходнаяНакладнаяРасходы.Всего
	|			КОНЕЦ
	|		ИНАЧЕ ПриходнаяНакладнаяРасходы.Всего
	...
	|						ТОГДА СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС
	|					ИНАЧЕ ВЫРАЗИТЬ((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС) /
	|						ПриходнаяНакладнаяРасходы.Количество КАК ЧИСЛО(15, 2))
	|				КОНЕЦ
	|			КОНЕЦ
	|		ИНАЧЕ ПриходнаяНакладнаяРасходы.Цена
	|	КОНЕЦ КАК Цена,
	...
	|		И
	|			СуммыДокументовРегламентированныйУчет.ТабличнаяЧастьДокумента = ЗНАЧЕНИЕ(Перечисление.ТабличныеЧастиДокументов.Расходы)
	|ГДЕ
	|	ПриходнаяНакладнаяРасходы.Ссылка В (&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|					ТОГДА ВЫРАЗИТЬ((ВременнаяТаблица_ПриходнаяНакладнаяЗапасы.Сумма -
	|						ВременнаяТаблица_ПриходнаяНакладнаяЗапасы.СуммаНДС) / ВременнаяТаблица_ПриходнаяНакладнаяЗапасы.Количество КАК
	|						ЧИСЛО(15, 2))
	...
	|		И
	|			ВременнаяТаблица_ПриходнаяНакладнаяЗапасы.ИдентификаторСтроки = ПриходнаяНакладнаяСведенияПрослеживаемости.ИдентификаторСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПриходнаяНакладнаяЗапасы.НомерСтроки КАК НомерСтроки,
	|	ПриходнаяНакладнаяЗапасы.Ссылка КАК Ссылка,
	|	ЕСТЬNULL(ШтрихкодыНоменклатуры.Штрихкод, ПриходнаяНакладнаяЗапасы.Номенклатура.Штрихкод) КАК Штрихкод
	|ИЗ
	|	Документ.ПриходнаяНакладная.Запасы КАК ПриходнаяНакладнаяЗапасы
	...
	|		И ПриходнаяНакладнаяЗапасы.Партия = ШтрихкодыНоменклатуры.Партия
	|ГДЕ
	|	ПриходнаяНакладнаяЗапасы.Ссылка В (&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	ТЗСтрокЗапасы = РезультатыЗапроса[2].Выгрузить();
	ТЗСтрокЗапасы.Индексы.Добавить("Ссылка");
	ТЗСтрокРасходы = РезультатыЗапроса[4].Выгрузить();
...
Функция УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов)
	...
	Для Каждого ДанныеДокумента Из ДанныеДокументов Цикл
		Для Каждого СтрокаТаблицыЗапасы Из ДанныеДокумента.ТаблицаЗапасы Цикл
			ПараметрыПоискаШтрихкодыНоменклатуры.Ссылка = ДанныеДокумента.Ссылка;
			ПараметрыПоискаШтрихкодыНоменклатуры.НомерСтроки = СтрокаТаблицыЗапасы.НомерСтроки;
			НайденныеСтроки = ШтрихкодыНоменклатуры.НайтиСтроки(ПараметрыПоискаШтрихкодыНоменклатуры);
			ПечатьДокументовУНФ.ПодобратьШтрихкодНоменклатуры(СтрокаТаблицыЗапасы, НайденныеСтроки);
		КонецЦикла;
	КонецЦикла;
...
//
// Параметры:
//  НомераТаблиц - Структура - Номера страниц с данными счет-фактуры
//
// Возвращаемое значение:
//  Строка - Текст запроса получения данных печаты счет фактуры
//
...
// 
// Возвращаемое значение:
//  ТабличныйДокумент - ТабличныйДокумент в который будет выводится печатная форма.
//  
Функция ПечатнаяФормаНакладная(МассивОбъектов, ОбъектыПечати)
	...
	|		КОНЕЦ КАК НеобходимоВыделитьКакСоставНабора,
	|		НоменклатураНабора КАК НоменклатураНабора,
	|		ХарактеристикаНабора КАК ХарактеристикаНабора) КАК ТаблицаЗапасы,
	|	ПриходнаяНакладная.ДобавленныеНаборы.(
	|		НоменклатураНабора КАК НоменклатураНабора,
	|		ХарактеристикаНабора КАК ХарактеристикаНабора,
	|		Количество КАК Количество,
	...
	|		НоменклатураНабора.ЕдиницаИзмерения КАК ЕдиницаИзмеренияНабора,
	|		НоменклатураНабора.ЕдиницаИзмерения.Код КАК КодЕдиницыИзмеренияНабора,
	|		ИСТИНА КАК ВыводитьИтоги) КАК ТаблицаДобавленныеНаборы,
	|	ПриходнаяНакладная.СерииНоменклатуры.(
	|		Серия КАК Серия,
	|		КлючСвязи КАК КлючСвязи) КАК ТаблицаСерииНоменклатуры
	|ИЗ
	|	Документ.ПриходнаяНакладная КАК ПриходнаяНакладная
	|ГДЕ
	|	ПриходнаяНакладная.Ссылка В (&МассивОбъектов)
...
Функция ПечатнаяФормаНакладнаяСРасходами(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Шапка Из ДанныеДокументов Цикл
		...
		ПараметрыПоискаШтрихкодыНоменклатуры.Ссылка = Шапка.Ссылка;
		...
		Для Каждого СтрокаЗапасы Из Шапка.ТаблицаЗапасы Цикл
			...
			ПараметрыПоискаШтрихкодыНоменклатуры.НомерСтроки = СтрокаЗапасы.НомерСтроки;
			НайденныеСтрокиШтрихкоды = ШтрихкодыНоменклатуры.НайтиСтроки(ПараметрыПоискаШтрихкодыНоменклатуры);
			ПечатьДокументовУНФ.ПодобратьШтрихкодНоменклатуры(СтрокаЗапасы, НайденныеСтрокиШтрихкоды);
...
//
// Параметры: 
//  МассивОбъектов - Массив из ДокументСсылка.ПриходнаяНакладная
//  ОбъектыПечати - Структура
// 
// Возвращаемое значение:
//  ТабличныйДокумент - ТабличныйДокумент в который будет выводится печатная форма.
Функция ПечатнаяФормаНакладнаяВРозничныхЦенах(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Запрос = Новый Запрос();
		Запрос.УстановитьПараметр("ТекущийДокумент", ТекущийДокумент); // ДокументСсылка.ПриходнаяНакладная
		Запрос.УстановитьПараметр("ДатаДокумента", КонецДня(ДатыДокументов[ТекущийДокумент]));
		...
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ШапкаДокумента.СтруктурнаяЕдиница.РозничныйВидЦен КАК Справочник.ВидыЦен).ВалютаЦены, ШапкаДокумента.ВалютаДокумента) КАК ВалютаДокумента,
		|	ВЫБОР
		|		КОГДА ШапкаДокумента.СтруктурнаяЕдиница.РозничныйВидЦен = ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыЦен.Оптовая)
		|		ИНАЧЕ ШапкаДокумента.СтруктурнаяЕдиница.РозничныйВидЦен
		|	КОНЕЦ КАК ВидЦены,
		|	ВЫБОР
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втШапкаДокумента.Ссылка КАК Ссылка,
		|	втШапкаДокумента.ДатаДокумента КАК ДатаДокумента,
		|	втШапкаДокумента.Организация КАК Организация,
		|	втШапкаДокумента.Контрагент КАК Контрагент,
		|	втШапкаДокумента.КонтактноеЛицоПодписант КАК КонтактноеЛицоПодписант,
		|	втШапкаДокумента.ДолжностьКладовщика КАК ДолжностьКладовщика,
		|	втШапкаДокумента.РасшифровкаПодписиКладовщика КАК РасшифровкаПодписиКладовщика,
		|	втШапкаДокумента.СуммаВключаетНДС КАК СуммаВключаетНДС,
		|	втШапкаДокумента.ВалютаДокумента КАК ВалютаДокумента,
		|	втШапкаДокумента.ВидЦены КАК ВидЦены,
		|	втШапкаДокумента.ВидОперации КАК ВидОперации,
		|	втШапкаДокумента.ВидОперацииСсылка КАК ВидОперацииСсылка,
		|	втШапкаДокумента.СтруктурнаяЕдиницаТипСтруктурнойЕдиницы КАК СтруктурнаяЕдиницаТипСтруктурнойЕдиницы,
		|	втШапкаДокумента.Номер КАК Номер,
		|	втШапкаДокумента.Префикс КАК Префикс
		|ИЗ
		...
		|				ВидЦен В
		|					(ВЫБРАТЬ
		|						втШапкаДокумента.ВидЦены
		|					ИЗ
		|						втШапкаДокумента КАК втШапкаДокумента)) КАК ЦеныНоменклатурыСрезПоследних
		...
		|		И ПриходнаяНакладнаяЗапасы.Партия = ШтрихкодыНоменклатуры.Партия
		|ГДЕ
		|	ПриходнаяНакладнаяЗапасы.Ссылка = &ТекущийДокумент
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	НомерСтроки,
		...
		ДанныеДокументов = РезультатыЗапроса[1].Выгрузить();
		ТаблицаЗапасы = РезультатыЗапроса[2].Выгрузить();
		ТаблицаСерииНоменклатуры = РезультатыЗапроса[3].Выгрузить();
		ТаблицаДобавленныеНаборы = РезультатыЗапроса[4].Выгрузить();
		ДанныеДокументов.Колонки.Добавить("ТаблицаЗапасы", Новый ОписаниеТипов("ТаблицаЗначений"));
		ДанныеДокументов.Колонки.Добавить("ТаблицаСерииНоменклатуры", Новый ОписаниеТипов("ТаблицаЗначений"));
		ДанныеДокументов.Колонки.Добавить("ТаблицаДобавленныеНаборы", Новый ОписаниеТипов("ТаблицаЗначений"));
		Для каждого Шапка Из ДанныеДокументов Цикл
			СтруктураОтбора = Новый Структура;
		...
		ШтрихкодыНоменклатуры = РезультатыЗапроса[5].Выгрузить();
		ШтрихкодыНоменклатуры.Индексы.Добавить("НомерСтроки");
...
//   МассивОбъектов  - Массив из ДокументСсылка   - Массив ссылок на объекты которые нужно распечатать
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Накладная") Тогда
		...
		ПечатнаяФормаНакладная = ПечатнаяФормаНакладная(МассивОбъектов, ОбъектыПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Накладная", НСтр(
			"ru = 'Приходная накладная'"), ПечатнаяФормаНакладная);
		
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "НакладнаяСРасходами") Тогда
		
		ПечатнаяФормаНакладнаяСРасходами = ПечатнаяФормаНакладнаяСРасходами(МассивОбъектов, ОбъектыПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "НакладнаяСРасходами", НСтр(
			"ru = 'Приходная накладная (с расходами)'"), ПечатнаяФормаНакладнаяСРасходами);
		
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "НакладнаяВРозничныхЦенах") Тогда
		
		ПечатнаяФормаНакладнаяВРозничныхЦенах = ПечатнаяФормаНакладнаяВРозничныхЦенах(МассивОбъектов, ОбъектыПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "НакладнаяВРозничныхЦенах", НСтр(
			"ru = 'Приходная накладная'"), ПечатнаяФормаНакладнаяВРозничныхЦенах);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Использование", Метаданные.Отчеты.РеестрДокументов) Тогда
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "РеестрДокументовПриходнаяНакладная";
...
Функция ОбработатьДанныеШтрихкода(Форма, ДанныеШтрихкода, ПараметрыСканирования,
	...
	Иначе
		...
		ВидПродукцииПоддерживаетОСУ = ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(
			ПараметрыШтрихкодаДляОбработки.ВидПродукцииИС);
		ДействуетОСУПоДатам = ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоОСУ(
			ПараметрыШтрихкодаДляОбработки.ВидПродукцииИС);
		...
		Если ПоддерживаетсяОбъемноСортовойУчет
			И ДанныеШтрихкодаДляОбработки.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар
			И ЗначениеЗаполнено(ДанныеШтрихкодаДляОбработки.ШтрихкодУпаковки) 
			И НЕ ДанныеШтрихкода.ОбработатьБезМаркировки Тогда
		
			ОбработатьДанныеШтрихкодаОбъемноСортовогоУчета(Форма, ДанныеШтрихкода, ДанныеШтрихкодаДляОбработки,
...
Процедура ОбработатьДанныеШтрихкодаОбъемноСортовогоУчета(Форма, ДанныеШтрихкода, ДанныеШтрихкодаДляОбработки,
	ИмяТабличнойЧастиШтрихкодыУпаковок)
	...
	ЗаполнитьЗначенияСвойств(ДанныеШтрихкодаУпаковки, ДанныеШтрихкода,
		"ВидПродукции,Номенклатура,Характеристика,Серия");
	ДанныеШтрихкодаУпаковки.ДатаУпаковки = ТекущаяДатаСеанса();
	...
	ВложенныеШтрихкодыУпаковки.Добавить(ДанныеШтрихкода.ШтрихкодУпаковки);
	
	ДанныеДляРасчетаХешСуммы = Новый СписокЗначений();
	УдаляемыеУпаковкиДокумента = Новый Массив;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеШтрихкода, "ХешСуммаНормализации")
		И ЗначениеЗаполнено(ДанныеШтрихкода.ХешСуммаНормализации) Тогда
		ДанныеДляРасчетаХешСуммы.Добавить(ДанныеШтрихкода.ХешСуммаНормализации);
	Иначе
		ДанныеДляРасчетаХешСуммы.Добавить(ДанныеШтрихкода.Штрихкод);
	КонецЕсли;
	
	ШтрихкодыУпаковокДокумента = Объект[ИмяТабличнойЧастиШтрихкодыУпаковок].Выгрузить(,"ШтрихкодУпаковки");
	КоличествоВложенныхШтрихкодов = 1;
	
	ЗапросДанныеДляРасчета = Новый Запрос;
	ЗапросДанныеДляРасчета.Текст = 
		"ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА СОКРЛП(ШтрихкодыУпаковокТоваров.ХешСуммаНормализации) = """"
		|			ТОГДА ШтрихкодыУпаковокТоваров.ЗначениеШтрихкода
		|		ИНАЧЕ ШтрихкодыУпаковокТоваров.ХешСуммаНормализации
		|	КОНЕЦ КАК ЗначениеДляРасчета,
		...
		|	ЛОЖЬ КАК МультитоварнаяУпаковка,
		|	ШтрихкодыУпаковокТоваров.Ссылка КАК УпаковкаДокумента
		|ИЗ
		|	Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваров
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ШтрихкодыУпаковокДокумента КАК ШтрихкодыУпаковокДокумента
		|		ПО ШтрихкодыУпаковокТоваров.Ссылка = ШтрихкодыУпаковокДокумента.ШтрихкодУпаковки
		|			И (ШтрихкодыУпаковокТоваров.ТипУпаковки = ЗНАЧЕНИЕ(Перечисление.ТипыУпаковок.МаркированныйТовар))
		|			И (СТРНАЙТИ(ШтрихкодыУпаковокТоваров.ЗначениеШтрихкода, &EAN) <> 0)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЕСТЬNULL(ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод, ШтрихкодыУпаковокТоваров.Ссылка),
		|	ВЫБОР
		|		КОГДА НЕ ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод ЕСТЬ NULL
		|				И СОКРЛП(ШтрихкодыУпаковокТоваровРасшифровка.ХешСуммаНормализации) = """"
		|			ТОГДА ШтрихкодыУпаковокТоваровРасшифровка.ЗначениеШтрихкода
		|		КОГДА НЕ ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод ЕСТЬ NULL
		|				И СОКРЛП(ШтрихкодыУпаковокТоваровРасшифровка.ХешСуммаНормализации) <> """"
		|			ТОГДА ШтрихкодыУпаковокТоваровРасшифровка.ХешСуммаНормализации
		|		КОГДА СОКРЛП(ШтрихкодыУпаковокТоваров.ХешСуммаНормализации) = """"
		|			ТОГДА ШтрихкодыУпаковокТоваров.ЗначениеШтрихкода
		|		ИНАЧЕ ШтрихкодыУпаковокТоваров.ХешСуммаНормализации
		|	КОНЕЦ,
		|	ЕСТЬNULL(ШтрихкодыУпаковокТоваровРасшифровка.Номенклатура, ШтрихкодыУпаковокТоваров.Номенклатура),
		|	ЕСТЬNULL(ШтрихкодыУпаковокТоваровРасшифровка.Характеристика, ШтрихкодыУпаковокТоваров.Характеристика),
		|	ЕСТЬNULL(ШтрихкодыУпаковокТоваровРасшифровка.Серия, ШтрихкодыУпаковокТоваров.Серия),
		|	ЕСТЬNULL(ШтрихкодыУпаковокТоваровРасшифровка.Количество, ШтрихкодыУпаковокТоваров.Количество),
		|	ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод ЕСТЬ NULL
		|			И ШтрихкодыУпаковокТоваров.ТипУпаковки = ЗНАЧЕНИЕ(Перечисление.ТипыУпаковок.МультитоварнаяУпаковка)
		|		ИЛИ НЕ ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод ЕСТЬ NULL
		|			И ШтрихкодыУпаковокТоваровРасшифровка.ТипУпаковки = ЗНАЧЕНИЕ(Перечисление.ТипыУпаковок.МультитоварнаяУпаковка),
		...
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваровРасшифровка
		|			ПО ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Штрихкод = ШтрихкодыУпаковокТоваровРасшифровка.Ссылка
		|		ПО ШтрихкодыУпаковокТоваров.Ссылка = ШтрихкодыУпаковокТоваровВложенныеШтрихкоды.Ссылка";
	
	ЗапросДанныеДляРасчета.УстановитьПараметр("EAN", ДанныеШтрихкода.EAN);
	ЗапросДанныеДляРасчета.УстановитьПараметр("ШтрихкодыУпаковок", ШтрихкодыУпаковокДокумента);
	ВыборкаДанныеДляРасчета = ЗапросДанныеДляРасчета.Выполнить().Выбрать();
	
	Пока ВыборкаДанныеДляРасчета.Следующий() Цикл
		ДанныеДляРасчетаХешСуммы.Добавить(ВыборкаДанныеДляРасчета.ЗначениеДляРасчета);
		ВложенныеШтрихкодыУпаковки.Добавить(ВыборкаДанныеДляРасчета.Ссылка);
		КоличествоВложенныхШтрихкодов = КоличествоВложенныхШтрихкодов + ВыборкаДанныеДляРасчета.Количество;
		Если ВыборкаДанныеДляРасчета.МультитоварнаяУпаковка Тогда
			ДанныеШтрихкодаУпаковки.ТипУпаковки = Перечисления.ТипыУпаковок.МультитоварнаяУпаковка;
		КонецЕсли;
		Если ДанныеШтрихкодаУпаковки.Номенклатура <> ВыборкаДанныеДляРасчета.Номенклатура Тогда
			ДанныеШтрихкодаУпаковки.ТипУпаковки = Перечисления.ТипыУпаковок.МультитоварнаяУпаковка;
			ДанныеШтрихкодаУпаковки.Номенклатура = Справочники.Номенклатура.ПустаяСсылка();
		КонецЕсли;
		Если ДанныеШтрихкодаУпаковки.Характеристика <> ВыборкаДанныеДляРасчета.Характеристика Тогда
			ДанныеШтрихкодаУпаковки.ТипУпаковки = Перечисления.ТипыУпаковок.МультитоварнаяУпаковка;
			ДанныеШтрихкодаУпаковки.Характеристика = Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
		КонецЕсли;
		Если (ЗначениеЗаполнено(ДанныеШтрихкодаУпаковки.Серия) И НЕ ЗначениеЗаполнено(ВыборкаДанныеДляРасчета.Серия))
			ИЛИ (НЕ ЗначениеЗаполнено(ДанныеШтрихкодаУпаковки.Серия) И ЗначениеЗаполнено(ВыборкаДанныеДляРасчета.Серия))
			ИЛИ ((ЗначениеЗаполнено(ДанныеШтрихкодаУпаковки.Серия) И ЗначениеЗаполнено(ВыборкаДанныеДляРасчета.Серия)
				И ДанныеШтрихкодаУпаковки.Серия <> ВыборкаДанныеДляРасчета.Серия)) Тогда
			
			ДанныеШтрихкодаУпаковки.ТипУпаковки = Перечисления.ТипыУпаковок.МультитоварнаяУпаковка;
			ДанныеШтрихкодаУпаковки.Серия = Неопределено;
		КонецЕсли;
		Если УдаляемыеУпаковкиДокумента.Найти(ВыборкаДанныеДляРасчета.УпаковкаДокумента) = Неопределено Тогда
			УдаляемыеУпаковкиДокумента.Добавить(ВыборкаДанныеДляРасчета.УпаковкаДокумента);
		КонецЕсли;
	КонецЦикла;
	ДанныеШтрихкодаУпаковки.Количество = КоличествоВложенныхШтрихкодов;
	ДанныеШтрихкодаУпаковки.ХешСумма = Справочники.ШтрихкодыУпаковокТоваров.ХешСуммаСодержимогоУпаковки(
		ДанныеДляРасчетаХешСуммы);
	ДанныеШтрихкодаУпаковки.Штрихкод = СтрШаблон("(02)%1(37)%2",
		ДанныеШтрихкода.GTIN, КоличествоВложенныхШтрихкодов);
	
	ЗапросШтрихкодаУпаковки = Новый Запрос;
	ЗапросШтрихкодаУпаковки.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ШтрихкодыУпаковокТоваров.Ссылка КАК Ссылка,
		...
		|	И ШтрихкодыУпаковокТоваров.ХешСумма = &ХешСумма
		...
		|	И ШтрихкодыУпаковокТоваров.Количество = &Количество
		|	И ШтрихкодыУпаковокТоваров.ТипУпаковки = &ТипУпаковки
		|
		|УПОРЯДОЧИТЬ ПО
		|	ШтрихкодыУпаковокТоваров.ДатаУпаковки УБЫВ";
	
	ЗапросШтрихкодаУпаковки.УстановитьПараметр("ЗначениеШтрихкода", ДанныеШтрихкодаУпаковки.Штрихкод);
	ЗапросШтрихкодаУпаковки.УстановитьПараметр("ХешСумма", ДанныеШтрихкодаУпаковки.ХешСумма);
	
	ЗапросШтрихкодаУпаковки.УстановитьПараметр("Номенклатура", ДанныеШтрихкодаУпаковки.Номенклатура);
	ЗапросШтрихкодаУпаковки.УстановитьПараметр("Характеристика", ДанныеШтрихкодаУпаковки.Характеристика);
	ЗапросШтрихкодаУпаковки.УстановитьПараметр("Серия", ДанныеШтрихкодаУпаковки.Серия);
	ЗапросШтрихкодаУпаковки.УстановитьПараметр("Количество", ДанныеШтрихкодаУпаковки.Количество);
	ЗапросШтрихкодаУпаковки.УстановитьПараметр("ТипУпаковки", ДанныеШтрихкодаУпаковки.ТипУпаковки);
	ВыборкаШтрихкодаУпаковки = ЗапросШтрихкодаУпаковки.Выполнить().Выбрать();
	
	НайденнаяУпаковка = Неопределено;
	Пока ВыборкаШтрихкодаУпаковки.Следующий() Цикл
		НайденнаяУпаковка = ВыборкаШтрихкодаУпаковки.Ссылка;
		ДанныеШтрихкодаУпаковки.ТипШтрихкода = ВыборкаШтрихкодаУпаковки.ТипШтрихкода;
	КонецЦикла;
	
	Если НайденнаяУпаковка = Неопределено Тогда
		КэшМаркируемойПродукции = ПолучитьИзВременногоХранилища(Форма.КэшМаркируемойПродукции);
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КэшМаркируемойПродукции,
			"СозданныеШтрихкодыУпаковокОСУ") Тогда
			
			СозданныеШтрихкодыУпаковокОСУ = КэшМаркируемойПродукции.СозданныеШтрихкодыУпаковокОСУ;
			НайденнаяУпаковка = СозданныеШтрихкодыУпаковокОСУ.Получить(ДанныеШтрихкода.GTIN);
			Если НЕ ОбщегоНазначения.СсылкаСуществует(НайденнаяУпаковка) Тогда
				НайденнаяУпаковка = Неопределено;
				СозданныеШтрихкодыУпаковокОСУ.Удалить(ДанныеШтрихкода.GTIN);
			КонецЕсли;
		Иначе
			СозданныеШтрихкодыУпаковокОСУ = Новый Соответствие;
		КонецЕсли;
		Если НайденнаяУпаковка = Неопределено Тогда
			...
			ПараметрыЗаписи.Вставить("ВидПродукции", ДанныеШтрихкодаУпаковки.ВидПродукции);
			
			НайденнаяУпаковка = Справочники.ШтрихкодыУпаковокТоваров.СоздатьШтрихкодУпаковки(ДанныеШтрихкодаУпаковки,
				ВложенныеШтрихкодыУпаковки, ПараметрыЗаписи,, Новый Структура("ОтключитьПоискСуществующего", Истина));
			Если НайденнаяУпаковка <> Неопределено Тогда
				СозданныеШтрихкодыУпаковокОСУ.Вставить(ДанныеШтрихкода.GTIN, НайденнаяУпаковка);
				КэшМаркируемойПродукции.Вставить("СозданныеШтрихкодыУпаковокОСУ", СозданныеШтрихкодыУпаковокОСУ);
				Форма.КэшМаркируемойПродукции = ПоместитьВоВременноеХранилище(КэшМаркируемойПродукции,
					Форма.КэшМаркируемойПродукции);
			КонецЕсли;
		Иначе
			ИзменяемыеРеквизиты = Новый Структура;
			ИзменяемыеРеквизиты.Вставить("Номенклатура", ДанныеШтрихкодаУпаковки.Номенклатура);
			ИзменяемыеРеквизиты.Вставить("Характеристика", ДанныеШтрихкодаУпаковки.Характеристика);
			ИзменяемыеРеквизиты.Вставить("Серия", ДанныеШтрихкодаУпаковки.Серия);
			ИзменяемыеРеквизиты.Вставить("Количество", ДанныеШтрихкодаУпаковки.Количество);
			ИзменяемыеРеквизиты.Вставить("ТипУпаковки", ДанныеШтрихкодаУпаковки.ТипУпаковки);
			ИзменяемыеРеквизиты.Вставить("ХешСумма", ДанныеШтрихкодаУпаковки.ХешСумма);
			ИзменяемыеРеквизиты.Вставить("ЗначениеШтрихкода", ДанныеШтрихкодаУпаковки.Штрихкод);
			
			Справочники.ШтрихкодыУпаковокТоваров.ИзменитьШтрихкодУпаковки(НайденнаяУпаковка,
				ИзменяемыеРеквизиты, ВложенныеШтрихкодыУпаковки);
		КонецЕсли;
	КонецЕсли;
	
	Если НайденнаяУпаковка <> Неопределено Тогда
		ПараметрыОтбора = Новый Структура;
		ПараметрыОтбора.Вставить("ШтрихкодУпаковки");
		Для Каждого УдаляемаяУпаковкаДокумента Из УдаляемыеУпаковкиДокумента Цикл
			ПараметрыОтбора.ШтрихкодУпаковки = УдаляемаяУпаковкаДокумента;
			НайденныеСтрокиУпаковок = Объект[ИмяТабличнойЧастиШтрихкодыУпаковок].НайтиСтроки(ПараметрыОтбора);
			Для Каждого НайденнаяСтрокаУпаковок Из НайденныеСтрокиУпаковок Цикл
				Объект[ИмяТабличнойЧастиШтрихкодыУпаковок].Удалить(НайденнаяСтрокаУпаковок);
			КонецЦикла;
		КонецЦикла;
		ЗаполнитьЗначенияСвойств(ДанныеШтрихкодаДляОбработки, ДанныеШтрихкодаУпаковки,,
			"Количество, Серия, Характеристика, Номенклатура");
		ДанныеШтрихкодаДляОбработки.ШтрихкодУпаковки = НайденнаяУпаковка;
	КонецЕсли;
...
//
// Параметры:
// ТекущийДокумент -  ДокументСсылка - документ, сумму НДС которого нужно вычислить
//
// Возвращаемое значение:
...
Функция НаличиеСуммыНДС(ТекущийДокумент) Экспорт
	...
	"ВЫБРАТЬ
	|    ВЫБОР
	|        КОГДА ПриходнаяНакладная.СуммаНДС > 0
	|        ИЛИ ПриходнаяНакладная.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
	|            ТОГДА 1
	|        ИНАЧЕ 0
	|    КОНЕЦ КАК СуммаНДС,
	|    ПриходнаяНакладная.Ссылка
...
//
// Параметры:
//   МассивДокументов - Массив из ДокументСсылка - массив документов, по которым формируется печатная форма.
//
// Возвращаемое значение:
//   Массив из ДокументСсылка - документы, для печати которых не требуется выписка счета-фактуры.
Функция СчетаФактурыНеТребуются(МассивДокументов) Экспорт
    ...
    Запрос.Текст = 
      ...
      |    Документ.ПриходнаяНакладная.Запасы КАК ПриходнаяНакладная
      |ГДЕ
      |    ПриходнаяНакладная.Ссылка В (&МассивДокументов)
      |    И ПриходнаяНакладная.СуммаНДС > 0
      |
      |ОБЪЕДИНИТЬ
      |
      |ВЫБРАТЬ РАЗЛИЧНЫЕ
      ...
      |ГДЕ
      |    ПриходнаяНакладная.Ссылка В (&МассивДокументов)
      |    И ПриходнаяНакладная.Договор.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоров.СКомитентом)
      |;
      |
      |////////////////////////////////////////////////////////////////////////////////
      |ВЫБРАТЬ
      ...
      |        ПО ВТ_ДокументыСНДС.Ссылка = ПриходнаяНакладная.Ссылка
      |ГДЕ
      |    ПриходнаяНакладная.Ссылка В (&МассивДокументов)
      |    И ПриходнаяНакладная.Договор.ВидДоговора <> ЗНАЧЕНИЕ(Перечисление.ВидыДоговоров.СКомитентом)";
