...
Процедура ЗаполнитьПоСтруктуре(ДанныеЗаполнения) Экспорт
	...
	// заполнение завершающего подразделения
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЭтапыПроизводства") 
		И ПолучитьФункциональнуюОпцию("ВыполнениеЭтаповРазнымиПодразделениями") Тогда
		
		ТипСтруктурнойЕдиницы 	= ?(ЗначениеЗаполнено(СтруктурнаяЕдиница),
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктурнаяЕдиница, "ТипСтруктурнойЕдиницы"),
			Неопределено);
		ТипПодразделение		= ПредопределенноеЗначение("Перечисление.ТипыСтруктурныхЕдиниц.Подразделение");	
			
		ПодразделениеЭтапа 		= ПроизводствоСервер.ПодразделениеЭтапа();
		ПодразделениеЭтапа 		= ?(Не ЗначениеЗаполнено(ПодразделениеЭтапа) И ТипСтруктурнойЕдиницы = ТипПодразделение,
			СтруктурнаяЕдиница,
			ПодразделениеЭтапа);
		...
		Для каждого СтрокаТабличнойЧасти Из Продукция Цикл
			...
			Если СпецификацииСЭтапами.Найти(СтрокаТабличнойЧасти.Спецификация) <> Неопределено Тогда 
				СтрокаТабличнойЧасти.ПодразделениеЗавершающегоЭтапа = ПодразделениеЭтапа;
			КонецЕсли; 
...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДанныеЗаполнения) Экспорт
	...
	|		И
	|			(ЗаказНаПроизводство.СтруктурнаяЕдиница.ИсточникПеремещения.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|		ИЛИ
	|			ЗаказНаПроизводство.СтруктурнаяЕдиница.ИсточникПеремещения.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение))
	...
	|		ВЫБОР
	|			КОГДА
	|				ЗаказНаПроизводство.Запасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
	...
	|		ЗаказПокупателя КАК ЗаказПокупателя) КАК Запасы,
	|	ЗаказНаПроизводство.СтруктурнаяЕдиницаПродукции
	|ИЗ
...
Процедура ПроверитьЗаполнениеЭтаповПроизводства(Отказ, ОтменаПроведения = Ложь)
	...
	ТаблицаКонтроляДублей.Свернуть("ЗаказПокупателя, ЗаказНаПроизводство, Номенклатура, Характеристика, Партия,
