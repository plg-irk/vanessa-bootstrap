...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НоменклатураВДокументахСервер.ОбновитьУсловноеОформлениеТабличнойЧастиДляХарактеристик(ЭтотОбъект, СоответствиеТЧ);
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
		Документы.ЗаказНаПроизводство.ПараметрыУказанияСерий(Объект));
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОткрытИзПланировщика И ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Если Не Элементы.Найти("ТЧРесурсыПредприятия") = Неопределено Тогда
			ТекущийЭлемент = Элементы.ТЧРесурсыПредприятия;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПодборПроизведен" 
		...
		Для Каждого НомерСтроки Из МассивНомеровИдентичныхСтрок Цикл
			...
			Если ИмяТабличнойЧасти = "Запасы" И РежимОстаткиИРезервы Тогда
...
Процедура ВидОперацииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого СтрокаПродукция Из Объект.Продукция Цикл
		...
		ТекстСообщения = СтрШаблон(НСтр(
			...
			СтрокаПродукция.ПолучитьИдентификатор(), СтрокаПродукция.Номенклатура);
		КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Продукция", СтрокаПродукция.ПолучитьИдентификатор(),
			"Номенклатура");
...
Процедура ПродукцияНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ВыбранноеЗначение.Свойство("СтруктураНастроек") Тогда
			НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтотОбъект, "ЗаказНаПроизводство", НастройкиФормыВыбораНоменклатуры, Истина, "Продукция", ВыбранноеЗначение.СтруктураНастроек);
...
Процедура ЗапасыНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ВыбранноеЗначение.Свойство("СтруктураНастроек") Тогда
			НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтотОбъект, "ЗаказНаПроизводство", НастройкиФормыВыбораНоменклатуры, Истина, "Запасы", ВыбранноеЗначение.СтруктураНастроек);
		КонецЕсли;
...
Процедура ОстаткиИРезервы(Команда)
	...
	Если РежимОстаткиИРезервы Тогда
		...
		Если СкладВШапке И НЕ ТолькоПросмотр Тогда
	...
	Если Результат.Свойство("ПоложениеСтруктурнойЕдиницыВСдельномНаряде") 
		...
		Если Результат.ПоложениеСтруктурнойЕдиницыВСдельномНаряде <> Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
			...
			Если НЕ ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаОпераций) Тогда
				Объект.СтруктурнаяЕдиницаОпераций = ПерсональныеНастройкиРУНФ.Прочитать("ОсновноеПодразделение"); 
			КонецЕсли; 
		КонецЕсли;
		Для каждого СтрокаТабличнойЧасти Из Объект.Операции Цикл
			СтрокаТабличнойЧасти.СтруктурнаяЕдиница = Объект.СтруктурнаяЕдиницаОпераций;
...
Процедура ПродукцияПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПодбор") И ДополнительныеПараметры.ЭтоПодбор Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Результат);
		МассивНомеровИдентичныхСтрок = Новый Массив;
		ПолучитьЗапасыИзХранилища(ДополнительныеПараметры.АдресЗапасовВХранилище, "Продукция", Истина, Ложь, МассивНомеровИдентичныхСтрок);
...
Функция СоответствиеДанныхРесурсов()
	...
	Для каждого СтрокаТаблицы Из СвернутаяТаблицаРесурсов Цикл
		...
		СтруктураРеквизитовРесурсПредприятия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РесурсПредприятия,
		...
		СтруктураДанных = Новый Структура("КонтролироватьШаг,КратностьПланирования"
											,СтруктураРеквизитовРесурсПредприятия.КонтролироватьШагИнтервалаВДокументах, 
											СтруктураРеквизитовРесурсПредприятия.КратностьПланирования);
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные, ИмяТЧ = "Запасы")
	...
	РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураДанные.Номенклатура, "ЕдиницаИзмерения,
	|	ЭтоНабор, ТипНоменклатуры, Склад, ИспользоватьХарактеристики, ПроверятьЗаполнениеХарактеристики, ИспользоватьПартии,
	|	ПроверятьЗаполнениеПартий");
	
	СтруктураДанные.Вставить("ЕдиницаИзмерения", РеквизитыНоменклатуры.ЕдиницаИзмерения);
	// Наборы
	СтруктураДанные.Вставить("ЭтоНабор", РеквизитыНоменклатуры.ЭтоНабор);
	
	
	СтруктураДанные.Вставить("ТипНоменклатуры", РеквизитыНоменклатуры.ТипНоменклатуры);
	...
	ИначеЕсли Не ЗначениеЗаполнено(СтруктураДанные.Склад) Тогда
		СтруктураДанные.Склад = РеквизитыНоменклатуры.Склад;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураДанные.Номенклатура) И РеквизитыНоменклатуры.ИспользоватьПартии
		Тогда
...
ПолучательПеремещения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "ПолучательПеремещения");
ТипСтруктурнойЕдиницы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПолучательПеремещения, "ТипСтруктурнойЕдиницы");
	Если ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад
		Или ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда

		Возврат ПолучательПеремещения;
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Если МассивДанныхШтрихкода <> Неопределено
			...
			Для Каждого ДанныеШтрихкода Из МассивДанныхШтрихкода Цикл
				...
				СтруктураДанныеНоменклатуры.Вставить("ТипНоменклатуры", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ДанныеШтрихкода.Номенклатура, "ТипНоменклатуры"));
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьСкладВТЧ(Форма)
...
Процедура УправлениеФормой(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыИзменитьРезервЗаполнитьПоОстаткамБезРазбиения", "Доступность", НЕ Форма.ТолькоПросмотр);
...
Процедура ОбновитьОтображениеКолонокВРазрезеЗапасов(Форма)
	...
	Если Форма.РежимОстаткиИРезервы Тогда
		...
		Элементы.ЗапасыСтруктурнаяЕдиницаОстатки.Видимость = Форма.РазрешитьСкладыВТабличныхЧастях И НЕ ЭтоРазборка;
		
		Если Форма.РезервированиеЗапасов Тогда
	...
	Иначе
		...
		Если Объект.ПоложениеЗаказаПокупателя = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке")
			...
			Элементы.ЗапасыРезерв.Видимость = Ложь;
		Иначе
			Элементы.ЗапасыРезерв.Видимость = Форма.РезервированиеЗапасов;
		КонецЕсли;
		
		Элементы.ЗапасыСтруктурнаяЕдиницаТабличнаяЧасть.Видимость = НЕ Форма.СкладВШапке И НЕ ЭтоРазборка 
			И (Форма.РезервированиеЗапасов ИЛИ Объект.УчетПотребностиПоСкладам);
		Элементы.ЗапасыКонтекстноеМенюГруппаРаботаССтрокамиОбновить.Видимость = Ложь;
		
	КонецЕсли;
	
	Элементы.ГруппаЗаполнитьПоВсемСкладам.Видимость = Не Форма.СкладВШапке Или Форма.РежимОстаткиИРезервы;
	Элементы.ЗапасыИзменитьРезервЗаполнитьПоОстаткам.Видимость = Форма.РазрешитьСкладыВТабличныхЧастях;
