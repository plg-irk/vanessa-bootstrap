...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ОтчетКомиссионера")
		...
		Если Не ВалютаДокумента = ВалютаУчета Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаЗапасовЦена", "Видимость", Ложь);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаЗапасовСумма", "Видимость", Ложь);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаЗапасовОстаток", "Видимость", Ложь);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаЗапасовЦенаПередачи", "Видимость", Ложь);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаЗапасовСуммаРасчетов", "Видимость", Ложь);
		КонецЕсли;
...
Процедура ЗаполнитьТаблицуЗапасов()
	...
	Запрос.УстановитьПараметр("Организация", Константы.УчетПоКомпании.Компания(ОтборОрганизация));
...
Процедура ЗаполнитьТаблицуЗапасовОВозвратах()
	...
	Если Не Проведен Тогда
		...
		|		ИНАЧЕ ЕСТЬNULL(ПродажиОбороты.СуммаОборот * КурсВалютыРасчетов.Курс * КурсВалютыДокумента.Кратность / (КурсВалютыДокумента.Курс * КурсВалютыРасчетов.Кратность), 0)
		|	КОНЕЦ КАК Сумма,
		...
		|	РегистрСведений.КурсыВалют.СрезПоследних(&ДатаОбработки, Валюта = &ВалютаДокумента) КАК КурсВалютыДокумента,
		|	РегистрСведений.КурсыВалют.СрезПоследних(&ДатаОбработки, Валюта = &ВалютаУчета) КАК КурсВалютыРасчетов
		|ГДЕ
		...
		|								ТОГДА ЗапасыПереданные.СуммаРасчетов
		|							ИНАЧЕ ЕСТЬNULL(ЗапасыПереданные.СуммаРасчетов * КурсВалютыРасчетов.Курс * КурсВалютыДокумента.Кратность / (КурсВалютыДокумента.Курс * КурсВалютыРасчетов.Кратность), 0)
		|						КОНЕЦ / ЗапасыПереданные.Количество КАК ЧИСЛО(15, 2))
		|			ИНАЧЕ ВЫБОР
		|					КОГДА &ВалютаДокумента = &ВалютаУчета
		|						ТОГДА ЗапасыПереданные.СуммаРасчетов
		|					ИНАЧЕ ЕСТЬNULL(ЗапасыПереданные.СуммаРасчетов * КурсВалютыРасчетов.Курс * КурсВалютыДокумента.Кратность / (КурсВалютыДокумента.Курс * КурсВалютыРасчетов.Кратность), 0)
	...
	Если ЗначениеЗаполнено(ОтчетКомиссионера) Тогда
		Запрос.УстановитьПараметр("ДатаОбработки", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтчетКомиссионера, "Дата"));
