...
Функция ПодготовитьТаблицуЗаказов()
	...
	ВестиРасчетыПоЗаказам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ВестиРасчетыПоЗаказам") = Истина;
...
Процедура СформироватьСчетаФактурыПокупателиНовыйВидДокумента(СсылкаНового, ИмяТабличнойЧастиПокупатели = "Покупатели") 
	...
	СчетФактурыПолученные = ЭтоВозврат И Не НаВозвратыВыставляетсяКорректировочныйСчетФактура;
	СчетФактурыКорректировки = ЭтоВозврат И НаВозвратыВыставляетсяКорректировочныйСчетФактура;
	...
	СтруктураВозвратаСуммСчетФактуры = Новый Структура("СуммаДокументаОснования, СуммаНДСДокументаОснования");
	ПараметрыОтбораКлючСвязи = Новый Структура("КлючСвязи");
	ПараметрыОтбораКлючСтроки = Новый Структура("КлючСтроки");
	...
	Для каждого СтрокаТабличнойЧасти Из ЭтотОбъект[ИмяТабличнойЧастиПокупатели] Цикл
		...
		Если СчетФактурыПолученные Тогда
			ПараметрыОтбора.Вставить("НомерВходящегоДокумента", СтрокаТабличнойЧасти.НомерСФ);
		ИначеЕсли СчетФактурыКорректировки Тогда
		...
		Если РезультатПоиска.Количество() = 0 Тогда
			...
			Если Не ЭтоВозврат Или СчетФактурыКорректировки Тогда
				ДокСчетФактура = Документы.СчетФактура.СоздатьДокумент();
		...
		ПараметрыОтбораКлючСвязи.Вставить("КлючСвязи", СтрокаТабличнойЧасти.КлючСвязи);
		РезультатПоиска = Запасы.НайтиСтроки(ПараметрыОтбораКлючСвязи);
		ИсходныйДокумент = Новый Структура("Номер, Дата");
		Если СчетФактурыКорректировки Тогда
			ПараметрыОтбораКлючСтроки.КлючСтроки = СтрокаТабличнойЧасти.КлючСвязи;
			ИсходныеДокументы = ВозвратыИсходныеДокументы.НайтиСтроки(ПараметрыОтбораКлючСтроки);
			Для Каждого СтрИсходный Из ИсходныеДокументы Цикл
				Если ЗначениеЗаполнено(СтрИсходный.ИсходныйДокумент) Тогда
					ДатаНомер = Документы.СчетФактура.ДатаНомерДляПечати(СтрИсходный.ИсходныйДокумент);
					ЗаполнитьЗначенияСвойств(ИсходныйДокумент, ДатаНомер);
				Иначе
					ИсходныйДокумент.Номер = СтрИсходный.НомерВходящегоИсходногоДокумента;
					ИсходныйДокумент.Дата = СтрИсходный.ДатаВходящегоИсходногоДокумента;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Если Не ЭтоВозврат Или СчетФактурыКорректировки Тогда
			...
			СтруктураДанных.Вставить("Кратность", Кратность);
			СтруктураДанных.Вставить("Запасы", РезультатПоиска);
			СтруктураДанных.Вставить("Корректировочный", СчетФактурыКорректировки);
			СтруктураДанных.Вставить("Номер", СтрокаТабличнойЧасти.НомерСФ);
			СтруктураДанных.Вставить("НомерИсходногоДокумента", ИсходныйДокумент.Номер);
			СтруктураДанных.Вставить("ДатаИсходногоДокумента", ИсходныйДокумент.Дата);
		...
		Иначе
			...
			ДокСчетФактура.Кратность = Кратность;
			ДокСчетФактура.УдалитьОбновлениеЗавершено = Истина;
			ДокСчетФактура.КодВидаОперации = "01";
		...
		Если ДокСчетФактура.ЭтоНовый() ИЛИ НЕ ДокСчетФактура.Проведен Тогда
			ДокСчетФактура.Записать();
		Иначе
			ДокСчетФактура.Записать(РежимЗаписиДокумента.Проведение);
		КонецЕсли;
...
Процедура СформироватьСчетаФактурыСводно(СсылкаНового)
	...
	Для каждого СтрокаТабличнойЧасти Из Покупатели Цикл
		...
		Если КлючСвязиСводно.Количество() > 1 Тогда
...
Процедура ОбработкаПодчиненныхСчетФактур(Провести = Истина)
	...
	Для каждого СтрокаРезультата Из РезультатЗапроса Цикл
		...
		Иначе
			...
			РежимЗаписи = ?(СостояниеПроведен, РежимЗаписиДокумента.Проведение, РежимЗаписиДокумента.Запись);
			ТекстСообщения = "";
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого Строка Из Предоплата Цикл
		...
		Если ИспользоватьНовыйВидДокумента Тогда
			...
			Если НайденнаяСтрокаПокупатели = Неопределено Тогда
				ТекстСообщения = СтрШаблон(НСтр(
				"ru = 'Отчет комиссионера в строке %1 таблицы ""Оплата"" не найден в табличной части ""Покупатели возвраты"".'"),
				Строка.НомерСтроки);
		...
		Если НайденнаяСтрокаПокупатели = Неопределено Тогда
			ТекстСообщения = СтрШаблон(НСтр(
				"ru = 'Отчет комиссионера в строке %1 таблицы ""Оплата"" не найден в табличной части ""Покупатели"".'"),
				Строка.НомерСтроки);
			КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Предоплата", Строка.НомерСтроки,
				"ОтчетКомиссионера");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, КонтекстноеПоле, , Отказ);
	...
	Если ИспользоватьНовыйВидДокумента Тогда 
		...
		Для Каждого СтрокаПредоплата Из Предоплата Цикл
			Если Не ЗначениеЗаполнено(СтрокаПредоплата.ВидЗачетаПредоплаты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не заполнен вид операции предоплаты в строке %1 таблицы ""Оплата""'"),
			...
			Если СтрокаПредоплата.ВидЗачетаПредоплаты = Перечисления.ВидыЗачетаПредоплаты.Возврат
				...
				НомерСтрокиРазделеннойТЧПредоплата =
					Документы.ОтчетКомиссионера.НомерСтрокиРазделеннойТЧПредоплата(Предоплата, СтрокаПредоплата);
				ТекстСообщения = СтрШаблон(НСтр(
					"ru = 'Не выбран отчет комиссионера для операции возврата в строке %1 таблицы ""Оплата возвраты""'"),
					НомерСтрокиРазделеннойТЧПредоплата);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ПредоплатаВозвраты",
					СтрокаПредоплата.НомерСтроки, "ОтчетКомиссионера");
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, КонтекстноеПоле, , Отказ);
			КонецЕсли;
	...
	Если ИспользоватьНовыйВидДокумента И ИспользоватьНовуюСхемуДвижений Тогда
		Для Каждого СтрокаПредоплата Из Предоплата Цикл
			Если СтрокаПредоплата.ВидЗачетаПредоплаты = Перечисления.ВидыЗачетаПредоплаты.Продажа Тогда
				ИмяТаблицы = НСтр("ru = '""Оплата""'");
			Иначе
				ИмяТаблицы = НСтр("ru = '""Оплата возвраты""'");
			КонецЕсли;
			
			ВестиРасчетыПоДокументам =
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ВестиРасчетыПоДокументам") = Истина;
			Если ВестиРасчетыПоДокументам И Не ЗначениеЗаполнено(СтрокаПредоплата.Документ) Тогда
				НомерСтрокиРазделеннойТЧПредоплата =
					Документы.ОтчетКомиссионера.НомерСтрокиРазделеннойТЧПредоплата(Предоплата, СтрокаПредоплата);
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не заполнена колонка ""Документ"" в строке %1 таблицы %2'"),
					НомерСтрокиРазделеннойТЧПредоплата,
					ИмяТаблицы);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Предоплата", СтрокаПредоплата.НомерСтроки,
					"Документ");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПроведениеДокументовУНФ.ОтразитьДвижения("НДСАвансыПоДоговорамКомиссии", ТаблицыДляДвижений, Движения, Отказ);
