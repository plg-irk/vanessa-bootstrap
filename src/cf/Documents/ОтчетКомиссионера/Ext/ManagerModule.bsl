...
//
// Параметры:
//   ИмяТабличнойЧастиЗапасы - Строка - название табличной части
//   
//	СтрокаТаблицы - СтрокаТаблицыЗначений
//   
//	СтрокаТабличнойЧасти - СтрокаТабличнойЧасти
//   
//	ЕстьКолонкаДоплата - Булево - Вспомогательные параметры.
//   
//	ЕстьКолонкаДоплатаВозвращено - Булево - Вспомогательные параметры.
Процедура ЗаполнитьЦенуИСуммуПоФорматуOzon(ИмяТабличнойЧастиЗапасы, СтрокаТаблицы, СтрокаТабличнойЧасти,
	ЕстьКолонкаДоплата = Ложь, ЕстьКолонкаДоплатаВозвращено = Ложь) Экспорт
	...
	Если ИмяТабличнойЧастиЗапасы <> "ЗапасыВозвраты" Тогда 
		...
		Если ЕстьКолонкаДоплата И ЗначениеЗаполнено(СтрокаТаблицы.Доплата) Тогда
			СтрокаТабличнойЧасти.Сумма 	= СтрокаТабличнойЧасти.Сумма + СтрокаТаблицы.Доплата;
			СтрокаТабличнойЧасти.Цена 	= СтрокаТабличнойЧасти.Сумма / СтрокаТабличнойЧасти.Количество;
		КонецЕсли;
	...
	Иначе 
		...
		Если ЕстьКолонкаДоплатаВозвращено И ЗначениеЗаполнено(СтрокаТаблицы.ДоплатаВозвращено) Тогда
			СтрокаТабличнойЧасти.Сумма 	= СтрокаТабличнойЧасти.Сумма + СтрокаТаблицы.ДоплатаВозвращено;
			СтрокаТабличнойЧасти.Цена 	= СтрокаТабличнойЧасти.Сумма / 
				?(СтрокаТабличнойЧасти.Количество = 0, 1, СтрокаТабличнойЧасти.Количество);
		КонецЕсли;
...
Функция НаличиеСуммыНДС(ТекущийДокумент) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|    ОтчетКомиссионера.СуммаНДСВознаграждения КАК СуммаНДСВознаграждения,
	|    ОтчетКомиссионера.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ТаблицаНДС
	|ИЗ
	|    Документ.ОтчетКомиссионера.Запасы КАК ОтчетКомиссионера
	|ГДЕ
	|    ОтчетКомиссионера.Ссылка = &СсылкаНаДокумент
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|    ОтчетКомиссионераВозвращенные.СуммаНДСВознаграждения,
	|    ОтчетКомиссионераВозвращенные.Ссылка
	|ИЗ
	|    Документ.ОтчетКомиссионера.ЗапасыВозвраты КАК ОтчетКомиссионераВозвращенные
	|ГДЕ
	|    ОтчетКомиссионераВозвращенные.Ссылка = &СсылкаНаДокумент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
// Определяет номер строки разделенной по виду операции ТЧ Предоплата по строке табличной части.
//
// Параметры:
//   Предоплата - ТабличнаяЧасть - Предоплата
//   СтрокаПредоплата - СтрокаТабличнойЧасти - строка табличной части Предоплата
//
// Возвращаемое значение:
//  Число - номер строки разделенной по виду операции ТЧ Предоплата.
//
Функция НомерСтрокиРазделеннойТЧПредоплата(Предоплата, СтрокаПредоплата) Экспорт
	
	НомерСтроки = 0;
	
	Для Каждого СтрокаТЧ Из Предоплата Цикл
		Если СтрокаТЧ.ВидЗачетаПредоплаты = СтрокаПредоплата.ВидЗачетаПредоплаты Тогда
			НомерСтроки = НомерСтроки + 1;
		КонецЕсли;	
		Если СтрокаТЧ = СтрокаПредоплата Тогда
			Прервать;
		КонецЕсли;	
	КонецЦикла;	
...
#Область ФормированиеДанныхДляПроведения
////////////////////////////////////////////////////////////////////////////////
// ПРОВОДКИ
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОтчетКомиссионера, СтруктураДополнительныеСвойства)
	...
	СпособЗачетаПредоплаты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаОтчетКомиссионера,
		"СпособЗачетаПредоплаты");
	
	Если СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто Тогда
			
		ТаблицаРасчетыСПокупателями =
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.Скопировать();
		
		ТаблицаРасчетыСПокупателями.Свернуть("ВидДвижения, ТипРасчетов", "Сумма, СуммаВал");
		ПараметрыОтбора = Новый Структура("ТипРасчетов", Перечисления.ТипыРасчетов.Аванс);
		СтрокиАванса = ТаблицаРасчетыСПокупателями.НайтиСтроки(ПараметрыОтбора);
		
		Для Каждого СтрокаАванса Из СтрокиАванса Цикл
			Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаОтчетКомиссионера, "Контрагент");
			СчетаУчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент,
				"СчетУчетаРасчетовСПокупателем, СчетУчетаАвансовПокупателя");
		
			ПроводкаАванса = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаУправленческий.Добавить();
			ЗаполнитьЗначенияСвойств(ПроводкаАванса, НоваяПроводка);
			Если СтрокаАванса.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
				ПроводкаАванса.СчетДт = СчетаУчета.СчетУчетаРасчетовСПокупателем;
				ПроводкаАванса.СчетКт = СчетаУчета.СчетУчетаАвансовПокупателя;
				ПроводкаАванса.Содержание = СодержанияПроводок.ВосстановлениеАванса();
			Иначе
				ПроводкаАванса.СчетДт = СчетаУчета.СчетУчетаАвансовПокупателя;
				ПроводкаАванса.СчетКт = СчетаУчета.СчетУчетаРасчетовСПокупателем;
				ПроводкаАванса.Содержание = СодержанияПроводок.ЗачетПредоплаты();
			КонецЕсли;	
			ПроводкаАванса.Сумма = СтрокаАванса.Сумма;
			Если ПроводкаАванса.СчетДт.Валютный Тогда
				ПроводкаАванса.СуммаВалДт = СтрокаАванса.СуммаВал; 
			КонецЕсли;	
			Если ПроводкаАванса.СчетКт.Валютный Тогда
				ПроводкаАванса.СуммаВалКт = СтрокаАванса.СуммаВал; 
			КонецЕсли;	
		КонецЦикла;
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений[ИмяТаблицаЗапасы].Количество() - 1 Цикл
		...
		Если КоличествоТребуется > 0  Тогда
			...
			Если СтрокаТаблицаЗапасы.УдержатьКомиссионноеВознаграждение Тогда
				...
				Если СтрокаТаблицаЗапасы.СуммаВознаграждения = 0 Тогда
					Продолжить;
				КонецЕсли;
				...
				Если НЕ СтрокаТаблицаЗапасы.НДСВключатьВСтоимость Тогда
					...
					Если НЕ ЗначениеЗаполнено(СтрокаТаблицаНДС.СчетДт) Тогда
						СтрокаТаблицаНДС.СчетДт = ПланыСчетов.Управленческий.НДСПоПриобретеннымЦенностям;
					КонецЕсли;
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаОтчетКомиссионера, СтруктураДополнительныеСвойства, ИмяТаблицы = "ТаблицаРасчетыСПокупателями") 
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылкаОтчетКомиссионера,
		"СпособЗачетаПредоплаты, Контрагент, Договор");
	...
	Запрос.УстановитьПараметр("СторнированиеДолга", НСтр("ru='Сторнирование обязательств покупателя'"));
	Запрос.УстановитьПараметр("Контрагент", РеквизитыДокумента.Контрагент);
	Запрос.УстановитьПараметр("Договор", РеквизитыДокумента.Договор);

	СпособЗачетаПредоплаты = РеквизитыДокумента.СпособЗачетаПредоплаты;

	СпособЗачетаПредоплатыАвто = СтруктураДополнительныеСвойства.ИспользоватьНовыйВидДокумента
		И СтруктураДополнительныеСвойства.ИспользоватьНовуюСхемуДвижений
		И СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто;
	
	СтруктураДополнительныеСвойства.ДляПроведения.Вставить("СпособЗачетаПредоплатыАвто", СпособЗачетаПредоплатыАвто);
	
	Если Не ВидОперацииВозврат Тогда
		Запрос.Текст = ТекстЗапросаОтчетКомиссионераОПродажахРасчетыСПокупателями();
	Иначе
		Если СпособЗачетаПредоплатыАвто Тогда
			Запрос.Текст = ТекстЗапросаОтчетКомиссионераОВозвратахРасчетыСПокупателямиАвто();
		Иначе
	...
	Иначе // Дополним таблицу данными прочих таблиц
		...
		// Зачет авансов по продажам
		Если СпособЗачетаПредоплатыАвто Тогда
			Авансы = Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("ВТ_ЗачетАвансов").ПолучитьДанные().Выгрузить();
			КЗачету = Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("КЗачету").ПолучитьДанные().Выгрузить();
			СформироватьТаблицаРасчетыСПокупателямиФрагмент(Авансы, КЗачету, СтруктураДополнительныеСвойства);
		КонецЕсли;
	...
	Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.Колонки.Найти("СпособЗачета") =
		Неопределено Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.Колонки.Добавить("СпособЗачета");
	КонецЕсли;
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.ЗаполнитьЗначения(
		СпособЗачетаПредоплаты, "СпособЗачета");
	
КонецПроцедуры // СформироватьТаблицаРасчетыСПокупателями()

Процедура СформироватьТаблицаРасчетыСПокупателямиФрагмент(Авансы, КЗачету, СтруктураДополнительныеСвойства)

	Если Авансы.Количество() = 0 Или КЗачету.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	СтруктураДанныхДляЗачетаАвансов =
		СтруктураДанныхДокументаДляЗачетаАвансов(СтруктураДополнительныеСвойства.ДляПроведения.Ссылка);
	
	ПустойЗаказ = Документы.ЗаказПокупателя.ПустаяСсылка();
	ПараметрыОтбораСтрок = Новый Структура("Заказ", Документы.ЗаказПокупателя.ПустаяСсылка());
	
	ТаблицаДвиженийПоВзаиморасчетам =
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.Скопировать();
	ТаблицаДвиженийПоВзаиморасчетам.Колонки.Добавить("ХозяйственнаяОперация");
	ТаблицаДвиженийПоВзаиморасчетам.Колонки.Добавить("СсылкаНаДокумент");
	ТаблицаДвиженийПоВзаиморасчетам.Очистить();
	
	ПараметрыДокумента = Новый Структура("ЭтоКорректировка, ЭтоЗаказНаряд, ЭтоОтгрузка", Ложь, Ложь, Истина);
	ПараметрыДокумента.Вставить("ТаблицаДвиженийПоВзаиморасчетам", ТаблицаДвиженийПоВзаиморасчетам);
	
	СтруктураДанныхДокумента = Новый Структура("СпособЗачетаПредоплаты",
	...
	СтруктураДополнительныеСвойства.Вставить("СпособЗачетаПредоплаты", СтруктураДанныхДокумента.СпособЗачетаПредоплаты);
	СтруктураДополнительныеСвойства.Вставить("КратностьДокумента", СтруктураДанныхДляЗачетаАвансов.КратностьДокумента);
	СтруктураДополнительныеСвойства.Вставить("КурсДокумента", СтруктураДанныхДляЗачетаАвансов.КурсДокумента);
	СтруктураДополнительныеСвойства.Вставить("ТекстЗачетПредоплаты", СодержанияПроводок.ЗачетПредоплаты());
	СтруктураДополнительныеСвойства.Вставить("ТекстВозникновениеОбязательств",
		СодержанияПроводок.ВозникновениеОбязательствПокупателя());
	...
	Для Каждого СтрокаКЗачету Из КЗачету Цикл
		...
		ПараметрыОтбораСтрок.Заказ = ?(ЗначениеЗаполнено(СтрокаКЗачету.Заказ), СтрокаКЗачету.Заказ, ПустойЗаказ);
		СтрокиАвансовДляЗачета = Авансы.НайтиСтроки(ПараметрыОтбораСтрок);
		
		РасчетыПроведениеДокументов.РаспределитьАванс(СтруктураДанныхДляЗачетаАвансов, СтрокиАвансовДляЗачета,
			СтрокаКЗачету, СтруктураДополнительныеСвойства);
		...
		РасчетыПроведениеДокументов.ДобавитьСтрокуЗадолженности(СтруктураДанныхДляЗачетаАвансов, СтрокаКЗачету,
			СтруктураДополнительныеСвойства);
		
	КонецЦикла;
	
	Для Каждого СтрокаТаблицы Из ТаблицаДвиженийПоВзаиморасчетам Цикл
		Если СтрокаТаблицы.ВидДвижения = ВидДвиженияНакопления.Приход
			И СтрокаТаблицы.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
			Продолжить;
		КонецЕсли;
		НоваяСтрока = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.Добавить();
...
КонецПроцедуры // СформироватьТаблицаРасчетыСПокупателямиФрагмент()
...
Процедура СформироватьТаблицаОплатаСчетовИЗаказов(ДокументСсылкаОтчетКомиссионера, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.СпособЗачетаПредоплатыАвто Тогда
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаРасчетыСПокупателями.Период КАК Период,
		|	ТаблицаРасчетыСПокупателями.Организация КАК Организация,
		|	ТаблицаРасчетыСПокупателями.Заказ КАК СчетНаОплату,
		|	0 КАК СуммаАванса,
		|	ТаблицаРасчетыСПокупателями.Сумма КАК СуммаОплаты
		|ПОМЕСТИТЬ ВременнаяТаблицаОплатыСчетов
		|ИЗ
		|	&ТаблицаРасчетыСПокупателями КАК ТаблицаРасчетыСПокупателями
		|ГДЕ
		|	ТИПЗНАЧЕНИЯ(ТаблицаРасчетыСПокупателями.Заказ) = ТИП(Документ.ЗаказПокупателя)
		|	И ТаблицаРасчетыСПокупателями.Заказ <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	И ТаблицаРасчетыСПокупателями.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаОплатыСчетов.Организация КАК Организация,
		|	ВременнаяТаблицаОплатыСчетов.СчетНаОплату КАК СчетНаОплату,
		|	СУММА(ВременнаяТаблицаОплатыСчетов.СуммаАванса) КАК СуммаАванса,
		|	СУММА(ВременнаяТаблицаОплатыСчетов.СуммаОплаты) КАК СуммаОплаты
		|ИЗ
		|	ВременнаяТаблицаОплатыСчетов КАК ВременнаяТаблицаОплатыСчетов
		|
		|СГРУППИРОВАТЬ ПО
		...
		Запрос.УстановитьПараметр("ТаблицаРасчетыСПокупателями",
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетКомиссионера, СтруктураДополнительныеСвойства) Экспорт
	...
	// Учет НДС
	УчетНДСУНФ.СформироватьДвиженияНДСЗаписиКнигиПродажРеализация(ДокументСсылкаОтчетКомиссионера,
		СтруктураДополнительныеСвойства);
	УчетНДСУНФ.СформироватьДвиженияНДСЗаписиКнигиПокупокОтчетКомиссионера(ДокументСсылкаОтчетКомиссионера,
		СтруктураДополнительныеСвойства);
	УчетНДСУНФ.СформироватьТаблицаНДСАвансыПоДоговорамКомиссии(ДокументСсылкаОтчетКомиссионера,
		СтруктураДополнительныеСвойства);
	УчетНДСУНФ.СформироватьТаблицаНДСПредъявленный(ДокументСсылкаОтчетКомиссионера, СтруктураДополнительныеСвойства);
...
Функция ТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|        И
	|            ОтчетКомиссионера.Договор.ДоговорОбслуживанияНаправлениеДеятельности <> ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|    ВЫБОР
	|        КОГДА
	|            ОтчетКомиссионера.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах)
	...
	|    ОтчетКомиссионера.Договор.ДоговорУслугиКомиссионногоВознаграждения КАК
	|        ДоговорДоговорУслугиКомиссионногоВознаграждения,
	|    ОтчетКомиссионера.Номер КАК Номер,
	|    ОтчетКомиссионера.ИспользоватьНовуюСхемуДвижений КАК ИспользоватьНовуюСхемуДвижений,
	|    ОтчетКомиссионера.НаВозвратыВыставляетсяКорректировочныйСчетФактура КАК
	...
	|                        ТОГДА ОтчетКомиссионераЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалютПоДатам.Кратность
	|                            / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДС * ВременнаяТаблицаШапка.Курс *
	|                        КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	...
	|            ИНАЧЕ ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                    ТОГДА ОтчетКомиссионераЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                        (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДС * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	...
	|                    ТОГДА ОтчетКомиссионераЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалютПоДатам.Кратность /
	|                        (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДС * ВременнаяТаблицаШапка.Курс * КурсыУпрВалютПоДатам.Кратность /
	|                    (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА ОтчетКомиссионераЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	...
	|            ТОГДА ВЫРАЗИТЬ(ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                    ТОГДА ОтчетКомиссионераЗапасы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалютПоДатам.Кратность /
	|                        (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасы.Всего * ВременнаяТаблицаШапка.Курс * КурсыУпрВалютПоДатам.Кратность /
	|                    (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА ОтчетКомиссионераЗапасы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ОтчетКомиссионераЗапасы.Всего * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|        КОНЕЦ КАК ЧИСЛО(15, 2))
	|    КОНЕЦ КАК Сумма,
	|    ВЫРАЗИТЬ(ВЫБОР
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА ОтчетКомиссионераЗапасы.СуммаНДС * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|            ТОГДА ОтчетКомиссионераЗапасы.Всего * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	|                (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ОтчетКомиссионераЗапасы.Всего
	...
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                        ТОГДА ОтчетКомиссионераЗапасы.СуммаНДСПередачи * КурсыРегВалюты.Курс *
	|                            КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДСПередачи * ВременнаяТаблицаШапка.Курс *
	|                        КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|            ИНАЧЕ ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                    ТОГДА ОтчетКомиссионераЗапасы.СуммаНДСПередачи * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                        (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДСПередачи * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность
	|                    / (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ
	|        КОНЕЦ КАК ЧИСЛО(15, 2))
	...
	|                            ТОГДА ОтчетКомиссионераЗапасы.СуммаПередачи * КурсыРегВалюты.Курс *
	|                                КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс *
	|                                КурсыРегВалюты.Кратность)
	|                        ИНАЧЕ (ОтчетКомиссионераЗапасы.СуммаПередачи + ОтчетКомиссионераЗапасы.СуммаНДСПередачи) *
	|                            КурсыРегВалюты.Курс * КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс *
	|                            КурсыРегВалюты.Кратность)
	|                    КОНЕЦ
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.СуммаВключаетНДС
	|                        ТОГДА ОтчетКомиссионераЗапасы.СуммаПередачи * ВременнаяТаблицаШапка.Курс *
	|                            КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс *
	|                            ВременнаяТаблицаШапка.Кратность)
	|                    ИНАЧЕ (ОтчетКомиссионераЗапасы.СуммаПередачи + ОтчетКомиссионераЗапасы.СуммаНДСПередачи) *
	|                        ВременнаяТаблицаШапка.Курс * КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс *
	|                        ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|                ТОГДА ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.СуммаВключаетНДС
	|                        ТОГДА ОтчетКомиссионераЗапасы.СуммаПередачи * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                            (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ (ОтчетКомиссионераЗапасы.СуммаПередачи + ОтчетКомиссионераЗапасы.СуммаНДСПередачи) *
	|                        КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                        КурсыРегВалюты.Кратность)
	|                КОНЕЦ
	|            ИНАЧЕ ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.СуммаВключаетНДС
	|                    ТОГДА ОтчетКомиссионераЗапасы.СуммаПередачи * ВременнаяТаблицаШапка.Курс *
	|                        КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|                ИНАЧЕ (ОтчетКомиссионераЗапасы.СуммаПередачи + ОтчетКомиссионераЗапасы.СуммаНДСПередачи) *
	|                    ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                    ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ
	|        КОНЕЦ КАК ЧИСЛО(15, 2))
	...
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                        ТОГДА ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения * КурсыРегВалюты.Курс *
	|                            КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения * ВременнаяТаблицаШапка.Курс *
	|                        КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|                КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                    ТОГДА ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения * КурсыРегВалюты.Курс *
	|                        КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения * ВременнаяТаблицаШапка.Курс *
	|                    КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ
	|        КОНЕЦ КАК ЧИСЛО(15, 2))
	...
	|                    КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                        ТОГДА ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения * КурсыРегВалюты.Курс *
	|                            ВременнаяТаблицаШапка.Кратность / (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|                            КОНЕЦ * КурсыРегВалюты.Курс * КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс
	|                                * КурсыРегВалюты.Кратность)
	|                        ИНАЧЕ ВЫБОР
	|                            КОГДА ВременнаяТаблицаШапка.НДСВключатьВСтоимость
	|                                ТОГДА ОтчетКомиссионераЗапасы.СуммаВознаграждения +
	...
	|                            ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаВознаграждения -
	|                                ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения
	|                        КОНЕЦ * ВременнаяТаблицаШапка.Курс * КурсыУпрВалютПоДатам.Кратность /
	|                            (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|                    ИНАЧЕ ВЫБОР
	|                        КОГДА ВременнаяТаблицаШапка.НДСВключатьВСтоимость
	|                            ТОГДА ОтчетКомиссионераЗапасы.СуммаВознаграждения +
	|                                ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения
	|                        ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаВознаграждения
	|                    КОНЕЦ * ВременнаяТаблицаШапка.Курс * КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс *
	|                        ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|                            ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаВознаграждения -
	|                                ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения
	|                        КОНЕЦ * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                            КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ ВЫБОР
	|                        КОГДА ВременнаяТаблицаШапка.НДСВключатьВСтоимость
	|                            ТОГДА ОтчетКомиссионераЗапасы.СуммаВознаграждения +
	...
	|                        ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаВознаграждения -
	|                            ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения
	|                    КОНЕЦ * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                        ВременнаяТаблицаШапка.Кратность)
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.НДСВключатьВСтоимость
	|                        ТОГДА ОтчетКомиссионераЗапасы.СуммаВознаграждения +
	...
	|    ЕСТЬNULL(ОтчетКомиссионераПокупатели.ОтчетКомиссионера.ИспользоватьНовуюСхемуДвижений, ЛОЖЬ) КАК
	|        ИспользоватьНовуюСхемуДвиженийВОтчете,
	|    ВЫБОР
	|        КОГДА
	|            ОтчетКомиссионераПокупатели.ОтчетКомиссионера.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах)
	|        И НЕ &ИспользоватьНовыйВидДокумента
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|            ТОГДА ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаНДСВознаграждения * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСВознагражденияДляУслуги,
	|    ВЫБОР
	|        КОГДА
	|            ВременнаяТаблицаШапка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах)
	|        И НЕ &ИспользоватьНовыйВидДокумента
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	|    КОНЕЦ КАК ЭтоВозврат,
	|    ВременнаяТаблицаШапка.Контрагент.СчетУчетаРасчетовСПоставщиком КАК СчетУчетаРасчетовСПоставщиком,
	|    ВременнаяТаблицаШапка.ДоговорДоговорУслугиКомиссионногоВознаграждения КАК
	|        ДоговорДоговорУслугиКомиссионногоВознаграждения,
	|    ВременнаяТаблицаШапка.УслугаКомиссионногоВознаграждения.СчетУчетаЗатрат КАК СчетУчетаЗатрат,
	|    ОтчетКомиссионераПокупатели.ОтчетКомиссионера.УдержатьКомиссионноеВознаграждение КАК
	|        ОтчетКомиссионераУдержатьКомиссионноеВознаграждение,
	|    ВременнаяТаблицаШапка.УслугаКомиссионногоВознаграждения.СчетУчетаЗатрат.ТипСчета КАК СчетУчетаЗатратТипСчета,
	|    ВЫБОР
	|        КОГДА
	|            ВременнаяТаблицаШапка.УслугаКомиссионногоВознаграждения.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|    ОтчетКомиссионераЗапасы.СуммаНДС КАК СуммаНДСВалютаДокумента,
	|    ОтчетКомиссионераПокупатели.СчетФактура КАК СчетФактура,
	|    ВЫБОР
	|        КОГДА НЕ ОтчетКомиссионераПокупатели.ОтчетКомиссионера = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионера.ПустаяСсылка)
	|            ТОГДА ВЫРАЗИТЬ(ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                    ТОГДА ОтчетКомиссионераЗапасы.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалютПоДатам.Кратность /
	|                        (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасы.Сумма * ВременнаяТаблицаШапка.Курс * КурсыУпрВалютПоДатам.Кратность /
	|                    (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА ОтчетКомиссионераЗапасы.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ОтчетКомиссионераЗапасы.Сумма * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|        КОНЕЦ КАК ЧИСЛО(15, 2))
	|    КОНЕЦ КАК СуммаРуб
	...
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|            ТОГДА ОтчетКомиссионераЗапасы.СуммаВознаграждения * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ОтчетКомиссионераЗапасы.СуммаВознаграждения * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|            (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	...
	|            ТОГДА -1 * ВЫРАЗИТЬ(ТаблицаДокумента.СуммаРасчетов * ТаблицаДокумента.Курс *
	|                КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс *
	|                ТаблицаДокумента.Кратность) КАК ЧИСЛО(15, 2))
	...
	|                    ТОГДА ТаблицаДокумента.СуммаРасчетов * -1
	|                ИНАЧЕ -1 * ВЫРАЗИТЬ(ТаблицаДокумента.СуммаРасчетов * ТаблицаДокумента.Курс /
	|                    ТаблицаДокумента.Кратность КАК ЧИСЛО(15, 2))
	|            КОНЕЦ
	|        ИНАЧЕ ВЫБОР
	|            КОГДА ТаблицаДокумента.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	...
	|        ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионера.СведенияПрослеживаемости КАК
	|            ОтчетКомиссионераСведенияПрослеживаемости
	...
	|    ВЫБОР
	|        КОГДА
	|            ТИПЗНАЧЕНИЯ(ОтчетКомиссионераЗапасыВозвраты.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	...
	|    ВЫБОР
	|        КОГДА НЕ
	|            ОтчетКомиссионераПокупателиВозвраты.ОтчетКомиссионера = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионера.ПустаяСсылка)
	|            ТОГДА ВЫРАЗИТЬ(ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.НДСВключатьВСтоимость
	|                    ТОГДА 0
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                        ТОГДА ОтчетКомиссионераЗапасыВозвраты.СуммаНДС * КурсыРегВалюты.Курс *
	|                            КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.СуммаНДС * ВременнаяТаблицаШапка.Курс *
	|                        КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|            ИНАЧЕ ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                    ТОГДА ОтчетКомиссионераЗапасыВозвраты.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                        (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.СуммаНДС * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность
	|                    / (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ
	|        КОНЕЦ КАК ЧИСЛО(15, 2))
	|    КОНЕЦ КАК СуммаНДС,
	...
	|            ТОГДА ВЫРАЗИТЬ(ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                    ТОГДА ОтчетКомиссионераЗапасыВозвраты.Всего * КурсыРегВалюты.Курс * КурсыУпрВалютПоДатам.Кратность
	|                        / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.Всего * ВременнаяТаблицаШапка.Курс *
	|                    КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА ОтчетКомиссионераЗапасыВозвраты.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.Всего * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|        КОНЕЦ КАК ЧИСЛО(15, 2))
	|    КОНЕЦ КАК Сумма,
	|    ВЫРАЗИТЬ(ВЫБОР
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА ОтчетКомиссионераЗапасыВозвраты.СуммаНДС * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность
	|                    / (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|            ТОГДА ОтчетКомиссионераЗапасыВозвраты.Всего * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность /
	|                (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	...
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                        ТОГДА ОтчетКомиссионераЗапасыВозвраты.СуммаНДСПередачи * КурсыРегВалюты.Курс *
	|                            КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.СуммаНДСПередачи * ВременнаяТаблицаШапка.Курс *
	|                        КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|                    ТОГДА ВЫБОР
	|                        КОГДА ВременнаяТаблицаШапка.СуммаВключаетНДС
	|                            ТОГДА ОтчетКомиссионераЗапасыВозвраты.СуммаПередачи * КурсыРегВалюты.Курс *
	|                                КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс *
	|                                КурсыРегВалюты.Кратность)
	|                        ИНАЧЕ (ОтчетКомиссионераЗапасыВозвраты.СуммаПередачи +
	|                            ОтчетКомиссионераЗапасыВозвраты.СуммаНДСПередачи) * КурсыРегВалюты.Курс *
	|                            КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                    КОНЕЦ
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.СуммаВключаетНДС
	|                        ТОГДА ОтчетКомиссионераЗапасыВозвраты.СуммаПередачи * ВременнаяТаблицаШапка.Курс *
	|                            КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс *
	|                            ВременнаяТаблицаШапка.Кратность)
	|                    ИНАЧЕ (ОтчетКомиссионераЗапасыВозвраты.СуммаПередачи +
	|                        ОтчетКомиссионераЗапасыВозвраты.СуммаНДСПередачи) * ВременнаяТаблицаШапка.Курс *
	|                        КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                        ТОГДА ОтчетКомиссионераЗапасыВозвраты.СуммаНДСВознаграждения * КурсыРегВалюты.Курс *
	|                            КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                    ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.СуммаНДСВознаграждения * ВременнаяТаблицаШапка.Курс *
	|                        КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|                КОНЕЦ
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА НЕ
	|                ОтчетКомиссионераПокупателиВозвраты.ОтчетКомиссионера.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	...
	|                ТОГДА ВЫБОР
	|                    КОГДА
	|                        ОтчетКомиссионераПокупателиВозвраты.ОтчетКомиссионера.Договор.УслугаКомиссионногоВознаграждения.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|                        ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.НДСПоПриобретеннымЦенностям)
	|                    ИНАЧЕ
	|                        ОтчетКомиссионераПокупателиВозвраты.ОтчетКомиссионера.Договор.УслугаКомиссионногоВознаграждения.СчетУчетаНДСПоПриобретеннымЦенностям
	|                КОНЕЦ
	|            ИНАЧЕ ВЫБОР
	|                КОГДА
	|                    ВременнаяТаблицаШапка.УслугаКомиссионногоВознаграждения.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|    ВременнаяТаблицаШапка.УслугаКомиссионногоВознаграждения.СчетУчетаЗатрат.ТипСчета КАК СчетУчетаЗатратТипСчета,
	|    ВременнаяТаблицаШапка.ВалютаДокумента КАК ВалютаДокумента,
	|    ОтчетКомиссионераЗапасыВозвраты.СуммаНДС КАК СуммаНДСВалютаДокумента,
	...
	|            ТОГДА ВЫРАЗИТЬ(ВЫБОР
	|                КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                    ТОГДА ОтчетКомиссионераЗапасыВозвраты.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалютПоДатам.Кратность
	|                        / (КурсыУпрВалютПоДатам.Курс * КурсыРегВалюты.Кратность)
	|                ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.Сумма * ВременнаяТаблицаШапка.Курс *
	|                    КурсыУпрВалютПоДатам.Кратность / (КурсыУпрВалютПоДатам.Курс * ВременнаяТаблицаШапка.Кратность)
	|            КОНЕЦ КАК ЧИСЛО(15, 2))
	|        ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	|            КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА ОтчетКомиссионераЗапасыВозвраты.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.Сумма * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|        КОНЕЦ КАК ЧИСЛО(15, 2))
	|    КОНЕЦ КАК СуммаРуб
	...
	|                КОГДА
	|                    ОтчетКомиссионераПокупателиВозвраты.ОтчетКомиссионера = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионера.ПустаяСсылка)
	|                    ТОГДА НАЧАЛОПЕРИОДА(ОтчетКомиссионераПокупателиВозвраты.Ссылка.Дата,
	|                        ДЕНЬ) = КурсыУпрВалютПоДатам.Период
	|                ИНАЧЕ НАЧАЛОПЕРИОДА(ОтчетКомиссионераПокупателиВозвраты.ОтчетКомиссионера.Дата,
	|                    ДЕНЬ) = КурсыУпрВалютПоДатам.Период
	|            КОНЕЦ
	...
	|    СУММА(ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|            ТОГДА ОтчетКомиссионераЗапасыВозвраты.СуммаВознаграждения * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность
	|                / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ОтчетКомиссионераЗапасыВозвраты.СуммаВознаграждения * ВременнаяТаблицаШапка.Курс *
	|            КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2))) КАК Сумма,
	|    ЛОЖЬ КАК ВключатьРасходыВСебестоимость,
	|    ВременнаяТаблицаШапка.УслугаКомиссионногоВознаграждения.СчетУчетаЗатрат.ТипСчета КАК ТипСчета,
	...
	|        ИНАЧЕ ОтчетКомиссионераПокупателиВозвраты.ОтчетКомиссионера
	|    КОНЕЦ КАК ОтчетКомиссионера,
	|    ВременнаяТаблицаШапка.НаВозвратыВыставляетсяКорректировочныйСчетФактура КАК
	|        НаВозвратыВыставляетсяКорректировочныйСчетФактура
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|    ОтчетКомиссионераДенежныеСредства.Ссылка КАК Ссылка,
	...
	|    ОтчетКомиссионераДенежныеСредства.СуммаНДС КАК СуммаНДС,
	|    ОтчетКомиссионераДенежныеСредства.Покупатель КАК Покупатель,
	|    ОтчетКомиссионераДенежныеСредства.ДатаСобытия КАК ДатаСобытия,
	|    ОтчетКомиссионераДенежныеСредства.Ссылка.Контрагент КАК Комиссионер,
	|    ОтчетКомиссионераДенежныеСредства.Ссылка.Организация КАК Организация,
	|    ОтчетКомиссионераДенежныеСредства.Ссылка.Дата КАК Дата
	|ПОМЕСТИТЬ ВременнаяТаблицаДенежныеСредства
	|ИЗ
	|    Документ.ОтчетКомиссионера.ДенежныеСредства КАК ОтчетКомиссионераДенежныеСредства
	|ГДЕ
	|    ОтчетКомиссионераДенежныеСредства.Ссылка = &Ссылка";
...
Функция ТекстЗапросаРаспределениеОтчетКомиссионераОПродажах()
	...
	|	И НЕ
	|		ОтчетКомиссионераЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ВтОстатки
	|ИЗ
	|	РегистрНакопления.ЗапасыПереданные.Остатки(&МоментВремени, Организация = &Организация
	...
	|			И (НЕ ОтчетКомиссионераЗапасы.ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|			ИЛИ
	|				ОтчетКомиссионераЗапасы.СпособРаспределения = ЗНАЧЕНИЕ(Перечисление.СпособыЗачетаИРаспределенияПлатежей.Вручную)))) КАК
...
Функция ТекстЗапросаРаспределениеОтчетКомиссионераОВозвратах(ИспользоватьНовыйВидДокумента = Ложь) 
	...
	Если ИспользоватьНовыйВидДокумента Тогда
		...
		|	И
		|		ОтчетКомиссионераЗапасыВозвраты.СпособРаспределения = ЗНАЧЕНИЕ(Перечисление.СпособыЗачетаИРаспределенияПлатежей.Авто)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ИЛИ
		|		ОтчетКомиссионераЗапасыВозвраты.СпособРаспределения = ЗНАЧЕНИЕ(Перечисление.СпособыЗачетаИРаспределенияПлатежей.Вручную))
		|
		|УПОРЯДОЧИТЬ ПО
		|	ОтчетКомиссионера
		|;
		...
		|		ИЛИ ЗапасыПереданные.Заказ = НЕОПРЕДЕЛЕНО
...
Функция ТекстЗапросаОтчетКомиссионераОВозвратахРасчетыСПокупателямиАвто() 
	...
	|	СУММА(ВложенныйЗапрос.СуммаВалОстаток) КАК СуммаВалОстаток,
	|	СУММА(ВЫБОР
	|			КОГДА НЕ &УчетВалютныхОпераций
	|				ТОГДА 0
	|			ИНАЧЕ ВложенныйЗапрос.СуммаРегОстаток
	|		КОНЕЦ) КАК СуммаРегОстаток
	|ПОМЕСТИТЬ ВременнаяТаблицаОстаткиПоДокументамВозврата
	|ИЗ
	|	(ВЫБРАТЬ
	|		1 КАК НомерЗапроса,
	...
	|				Документ В
	|						(ВЫБРАТЬ
	|							Возвраты.ОтчетКомиссионера
	|						ИЗ
	|							ВременнаяТаблицаПокупателиДокументыВозвратаВозвраты КАК Возвраты)
	|					И Документ <> ЗНАЧЕНИЕ(Документ.ОтчетКомиссионера.ПустаяСсылка)
	|					И (Документ.МоментВремени < &МоментВремени
	...
	|	ГДЕ
	|		РасчетыСПокупателями.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)
	|		И РасчетыСПокупателями.Регистратор = &Ссылка
	|		И РасчетыСПокупателями.Документ <> &Ссылка) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВложенныйЗапрос.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
	...
	|	СУММА(ВложенныйЗапрос.СуммаДляОстатка) КАК СуммаДляОстатка,
	|	СУММА(ВложенныйЗапрос.СуммаВалДляОстатка) КАК СуммаВалДляОстатка,
	|	ВложенныйЗапрос.СодержаниеПроводки КАК СодержаниеПроводки
	|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПокупателямиВозвратыПредварительная
	|ИЗ
	|	(ВЫБРАТЬ
	|	МАКСИМУМ(ТаблицаДокумента.НомерСтроки) КАК НомерСтроки,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	...
	|	ВложенныйЗапрос.ВестиРасчетыПоДокументам,
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг) КАК ТипРасчетов,
	|	&ВосстановлениеАванса КАК СодержаниеПроводки,
	|	ТаблицаДокумента.Дата КАК Дата,
	|	ТаблицаДокумента.Организация КАК Организация,
	|	ТаблицаДокумента.Контрагент КАК Контрагент,
	|	ТаблицаДокумента.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
	...
	|	ТаблицаДокумента.Валюта КАК Валюта,
	|	ТаблицаДокумента.Сумма - ЕСТЬNULL(Остатки.СуммаОстаток, 0) КАК Сумма,
	|	ТаблицаДокумента.СуммаВал - ЕСТЬNULL(Остатки.СуммаВалОстаток, 0) КАК СуммаВал,
	|	ТаблицаДокумента.СуммаРег - ЕСТЬNULL(Остатки.СуммаРегОстаток, 0) КАК СуммаРег,
	|	ТаблицаДокумента.СуммаДляОстатка - ЕСТЬNULL(Остатки.СуммаОстаток, 0) КАК СуммаДляОстатка,
	|	ТаблицаДокумента.СуммаВалДляОстатка - ЕСТЬNULL(Остатки.СуммаВалОстаток, 0) КАК СуммаВалДляОстатка
	|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПокупателямиВозвратыДополненная
	|ИЗ
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыПредварительная КАК ТаблицаДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаОстаткиПоДокументамВозврата КАК Остатки
	|		ПО ТаблицаДокумента.Документ = Остатки.Документ
	|			И ТаблицаДокумента.Заказ = Остатки.Заказ
	|ГДЕ
	|	ТаблицаДокумента.Документ <> &Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ТаблицаДокумента.Валюта,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Сумма < ЕСТЬNULL(Остатки.СуммаОстаток, 0)
	|			ТОГДА ТаблицаДокумента.Сумма
	|		КОГДА ТаблицаДокумента.Документ = &Ссылка
	|			ТОГДА ТаблицаДокумента.Сумма
	|		ИНАЧЕ ЕСТЬNULL(Остатки.СуммаОстаток, 0)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаВал < ЕСТЬNULL(Остатки.СуммаВалОстаток, 0)
	|			ТОГДА ТаблицаДокумента.СуммаВал
	|		КОГДА ТаблицаДокумента.Документ = &Ссылка
	|			ТОГДА ТаблицаДокумента.СуммаВал
	|		ИНАЧЕ ЕСТЬNULL(Остатки.СуммаВалОстаток, 0)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаРег < ЕСТЬNULL(Остатки.СуммаРегОстаток, 0)
	|			ТОГДА ТаблицаДокумента.СуммаРег
	|		КОГДА ТаблицаДокумента.Документ = &Ссылка
	|			ТОГДА ТаблицаДокумента.СуммаРег
	|		ИНАЧЕ ЕСТЬNULL(Остатки.СуммаРегОстаток, 0)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаДляОстатка < ЕСТЬNULL(Остатки.СуммаОстаток, 0)
	|			ТОГДА ТаблицаДокумента.СуммаДляОстатка
	|		КОГДА ТаблицаДокумента.Документ = &Ссылка
	|			ТОГДА ТаблицаДокумента.СуммаДляОстатка
	|		ИНАЧЕ ЕСТЬNULL(Остатки.СуммаОстаток, 0)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаВалДляОстатка < ЕСТЬNULL(Остатки.СуммаВалОстаток, 0)
	|			ТОГДА ТаблицаДокумента.СуммаВалДляОстатка
	|		КОГДА ТаблицаДокумента.Документ = &Ссылка
	|			ТОГДА ТаблицаДокумента.СуммаВалДляОстатка
	|		ИНАЧЕ ЕСТЬNULL(Остатки.СуммаВалОстаток, 0)
	|	КОНЕЦ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаДокумента.НомерЗапроса КАК НомерЗапроса,
	|	ТаблицаДокумента.НомерСтроки КАК НомерСтроки,
	...
	|	ТаблицаДокумента.СуммаДляОстатка КАК СуммаДляОстатка,
	|	ТаблицаДокумента.СуммаВалДляОстатка КАК СуммаВалДляОстатка
	|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая
	|ИЗ
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыДополненная КАК ТаблицаДокумента
	|ГДЕ
	|	(ТаблицаДокумента.Сумма > 0
	|			ИЛИ ТаблицаДокумента.СуммаВал > 0
	|			ИЛИ ТаблицаДокумента.СуммаРег > 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Дата КАК Дата,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.ВидДвижения КАК ВидДвижения,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.ТипРасчетов КАК ТипРасчетов,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СчетУчета КАК СчетУчета,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Организация КАК Организация,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Контрагент КАК Контрагент,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Договор КАК Договор,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Документ КАК Документ,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Заказ КАК Заказ,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Валюта КАК Валюта,
	|	СУММА(ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Сумма) КАК Сумма,
	|	СУММА(ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СуммаВал) КАК СуммаВал,
	|	СУММА(ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СуммаРег) КАК СуммаРег,
	|	СУММА(ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СуммаДляОстатка) КАК СуммаДляОстатка,
	|	СУММА(ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СуммаВалДляОстатка) КАК СуммаВалДляОстатка
	|ПОМЕСТИТЬ ТекДокументУменьшениеДолгаОбщее
	|ИЗ
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая КАК ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая
	|ГДЕ
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Документ = &Ссылка
	|	И ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Дата,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.ВидДвижения,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.ТипРасчетов,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СчетУчета,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Организация,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Контрагент,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.ВестиРасчетыПоДокументам,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Договор,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Документ,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Заказ,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Валюта
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ВременнаяТаблицаРасчетыСПокупателями.ТипРасчетов КАК ТипРасчетов,
	|	ВременнаяТаблицаРасчетыСПокупателями.СчетУчета КАК СчетУчета,
	|	ВременнаяТаблицаРасчетыСПокупателями.Организация КАК Организация,
	|	ВременнаяТаблицаРасчетыСПокупателями.Контрагент КАК Контрагент,
	|	ВременнаяТаблицаРасчетыСПокупателями.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
	|	ВременнаяТаблицаРасчетыСПокупателями.Договор КАК Договор,
	|	ВременнаяТаблицаРасчетыСПокупателями.Документ КАК Документ,
	|	ВременнаяТаблицаРасчетыСПокупателями.Заказ КАК Заказ,
	|	ВременнаяТаблицаРасчетыСПокупателями.Валюта КАК Валюта,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.Сумма, 0) < ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.Сумма, 0)
	|			ТОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.Сумма, 0)
	|		ИНАЧЕ ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.Сумма, 0)
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.СуммаВал, 0) < ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.СуммаВал, 0)
	|			ТОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.СуммаВал, 0)
	|		ИНАЧЕ ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.СуммаВал, 0)
	|	КОНЕЦ КАК СуммаВал,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.СуммаРег, 0) < ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.СуммаРег, 0)
	|			ТОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.СуммаРег, 0)
	|		ИНАЧЕ ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.СуммаРег, 0)
	|	КОНЕЦ КАК СуммаРег,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.СуммаДляОстатка, 0) < ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.СуммаДляОстатка, 0)
	|			ТОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.СуммаДляОстатка, 0)
	|		ИНАЧЕ ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.СуммаДляОстатка, 0)
	|	КОНЕЦ КАК СуммаДляОстатка,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.СуммаВалДляОстатка, 0) < ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.СуммаВалДляОстатка, 0)
	|			ТОГДА ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателями.СуммаВалДляОстатка, 0)
	|		ИНАЧЕ ЕСТЬNULL(ТекДокументУменьшениеДолгаОбщее.СуммаВалДляОстатка, 0)
	|	КОНЕЦ КАК СуммаВалДляОстатка
	|ПОМЕСТИТЬ ТекДокументУменьшениеДолга
	|ИЗ
	|	ВременнаяТаблицаРасчетыСПокупателями КАК ВременнаяТаблицаРасчетыСПокупателями
	|		ПОЛНОЕ СОЕДИНЕНИЕ ТекДокументУменьшениеДолгаОбщее КАК ТекДокументУменьшениеДолгаОбщее
	|		ПО ВременнаяТаблицаРасчетыСПокупателями.Заказ = ТекДокументУменьшениеДолгаОбщее.Заказ
	|ГДЕ
	|	ВременнаяТаблицаРасчетыСПокупателями.Документ = &Ссылка
	...
	|	(ВЫБРАТЬ
	|		ВременнаяТаблицаРасчетыСПокупателями.Дата КАК Дата,
	|		ВременнаяТаблицаРасчетыСПокупателями.ВидДвижения КАК ВидДвижения,
	|		ВременнаяТаблицаРасчетыСПокупателями.ТипРасчетов КАК ТипРасчетов,
	|		ВременнаяТаблицаРасчетыСПокупателями.СчетУчета КАК СчетУчета,
	|		ВременнаяТаблицаРасчетыСПокупателями.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
	|		ВременнаяТаблицаРасчетыСПокупателями.Организация КАК Организация,
	...
	|		ВременнаяТаблицаРасчетыСПокупателями.Заказ КАК Заказ,
	|		ВременнаяТаблицаРасчетыСПокупателями.Валюта КАК Валюта,
	|		ВременнаяТаблицаРасчетыСПокупателями.Сумма КАК Сумма,
	|		ВременнаяТаблицаРасчетыСПокупателями.СуммаВал КАК СуммаВал,
	|		ВременнаяТаблицаРасчетыСПокупателями.СуммаРег КАК СуммаРег,
	|		ВременнаяТаблицаРасчетыСПокупателями.СуммаДляОстатка КАК СуммаДляОстатка,
	|		ВременнаяТаблицаРасчетыСПокупателями.СуммаВалДляОстатка КАК СуммаВалДляОстатка
	|	ИЗ
	|		ВременнаяТаблицаРасчетыСПокупателями КАК ВременнаяТаблицаРасчетыСПокупателями
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ТекДокументУменьшениеДолгаОбщее.Дата,
	|		ТекДокументУменьшениеДолгаОбщее.ВидДвижения,
	|		ТекДокументУменьшениеДолгаОбщее.ТипРасчетов,
	|		ТекДокументУменьшениеДолгаОбщее.СчетУчета,
	|		ТекДокументУменьшениеДолгаОбщее.ВестиРасчетыПоДокументам,
	|		ТекДокументУменьшениеДолгаОбщее.Организация,
	|		ТекДокументУменьшениеДолгаОбщее.Контрагент,
	|		ТекДокументУменьшениеДолгаОбщее.Договор,
	|		ТекДокументУменьшениеДолгаОбщее.Документ,
	|		ТекДокументУменьшениеДолгаОбщее.Заказ,
	|		ТекДокументУменьшениеДолгаОбщее.Валюта,
	|		-ТекДокументУменьшениеДолгаОбщее.Сумма,
	|		-ТекДокументУменьшениеДолгаОбщее.СуммаВал,
	|		-ТекДокументУменьшениеДолгаОбщее.СуммаРег,
	|		-ТекДокументУменьшениеДолгаОбщее.СуммаДляОстатка,
	|		-ТекДокументУменьшениеДолгаОбщее.СуммаВалДляОстатка
	|	ИЗ
	|		ТекДокументУменьшениеДолгаОбщее КАК ТекДокументУменьшениеДолгаОбщее) КАК ВложенныйЗапрос
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПродажиИВозвраты.Дата КАК Дата,
	|	ПродажиИВозвраты.СчетУчета КАК СчетУчета,
	|	ПродажиИВозвраты.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
	|	ПродажиИВозвраты.Организация КАК Организация,
	|	ПродажиИВозвраты.Контрагент КАК Контрагент,
	|	ПродажиИВозвраты.Договор КАК Договор,
	|	ПродажиИВозвраты.Документ КАК Документ,
	|	ПродажиИВозвраты.Заказ КАК Заказ,
	|	ПродажиИВозвраты.Валюта КАК Валюта,
	|	-ПродажиИВозвраты.Сумма КАК Сумма,
	|	-ПродажиИВозвраты.СуммаВал КАК СуммаВал,
	|	-ПродажиИВозвраты.СуммаРег КАК СуммаРег,
	|	-ПродажиИВозвраты.СуммаДляОстатка КАК СуммаДляОстатка,
	|	-ПродажиИВозвраты.СуммаВалДляОстатка КАК СуммаВалДляОстатка
	|ПОМЕСТИТЬ АвансВозвраты
	|ИЗ
	|	ПродажиИВозвраты КАК ПродажиИВозвраты
	|ГДЕ
	|	(-ПродажиИВозвраты.Сумма > 0
	|			ИЛИ -ПродажиИВозвраты.СуммаВал > 0
	|			ИЛИ -ПродажиИВозвраты.СуммаРег > 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ПродажиИВозвраты.Заказ КАК Заказ,
	|	ПродажиИВозвраты.Валюта КАК Валюта,
	|	ПродажиИВозвраты.СуммаВал КАК СуммаВалОстаток
	|ПОМЕСТИТЬ КЗачету
	|ИЗ
	|	ПродажиИВозвраты КАК ПродажиИВозвраты
	|ГДЕ
	|	ПродажиИВозвраты.СуммаВал > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	1 КАК НомерЗапроса,
	|	ТекДокументУменьшениеДолга.Дата КАК Дата,
	|	ТекДокументУменьшениеДолга.ВидДвижения КАК ВидДвижения,
	|	ТекДокументУменьшениеДолга.ТипРасчетов КАК ТипРасчетов,
	|	&СторнированиеДолга КАК СодержаниеПроводки,
	|	ТекДокументУменьшениеДолга.СчетУчета КАК СчетУчета,
	|	ТекДокументУменьшениеДолга.Организация КАК Организация,
	|	ТекДокументУменьшениеДолга.Контрагент КАК Контрагент,
	|	ТекДокументУменьшениеДолга.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
	|	ТекДокументУменьшениеДолга.Договор КАК Договор,
	|	ТекДокументУменьшениеДолга.Документ КАК Документ,
	|	ТекДокументУменьшениеДолга.Заказ КАК Заказ,
	|	ТекДокументУменьшениеДолга.Валюта КАК Валюта,
	|	ТекДокументУменьшениеДолга.Сумма КАК Сумма,
	|	ТекДокументУменьшениеДолга.СуммаВал КАК СуммаВал,
	|	ТекДокументУменьшениеДолга.СуммаРег КАК СуммаРег,
	|	ТекДокументУменьшениеДолга.СуммаДляОстатка КАК СуммаДляОстатка,
	|	ТекДокументУменьшениеДолга.СуммаВалДляОстатка КАК СуммаВалДляОстатка
	|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПокупателямиВозвраты
	|ИЗ
	|	ТекДокументУменьшениеДолга КАК ТекДокументУменьшениеДолга
	|ГДЕ
	|	(ТекДокументУменьшениеДолга.Сумма > 0
	|			ИЛИ ТекДокументУменьшениеДолга.СуммаВал > 0
	|			ИЛИ ТекДокументУменьшениеДолга.СуммаРег > 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	АвансВозвраты.СчетУчета,
	|	АвансВозвраты.Организация,
	|	АвансВозвраты.Контрагент,
	|	АвансВозвраты.ВестиРасчетыПоДокументам,
	|	АвансВозвраты.Договор,
	|	АвансВозвраты.Документ,
	|	АвансВозвраты.Заказ,
	|	АвансВозвраты.Валюта,
	|	АвансВозвраты.Сумма,
	|	АвансВозвраты.СуммаВал,
	|	АвансВозвраты.СуммаРег,
	|	АвансВозвраты.СуммаДляОстатка,
	|	АвансВозвраты.СуммаВалДляОстатка
	|ИЗ
	|	АвансВозвраты КАК АвансВозвраты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Заказ,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Валюта,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Сумма,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СуммаВал,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СуммаРег,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СуммаДляОстатка,
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.СуммаВалДляОстатка
	|ИЗ
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая КАК ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая
	|ГДЕ
	|	ВременнаяТаблицаРасчетыСПокупателямиВозвратыБазовая.Документ <> &Ссылка
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		РасчетыСПокупателямиЗаказы.Заказ КАК Заказ,
	|		РасчетыСПокупателямиЗаказы.Сумма КАК Сумма,
	|		РасчетыСПокупателямиЗаказы.СуммаВал КАК СуммаВал,
	|		ВЫБОР
	|			КОГДА НЕ &УчетВалютныхОпераций
	|				ТОГДА 0
	|			ИНАЧЕ -РасчетыСПокупателямиЗаказы.СуммаРег 
	|		КОНЕЦ КАК СуммаРег,
	|		РасчетыСПокупателямиЗаказы.СуммаДляОстатка КАК СуммаДляОстатка,
	|		РасчетыСПокупателямиЗаказы.СуммаВалДляОстатка КАК СуммаВалДляОстатка
	|	ИЗ
	|		ВременнаяТаблицаРасчетыСПокупателями КАК РасчетыСПокупателямиЗаказы
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	СУММА(ВложенныйЗапрос.СуммаОстаток) КАК СуммаОстаток,
	|	СУММА(ВложенныйЗапрос.СуммаВалОстаток) КАК СуммаВалОстаток,
	|	СУММА(ВложенныйЗапрос.СуммаРегОстаток) КАК СуммаРегОстаток,
	|	0 КАК Курс,
	|	0 КАК Кратность,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВложенныйЗапрос.СуммаВалОстаток) > 0";
		
	Возврат ЗапросТекст;
		
КонецФункции // ТекстЗапросаОтчетКомиссионераОВозвратахРасчетыСПокупателямиАвто()
...
Процедура ВыполнитьКонтрольНовыйВидДокумента(ДокументСсылкаОтчетКомиссионера, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт  
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		ИЛИ СтруктураВременныеТаблицы.ДвиженияНДСАвансыПоДоговорамКомиссииИзменение Тогда
		...
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.Организация) КАК ОрганизацияПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.Комиссионер) КАК КомиссионерПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.СчетФактура) КАК СчетФактураПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.СтавкаНДС) КАК СтавкаНДСПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.Покупатель) КАК ПокупательПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.ДатаСобытия) КАК ДатаСобытияПредставление,
		|	ЕСТЬNULL(ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(НДСАвансыПоДоговорамКомиссииОстатки.СуммаБезНДСОстаток, 0) КАК ОстатокНДСАвансыПоДоговорамКомиссии,
		|	ЕСТЬNULL(НДСАвансыПоДоговорамКомиссииОстатки.СуммаБезНДСОстаток, 0) КАК СуммаБезНДСОстатокНДСАвансыПоДоговорамКомиссии
		|ИЗ
		|	ДвиженияНДСАвансыПоДоговорамКомиссииИзменение КАК ДвиженияНДСАвансыПоДоговорамКомиссииИзменение
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСАвансыПоДоговорамКомиссии.Остатки(
		|				&МоментКонтроля,
		|				(Организация, Комиссионер, СчетФактура, СтавкаНДС, Покупатель, ДатаСобытия) В
		|					(ВЫБРАТЬ
		...
		|					ИЗ
		|						ДвиженияНДСАвансыПоДоговорамКомиссииИзменение КАК ДвиженияНДСАвансыПоДоговорамКомиссииИзменение)) КАК НДСАвансыПоДоговорамКомиссииОстатки
		|		ПО ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.Организация = НДСАвансыПоДоговорамКомиссииОстатки.Организация
		|			И ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.Комиссионер = НДСАвансыПоДоговорамКомиссииОстатки.Комиссионер
		|			И ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.СчетФактура = НДСАвансыПоДоговорамКомиссииОстатки.СчетФактура
		|			И ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.СтавкаНДС = НДСАвансыПоДоговорамКомиссииОстатки.СтавкаНДС
		|			И ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.Покупатель = НДСАвансыПоДоговорамКомиссииОстатки.Покупатель
		|			И ДвиженияНДСАвансыПоДоговорамКомиссииИзменение.ДатаСобытия = НДСАвансыПоДоговорамКомиссииОстатки.ДатаСобытия
		|ГДЕ
		|	ЕСТЬNULL(НДСАвансыПоДоговорамКомиссииОстатки.СуммаБезНДСОстаток, 0) < 0
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки");
		...
		// Отрицательный остаток по Авансам по договорам комиссии.
		Если НЕ МассивРезультатов[15].Пустой() Тогда
			
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[15].Выбрать();
			КонтрольОстатковУНФ.НДСАвансыПоДоговорамКомиссии(ДокументОбъектОтчетКомиссионера, ВыборкаИзРезультатаЗапроса, Отказ);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Использование", Метаданные.Отчеты.РеестрДокументов) Тогда
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "РеестрДокументовОтчетКомиссионера";
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ИмяГруппыПолейWildberries = "Wildberries";
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Доплата", 
		НСтр("ru = 'Сумма доплаты (Выплаты по механикам лояльности)'"),
			ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_2, ИмяГруппыПолейOzon, , Ложь); 
			
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДоплатаВозвращено", 
		НСтр("ru = 'Сумма доплаты возвращено (Выплаты по механикам лояльности)'"),
			ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_2, ИмяГруппыПолейOzon, , Ложь);   
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ТипДокумента", 
		НСтр("ru = 'Тип документа (продажа/возврат)'"), ОписанияТиповПолей.ОписаниеТиповСтрока25,
		ОписанияТиповПолей.ОписаниеТиповСтрока25, ИмяГруппыПолейWildberries, , Ложь); 
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СтранаПроисхождения",
		НСтр("ru = 'Страна происхождения'"), ОписанияТиповПолей.ОписаниеТиповСтрока100, 
		ОписаниеТиповКолонка, ИмяГруппыПолейWildberries, , Ложь);
		
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ОбоснованиеДляОплаты", 
		НСтр("ru = 'Обоснование для оплаты'"), ОписанияТиповПолей.ОписаниеТиповСтрока100, 
		ОписанияТиповПолей.ОписаниеТиповСтрока100, ИмяГруппыПолейWildberries, , Ложь);
...
Процедура ЗагрузитьРезультатИзФайлаВТабличнуюЧасть(РезультатЗагрузки, ТаблицаСопоставленияДанных, 
	...
	ЕстьКолонкаДоплата 				= Ложь;  
	ЕстьКолонкаДоплатаВозвращено 		= Ложь;
	...
	Если ТаблицаСопоставленияДанных.Количество() > 0 Тогда 
		...
		ЕстьКолонкаЗаказ = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ПерваяСтрокаТаблицы, "Заказ");
						
		ЕстьКолонкаСтавкаНДС = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ПерваяСтрокаТаблицы, "СтавкаНДС");  
			
		ЕстьКолонкаДоплата = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ПерваяСтрокаТаблицы, "Доплата"); 
			
		ЕстьКолонкаДоплатаВозвращено = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ПерваяСтрокаТаблицы, "ДоплатаВозвращено");
		...
		Для Каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				ЗаполнитьЦенуИСуммуПоФорматуOzon(ИмяТабличнойЧастиЗапасы, СтрокаТаблицы, СтрокаТабличнойЧасти, 
						ЕстьКолонкаДоплата, ЕстьКолонкаДоплатаВозвращено);  
...
Функция СтруктураДанныхДокументаДляЗачетаАвансов(Ссылка)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ОтчетКомиссионера.Контрагент КАК Контрагент,
	                      |	ОтчетКомиссионера.Договор КАК Договор,
	                      |	ОтчетКомиссионера.Контрагент.ВестиРасчетыПоЗаказам КАК ВестиРасчетыПоЗаказам,
	                      |	ОтчетКомиссионера.Контрагент.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
	                      |	ОтчетКомиссионера.Дата КАК Период,
	                      |	ОтчетКомиссионера.ВалютаДокумента КАК ВалютаДокумента,
	                      |	ОтчетКомиссионера.Договор.ВалютаРасчетов КАК ДоговорВалютаРасчетов,
	                      |	ОтчетКомиссионера.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	                      |	ОтчетКомиссионера.Курс КАК КурсДокумента,
	                      |	ОтчетКомиссионера.Кратность КАК КратностьДокумента,
	                      |	ОтчетКомиссионера.Кратность КАК Кратность
	                      |ИЗ
	                      |	Документ.ОтчетКомиссионера КАК ОтчетКомиссионера
	                      |ГДЕ
