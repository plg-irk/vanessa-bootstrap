...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтотОбъект, "ЗаказНаПеремещение",
	...
	//
	ПоказыватьКолонкуВесВЗаказеНаПеремещение = ПерсональныеНастройкиРУНФ.Прочитать(
		"ПоказыватьКолонкуВесВЗаказеНаПеремещение");
	ПоказыватьКолонкуОбъемВЗаказеНаПеремещение = ПерсональныеНастройкиРУНФ.Прочитать(
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
		Документы.ЗаказНаПеремещение.ПараметрыУказанияСерий(Объект));
...
Процедура ПриОткрытии(Отказ)
	...
	Если ФОСборкаЗаказов Тогда
		СборкаЗаказовКлиент.СборкаЗаказовПриОткрытии(ЭтотОбъект);
	КонецЕсли;
	// Конец СборкаЗаказов
...
Процедура ПоказатьДублиСтрок(Команда)
	...
	Если Не Элементы.ДекорацияОтборПоДублям.Видимость Тогда
		...
		Элементы.ДекорацияОтборПоДублям.Заголовок = ИнформацияОбОтборе(Строка(НомерСтрокиТабличнойЧасти.НомерСтроки));
	...
	Иначе
		...
		Если МассивСтрокТЧ.Количество() = 0 Или НЕ ОбъединятьСтрокиПриДобавлении Тогда
			Если ФОСборкаЗаказов И РежимСборки Тогда
				...
				МассивСтрокТЧ = Объект[ИмяТабличнойЧасти].НайтиСтроки(СтруктураПоиска);
				Если МассивСтрокТЧ.Количество() = 0 Тогда
					ТекстСообщения = СтрШаблон("В заказе отсутствует ""%1"". Позиция не добавлена",
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Запасы.ЗаказПокупателя",
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) И ПоказыватьКолонкуВесВЗаказеНаПеремещение Тогда
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	СписокРеквизитовНоменклатуры.Добавить("Вес");
	СписокРеквизитовНоменклатуры.Добавить("Объем");
	...
	СтруктураДанные.Вставить("ВесЕдиницыТовара", ДанныеНоменклатуры.Вес);
	СтруктураДанные.Вставить("ОбъемЕдиницыТовара", ДанныеНоменклатуры.Объем);
...
Функция ПолучитьДанныеСтруктурнаяЕдиницаПриИзменении(СтруктураДанные)
	...
	РеквизитыСтруктурнойЕдиницы = Новый Массив;
	РеквизитыСтруктурнойЕдиницы.Добавить("ПодписьМОЛ");
	РеквизитыСтруктурнойЕдиницы.Добавить("ТипСтруктурнойЕдиницы");
	РеквизитыСтруктурнойЕдиницы.Добавить("ОрдерныйСклад");
	РеквизитыСтруктурнойЕдиницы.Добавить("ПолучательПеремещения");
	РеквизитыСтруктурнойЕдиницы.Добавить("ИсточникПеремещения");
	
	ДанныеСтруктурнойЕдиницы = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураДанные.СтруктурнаяЕдиница,
		РеквизитыСтруктурнойЕдиницы);
		
	Если Не ЗначениеЗаполнено(ДанныеСтруктурнойЕдиницы.ПолучательПеремещения) Тогда
		ДанныеСтруктурнойЕдиницы.ПолучательПеремещения = Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
	КонецЕсли;
	
	ПараметрыПолучателяПеремещения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеСтруктурнойЕдиницы.ПолучательПеремещения,
		"ОрдерныйСклад, ТипСтруктурнойЕдиницы");
	
	СтруктураДанные.Вставить("ПодписьКладовщика", ДанныеСтруктурнойЕдиницы.ПодписьМОЛ);
	СтруктураДанные.Вставить("ТипСтруктурнойЕдиницы", ДанныеСтруктурнойЕдиницы.ТипСтруктурнойЕдиницы);
	
	Если НЕ ЗначениеЗаполнено(СтруктураДанные.СтруктурнаяЕдиница)
		ИЛИ ДанныеСтруктурнойЕдиницы.ОрдерныйСклад 
		ИЛИ ДанныеСтруктурнойЕдиницы.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Розница
		ИЛИ ДанныеСтруктурнойЕдиницы.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет Тогда
		СтруктураДанные.Вставить("ОрдерныйСклад", Ложь);
	Иначе
		СтруктураДанные.Вставить("ОрдерныйСклад", Истина);
	КонецЕсли;
	
	СтруктураДанные.Вставить("СтруктурнаяЕдиницаПолучатель", ДанныеСтруктурнойЕдиницы.ПолучательПеремещения);
	СтруктураДанные.Вставить("СтруктурнаяЕдиницаОтправитель", ДанныеСтруктурнойЕдиницы.ИсточникПеремещения);
	
	Если НЕ ЗначениеЗаполнено(ДанныеСтруктурнойЕдиницы.ПолучательПеремещения)
		ИЛИ ПараметрыПолучателяПеремещения.ОрдерныйСклад 
		ИЛИ ПараметрыПолучателяПеремещения.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Розница
		ИЛИ ПараметрыПолучателяПеремещения.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет Тогда
...
Процедура ПослеВыбораИзВыпадающегоСпискаТабличнойЧасти(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ВыбранноеПоле = "ЗапасыОстатокОбщий" Тогда
...
Процедура ЗапасыПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПодбор") И ДополнительныеПараметры.ЭтоПодбор Тогда
		...
		МассивНомеровИдентичныхСтрок = Новый Массив;
		ПолучитьЗапасыИзХранилища(ДополнительныеПараметры.АдресЗапасовВХранилище, "Запасы", Истина, Ложь,
