...
//
// Параметры:
//  ДокументСсылкаВзаимозачет - ДокументСсылка.Взаимозачет - документ-источник движений
//  СтруктураДополнительныеСвойства	 - Структура - свойство объекта ДополнительныеСвойства
//
Процедура СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ТаблицаДокумента.Контрагент КАК Контрагент,
	               |	ТаблицаДокумента.КонтрагентИсточник КАК КонтрагентИсточник,
	               ...
	               |	ТаблицаДокумента.Дата КАК Период,
	               |	ТаблицаДокумента.КонтрагентИсточник.Наименование КАК НаименованиеКонтрагентаИсточника,
	               |	ТаблицаДокумента.КонтрагентИсточник.НаименованиеПолное КАК ЮридическоеНаименованиеКонтрагентаИсточника
	               |ИЗ
	               |	ВременнаяТаблицаРасчетыСПоставщиками КАК ВременнаяТаблицаРасчетыСПоставщиками
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Взаимозачет КАК ТаблицаДокумента
	               ...
	               |			И (ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПоставщику)
	               |				ИЛИ ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.Взаимозачет)
	               |				ИЛИ ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.ПрочийВзаимозачет)
	               |					И ТаблицаДокумента.ВидПрочегоВзаимозачета = ЗНАЧЕНИЕ(Перечисление.ВидыПрочегоВзаимозачета.ДолгПрочегоКонтрагентаИДолгПередПоставщиком))";
	...
	ВыборкаЗапасы = Запрос.Выполнить().Выбрать();
	
	УчетПартийУСН.ЗаполнитьДвиженияОплатаПартий(ДокументСсылкаВзаимозачет, ВыборкаЗапасы, ТаблицаОплаты, ТаблицаПартииТоваров);
	
	Если ТаблицаПартииТоваров.Найти(Истина, "ОтражатьВКУДиР") <> Неопределено Тогда
		ДанныеДокумента = ТаблицаОплаты[0];
		ЕстьУслуги = ТаблицаПартииТоваров.НайтиСтроки(Новый Структура("ОтражатьВКУДиР, ЭтоТоварыКРеализации", Истина, Ложь)).Количество() > 0; 
		...
		Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийВзаимозачет.Взаимозачет
			ИЛИ СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийВзаимозачет.ПрочийВзаимозачет Тогда
			
			ТекстКредитор = ?(ПустаяСтрока(ДанныеДокумента.ЮридическоеНаименованиеКонтрагента),
				ДанныеДокумента.НаименованиеКонтрагента, ДанныеДокумента.ЮридическоеНаименованиеКонтрагента);
			ТекстДебитор = ?(ПустаяСтрока(ДанныеДокумента.ЮридическоеНаименованиеКонтрагентаИсточника),
				ДанныеДокумента.НаименованиеКонтрагентаИсточника, ДанныеДокумента.ЮридическоеНаименованиеКонтрагентаИсточника);
				
			Если ЕстьУслуги Тогда
				СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеВзаимозачетУслуги(ТекстДебитор, ТекстКредитор);
			Иначе
				СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеВзаимозачетТовары(ТекстДебитор, ТекстКредитор);
			КонецЕсли;
		Иначе
			Если ЕстьУслуги Тогда
				СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеРасходыНаУслуги();
			Иначе
				СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеРасходыНаТовары();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства) Экспорт
	...
	СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства);
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ТипДокумента)
	...
	Пока Шапка.Следующий() Цикл
		...
		ПредставлениеОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации, 
