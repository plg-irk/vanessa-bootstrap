...
Процедура СпособОтправкиПриИзменении(Элемент)
	...
	СпособОтправкиПриИзмененииНаСервере();
	УправлениеФормой();
...
&НаСервере
Процедура СпособОтправкиПриИзмененииНаСервере()
	
	ЗаполнитьКакСвязатьсяПоСпособуОтправки();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьКакСвязатьсяПоСпособуОтправки()
	...
	Для Каждого Участник Из ТаблицаУчастников Цикл
		...
		Если ЗначениеЗаполнено(Участник.КонтактCRM) Тогда
			ГлавныйКонтакт = Участник.КонтактCRM;
		ИначеЕсли ОбычныйКонтактЭтоКонтактCRM(Участник.Контакт) Тогда
			ГлавныйКонтакт = Участник.Контакт;
		Иначе
			Продолжить;
		КонецЕсли;
		
		Контакты.Добавить(ГлавныйКонтакт);
	...
	КонтрагентыПоЛидам = РаботаСЛидами.СвязанныеКонтрагенты(Лиды);
	КоличествоКонтактов = Контакты.Количество();
	КонтрагентыПовторныхЛидов = Новый Массив;
	Индекс = 0;
	Пока Индекс < КоличествоКонтактов Цикл
		
		Контакт = Контакты[Индекс];
		
		Если ТипЗнч(Контакт) = Тип("СправочникСсылка.Лиды") И ЗначениеЗаполнено(КонтрагентыПоЛидам[Контакт]) Тогда
			КонтрагентыПовторныхЛидов.Добавить(КонтрагентыПоЛидам[Контакт]);
	...
	КонтрагентыПовторныхЛидов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(КонтрагентыПовторныхЛидов);
	
	КИКонтактов = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(Контакты, ТипКИ, , ТекущаяДатаСеанса());
	КИКонтрагентовПовторныхЛидов = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(
	КонтрагентыПовторныхЛидов, ТипКИ, , ТекущаяДатаСеанса());
	
	Индекс = 0;
	ОбработанныеОбъекты = Новый Массив;
	КоличествоКонтактов = ТаблицаУчастников.Количество();
	Пока Индекс < КоличествоКонтактов Цикл
		
		Участник = ТаблицаУчастников[Индекс];
		...
		ИначеЕсли ОбычныйКонтактЭтоКонтактCRM(Участник.Контакт) Тогда
			ГлавныйКонтакт = Участник.Контакт;
		Иначе
			Участник.КакСвязаться = "";
			Индекс = Индекс + 1;
			Продолжить;
		КонецЕсли;
		
		Если ОбработанныеОбъекты.Найти(ГлавныйКонтакт) <> Неопределено Тогда
			ТаблицаУчастников.Удалить(Индекс);
			КоличествоКонтактов = КоличествоКонтактов - 1;
			Продолжить;
		КонецЕсли;
		
		ОбработанныеОбъекты.Добавить(ГлавныйКонтакт);
		
		Если ТипЗнч(ГлавныйКонтакт) = Тип("СправочникСсылка.Лиды")
			И ЗначениеЗаполнено(КонтрагентыПоЛидам[ГлавныйКонтакт]) Тогда
			
			ОбъектКИ = КонтрагентыПоЛидам[ГлавныйКонтакт];
			ТаблицаКИ = КИКонтрагентовПовторныхЛидов;
			
		Иначе
			ОбъектКИ = ГлавныйКонтакт;
			ТаблицаКИ = КИКонтактов;
		КонецЕсли;
		
		Отбор = Новый Структура("Объект", ОбъектКИ);
		КИКонтакта = ТаблицаКИ.НайтиСтроки(Отбор);
		КоличествоКИ = КИКонтакта.Количество();
		...
		Участник.КакСвязаться = КИКонтакта[0].Представление;
		
		Для ИндексКИ = 1 По КоличествоКИ - 1 Цикл
			
			НовыйУчастник = ТаблицаУчастников.Вставить(Индекс + 1);
			ЗаполнитьЗначенияСвойств(НовыйУчастник, Участник);
			Участник.КакСвязаться = КИКонтакта[ИндексКИ].Представление;
	...
	ПолнаяОбработкаПослеИзмененияСпискаПолучателей();
	
КонецПроцедуры

&НаСервере
Процедура ПолнаяОбработкаПослеИзмененияСпискаПолучателей()
...
&НаСервере
Функция ОбычныйКонтактЭтоКонтактCRM(ОбычныйКонтакт)
	
	Возврат ЗначениеЗаполнено(ОбычныйКонтакт)
	И ТипЗнч(ОбычныйКонтакт) <> Тип("Строка") И ТипЗнч(ОбычныйКонтакт) <> Тип("СправочникСсылка.АдресатыПисем");
...
Процедура ЗаполнитьНовоеПисьмоПоУмолчанию()
	...
	Объект.Ответственный = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойОтветственный");
