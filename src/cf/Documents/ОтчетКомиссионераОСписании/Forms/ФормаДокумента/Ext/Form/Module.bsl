...
// Прослеживаемость
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
		Документы.ОтчетКомиссионераОСписании.ПараметрыУказанияСерий(Объект));
...
Процедура ЗапасыНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ВыбранноеЗначение.Свойство("СтруктураНастроек") Тогда
			НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтотОбъект, "РасходнаяНакладная", НастройкиФормыВыбораНоменклатуры, Истина, "Запасы", ВыбранноеЗначение.СтруктураНастроек);
		КонецЕсли;
...
Процедура ОбработатьИзменениеУсловийДоговораКлиентПослеВызоваНаСервере()
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Договор) И ИзменилсяВидЦен Тогда
		...
		Если ПересчетНеобходим Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ОбработатьИзменениеДоговораФрагментЗавершение", ЭтотОбъект,
				ПараметрыДокумента), Сообщение, РежимДиалогаВопрос.ДаНет);
...
Процедура ДополнитьДанныеПоШтрихкодам(Контекст)
	...
	Для Каждого ТекШтрихкод Из Контекст.МассивШтрихкодов Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекШтрихкод, "ЭтоКодМаркировки")
			...
			ДополнитьДанныеПоШтрихкодамГОСИС(Контекст.ПараметрыСканированияМаркировки,
				Контекст.ДанныеФормы, ТекШтрихкод, ДанныеПоШтрихКодамГОСИС);
		Иначе
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрихкодов)
	...
	Контекст.Вставить("МассивШтрихкодов", ПодготовленныеШтрихкоды);
	...
	Если КодМаркировки Тогда
		...
		Контекст.Вставить("ПараметрыСканированияМаркировки", ПараметрыСканированияМаркировки);
		Контекст.Вставить("ДанныеФормы", ДанныеФормы);
	КонецЕсли;
	...
	Для Каждого ТекШтрихкод Из Контекст.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если ДанныеШтрихкода.Свойство("Серия") И ЗначениеЗаполнено(ДанныеШтрихкода.Серия) Тогда
				СерииНоменклатурыУНФКлиентСервер.ДобавитьСерияВСтроку(НоваяСтрока, ДанныеШтрихкода.Серия, Объект, , , "КлючСвязи");
			КонецЕсли;
