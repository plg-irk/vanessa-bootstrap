...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		КэшЗначений.Вставить("РеализацияВЕАЭС", Справочники.Контрагенты.КонтрагентРезидентТаможенногоСоюза(
			Объект.Контрагент));
	КонецЕсли;
	...
	ПоказыватьКолонкуСодержаниеВАктеВыполненныхРабот = ПерсональныеНастройкиРУНФ.Прочитать(
	...
	НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтотОбъект,
		"АктВыполненныхРабот", НастройкиФормыВыбораНоменклатуры, Истина, "РаботыИУслуги");
	НоменклатураВДокументахКлиентСервер.ОбновитьПараметрыОткрытияФормыВыбора(ЭтотОбъект, "РаботыИУслуги");
...
Процедура ПриОткрытии(Отказ)
	...
	СтатистикаИспользованияФормКлиент.ПроверитьЗаписатьСтатистикуКоманды("СоздатьНаОсновании.АктВыполненныхРабот",
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновлениеФормыАктВыполненныхРабот" И УникальныйИдентификатор = Источник Тогда
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если Не КэшЗначений.Свойство("СчетФактураСсылка") Тогда
		...
		Если ТипЗнч(СчетФактураСсылка) = Тип("Структура") Тогда
			КэшЗначений.СчетФактураСсылка = СчетФактураСсылка.Ссылка;
		КонецЕсли;
	КонецЕсли;
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	ЗадатьВопросОбОчисткеПредоплатыИОбработатьИзменениеКонтрагента(Элемент);
...
Процедура ДоговорПриИзменении(Элемент)
	...
	Иначе
		...
		Объект.ЗаказПокупателя = Заказ;	  // Восстанавливаем автоматически очищенный Заказ.
...
Процедура ДоговорНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы = ПараметрыФормыВыбора(Объект.Ссылка, Объект.Организация, Объект.Контрагент, Объект.Договор);
...
Процедура ДокументОснованиеНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "выбрать" Тогда
		...
		Иначе
			ПараметрРежимВыбора = Новый Структура;
			ПараметрРежимВыбора.Вставить("РежимВыбора", Истина);
			ПараметрыФормы = Новый Структура("Отбор", ПараметрРежимВыбора);
			ОповещениеОЗакрытии = Новый ОписаниеОповещения("ВыбратьОснованиеЗавершение", ЭтотОбъект);
			ОткрытьФорму("Документ.СчетНаОплату.ФормаВыбора", ПараметрыФормы, ЭтотОбъект, , , , ОповещениеОЗакрытии);
		КонецЕсли; 
...
Процедура СкидкаПроцентПриИзменении(Элемент)
	...
	РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРаботыИУслуги
...
Процедура РаботыИУслугиНоменклатураПриИзменении(Элемент)
	...
	Если ЗапрашиватьСбросКоличества И СбрасыватьКоличество
		...
		Оповещение = Новый ОписаниеОповещения("РаботыИУслугиПодтверждениеСбросаКоличестваЗавершение", ЭтотОбъект,
			ДопПараметр);
...
&НаКлиенте
Процедура РаботыИУслугиНоменклатураАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
	СтандартнаяОбработка)
...
&НаКлиенте
Процедура РаботыИУслугиНоменклатураОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных,
	СтандартнаяОбработка)
...
Процедура РаботыИУслугиСодержаниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокФормыРедактирования = СтрШаблон(НСтр("ru='%1: содержание'"), ТекущаяСтрока.Номенклатура);
...
Процедура РаботыИУслугиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.РаботыИУслугиРНПТ И СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
		ПрослеживаемостьФормыКлиентУНФ.ОткрытьФормуПодбораРНПТ(ЭтотОбъект, "РаботыИУслуги", Ложь);
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура РаботыИУслугиПриАктивизацииЯчейки(Элемент)
	...
	Если Элемент.ТекущийЭлемент <> Неопределено И Элемент.ТекущийЭлемент.Имя = "РаботыИУслугиКоличество" Тогда
		Элементы.РаботыИУслугиКоличество.ФорматРедактирования = ?(СтрокаТабличнойЧасти.ЗапретитьВводДробногоКоличества
...
Процедура РаботыИУслугиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ЗаполнениеОбъектовУНФКлиент.ЗаполнитьСтрокуПоШапке(СтрокаТабличнойЧасти, Объект, "ЗаказНаПроизводство",
...
Процедура РаботыИУслугиПометкаПриИзменении(Элемент)
	ГрупповоеИзменениеСтрокКлиент.СтрокаПометкаПриИзменении(Объект.РаботыИУслуги,
		Элементы.РаботыИУслугиУстановитьФлажки, Элементы.РаботыИУслугиСнятьФлажки);
КонецПроцедуры
...
Процедура ПредоплатаДокументПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Документ) Тогда
		...
		СтруктураДанные = РасчетыРаботаСФормамиВызовСервера.ПолучитьДанныеДокументПриИзменении(
			СтрокаТабличнойЧасти.Документ, Объект.Контрагент, Объект.Договор);
...
Процедура ПредоплатаПередУдалением(Элемент, Отказ)
	...
	Если Не Элемент.ТекущиеДанные.ОплатаБонусами Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьОплатуСертификатом(Команда)
	...
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("КОплате", Объект.РаботыИУслуги.Итог("Всего") - Объект.Предоплата.Итог(
		"СуммаРасчетов"));
	СтруктураПараметров.Вставить("Документ", Объект.Ссылка);
	СтруктураПараметров.Вставить("Товары", Объект.РаботыИУслуги);
...
Процедура РассчитатьСкидкиНаценки(Команда)
	...
	ОчиститьИРассчитатьСкидкиНаценки();
...
Процедура ОткрытьПомощникЗачетаПредоплаты(Команда) Экспорт
	...
	Если Объект.Ссылка.Пустая() Или Объект.Дата <> ДатаПриСозданииНаСервере Тогда
		
		Оповещение = Новый ОписаниеОповещения("ЗаписатьИОткрытьФормуРаспределенияПлатежа", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Перед открытием помощника данные будут записаны.
...
&НаКлиенте
Процедура ЗадатьВопросОбОчисткеПредоплатыИОбработатьИзменениеКонтрагента(Элемент)
...
Процедура РаботыИУслугиПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПодбор") И ДополнительныеПараметры.ЭтоПодбор Тогда
		...
		МассивНомеровИдентичныхСтрок = Новый Массив;
		ПолучитьРаботыИУслугиИзХранилища(ДополнительныеПараметры.АдресЗапасовВХранилище, "РаботыИУслуги", Истина, Ложь,
...
Процедура ВыборТипаОснованияЗавершение(ВыбранноеИмяФормы, Параметры) Экспорт
	...
	Если ВыбранноеИмяФормы.Значение = "Документ.ПриемИПередачаВРемонт.ФормаВыбора" Тогда
		ПараметрыОтбора.Вставить("Отбор", Новый Структура("ВариантРемонта", ПредопределенноеЗначение(
...
Функция ПолучитьДанныеНоменклатураПриИзменении(Знач ДанныеНоменклатуры)
	...
	Если Не ДанныеНоменклатуры.Свойство("Коэффициент") Тогда
		Если ТипЗнч(ДанныеНоменклатуры.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
			ДанныеНоменклатуры.Вставить("Коэффициент", ДанныеНоменклатуры.ЕдиницаИзмерения.Коэффициент);
		Иначе
	...
	Если ДанныеНоменклатуры.Свойство("ВидСкидкиНаценки") И ЗначениеЗаполнено(ДанныеНоменклатуры.ВидСкидкиНаценки) Тогда
		ПроцентСкидкиНаценки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеНоменклатуры.ВидСкидкиНаценки, "Процент");
		ДанныеНоменклатуры.Вставить("ПроцентСкидкиНаценки", ПроцентСкидкиНаценки);
	КонецЕсли;
	...
	Если ДанныеНоменклатуры.Свойство("ПроцентСкидкиПоДисконтнойКарте")
		...
		ДанныеНоменклатуры.Вставить("ПроцентСкидкиНаценки", ТекПроцент
			+ ДанныеНоменклатуры.ПроцентСкидкиПоДисконтнойКарте);
	КонецЕсли;
	...
	ДанныеНоменклатуры.Вставить("ТипНоменклатуры", СвойстваНоменклатуры.ТипНоменклатуры);
...
Процедура УстановитьВидимостьОтПользовательскихНастроек(Форма)
	...
	Если Форма.Объект.ПоложениеЗаказаПокупателя <> ПредопределенноеЗначение(
	...
	Если Форма.Объект.ПоложениеЗаказаНаПроизводство <> ПредопределенноеЗначение(
		"Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
	...
	Если Форма.Объект.ПоложениеПроекта <> ПредопределенноеЗначение(
		"Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
...
&НаСервереБезКонтекста
Функция ПараметрыФормыВыбора(Знач Документ, Знач Организация, Знач Контрагент, Знач Договор)
	
	СписокВидовДоговоров = Справочники.ДоговорыКонтрагентов.ПолучитьСписокВидовДоговораДляДокумента(Документ);
	
	Результат = Новый Структура;
	Результат.Вставить("КонтролироватьВыборДоговора", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент,
...
&НаСервере
Процедура ПолучитьРаботыИУслугиИзХранилища(Знач АдресЗапасовВХранилище, Знач ИмяТабличнойЧасти,
	Знач ЕстьХарактеристики, Знач ЕстьПартии, МассивНомеровИдентичныхСтрок = Неопределено)
	...
	Для каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаЗагрузки.Номенклатура) Тогда
			...
			ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаЗагрузки.Номенклатура,
				"ЭтоНабор, СпособРасчетаЦеныНабора, ТипНоменклатуры");
			...
			Если ЗначенияРеквизитов.ЭтоНабор Тогда
				...
				Если ЗначениеЗаполнено(НоваяСтрока.Цена) 
					И ЗначенияРеквизитов.СпособРасчетаЦеныНабора
				...
				ЦенообразованиеКлиентСервер.ДополнитьСтруктуруДанныхНоменклатурыПолямиЦенообразования(Объект,
					Неопределено, СтруктураДанныеНоменклатуры);
...
// Помещает список авансов во временное хранилище и возвращает адрес
//
&НаСервере
...
// Получает список авансов из временного хранилища
//
&НаСервере
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РаботыИУслуги.НоменклатураНабора", Справочники.Номенклатура.ПустаяСсылка(),
		ВидСравненияКомпоновкиДанных.НеРавно);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление,
		"РаботыИУслугиНоменклатура,РаботыИУслугиХарактеристика,РаботыИУслугиКоличество,РаботыИУслугиЕдиницаИзмерения");
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(ГруппаОтборов, "Объект.РаботыИУслуги.СуммаСкидкиНаценки",
		Новый ПолеКомпоновкиДанных("Объект.РаботыИУслуги.Сумма"), ВидСравненияКомпоновкиДанных.Равно);
	...
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РаботыИУслуги.ПроцентСкидкиНаценки", 0, ВидСравненияКомпоновкиДанных.Равно);
	...
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РаботыИУслуги.СуммаСкидкиНаценки", 0, ВидСравненияКомпоновкиДанных.Равно);
	...
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РаботыИУслуги.ПроцентАвтоматическойСкидки", 0, ВидСравненияКомпоновкиДанных.Равно);
	...
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РаботыИУслуги.СуммаАвтоматическойСкидки", 0, ВидСравненияКомпоновкиДанных.Равно);
	...
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РаботыИУслуги.ТипНоменклатуры", Перечисления.ТипыНоменклатуры.Услуга);
	...
	// Минимальные цены
	ЦенообразованиеСервер.УстановитьУсловноеОформлениеМинимальнойЦены(УсловноеОформление, "РаботыИУслуги", ?(
...
Процедура УстановитьВидимостьИДоступность(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаДокументЗаказНаПроизводство",
		"Видимость", Форма.ИспользоватьПроизводство);
	...
	Если ЗначениеЗаполнено(Объект.ДисконтнаяКарта) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПредоплатаДобавитьОплатуБонусами",
		"Видимость", ВидимостьОплатыБонусами);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "РаботыИУслугиРНПТ", "Видимость",
		Форма.КэшЗначений.ВестиУчетПрослеживаемыхТоваров);
...
Функция ДанныеКонтрагентаДляИзменения(Знач Дата, Знач ВалютаДокумента, Знач Контрагент, Знач Организация)
	...
	РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент,
		"КонтактноеЛицоПодписант, ВестиРасчетыПоЗаказам");
	Результат.Вставить("КонтактноеЛицоПодписант", РеквизитыКонтрагента.КонтактноеЛицоПодписант);
	Результат.Вставить("ВестиРасчетыПоЗаказам", РеквизитыКонтрагента.ВестиРасчетыПоЗаказам);
...
Функция ДанныеДоговораДляИзменения(Знач Дата, Знач Договор, Знач Контрагент)
	...
	ВалютаРасчетовКурсКратность = РаботаСКурсамиВалют.ПолучитьКурсВалюты(СвойстваДоговора.ВалютаРасчетов, Дата);
	...
	Если ЗначениеЗаполнено(СвойстваДоговора.ВидЦен) Тогда
		Результат.СуммаВключаетНДС = СвойстваДоговора.ВидЦенЦенаВключаетНДС;
	КонецЕсли;
...
Процедура ОткрытьФормуРеквизитыПечати()
	...
	ОткрытьФорму("Обработка.РеквизитыПечати.Форма.РеквизитыПечатиАктВыполненныхРабот", ПараметрыОткрытия, ЭтотОбъект, ,
		, , ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьИРассчитатьСкидкиНаценки()
...
Процедура ВыбранаДисконтнаяКарта(ДисконтнаяКарта)
	...
	ВладелецДисконтнойКарты = ВладелецДисконтнойКарты(ДисконтнаяКарта);
	Если Объект.Контрагент.Пустая() И Не ВладелецДисконтнойКарты.Пустая() Тогда
		Объект.Контрагент = ВладелецДисконтнойКарты;
		ЗадатьВопросОбОчисткеПредоплатыИОбработатьИзменениеКонтрагента(Элементы.Контрагент);
		
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Заполнен контрагент и считана дисконтная карта'"),
			ПолучитьНавигационнуюСсылку(ДисконтнаяКарта),
			СтрШаблон(НСтр("ru = 'В документе заполнен контрагент и считана дисконтная карта %1'"), ДисконтнаяКарта),
	...
	Иначе
		ПоказатьОповещениеПользователя(
			...
			ПолучитьНавигационнуюСсылку(ДисконтнаяКарта),
			СтрШаблон(НСтр("ru = 'Считана дисконтная карта %1'"), ДисконтнаяКарта),
			БиблиотекаКартинок.Информация32);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ВладелецДисконтнойКарты(Знач ДисконтнаяКарта)
...
Функция СкидкиИзменились(Объект, РасчетныеСкидки)
	...
	Иначе
		...
		Если РасчетныеСкидки.Свойство("СуммаОплатыБонусами") 
			И НЕ Объект.РаботыИУслуги.Итог("СуммаСкидкиОплатыБонусом") = РасчетныеСкидки.СуммаОплатыБонусами Тогда
			СкидкиИзменились = Истина;
		КонецЕсли;
...
Процедура ОпределитьОбъектИзменений()
	...
	Если РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьЦеныНаПроцент") Тогда
	...
	ИначеЕсли РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.ОкруглитьЦены") Тогда
		
		РаботыИУслугиИзменениеСтрокОбъектИзмененийРеквизит = "Цена";
		РаботыИУслугиИзменениеСтрокОбъектИзмененийЭлемент = "РаботыИУслугиЦена";
		
	ИначеЕсли РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.ОкруглитьСуммы") Тогда
	...
	ИначеЕсли РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьПроцентСкидкиНаценки") Тогда
		
		РаботыИУслугиИзменениеСтрокОбъектИзмененийРеквизит = "ПроцентСкидкиНаценки";
		РаботыИУслугиИзменениеСтрокОбъектИзмененийЭлемент = "РаботыИУслугиПроцентСкидкиНаценки";
		
	ИначеЕсли РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоКоличеству") Тогда
		
		РаботыИУслугиИзменениеСтрокОбъектИзмененийРеквизит = "Сумма";
		РаботыИУслугиИзменениеСтрокОбъектИзмененийЭлемент = "РаботыИУслугиСумма";
		
	ИначеЕсли РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам") Тогда
		
		РаботыИУслугиИзменениеСтрокОбъектИзмененийРеквизит = "Сумма";
		РаботыИУслугиИзменениеСтрокОбъектИзмененийЭлемент = "РаботыИУслугиСумма";
		
	ИначеЕсли РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьСтавкуНДС") Тогда
	...
	ИначеЕсли РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.УстановитьЗаказПокупателя") Тогда
	...
	ИначеЕсли РаботыИУслугиИзменениеСтрокДействие = ПредопределенноеЗначение(
		"Перечисление.ДействияГрупповогоИзмененияСтрок.УдалитьСтроки") Или РаботыИУслугиИзменениеСтрокДействие
		= ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ДобавитьИзДокумента") Тогда
...
Функция НаборЭлементовГрупповогоИзмененияСтрокКлиент()
	...
	НаборЭлементов.Вставить("Действие", РаботыИУслугиИзменениеСтрокДействие);
	НаборЭлементов.Вставить("ДействиеЭлемент", Элементы.РаботыИУслугиИзменениеСтрокДействие);
	НаборЭлементов.Вставить("Значение", РаботыИУслугиИзменениеСтрокЗначение);
	...
	НаборЭлементов.Вставить("КолонкаОбъектИзменений", ?(ЗначениеЗаполнено(
		РаботыИУслугиИзменениеСтрокОбъектИзмененийЭлемент),
		Элементы[РаботыИУслугиИзменениеСтрокОбъектИзмененийЭлемент], Неопределено));
...
Процедура ЗаполнитьСписокДействий(РазрешеноРедактированиеЦенДокументов = Неопределено)
	...
	Если РаботыИУслугиИзменениеСтрокДействие = Перечисления.ДействияГрупповогоИзмененияСтрок.УстановитьЗаказПокупателя
		...
		ГрупповоеИзменениеСтрокСервер.НастроитьИсходнуюПанельРедактирования(РаботыИУслугиИзменениеСтрокЗначение,
			НаборЭлементовГрупповогоИзмененияСтрокСервер());
	КонецЕсли;
...
Процедура ПолучитьДанныеПоШтрихКодам(СтруктураДанные)
			...
			Для Каждого ДанныеШтрихкода Из МассивДанныхШтрихкода Цикл
				...
				Если ЗначениеЗаполнено(СтруктураДанные.ВидЦен) Тогда
					...
					Если ЗначениеЗаполнено(ДанныеШтрихкода.ЕдиницаИзмерения)
						...
						СтруктураДанныеНоменклатуры.Вставить("Коэффициент", ДанныеШтрихкода.ЕдиницаИзмеренияКоэффициент);
					Иначе
	...
	ПараметрыВыбораНоменклатуры = Метаданные.Документы.АктВыполненныхРабот.ТабличныеЧасти.РаботыИУслуги.Реквизиты.Номенклатура.ПараметрыВыбора;
	Для Каждого ТекущийПараметрВыбора Из ПараметрыВыбораНоменклатуры Цикл
		Если ТекущийПараметрВыбора.Имя = "Отбор.ТипНоменклатуры" Тогда
			СтруктураДанные.Вставить("ОтборТипНоменклатуры", ТекущийПараметрВыбора.Значение);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если ЗапрашиватьОбъединениеСтрок И МассивСтрокТЧ.Количество() Тогда
				...
				Оповещение = Новый ОписаниеОповещения("РаботыИУслугиПодтверждениеСбросаКоличестваЗавершение",
			...
			Если МассивСтрокТЧ.Количество() = 0 Или НЕ ОбъединятьСтрокиПриДобавлении Тогда
				...
				ЗаполнениеОбъектовУНФКлиент.ЗаполнитьСтрокуПоШапке(НоваяСтрока, Объект, "ЗаказПокупателя",
					"ПоложениеЗаказаПокупателя");
				ЗаполнениеОбъектовУНФКлиент.ЗаполнитьСтрокуПоШапке(НоваяСтрока, Объект, "ЗаказНаПроизводство",
					"ПоложениеЗаказаНаПроизводство");
				...
				Если ДанныеШтрихкода.СтруктураДанныеНоменклатуры.ЭтоНабор Тогда
					// Наборы
					ЗаменитьСтрокуРаботИУслугДаннымиНабора(ЭтотОбъект, НоваяСтрока,
...
Процедура ПолученыШтрихкоды(ДанныеШтрихкодов) Экспорт
	...
	Для Каждого ТекШтрихкод Из МассивШтрихкодов Цикл
		ДополнительныеДанныеПоШтрихкоду = ШтрихкодированиеУНФКлиент.ДополнительныеДанныеПоШтрихкоду(
			ТекШтрихкод.Штрихкод, ТипЗнч(Объект.Ссылка));
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьПредставлениеРНПТРаботы(Форма, СтрокаТабличнойЧасти,
	...
	Объект = Форма.Объект;
	СтрокиСРНПТ = Объект[ИмяТабличнойЧастиПрослеживаемости].НайтиСтроки(Новый Структура("ИдентификаторСтроки",
		СтрокаТабличнойЧасти.ИдентификаторСтроки));
	...
	Иначе
		СтрокаТабличнойЧасти.РНПТ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 и еще %2'"),
			СокрЛП(Строка(СтрокиСРНПТ[0].РНПТ)), КоличествоРНПТ - 1);
	КонецЕсли;
...
Процедура УдалитьСвязанныеЗаписи(СтрокаЗапасы)
	...
	КонецЦикла;
	ЗаполнитьПредставлениеРНПТРаботы(ЭтотОбъект, СтрокаЗапасы);
...
&НаСервере
Процедура Подключаемый_ИнформационнаяПанельСовместногоИспользованияЗавершениеДействияНаСервере(Знач Результат,
