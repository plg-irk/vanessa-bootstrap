...
// Определяет документы из массива, для которых не требуется счет-фактура.
//
// Параметры:
//  МассивДокументов - Массив из ДокументСсылка.АктВыполненныхРабот - массив ссылок на документы АктВыполненныхРабот.
//
// Возвращаемое значение:
//  Массив из ДокументСсылка.АктВыполненныхРабот - документы, для которых не требуется счет-фактура.
//
Функция СчетаФактурыНеТребуются(МассивДокументов) Экспорт
	...
	|ИЗ
	|	Документ.АктВыполненныхРабот.РаботыИУслуги КАК АктВыполненныхРабот
	|ГДЕ
	|	АктВыполненныхРабот.Ссылка В (&МассивДокументов)
	|	И (АктВыполненныхРабот.СуммаНДС > 0
	|	ИЛИ АктВыполненныхРабот.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|		ПО ВТ_ДокументыСНДС.Ссылка = АктВыполненныхРабот.Ссылка
	|ГДЕ
	|	АктВыполненныхРабот.Ссылка В (&МассивДокументов)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не Выборка.ЕстьНДС И (Не УчетнаяПолитика.ПлательщикНДС(Выборка.Организация, Выборка.Дата)
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(
			КомандыСозданияНаОсновании, Метаданные.Документы.АктВыполненныхРабот);
...
// Инициализирует таблицы значений, содержащие данные табличных частей документа Акт выполненных работ.
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
// 
// Параметры:
//  ДокументСсылкаАктВыполненныхРабот - ДокументСсылка.АктВыполненныхРабот - Ссылка на документ.
//  СтруктураДополнительныеСвойства - Структура - Структура дополнительных свойств для проведения документа.
//
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства) Экспорт
	...
	ДокументСсылкаАктВыполненныхРаботВалютаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДокументСсылкаАктВыполненныхРабот, "ВалютаДокумента");
	Запрос.УстановитьПараметр("ВалютаНакладной", ДокументСсылкаАктВыполненныхРаботВалютаДокумента);
	...
	// Прослеживаемость
	Запрос.УстановитьПараметр("ВестиУчетПрослеживаемыхТоваров",	Константы.ВестиУчетПрослеживаемыхТоваров.Получить());
	
	// Перед выполнением запроса проверим, необходим ли пересчет из валюты в валюту
	РасчетыПроведениеДокументов.МодифицироватьЗапросДляПересчетаВВалюту(ДокументСсылкаАктВыполненныхРабот, Запрос,
	...
	СформироватьТаблицаЗапасыНаСкладах(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства);
	...
	ЭквайринговыеОперацииСервер.СформироватьТаблицаДоходыИРасходыКассовыйМетодЭквайринг(
		ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства);
	...
	// Прослеживаемость
	ПрослеживаемостьУНФ.СформироватьДвиженияРеализацияТоваров(ДокументСсылкаАктВыполненныхРабот,
		СтруктураДополнительныеСвойства, "ВременнаяТаблицаРаботыИУслуги");
	// Конец Прослеживаемость
	
	СформироватьТаблицаСерииНоменклатуры(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства);
	
	// НДС
	УчетНДСУНФ.СформироватьДвиженияНДСЗаписиКнигиПродажРеализация(ДокументСсылкаАктВыполненныхРабот,
		СтруктураДополнительныеСвойства);
	УчетНДСУНФ.СформироватьДвиженияНДСРеализация0(ДокументСсылкаАктВыполненныхРабот,
		СтруктураДополнительныеСвойства);
...
Процедура ВыполнитьКонтроль(ДокументСсылкаАктВыполненныхРабот, ДополнительныеСвойства, Отказ,
	УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияПодарочныеСертификатыИзменение Тогда
		...
		|	ДвиженияПрослеживаемыеТоварыИзменение.ВидЗапасов КАК ВидЗапасов,
		...
		|			И ДвиженияПрослеживаемыеТоварыИзменение.ВидЗапасов = ПрослеживаемыеТоварыОстатки.ВидЗапасов
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки");
		...
		// Отрицательный остаток по прослеживаемым товарам
		Если Константы.ВестиУчетПрослеживаемыхТоваров.Получить()
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструПрослеживаемыеТовары(
				ДокументОбъектАктВыполненныхРабот, ВыборкаИзРезультатаЗапроса, Отказ);
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	Если ТаблицаОприходованиеПодарочныхСертификатов.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаОприходованиеПодарочныхСертификатов, ВременнаяТаблицаЗапасы);
	КонецЕсли;
...
Процедура СформироватьТаблицаЗапасыНаСкладах(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыНаСкладах", ТаблицаОприходованиеПодарочныхСертификатов);
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	//|Объединить ВСЕ
	//|
	//|" + РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийДоходыИРасходы(4);	
	ТекстЗапроса = ТекстЗапроса + "
	|
	|Объединить ВСЕ
	|
	...
	|ИЗ
	|	(" + РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийДоходыИРасходы(3)+") КАК ДанныеСертификатов
	|";	
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	РасчетыПроведениеДокументов.СформироватьДвиженияПоВзаиморасчетам(СтруктураДополнительныеСвойства,
		ДокументСсылкаАктВыполненныхРабот, Запрос.Параметры);
	РасчетыПроведениеДокументов.ПересчитатьТаблицыПоКурсамВалют(ДокументСсылкаАктВыполненныхРабот,
		СтруктураДополнительныеСвойства);
	...
	Запрос.Текст = КурсовыеРазницыУНФ.ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателями(
		Запрос.МенеджерВременныхТаблиц, Истина, НомерЗапроса, Истина);
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	//|	ВложенныйЗапросСертификаты КАК ТаблицаСертификаты
	//|";
	//ТекстЗапросаСертификаты = СтрЗаменить(ТекстЗапросаСертификаты, "ВложенныйЗапросСертификаты", "(" + РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийУправлченческий(5) + ")");
    ТекстЗапросаСертификаты = РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийУправлченческий(5);
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаСуммыДокументовРегламентированныйУчет(ДокументСсылкаАктВыполненныхРабот,
	СтруктураДополнительныеСвойства)
	
	ДоговорВалютаРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаАктВыполненныхРабот,
		"Договор.ВалютаРасчетов");
	Если ДоговорВалютаРасчетов <> СтруктураДополнительныеСвойства.НациональнаяВалюта Тогда
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаЗапасыИАгентскиеУслугиПринятые(ДокументСсылкаАктВыполненныхРабот,
	СтруктураДополнительныеСвойства)
...
Функция ПолучитьТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|        И
	|            АктВыполненныхРабот.Договор.ДоговорОбслуживанияНаправлениеДеятельности <> ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|    АктВыполненныхРабот.Организация КАК Организация,
	|    АктВыполненныхРабот.Подразделение КАК СтруктурнаяЕдиница,
	|    АктВыполненныхРабот.Подразделение КАК Подразделение,
	|    АктВыполненныхРабот.СуммаВключаетНДС
	...
	|    ВЫБОР
	|        КОГДА
	|            АктВыполненныхРаботРаботыИУслуги.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	...
	|    АктВыполненныхРаботРаботыИУслуги.Ссылка.Контрагент.СчетУчетаРасчетовСПокупателем КАК
	...
	|    ВЫБОР
	|        КОГДА
	|            АктВыполненныхРаботРаботыИУслуги.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|    ВЫБОР
	|        КОГДА
	|            ТИПЗНАЧЕНИЯ(АктВыполненныхРаботРаботыИУслуги.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	|            ТОГДА АктВыполненныхРаботРаботыИУслуги.Количество
	|        ИНАЧЕ АктВыполненныхРаботРаботыИУслуги.Количество *
	|            АктВыполненныхРаботРаботыИУслуги.ЕдиницаИзмерения.Коэффициент
	...
	|            ТОГДА АктВыполненныхРаботРаботыИУслуги.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ АктВыполненныхРаботРаботыИУслуги.Всего * АктВыполненныхРаботРаботыИУслуги.Ссылка.Курс *
	|            КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * АктВыполненныхРаботРаботыИУслуги.Ссылка.Кратность)
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА АктВыполненныхРаботРаботыИУслуги.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ АктВыполненныхРаботРаботыИУслуги.СуммаНДС * АктВыполненныхРаботРаботыИУслуги.Ссылка.Курс *
	|                КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * АктВыполненныхРаботРаботыИУслуги.Ссылка.Кратность)
	|        КОНЕЦ
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|                ТОГДА АктВыполненныхРаботРаботыИУслуги.СуммаНДС * КурсыРегВалюты.Курс *
	|                    АктВыполненныхРаботРаботыИУслуги.Ссылка.Кратность / (АктВыполненныхРаботРаботыИУслуги.Ссылка.Курс
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|            ТОГДА АктВыполненныхРаботРаботыИУслуги.Всего * КурсыРегВалюты.Курс *
	|                АктВыполненныхРаботРаботыИУслуги.Ссылка.Кратность / (АктВыполненныхРаботРаботыИУслуги.Ссылка.Курс *
	|                КурсыРегВалюты.Кратность)
	...
	|    ВЫБОР
	|        КОГДА
	|            АктВыполненныхРаботРаботыИУслуги.Ссылка.ПоложениеПроекта <> ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	...
	|    АктВыполненныхРаботРаботыИУслуги.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|    АктВыполненныхРаботРаботыИУслуги.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект,
	|    АктВыполненныхРаботРаботыИУслуги.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|            ТОГДА АктВыполненныхРаботРаботыИУслуги.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ АктВыполненныхРаботРаботыИУслуги.Сумма * АктВыполненныхРаботРаботыИУслуги.Ссылка.Курс *
	|            КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * АктВыполненныхРаботРаботыИУслуги.Ссылка.Кратность)
	...
	|        ПО (ИСТИНА)
	|        ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, Валюта = &ВалютаНакладной) КАК
	|            КурсыВалютДокументаСрезПоследних
	|        ПО (ИСТИНА),
	|    Константа.НациональнаяВалюта КАК КонстантаНациональнаяВалюта,
	...
	|            ТОГДА АктВыполненныхРаботСкидкиНаценки.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ АктВыполненныхРаботСкидкиНаценки.Сумма * АктВыполненныхРаботСкидкиНаценки.Ссылка.Курс *
	|            КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * АктВыполненныхРаботСкидкиНаценки.Ссылка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	...
	|ВЫБРАТЬ
	|    ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|    АктВыполненныхРаботСведенияПрослеживаемости.НомерСтроки КАК НомерСтроки,
	|    АктВыполненныхРаботСведенияПрослеживаемости.РНПТ КАК РНПТ,
	|    АктВыполненныхРаботСведенияПрослеживаемости.Количество КАК Количество,
	|    АктВыполненныхРаботСведенияПрослеживаемости.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|    АктВыполненныхРаботСведенияПрослеживаемости.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|    НЕОПРЕДЕЛЕНО КАК Комплектующие,
	|    АктВыполненныхРаботСведенияПрослеживаемости.Сумма КАК Сумма,
	|    АктВыполненныхРаботСведенияПрослеживаемости.СтранаПроисхождения КАК СтранаПроисхождения,
	|    ИСТИНА КАК ОтразитьВОперациях,
	|    ИСТИНА КАК МатериалыПередаютсяВСоставеРабот,
	|    АктВыполненныхРаботСведенияПрослеживаемости.Номенклатура КАК Номенклатура
	|ПОМЕСТИТЬ ВременнаяТаблицаСведенияПрослеживаемости
	|ИЗ
	|    ВременнаяТаблицаРаботыИУслуги КАК ВременнаяТаблицаРаботыИУслуги
	|        ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АктВыполненныхРабот.СведенияПрослеживаемости КАК
	|            АктВыполненныхРаботСведенияПрослеживаемости
	|        ПО ВременнаяТаблицаРаботыИУслуги.ИдентификаторСтроки = АктВыполненныхРаботСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
	|    АктВыполненныхРаботСведенияПрослеживаемости.Ссылка = &Ссылка
	...
	ТекстЗапроса = ТекстЗапроса 
		...
		"ВременнаяТаблицаПредоплата", Ложь, "ВременнаяТаблицаРаботыИУслуги");
...
//
// Параметры:
//  ДокументСсылка - ДокументСсылка - Ссылка на документ, для которого необходимо зарегистрировать цены продажи.
//  Отказ - Булево - Флаг отказа от проведения документа. Если установлен в Истина, то процедура завершается
...
Процедура ЗарегистрироватьЦеныПродажи(ДокументСсылка, Отказ) Экспорт
	...
	ТЧИсточникДанных = ЗначенияРеквизитов.РаботыИУслуги;
...
//
// Параметры:
//  ДокументСсылка - ДокументСсылка - Ссылка на документ, для которого необходимо удалить записи.
//  ТЧИсточникДанных - ТабличнаяЧасть - Табличная часть документа, содержащая номенклатуру и характеристики.
//  ВидЦен - СправочникСсылка.ВидыЦен - Вид цен, для которого необходимо удалить записи.
//  ДатаДокумента - Дата - Дата документа, для которой необходимо удалить записи.
//  Отказ - Булево - Флаг отказа от проведения операции. Если в процессе выполнения процедуры произошла ошибка,
//  то флаг устанавливается в Истина.
...
Процедура СформироватьТаблицаПродажиПоДисконтнойКарте(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	ДокументСсылкаАктВыполненныхРаботДисконтнаяКарта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДокументСсылкаАктВыполненныхРабот, "ДисконтнаяКарта");
	
	Если Не ЗначениеЗаполнено(ДокументСсылкаАктВыполненныхРаботДисконтнаяКарта) Тогда
	...
	Запрос.УстановитьПараметр("ПроцентСкидки", ДисконтныеКартыУНФСервер.ВычислитьПроцентСкидкиПоДисконтнойКарте(
		СтруктураДополнительныеСвойства.ДляПроведения.Дата, ДокументСсылкаАктВыполненныхРаботДисконтнаяКарта));
...
Процедура СформироватьТаблицаПродажиПоПредоставленнымСкидкам(ДокументСсылкаАктВыполненныхРабот,
	СтруктураДополнительныеСвойства)
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина,
	...
	|	АктВыполненныхРаботРаботыИУслуги.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	АктВыполненныхРаботРаботыИУслуги.Ссылка.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	АктВыполненныхРаботРаботыИУслуги.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект,
	|	АктВыполненныхРаботРаботыИУслуги.ИдентификаторСтроки КАК ИдентификаторСтроки1,
	|	АктВыполненныхРаботРаботыИУслуги.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар
	...
	|	ВЫБОР
	|		КОГДА АктВыполненныхРаботРаботыИУслуги.ПрослеживаемыйТовар
	|			ТОГДА 1
	|		ИНАЧЕ 0
	...
	|	АктВыполненныхРаботСведенияПрослеживаемости.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА АктВыполненныхРаботСведенияПрослеживаемости.Сумма = 0
	|			ТОГДА АктВыполненныхРаботСведенияПрослеживаемости.Количество * ВЫБОР
	|					КОГДА ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ((ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги.Сумма - ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги.СуммаНДС) / ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги.Количество КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги.Цена
	|				КОНЕЦ
	|		ИНАЧЕ АктВыполненныхРаботСведенияПрослеживаемости.Сумма
	|	КОНЕЦ КАК СтоимостьБезНалога,
	|	АктВыполненныхРаботСведенияПрослеживаемости.Номенклатура.ТоварнаяНоменклатураВЭД.ЕдиницаИзмерения.Код КАК КодТНВЭДЕдиницаИзмеренияКод,
	|	АктВыполненныхРаботСведенияПрослеживаемости.Номенклатура.ТоварнаяНоменклатураВЭД.ЕдиницаИзмерения КАК КодТНВЭДЕдиницаИзмеренияНаименование,
	|	АктВыполненныхРаботСведенияПрослеживаемости.СтранаПроисхождения.Наименование КАК ПредставлениеСтраны,
	|	АктВыполненныхРаботСведенияПрослеживаемости.СтранаПроисхождения.Код КАК СтранаПроисхожденияКод
	|ИЗ
	|	ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги КАК ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.АктВыполненныхРабот.СведенияПрослеживаемости КАК АктВыполненныхРаботСведенияПрослеживаемости
	|		ПО ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги.Ссылка = АктВыполненныхРаботСведенияПрослеживаемости.Ссылка
	|			И ВременнаяТаблица_АктВыполненныхРаботРаботыИУслуги.ИдентификаторСтроки = АктВыполненныхРаботСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
	|	АктВыполненныхРаботСведенияПрослеживаемости.Ссылка В(&МассивОбъектов)";
	...
	Для Каждого ТекущаяСтрока Из Результат Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			ТекущаяСтрока.ЕстьПрослеживаемыеТовары = ТекущаяСтрока.ТаблицаЗапасы.Итог("ПрослеживаемыйТоварЧисло") > 0 И ТзСтрокСведенияПрослеживаемости.Количество() > 0;
...
//
// Параметры:
//  МассивОбъектов  - Массив из ДокументСсылка.АктВыполненныхРабот - документы, которые нужно распечатать
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Использование", Метаданные.Отчеты.РеестрДокументов) Тогда
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "РеестрДокументовАктВыполненныхРабот";
...
Процедура СформироватьТаблицаОплатаПодарочнымиСертификатами(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст = РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийОплатыПодарочнымиСертификатами(Истина);
...
Процедура СформироватьТаблицаСерииНоменклатуры(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст = ТекстЗапросаСертификаты;
	ТаблицаДвиженияСерииНоменклатуры = Запрос.Выполнить().Выгрузить();
	ТаблицаСерииНоменклатуры = ТаблицаДвиженияСерииНоменклатуры.Скопировать();

	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаДвиженияСерииНоменклатуры", ТаблицаДвиженияСерииНоменклатуры);
