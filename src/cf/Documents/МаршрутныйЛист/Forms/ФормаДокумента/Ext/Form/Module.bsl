...
Процедура ЗаполнитьЗаказы(ПервоеЗаполнение = Ложь)
	...
	Иначе
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("ПринятьВРаботу", Истина);
		ОтгружаемыеДокументы = Заказы.Выгрузить(СтруктураОтбора, "НомерСтроки, Заказ, РасходнаяНакладная, Доставлен");
		ОтгружаемыеДокументы.Свернуть("НомерСтроки, Заказ, РасходнаяНакладная, Доставлен");    
		Для Каждого СтрокаДокумента Из ОтгружаемыеДокументы Цикл    
			СтрокаДокумента.НомерСтроки = ОтгружаемыеДокументы.Индекс(СтрокаДокумента) + 1;
		КонецЦикла;
	КонецЕсли; 
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОтгружаемыеДокументы.НомерСтроки КАК НомерСтроки,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	МИНИМУМ(МаршрутныйЛистЗаказы.НомерСтроки) КАК НомерСтроки,
	...
	|		ОтгружаемыеДокументыПлан.НомерСтроки
	|	ИЗ
	...
	|		ОтгружаемыеДокументыФакт.НомерСтроки
	|	ИЗ
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Отгружен,
	|	ЗаполняемыеЗаказыИНакладные.НомерСтроки КАК НомерСтроки
	|ИЗ
...
Процедура ЗаполнитьДанныеСклада()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Склад) И ФормаПараметры.НесколькоСкладов Тогда
		Объект.Склад = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойСклад");
		Если НЕ ЗначениеЗаполнено(Объект.Склад) Тогда
