...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_РасходыНаКассовуюТехнику" Тогда
...
Процедура УстановитьОтборыДинамическогоСписка()
	...
	Если УменьшаемыйНалог <> Перечисления.ВидыНалоговУменьшаемыхНаРасходыККТ.УСН Тогда
		// Отбор по периоду устанавливаем только для расходов, уменьшающих УСН.
		Возврат;
	КонецЕсли;
...
Функция КонецНалоговогоПериода(НалоговыйПериод, УменьшаемыйНалог)
	...
	Если УменьшаемыйНалог = ПредопределенноеЗначение("Перечисление.ВидыНалоговУменьшаемыхНаРасходыККТ.УСН") Тогда
