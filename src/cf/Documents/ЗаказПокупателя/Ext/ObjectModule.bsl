...
// * Высота - Число - 
// * Длина - Число - 
// * Вес - ОпределяемыйТип.Вес - 
// * ОбъявленнаяЦенность - Число - 
// * СуммаЗаказа - Число, Неопределено - 
// * КоличествоТоваров - Число, Неопределено - 
...
Функция РежимЗаписиПриСменеСостояния() Экспорт
	...
	Если РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ВидОперации) Тогда
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Заметки = "";
	СостояниеЗаказа = РаботаСЗаявками.ПервоеДоступноеСостояние(ВидЗаказа, ВидОперации);
	УдалитьЗакрыт = Ложь;
	КалькуляцияРассчитана = Ложь;
	Событие = Документы.Событие.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(Контрагент) Или РаботаСЛидами.ЛидЗавершен(Лид) Тогда
		Лид = Неопределено;
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		И ДанныеЗаполнения.Свойство("КонтрагентИлиЛид") Тогда
		
		Если ТипЗнч(ДанныеЗаполнения.КонтрагентИлиЛид) = Тип("СправочникСсылка.Контрагенты") Тогда
			ДанныеЗаполнения.Вставить("Контрагент", ДанныеЗаполнения.КонтрагентИлиЛид);
		Иначе
			ДанныеЗаполнения.Вставить("Лид", ДанныеЗаполнения.КонтрагентИлиЛид);
		КонецЕсли;
	...
	КонецЕсли;
 	...
 	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		И РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ДанныеЗаполнения.ВидОперации)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		И ДанныеЗаполнения.Свойство("ВидОперации")
		И РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ДанныеЗаполнения.ВидОперации) Тогда
	...
	СкоректироватьИЗаполнитьСвязанныеССостояниемРеквизитыШапки();
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПолучитьФункциональнуюОпцию("ПланироватьЗагрузкуРесурсовПредприятияРаботы") 
		...
		Если ЗначениеЗаполнено(Событие) Тогда
			СтруктураСобытие = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Событие, "ПометкаУдаления,Состояние");
			Если Не СтруктураСобытие.ПометкаУдаления И Не СтруктураСобытие.Состояние = Справочники.СостоянияСобытий.НайтиПоНаименованию(
				"Отменено") Тогда
	...
	ПроведениеДокументовУНФ.ОтразитьДвижения("НДСРеализация0", ТаблицыДляДвижений, Движения, Отказ); 
    // НДС
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если ПолучитьФункциональнуюОпцию("ПланироватьЗагрузкуРесурсовПредприятияРаботы")
		...
		Если ЗначениеЗаполнено(Событие) Тогда
			ПараметрыСобытия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Событие, "ПометкаУдаления, Состояние");
			Если Не ПараметрыСобытия.ПометкаУдаления 
				И Не ПараметрыСобытия.Состояние = Справочники.СостоянияСобытий.НайтиПоНаименованию("Отменено") Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Данные.Вставить("Лид", Лид);
	Данные.Вставить("ВидОперации", ВидОперации);
	Данные.Вставить("ЭтоНовый", ЭтоНовый());
	РаботаСЗаявками.ОбработкаПроверкиЗаполненияЗаказа(Отказ, ПроверяемыеРеквизиты, Данные);
	Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(ВидОперации) Тогда
		Возврат;
	КонецЕсли;
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() Тогда
		...
		ИначеЕсли ЭтоЗаказНаряд Тогда
			...
			Если СтрокаСостояния <> Неопределено Тогда
				...
				НомерСостоянияВыполненияВидЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидЗаказа,"НомерСостоянияВыполнения");
				
				ЗаказНарядВСостоянииВыполнения = СтрокаСостояния.НомерСтроки >= НомерСостоянияВыполненияВидЗаказа;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ЭтоТипЗаказНаряда = РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ВидОперации);
	ЭтоТипЗаказаНаПродажу = РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказаНаПродажу(ВидОперации);
	...
	Иначе
		...
		Если ОснованиеПечати = ПечатьДокументовУНФ.ПараметрЭтотДокументЗаказПокупателя() Тогда
			...
			НомерДокумента	= ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(Дата, Номер, ПрефиксОрганизации);
	...
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийЗаказПокупателя.ЗаявкаНаПродажу);
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу);
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийЗаказПокупателя.ЗаявкаЗаказНаряд);
	...
	КонецЕсли;
	Если ЭтоТипЗаказаНаПродажу И НЕ ВестиРасчетыПоЗаказам Тогда
		Предоплата.Очистить();
	КонецЕсли;
	// Конец Взаиморасчеты
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И ЭтоТипЗаказаНаПродажу Тогда
	...
	РаботаСЗаявками.ОбработкаПередЗаписьюЗаказа(Отказ, Данные);
	
	СкоректироватьИЗаполнитьСвязанныеССостояниемРеквизитыШапки(Отказ);
...
Процедура ПриЗаписи(Отказ)
	...
	Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаказ(ВидОперации) Тогда
		Данные = Новый Структура;
		Данные.Вставить("Лид", Лид);
		Данные.Вставить("Контрагент", Контрагент);
		Данные.Вставить("Ссылка", Ссылка);
		РаботаСЗаявками.ОбработкаПриЗаписиЗаказа(Отказ, Данные);
	КонецЕсли;
...
Процедура ОбработчикЗаполнения(ДанныеЗаполнения) Экспорт
	...
	Если ВалютаДокумента <> Константы.НациональнаяВалюта.Получить() Тогда
		ВалютаРасчетовДоговор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор,"ВалютаРасчетов");
		ВалютаКурсКратность = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаРасчетовДоговор, Дата);
...
Процедура ЗаполнитьПоПриемуВРемонт(ДокументСсылкаПриемВРемонт) Экспорт
	...
	СтруктураСостояниеВариантЗавершения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор,"ВидЦен,ВидСкидкиНаценки,ВалютаРасчетов");
	ЦенаВидаЦенВключаетНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураСостояниеВариантЗавершения.ВидЦен, "ЦенаВключаетНДС");
	ВидЦен = СтруктураСостояниеВариантЗавершения.ВидЦен;
	ВидСкидкиНаценки = СтруктураСостояниеВариантЗавершения.ВидСкидкиНаценки;
	ВалютаДокумента = СтруктураСостояниеВариантЗавершения.ВалютаРасчетов;
	СуммаВключаетНДС = ?(ЗначениеЗаполнено(СтруктураСостояниеВариантЗавершения.ВидЦен), ЦенаВидаЦенВключаетНДС, Неопределено);
	НалогообложениеНДС = НалогиУНФ.НалогообложениеНДС(Организация,, Дата);
	НалогиУНФ.ЗаполнитьНДСВключенВСтоимостьВДокументе(ЭтотОбъект);
	
	ВалютаКурсКратность = РаботаСКурсамиВалют.ПолучитьКурсВалюты(СтруктураСостояниеВариантЗавершения.ВалютаРасчетов, Дата);
	Курс = ВалютаКурсКратность.Курс;
	Кратность = ВалютаКурсКратность.Кратность;
...
Процедура ЗаполнитьЗаказНарядПоСчетуНаОплату(ДокументСсылкаСчетНаОплату) Экспорт
	...
	НоменклатураДоставкиДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование,"НоменклатураДоставки");
	Если ЗначениеЗаполнено(НоменклатураДоставкиДокумента) Тогда
		НоваяСтрока = Работы.Добавить();
		...
		ТипНоменклатурыНоменклатураДоставки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоменклатураДоставки,"ТипНоменклатуры");
		СтруктураПараметрыДоставки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "СтоимостьДоставки, СтавкаНДСДоставки, СуммаНДСДоставки");
		
		НоваяСтрока.ТипНоменклатурыУслуга = ТипНоменклатурыНоменклатураДоставки = Перечисления.ТипыНоменклатуры.Услуга;
		НоваяСтрока.Количество = 1;
		НоваяСтрока.Кратность = 1;
		НоваяСтрока.Коэффициент = 1;
		НоваяСтрока.Цена = СтруктураПараметрыДоставки.СтоимостьДоставки;
		НоваяСтрока.СтавкаНДС = СтруктураПараметрыДоставки.СтавкаНДСДоставки;
		НоваяСтрока.СуммаНДС = СтруктураПараметрыДоставки.СуммаНДСДоставки;
		НоваяСтрока.Сумма = СтруктураПараметрыДоставки.СтоимостьДоставки;
		НоваяСтрока.Всего = СтруктураПараметрыДоставки.СтоимостьДоставки;
...
// Заполнение шапки документа.
	...
	Если ЗначениеЗаполнено(Договор) Или НЕ ЗначениеЗаполнено(Контрагент) Тогда
		...
		ПараметрыПоискаОстаткиСерииНоменклатуры = Новый Структура("Номенклатура, Характеристика, Партия");
		...
		Пока ВыборкаЗапасы.Следующий() Цикл
			...
			Иначе
				...
				КоэффициентЕИ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаЗапасы.ЕдиницаИзмерения,"Коэффициент");
				
				КоличествоСтрокиВЕдиницахХранения = НоваяСтрокаЗапасы.Количество * КоэффициентЕИ;
				...
				НоваяСтрокаЗапасы.Количество = КоличествоВСтрокеЗапасов/ КоэффициентЕИ;
			...
			Если ОблагаетсяНДС Тогда
				...
				СтатусНалогообложениеПартии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Партия, "Статус, НалогообложениеНДС");
				Если ЗначениеЗаполнено(Партия) И СтатусНалогообложениеПартии.Статус = Перечисления.СтатусыПартий.ТоварыНаКомиссии 
					И СтатусНалогообложениеПартии.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
					
					НоваяСтрокаЗапасы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(Перечисления.ВидыСтавокНДС.БезНДС);;
				Иначе
					ВидСтавкиНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаЗапасы.Номенклатура, "ВидСтавкиНДС");
					Если ЗначениеЗаполнено(ВидСтавкиНоменклатуры) Тогда
						НоваяСтрокаЗапасы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ВидСтавкиНоменклатуры, ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
					Иначе
						ВидСтавкиКомпания = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Компания, "ВидСтавкиНДСПоУмолчанию");					
						НоваяСтрокаЗапасы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ВидСтавкиКомпания, ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
					КонецЕсли;
			...
			Если УчетГТД Тогда
				СтранаПроисхожденияНоменклатура = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаЗапасы.Номенклатура, "СтранаПроисхождения");					
				НоваяСтрокаЗапасы.СтранаПроисхождения = СтранаПроисхожденияНоменклатура;
			КонецЕсли;
...
//
// Параметры:
//	ДанныеЗаполнения - ДокументСсылка.ПриходныйОрдер, ДокументСсылка.РасходныйОрдер - данные заполнения.
...
Процедура ЗаполнитьТабличнуюЧастьИсполнителиПоРесурсам(ИсполнителиКлючСвязи) Экспорт
	...
	Для каждого СтрокаТЧ Из РесурсыПредприятия Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.РесурсПредприятия) Тогда
			...
			ЗначениеРесурсаРесурсПредприятия =ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТЧ.РесурсПредприятия, "ЗначениеРесурса");
			ЗначениеРесурса = ЗначениеРесурсаРесурсПредприятия;
	...
	Если ИсполнителиКлючСвязи = Неопределено Тогда
		...
		Для каждого СтрокаТабличнойЧасти Из Работы Цикл
			ТипНоменклатурыНоменклатура =ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТабличнойЧасти.Номенклатура, "ТипНоменклатуры");
			
			Если ТипНоменклатурыНоменклатура = Перечисления.ТипыНоменклатуры.Работа Тогда
...
Процедура ТоварыЗаполнитьКолонкуРезервПоОстаткам() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивСтрокЗапасы = Запасы.НайтиСтроки(СтруктураДляПоиска);
		Для каждого СтрокаЗапасы Из МассивСтрокЗапасы Цикл
			Если ТипЗнч(СтрокаЗапасы.ЕдиницаИзмерения) <> ТИП("СправочникСсылка.КлассификаторЕдиницИзмерения") Тогда
				КоэффициентЕИ =  ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаЗапасы.ЕдиницаИзмерения,"Коэффициент");
			КонецЕсли;
			
			ВсегоОстаток = ?(ТипЗнч(СтрокаЗапасы.ЕдиницаИзмерения) = ТИП("СправочникСсылка.КлассификаторЕдиницИзмерения"), ВсегоОстаток, ВсегоОстаток / КоэффициентЕИ);
			Если СтрокаЗапасы.Количество >= ВсегоОстаток Тогда
				СтрокаЗапасы.Резерв = ВсегоОстаток;
...
Процедура ЗаполнитьДоставкуНаОснованииСчета(ДанныеЗаполнения)
	...
	ДоступныеСпособыДоставкиСлужбы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СлужбаДоставки, "ДоступныеСпособыДоставки");								

	Если ТипЗнч(ДоступныеСпособыДоставкиСлужбы) = Тип("РезультатЗапроса") Тогда
		ДоступныеСпособыДоставкиСлужбы = ДоступныеСпособыДоставкиСлужбы.Выгрузить();			
	Иначе
		Возврат;
	КонецЕсли;
	
	Для каждого СтрокаСпособДоставки Из ДоступныеСпособыДоставкиСлужбы Цикл
...
Процедура СкоректироватьИЗаполнитьСвязанныеССостояниемРеквизитыШапки(Отказ = Ложь)
	
	Если Не Ссылка.Пустая() Тогда
		
		Если ЗначениеЗаполнено(СостояниеЗаказа)
			И Не РаботаСЗаявками.СостояниеДоступно(СостояниеЗаказа, ВидЗаказа, ВидОперации) Тогда
			
			Отказ = Истина;
			ВызватьИсключение РаботаСЗаявками.СообщениеОНедоступностиСостоянияВЗаказе(
			ВидОперации, ВидЗаказа, СостояниеЗаказа);
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Контрагент) И РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(ВидОперации) Тогда
			
			Отказ = Истина;
			ВызватьИсключение НСтр("ru = 'Контрагента нельзя использовать в заявке, так как заявки поддерживаются только для лидов'");
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(ВидОперации) Тогда
			...
			Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаявкаЗаказНаряд(ВидОперации) Тогда
				ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд;
			Иначе
				ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу;
			КонецЕсли;
			
		ИначеЕсли Не РаботаСЗаявкамиКлиентСервер.ЭтоЗаказ(ВидОперации) Тогда
			ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу;
		КонецЕсли;
		
	ИначеЕсли ЗначениеЗаполнено(Лид) Тогда
		
		Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаказ(ВидОперации) Тогда
			
			Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаказНаряд(ВидОперации) Тогда
				ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаявкаЗаказНаряд;
			Иначе
				ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаявкаНаПродажу;
			КонецЕсли;
			
		ИначеЕсли Не РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(ВидОперации) Тогда
			ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаявкаНаПродажу;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидЗаказа) Тогда
		...
		ТипВидаЗаказа = ТипЗнч(ВидЗаказа);
		
		Если Не ЗначениеЗаполнено(ВидОперации) Тогда
			
			Если ТипВидаЗаказа = Тип("СправочникСсылка.ВидыЗаказНарядов") Тогда
				ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд;
			Иначе
				ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу;
			КонецЕсли;
			
		ИначеЕсли РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ВидОперации)
			И ТипВидаЗаказа = Тип("СправочникСсылка.ВидыЗаказовПокупателей") Тогда
			
			ВидЗаказа = Справочники.ВидыЗаказНарядов.Основной;
			
		ИначеЕсли Не РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ВидОперации)
			И ТипВидаЗаказа = Тип("СправочникСсылка.ВидыЗаказНарядов") Тогда
			
			ВидЗаказа = Справочники.ВидыЗаказовПокупателей.Основной;
			
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СостояниеЗаказа)
			Или Не РаботаСЗаявками.СостояниеДоступно(СостояниеЗаказа, ВидЗаказа, ВидОперации) Тогда
			
			НовоеСостояниеЗаказа = РаботаСЗаявками.ПервоеДоступноеСостояние(ВидЗаказа, ВидОперации);
			Если Не ЗначениеЗаполнено(НовоеСостояниеЗаказа) Тогда
			...
			СостояниеЗаказа = НовоеСостояниеЗаказа;
			
		КонецЕсли;
		
	ИначеЕсли ЗначениеЗаполнено(СостояниеЗаказа) Тогда
		
		ТипСостоянияЗаказа = ТипЗнч(СостояниеЗаказа);
		
		Если Не ЗначениеЗаполнено(ВидОперации) Тогда
			
			Если ТипСостоянияЗаказа = Тип("СправочникСсылка.СостоянияЗаказНарядов") Тогда
				ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд;
			Иначе
				ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу;
			КонецЕсли;
		...
		Иначе
			...
			СостояниеНеСоответствуетВидуОперации =
			РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ВидОперации)
			И ТипСостоянияЗаказа = Тип("СправочникСсылка.СостоянияЗаказовПокупателей")
			Или Не РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ВидОперации)
			И ТипСостоянияЗаказа = Тип("СправочникСсылка.СостоянияЗаказНарядов");
			
			Если СостояниеНеСоответствуетВидуОперации Тогда
		...
		ВидЗаказа = РаботаСЗаявками.ПоследнийИспользованныйДоступныйВидЗаказа(ВидОперации, СостояниеЗаказа);
	...
	Иначе
		...
		ВидЗаказа = РаботаСЗаявками.ПоследнийИспользованныйДоступныйВидЗаказа(ВидОперации);
...
Процедура ДозаполнитьПоУмолчанию()
	...
	Если РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ВидОперации) Тогда
		...
		Если ЗначениеЗаполнено(СтруктурнаяЕдиницаРезерв) И ПоложениеСклада <> Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
			ПодписьМОЛСтруктурнаяЕдиницаРезерв = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктурнаяЕдиницаРезерв,"ПодписьМОЛ");		
			ПодписьКладовщика = ПодписьМОЛСтруктурнаяЕдиницаРезерв;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеЗапасовПоДаннымФормы(ДанныеСобытия, Операция, ДанныеЗаполнения = Неопределено)
	...
	СтруктураТемаСодержание = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеСобытия,"Тема,Содержание");
	Если СтрНайти(СтруктураТемаСодержание.Тема,"Заполнена форма") = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ИндексИдентификатораФормы = СтрНайти(СтруктураТемаСодержание.Содержание,"formId:");
	
	Если ИндексИдентификатораФормы = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ИдентификаторФормы = СокрЛП(Сред(СтруктураТемаСодержание.Содержание,ИндексИдентификатораФормы + 7, СтрНайти(СтруктураТемаСодержание.Содержание,";") - (ИндексИдентификатораФормы + 7)));
	...
	Пока Выборка.Следующий() Цикл
		...
		ИндексОтвета = СтрНайти(СтруктураТемаСодержание.Содержание, Выборка.Заголовок);
		Если ИндексОтвета = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		НачалоОтвета = ИндексОтвета + СтрДлина(Выборка.Заголовок) + 1;
		ДлинаОтвета  = СтрНайти(СтруктураТемаСодержание.Содержание,";;",,НачалоОтвета) - НачалоОтвета;
		Ответ = Сред(СтруктураТемаСодержание.Содержание, НачалоОтвета, ДлинаОтвета);
...
Функция НовыйЗапросРасшифровкиПредоплаты(Знач ТаблицаЗаказов)
	...
	ВалютаРасчетовДоговор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор,"ВалютаРасчетов");
	Если ВалютаРасчетовДоговор = ВалютаДокумента Тогда
...
Функция ТаблицаЗаказовДляЗаполненияПредоплаты()
	...
	Для каждого ТекСтрока Из Результат Цикл
		ВестиРасчетыПоЗаказамКонтрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент,"ВестиРасчетыПоЗаказам");
		ВалютаРасчетовДоговор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор,"ВалютаРасчетов");
		Если НЕ ВестиРасчетыПоЗаказамКонтрагент Тогда
		...
		ТекСтрока.Всего,
		?(ВалютаРасчетовДоговор = ВалютаДокумента, Курс, 1),
		Курс,
		?(ВалютаРасчетовДоговор = ВалютаДокумента, Кратность, 1),
		Кратность);
	КонецЦикла;
...
Процедура ЗаполнитьРеквизитыПечати()
	...
	Автомобиль = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойАвтомобиль", Организация);
	Водитель = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойВодитель", Организация);
	Перевозчик = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойПеревозчик", Организация);
	
	Если НЕ ЗначениеЗаполнено(УсловияСчетаЗаказа) Тогда
		УсловияСчетаЗаказа = ПерсональныеНастройкиРУНФ.Прочитать("УсловияСчетаЗаказа", 
			Организация);
		Если НЕ ЗначениеЗаполнено(УсловияСчетаЗаказа) Тогда
	...
	Если НЕ ЗначениеЗаполнено(УсловияГарантийногоТалона) Тогда
		УсловияГарантийногоТалона = ПерсональныеНастройкиРУНФ.Прочитать("УсловияГарантии", 
			Организация);
	КонецЕсли;

	Если НЕ ЗначениеЗаполнено(УсловияКоммерческогоПредложения) Тогда
		УсловияКоммерческогоПредложения =
			ПерсональныеНастройкиРУНФ.Прочитать("УсловияКоммерческогоПредложения", Организация);
	КонецЕсли;	

	Если НЕ ЗначениеЗаполнено(УсловияЗаказаПокупателя) Тогда
		УсловияЗаказаПокупателя = ПерсональныеНастройкиРУНФ.Прочитать("УсловияЗаказаПокупателя",
			Организация);
	КонецЕсли;	
	
	НастройкаКладовщик = ПерсональныеНастройкиРУНФ.Прочитать("ПодписьМОЛ", Организация);
...
Процедура ВыполнитьПроверкуОграниченийСертификатов(Отказ)
	...
	ИсходнаяТаблица = Запасы.Выгрузить( , "Номенклатура, Характеристика, Всего");
	ИсходнаяТаблица.Колонки.Всего.Имя = "Сумма";
	...
	КонецЦикла;
	ИсходнаяТаблица = Работы.Выгрузить( , "Номенклатура, Характеристика, Всего");
	ИсходнаяТаблица.Колонки.Всего.Имя = "Сумма";
	Для Каждого СтрокаИсходнойТаблицы Из ИсходнаяТаблица Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаТоварыУслуги.Добавить(), СтрокаИсходнойТаблицы);
	КонецЦикла;	ТаблицаТоварыУслуги.ЗаполнитьЗначения(Константы.УчетПоКомпании.Компания(Организация), "Организация");
	СтруктураДляПроверки.Вставить("Запасы", ТаблицаТоварыУслуги);
	
	Сертификаты = Предоплата.Выгрузить(
		Новый Структура("ОплатаСертификатом", Истина), "Документ, НомерСертификата, СуммаРасчетов");
	Сертификаты.Колонки.Документ.Имя = "ПодарочныйСертификат";
	Сертификаты.Колонки.СуммаРасчетов.Имя = "Сумма";
	Сертификаты.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	Сертификаты.ЗаполнитьЗначения(Константы.УчетПоКомпании.Компания(Организация), "Организация");	
	Сертификаты.Колонки.Добавить("Регистратор", Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя"));
	Сертификаты.ЗаполнитьЗначения(Ссылка, "Регистратор");
