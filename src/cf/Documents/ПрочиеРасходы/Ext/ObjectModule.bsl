...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Константы.ФункциональнаяОпцияУчетПоНесколькимНаправлениямДеятельности.Получить() Тогда
		...
		Для каждого СтрокаРасходы Из Расходы Цикл
			ТипСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаРасходы.СчетЗатрат, "ТипСчета");
			Если ТипСчета = Перечисления.ТипыСчетов.Расходы Тогда
	...
	Если НЕ ЗначениеЗаполнено(ВалютаДокумента) Тогда
		ВалютаДокумента = УправлениеНебольшойФирмойПовтИсп.ПолучитьВалютуУчета();
		Курс = 1;
		Кратность = 1;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если УчетПрочихРасчетов Тогда
		ТипСчетаКорреспонденция = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Корреспонденция, "ТипСчета");
		Если ТипСчетаКорреспонденция <> Перечисления.ТипыСчетов.Дебиторы 
			И ТипСчетаКорреспонденция <> Перечисления.ТипыСчетов.Кредиторы Тогда
		...
		Для Каждого ТекущаяСтрокаРасходы Из Расходы Цикл
			...
			ТипСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрокаРасходы.СчетЗатрат, "ТипСчета");
			
			Если НЕ ТекущаяСтрокаРасходы.Контрагент.Пустая() Тогда
				ВестиРасчетыПоДоговорам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрокаРасходы.Контрагент, "ВестиРасчетыПоДоговорам");
			Иначе
				ВестиРасчетыПоДоговорам = Ложь;
			КонецЕсли;
			
			Если ТипСчета = Перечисления.ТипыСчетов.Дебиторы 
				...
				ИначеЕсли ВестиРасчетыПоДоговорам И ТекущаяСтрокаРасходы.Договор.Пустая() Тогда
