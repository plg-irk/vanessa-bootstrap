...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ТекущаяСтрока Из Объект.Расходы Цикл
		ТекущаяСтрока.ТипСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.СчетЗатрат, "ТипСчета");
	КонецЦикла;
	...
	// Предзаполнение для старых документов
	Если НЕ ЗначениеЗаполнено(Объект.ВалютаДокумента)
		И ПравоДоступа("Редактирование", Метаданные.Документы.ПрочиеРасходы) Тогда
		
		Объект.ВалютаДокумента = УправлениеНебольшойФирмойПовтИсп.ПолучитьВалютуУчета();
		Объект.Курс = 1;
		Объект.Кратность = 1;
...
Процедура РедактироватьВалютуДокумента(Элемент, СтандартнаяОбработка)
	...
	// В данном документе курс валюты расчетов используется только для движений
	ПараметрыФормыЦеныИВалюта.КонтрагентЕстьРеквизит = Ложь;
	ПараметрыФормыЦеныИВалюта.ДоговорЕстьРеквизит = Ложь;
...
&НаКлиенте
Процедура ОбновитьДекорациюВалюты()
...
Процедура ЦеныИВалютаФормаЗакрыта(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.БылиВнесеныИзменения Тогда
		...
		ЗаполнитьЗначенияСвойств(Объект, Результат, "ВалютаДокумента");
		
		Если Результат.ПересчитатьЦены Тогда
			
			КурсДо = ?(Объект.Курс = 0, 1, Объект.Курс);
			КратностьДо = ?(Объект.Кратность = 0, 1, Объект.Кратность);
			Объект.Курс = Результат.КурсПересчетаЦен.Курс;
			Объект.Кратность = Результат.КурсПересчетаЦен.Кратность;
			
			Для каждого СтрокаРасходов Из Объект.Расходы Цикл
				
				СтрокаРасходов.Сумма = ВалютыУНФКлиентСервер.Пересчитать(СтрокаРасходов.Сумма,
									КурсДо, Объект.Курс, КратностьДо, Объект.Кратность);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ВалютаДокумента.Видимость = Форма.УчетВалютныхОпераций;
...
Функция ДанныеКонтрагентаДляИзменения(Знач Контрагент, Знач Организация, Знач Дата)
	...
	Результат.Вставить("КонтрагентНаименованиеПолное", КонтрагентНаименованиеПолное);
	Результат.Вставить("Договор", ДоговорПоУмолчанию);
	Результат.Вставить("ДоговорВалютаКурсКратность", РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаРасчетов, Дата));
	...
	УстановитьВидимостьРеквизитовРасчетов(ЭтотОбъект);
...
Функция ПолучитьПараметрыСчетаЗатратПриИзменении(Счет)
	...
	РеквизитыСчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Счет, "АналитикаДоходовИРасходов, ТипСчета");
	
	Если ЗначениеЗаполнено(РеквизитыСчета.АналитикаДоходовИРасходов) И (РеквизитыСчета.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.ПрочиеДоходы")
		ИЛИ РеквизитыСчета.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.ПрочиеРасходы") ИЛИ РеквизитыСчета.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.ПрочиеОборотныеАктивы")
		ИЛИ РеквизитыСчета.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.Расходы")) Тогда
		Параметры.Вставить("АналитикаПрочихДоходовИРасходов", Новый(РеквизитыСчета.АналитикаДоходовИРасходов));
	Иначе
		Параметры.Вставить("АналитикаПрочихДоходовИРасходов", Неопределено);
	КонецЕсли;
	Параметры.Вставить("ТипСчета", РеквизитыСчета.ТипСчета);
