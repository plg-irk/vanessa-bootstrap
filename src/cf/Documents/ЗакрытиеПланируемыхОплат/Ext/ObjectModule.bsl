...
Процедура ЗаполнитьБезОсвобожденияПредоплаты(ДокументСсылка) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Организация,ВидОперации");
		Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(ЗначенияРеквизитов.ВидОперации) Тогда
			ВызватьИсключение РаботаСЗаявкамиКлиентСервер.СообщениеОНеобходимостиПереводаЗаявкиВЗаказ(ДокументСсылка);
		КонецЕсли;
