...
Функция ПодготовитьДанныеДляПробитияЧека(ЧекКоррекции, ИдентификаторУстройства) Экспорт
	...
	СтруктураРеквизитов.Вставить("РасчетВИнтернете");
	СтруктураРеквизитов.Вставить("АдресСайта");
	СтруктураРеквизитов.Вставить("ЧасоваяЗона");
	...
	Если РеквизитыДокумента.РасчетВИнтернете Тогда
		ОбщиеПараметры.Интернет = РеквизитыДокумента.РасчетВИнтернете;
		ОбщиеПараметры.МестоРасчетов = РеквизитыДокумента.АдресСайта;
	КонецЕсли;
	...
	Для Каждого ПозицияЧека Из ЧекКоррекции.ПозицииЧека Цикл
		...
		Если ЗначениеЗаполнено(ПозицияЧека.МераКоличестваПредметаРасчета) Тогда
			Если ТипЗнч(ПозицияЧека.МераКоличестваПредметаРасчета) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
				ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ПозицияЧека.МераКоличестваПредметаРасчета, "ЕдиницаИзмеренияПоКлассификатору");
			ИначеЕсли ТипЗнч(ПозицияЧека.МераКоличестваПредметаРасчета) = Тип("СправочникСсылка.КлассификаторЕдиницИзмерения") Тогда
				ЕдиницаИзмерения = ПозицияЧека.МераКоличестваПредметаРасчета;
			ИначеЕсли ТипЗнч(ПозицияЧека.МераКоличестваПредметаРасчета) = Тип("Строка") Тогда
				Упаковка = Справочники.ЕдиницыИзмерения.НайтиПоНаименованию(
					ПозицияЧека.МераКоличестваПредметаРасчета);
				Если Упаковка.Пустая() Тогда
					ЕдиницаИзмерения = Справочники.КлассификаторЕдиницИзмерения.НайтиПоНаименованию(
						ПозицияЧека.МераКоличестваПредметаРасчета);
				Иначе
					ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
						Упаковка, "ЕдиницаИзмеренияПоКлассификатору");
				КонецЕсли;
			Иначе
				ЕдиницаИзмерения = Неопределено;
			КонецЕсли;
			Если ЗначениеЗаполнено(ЕдиницаИзмерения) Тогда
				КодЕдиницыИзмеренияПозицииЧека = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЕдиницаИзмерения, "Код");
