...
// Функция формирует и выводит печатную форму документа "Платежное поручение" по указанному макету.
//
// Параметры:
//  МассивОбъектов - Массив из ДокументСсылка.ПлатежноеПоручение - массив ссылок на документы "Платежное поручение".
//  ОбъектыПечати   - СписокЗначений из Структура - см. УправлениеПечатьюПереопределяемый.ПриПечати.ОбъектыПечати
//
// Возвращаемое значение:
//  ТабличныйДокумент - печатная форма документа "Платежное поручение".
//
...
//
// Параметры:
//  БанковскийСчет - СправочникСсылка.БанковскиеСчета - Банковский счет, для которого необходимо получить текст банка для расчетов.
//
// Возвращаемое значение:
//  Строка - Текст банка для расчетов в формате "р/с <НомерСчета> в <Банк> <Город>".
//          Если банк для расчетов не заполнен, то возвращается пустая строка.
//
Функция ПолучитьТекстБанкДляРасчетов(БанковскийСчет) Экспорт
	
	РеквизитыБанковскогоСчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(БанковскийСчет, "БанкРасчетов, НомерСчета, Банк");
	
	Если ЗначениеЗаполнено(РеквизитыБанковскогоСчета.БанкРасчетов) Тогда
		
		ГородБанка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыБанковскогоСчета.Банк, "Город");
		
		ТекстБанкДляРасчетов =
			" р/с "
			+ РеквизитыБанковскогоСчета.НомерСчета
			+ " в "
			+ РеквизитыБанковскогоСчета.Банк
			+ " "
			+ ГородБанка;
...
// Заполняет реквизиты документа для единого налогового платежа.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект.ПлатежноеПоручение - объект документа, реквизиты которого необходимо заполнить.
//
Процедура ЗаполнитьРеквизитыЕНП(ДокументОбъект) Экспорт
	...
	ДокументОбъект.КодБК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Налог, "КодБК");
	...
	Если ЗначениеЗаполнено(Получатель) Тогда
		...
		РеквизитыПолучателя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Получатель, "ИНН, КПП, НаименованиеПолное, БанковскийСчетПоУмолчанию");
		
		ДокументОбъект.Контрагент = Получатель;
		ДокументОбъект.ИННПолучателя = РеквизитыПолучателя.ИНН;
		ДокументОбъект.КПППолучателя = РеквизитыПолучателя.КПП;
		
		ДокументОбъект.СчетКонтрагента
		= ЕдиныйНалоговыйСчет.ЕдиныйБанковскийСчетГосударственногоОргана(Получатель);
		
		ТекстКорреспондента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыПолучателя.БанковскийСчетПоУмолчанию, "ТекстКорреспондента");
		ТекстБанкДляРасчетов = ПолучитьТекстБанкДляРасчетов(ДокументОбъект.СчетКонтрагента);
		Если ПустаяСтрока(ТекстКорреспондента) Тогда
			ДокументОбъект.ТекстПолучателя = РеквизитыПолучателя.НаименованиеПолное;
...
//
// Параметры:
// ТекущийДокумент - ДокументСсылка.ПлатежноеПоручение - Документ, ставку по которому надо проверить
...
Функция НаличиеСтавкиУСН(ТекущийДокумент, ВидСтавкиУСН) Экспорт
	...
	|	ПлатежноеПоручение.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ПлатежноеПоручение.РасшифровкаПлатежа КАК ПлатежноеПоручение
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СтавкиНДС КАК СтавкиНДС
	|		ПО ПлатежноеПоручение.СтавкаНДС = СтавкиНДС.Ссылка
	|			И (ПлатежноеПоручение.Ссылка = &СсылкаНаДокумент)
...
//
// Параметры:
//  ТекущийДокумент - ДокументСсылка.ПлатежноеПоручение - Документ, сумму НДС которого нужно вычислить.
//
// Возвращаемое значение:
//  Булево - Истина, если сумма НДС в документе не нулевая, иначе Ложь.
//
