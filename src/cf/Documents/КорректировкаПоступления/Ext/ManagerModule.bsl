...
// Обновляет реквизиты связанных документов корректировки поступления.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект - объект документа, для которого необходимо обновить реквизиты связанных документов корректировки.
//  Отказ - Булево - флаг отказа от проведения документа. Если в процессе обновления реквизитов произошла ошибка,
//                  то необходимо установить данный флаг в значение Истина.
...
Функция НаличиеСуммыНДС(ТекущийДокумент) Экспорт
	...
	|        ИЛИ КорректировкаПоступления.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
	|        ИЛИ
	|            КорректировкаПоступления.СтавкаНДСДоИзменения.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
	...
	|ИЗ
	|    Документ.КорректировкаПоступления.Расходы КАК КорректировкаПоступления
	|ГДЕ
	...
	|    КорректировкаПоступления.Ссылка
	|ИЗ
	|    Документ.КорректировкаПоступления.запасы КАК КорректировкаПоступления
	|ГДЕ
	|    КорректировкаПоступления.Ссылка = &СсылкаНаДокумент
	|;
	|
...
Процедура СформироватьТаблицаЗапасыКПоступлениюНаСклады(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасыКПоступлениюНаСклады.Количество < 0
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	Для Каждого СтрокаТаблицы Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПоставщиками Цикл
		Если СтрокаТаблицы.ВидДвижения = ВидДвиженияНакопления.Расход
			...
			СтрокаТаблицы.Заказ = Документы.ЗаказПоставщику.ПустаяСсылка();
		КонецЕсли;
	КонецЦикла;	
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	РезультатЗапроса = Запрос.Выполнить();
	ТаблицаУправленческийРасчетыСПоставщиками = РезультатЗапроса.Выгрузить();
	ТаблицаУправленческийРасчетыСПоставщиками.Свернуть("Период, Организация, СценарийПланирования, СчетДт, ВалютаДт,
	...
	Для Каждого СтрокаТЗ Из ТаблицаУправленческийРасчетыСПоставщиками Цикл  
		Если СтрокаТЗ.ЭтоВозвратВзаиморасчеты Тогда
			
			СтрокаРасчетов = ТаблицаРасчетыСПоставщиками.Найти(СтрокаТЗ.ТипРасчетов, "ТипРасчетов");
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства) Экспорт
	...
	|        И
	|            КорректировкаПоступления.Ссылка.Договор.ДоговорОбслуживанияНаправлениеДеятельности <> ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|        КОГДА КорректировкаПоступленияЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|            ТОГДА КорректировкаПоступленияЗапасы.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|        ИНАЧЕ КорректировкаПоступленияЗапасы.Номенклатура.ПолитикаУчетаСерий
	|    КОНЕЦ) КАК ПолитикаУчетаСерий,
	|    КорректировкаПоступленияЗапасы.Ссылка.ИсправляемыйДокументПоступления.ВидОперации КАК
	|        ВидОперацииДокументаПоступления,
	...
	|        ПО ВЫБОР
	|            КОГДА КорректировкаПоступленияЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|                ТОГДА КорректировкаПоступленияЗапасы.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|            ИНАЧЕ КорректировкаПоступленияЗапасы.Номенклатура = ПолитикиУчетаСерий.Владелец
	|        КОНЕЦ
	...
	|    ВЫБОР
	|        КОГДА
	|            ИзмененияЗапасов.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|    ВЫБОР
	|        КОГДА
	|            ИзмененияЗапасов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|            ИНАЧЕ ИзмененияЗапасов.СуммаНДС * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                &КратностьДокумента)
	|        КОНЕЦ
	...
	|                КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ИзмененияЗапасов.Всего * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|            &КратностьДокумента)
	...
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА (ИзмененияЗапасов.Всего - ИзмененияЗапасов.СуммаНДС) * КурсыРегВалюты.Курс *
	|                КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ (ИзмененияЗапасов.Всего - ИзмененияЗапасов.СуммаНДС) * &КурсДокумента * КурсыУпрВалюты.Кратность /
	...
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.СуммаРасходов * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ИзмененияЗапасов.СуммаРасходов * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|            &КратностьДокумента)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасходов,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.СуммаРасходов * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента *
	|                КурсыРегВалюты.Кратность)
	...
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.СуммаРасходовБезНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ИзмененияЗапасов.СуммаРасходовБезНДС * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс
	|            * &КратностьДокумента)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасходовБезНДС,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.СуммаРасходовБезНДС * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента *
	|                КурсыРегВалюты.Кратность)
	...
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.Себестоимость * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ИзмененияЗапасов.Себестоимость * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|            &КратностьДокумента)
	...
	|    ИзмененияЗапасов.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщиком.Валютный КАК
	...
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.Всего * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента *
	|                КурсыРегВалюты.Кратность)
	...
	|                ТОГДА ИзмененияЗапасов.СуммаНДС
	|            КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|                ТОГДА ИзмененияЗапасов.СуммаНДС * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента *
	|                    КурсыРегВалюты.Кратность)
	...
	|            ТОГДА ВременнаяТаблицаШапка.КорректируемыйДокумент
	|        КОГДА
	...
	|            ТОГДА ИзмененияРасходов.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ИзмененияРасходов.Всего * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|            &КратностьДокумента)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияРасходов.Всего * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента *
	|                КурсыРегВалюты.Кратность)
	...
	|            КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|                ТОГДА ИзмененияРасходов.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ИзмененияРасходов.СуммаНДС * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                &КратностьДокумента)
	|        КОНЕЦ
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДС,
	|    ВЫРАЗИТЬ(ВЫБОР
	...
	|        ИНАЧЕ ВЫБОР
	|            КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|                ТОГДА ИзмененияРасходов.СуммаНДС * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента *
	|                    КурсыРегВалюты.Кратность)
	...
	|            ТОГДА ИзмененияРасходов.СуммаНДС
	|        КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияРасходов.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ИзмененияРасходов.СуммаНДС * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|            &КратностьДокумента)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСЗакупкиПродажи,
	...
	|    ВЫБОР
	|        КОГДА
	|            ИзмененияРасходов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|            ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.НДСПоПриобретеннымЦенностям)
	...
	|    КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаРасчетовСПокупателем КАК
	...
	|    КорректировкаПоступленияПредоплата.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщиком КАК
	...
	|        ИНАЧЕ КорректировкаПоступленияПредоплата.СуммаПлатежа -
	|            КорректировкаПоступленияПредоплата.СуммаПлатежаДоИзменения
	|    КОНЕЦ * &КурсДокумента * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс *
	|        &КратностьДокумента) КАК ЧИСЛО(15, 2))) КАК Сумма,
	|    СУММА(ВЫБОР
	|        КОГДА НЕ КорректировкаПоступленияПредоплата.ЕстьВДокументеПоступления
	|            ТОГДА КорректировкаПоступленияПредоплата.СуммаРасчетов
	|        ИНАЧЕ КорректировкаПоступленияПредоплата.СуммаРасчетов -
	|            КорректировкаПоступленияПредоплата.СуммаРасчетовДоИзменения
	|    КОНЕЦ) КАК СуммаВал
	...
	|ИЗ
	|    Документ.КорректировкаПоступления.СерииНоменклатурыДоКорректировки КАК
	|        КорректировкаПоступленияСерииНоменклатурыДоКорректировки
	|        ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы
	...
	|    ИЗ
	|        Документ.КорректировкаПоступления.СведенияПрослеживаемости КАК
	|            КорректировкаПоступленияСведенияПрослеживаемости
	|    ГДЕ
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаКорректировкаПоступления);
	ДокументПоступления = ПолучитьИсправляемыйДокументПоступления(ДокументСсылкаКорректировкаПоступления, Истина);
	...
	// НДС
	УчетНДСУНФ.СформироватьДвиженияНДСКорректировкиПоступлениеТоваровУслуг(ДокументСсылкаКорректировкаПоступления,
		СтруктураДополнительныеСвойства, ДокументПоступления);
...
//
Процедура ЗарегистрироватьЦеныПоставщика(ДокументСсылка) Экспорт
	...
	Если ТипЗнч(ДокументОснование) <> Тип("ДокументСсылка.ПриходнаяНакладная")
		ИЛИ НЕ ЗначениеЗаполнено(ВидЦенКонтрагента) Тогда
		Возврат;
	КонецЕсли; 
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗаписей Цикл
		...
		Запись.ВидЦенКонтрагента 	= ВидЦенКонтрагента;
		Запись.Период 				= Дата;
