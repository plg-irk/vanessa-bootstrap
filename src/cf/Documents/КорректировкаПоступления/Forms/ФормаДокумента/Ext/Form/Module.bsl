...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтотОбъект,
		"КорректировкаПоступления", НастройкиФормыВыбораНоменклатуры);
	НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтотОбъект,
		"КорректировкаПоступления", НастройкиФормыВыбораНоменклатуры, , "Расходы");
	...
	ПоказыватьКолонкуСодержаниеВКорректировкеПоступленияЗапасы = ПерсональныеНастройкиРУНФ.Прочитать(
		"ПоказыватьКолонкуСодержаниеВКорректировкеПоступленияЗапасы");
	ПоказыватьКолонкуСодержаниеВКорректировкеПоступленияРасходы = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура СчетФактураНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Элементы.ГруппаСчетФактураНомерДата.Видимость Тогда
		...
		Если Элементы.СчетФактураНомер.Видимость И Не ЗначениеЗаполнено(СчетФактураНомер) Тогда
			Сообщение = Новый СообщениеПользователю;
		...
		Если Элементы.СчетФактураИсправление.Видимость И Не ЗначениеЗаполнено(СчетФактураИсправление) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Не заполнен номер исправления счет-фактуры.'");
			Сообщение.Поле = "СчетФактураИсправление";
			Сообщение.Сообщить();
			ОшибкиЗаполнения = Истина;
		КонецЕсли;
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.СчетФактураНомер.Видимость = НЕ Элементы.ИсправлениеНомерДата.Видимость;
	Элементы.СчетФактураИсправление.Видимость = Элементы.ИсправлениеНомерДата.Видимость;
	...
	//НДС
	КорректировочныйСчетФактура = Объект.ВидОперации = ПредопределенноеЗначение(
		"Перечисление.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение");

	ИсправлениеКорректировки = Объект.ВидОперации = ПредопределенноеЗначение(
		"Перечисление.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки") И ТипЗнч(
		Объект.ИсправляемыйДокументПоступления) = Тип("ДокументСсылка.КорректировкаПоступления");

	Элементы.ВосстановитьНДС.Видимость = (КорректировочныйСчетФактура Или ИсправлениеКорректировки)
	И ПлательщикНДС; 
