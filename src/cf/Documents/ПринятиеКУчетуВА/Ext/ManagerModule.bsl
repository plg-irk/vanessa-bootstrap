...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПринятиеКУчетуВА, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА
	|			ТаблицаДокумента.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ТаблицаДокумента.ДокументПоступления
	|		КОГДА
	|			ТаблицаДокумента.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаПринятиеКУчетуВА, СтруктураДополнительныеСвойства)
	...
	// Получение суммы без НДС при отключенной опции выделения НДС
	Если НЕ ПолучитьФункциональнуюОпцию("СебестоимостьБезНДС") Тогда
		ЗаполнитьСуммуБезНДСПоДаннымЗапасов(ДокументСсылкаПринятиеКУчетуВА, СтруктураДополнительныеСвойства);
	КонецЕсли;
...
Процедура ЗаполнитьСуммуБезНДСПоДаннымЗапасов(ДокументСсылкаПринятиеКУчетуВА, СтруктураДополнительныеСвойства)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при попытке заблокировать регистр Запасы'", ОбщегоНазначения.КодОсновногоЯзыка()), 
