...
Процедура ЗаполнитьУволенными(Команда)
	...
	Если Объект.ОценочныеОбязательстваПоСотрудникам.Количество() > 0
		Или Объект.ОценочныеОбязательства.Количество() > 0 Тогда
		Оповещение = Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект, Новый Структура("ИмяПоказателяДопОтбора", "Уволен"));
		ТекстВопроса = НСтр("ru = 'Табличные части документа будут очищены. Продолжить?'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьЗавершение(КодВозвратаДиалога.Да, Новый Структура("ИмяПоказателяДопОтбора", "Уволен"));
	КонецЕсли;
...
Процедура ЗаполнитьПереведенными(Команда)
	...
	Если Объект.ОценочныеОбязательстваПоСотрудникам.Количество() > 0
		Или Объект.ОценочныеОбязательства.Количество() > 0 Тогда
		Оповещение = Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект, Новый Структура("ИмяПоказателяДопОтбора", "Перемещен"));
		ТекстВопроса = НСтр("ru = 'Табличные части документа будут очищены. Продолжить?'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьЗавершение(КодВозвратаДиалога.Да, Новый Структура("ИмяПоказателяДопОтбора", "Перемещен"));
	КонецЕсли;
