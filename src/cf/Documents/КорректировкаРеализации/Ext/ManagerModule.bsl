...
Функция СчетаФактурыНеТребуются(МассивДокументов) Экспорт
    ...
    |ИЗ
    |    Документ.КорректировкаРеализации.Запасы КАК КорректировкаРеализации
    |ГДЕ
    ...
    |ГДЕ
    |    КорректировкаРеализации.Ссылка В (&МассивДокументов)
    |    И (КорректировкаРеализации.Договор.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоров.СКомиссионером)
    |    И КорректировкаРеализации.ИсправляемыйДокументРеализации ССЫЛКА Документ.РасходнаяНакладная)";
...
Процедура СформироватьТаблицаЗапасыКРасходуСоСкладов(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасыКРасходуСоСкладов.Количество > 0
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	Для Каждого СтрокаТаблицы Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями Цикл
		Если СтрокаТаблицы.ВидДвижения = ВидДвиженияНакопления.Расход
			И СтрокаТаблицы.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	|	ИСТИНА КАК ЭтоВозвратВзаиморасчеты,
	...
	Для Каждого СтрокаТЗ Из ТаблицаУправленческийРасчетыСПокупателями Цикл  
		Если СтрокаТЗ.ЭтоВозвратВзаиморасчеты Тогда
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("СуммаВключаетНДС", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаКорректировкаРеализации, "СуммаВключаетНДС"));
	...
	// НДС
	УчетНДСУНФ.СформироватьДвиженияКорректировкиРеализацияТоваровУслуг(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства, ДокументРеализации);
...
Функция ТекстЗапросаИнициализироватьДанныеДокумента() 
	...
	|        И
	|            КорректировкаРеализации.Договор.ДоговорОбслуживанияНаправлениеДеятельности <> ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
	...
	|    ЕСТЬNULL(КорректировкаРеализации.Договор.НеУчитыватьЗаказыПриПередачеНаКомиссию, ЛОЖЬ) КАК
	...
	|    КорректировкаРеализацииЗапасы.Ссылка.ИсправляемыйДокументРеализации.ВидОперации КАК
	|        ВидОперацииИсправляемогоДокумента,
	|    ВЫБОР
	|        КОГДА
	...
	|    ВЫБОР
	|        КОГДА
	|            ИзмененияЗапасов.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|                ТОГДА ИзмененияЗапасов.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность /
	|                    (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ИзмененияЗапасов.СуммаНДС * ИзмененияЗапасов.Ссылка.Курс * КурсыУпрВалюты.Кратность /
	|                (КурсыУпрВалюты.Курс * ИзмененияЗапасов.Ссылка.Кратность)
	|        КОНЕЦ
	...
	|            ТОГДА ИзмененияЗапасов.СуммаНДС
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	...
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|                КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ИзмененияЗапасов.Всего * ИзмененияЗапасов.Ссылка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс
	|            * ИзмененияЗапасов.Ссылка.Кратность)
	...
	|                ТОГДА ИзмененияЗапасов.СуммаНДС
	|            КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|                ТОГДА ИзмененияЗапасов.СуммаНДС * КурсыРегВалюты.Курс * ИзмененияЗапасов.Ссылка.Кратность /
	|                    (ИзмененияЗапасов.Ссылка.Курс * КурсыРегВалюты.Кратность)
	|            ИНАЧЕ ИзмененияЗапасов.СуммаНДС
	|        КОНЕЦ
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСВал,
	|    ВЫРАЗИТЬ(ВЫБОР
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.Всего * КурсыРегВалюты.Курс * ИзмененияЗапасов.Ссылка.Кратность /
	|                (ИзмененияЗапасов.Ссылка.Курс * КурсыРегВалюты.Кратность)
	...
	|    ИзмененияЗапасов.Ссылка.Контрагент.СчетУчетаАвансовПоставщику КАК СчетУчетаАвансовПоставщику,
	|    ИзмененияЗапасов.Ссылка.Контрагент.СчетУчетаАвансовПокупателя КАК СчетУчетаАвансовПокупателя,
	...
	|        КОГДА ИзмененияЗапасов.Номенклатура.ИспользоватьСерииИзКатегории
	|            ТОГДА ИзмененияЗапасов.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|        ИНАЧЕ ИзмененияЗапасов.Номенклатура.ПолитикаУчетаСерий
	|    КОНЕЦ) КАК ПолитикаУчетаСерий,
	|    ИзмененияЗапасов.ВидОперацииИсправляемогоДокумента КАК ВидОперацииИсправляемогоДокумента,
	|    ВЫБОР
	|        КОГДА
	|            ИзмененияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	...
	|    ИзмененияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|    ИЛИ
	|        ИзмененияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом) КАК
	|        ВидУчетаПоВЦеломИлиПоДокументам,
	|    &ПередачаНаКомиссию КАК ПередачаНаКомиссию,
	...
	|        КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|            ТОГДА ИзмененияЗапасов.Цена * ИзмененияЗапасов.Количество * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность
	|                / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|        ИНАЧЕ ИзмененияЗапасов.Цена * ИзмененияЗапасов.Количество * ИзмененияЗапасов.Ссылка.Курс *
	|            КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ИзмененияЗапасов.Ссылка.Кратность)
	|    КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаБезСкидки,
	|    ИзмененияЗапасов.СуммаНДСРубДоКорректировки,
	|    ИзмененияЗапасов.СуммаНДСРуб,
	|    ИзмененияЗапасов.СуммаБезНДСРубДоКорректировки,
	|    ИзмененияЗапасов.СуммаБезНДСРуб,
	|    ИзмененияЗапасов.СуммаРубДоКорректировки,
	|    ИзмененияЗапасов.СуммаРуб,
	|    ИзмененияЗапасов.СуммаВзаиморасчетовДоКорректировки,
	|    ИзмененияЗапасов.СуммаВзаиморасчетовДоИзменения,
	|    ИзмененияЗапасов.СуммаВзаиморасчетов
	...
	|        ПО ВЫБОР
	|            КОГДА ИзмененияЗапасов.Номенклатура.ИспользоватьСерииИзКатегории
	|                ТОГДА ИзмененияЗапасов.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|            ИНАЧЕ ИзмененияЗапасов.Номенклатура = ПолитикиУчетаСерий.Владелец
	|        КОНЕЦ
	...
	|    КорректировкаРеализацииПредоплата.Ссылка.Контрагент.СчетУчетаРасчетовСПокупателем КАК
	...
	|    КорректировкаРеализацииПредоплата.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщиком КАК
	...
	|        ИНАЧЕ КорректировкаРеализацииПредоплата.СуммаРасчетов -
	|            КорректировкаРеализацииПредоплата.СуммаРасчетовДоИзменения
	|    КОНЕЦ * КурсыВалютДокументаСрезПоследних.Курс * КурсыВалютУчетаСрезПоследних.Кратность /
	|        (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютДокументаСрезПоследних.Кратность) КАК ЧИСЛО(15, 2))) КАК Сумма,
	|    СУММА(ВЫБОР
	|        КОГДА НЕ КорректировкаРеализацииПредоплата.ЕстьВДокументеРеализации
	|            ТОГДА КорректировкаРеализацииПредоплата.СуммаРасчетов
	|        ИНАЧЕ КорректировкаРеализацииПредоплата.СуммаРасчетов -
	...
	|    СУММА(ВЫРАЗИТЬ(КорректировкаРеализацииПредоплата.СуммаРасчетов * КорректировкаРеализацииПредоплата.Курс /
	|        КорректировкаРеализацииПредоплата.Кратность КАК ЧИСЛО(15, 2))) КАК СуммаРубПоКурсуАванса,
	...
	|    КорректировкаРеализацииПредоплата.Ссылка.ДокументОснование КАК ДокументОснование,
	|    КорректировкаРеализацииПредоплата.Документ КАК Документ,
	|    СУММА(ВЫРАЗИТЬ(КорректировкаРеализацииПредоплата.СуммаПлатежа * КурсыВалютДокументаСрезПоследних.Курс *
	|        КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс *
	|        КурсыВалютДокументаСрезПоследних.Кратность) КАК ЧИСЛО(15, 2))) КАК Сумма,
	...
	|    ИЗ
	|        Документ.АктОРасхожденияхПолученный.СведенияПрослеживаемости КАК
	|            АктОРасхожденияхПолученныйСведенияПрослеживаемости
	|    ГДЕ
	...
	|ИЗ
	|    Документ.КорректировкаРеализации.СерииНоменклатурыДоКорректировки КАК
	|        КорректировкаРеализацииСерииНоменклатурыДоКорректировки
	|        ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы
	...
	|ВЫБРАТЬ
	|    ВЫБОР
	|        КОГДА ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Количество,
	|            0) > ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Количество, 0)
	|            ТОГДА ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Количество, 0) -
	|                ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Количество, 0)
	|        ИНАЧЕ ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Количество, 0) -
	|            ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Количество, 0)
	|    КОНЕЦ КАК Количество,
	|    ВЫБОР
	...
	|        ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСерииНоменклатурыПослеКорректировки КАК
	|            ВременнаяТаблицаСерииНоменклатурыПослеКорректировки
	...
	|        И
	|            ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Серия = ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Серия";
...
Процедура СформироватьТаблицаЗапасыПереданные(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства) 
	...
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасыПринятые.Заказ = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ТаблицаЗапасыПринятые.Заказ
	|	КОНЕЦ КАК Заказ,
	|	ТаблицаЗапасыПринятые.СчетУчетаРасчетовСПоставщиком КАК СчетУчета,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыПриемаПередачиТоваров.ПередачаКомиссионеру) КАК ТипПриемаПередачи,
	|	СУММА(ТаблицаЗапасыПринятые.Количество) КАК Количество,
	|	СУММА(ТаблицаЗапасыПринятые.Сумма) КАК СуммаРасчетов,
	|	СУММА(ТаблицаЗапасыПринятые.Сумма) КАК Сумма,
	|	СУММА(ТаблицаЗапасыПринятые.Сумма) КАК СуммаПродажи,
	|	&ВалютаУпрУчета КАК Валюта,
	|	СУММА(ТаблицаЗапасыПринятые.Сумма) КАК СуммаВал
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	"ВЫБРАТЬ
	|	КорректировкаРеализации.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА КорректировкаРеализации.ДатаИсходногоДокумента > ДАТАВРЕМЯ(1, 1, 1)
	|   	ТОГДА КорректировкаРеализации.ДатаИсходногоДокумента
	|	ИНАЧЕ
	|		КорректировкаРеализации.Дата
	|	КОНЕЦ КАК ДатаДокумента,
	|	ВЫБОР КОГДА КорректировкаРеализации.НомерИсходногоДокумента > """"
	|   	ТОГДА КорректировкаРеализации.НомерИсходногоДокумента
	|	ИНАЧЕ
	|		ВЫРАЗИТЬ(КорректировкаРеализации.Номер КАК СТРОКА(12))
	|	КОНЕЦ КАК Номер,
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Реквизиты.Организация.ГоловнаяОрганизация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|				И Реквизиты.Организация.ГоловнаяОрганизация <> Реквизиты.Организация
	...
	|	Реквизиты.ОставлятьПустымКоличествоВУПД КАК ОставлятьПустымКоличествоВУПД
	...
	|			ТОГДА ВЫРАЗИТЬ(ТаблицаНоменклатуры.Сумма / (100 + ВЫБОР
	|						КОГДА Реквизиты.СуммаВключаетНДС
	|							ТОГДА ТаблицаНоменклатуры.СтавкаНДС.Ставка
	|						ИНАЧЕ 0
	|					КОНЕЦ) * ТаблицаНоменклатуры.СтавкаНДС.Ставка КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ 0
	|	КОНЕЦ * ВЫБОР
	|		КОГДА Реквизиты.ИспользоватьПересчет
	|			ТОГДА Реквизиты.Курс / Реквизиты.Кратность
	...
	|	ТаблицаНоменклатуры.СуммаАкциза КАК АкцизПослеИзменения,
	...
	|			И (ВЫБОР
	|				КОГДА НЕ ТаблицаНоменклатуры.НомерСтрокиДокументаОснования = 0
	|					ТОГДА ТаблицаНоменклатуры.НомерСтрокиДокументаОснования = СуммыДокументовРегламентированныйУчет.НомерСтрокиДокумента
	|				ИНАЧЕ ТаблицаНоменклатуры.НомерСтроки = СуммыДокументовРегламентированныйУчет.НомерСтрокиДокумента
	|			КОНЕЦ),
	|	Константа.ПечатьПользовательскихЕдиницИзмерения КАК ПечатьПользовательскихЕдиницИзмерения
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.ОставлятьПустымКоличествоВУПД
	|				И ТаблицаНоменклатуры.НоменклатураУслуга
	|			ТОГДА """"
	...
	|				И ТаблицаНоменклатуры.НоменклатураУслуга
	|			ТОГДА """"
	|		ИНАЧЕ ТаблицаНоменклатуры.КодТНВЭДЕдиницаИзмеренияКод
	|	КОНЕЦ КАК КодТНВЭДЕдиницаИзмеренияКод,
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.ОставлятьПустымКоличествоВУПД
	|				И ТаблицаНоменклатуры.НоменклатураУслуга
	|			ТОГДА """"
	|		ИНАЧЕ ТаблицаНоменклатуры.КодТНВЭДЕдиницаИзмеренияНаименование
	|	КОНЕЦ КАК КодТНВЭДЕдиницаИзмеренияНаименование,
	|	ТаблицаНоменклатуры.НоменклатураНабора КАК НоменклатураНабора,
	|	ТаблицаНоменклатуры.ХарактеристикаНабора КАК ХарактеристикаНабора,
	|	ТаблицаНоменклатуры.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.АкцизДоИзменения = 0
	|			ТОГДА ""Без акциза""
	|		ИНАЧЕ ТаблицаНоменклатуры.АкцизДоИзменения
	|	КОНЕЦ КАК АкцизДоИзменения,
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.АкцизПослеИзменения = 0
	|			ТОГДА ""Без акциза""
	|		ИНАЧЕ ТаблицаНоменклатуры.АкцизПослеИзменения
	|	КОНЕЦ КАК АкцизПослеИзменения,
	|	ТаблицаНоменклатуры.ЕстьКолонкаАкциз КАК ЕстьКолонкаАкциз,
	|	ТаблицаНоменклатуры.ЦенаДоИзменения КАК ЦенаДоИзмененияДляПрослеживаемости,
	|	ТаблицаНоменклатуры.ЦенаПослеИзменения КАК ЦенаПослеИзмененияДляПрослеживаемости,
	|	ТаблицаНоменклатуры.КоличествоДоИзменения КАК КоличествоДоИзмененияДляПрослеживаемости,
	|	ТаблицаНоменклатуры.КоличествоПослеИзменения КАК КоличествоПослеИзмененияДляПрослеживаемости
	...
	|						ТОГДА ВЫРАЗИТЬ((ТаблицаДокумента.СтоимостьСНДСПослеИзменения - ТаблицаДокумента.СуммаНДСПослеИзменения) / ТаблицаДокумента.КоличествоПослеИзмененияДляПрослеживаемости КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ТаблицаДокумента.ЦенаПослеИзмененияДляПрослеживаемости
	|				КОНЕЦ
	|		ИНАЧЕ КорректировкаРеализацииСведенияПрослеживаемости.Сумма
	|	КОНЕЦ КАК СтоимостьБезНалога
	...
	|			ТОГДА КорректировкаРеализацииСведенияПрослеживаемости.Количество * ВЫБОР
	|					КОГДА ТаблицаДокумента.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ((ТаблицаДокумента.СтоимостьСНДСДоИзменения - ТаблицаДокумента.СуммаНДСДоИзменения) / ТаблицаДокумента.КоличествоДоИзмененияДляПрослеживаемости КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ТаблицаДокумента.ЦенаДоИзмененияДляПрослеживаемости
	|				КОНЕЦ
	|		ИНАЧЕ КорректировкаРеализацииСведенияПрослеживаемости.Сумма
	|	КОНЕЦ КАК СтоимостьБезНалога
	...
	|		ПО (КорректировкаРеализацииСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокументРеализации)
	|			И ТаблицаДокумента.ИдентификаторСтроки = КорректировкаРеализацииСведенияПрослеживаемости.ИдентификаторСтроки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|		ПО ТаблицаДокумента.ИдентификаторСтроки = РасходнаяНакладнаяСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
	...
	|		ПО ТаблицаДокумента.ИдентификаторСтроки = ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
...
Функция ТекстЗапросаПечатьУниверсальныхКорректировочныхДокументов() Экспорт
	...
	|	КорректировкаРеализации.Дата КАК Дата,
	|	КорректировкаРеализации.Номер КАК Номер,
	|	Ложь КАК ОтредактированНомерСчетаФактуры,
