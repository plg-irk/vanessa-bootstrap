...
//
// Параметры:
//  ДокументСсылкаРаспределениеЗатрат  - ДокументСсылка.РаспределениеЗатрат - ссылка на проводимый документ
//  СтруктураДополнительныеСвойства  - Структура - поле "ДополнительныеСвойства" проводимого документа
//
Процедура СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаРаспределениеЗатрат, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ТЧРаспределениеЗапасов.Количество * ЕСТЬNULL(ЕдиницыИзмерения.Коэффициент, 1) КАК Количество,
	               |	ДокРаспределениеЗатрат.УчитыватьВНУ КАК УчитыватьВНУ
	               |ИЗ
	               |	Документ.РаспределениеЗатрат.Запасы КАК ТЧЗапасы
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РаспределениеЗатрат КАК ДокРаспределениеЗатрат
	               |		ПО (ДокРаспределениеЗатрат.Ссылка = ТЧЗапасы.Ссылка)
	               |			И (ДокРаспределениеЗатрат.Ссылка = &Ссылка)
	               |			И (ТЧЗапасы.Количество > 0)
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РаспределениеЗатрат.РаспределениеЗапасов КАК ТЧРаспределениеЗапасов
	               |		ПО ТЧЗапасы.Ссылка = ТЧРаспределениеЗапасов.Ссылка
	               |			И ТЧЗапасы.КлючСвязи = ТЧРаспределениеЗапасов.КлючСвязи
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЕдиницыИзмерения КАК ЕдиницыИзмерения
	               |		ПО ТЧЗапасы.ЕдиницаИзмерения = ЕдиницыИзмерения.Ссылка";
	...
	УчетПартийУСН.ЗаполнитьДвиженияСписаниеПартий(ДокументСсылкаРаспределениеЗатрат, ПартииСписать, ТаблицаЗапасы, ТаблицаПартииТоваров);
...
Процедура ИнициализироватьДанныеПоЗапасам(ДокументСсылкаРаспределениеЗатрат, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ИЛИ ВЫБОР
	|		КОГДА ДокументРаспределениеЗатрат.ЗаказНаПроизводство <> ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ДокументРаспределениеЗатрат.ЗаказНаПроизводство КАК
	|				Документ.ЗаказНаПроизводство).УчетПотребностиПоЗаказам, ЛОЖЬ)
	...
	|	&ИспользоватьРезервирование
	|	И ЕСТЬNULL(ВЫРАЗИТЬ(ДокументРаспределениеЗатрат.ЗаказНаПроизводство КАК
	|		Документ.ЗаказНаПроизводство).УчетПотребностиПоЗаказамНаПроизводство, ЛОЖЬ)
	|	ИЛИ ЕСТЬNULL(ВЫРАЗИТЬ(ДокументРаспределениеЗатрат.ЗаказНаПроизводство КАК
	|		Документ.ЗаказНаПроизводство).УчетПотребностиПоЗаказам, ЛОЖЬ) КАК УчетПотребностиПоЗаказамНаПроизводство,
	|	ВЫБОР
	|		КОГДА &ИспользоватьРезервирование
	|			ТОГДА ТЧРаспределениеЗапасов.ЗаказПокупателя
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	...
	|		КОГДА ТЧЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|			ТОГДА ТЧЗапасы.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|		ИНАЧЕ ТЧЗапасы.Номенклатура.ПолитикаУчетаСерий
	...
	|		ПО ВЫБОР
	|			КОГДА ТЧЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|				ТОГДА ТЧЗапасы.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|			ИНАЧЕ ТЧЗапасы.Номенклатура = ПолитикиУчетаСерий.Владелец
	|		КОНЕЦ
	...
	|		ИНАЧЕ ТаблицаЗапасы.ЗаказПокупателя
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыПриемаПередачиТоваров.ОтчетПоПереработке)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	//Отчетность
	СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаРаспределениеЗатрат, СтруктураДополнительныеСвойства);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.РаспределениеЗатрат";
