...
Процедура МатериалыПодбор(Команда)
	...
	ПараметрыПодбора = Новый Структура("ПолностьюОбновить, ВидСтавкиНДСПоУмолчанию", 
		Истина, ПредопределенноеЗначение("Перечисление.ВидыСтавокНДС.Общая"));	
...
Процедура ПродукцияПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПоискПоШтрихКоду") И ДополнительныеПараметры.ЭтоПоискПоШтрихКоду Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Результат);
		ДополнительныеПараметры.Вставить("ИмяТабличнойЧасти", "Продукция");
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрихкодов)
	...
	ТабличнаяЧастьЗапасы = Ложь;
	Если ИмяТекущейСтраницы = "ГруппаПродукция" Тогда
		ИмяТабличнойЧасти = "Продукция";
	Иначе
		ИмяТабличнойЧасти = "Запасы";
		ТабличнаяЧастьЗапасы = Истина;
	КонецЕсли;
	...
	СтруктураДанные.Вставить("СуммаВключаетНДС", Объект.СуммаВключаетНДС);
	СтруктураДанные.Вставить("НалогообложениеНДС", Объект.НалогообложениеНДС);
	СтруктураДанные.Вставить("ВидСтавкиНДСПоУмолчанию", ПредопределенноеЗначение("Перечисление.ВидыСтавокНДС.Общая"));
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если ЗапрашиватьОбъединениеСтрок И МассивСтрокТЧ.Количество() Тогда
				...
				Если ТабличнаяЧастьЗапасы Тогда
					Оповещение = Новый ОписаниеОповещения("ЗапасыПодтверждениеСбросаКоличестваЗавершение",
						ЭтотОбъект, ДопПараметр);
				Иначе
					Оповещение = Новый ОписаниеОповещения("ПродукцияПодтверждениеСбросаКоличестваЗавершение",
						ЭтотОбъект, ДопПараметр);
				КонецЕсли;
				ОткрытьФорму("ОбщаяФорма.ПодтверждениеОбъединенияСтрокПриДобавленииВТабличнуюЧасть", , , , , , Оповещение);
			...
			Если ДанныеШтрихкода.Свойство("Серия") И ЗначениеЗаполнено(ДанныеШтрихкода.Серия)
				И ИмяТабличнойЧасти = "Продукция" Тогда
...
Процедура ЗаполнитьПоДаннымШтрихкодовЗавершение(СтруктураНоменклатуры, Параметры) Экспорт
	...
	ТабличнаяЧастьЗапасы = ИмяТабличнойЧасти = "Запасы";
...
Функция СкидкиИзменились()
	...
	Иначе
		...
		Если ПримененныеСкидки.Свойство("СуммаОплатыБонусами") 
			И НЕ Объект.Продукция.Итог("СуммаСкидкиОплатыБонусом") = ПримененныеСкидки.СуммаОплатыБонусами Тогда
			СкидкиИзменились = Истина;
		КонецЕсли;
...
Процедура ОбработатьВставленныеСтрокиНаСервере(ИмяТЧ, КоличествоВставленных)
	...
	ЭтоЗапасы = (ИмяТЧ = "Запасы");
	ВидСтавкиНДС = ?(ЭтоЗапасы, Перечисления.ВидыСтавокНДС.Общая, ВидСтавкиНДСПоУмолчанию);
	Количество = Объект[ИмяТЧ].Количество();
	Для Итератор = 1 По КоличествоВставленных Цикл
		...
		СтруктураДанные.Вставить("Характеристика",     Строка.Характеристика);
		СтруктураДанные.Вставить("НалогообложениеНДС", Объект.НалогообложениеНДС);
		СтруктураДанные.Вставить("ВидСтавкиНДСПоУмолчанию", ВидСтавкиНДС);
		СтруктураДанные.Вставить("ДатаОбработки", Объект.Дата);
