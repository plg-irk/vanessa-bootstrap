...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетОПереработке, СтруктураДополнительныеСвойства) Экспорт
	...
	|		И
	|			ОтчетОПереработке.Договор.ДоговорОбслуживанияНаправлениеДеятельности <> ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	ВЫБОР
	|		КОГДА
	|			ОтчетОПереработкеПродукция.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|			ТОГДА ВЫРАЗИТЬ(ОтчетОПереработкеПродукция.Ссылка.ЗаказПокупателя КАК
	|				Документ.ЗаказПокупателя).УчетПотребностиПоСкладам
	|		ИНАЧЕ ЛОЖЬ
	...
	|				ТОГДА ОтчетОПереработкеПродукция.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс
	|					* КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ОтчетОПереработкеПродукция.СуммаНДС * ОтчетОПереработкеПродукция.Ссылка.Курс * КурсыУпрВалюты.Кратность /
	|				(КурсыУпрВалюты.Курс * ОтчетОПереработкеПродукция.Ссылка.Кратность)
	|		КОНЕЦ
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА ОтчетОПереработкеПродукция.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|			ТОГДА ОтчетОПереработкеПродукция.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|				КурсыРегВалюты.Кратность)
	|		ИНАЧЕ ОтчетОПереработкеПродукция.Всего * ОтчетОПереработкеПродукция.Ссылка.Курс * КурсыУпрВалюты.Кратность /
	|			(КурсыУпрВалюты.Курс * ОтчетОПереработкеПродукция.Ссылка.Кратность)
	...
	|		ИНАЧЕ ВЫБОР
	|			КОГДА ОтчетОПереработкеПродукция.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА ОтчетОПереработкеПродукция.СуммаНДС * КурсыРегВалюты.Курс * ОтчетОПереработкеПродукция.Ссылка.Кратность /
	|					(ОтчетОПереработкеПродукция.Ссылка.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ОтчетОПереработкеПродукция.СуммаНДС
	|		КОНЕЦ
	|	КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСВал,
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА ОтчетОПереработкеПродукция.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|			ТОГДА ОтчетОПереработкеПродукция.Всего * КурсыРегВалюты.Курс * ОтчетОПереработкеПродукция.Ссылка.Кратность /
	|				(ОтчетОПереработкеПродукция.Ссылка.Курс * КурсыРегВалюты.Кратность)
	...
	|		ИНАЧЕ ОтчетОПереработкеПродукция.Всего * ОтчетОПереработкеПродукция.Ссылка.Курс /
	|			ОтчетОПереработкеПродукция.Ссылка.Кратность
	...
	|		КОГДА ОтчетОПереработкеПродукция.Номенклатура.ИспользоватьСерииИзКатегории
	|			ТОГДА ОтчетОПереработкеПродукция.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|		ИНАЧЕ ОтчетОПереработкеПродукция.Номенклатура.ПолитикаУчетаСерий
	|	КОНЕЦ) КАК ПолитикаУчетаСерий,
	|	ВЫБОР
	|		КОГДА
	|			ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ОтчетОПереработкеПродукция.Ссылка.ДокументПоступления
	|		КОГДА
	|			ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	...
	|	И ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	|	И НЕ
	|		ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом)
	|	И НЕ
	|		ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПустаяСсылка) КАК
	|		ОрдерныйСкладДляДвиженияСерий,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке КАК ОрдерныйСкладКОтгрузке,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом) КАК
	|		УчетПоСкладуВЦелом,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам) КАК
	|		УтетПоПрочимДокументам,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам) КАК
	|		УтетПоСкладскимОрдерам
	...
	|		ПО ВЫБОР
	|			КОГДА ОтчетОПереработкеПродукция.Номенклатура.ИспользоватьСерииИзКатегории
	|				ТОГДА ОтчетОПереработкеПродукция.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|			ИНАЧЕ ОтчетОПереработкеПродукция.Номенклатура = ПолитикиУчетаСерий.Владелец
	|		КОНЕЦ
	...
	|				ТОГДА ОтчетОПереработкеЗапасы.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|					КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ОтчетОПереработкеЗапасы.СуммаНДС * ОтчетОПереработкеЗапасы.Ссылка.Курс * КурсыУпрВалюты.Кратность /
	|				(КурсыУпрВалюты.Курс * ОтчетОПереработкеЗапасы.Ссылка.Кратность)
	|		КОНЕЦ
	|	КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДС,
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА ОтчетОПереработкеЗапасы.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА ОтчетОПереработкеЗапасы.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|			ТОГДА ОтчетОПереработкеЗапасы.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
	|				КурсыРегВалюты.Кратность)
	|		ИНАЧЕ ОтчетОПереработкеЗапасы.Всего * ОтчетОПереработкеЗапасы.Ссылка.Курс * КурсыУпрВалюты.Кратность /
	|			(КурсыУпрВалюты.Курс * ОтчетОПереработкеЗапасы.Ссылка.Кратность)
	|	КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	...
	|	И НЕ
	|		ОтчетОПереработкеЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом)
	|	И НЕ
	|		ОтчетОПереработкеЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПустаяСсылка) КАК
	|		ОрдерныйСкладДляДвиженияСерий
	...
	|	ВЫБОР
	|		КОГДА
	|			ОтчетОПереработкеОтходы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ОтчетОПереработкеОтходы.Ссылка.ДокументПоступления
	|		КОГДА
	|			ОтчетОПереработкеОтходы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	...
	|	СУММА(ВЫРАЗИТЬ(ТаблицаДокумента.СуммаРасчетов * ТаблицаДокумента.Курс * КурсыВалютУчетаСрезПоследних.Кратность /
	|		(КурсыВалютУчетаСрезПоследних.Курс * ТаблицаДокумента.Кратность) КАК ЧИСЛО(15, 2))) КАК Сумма,
	...
	|			ТОГДА ОтчетОПереработкеСкидкиНаценки.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс
	|				* КурсыРегВалюты.Кратность)
	|		ИНАЧЕ ОтчетОПереработкеСкидкиНаценки.Сумма * ОтчетОПереработкеСкидкиНаценки.Ссылка.Курс * КурсыУпрВалюты.Кратность /
	|			(КурсыУпрВалюты.Курс * ОтчетОПереработкеСкидкиНаценки.Ссылка.Кратность)
	|	КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	...
	|		ИНАЧЕ ВЫБОР
	|			КОГДА
	|				ВременнаяТаблицаПродукция.ПолитикаУчетаСерий.ТипПолитики = ЗНАЧЕНИЕ(Перечисление.ТипыПолитикУказанияСерий.УправлениеОстаткамиСерий)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = Обработки.ПечатьТОРГ12.СинонимМакета(Ложь);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ОтчетОПереработке";
