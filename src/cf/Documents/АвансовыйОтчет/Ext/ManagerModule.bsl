...
//
// Параметры:
//  ДокументСсылкаАвансовыйОтчет - ДокументСсылка.АвансовыйОтчет - документ-источник движений
//  СтруктураДополнительныеСвойства	 - Структура - свойство объекта ДополнительныеСвойства
//
Процедура СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	АвансовыйОтчет.УчитыватьВНУ КАК УчитыватьВНУ,
	               |	АвансовыйОтчет.ВалютаДокумента КАК ВалютаДокумента,
	               |	ВЫБОР
	               |		КОГДА АвансовыйОтчет.Кратность = 0
	               |			ТОГДА АвансовыйОтчет.Курс
	               |		ИНАЧЕ АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность
	               |	КОНЕЦ КАК Курс
	               ...
	               |ВЫБРАТЬ
	               |	&Организация КАК Организация,
	               |	АвансовыйОтчетЗапасы.Ссылка КАК ДокументПартии,
	               |	АвансовыйОтчетЗапасы.Ссылка КАК ДокументРегистрацииВКУДиР,
	               |	&ДатаДокумента КАК Период,
	               |	АвансовыйОтчетЗапасы.Номенклатура КАК Номенклатура,
	               |	АвансовыйОтчетЗапасы.Количество * ЕСТЬNULL(ЕдиницыИзмерения.Коэффициент, 1) КАК Количество,
	               |	АвансовыйОтчетЗапасы.Всего * ДанныеДокументаДляНУ.РасчетКурса КАК Сумма,
	               |	АвансовыйОтчетЗапасы.СуммаНДС * ДанныеДокументаДляНУ.РасчетКурса КАК СуммаНДС,
	               ...
	               |		ПО АвансовыйОтчетЗапасы.Номенклатура = НоменклатураЗапасы.Ссылка
	               ...
	               |		ПО АвансовыйОтчетЗапасы.Ссылка = ДанныеДокументаДляНУ.Ссылка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЕдиницыИзмерения КАК ЕдиницыИзмерения
	               |		ПО АвансовыйОтчетЗапасы.ЕдиницаИзмерения = ЕдиницыИзмерения.Ссылка
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               |ВЫБРАТЬ
	               ...
	               |	АвансовыйОтчетРасходы.Всего * ДанныеДокументаДляНУ.РасчетКурса,
	               |	АвансовыйОтчетРасходы.СуммаНДС * ДанныеДокументаДляНУ.РасчетКурса,
	               |	ЛОЖЬ,
	               ...
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументаДляНУ КАК ДанныеДокументаДляНУ
	               |		ПО АвансовыйОтчетРасходы.Ссылка = ДанныеДокументаДляНУ.Ссылка
	               |
	               |УПОРЯДОЧИТЬ ПО
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ШапкаДокумента.Ссылка КАК ДенежныйДокумент,
	               ...
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АвансовыйОтчет.Оплаты КАК ТаблицаДокумента
	               |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АвансовыйОтчет КАК ШапкаДокумента
	               |			ПО (ШапкаДокумента.Ссылка = ТаблицаДокумента.Ссылка)
	               |				И (ШапкаДокумента.Ссылка = &Ссылка)
	               |		ПО ВременнаяТаблицаРасчетыСПоставщиками.Документ = ТаблицаДокумента.Документ
	...
	ОстаткиПартий = Запрос.Выполнить().Выбрать();
		
	УчетПартийУСН.ЗаполнитьДвиженияОплатаПартий(ДокументСсылкаАвансовыйОтчет, ОстаткиПартий, ТаблицаОплаты, ТаблицаПартииТоваров);
	...
	Если ТаблицаПартииТоваров.Найти(Истина, "ОтражатьВКУДиР") <> Неопределено Тогда
		...
		СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.РасходыПодотчетныеСредства();
	КонецЕсли;
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства) Экспорт
	...
	СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства);
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|		Номенклатура.Артикул КАК Артикул,
		|		Номенклатура.Штрихкод КАК Штрихкод,
		|		ВидДокВходящий КАК ВидДокВходящий,
		...
		ПредставлениеОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечати");
		...
		Пока Запасы.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(СокрЛП(Запасы.ВидДокВходящий)) Тогда
				ОбластьМакета.Параметры.НаименованиеРасхода = СокрЛП(Запасы.ВидДокВходящий);
			ИначеЕсли ЗначениеЗаполнено(Запасы.Содержание) Тогда
		...
		Пока Расходы.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(СокрЛП(Расходы.ВидДокВходящий)) Тогда
				ОбластьМакета.Параметры.НаименованиеРасхода = СокрЛП(Расходы.ВидДокВходящий);
			ИначеЕсли ЗначениеЗаполнено(Расходы.Содержание) Тогда
		...
		Пока Оплаты.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(СокрЛП(Оплаты.ВидДокВходящий)) Тогда
				ОбластьМакета.Параметры.НаименованиеРасхода = СокрЛП(Оплаты.ВидДокВходящий);
			Иначе
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Использование", Метаданные.Отчеты.РеестрДокументов) Тогда
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "РеестрДокументовАвансовыйОтчет";
...
Процедура ЗарегистрироватьЦеныПоставщика(ДокументСсылка, Отказ) Экспорт 
	...
	ВидЦенКонтрагента 				= ЗначенияРеквизитов.СчетФактураДоговорКонтрагентаВидЦенКонтрагента;
	ЦенаВключаетНДС 				= ЗначенияРеквизитов.СчетФактураДоговорКонтрагентаВидЦенКонтрагентаЦенаВключаетНДС;
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаЦен.Ссылка.Дата КАК Период,
	|	ТаблицаЦен.Ссылка.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	МИНИМУМ(ТаблицаЦен.НомерСтроки) КАК НомерСтроки,
	|	ТаблицаЦен.Номенклатура КАК Номенклатура,
	|	ТаблицаЦен.Характеристика КАК Характеристика,
	|	&ВидЦенКонтрагента КАК ВидЦенКонтрагента,
	|	&ЦенаВключаетНДС КАК ЦенаВключаетНДС
	|ПОМЕСТИТЬ НоменклатураБезДублей
	|ИЗ
	|	Документ.АвансовыйОтчет.Запасы КАК ТаблицаЦен
	|ГДЕ
	|	ТаблицаЦен.Ссылка = &Ссылка
	|	И ТаблицаЦен.Цена <> 0
	|	И ТаблицаЦен.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаЦен.Ссылка.Дата,
	|	ТаблицаЦен.Ссылка.СуммаВключаетНДС,
	...
	|	ИСТИНА КАК ЭтоЗапас,
	|	ВЫБОР
	|		КОГДА НоменклатураБезДублей.СуммаВключаетНДС = НоменклатураБезДублей.ЦенаВключаетНДС
	...
	|		КОГДА НоменклатураБезДублей.СуммаВключаетНДС > ТаблицаЦены.Ссылка.СчетФактураДоговорКонтрагента.ВидЦенКонтрагента.ЦенаВключаетНДС
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.АвансовыйОтчет.Запасы КАК ТаблицаЦены
	...
	|		ПО (ТаблицаЦены.Ссылка.СчетФактураДоговорКонтрагента.ВидЦенКонтрагента = ЦеныНоменклатурыКонтрагентов.ВидЦенКонтрагента)
	...
	|		ПО (ТаблицаЦены.Ссылка.СчетФактураДоговорКонтрагента.ВидЦенКонтрагента.ВалютаЦены = КурсВалютыВидЦен.Валюта),
	...
	|	МИНИМУМ(АвансовыйОтчетРасходы.НомерСтроки) КАК НомерСтроки,
	|	АвансовыйОтчетРасходы.Номенклатура КАК Номенклатура,
	|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК Характеристика,
	|	&ВидЦенКонтрагента КАК ВидЦенКонтрагента,
	|	&ЦенаВключаетНДС КАК ЦенаВключаетНДС
	...
	|	И АвансовыйОтчетРасходы.Цена <> 0
	|	И АвансовыйОтчетРасходы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|
	|СГРУППИРОВАТЬ ПО
	|	АвансовыйОтчетРасходы.Ссылка.Дата,
	|	АвансовыйОтчетРасходы.Ссылка.СуммаВключаетНДС,
	|	АвансовыйОтчетРасходы.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ТаблицаРасходы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ИСТИНА КАК Актуальность,
	|	ТаблицаРасходы.Ссылка КАК ДокументРегистратор,
	|	ТаблицаРасходы.Ссылка.Автор КАК Автор,
	|	ЛОЖЬ КАК ЭтоЗапас,
	|	ВЫБОР
	|		КОГДА НоменклатураУслугБезДублей.СуммаВключаетНДС = НоменклатураУслугБезДублей.ЦенаВключаетНДС
	|			ТОГДА ЕСТЬNULL(ТаблицаРасходы.Цена * КурсВалютыДокумента.Курс * КурсВалютыВидЦен.Кратность / (КурсВалютыВидЦен.Курс * КурсВалютыДокумента.Кратность), 0)
	|		КОГДА НоменклатураУслугБезДублей.СуммаВключаетНДС > ТаблицаРасходы.Ссылка.СчетФактураДоговорКонтрагента.ВидЦенКонтрагента.ЦенаВключаетНДС
	|			ТОГДА ЕСТЬNULL(ТаблицаРасходы.Цена * КурсВалютыДокумента.Курс * КурсВалютыВидЦен.Кратность / (КурсВалютыВидЦен.Курс * КурсВалютыДокумента.Кратность) * 100 / (100 + ТаблицаРасходы.СтавкаНДС.Ставка), 0)
	|		ИНАЧЕ ЕСТЬNULL(ТаблицаРасходы.Цена * КурсВалютыДокумента.Курс * КурсВалютыВидЦен.Кратность / (КурсВалютыВидЦен.Курс * КурсВалютыДокумента.Кратность) * (100 + ТаблицаРасходы.СтавкаНДС.Ставка) / 100, 0)
	|	КОНЕЦ КАК Цена
	|ПОМЕСТИТЬ ЦеныНоменклатурыТипаРасход
	|ИЗ
	|	НоменклатураУслугБезДублей КАК НоменклатураУслугБезДублей
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.АвансовыйОтчет.Расходы КАК ТаблицаРасходы
	|		ПО НоменклатураУслугБезДублей.НомерСтроки = ТаблицаРасходы.НомерСтроки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатурыКонтрагентов КАК ЦеныНоменклатурыКонтрагентов
	|		ПО (ТаблицаРасходы.Номенклатура = ЦеныНоменклатурыКонтрагентов.Номенклатура)
	|			И (ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) = ЦеныНоменклатурыКонтрагентов.Характеристика)
	|			И (НАЧАЛОПЕРИОДА(ТаблицаРасходы.Ссылка.Дата, ДЕНЬ) = ЦеныНоменклатурыКонтрагентов.Период)
	|			И (ТаблицаРасходы.Ссылка.Дата <= ЦеныНоменклатурыКонтрагентов.ДокументРегистратор.Дата)
	|			И (ТаблицаРасходы.Ссылка.СчетФактураДоговорКонтрагента.ВидЦенКонтрагента = ЦеныНоменклатурыКонтрагентов.ВидЦенКонтрагента)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаОбработки, ) КАК КурсВалютыВидЦен
	|		ПО (ТаблицаРасходы.Ссылка.СчетФактураДоговорКонтрагента.ВидЦенКонтрагента.ВалютаЦены = КурсВалютыВидЦен.Валюта),
	|	РегистрСведений.КурсыВалют.СрезПоследних(&ДатаОбработки, Валюта = &ВалютаДокумента) КАК КурсВалютыДокумента
	|ГДЕ
	|	ЦеныНоменклатурыКонтрагентов.ВидЦенКонтрагента ЕСТЬ NULL
	|	И ТаблицаРасходы.Ссылка = &Ссылка
	|	И ТаблицаРасходы.Цена <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.АвансовыйОтчет";
