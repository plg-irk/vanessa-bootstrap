...
Процедура ЗаполнитьПоЧекуККМ(ДанныеЗаполнения) Экспорт
	...
	|		Сумма КАК Сумма,
	|		СерииНоменклатуры КАК СерииНоменклатуры,
	|		ШтрихкодУпаковки КАК ШтрихкодУпаковки) КАК Подарки,
	|	ЧекККМ.СерииПодарков.(
	|		Серия КАК Серия,
	|		Количество КАК Количество,
	|		КлючСвязи КАК КлючСвязи) КАК СерииПодарков,
	|	ЧекККМ.ДобавленныеНаборы.(
	|		НоменклатураНабора КАК НоменклатураНабора,
	|		ХарактеристикаНабора КАК ХарактеристикаНабора,
	|		Количество КАК Количество) КАК ДобавленныеНаборы,
	...
	|		СуммаБонусов КАК СуммаБонусов) КАК БезналичнаяОплата,
	...
	|		СкидкаНаценка КАК СкидкаНаценка,
	|		Сумма КАК Сумма,
	|		ЗначениеСкидкиНаценки КАК ЗначениеСкидкиНаценки) КАК СкидкиНаценки,
	...
	|		КоличествоБонусныхБаллов КАК КоличествоБонусныхБаллов) КАК БонусныеБаллыКНачислению,
	...
	|		СуммаПлатежа КАК СуммаПлатежа) КАК Предоплата,
	|	ЧекККМ.ПоложениеСклада КАК ПоложениеСклада,
	|	ЧекККМ.ПоложениеПатента
	|ИЗ
	...
	Для Каждого СтрокаЗапасы Из Выборка.Запасы.Выгрузить() Цикл
		...
		НоваяСтрокаЗапасы.Удалить_СуммаАвтоматическойСкидкиЧекаПродажи = СтрокаЗапасы.СуммаАвтоматическойСкидки;
		НоваяСтрокаЗапасы.Удалить_СуммаСкидкиНаценкиЧекаПродажи = СтрокаЗапасы.СуммаСкидкиНаценки;
		НоваяСтрокаЗапасы.Удалить_СуммаСкидкиОплатыБонусомЧекаПродажи = СтрокаЗапасы.СуммаСкидкиОплатыБонусом;
	КонецЦикла;
	...
	СторноНачисленныхБаллов = Выборка.БонусныеБаллыКНачислению.Выгрузить();
	СторноНачисленныхБаллов.ЗаполнитьЗначения(ТекущаяДатаСеанса(), "ДатаНачисления");
	БонусныеБаллыКНачислению.Загрузить(СторноНачисленныхБаллов);
	...
	СтрокиБонусов = БезналичнаяОплата.НайтиСтроки(Новый Структура("ВидОплаты", ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.Бонусы")));
	ТаблицаОплатаБонусами = БезналичнаяОплата.Выгрузить(СтрокиБонусов);
	СкидкиНаценкиСервер.ПеренестиАвтоматическиеСкидкиИзДокументаОснования(ЭтотОбъект, , , ТаблицаОплатаБонусами);
	...
	СуммаТоваров = Запасы.Итог("Всего");
	СуммаПредоплаты = 0;
	Если Не ОперацияСДенежнымиСредствами Тогда
		СуммаДокумента = СуммаТоваров;
		СуммаПредоплаты = Предоплата.Итог("СуммаРасчетов");
	ИначеЕсли ОперацияСДенежнымиСредствами И Предоплата.Количество() Тогда
		СуммаДокумента = Предоплата.Итог("СуммаРасчетов");
	КонецЕсли;
	СуммаОплатыПлатежнымиКартами = БезналичнаяОплата.Итог("Сумма");
	
	Если СуммаОплатыПлатежнымиКартами > СуммаДокумента Тогда
		УжеРаспределено = 0;
		Для Каждого СтрокаОплаты Из БезналичнаяОплата Цикл
			МожноРаспределить = Макс(СуммаДокумента - УжеРаспределено, 0);
			СтрокаОплаты.Сумма = Мин(МожноРаспределить, СтрокаОплаты.Сумма);
			УжеРаспределено = УжеРаспределено + СтрокаОплаты.Сумма;
		КонецЦикла;
	КонецЕсли;
	
	СуммаБезналичнойОплаты = СуммаОплатыПлатежнымиКартами + СуммаПредоплаты;
	ПолученоНаличными = СуммаДокумента - Мин(СуммаБезналичнойОплаты, СуммаДокумента);
...
Процедура ЗаполнитьКомиссиюЭквайрера()
	...
	|	И НЕ ЕСТЬNULL(ЧекККМБезналичнаяОплата.ЭквайринговыйТерминал.Договор.РасчетКомиссииВОтчетеЭквайера, ЛОЖЬ)
...
Процедура ПроверитьНачиличеПодарочныхСертификатов()
	
	ЕстьПодарочныеСертификаты = Ложь;
	Для Каждого СтрокаТаблицы Из Запасы Цикл
		...
		Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.ПодарочныйСертификат Тогда
			ЕстьПодарочныеСертификаты = Истина;
		КонецЕсли;
	КонецЦикла;
	Если ЕстьПодарочныеСертификаты Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ТипКассы = ?(ЗначениеЗаполнено(КассаККМ), 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КассаККМ, "ТипКассы"),
		Перечисления.ТипыКассККМ.ПустаяСсылка());
	
	Если НЕ ТипКассы = Перечисления.ТипыКассККМ.АвтономнаяККМ 
		И НЕ ТипКассы = Перечисления.ТипыКассККМ.ККМED Тогда
	...
	Иначе
		...
		ЗаполнениеОбъектовУНФ.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ПричинаВозврата");
	КонецЕсли;
	...
	Если Константы.ФункциональнаяОпцияИспользоватьПодарочныеСертификаты.Получить() Тогда
		...
		Для Каждого СтрокаЗапасы Из Запасы Цикл
			ОписаниеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаЗапасы.Номенклатура, "ТипНоменклатуры, ИспользоватьСерииНоменклатуры"); 
			ТипНоменклатуры = ОписаниеНоменклатуры.ТипНоменклатуры;
			УчетСерий = ОписаниеНоменклатуры.ИспользоватьСерииНоменклатуры;
			Если НЕ ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПодарочныйСертификат") Тогда
				Продолжить;
			ИначеЕсли НЕ УчетСерий Тогда
				Продолжить;
			...
			Для Каждого СтрокаСерии Из СерииЭтогоСертификата Цикл
				ДанныеСертификата = РаботаСПодарочнымиСертификатами.ПолучитьСтруктуруДанныхСертификата(СтрокаЗапасы.Номенклатура, 
					СтрокаСерии.Серия, Ссылка);
				СуммаСертификатов = СуммаСертификатов + ДанныеСертификата.Остаток;
				ОписаниеОстаткаСерии = ОписаниеОстаткаСерии + Символы.ПС + СтрШаблон(НСтр("ru = '		Серия %1 = %2'"),
					СтрокаСерии.Серия, Формат(ДанныеСертификата.Остаток, "ЧДЦ=2; ЧН=0;"));
			КонецЦикла;
			ОписаниеОстатка = СтрШаблон(НСтр("ru = 'Баланс %1 = %2'"),
				СтрокаЗапасы.Номенклатура, Формат(ДанныеСертификата.Остаток, "ЧДЦ=2; ЧН=0;"));
			Если СерииЭтогоСертификата.Количество() > 1 Тогда
				ОписаниеОстатка = ОписаниеОстатка + ОписаниеОстаткаСерии;
			КонецЕсли;
			Если СтрокаЗапасы.Цена > СуммаСертификатов Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Цена возвращаемого сертификата в строке %1 превышает оставшийся баланс этого сертификата.
					|	%2'"), СтрокаЗапасы.НомерСтроки, ОписаниеОстатка);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Запасы", СтрокаЗапасы.НомерСтроки,
							"Сумма");
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, КонтекстноеПоле, , Отказ);
			КонецЕсли;
	...
	// ИнтеграцияГосИС
	Если ТипКассы <> Перечисления.ТипыКассККМ.ККМED
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ТипКассыККМ = Перечисления.ТипыКассККМ.ККМED
		И Константы.ИспользоватьСерверЛояльностиПоставщикДанных.Получить() Тогда
		ПроверитьНачиличеПодарочныхСертификатов();
КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ДополнительныеСвойства.Свойство("СкорректироватьРегистрСведенийПодарочныеСертификатыКСписанию") Тогда
		СерверЛояльностиПоставщикДанных.СкорректироватьРегистрСведенийПодарочныеСертификатыКСписаниюВозврат(ЭтотОбъект);
	КонецЕсли;
	
	Если ТипКассы = Перечисления.ТипыКассККМ.ККМED Или ТипКассы = Перечисления.ТипыКассККМ.ККМOffline Тогда
		ОрганизацияЕГАИС = ИнтеграцияЕГАИСУНФВызовСервера.ОрганизацияЕГАИСПоОрганизацииИТорговомуОбъекту(Организация, СтруктурнаяЕдиница);
		Если ЗначениеЗаполнено(ОрганизацияЕГАИС) Тогда
			ПараметрыОбновленияСтатуса = ИнтеграцияЕГАИС.ПараметрыОбновленияСтатуса();
			ПараметрыОбновленияСтатуса.ФорматОбмена = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОрганизацияЕГАИС, "ФорматОбмена");
			Документы.ЧекККМВозврат.ПриИзмененииСтатусаДокумента(Ссылка, Перечисления.СтатусыИнформированияЕГАИС.КПередаче,
				Перечисления.СтатусыИнформированияЕГАИС.ПереданВУТМ, ПараметрыОбновленияСтатуса);
		КонецЕсли;
	КонецЕсли;
