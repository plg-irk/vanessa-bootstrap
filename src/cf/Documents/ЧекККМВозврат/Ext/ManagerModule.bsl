...
Функция ТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|	ЧекККМВозврат.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	НЕОПРЕДЕЛЕНО КАК НаправлениеДеятельности
	...
	|	ВременнаяТаблицаШапка.КассаККМ.Владелец КАК КассаККМВладелец,
	...
	|	&Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА ЧекККМЗапасы.Ссылка.ПоложениеСклада В (ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВШапке),
	...
	|	ВременнаяТаблицаШапка.ВалютаДокумента КАК ВалютаРасчетов,
	...
	|	ЧекККМВозвратПредоплата.Заказ КАК Заказ,
	...
	|			ТОГДА ЧекККМВозвратСкидкиНаценки.Сумма * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс *
...
Функция СформироватьТекстЗапросаОстаткиЗапасы()
	...
	|	И НЕ (ДвиженияЗапасыИзменение.Номенклатура, ДвиженияЗапасыИзменение.СтруктурнаяЕдиница) В
	|				(ВЫБРАТЬ
...
Функция СформироватьПечатнуюФормуТоварногоЧека(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		ПредставлениеПоставщика = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечати,ИНН,КПП,ЮридическийАдрес,Телефоны,");
...
Функция СформироватьПечатнуюФормуКМ3(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ЧекККМВозврат Из МассивОбъектов Цикл
		...
		ОбластьМакета.Параметры.ПредставлениеОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечати,ИНН,КПП,Телефоны,");
...
Процедура СформироватьТаблицаОплатаПодарочнымиСертификатами(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст = РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийОплатыПодарочнымиСертификатами(ФормироватьДвиженияПодарочныеСертификаты );
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаРасшифровкаПлатежа.Документ КАК Документ,
	...
	|
	|ВЫБРАТЬ
	|	ДвижениеСертификатов.Период,
	|	ДвижениеСертификатов.ВидДвижения,
	|	ДвижениеСертификатов.Организация,
	|	ДвижениеСертификатов.Контрагент,
	|	ДвижениеСертификатов.Договор,
	|	ДвижениеСертификатов.Документ,
	|	ДвижениеСертификатов.Заказ,
	|	ДвижениеСертификатов.ТипРасчетов,
	|	ДвижениеСертификатов.Сумма,
	|	ДвижениеСертификатов.СуммаВал,
	|	ДвижениеСертификатов.СчетУчета,
	|	ДвижениеСертификатов.Валюта,
	|	ДвижениеСертификатов.СодержаниеПроводки,
	|	ДвижениеСертификатов.СуммаРег,
	|	ДвижениеСертификатов.Курс,
	|	ДвижениеСертификатов.СпособЗачета,
	|	ДвижениеСертификатов.Кратность,
	|	ДвижениеСертификатов.ДокументОплаты,
	|	ДвижениеСертификатов.СуммаПлатежа,
	|	ДвижениеСертификатов.ВалютаПлатежа,
	|	ДвижениеСертификатов.СуммаНДС,
	|	ДвижениеСертификатов.СтавкаНДС
	|ИЗ
...
Процедура СформироватьТаблицаБонусныеБаллы(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ЧекПробит
		...
		ДисконтнаяКарта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаЧекККМВозврат, "ДисконтнаяКарта");
		...
		Для Каждого СтрокаОплаты Из ТаблицаОплатаБонусами Цикл
			...
			// Начислено
			Если НЕ РеквизитыБП.НачислятьБонусыПриВозврате Тогда
				Прервать;
			КонецЕсли; 
