...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭТИспользоватьБезПодключенияОборудования = ?(ЗначениеЗаполнено(Объект.ЭквайринговыйТерминал),
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЭквайринговыйТерминал, "ИспользоватьБезПодключенияОборудования"),
		Ложь);
	...
	Элементы.ЗапасыСуммаСкидкиНаценки.Видимость = ИспользоватьРучныеСкидки;
	...
	НеПечататьБумажныйЧек = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.КассаККМ, "НеПечататьБумажныйЧек");
	...
	КонецЕсли;
	Элементы.ВозвратОплатыНСПК.Отображение = ОтображениеКнопки.Текст;
...
Процедура ЗапасыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ЗаполнениеОбъектовУНФКлиент.ЗаполнитьСтрокуПоШапке(Элемент.ТекущиеДанные, Объект, "Патент", "ПоложениеПатента");
	КонецЕсли;
	...
	Если РезультатВыполнения.Свойство("СуммаЭлектронногоСертификата") И РезультатВыполнения.СуммаЭлектронногоСертификата > 0 Тогда
		...
		Если ЗначениеЗаполнено(ЭквайринговыйТерминалНСПК) Тогда
			...
			Если СписокВидовКарт.Количество() > 0 Тогда
				ПроцентКомиссии = СписокВидовКарт[0].ПроцентКомиссииПриВозврате;
...
Процедура РассчитатьСкидкиНаценкиКлиент()
	...
	ИДЗамера = ОценкаПроизводительностиКлиент.ЗамерВремени("ЧекККМВозврат_РассчитатьСкидкиНаценкиКлиент", Истина, Ложь);
	ПараметрыЗамера = Новый Структура("КлючеваяОперация, ВесЗамера, Комментарий, ВыполненаСОшибкой", 
		"ЧекККМВозврат_РассчитатьСкидкиНаценкиКлиент",
		Объект.Запасы.Количество(),
		"",
		Ложь);
	ОценкаПроизводительностиКлиент.УстановитьПараметрыЗамера(ИДЗамера, ПараметрыЗамера);
...
Процедура РассчитатьСкидкиНаценкиНаСервере(СтруктураПараметры, Знач ОбъектДляРасчета = Неопределено)
	...
	КонецЕсли;	
	Если РезультатПрименения.Свойство("СуммаОплатыБонусами") Тогда
		СтруктураПараметры.Вставить("СуммаОплатыБонусами", РезультатПрименения.СуммаОплатыБонусами);
	КонецЕсли;
...
Процедура ОтразитьСкидкиНаФормеКлиент(СтруктураПараметры)
	...
	СуммаОплатыБонусами = Ложь;
	Если ЗначениеЗаполнено(Объект.ДисконтнаяКарта)
		И СтруктураПараметры.Свойство("СуммаОплатыБонусами", СуммаОплатыБонусами) 
		И НЕ СуммаОплатыБонусами = Неопределено 
		И НЕ СуммаОплатыБонусами = 0 Тогда
		// Обновляем сумму оплаты бонусами, в этом документе нет выбора, так что берем точно так, как пришло из рассчета.
		КурсБаллов = 1;
		ПараметрыБП = РеквизитыБонуснойПрограммы(Объект.ДисконтнаяКарта);
		Если НЕ ПараметрыБП = Неопределено Тогда
			КурсБаллов = ?(ПараметрыБП.КурсБонусов = 0, 1, ПараметрыБП.КурсБонусов);
		КонецЕсли;
		ОтборОплатыБаллами = Новый Структура("ВидОплаты", ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.Бонусы"));
		СтрокиОплатыБонусами = Объект.БезналичнаяОплата.НайтиСтроки(ОтборОплатыБаллами);
		Если СтрокиОплатыБонусами.Количество() = 0 Тогда
			НоваяСтрока = Объект.БезналичнаяОплата.Добавить();
			НоваяСтрока.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.Бонусы");
			НоваяСтрока.БонуснаяКарта = Объект.ДисконтнаяКарта;
			СтрокиОплатыБонусами.Добавить(НоваяСтрока);
		КонецЕсли;
		Для Каждого СтрокаОплаты Из СтрокиОплатыБонусами Цикл
			Если СуммаОплатыБонусами = 0 Тогда
				Объект.БезналичнаяОплата.Удалить(СтрокаОплаты);
			Иначе
				СтрокаОплаты.СуммаБонусов = СуммаОплатыБонусами / КурсБаллов;
			КонецЕсли;
			СуммаОплатыБонусами = СуммаОплатыБонусами - СтрокаОплаты.СуммаБонусов;
		КонецЦикла;
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура ПослеОбработкиШтрихкодов(РезультатОбработкиШтрихкода, КэшированныеЗначения) Экспорт
	...
	Иначе
		...
		СуммаТоваров = ОбновитьСуммуДокумента(Объект);
...
Процедура УстановитьСтатусСерийСтроки(СтрокаТЧ)
	...
	// Подарки ранее были списаны, поэтому выяснения политики указания серий, используем тип документа СписаниеЗапасов
...
Функция РассчитатьАвтоматическиеСкидкиПриНеобходимостиПередЗаписью(Знач ДокументОбъект, ПараметрыРасчетаСкидок = Неопределено)
	...
	Если РасчетныеСкидки.Свойство("ОплатаБонусамиПревысилаПредел") Тогда
		ПараметрыРасчетаСкидок.Вставить("ОплатаБонусамиПревысилаПредел", РасчетныеСкидки.ОплатаБонусамиПревысилаПредел);
	КонецЕсли;	
	Если РасчетныеСкидки.Свойство("СуммаОплатыБонусами") Тогда
		ПараметрыРасчетаСкидок.Вставить("СуммаОплатыБонусами", РасчетныеСкидки.СуммаОплатыБонусами);
	КонецЕсли;
	Если РасчетныеСкидки.Свойство("НетСработавшихСкидок") Тогда
...
Процедура УправлениеФормой()
	...
	Если Не ВводНаОсновании Тогда
		...
		Объект.ПоложениеПатента = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке;
...
&НаСервереБезКонтекста
Функция ОбновитьСуммуДокумента(Объект)
	СуммаТоваров = Объект.Запасы.Итог("Всего");
	Если Не Объект.ОперацияСДенежнымиСредствами Тогда
		Объект.СуммаДокумента = СуммаТоваров;
	КонецЕсли;
...
Процедура РассчитатьПроцентСкидки(СтрокаТабличнойЧасти = Неопределено)
	...
	Если СтрокаТабличнойЧасти.Количество * СтрокаТабличнойЧасти.Цена < СтрокаТабличнойЧасти.СуммаСкидкиНаценки Тогда
		СтрокаТабличнойЧасти.СуммаСкидкиНаценки = СтрокаТабличнойЧасти.Количество * СтрокаТабличнойЧасти.Цена;
	КонецЕсли;
	
	СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Количество * СтрокаТабличнойЧасти.Цена - СтрокаТабличнойЧасти.СуммаСкидкиНаценки;
...
Процедура ЗаполнитьКомиссиюЭквайрераПоСтроке(ИдентификаторСтроки)
	...
	СписокВидовКарт = Справочники.СпособыОплаты.ВидыПлатежныхКартИПроцентыКомиссии(Терминал);
	СтрокаКомиссий = СписокВидовКарт.Найти(СтрокаОплаты.ВидПлатежнойКарты, "ВидПлатежнойКарты");
...
Процедура ОбработатьДобавлениеСтрокиЗапасов(НоваяСтрока, ИмяТабличнойЧасти = "Запасы", КодАкцизнойМарки = Неопределено, ПересчитыватьНДС = Ложь)
	...
	Иначе
		...
		Если НоваяСтрока.НеобходимостьВводаАкцизнойМарки
			И ЗначениеЗаполнено(НоваяСтрока.КодАкцизнойМарки) Тогда
			...
			СтрокаАкцизнаяМарка.КодАкцизнойМарки = НоваяСтрока.КодАкцизнойМарки;
...
Процедура ЗаполнитьПатент()
	...
	Если Объект.ПоложениеПатента = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
		Для Каждого СтрокаЗапасы Из Объект.Запасы Цикл
			Если Не ЗначениеЗаполнено(СтрокаЗапасы.Патент) И ЗначениеЗаполнено(Объект.Патент) Тогда
...
Процедура ЗаполнитьПодписьКассира()
	...
	Подпись = ПерсональныеНастройкиРУНФ.Прочитать("ПодписьКассира", Объект.Организация);
