...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
		Документы.СписаниеЗапасов.ПараметрыУказанияСерий(Объект));
	...
	ПоказыватьКолонкуВесВСписанииЗапасов = ПерсональныеНастройкиРУНФ.Прочитать("ПоказыватьКолонкуВесВСписанииЗапасов");
	ПоказыватьКолонкуОбъемВСписанииЗапасов = ПерсональныеНастройкиРУНФ.Прочитать(
		"ПоказыватьКолонкуОбъемВСписанииЗапасов");
	ПоказыватьКолонкуЦенаВСписанииЗапасов = ПерсональныеНастройкиРУНФ.Прочитать("ПоказыватьКолонкуЦенаВСписанииЗапасов");
	ПоказыватьКолонкуСуммаВСписанииЗапасов = ПерсональныеНастройкиРУНФ.Прочитать(
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ВыбранноеЗначение.Свойство("СтруктураНастроек") Тогда
			НоменклатураВДокументахКлиентСервер.ЗаполнитьТаблицуНастроекФормыВыбораНоменклатуры(ЭтотОбъект,
				"СписаниеЗапасов", НастройкиФормыВыбораНоменклатуры, Истина, "Запасы",
...
&НаСервере
Процедура ЗаполнитьДанныеЯчейки()
	Если ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ПоложениеЯчейки) 
			...
			ЗаполнитьЯчейкиВТЧ(ЯчейкаВСтроке);

		Иначе
			Объект.Ячейка = Неопределено;
		КонецЕсли;
	Иначе
		ЗаполнитьЯчейкиВТЧ(Неопределено);
		Объект.Ячейка = Неопределено;
...
&НаСервере
Процедура ЗаполнитьЯчейкиВТЧ(ЯчейкаВСтроке) 
...
Процедура ОткрытьФормуНастройки(ПередаватьВидДокумента)
	...
	ПараметрыФормы.Вставить("Доступность", СтруктураДоступности);
