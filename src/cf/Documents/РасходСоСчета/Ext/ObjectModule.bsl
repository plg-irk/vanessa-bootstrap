...
Процедура ЗаполнитьПоДоговоруКредитаЗайма(ДокументСсылкаДоговорКредитаИЗайма, Сумма = Неопределено) Экспорт
	...
	ВидДоговораДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаДоговорКредитаИЗайма,
		"ВидДоговора");
	Если ВидДоговораДокумента = Перечисления.ВидыДоговоровКредитаИЗайма.ДоговорЗаймаСотруднику Тогда
...
Процедура ЗаполнитьРасшифровкуПлатежа() Экспорт
	...
	ВидСтавкиОрганизацииДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация,
		"ВидСтавкиНДСПоУмолчанию");
		
	Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
		СтавкаНДСПоУмолчанию = Справочники.СтавкиНДС.СтавкаНДС(ВидСтавкиОрганизацииДокумента);
	...
	ВестиРасчетыПоДоговорамКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент,
		"ВестиРасчетыПоДоговорам");
	НуженОтборПоВидуДоговора = НуженОтборПоДоговорам И ВестиРасчетыПоДоговорамКонтрагента;
	...
	Запрос.УстановитьПараметр("НуженОтборПоВидуДоговора", НуженОтборПоВидуДоговора);
	...
	ВалютаДоговораПоУмолчанию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорПоУмолчанию, "ВалютаРасчетов");
	
	СтруктураКурсВалютыДоговораПоУмолчанию = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(
		Дата,
		Новый Структура("Валюта", ВалютаДоговораПоУмолчанию));
...
Процедура ЗаполнитьРасшифровкуПлатежаАвтоЗачетДолгов() Экспорт
	...
	Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
		СтавкаНДСПоУмолчанию = Справочники.СтавкиНДС.СтавкаНДС(ВидСтавкиОрганизации);
	ИначеЕсли НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
		СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
	Иначе
		СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСНоль();
	...
	НуженОтборПоДоговорам = УправлениеНебольшойФирмойПовтИсп.ТребуетсяКонтрольДоговоровКонтрагентов();
	СписокВидовДоговоров = Справочники.ДоговорыКонтрагентов.ПолучитьСписокВидовДоговораДляДокумента(Ссылка,
		ВидОперации);
	ВестиРасчетыПоДоговорамКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, 
		"ВестиРасчетыПоДоговорам");
		
	Если НуженОтборПоДоговорам И ВестиРасчетыПоДоговорамКонтрагента Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "// ТекстДоговорОтбор", "И Договор.ВидДоговора В (&СписокВидовДоговоров)");
		Запрос.УстановитьПараметр("СписокВидовДоговоров", СписокВидовДоговоров);
	КонецЕсли;
	
	Если ВестиРасчетыПоДоговорамКонтрагента Тогда
...
//  Сумма - Неопределено, Число- Сумма
//
Процедура ЗаполнитьПоПеремещениеДСПлан(ДокументСсылкаПеремещениеДСПлан, Сумма = Неопределено) Экспорт
	
	СтатусУтвержденияПлатежаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПеремещениеДСПлан,
		"СтатусУтвержденияПлатежа");
	
	Если СтатусУтвержденияПлатежаДокумента = Перечисления.СтатусыУтвержденияПлатежей.НеУтвержден Тогда
...
Процедура ЗаполнитьПоПлатежноеПоручение(ДокументСсылкаПлатежноеПоручение) Экспорт
	...
	ДокументОснованияПоручения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПлатежноеПоручение,
		"ДокументОснование");
		
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаПлатежноеПоручение);
	Запрос.УстановитьПараметр("ОснованиеПлатежногоПоручения", ДокументОснованияПоручения);
	Запрос.УстановитьПараметр("Дата", ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.РасчетыПоКредитам)
	|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежноеПоручение.ВыплатаСамозанятому)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ВыплатаСамозанятому)
	|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежноеПоручение.ВыплатыСамозанятым)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ВыплатыСамозанятым)
	...
	|	ТаблицаДокумента.РеестрВыплатСамозанятым,
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата Тогда
	...
	Иначе
		...
		ТаблицаРасшифровкиПуста = Истина;
		Если ЗначениеЗаполнено(ДокументОснование) Тогда
			РасшифровкаПлатежаОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование,
				"РасшифровкаПлатежа");
			//@skip-check bsl-legacy-check-static-feature-access-for-unknown-left-part
			ТаблицаРасшифровкиПуста = РасшифровкаПлатежаОснования.Пустой();
		КонецЕсли;
			
		Если ТаблицаРасшифровкиПуста Тогда
				...
				ВладелецДоговораРасшифровки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрока.Договор,
					"Владелец");
				Если ВладелецДоговораРасшифровки <> Контрагент Тогда
					НоваяСтрока.Договор = Выборка.ДоговорПП;
				КонецЕсли;
		Иначе
	...
	Если ВидНалога = Справочники.ВидыНалогов.Патент Тогда
		УчитыватьВНУ = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	|" + "
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(РасчетыСПокупателямиОбороты.СуммаВалОборот), 0)
	|ИЗ
	|	РегистрНакопления.РасчетыСПокупателями.Обороты(, , Регистратор, Документ = &Ссылка) КАК РасчетыСПокупателямиОбороты
	|ГДЕ
	|	РасчетыСПокупателямиОбороты.Регистратор <> &СсылкаНаТекущийДокумент
	|	И РасчетыСПокупателямиОбороты.Регистратор <> &Ссылка
	|	И РасчетыСПокупателямиОбороты.Документ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
//
// Параметры:
//	ДокументСсылкаПередачаТоваровМеждуОрганизациями - ДокументСсылка.
//	
Процедура ЗаполнитьПоПередачаТоваровМеждуОрганизациями(ДокументСсылкаПередачаТоваровМеждуОрганизациями) Экспорт
	...
	ОрганизацияПолучательДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДокументСсылкаПередачаТоваровМеждуОрганизациями, "ОрганизацияПолучатель");
		
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаПередачаТоваровМеждуОрганизациями);
	Запрос.УстановитьПараметр("Организация", ОрганизацияПолучательДокумента);
	Запрос.УстановитьПараметр("Дата", ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
	Запрос.УстановитьПараметр("СсылкаНаТекущийДокумент", Ссылка);
...
// 
// Параметры:
//  ДокументСсылкаЗаказПоставщику - ДокументСсылка.ЗаказПоставщику - Документ 
//  НомерСтроки - Неопределено, Число - Номер строки
//  Сумма - Неопределено, Число - Сумма
//  
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументСсылкаНачислениеНалогов) Экспорт
	...
	ВидОперацииДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаНачислениеНалогов,
		"ВидОперации");
		
	Если ВидОперацииДокумента <> Перечисления.ВидыОперацийНачислениеНалогов.Начисление Тогда
	...
	|	ТаблицаДокументаНалоги.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ЕСТЬNULL(ТаблицаДокументаНалоги.СрокУплаты, ДАТАВРЕМЯ(1, 1, 1)) КАК СрокУплатыНалога
	|ИЗ
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.СрокУплатыНалога > '20230101' Тогда
			ВидНалога = Справочники.ВидыНалогов.ЕдиныйНалоговыйПлатеж;
			Контрагент = ДанныеГосударственныхОрганов.АдминистраторНалогаОрганизации(ВидНалога,
				Организация, Выборка.СрокУплатыНалога);
			СрокУплаты = РегламентированнаяОтчетностьУСН.ОпределитьСрокПлатежаПоЕНП(Выборка.СрокУплатыНалога);  
			ПоказателиНалоговогоПериода = ПлатежиВБюджетКлиентСервер.ПоказателиНалоговогоПериода(Организация,
				ВидНалога, Выборка.СрокУплатыНалога);
...
Процедура ЗаполнитьПоВыплатыСамозанятым(ДокументСсылкаВыплатыСамозанятым) Экспорт
	...
	"ВЫБРАТЬ
	|	ШапкаДокумента.Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ВыплатыСамозанятым) КАК ВидОперации,
	|	&Ссылка КАК ДокументОснование,
...
Процедура ЗаполнитьПоПоступлениюНаСчет(ДанныеЗаполнения) Экспорт
	...
	Исключения = "Номер, Дата, НомерЧекаККМ";
	
	СоответствиеВидовОпераций = Новый Соответствие();
	СоответствиеВидовОпераций.Вставить(
		Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя, Перечисления.ВидыОперацийРасходСоСчета.Покупателю);
	СоответствиеВидовОпераций.Вставить(
		Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика, Перечисления.ВидыОперацийРасходСоСчета.Поставщику);
	СоответствиеВидовОпераций.Вставить(
		Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПодотчетника, Перечисления.ВидыОперацийРасходСоСчета.Подотчетнику);
	СоответствиеВидовОпераций.Вставить(
		Перечисления.ВидыОперацийПоступлениеНаСчет.Прочее, Перечисления.ВидыОперацийРасходСоСчета.Прочее);
	СоответствиеВидовОпераций.Вставить(
		Перечисления.ВидыОперацийПоступлениеНаСчет.ПрочиеРасчеты, Перечисления.ВидыОперацийРасходСоСчета.ПрочиеРасчеты);
	СоответствиеВидовОпераций.Вставить(Перечисления.ВидыОперацийПоступлениеНаСчет.ОтНашейОрганизации,
		Перечисления.ВидыОперацийРасходСоСчета.НашейОрганизации);
	СоответствиеВидовОпераций.Вставить(Перечисления.ВидыОперацийПоступлениеНаСчет.ВзносНаличными,
		Перечисления.ВидыОперацийРасходСоСчета.СнятиеНаличных);
	СоответствиеВидовОпераций.Вставить(Перечисления.ВидыОперацийПоступлениеНаСчет.ЛичныеСредстваПредпринимателя,
		Перечисления.ВидыОперацийРасходСоСчета.ЛичныеСредстваПредпринимателя);
	СоответствиеВидовОпераций.Вставить(Перечисления.ВидыОперацийПоступлениеНаСчет.Налоги,
		Перечисления.ВидыОперацийРасходСоСчета.Налоги);	
	СоответствиеВидовОпераций.Вставить(Перечисления.ВидыОперацийПоступлениеНаСчет.ПереводСДругогоСчета,
		Перечисления.ВидыОперацийРасходСоСчета.ПереводНаДругойСчет);
	
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеЗаполнения, , Исключения);
	
	Если СоответствиеВидовОпераций[ДанныеЗаполнения.ВидОперации] = Неопределено Тогда
		ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю;
	Иначе
		ВидОперации = СоответствиеВидовОпераций[ДанныеЗаполнения.ВидОперации];
	КонецЕсли;
	
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ПереводНаДругойСчет Тогда
		СчетКэш = БанковскийСчет;
		БанковскийСчет = СчетКонтрагента;
		СчетКонтрагента = СчетКэш;
	КонецЕсли;
	...
	ЕстьСтрокиСАвтоЗачетом = Ложь;
	Для Каждого СтрокаРасшифровки Из ДанныеЗаполнения.РасшифровкаПлатежа Цикл
		Если СтрокаРасшифровки.СпособЗачета = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто Тогда
			ЕстьСтрокиСАвтоЗачетом = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю И ЕстьСтрокиСАвтоЗачетом Тогда
		ЗаполнитьРасшифровкуПлатежаПоРегистру(ДанныеЗаполнения);	
	Иначе
		ЗаполнитьРасшифровкуПлатежаПоДокументу(ДанныеЗаполнения);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ЛичныеСредстваПредпринимателя
		И ТипОрганизации = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
	...
	ДобавитьПервуюСтрокуРасшифровкиПлатежа();
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику
		...
		ДанныеУчетаКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент,
			"ВестиРасчетыПоДоговорам, ВестиРасчетыПоДокументам");
		
		Если НЕ ДанныеУчетаКонтрагента.ВестиРасчетыПоДоговорам = Истина Тогда
			ЗаполнениеОбъектовУНФ.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "РасшифровкаПлатежа.Договор");
		КонецЕсли;
		
		Если ДанныеУчетаКонтрагента.ВестиРасчетыПоДокументам = Истина
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата Тогда
		...
		Для Каждого СтрокаВедомости Из ВыплатаЗаработнойПлаты Цикл
			...
			ВалютаДокументаВедомости = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаВедомости.Ведомость,
				"ВалютаДокумента");
				
			Если ВалютаДокументаВедомости <> ВалютаДенежныхСредств Тогда
				ТекстСообщения = СтрШаблон(НСтр(
					"ru = 'Валюта документа (%1) не соответствует валюте платежной ведомости (%2)'"),
					СокрЛП(ВалютаДенежныхСредств), ВалютаДокументаВедомости);
		...
		Если ВыплатаРегламентированнойЗаработнойПлаты.Количество() > 0 И ВалютаДенежныхСредств <> Константы.НациональнаяВалюта.Получить()
			И Справочники.Организации.НужноПоказыватьЭлементыБЗКБДляОрганизации(Организация) Тогда
			ТекстСообщения = НСтр(
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Прочее Тогда
		...
		ТипСчетаКорреспонденции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Корреспонденция, "ТипСчета");
		Если ТипСчетаКорреспонденции <> Перечисления.ТипыСчетов.Расходы Тогда
			ЗаполнениеОбъектовУНФ.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Подразделение");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты
		...
		ТипСчетаТерминала = Перечисления.ТипыСчетов.ПустаяСсылка();
		Если ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
			
			СчетЗатратТерминала = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭквайринговыйТерминал, "СчетЗатрат");
			Если ЗначениеЗаполнено(СчетЗатратТерминала) Тогда
				ТипСчетаТерминала = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетЗатратТерминала, "ТипСчета");
			КонецЕсли;
			
		КонецЕсли;
		
		Если ТипСчетаТерминала <> Перечисления.ТипыСчетов.Расходы Тогда
			ЗаполнениеОбъектовУНФ.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ПодразделениеЗатраты");
		КонецЕсли;
		...
		Если ПараметрыЭквайринга.КонтрольВзаиморасчетовЭквайринг Тогда
			...
			Если СуммаПлатежа <> СуммаДокумента Тогда
				...
				Иначе
					ТекстСообщения = НСтр(
						"ru = 'Сумма документа плюс сумма комиссии не соответствует сумме разнесенных платежей в табличной части'");
				КонецЕсли;
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "СуммаДокумента", , Отказ);
			КонецЕсли;
		Иначе
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ПрочиеРасчеты Тогда
		...
		ТипСчетаКоррекспонденции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Корреспонденция, "ТипСчета");
		Если ТипСчетаКоррекспонденции <> Перечисления.ТипыСчетов.Расходы Тогда
			ЗаполнениеОбъектовУНФ.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Подразделение");
			ЗаполнениеОбъектовУНФ.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "РасшифровкаПлатежа.Подразделение");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВыплатаСамозанятому
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВыплатыСамозанятым Тогда
		...
		Если ЗначениеЗаполнено(РеестрВыплатСамозанятым) Тогда
			...
			Если РегистрыНакопления.РасчетыСПрочимиКонтрагентами.СуммаЗадолженностиПоРеестру(
				...
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "РеестрВыплатСамозанятым", "Объект", Отказ);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Константы.ФункциональнаяОпцияУчетПоНесколькимНаправлениямДеятельности.Получить()
		  И ТипСчетаКоррекспонденции = Перечисления.ТипыСчетов.Расходы Тогда
	...
	Для каждого СтрокаТЧ Из РасшифровкаПлатежа Цикл
		...
		И НЕ ВестиРасчетыПоДоговорамКонтрагента
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ПереводНаДругойСчет Тогда
		...
		Иначе
			Корреспонденция = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетКонтрагента, "СчетУчета");
		КонецЕсли;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.СнятиеНаличных Тогда
		Если РегистрыСведений.ПрименениеПереводовВПути.ИспользуютсяПереводыВПути(Дата,Константы.УчетПоКомпании.Компания(Организация)) Тогда
			Корреспонденция = ПланыСчетов.Управленческий.ПереводыВПути;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ВыплатыСамозанятым.УстановитьСостояниеДокументаПриОплате(Ссылка, Движения, Отказ);
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата
		...
		ЗарплатаКадры.ЗарегистрироватьОплатуВедомостей(
			...
			Отказ);
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Налоги Тогда
		Если ВидНалога = Справочники.ВидыНалогов.ФССТравматизмСотрудники
			...
			СтрокаПеречисленныйНалог.Сумма = СуммаДокумента;
			СтрокаПеречисленныйНалог.МесяцНалоговогоПериода = СрокУплаты;
			
			Ставка = Неопределено;
			ФизическиеЛица = Неопределено;
			РегистрацияВНалоговомОргане = Неопределено;
			ЗаписыватьДвижения = Ложь;
			ДатаОперации = СрокУплаты;
			...
			СтрокаРасчетыСФондом.Период = СрокУплаты;
			СтрокаРасчетыСФондом.Сумма = СуммаДокумента;
			...
			СтрокаРасчетыСФондом.ГодЗадолженностиПоЕСН = Год(СрокУплаты);
...
Процедура ЗаполнитьПоСвойствуОснование(ДанныеЗаполнения)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения.Документ) = Тип("ДокументСсылка.ПоступлениеНаСчет") Тогда
		
		ЗаполнитьПоПоступлениюНаСчет(ДанныеЗаполнения);
...
// 
// Параметры:
//  ДокументСсылкаРасходДСПлан - ДокументСсылка.РасходДСПлан - Документ 
//  Сумма - Неопределено, Число - Сумма
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	
	ДанныеДокументаДСПлан = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылкаРасходДСПлан, "СтатусУтвержденияПлатежа,
																								   |ТипДенежныхСредств,
																								   |ДокументОснование,
	...
	Если ДанныеДокументаДСПлан.СтатусУтвержденияПлатежа = Перечисления.СтатусыУтвержденияПлатежей.НеУтвержден Тогда
		ВызватьИсключение НСтр("ru = 'Нельзя ввести расход денег на основании неутвержденной заявки!'");
	КонецЕсли;
	Если ДанныеДокументаДСПлан.ТипДенежныхСредств = Перечисления.ТипыДенежныхСредств.Наличные Тогда
		ВызватьИсключение НСтр("ru = 'Нельзя ввести расход денег. В заявке указан не верный способ оплаты (тип денежных средств)!'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДанныеДокументаДСПлан.ДокументОснование) Тогда
		
		ЗаполненПоОснованиюЗаявкиНаРасход = Истина;
		
		Если ТипЗнч(ДанныеДокументаДСПлан.ДокументОснование) = Тип("ДокументСсылка.ДоговорКредитаИЗайма") Тогда
			ЗаполнитьПоДоговоруКредитаЗайма(ДанныеДокументаДСПлан.ДокументОснование);
		ИначеЕсли ТипЗнч(ДанныеДокументаДСПлан.ДокументОснование) = Тип("ДокументСсылка.ПлатежнаяВедомость") Тогда
			ЗаполнитьПоПлатежнаяВедомость(ДанныеДокументаДСПлан.ДокументОснование);
		ИначеЕсли ТипЗнч(ДанныеДокументаДСПлан.ДокументОснование) = Тип("ДокументСсылка.НачислениеНалогов") Тогда
			ЗаполнитьПоНачислениеНалогов(ДанныеДокументаДСПлан.ДокументОснование);
		Иначе
			ЗаполненПоОснованиюЗаявкиНаРасход = Ложь;
		КонецЕсли;
		
		Если ЗаполненПоОснованиюЗаявкиНаРасход Тогда
			Статья = ДанныеДокументаДСПлан.СтатьяДвиженияДенежныхСредств;
			ДокументОснование = ДокументСсылкаРасходДСПлан;
			СуммаДокумента = ДанныеДокументаДСПлан.СуммаДокумента;
			Контрагент = ДанныеДокументаДСПлан.Контрагент;
			СчетКонтрагента = ДанныеДокументаДСПлан.СчетКонтрагента;
			Если ТипЗнч(ДанныеДокументаДСПлан.ДокументОснование) = Тип("ДокументСсылка.ПлатежнаяВедомость") Тогда
				
				Если ВыплатаЗаработнойПлаты.Количество() = 1 Тогда
					ВыплатаЗаработнойПлаты.Получить(0).ДокументПланирования = ДокументСсылкаРасходДСПлан;
				Иначе
					НоваяСтрока = ВыплатаЗаработнойПлаты.Добавить();
			...
			Иначе
				...
				Если РасшифровкаПлатежа.Количество() = 1 Тогда
					РасшифровкаПлатежа.Получить(0).ДокументПланирования = ДокументСсылкаРасходДСПлан;
				Иначе
					НоваяСтрока = РасшифровкаПлатежа.Добавить();
					НоваяСтрока.ДокументПланирования = ДокументСсылкаРасходДСПлан;
				КонецЕсли;
	...
	Если Сумма <> Неопределено Тогда
		...
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.Зарплата)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Зарплата)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.ЗарплатаСотруднику)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ЗарплатаСотруднику)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.ЛичныеСредстваПредпринимателя)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ЛичныеСредстваПредпринимателя)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.НаРасходы)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.НаРасходы)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.Поставщику)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Поставщику)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.Подотчетнику)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Подотчетнику)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.Покупателю)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Покупателю)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.Прочее)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Прочее)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.ПрочиеРасчеты)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ПрочиеРасчеты)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.РасчетыПоКредитам)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.РасчетыПоКредитам)
		|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.Налоги)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Налоги)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Поставщику)
		|	КОНЕЦ КАК ВидОперации,
		|	&Ссылка КАК ДокументОснование,
		...
		|	ТаблицаДокумента.СтатьяДвиженияДенежныхСредств КАК Статья,
		|	ТаблицаДокумента.Налог КАК ВидНалога,
		|	ВЫБОР
		|		КОГДА ТаблицаДокумента.БанковскийСчет <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчета.ПустаяСсылка)
		|			ТОГДА ТаблицаДокумента.БанковскийСчет
		|		КОГДА ТаблицаДокумента.Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств = ТаблицаДокумента.Договор.ВалютаРасчетов
	...
	Иначе
		...
		ДанныеУчетаКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент,
			"ВестиРасчетыПоЗаказам, ВестиУчетОплатыПоСчетам");
		
		Если ЗначениеЗаполнено(ДанныеДокументаДСПлан.ДокументОснование)
			И ТипЗнч(ДанныеДокументаДСПлан.ДокументОснование) = Тип("ДокументСсылка.ЗаказПоставщику")
			И ДанныеУчетаКонтрагента.ВестиРасчетыПоЗаказам Тогда
		
			НоваяСтрока.Заказ = ДанныеДокументаДСПлан.ДокументОснование;
		...
		Если ЗначениеЗаполнено(Выборка.ДокументОснованиеЗаявки)
			...
			И ДанныеУчетаКонтрагента.ВестиУчетОплатыПоСчетам Тогда
...
Процедура ДобавитьПервуюСтрокуРасшифровкиПлатежа()
...
Процедура ЗаполнитьРасшифровкуПлатежаПоРегистру(ДанныеЗаполнения)
	...
	|	РасчетыСПокупателями.Сумма КАК СуммаПлатежа,
	...
	Если ТаблицаРегистра.Количество() = 0
		Или ТаблицаРегистра.Количество() = 1 И ТаблицаРегистра[0].Документ = ДанныеЗаполнения Тогда
		ЗаполнитьРасшифровкуПлатежаПоДокументу(ДанныеЗаполнения);
		Возврат;
	КонецЕсли;
	...
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		СтрокаРасшифровкиЭталон = ДанныеЗаполнения.РасшифровкаПлатежа.Найти(Выборка.Договор, "Договор");
		Если СтрокаРасшифровкиЭталон = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока = РасшифровкаПлатежа.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаРасшифровкиЭталон);
		НоваяСтрока.СпособЗачета = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Вручную;
		...
		НоваяСтрока.ПризнакАванса = НоваяСтрока.Документ = ДанныеЗаполнения;
		РасчетыРаботаСФормамиВызовСервера.РассчитатьСуммуНДСНаСервере(НоваяСтрока);
	КонецЦикла;
	
	СуммаНДСДокументаОснования = ДанныеЗаполнения.РасшифровкаПлатежа.Итог("СуммаНДС");
	СуммаНДСТекущегоДокумента = РасшифровкаПлатежа.Итог("СуммаНДС");
	РазницаСуммНДС = СуммаНДСТекущегоДокумента - СуммаНДСДокументаОснования;
	
	Если РазницаСуммНДС <> 0 Тогда
		РасшифровкаПлатежа[0].СуммаНДС = РасшифровкаПлатежа[0].СуммаНДС - РазницаСуммНДС;
	КонецЕсли;

КонецПроцедуры // ЗаполнитьРасшифровкуПлатежаПоРегистру()

Процедура ЗаполнитьРасшифровкуПлатежаПоДокументу(ДанныеЗаполнения)
	...
	Для Каждого СтрокаРасшифровки Из ДанныеЗаполнения.РасшифровкаПлатежа Цикл
		...
		Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю
			И Не ЗначениеЗаполнено(НоваяСтрока.Документ) Тогда
			НоваяСтрока.Документ = ДокументОснование;
		КонецЕсли;
	КонецЦикла;

КонецПроцедуры // ЗаполнитьРасшифровкуПлатежаПоДокументу()
