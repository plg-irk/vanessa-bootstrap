...
Процедура ПодготовитьОтчет(СтруктураПараметров) Экспорт
	...
	Если ЗначениеЗаполнено(СтруктураПараметров.КонтрагентДляОтбора) Или СтруктураПараметров.ВыводитьПродавцовПоАвансам
	...
	Иначе
		СписокОрганизацийОтбора = УчетНДСУНФВызовСервераПовтИсп.ПолучитьСписокОбособленныхПодразделений(
            СтруктураПараметров.Организация);
	КонецЕсли;

	СписокДоступныхОрганизаций = УчетНДСУНФВызовСервераПовтИсп.ВсеОрганизацииДанныеКоторыхДоступныПоRLS(Ложь);
	...
	СтруктураПараметров.ОткрыватьПомощникИзМакета = УчетНДСПереопределяемый.ОткрыватьПомощникИзМакета(
		СтруктураПараметров.Организация, СтруктураПараметров.КонецПериода) И Не СтруктураПараметров.ЗаполнениеДекларации
	...
	Если СтруктураПараметров.ДополнительныеЛистыЗаТекущийПериод
		Или Не СтруктураПараметров.ФормироватьДополнительныеЛисты Тогда
	...
	Если Не СтруктураПараметров.ДополнительныеЛистыЗаТекущийПериод
		Или Не СтруктураПараметров.ФормироватьДополнительныеЛисты Тогда
...
Функция ПроверитьНаличиеДопЛистовКнигиПродаж(СтруктураПараметров) Экспорт
	...
	СтруктураПараметров.Вставить("ПрименяетсяОсвобождениеОтУплатыНДС",
		УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(СтруктураПараметров.Организация,
		ДатаПроверкиПараметровУчета));
	СтруктураПараметров.Вставить("ПлательщикНДС", УчетнаяПолитика.ПлательщикНДС(СтруктураПараметров.Организация,
		ДатаПроверкиПараметровУчета));
	СтруктураПараметров.Вставить("ПлательщикНДСПрименяющийУСН", УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(
	...
	Запрос.Текст = СтрЗаменить(
        Запрос.Текст, "&ДополнительноеУсловие", ДополнительноеУсловиеЗапроса(СтруктураПараметров));
...
Функция ПолучитьИтогиЗаПериодКнигаПродаж(СтруктураПараметров) Экспорт
	...
	Запрос.УстановитьПараметр("Организация", СтруктураПараметров.СписокОрганизаций);
	Запрос.УстановитьПараметр("МоментФормированияДопЛиста", Новый Граница(НачалоДня(
		СтруктураПараметров.ДатаФормированияДопЛиста), ВидГраницы.Исключая));

	Если ЭтоПечатьВыписок Тогда
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, "&МесяцДляВыписок", "НДСЗаписиКнигиПродажОбороты.Период");
	Иначе
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, "СГРУППИРОВАТЬ ПО
					  |	ЗаписиКнигиПродаж.МесяцДляВыписок", "");
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, "&МесяцДляВыписок", "Истина");
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, "Месяц", "Период");
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, "ЗаписиКнигиПродаж.ПериодДляВыписок", "Истина");
	КонецЕсли;
	...
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст, "&ДополнительноеУсловие", ДополнительноеУсловиеЗапроса(СтруктураПараметров, Ложь));
...
Процедура ОпределитьПараметрыФормированияОтчета(СтруктураПараметров)
	...
	ПлательщикНДСПрименяющийУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(СтруктураПараметров.Организация,
		ДатаПроверкиПараметровУчета);
	...
	ВычетыНалоговогоАгентаОтражаютсяВКнигеПродаж = ДатаПроверкиПараметровУчета >= '20180101'
		И ДатаПроверкиПараметровУчета < '20190101' И Не ПлательщикНДС;

	СтруктураПараметров.Вставить("ВерсияПостановленияНДС1137", УчетНДСПереопределяемый.ВерсияПостановленияНДС1137(
		ДатаПроверкиПараметровУчета));
	СтруктураПараметров.Вставить("Постановление1137_2019", УчетНДСПереопределяемый.ПрименяетсяПостановление15(
		ДатаПроверкиПараметровУчета));
	...
	СтруктураПараметров.Вставить("ПрименяетсяОсвобождениеОтУплатыНДС",
		УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(СтруктураПараметров.Организация,
		ДатаПроверкиПараметровУчета));
	СтруктураПараметров.Вставить("ВычетыНалоговогоАгентаОтражаютсяВКнигеПродаж",
		ВычетыНалоговогоАгентаОтражаютсяВКнигеПродаж);
...
Процедура СформироватьОсновнойРаздел(СтруктураПараметров, Результат)
	...
	ПрименяетсяПостановление1448 = УчетНДСПереопределяемый.ПрименяетсяПостановление1448(
		СтруктураПараметров.КонецПериода);
	ПрименяетсяПриказФНС000 = УчетНДСПереопределяемый.ПрименяетсяПриказФНС000(СтруктураПараметров.КонецПериода);

	Если ПрименяетсяПриказФНС000 Тогда
		Макет = ПолучитьОбщийМакет("КнигаПродаж2026"); // Макет с учетом предполагаемых изменений ставки НДС до 22%
	...
	ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 = 3 Тогда
		Если СтруктураПараметров.ЗаполнениеДекларации И СтруктураПараметров.ПравилаЗаполнения = "2017Кв1" Тогда
	...
	Иначе
		...
		СведенияОбОрганизации = ПечатьДокументовУНФ.СведенияОЮрФизЛице(
            СтруктураПараметров.Организация, СтруктураПараметров.КонецПериода);
		НазваниеОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации,
			"НаименованиеДляПечатныхФорм");
		ИННОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации, "ИНН", Ложь);
		КППОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОбОрганизации, "КПП", Ложь);
		...
		Секция.Параметры.ИННКППОрганизации   = ИННОрганизации + ?(Не ЗначениеЗаполнено(КППОрганизации), "", ("/"
			+ КППОрганизации));
	...
	Если Результат.Пустой() Тогда
		...
		Если СтруктураПараметров.СформироватьОтчетПоСтандартнойФорме И Не СтруктураПараметров.ЗаполнениеДекларации Тогда
			ВывестиПодвал(СтруктураПараметров, ТабличныйДокумент, Макет);
		КонецЕсли;

		УправлениеКолонтитулами.УстановитьКолонтитулы(ТабличныйДокумент);
		ПроверкаКонтрагентов.ДобавитьРазделОтчета(СтруктураПараметров, НазваниеРаздела,
			СтруктураПараметров.СчетчикСегментов, ТабличныйДокумент);
	...
	ПреобразоватьЗаписиКнигиПродаж(
        СтруктураПараметров, Результат, ТабличныйДокумент, СтрокаИтогПоОрганизации, СтруктураСекций, ?(
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		Если СтруктураПараметров.СчетчикСтрокТекущегоСегмента <> 0 Тогда
			...
			РегламентированнаяОтчетность.ЗаписатьСегментДанныхРазделаДекларацииНДС(
                СтруктураПараметров.СсылкаНаДекларацию, ИмяРаздела, ПерваяСтрокаСегмента,
				СтруктураПараметров.ТаблицаРаздел9);

			РегламентированнаяОтчетность.ЗаписатьСегментТабличногоДокументаРазделаДекларацииНДС(
                СтруктураПараметров.СсылкаНаДекларацию, ИмяРаздела, ПерваяСтрокаСегмента, ТабличныйДокумент);

		КонецЕсли;

	ИтогиРаздел9 = Новый Структура(
			"СтПродБезНДС22,СтПродБезНДС20,СтПродБезНДС18,СтПродБезНДС10,СтПродБезНДС0, СтПродБезНДС7, СтПродБезНДС5,
			|СумНДСВсКПр22,СумНДСВсКПр20,СумНДСВсКПр18,СумНДСВсКПр10,СтПродОсвВсКПр, СумНДСВсКПр7, СумНДСВсКПр5");
	...
	ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента И СтруктураПараметров.ФорматПоПостановлению735 Тогда

		ОбщиеСведения = Новый Структура("СтТовРубКоп20, СтТовРубКоп18,СтТовРубКоп10,СтТовРубКоп0,
	...
	ИначеЕсли Не СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		Если СтруктураПараметров.СчетчикСтрокТекущегоСегмента <> 0 Тогда
			...
			Иначе
				ЗаголовокЛиста = УчетНДС.СформироватьЗаголовокЛиста(НазваниеРаздела,
					СтруктураПараметров.СчетчикСегментов);
			КонецЕсли;

			ПроверкаКонтрагентов.ДобавитьРазделОтчета(СтруктураПараметров, ЗаголовокЛиста,
				СтруктураПараметров.СчетчикСегментов, ТабличныйДокумент);
...
Процедура СформироватьДополнительныеЛисты(СтруктураПараметров, Результат)
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		ИтогиРаздел91 = Новый Структура("ИтСтПродКПр22,ИтСтПродКПр20,ИтСтПродКПр18,ИтСтПродКПр10,ИтСтПродКПр7,ИтСтПродКПр5,ИтСтПродКПр0,
										|СумНДСИтКПр22,СумНДСИтКПр20,СумНДСИтКПр18,СумНДСИтКПр10,СумНДСИтКПр7,СумНДСИтКПр5,ИтСтПродОсвКПр,
										|СтПродВсП1Р9_22,СтПродВсП1Р9_20,СтПродВсП1Р9_18,СтПродВсП1Р9_10,СтПродВсП1Р9_7,СтПродВсП1Р9_5,СтПродВсП1Р9_0,
										|СумНДСВсП1Р9_22,СумНДСВсП1Р9_20,СумНДСВсП1Р9_18,СумНДСВсП1Р9_10,СумНДСВсП1Р9_7,СумНДСВсП1Р9_5,СтПродОсвП1Р9Вс");
		...
		ПараметрыПолученияИтогов.Вставить("КонецНалоговогоПериода", КонецКвартала(
			СтруктураПараметров.КонецНалоговогоПериода));
	...
	Пока ИтогПоПериодам.Следующий() Цикл
		...
		Пока ИтогПоПериодамКорректировки.Следующий() Цикл
			...
			ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 >= 5 Тогда
				Макет = ПолучитьОбщийМакет("КнигаПродаж534"); // Общий макет для книги и доп. листов
				СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
			ИначеЕсли УчетНДСПереопределяемый.ПрименяетсяПостановление15(
				ИтогПоПериодамКорректировки.КонецНалоговогоПериода) Тогда
			...
			СтруктураПараметров.Вставить("НалоговыйПериод", ?(СтруктураПараметров.ЗаполнениеДекларации,
				СтруктураПараметров.НачалоНалоговогоПериода, ИтогПоПериодамКорректировки.НалоговыйПериод));
			СтруктураПараметров.Вставить("КонецНалоговогоПериода", КонецКвартала(
				ИтогПоПериодамКорректировки.КонецНалоговогоПериода));
			...
			Если СтруктураПараметров.ЗаполнениеДокумента И СтруктураПараметров.ФорматПоПостановлению735 Тогда
				...
				Итого = Новый Структура("СтТовРубКоп22,СтТовРубКоп20, СтТовРубКоп18,СтТовРубКоп10,СтТовРубКоп0,
			...
			ПреобразоватьЗаписиДополнительногоЛистаКнигиПродаж(
				СтруктураПараметров, ИтогПоПериодамКорректировки, ?(СтруктураПараметров.Свойство("РеквизитыДокумента"),
				СтруктураПараметров.РеквизитыДокумента, ИтогЗаПериод), ТабличныйДокумент, СтруктураСекций, ?(
				СтруктураПараметров.Свойство("ТабличнаяЧасть"), СтруктураПараметров.ТабличнаяЧасть, Неопределено));
			...
			Если СтруктураПараметров.ЗаполнениеДокумента И СтруктураПараметров.ФорматПоПостановлению735 Тогда
				...
				Всего = Новый Структура("СтТовРубКоп22,СтТовРубКоп20, СтТовРубКоп18,СтТовРубКоп10,СтТовРубКоп0,
										|СумНДСРубКоп22,СумНДСРубКоп20,СумНДСРубКоп18,СумНДСРубКоп10,СтПродОсвНДСРубКоп");
				...
				СтруктураДанных = Новый Структура("ОбщиеСведения,Записи", ОбщиеСведения,
					СтруктураПараметров.Записи.Скопировать());
		...
		ПроверкаКонтрагентов.ДобавитьРазделОтчета(
			СтруктураПараметров, НазваниеРаздела, СтруктураПараметров.СчетчикСегментов, ТабличныйДокумент,
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		Если СтруктураПараметров.СчетчикСтрокТекущегоСегмента <> 0 Тогда
			...
			Если ПерваяСтрокаСегмента = 1 Тогда
				...
				Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Или СтруктураПараметров.ПравилаЗаполнения
					= "2026Кв1" Тогда
		...
		ИначеЕсли СтруктураПараметров.СчетчикСтрокТекущегоСегмента = 0 И СтруктураПараметров.СчетчикСтрокДопЛиста <> 0 Тогда
		...
		РегламентированнаяОтчетность.ЗаписатьИтогиРазделаДекларацииНДС(
			СтруктураПараметров.СсылкаНаДекларацию, "Раздел9_Прил1", ИтогиРаздел91);
...
Процедура ЗаписатьИтогиВПервыйСегментРаздела91(СтруктураПараметров, ИтогЗаПериод)
	...
	Если ТабличныйДокументРазделаДекларации <> Неопределено Тогда
		...
		ТабличныйДокументРазделаДекларации.Область(
			"СуммаСовсемБезНДСПриложение").Значение = ИтогЗаПериод.СуммаСовсемБезНДС;
...
Процедура ВывестиПодвал(СтруктураПараметров, ТабличныйДокумент, Макет)
	...
	ОтветственныеЛица = УправлениеНебольшойФирмойЭлектронныеДокументыСервер.ОтветственныеЛицаОрганизационнойЕдиницы(
		СтруктураПараметров.Организация);
	Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЭтоЮрЛицо(СтруктураПараметров.Организация) Тогда
		ИмяРук = ОтветственныеЛица.РасшифровкаПодписиРуководителя;
		ИмяОрг = "";
		Свидетельство = "";
	Иначе
		ИмяРук = "";
		ИмяОрг = ОтветственныеЛица.РасшифровкаПодписиРуководителя;
		Свидетельство = "";
	КонецЕсли;
...
Процедура ИнициализироватьТаблицыДанныхНДС(СтруктураПараметров)
	...
	Иначе
		...
		ИтогиРаздел9 = Новый Структура(
			"СтПродБезНДС22,СтПродБезНДС20,СтПродБезНДС18,СтПродБезНДС10,СтПродБезНДС0,
			|СумНДСВсКПр22,СумНДСВсКПр20,СумНДСВсКПр18,СумНДСВсКПр10,СтПродОсвВсКПр");
		ИтогиРаздел91 = Новый Структура(
			"ИтСтПродКПр22,ИтСтПродКПр20,ИтСтПродКПр18,ИтСтПродКПр10,ИтСтПродКПр0,
			|СумНДСИтКПр22,СумНДСИтКПр20,СумНДСИтКПр18,СумНДСИтКПр10,ИтСтПродОсвКПр,
			|СтПродВсП1Р9_22,СтПродВсП1Р9_20,СтПродВсП1Р9_18,СтПродВсП1Р9_10,СтПродВсП1Р9_0,
			|СумНДСВсП1Р9_22,СумНДСВсП1Р9_20,СумНДСВсП1Р9_18,СумНДСВсП1Р9_10,СтПродОсвП1Р9Вс");
		
		Если СтруктураПараметров.ЗаполнениеДекларации
		   И (СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Или СтруктураПараметров.ПравилаЗаполнения = "2026Кв1") Тогда
...
Процедура ПреобразоватьЗаписиКнигиПродаж(СтруктураПараметров, НаборЗаписей, ТабличныйДокумент, СтрокаИтогПоОрганизации,
	СтруктураСекций, ТаблицаДокумента)
	...
	Если Не СтруктураПараметров.ЗаполнениеДокумента И Не СтруктураПараметров.ЗаполнениеДекларации И СтруктураПараметров.ГруппироватьПоКонтрагентам
		...
		Пока ИтогПоКонтрагенту.Следующий() Цикл 
				...
				Пока ИтогПоСчетФактураДокумент.Следующий() Цикл
					...
					Пока ИтогПоДоговорАванса.Следующий() Цикл
						...
						Пока ЗаписьКниги.Следующий() Цикл
							...
							Если ЗаписьКниги.НачислениеПоОказаниюУслуг Или (ЗаписьКниги.СводныйКорректировочный
								...
								Пока ЗаписьКнигиДетальная.Следующий() Цикл
									...
									Если УчетНДС.ВыводитьСтроку(СтруктураПараметров, ЗаписьКнигиДетальная) Тогда
										...
										ОбластьТабличногоДокумента = ТабличныйДокумент.Вывести(
											СтруктураСекций.СекцияСтрока);
										УчетНДС.ДобавитьКонтрагентаНаПроверку(
                                            СтруктураПараметров, ЗаписьКнигиДетальная, ОбластьТабличногоДокумента, ,
											СчетчикСегментов);

										УчетНДС.ЗаполнитьПодстрочникРНПТ(
                                            ЗаписьКнигиДетальная, ЗаписьКниги, ПараметрыСтрокиРНПТ, СтруктураПараметров,
											СтруктураСекций, СчетчикСегментов, ТабличныйДокумент);

									Иначе
										УчетНДС.УменьшитьСуммуИтога(СтрокаИтогПоКонтрагенту, ЗаписьКнигиДетальная,
											СтруктураПараметров);
										УчетНДС.УменьшитьСуммуИтога(СтрокаИтогПоОрганизации, ЗаписьКнигиДетальная,
											СтруктураПараметров);
									КонецЕсли;
								КонецЦикла; // ЗаписьКнигиДетальная.Следующий()

							ИначеЕсли ЗаписьКниги.СводныйКомиссионный И СтруктураПараметров.ВерсияПостановленияНДС1137
								< 4 Тогда

								Счетчик = Счетчик + 1;
								СчетчикСтрокТекущегоСегмента = СчетчикСтрокТекущегоСегмента + 1;
								ЗаполнитьСтрокуКнигиПродажСводныйКомиссионный(ЗаписьКниги, СтруктураПараметров,
									ПараметрыСтроки, Счетчик);
							...
							Иначе
								...
								Если УчетНДС.ВыводитьСтроку(СтруктураПараметров, ЗаписьКниги, , ЗаписьКнигиДетальная) Тогда
									...
									ОбластьТабличногоДокумента = ТабличныйДокумент.Вывести(СтруктураСекций.СекцияСтрока);
									УчетНДС.ДобавитьКонтрагентаНаПроверку(
                                        СтруктураПараметров, ЗаписьКниги, ОбластьТабличногоДокумента,
										ЗаписьКнигиДетальная, СчетчикСегментов);
								...
								Иначе
									УчетНДС.УменьшитьСуммуИтога(СтрокаИтогПоКонтрагенту, ЗаписьКниги,
										СтруктураПараметров);
									УчетНДС.УменьшитьСуммуИтога(СтрокаИтогПоОрганизации, ЗаписьКниги,
										СтруктураПараметров);
								КонецЕсли;
							КонецЕсли;
			...
			Если СчетчикСтрокТекущегоСегмента >= СтруктураПараметров.ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС Тогда
				...
				ПроверкаКонтрагентов.ДобавитьРазделОтчета(СтруктураПараметров, ЗаголовокЛиста, СчетчикСегментов,
					ТабличныйДокумент);
			...
			Пока ИтогПоСчетФактураДокумент.Следующий() Цикл
				...
				Пока ИтогПоДоговорАванса.Следующий() Цикл
					...
					Пока ЗаписьКниги.Следующий() Цикл
						...
						Если Не ЗаписьКниги.СводнаяСправка И ((ЗаписьКниги.Покупатель = ТекстРозничнаяПродажа
							И ЗаписьКнигиДетальная.Количество() > 1) Или ЗаписьКниги.НалоговыйАгентПокупательТоваров
							Или ЗаписьКниги.НачислениеПоОказаниюУслуг Или (ЗаписьКниги.СводныйКорректировочный
							И СтруктураПараметров.ВерсияПостановленияНДС1137 < 4)) Тогда
							...
							Пока ЗаписьКнигиДетальная.Следующий() Цикл
								...
								Если УчетНДС.ВыводитьСтроку(СтруктураПараметров, ЗаписьКнигиДетальная) Тогда
									...
									ИначеЕсли Не СтруктураПараметров.ЗаполнениеДокумента Тогда
										СтруктураСекций.СекцияСтрока.Область(1, ,
											СтруктураСекций.СекцияСтрока.ВысотаТаблицы).Имя = "Строка_" + Формат(
											Счетчик, "ЧГ=");
										ОбластьТабличногоДокумента = ТабличныйДокумент.Вывести(
											СтруктураСекций.СекцияСтрока);
										УчетНДС.ДобавитьКонтрагентаНаПроверку(СтруктураПараметров,
											ЗаписьКнигиДетальная, ОбластьТабличногоДокумента, , СчетчикСегментов);
									КонецЕсли;
								...
								Если Не СтруктураПараметров.ЗаполнениеДокумента И СчетчикСтрокТекущегоСегмента
									= СтруктураПараметров.ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС Тогда
									...
									Если СтруктураПараметров.ЗаполнениеДекларации Тогда
										...
										РегламентированнаяОтчетность.ЗаписатьСегментДанныхРазделаДекларацииНДС(
                                            СтруктураПараметров.СсылкаНаДекларацию, "Раздел9", ПерваяСтрокаСегмента,
											СтруктураПараметров.ТаблицаРаздел9);
									...
									Иначе
										...
										ЗаголовокЛиста = УчетНДС.СформироватьЗаголовокЛиста(НазваниеРаздела,
											СчетчикСегментов);
										ПроверкаКонтрагентов.ДобавитьРазделОтчета(СтруктураПараметров, ЗаголовокЛиста,
											СчетчикСегментов, ТабличныйДокумент);
						...
						ИначеЕсли ЗаписьКниги.СводныйКомиссионный И СтруктураПараметров.ВерсияПостановленияНДС1137 < 4 Тогда
							...
							ЗаполнитьСтрокуКнигиПродажСводныйКомиссионный(ЗаписьКниги, СтруктураПараметров,
								ПараметрыСтроки, Счетчик);
							Если Не СтруктураПараметров.ЗаполнениеДокумента Или (СтруктураПараметров.ЗаполнениеДокумента
								И СтруктураПараметров.ФорматПоПостановлению735) Тогда
								СтруктураСекций.СекцияСтрока.Область(1, ,
									СтруктураСекций.СекцияСтрока.ВысотаТаблицы).Имя = "Строка_" + Формат(Счетчик, "ЧГ=");
								ТабличныйДокумент.Вывести(СтруктураСекций.СекцияСтрока);
							КонецЕсли;
						...
						Иначе
							...
							Если УчетНДС.ВыводитьСтроку(СтруктураПараметров, ЗаписьКниги, , ЗаписьКнигиДетальная) Тогда
								...
								ЗаполнитьСтрокуКнигиПродаж(СтруктураПараметров, ПараметрыСтроки, ЗаписьКниги, Счетчик,
									ДополнительнаяИнформация, ЗаписьКнигиДетальная);
								...
								ИначеЕсли Не СтруктураПараметров.ЗаполнениеДокумента Тогда
									СтруктураСекций.СекцияСтрока.Область(1, ,
										СтруктураСекций.СекцияСтрока.ВысотаТаблицы).Имя = "Строка_" + Формат(Счетчик,
										"ЧГ=");
									ОбластьТабличногоДокумента = ТабличныйДокумент.Вывести(СтруктураСекций.СекцияСтрока);
									УчетНДС.ДобавитьКонтрагентаНаПроверку(СтруктураПараметров, ЗаписьКниги,
										ОбластьТабличногоДокумента, ЗаписьКнигиДетальная, СчетчикСегментов);
								КонецЕсли;
...
Процедура ПреобразоватьЗаписиДополнительногоЛистаКнигиПродаж(СтруктураПараметров, ИтогПоПериодамКорректировки,
	ИтогЗаПериод, ТабличныйДокумент, СтруктураСекций, ТаблицаДокумента)
	...
	Если Не СтруктураПараметров.ЗаполнениеДокумента И Не СтруктураПараметров.ЗаполнениеДекларации И СтруктураПараметров.ГруппироватьПоКонтрагентам
		...
		Пока ИтогПоКонтрагенту.Следующий() Цикл
			...
			Пока ЗаписьПоСФ.Следующий() Цикл
				...
				Пока РазделениеПоДоговоруДляАвансов.Следующий() Цикл
					...
					Пока ЗаписьКниги.Следующий() Цикл
						...
						ЗаполнитьСтрокуКнигиПродаж(СтруктураПараметров, ПараметрыСтроки, ЗаписьКниги, Счетчик,
							ДополнительнаяИнформация, ДетальнаяЗапись);

						ТабличныйДокумент.Вывести(СтруктураСекций.СекцияСтрока);

						УчетНДС.ЗаполнитьПодстрочникРНПТ(
							ДетальнаяЗапись, ЗаписьКниги, ПараметрыСтрокиРНПТ, СтруктураПараметров, СтруктураСекций,
							СчетчикСегментов, ТабличныйДокумент);
						...
						ИтогЗаПериод.СуммаБезНДС20Или18 = ИтогЗаПериод.СуммаБезНДС20Или18
							+ ЗаписьКниги.СуммаБезНДС20Или18;
			...
			Если СчетчикСтрокТекущегоСегмента >= СтруктураПараметров.ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС Тогда
				...
				ДополнительныеПараметры.Вставить("ЭтоДопЛист", Истина);
				ПроверкаКонтрагентов.ДобавитьРазделОтчета(
					СтруктураПараметров, ЗаголовокЛиста, СчетчикСегментов, ТабличныйДокумент, ДополнительныеПараметры);
					...
					Если ЗаписьКниги.НалоговыйАгентПокупательТоваров Тогда
						...
						Пока ДетальнаяЗапись.Следующий() Цикл
							...
							ЗаполнитьСтрокуКнигиПродаж(
								СтруктураПараметров, ПараметрыСтроки, ДетальнаяЗапись, Счетчик,
								ДополнительнаяИнформация);

							УчетНДС.ВывестиГраницуСтроки(СтруктураПараметров, СтруктураСекций);

							Если СтруктураПараметров.ЗаполнениеДокумента И СтруктураПараметров.ФорматПоПостановлению735
								Или Не СтруктураПараметров.ЗаполнениеДокумента Тогда
								СтруктураСекций.СекцияСтрока.Область(
									1, , СтруктураСекций.СекцияСтрока.ВысотаТаблицы).Имя = "Строка_" + Формат(Счетчик,
									"ЧГ=");
								ТабличныйДокумент.Вывести(СтруктураСекций.СекцияСтрока);
							КонецЕсли;
							...
							Если Не СтруктураПараметров.ЗаполнениеДокумента И СчетчикСтрокТекущегоСегмента
								...
								Если СтруктураПараметров.ЗаполнениеДекларации Тогда
									...
									РегламентированнаяОтчетность.ЗаписатьСегментДанныхРазделаДекларацииНДС(
										СтруктураПараметров.СсылкаНаДекларацию, "Раздел9_Прил1", ПерваяСтрокаСегмента,
										СтруктураПараметров.ТаблицаРаздел91);
								...
								Иначе
									...
									// В заголовке в качестве номера порции выводим порцию в пределах налогового периода.
									ЗаголовокЛиста = УчетНДС.СформироватьЗаголовокЛиста(НазваниеРаздела,
										СчетчикСегментовНалоговогоПериода);
									// В общую таблицу передаем в качестве номера раздела глобальный счетчик сегментов (с учетом основных разделов книги).
									ДополнительныеПараметры = Новый Структура;
									ДополнительныеПараметры.Вставить("ЭтоДопЛист", Истина);
									ПроверкаКонтрагентов.ДобавитьРазделОтчета(
										СтруктураПараметров, ЗаголовокЛиста, СчетчикСегментов, ТабличныйДокумент,
										ДополнительныеПараметры);
							...
							ИтогЗаПериод.СуммаБезНДС20Или18 = ИтогЗаПериод.СуммаБезНДС20Или18
								+ ДетальнаяЗапись.СуммаБезНДС20Или18;
							...
							ИтогЗаПериод.СуммаСовсемБезНДС  = ИтогЗаПериод.СуммаСовсемБезНДС
								+ ДетальнаяЗапись.СуммаСовсемБезНДС;

						КонецЦикла;

					Иначе

						Если СтруктураПараметров.ЗаполнениеДокумента И Не СтруктураПараметров.ФорматПоПостановлению735 Тогда
							ПараметрыСтроки = ТаблицаДокумента.Добавить();
						КонецЕсли;
						...
						ЗаполнитьСтрокуКнигиПродаж(
							СтруктураПараметров, ПараметрыСтроки, ЗаписьКниги, Счетчик, ДополнительнаяИнформация,
							ДетальнаяЗапись);
...
Процедура ЗаполнитьСтрокуКнигиПродаж(СтруктураПараметров, Секция, ЗаписьКниги, НомерСтроки, ДополнительнаяИнформация,
	ДетальнаяЗапись = Неопределено)
	...
	СоставСтроки.Вставить("НомерДатаИсправленияКорректировки", ПредставлениеСФ.НомерДатаИсправленныйКорректировочный);
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполнения.Покупатель) Тогда
		...
		Иначе
			...
			СоставСтроки.Вставить("Графа8", Строка(ПараметрыЗаполнения.ПокупательИНН) + ?(ЗначениеЗаполнено(
				ПараметрыЗаполнения.ПокупательКПП), "/" + Строка(ПараметрыЗаполнения.ПокупательКПП), ""));
		КонецЕсли;
		СоставСтроки.Вставить("Посредник", ПараметрыЗаполнения.Посредник);
		СоставСтроки.Вставить("ПосредникИНН_КПП", "" + Строка(ПараметрыЗаполнения.ПосредникИНН) + ?(ЗначениеЗаполнено(
			ПараметрыЗаполнения.ПосредникКПП), "/" + Строка(ПараметрыЗаполнения.ПосредникКПП), ""));
	...
	СоставСтроки.Вставить("Валюта", ?(ЗначениеЗаполнено(ПараметрыЗаполнения.Валюта), "" + ПараметрыЗаполнения.Валюта
		+ ", " + ПараметрыЗаполнения.ВалютаКод, ""));
	...
	Если ДетальнаяЗапись = Неопределено Тогда
		...
		// Графа 11а
		ЗаполнитьСведенияОСчетеФактуреНаАванс(АвансовыеСчетаФактурыМассив, АвансовыеСчетаФактурыТекст, ЗаписьКниги);
		...
		Если ЗначениеЗаполнено(ЗаписьКниги.ДатаОплаты) Тогда
			Если СписокДатОплат.НайтиПоЗначению(Формат(ЗаписьКниги.ДатаОплаты, "ДФ=dd.MM.yyyy")) = Неопределено Тогда
				ТекстОплаты = ТекстОплаты + ?(Не ЗначениеЗаполнено(ТекстОплаты), "", "," + Символы.ПС) + Формат(
					ЗаписьКниги.ДатаОплаты, "ДФ=dd.MM.yyyy");
		...
		Если ЗначениеЗаполнено(ЗаписьКниги.НомерДокументаОплаты) Тогда
			...
			НомерДатаОплаты = СтрШаблон(НСтр("ru='%1;%2'"), ЗаписьКниги.НомерДокументаОплаты, Формат(
				ЗаписьКниги.ДатаДокументаОплаты, "ДФ=dd.MM.yyyy"));
			НомерОтДатаОплаты = СтрШаблон(НСтр("ru='%1 от %2'"), ЗаписьКниги.НомерДокументаОплаты, Формат(
				ЗаписьКниги.ДатаДокументаОплаты, "ДФ=dd.MM.yyyy"));

			Если СписокНомеровИДатОплат.НайтиПоЗначению(НомерДатаОплаты) = Неопределено Тогда

				ТекстОплатыСНомером = ТекстОплатыСНомером + ?(Не ЗначениеЗаполнено(ТекстОплатыСНомером), "", ","
					+ Символы.ПС) + НомерДатаОплаты;
				ТекстНомерОтДатаОплаты = ТекстНомерОтДатаОплаты + ?(Не ЗначениеЗаполнено(ТекстНомерОтДатаОплаты), "",
					";" + Символы.ПС) + НомерОтДатаОплаты;
	...
	Иначе
		...
		Пока СтрокаЗаписи.Следующий() Цикл
			...
			// Графа 11а
			ЗаполнитьСведенияОСчетеФактуреНаАванс(АвансовыеСчетаФактурыМассив, АвансовыеСчетаФактурыТекст, СтрокаЗаписи);
			...
			Если ЗначениеЗаполнено(СтрокаЗаписи.ДатаОплаты) Тогда
				Если СписокДатОплат.НайтиПоЗначению(Формат(СтрокаЗаписи.ДатаОплаты, "ДФ=dd.MM.yyyy")) = Неопределено Тогда
					ТекстОплаты = ТекстОплаты + ?(Не ЗначениеЗаполнено(ТекстОплаты), "", "," + Символы.ПС) + Формат(
						СтрокаЗаписи.ДатаОплаты, "ДФ=dd.MM.yyyy");
			...
			Если ЗначениеЗаполнено(СтрокаЗаписи.НомерДокументаОплаты) Тогда
				...
				НомерДатаОплаты = СтрШаблон(НСтр("ru='%1;%2'"), СтрокаЗаписи.НомерДокументаОплаты, Формат(
					СтрокаЗаписи.ДатаДокументаОплаты, "ДФ=dd.MM.yyyy"));
				НомерОтДатаОплаты = СтрШаблон(НСтр("ru='%1 от %2'"), СтрокаЗаписи.НомерДокументаОплаты, Формат(
					СтрокаЗаписи.ДатаДокументаОплаты, "ДФ=dd.MM.yyyy"));
	...
	СчетаФактуры = ОбщегоНазначенияПереопределяемый.УдалитьПовторяющиесяЭлементыМассива(СчетаФактуры);
	СтрокаКодВидаТовара = "";
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		МассивКодВидОпер = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
			ПараметрыЗаполнения.КодВидаОперации, ";");
		...
		Если СоставСтроки.Свойство("ПокупательИНН") И (ЗначениеЗаполнено(СоставСтроки.ПокупательИНН)
		...
		Если СоставСтроки.Свойство("Посредник") И (ЗначениеЗаполнено(ПараметрыЗаполнения.ПосредникИНН)
			Или ЗначениеЗаполнено(ПараметрыЗаполнения.ПосредникКПП)) Тогда
			СтруктураРеквизитовПосредника = УчетНДС.ПолучитьСтруктуруРеквизитовКонтрагента(
				ПараметрыЗаполнения.ПосредникИНН, ПараметрыЗаполнения.ПосредникКПП);
		...
		Если ЗначениеЗаполнено(ПредставлениеСФ.НомерКорректировочный) Тогда
			ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
				...
				ПредставлениеСФ.НомерКорректировочный, ПредставлениеСФ.ДатаКорректировочный, Формат(
				ЗаписьКниги.ВсегоПродаж, "ЧДЦ=2; ЧН="), ПараметрыЗаполнения.ВалютаНаименование);
		Иначе
			ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
				НСтр("ru='%1 на сумму %2 %3'"), ПредставлениеСФ.Представление, Формат(ЗаписьКниги.ВсегоПродаж,
				"ЧДЦ=2; ЧН="), ПараметрыЗаполнения.ВалютаНаименование);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1" Или СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
			Или СтруктураПараметров.ПравилаЗаполнения = "2021Кв3" Или СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			Или СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Или СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			СоставСтроки.Вставить("КодВидТовар", МассивКодВидаТовара);
		КонецЕсли;

		Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3" Или СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			Или СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Или СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
	...
	ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента И СтруктураПараметров.ФорматПоПостановлению735 Тогда
		...
		Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 5 И Не ЗначениеЗаполнено(ПараметрыЗаполнения.ВалютаКод) Тогда
			ВалютаКод = "643";
		Иначе
		...
		Если СоставСтроки.Свойство("Посредник") И Не ПустаяСтрока(ПараметрыЗаполнения.ПосредникИНН) Тогда
			СоставСтроки.Вставить("СвПосредн", УчетНДС.ПолучитьСтруктуруРеквизитовКонтрагента(
		...
		СоставСтроки.Вставить("СтТовРубКоп20Или18", ЗаписьКниги.СуммаБезНДС20Или18);
		...
		СоставСтроки.Вставить("СумНДСРубКоп20Или18", ЗаписьКниги.НДС20Или18);
...
Процедура ЗаполнитьСведенияОСчетеФактуреНаАванс(АвансовыеСчетаФактурыМассив, АвансовыеСчетаФактурыТекст, ЗаписьКниги)

	Если ЗначениеЗаполнено(ЗаписьКниги.НомерСчетаФактурыНаАванс) Тогда
		АвансовыеСчетаФактурыНомерДата = СтрШаблон(НСтр("ru='%1 от %2'"), ЗаписьКниги.НомерСчетаФактурыНаАванс, Формат(
			ЗаписьКниги.ДатаСчетаФактурыНаАванс, "ДФ=dd.MM.yyyy"));

		Если АвансовыеСчетаФактурыМассив.Найти(АвансовыеСчетаФактурыНомерДата) = Неопределено Тогда
			АвансовыеСчетаФактурыТекст = АвансовыеСчетаФактурыТекст + ?(Не ЗначениеЗаполнено(
				АвансовыеСчетаФактурыТекст), "", ";" + Символы.ПС) + АвансовыеСчетаФактурыНомерДата;
...
Процедура ЗаполнитьСтрокуКнигиПродажСводныйКомиссионный(ЗаписиКниги, СтруктураПараметров, Секция, НомерСтроки)
	...
	Пока ЗаписьКнигиДетальная.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ЗаписьКнигиДетальная.НомерДокументаОплаты) Тогда
			...
			НомерДатаОплаты = СтрШаблон(НСтр("ru='%1;%2'"), ЗаписьКнигиДетальная.НомерДокументаОплаты, Формат(
				ЗаписьКнигиДетальная.ДатаДокументаОплаты, "ДФ=dd.MM.yyyy"));
			НомерОтДатаОплаты = СтрШаблон(НСтр("ru='%1 от %2'"), ЗаписьКнигиДетальная.НомерДокументаОплаты, Формат(
				ЗаписьКнигиДетальная.ДатаДокументаОплаты, "ДФ=dd.MM.yyyy"));
		...
		Иначе
			...
			ВсегоПродажВВалюте  = ВсегоПродажВВалюте + ?(ЗначениеЗаполнено(ЗаписьКнигиДетальная.Валюта),
				ЗаписьКнигиДетальная.ВсегоПродажВВалюте, 0);
			...
			Если ЗначениеЗаполнено(ЗаписьКнигиДетальная.ПокупательИНН) Или ЗначениеЗаполнено(
				ЗаписьКнигиДетальная.ПокупательКПП) Тогда

				СтруктураРеквизитовКонтрагента = УчетНДС.ПолучитьСтруктуруРеквизитовКонтрагента(
					ЗаписьКнигиДетальная.ПокупательИНН, ЗаписьКнигиДетальная.ПокупательКПП);
		...
		Если ТекущаяЗапись = КоличествоЗаписей Тогда
			...
			СоставСтроки.Вставить("ПосредникИНН_КПП", "" + ЗаписьКнигиДетальная.ПосредникИНН + ?(ЗначениеЗаполнено(
				ЗаписьКнигиДетальная.ПосредникКПП), "/" + ЗаписьКнигиДетальная.ПосредникКПП, ""));

			Если ЗначениеЗаполнено(ЗаписьКнигиДетальная.ПосредникИНН) Или ЗначениеЗаполнено(
				ЗаписьКнигиДетальная.ПосредникКПП) Тогда

				СтруктуруРеквизитовПосредника = УчетНДС.ПолучитьСтруктуруРеквизитовКонтрагента(
					ЗаписьКнигиДетальная.ПосредникИНН, ЗаписьКнигиДетальная.ПосредникКПП);
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
            НСтр("ru='%1 на сумму %2 %3'"), ПредставлениеСФ.Представление, Формат(ВсегоПродаж, "ЧДЦ=2; ЧН="),
			ВалютаНаименование);
		СоставСтроки.Вставить("ДополнительныеСведения", ДополнительныеСведения);
...
Процедура УстановитьПараметрыЗапросаКнигиПродаж(Запрос, СтруктураПараметров)
	...
	Запрос.УстановитьПараметр("КонецПериода", КонецДня(СтруктураПараметров.КонецПериода));
	Запрос.УстановитьПараметр("Организация", СтруктураПараметров.СписокОрганизаций);
	Запрос.УстановитьПараметр("ОтбиратьПоКонтрагенту", Не СтруктураПараметров.СформироватьОтчетПоСтандартнойФорме
	...
	Запрос.УстановитьПараметр("ВалютаРегУчета", Константы.НациональнаяВалюта.Получить());
	Запрос.УстановитьПараметр("НаименованиеВалютыРубль", НСтр("ru='руб.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	Запрос.УстановитьПараметр("НачалоПримененияПостановления1137",
		УчетНДСПереопределяемый.ПолучитьДатуНачалаДействияПостановления1137());

	ЗаполнениеДокументовОтчетности = СтруктураПараметров.ЗаполнениеДокумента
	...
	Запрос.УстановитьПараметр("МассивСтавокНДС", Перечисления.СтавкиНДС.СтавкиПоОперациямОтражаемымВКнигеПродаж(
		СтруктураПараметров.ВерсияПостановленияНДС1137));
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
        ВидыЦенностей_СобственныеСФ, Перечисления.ВидыЦенностей.МассивВидовЦенностиНалоговыйАгент(Ложь));
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
        ВидыЦенностей_СобственныеСФ, Перечисления.ВидыЦенностей.МассивВидовЦенностиНалоговыйАгентРеализацияТоваров());
	...
	Запрос.УстановитьПараметр("ПКО_РозничнаяВыручка", Перечисления.ВидыОперацийПоступлениеВКассу.РозничнаяВыручка);
	Запрос.УстановитьПараметр("ОПК_РозничнаяВыручка", Перечисления.ВидыОперацийЭквайринга.ПоступлениеОплатыОтПокупателя);

	Запрос.УстановитьПараметр("ТекстРозничнаяПродажа", НСтр("ru = 'Розничная продажа'",
		ОбщегоНазначения.КодОсновногоЯзыка()));

	Запрос.УстановитьПараметр("ТипыДокументовРучныхОперацийНДС",
		УчетНДСПереопределяемый.ТипыДокументовРучныхОперацийНДС());
...
Функция ТекстЗапросаЗаписиРегистраНДСПродажи(СтруктураПараметров)
	...
	|                   ИЛИ НДСЗаписиКнигиПродаж.Регистратор ССЫЛКА Документ.КорректировкаПоступления
	|                   ИЛИ НДСЗаписиКнигиПродаж.Регистратор ССЫЛКА Документ.СчетФактура)
	|           ТОГДА ИСТИНА
	|       ИНАЧЕ ЛОЖЬ
	...
	ДополнительноеУсловиеЗапроса = ДополнительноеУсловиеЗапроса(
        СтруктураПараметров, СтруктураПараметров.ЗаписьДополнительногоЛиста, "НДСЗаписиКнигиПродаж");

	ТекстЗапроса = СтрЗаменить(
        ТекстЗапроса, "&ДополнительноеУсловие", ДополнительноеУсловиеЗапроса);

	Если Не СтруктураПараметров.СформироватьОтчетПоСтандартнойФорме И СтруктураПараметров.ОтбиратьПоКонтрагенту Тогда

		ТекстЗапросаДляОтбора =
		"(НДСЗаписиКнигиПродаж.Покупатель = &КонтрагентДляОтбора
		|               ИЛИ НДСЗаписиКнигиПродаж.Покупатель.ГоловнойКонтрагент = &КонтрагентДляОтбора)";

		Если ОбщегоНазначения.ОбъектЯвляетсяГруппой(СтруктураПараметров.КонтрагентДляОтбора) Тогда
			ТекстЗапросаДляОтбора = СтрЗаменить(ТекстЗапросаДляОтбора, " = &КонтрагентДляОтбора",
				" В ИЕРАРХИИ(&КонтрагентДляОтбора)");
		КонецЕсли;
	...
	Если СтруктураПараметров.ЗаписьДополнительногоЛиста Тогда
		Если СтруктураПараметров.ЗаполнениеДокумента И Не СтруктураПараметров.ФорматПоПостановлению735
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И НЕ НДСЗаписиКнигиПродаж.ЗаписьДополнительногоЛиста", "И НДСЗаписиКнигиПродаж.ЗаписьДополнительногоЛиста 
		...
		ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента И СтруктураПараметров.ФорматПоПостановлению735
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Период МЕЖДУ &НачалоПериода И &КонецПериода",
	...
	Если СтруктураПараметров.ВычетыНалоговогоАгентаОтражаютсяВКнигеПродаж Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "{ТекстДляЗамены}", ТекстЗапросаОбъединениеСВычетамиНеплательщикаНДС());
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "{ТекстДляЗамены}", "");
	КонецЕсли;
...
Функция ТекстЗапросаПодготовленныеЗаписиКнигиПродаж(СтруктураПараметров)
	...
	|        И НЕ ЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.СчетФактура
	...
	|        КОГДА ЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.СчетФактура
	|            ТОГДА ВЫБОР
	|                КОГДА ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК Документ.СчетФактура).Исправление
	|                ИЛИ ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК
	|                    Документ.СчетФактура).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Корректировка)
	|                    ТОГДА ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК Документ.СчетФактура).НомерИсходногоДокумента
	|                ИНАЧЕ ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК Документ.СчетФактура).Номер
	|            КОНЕЦ
	...
	|                ИЛИ ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК
	|                    Документ.СчетФактура).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Корректировка)
	|                    ТОГДА ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК Документ.СчетФактура).ДатаИсходногоДокумента
	|                ИНАЧЕ ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК Документ.СчетФактура).Дата
	|            КОНЕЦ
	|        КОГДА ЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.СчетФактураПолученный
	...
	|        ИЛИ ЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ОтчетКомиссионера
	...
	|        ИЛИ ЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ПоступлениеВКассу
	|        И ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК Документ.ПоступлениеВКассу).ВидОперации = &ПКО_РозничнаяВыручка
	|        ИЛИ ЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ОперацияПоПлатежнымКартам
	|        И ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК
	|            Документ.ОперацияПоПлатежнымКартам).ВидОперации = &ОПК_РозничнаяВыручка
	|            ТОГДА ИСТИНА
	|        ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаКнигаПродаж(СтруктураПараметров)
	...
	Если СтруктураПараметров.ГруппироватьПоКонтрагентам И Не СтруктураПараметров.СформироватьОтчетПоСтандартнойФорме Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПО
												 |	ОБЩИЕ,", "ПО
															   |	ОБЩИЕ, Контрагент, ");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "УПОРЯДОЧИТЬ ПО", "УПОРЯДОЧИТЬ ПО Покупатель, ");
	КонецЕсли;

	Если СтруктураПараметров.ВерсияПостановленияНДС1137 < 3 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "МАКСИМУМ(ВсегоПродаж)", "СУММА(ВсегоПродаж)");
	КонецЕсли;
...
Функция ПолучитьДополнительнуюИнформациюПоСчетуФактуре(СтруктураПараметров)
	...
	УчетНДСПереопределяемый.ДополнитьПродажиИнформациейОНомерахДекларацийНаТовары(ДополнительнаяИнформация,
		СтруктураПараметров, Запрос);
