...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	СведенияОрганизации = ПечатьДокументовУНФ.СведенияОЮрФизЛице(
		ПараметрыОтчета.Организация, КонецПериодаОтчета);
	НаименованиеОрганизацииДляПечатныхФорм = ПечатьДокументовУНФ.ОписаниеОрганизации(
		СведенияОрганизации, "НаименованиеДляПечати,");
	ИННОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОрганизации, "ИНН", Ложь);
	КППОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(СведенияОрганизации, "КПП", Ложь);
		...
		Если ГруппировкаПоКонтрагенту.Количество() > 0 Тогда
			...
			Пока ГруппировкаПоКонтрагенту.Следующий() Цикл
				Если УчетНДС.ВыводитьСтроку(СтруктураПараметров, ГруппировкаПоКонтрагенту, Истина) Тогда
					...
					ГруппировкаПоИсходнымИИсправительнымЗаписям = ГруппировкаПоКонтрагенту.Выбрать(
						ОбходРезультатаЗапроса.ПоГруппировкам);
					Пока ГруппировкаПоИсходнымИИсправительнымЗаписям.Следующий() Цикл
						ГруппировкаПоСчетамФактурам = ГруппировкаПоИсходнымИИсправительнымЗаписям.Выбрать(
							ОбходРезультатаЗапроса.ПоГруппировкам);
						Пока ГруппировкаПоСчетамФактурам.Следующий() Цикл

							ГруппировкаПоРегистратору = ГруппировкаПоСчетамФактурам.Выбрать(
								ОбходРезультатаЗапроса.ПоГруппировкам);
							Пока ГруппировкаПоРегистратору.Следующий() Цикл
								ГруппировкаПоПризнакуСторно = ГруппировкаПоРегистратору.Выбрать(
									ОбходРезультатаЗапроса.ПоГруппировкам);
									...
									Пока СчетаФактурыЗаписи.Следующий() Цикл
										Если УчетНДС.ВыводитьСтроку(СтруктураПараметров, СчетаФактурыЗаписи) Тогда
											...
											Если ВерсияПостановленияНДС1137 >= 3 Тогда
												...
												СуммаНДСИтоговая                             = СуммаНДСИтоговая
													- СчетаФактурыЗаписи.СуммаНДС;
												СуммаНДСРазницаУменьшениеИтоговая            = СуммаНДСРазницаУменьшениеИтоговая
													- СчетаФактурыЗаписи.СуммаНДСРазницаУменьшение;
												СуммаНДСРазницаУвеличениеИтоговая            = СуммаНДСРазницаУвеличениеИтоговая
													- СчетаФактурыЗаписи.СуммаНДСРазницаУвеличение;
											КонецЕсли;

											Если КоличествоСгруппированных > 1 И ВерсияПостановленияНДС1137 >= 3
											...
											Иначе
												...
												ЕстьПодстроки = Ложь;
												Если ВерсияПостановленияНДС1137 >= 3 И Не СчетаФактурыЗаписи.ИсправлениеСобственнойОшибки
													И Не СчетаФактурыЗаписи.СводныйКорректировочный
													И Не СчетаФактурыЗаписи.СводныйКомиссионный И (СуммаНДСИтоговая <> 0
													Или СуммаНДСРазницаУменьшениеИтоговая <> 0
													Или СуммаНДСРазницаУвеличениеИтоговая <> 0) Тогда
											...
											ЗаполнитьСтрокуЖурналаУчетаСчетовФактур(
                                                Часть1Строка.Параметры, СчетаФактурыЗаписи, СтруктураПараметров, ,
												НомерПП, ЕстьПодстроки);
											...
											УчетНДС.ДобавитьКонтрагентаНаПроверку(СтруктураПараметров,
												СчетаФактурыЗаписи, ОбластьТабличногоДокумента);

											ЗаполнитьПодстрочникРНПТ(
                                                СчетаФактурыЗаписи, ПараметрыСтрокиРНПТЧасть1, СтруктураПараметров,
												Часть1СтрокаРНПТ, ТабличныйДокумент, "ОбластьЗаписьЧасть1",
												НомерСтрокиВГруппировке = КоличествоСгруппированных);

											Если ВыводитьОстатокПоСчетуФактуре Тогда
												СтрокаОстаток = Макет.ПолучитьОбласть("Часть1Строка_Многострочная3");
												СтрокаОстаток.Параметры.Валюта = СокрП(
													СчетаФактурыЗаписи.НаименованиеВалюты) + ", "
													+ СчетаФактурыЗаписи.КодВалюты;
												СтрокаОстаток.Параметры.СуммаДокумента = ?(
													ГруппировкаПоПризнакуСторно.СуммаДокумента = 0, "",
													ГруппировкаПоПризнакуСторно.СуммаДокумента);
												СтрокаОстаток.Параметры.СуммаНДС = ?(СуммаНДСИтоговая = 0, "",
													СуммаНДСИтоговая);
												СтрокаОстаток.Параметры.СуммаДокументаРазницаУменьшение = ?(
													ГруппировкаПоПризнакуСторно.СуммаДокументаРазницаУменьшение = 0,
													"", ГруппировкаПоПризнакуСторно.СуммаДокументаРазницаУменьшение);
												СтрокаОстаток.Параметры.СуммаДокументаРазницаУвеличение = ?(
													ГруппировкаПоПризнакуСторно.СуммаДокументаРазницаУвеличение = 0,
													"", ГруппировкаПоПризнакуСторно.СуммаДокументаРазницаУвеличение);
												СтрокаОстаток.Параметры.СуммаНДСРазницаУменьшение = ?(
													СуммаНДСРазницаУменьшениеИтоговая = 0, "",
													СуммаНДСРазницаУменьшениеИтоговая);
												СтрокаОстаток.Параметры.СуммаНДСРазницаУвеличение = ?(
													СуммаНДСРазницаУвеличениеИтоговая = 0, "",
													СуммаНДСРазницаУвеличениеИтоговая);

												ВывестиГраницуСтроки(СтруктураПараметров, СтрокаОстаток,
													"Часть1Строка_Многострочная3", НомерСтрокиВГруппировке
													= КоличествоСгруппированных);
						...
						Пока СчетаФактурыЗаписи.Следующий() Цикл
							Если УчетНДС.ВыводитьСтроку(СтруктураПараметров, СчетаФактурыЗаписи) Тогда
								...
								Иначе
									...
									ЕстьПодстроки = Ложь;
									Если ВерсияПостановленияНДС1137 >= 3 И Не СчетаФактурыЗаписи.ИсправлениеСобственнойОшибки
										И Не СчетаФактурыЗаписи.СводныйКорректировочный И Не СчетаФактурыЗаписи.СводныйКомиссионный
										И (СуммаНДСИтоговая <> 0 Или СуммаНДСРазницаУменьшениеИтоговая <> 0
										Или СуммаНДСРазницаУвеличениеИтоговая <> 0) Тогда
                                        // Если пока только один СФ от комитента
										ВыводитьОстатокПоСчетуФактуре = Истина;
										Часть1Строка = Часть1Строка_Многострочная1;
								...
								ЗаполнитьСтрокуЖурналаУчетаСчетовФактур(
									...
									ЕстьПодстроки);
								Часть1Строка.Область(1, , Часть1Строка.ВысотаТаблицы).Имя = "Строка_" + Формат(НомерПП
									- 1, "ЧГ=");
								...
								УчетНДС.ДобавитьКонтрагентаНаПроверку(СтруктураПараметров, СчетаФактурыЗаписи,
									ОбластьТабличногоДокумента);

								ЗаполнитьПодстрочникРНПТ(
                                    СчетаФактурыЗаписи, ПараметрыСтрокиРНПТЧасть1, СтруктураПараметров, Часть1СтрокаРНПТ,
									ТабличныйДокумент, "ОбластьЗаписьЧасть1", НомерСтрокиВГруппировке
									= КоличествоСгруппированных);
								...
								Если ВыводитьОстатокПоСчетуФактуре Тогда
									...
									Если СтруктураПараметров.ЗаполнениеДекларации Тогда
										...
										СтруктураСвПосрДеят = Новый Структура("НомСчФОтПрод,ДатаСчФОтПрод,ОКВ,СтоимТовСчФВс,СумНДССчФ,РазСтКСчФУм,РазСтКСчФУв,РазНДСКСчФУм,РазНДСКСчФУв,СвПрод");
										...
										СтруктураСвПосрДеят.СтоимТовСчФВс = ?(
											ГруппировкаПоПризнакуСторно.СуммаДокумента = 0, "",
											ГруппировкаПоПризнакуСторно.СуммаДокумента);
										СтруктураСвПосрДеят.СумНДССчФ     = ?(СуммаНДСИтоговая = 0, "",
											СуммаНДСИтоговая);
										...
										СтруктураСвПосрДеят.РазСтКСчФУв   = ?(
											ГруппировкаПоПризнакуСторно.СуммаДокументаРазницаУвеличение = 0, "",
											ГруппировкаПоПризнакуСторно.СуммаДокументаРазницаУвеличение);
										СтруктураСвПосрДеят.РазНДСКСчФУм  = ?(СуммаНДСРазницаУменьшениеИтоговая = 0,
											"", СуммаНДСРазницаУменьшениеИтоговая);
										СтруктураСвПосрДеят.РазНДСКСчФУв  = ?(СуммаНДСРазницаУвеличениеИтоговая = 0,
											"", СуммаНДСРазницаУвеличениеИтоговая);

										СтрокаРаздела = СтруктураПараметров.ТаблицаРаздел10[СчетчикСтрокТекущегоСегмента
											- 1];
									...
									СтрокаОстаток = Макет.ПолучитьОбласть("Часть1Строка_Многострочная3");
									СтрокаОстаток.Параметры.Валюта = СокрП(СчетаФактурыЗаписи.НаименованиеВалюты)
										+ ", " + СчетаФактурыЗаписи.КодВалюты;
									СтрокаОстаток.Параметры.СуммаДокумента = ?(
										ГруппировкаПоПризнакуСторно.СуммаДокумента = 0, "",
										ГруппировкаПоПризнакуСторно.СуммаДокумента);
									...
									СтрокаОстаток.Параметры.СуммаДокументаРазницаУвеличение = ?(
										...
										ГруппировкаПоПризнакуСторно.СуммаДокументаРазницаУвеличение);
									СтрокаОстаток.Параметры.СуммаНДСРазницаУменьшение = ?(
										СуммаНДСРазницаУменьшениеИтоговая = 0, "", СуммаНДСРазницаУменьшениеИтоговая);
									СтрокаОстаток.Параметры.СуммаНДСРазницаУвеличение = ?(
										СуммаНДСРазницаУвеличениеИтоговая = 0, "", СуммаНДСРазницаУвеличениеИтоговая);
								...
								Если СтруктураПараметров.ЗаполнениеДекларации И СчетчикСтрокТекущегоСегмента
									= ПараметрыОтчета.ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС Тогда
									...
									РегламентированнаяОтчетность.ЗаписатьСегментДанныхРазделаДекларацииНДС(
                                        ПараметрыОтчета.СсылкаНаДекларацию, "Раздел10", ПерваяСтрокаСегмента,
...
// Конец СтандартнаяФорма 
	...
	Если СтруктураПараметров.ЗаполнениеДекларации И СчетчикСтрокТекущегоСегмента <> 0 И СчетчикСтрокТекущегоСегмента
		<> ПараметрыОтчета.ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС Тогда
		...
		РегламентированнаяОтчетность.ЗаписатьСегментДанныхРазделаДекларацииНДС(
            ПараметрыОтчета.СсылкаНаДекларацию, "Раздел10", ПерваяСтрокаСегмента, СтруктураПараметров.ТаблицаРаздел10);

		РегламентированнаяОтчетность.ЗаписатьСегментТабличногоДокументаРазделаДекларацииНДС(
            ПараметрыОтчета.СсылкаНаДекларацию, "Раздел10", ПерваяСтрокаСегмента, ТабличныйДокумент);
...
#КонецОбласти
	...
	ОтветственныеЛица = УправлениеНебольшойФирмойЭлектронныеДокументыСервер.ОтветственныеЛицаОрганизационнойЕдиницы(СтруктураПараметров.Организация);
...
Функция ПолучитьЗаписиЖурналаСчетовФактур(СтруктураПараметров) Экспорт
	...
	Если ОтбиратьПоКонтрагенту Тогда
		Если ПолучитьФункциональнуюОпцию("ПередачаТоваровНаКомиссию")
			Или ПолучитьФункциональнуюОпцию("ПриемТоваровНаКомиссию") Тогда
...
Функция ТекстЗапросаЖурналСчетовФактур981()
	...
	|	И ЖурналУчетаСчетовФактур.СчетФактура ССЫЛКА Документ.СчетФактура
	...
	|	ВЫБОР
	|		КОГДА ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура ССЫЛКА Документ.СчетФактура
	|			ТОГДА ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура.СводныйКорректировочный
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК СводныйКорректировочный,
	|	ВЫБОР
	|		КОГДА ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура ССЫЛКА Документ.СчетФактура
	|			ТОГДА ВЫРАЗИТЬ(ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура КАК Документ.СчетФактура).СводныйКомиссионный
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура КАК Документ.СчетФактура).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Корректировка)
	|				ИЛИ ВЫРАЗИТЬ(ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура КАК Документ.СчетФактураПолученный).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактураПолученный.Корректировка)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК КорректировочныйСчетФактура,
	|	ВЫРАЗИТЬ(ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура КАК Документ.СчетФактура).Номер КАК СчетФактураВыданныйНомер,
...
Функция ТекстЗапросаЖурналСчетовФактур1137()
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ВЫРАЗИТЬ(ЖурналУчетаСчетовФактурСрезПоследних.СчетФактура КАК Документ.СчетФактура).Номер,
