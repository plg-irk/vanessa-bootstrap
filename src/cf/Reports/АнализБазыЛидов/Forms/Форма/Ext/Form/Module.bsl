...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолучитьЗначениеОтбораПоПериоду();
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ОтборПериодРасшифровка) Тогда
		ОтборПериодЗначение = ОтборПериодРасшифровка;
	КонецЕсли;	
...
&НаКлиенте
Процедура ОтборВидНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ДанныеВыбора = ИнтерактивныйСписокВыбораВидаЛида;
	
КонецПроцедуры

&НаКлиенте
Процедура ОтборВидОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ОтборВид = ВыбранноеЗначение;
	
КонецПроцедуры

&НаКлиенте
Процедура ОтборВидАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
Процедура МенеджерыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("КлючУникальности",							КлючНазначенияИспользования);
...
Процедура ДиаграммаНовыеОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, ЖирныйШрифт()));
...
&НаКлиентеНаСервереБезКонтекста
Функция МаксимумСерийНеЗадано()
	
	//@skip-check bsl-legacy-check-dynamic-feature-access
	Возврат МаксимумСерий.НеЗадано;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ВидПодписейКДиаграммеСерияЗначение()
	
	//@skip-check bsl-legacy-check-dynamic-feature-access
	Возврат ВидПодписейКДиаграмме.СерияЗначение;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ВидПодписейКДиаграммеЗначение()
	
	//@skip-check bsl-legacy-check-dynamic-feature-access
	Возврат ВидПодписейКДиаграмме.Значение;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ВидПодписейКДиаграммеПроцент()
	
	//@skip-check bsl-legacy-check-dynamic-feature-access
	Возврат ВидПодписейКДиаграмме.Процент;
...
Процедура ОбновитьДиаграммуНовые()
	...
	РезультатЛида = РезультатЛидаПоОтбору();
	Если РезультатЛида <> Неопределено Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Лиды.ВариантЗавершения = &ОтборРезультат";
		Запрос.УстановитьПараметр("ОтборРезультат", РезультатЛида);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборПокупатель) Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И СвязиКонтрагентЛид.Контрагент = &ОтборПокупатель";
		Запрос.УстановитьПараметр("ОтборПокупатель", ОтборПокупатель);
	Иначе
	...
	ВидЛида = ВидЛидаПоОтбору();
	Если ЗначениеЗаполнено(ВидЛида) Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Лиды.Вид = &ОтборВид";
		Запрос.УстановитьПараметр("ОтборВид", ВидЛида);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОтборПериодЗначение.ДатаНачала) И ЗначениеЗаполнено(ОтборПериодЗначение.ДатаОкончания) Тогда
		Элементы.ЗаголовокНовые.Заголовок = НСтр("ru = 'Новых за:'") +Символы.НПП + Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yy") + " - " + Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yy");
	Иначе
	...
	ДиаграммаНовые.МаксимумСерий = МаксимумСерийНеЗадано();
	ДиаграммаНовые.ВидПодписей = ВидПодписейКДиаграммеСерияЗначение();
	...
	Подсказка = НСтр("ru = 'Создано новых лидов за период:'") + Символы.НПП + Новых;
	...
	Подсказка = НСтр("ru = 'Существующих лидов:'") + Символы.НПП + Существующих;
...
Процедура ОбновитьДиаграммуНовыеПоПериодам()
	...
	Если Не ЗначениеЗаполнено(ОтборПокупатель) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиКонтрагентЛид КАК СвязиКонтрагентЛид
		|		ПО Лиды.Ссылка = СвязиКонтрагентЛид.Лид", "");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОтборПокупатель) Тогда
		ОтборЗапроса.Добавить("СвязиКонтрагентЛид.Контрагент = &ОтборПокупатель");
		Запрос.УстановитьПараметр("ОтборПокупатель", ОтборПокупатель);
	КонецЕсли;
	
	ВидЛида = ВидЛидаПоОтбору();
	Если ЗначениеЗаполнено(ВидЛида) Тогда
		ОтборЗапроса.Добавить("Лиды.Вид = &ОтборВид");
		Запрос.УстановитьПараметр("ОтборВид", ВидЛида);
	КонецЕсли;
	...
	ДиаграммаНовыеПоПериодам.МаксимумСерий = МаксимумСерийНеЗадано();
	ДиаграммаНовыеПоПериодам.ВидПодписей = ВидПодписейКДиаграммеЗначение();
	//@skip-check new-font
	...
	Пока ВыборкаДатаСоздания.Следующий() Цикл
		...
		Подсказка = НСтр("ru = 'Создано'") + Символы.НПП + ВыборкаДатаСоздания.Новый + Символы.НПП + НСтр("ru = 'лидов'") + Символы.НПП + Текст;
...
Процедура ОбновитьДиаграммуПоМенеджерам()
	...
	ДиаграммаПоМенеджерам.МаксимумСерий = МаксимумСерийНеЗадано();
	ДиаграммаПоМенеджерам.ВидПодписей = ВидПодписейКДиаграммеПроцент();
	//@skip-check new-font
	...
	МассивЦветов = ДиаграммаПоМенеджерам.ОписаниеПалитрыЦветов.ПолучитьПалитру();
	
	//@skip-check new-font
	ШрифтArial = Новый Шрифт(ШрифтыСтиля.ОбычныйШрифтТекста, "@Arial Unicode MS");
	...
	Пока ВыборкаОтветственные.Следующий() Цикл
		...
		Если ПорядковыйНомерСерии < МаксимумСерийДиаграммы Тогда
			...
			Подсказка = ПредставлениеМенеджера + НСтр("ru = ', лидов:'") + Символы.НПП + ВыборкаОтветственные.Лиды;
			...
			ЛегендаДиаграммы = Новый ФорматированнаяСтрока("█", ШрифтArial, МассивЦветов[ПорядковыйНомерСерии]);
		...
		Иначе
			...
			Подсказка = ПредставлениеМенеджера + НСтр("ru = ', лидов:'") + Символы.НПП + (СводноеЗначение.Значение + ВыборкаОтветственные.Лиды);
			...
			ЛегендаДиаграммы = Новый ФорматированнаяСтрока("█", ШрифтArial, Серия.Цвет);
			...
			ТекстПодробно = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Ответственный), ВыборкаОтветственные.ОтветственныйПредставление, НСтр("ru = '<Менеджер не указан>'"))
				+ НСтр("ru = ', лидов:'") + Символы.НПП + ВыборкаОтветственные.Лиды;
...
&НаСервере
Функция РезультатЛидаПоОтбору()
	
	Если ОтборРезультат = "ВРаботе" Тогда
		Возврат Перечисления.ВариантЗавершенияРаботыСЛидом.ПустаяСсылка();
	ИначеЕсли ОтборРезультат = "КачественныйЛид" Тогда
		Возврат Перечисления.ВариантЗавершенияРаботыСЛидом.ПереведенВПокупателя;
	ИначеЕсли ОтборРезультат = "НекачественныйЛид" Тогда
		Возврат Перечисления.ВариантЗавершенияРаботыСЛидом.НекачественныйЛид;
	Иначе
...
&НаСервере
Функция ВидЛидаПоОтбору()
	
	Если ОтборВид = "Первичное" Тогда
		Возврат Перечисления.ВидыЛидов.ПервичноеОбращение;
	ИначеЕсли ОтборВид = "Повторное" Тогда
		Возврат Перечисления.ВидыЛидов.ПовторноеОбращение;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ЗаполнитьСпискиВыбора()
	...
	ПредставлениеЗначения = Новый ФорматированнаяСтрока(БиблиотекаКартинок.ПервичныйЛид, НСтр("ru = ' Первичное'"));
	ИнтерактивныйСписокВыбораВидаЛида.Добавить("Первичное", ПредставлениеЗначения);
	ПредставлениеЗначения = Новый ФорматированнаяСтрока(БиблиотекаКартинок.ПовторныйЛид, НСтр("ru = ' Повторное'"));
	ИнтерактивныйСписокВыбораВидаЛида.Добавить("Повторное", ПредставлениеЗначения);
	
КонецПроцедуры

&НаСервере
Процедура ПолучитьЗначениеОтбораПоПериоду()
	...
	Если Параметры.Свойство("Отбор") И ТипЗнч(Параметры.Отбор) = Тип("Структура") Тогда
		Если Параметры.Отбор.Свойство("ПериодРасшифровки") Тогда	
			ОтборПериодРасшифровка = Параметры.Отбор.ПериодРасшифровки; 
		КонецЕсли;
	КонецЕсли;
