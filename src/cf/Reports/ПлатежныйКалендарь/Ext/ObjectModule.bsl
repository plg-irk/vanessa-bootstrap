...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Для Индекс = -ДокументРезультат.ВысотаТаблицы По -1 Цикл
		ОбластьСтроки = ДокументРезультат.Область(-Индекс, , -Индекс);
		Если СтрНайти(ОбластьСтроки.Текст, НСтр("ru = 'Не выбран'")) > 0 Или ОбластьСтроки.Текст = "" Тогда
...
Процедура ДополнитьНаборыПриОтсутствииДвижений(ВнешниеНаборыДанных)
	...
	ПараметрыОтбора.Вставить("БанковскийСчетКасса");
	ПараметрыОтбора.Вставить("Валюта");
	ПараметрыОтбора.Вставить("Период");
	ПараметрыОтбора.Вставить("ТипДенежныхСредств");
	...
	Для каждого АналитикаПлатежа Из ВнешниеНаборыДанных.ТаблицаОстатков Цикл
		...
		Для каждого Период Из ПериодыПланирования Цикл
			...
			ЗаполнитьЗначенияСвойств(ПараметрыОтбора, АналитикаПлатежа);
			ПараметрыОтбора.Период = НачалоДня(Период.Значение);
			ПараметрыОтбора.ТипДенежныхСредств = ТипДенежныхСредств;
			
			СтрокиТаблицыПлатежей = ВнешниеНаборыДанных.ТаблицаПлатежей.НайтиСтроки(ПараметрыОтбора);
			
			Если СтрокиТаблицыПлатежей.Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;	
