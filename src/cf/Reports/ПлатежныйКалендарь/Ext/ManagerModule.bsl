...
Функция ПолучитьТекстЗапросаПлатежныйКалендарь(Объект, ДанныеМеток) Экспорт
	...
	|ПОМЕСТИТЬ ДокументыДляИсходныхДанных
	|ИЗ
	...
	|			СчетНаОплату В
	|				(ВЫБРАТЬ
	|					ДокументыДляИсходныхДанных.СчетНаОплату КАК СчетНаОплату
	|				ИЗ
	|					ДокументыДляИсходныхДанных КАК ДокументыДляИсходныхДанных)) КАК ПлатежныйКалендарь
	...
	|
	|ВЫБРАТЬ
	|	РегулярныеПлатежиГрафик.Период,
	|	РегулярныеПлатежиГрафик.Период,
	|	РегулярныеПлатежиГрафик.ИсходныйДокумент,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РегулярныеПлатежиГрафик.ИсходныйДокумент) = ТИП(Документ.ПлатежноеПоручение)
	|			ТОГДА ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.ПлатежноеПоручение).Организация
	|		ИНАЧЕ ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.РасходДСПлан).Организация
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РегулярныеПлатежиГрафик.ИсходныйДокумент) = ТИП(Документ.ПлатежноеПоручение)
	|			ТОГДА ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.ПлатежноеПоручение).ВалютаДокумента
	|		ИНАЧЕ ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.РасходДСПлан).ВалютаДокумента
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РегулярныеПлатежиГрафик.ИсходныйДокумент) = ТИП(Документ.ПлатежноеПоручение)
	|			ТОГДА ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.ПлатежноеПоручение).Статья
	|		ИНАЧЕ ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.РасходДСПлан).СтатьяДвиженияДенежныхСредств
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РегулярныеПлатежиГрафик.ИсходныйДокумент) = ТИП(Документ.ПлатежноеПоручение)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыДенежныхСредств.БезНаличные)
	|		ИНАЧЕ ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.РасходДСПлан).ТипДенежныхСредств
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РегулярныеПлатежиГрафик.ИсходныйДокумент) = ТИП(Документ.ПлатежноеПоручение)
	|			ТОГДА ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.ПлатежноеПоручение).БанковскийСчет
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.РасходДСПлан).ТипДенежныхСредств = ЗНАЧЕНИЕ(Перечисление.ТипыДенежныхСредств.Наличные)
	|					ТОГДА ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.РасходДСПлан).Касса
	|				ИНАЧЕ ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент КАК Документ.РасходДСПлан).БанковскийСчет
	|			КОНЕЦ
	|	КОНЕЦ,
	|	РегулярныеПлатежиГрафик.ИсходныйДокумент,
	|	ЗНАЧЕНИЕ(Перечисление.СтатусыУтвержденияПлатежей.Утвержден),
	|	-РегулярныеПлатежиГрафик.ИсходныйДокумент.СуммаДокумента,
	|	0,
	|	-ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент.СуммаДокумента * КурсыВалютДокумента.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютДокумента.Кратность) КАК ЧИСЛО(15, 2)),
	|	0,
	|	-ВЫРАЗИТЬ(РегулярныеПлатежиГрафик.ИсходныйДокумент.СуммаДокумента * КурсыВалютДокумента.Курс / КурсыВалютДокумента.Кратность КАК ЧИСЛО(15, 2)),
	|	0
	|ИЗ
	...
	|		ПО (КурсыВалютУчета.Валюта = &ВалютаУчета)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютДокумента
	|		ПО (КурсыВалютДокумента.Валюта = РегулярныеПлатежиГрафик.ИсходныйДокумент.ВалютаДокумента)
	|ГДЕ
	|	РегулярныеПлатежиГрафик.Период <= &КонецПериода
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор,
	|	СчетНаОплату
	...
	|	ПлатежныйКалендарь.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ПлатежныйКалендарь.СчетНаОплату) = ТИП(Документ.РасходДСПлан)
	|			ТОГДА ЕСТЬNULL(ПлатежныйКалендарь.СчетНаОплату.ВидОперации, ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.ПустаяСсылка))
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходДСПлан.ПустаяСсылка)
	...
	|			КОНЕЦ)
	|ГДЕ                                                                    
	|	(&ОтображатьБолееРанниеПлатежи                                      
	|			ИЛИ ПлатежныйКалендарь.ПериодДень >= &НачалоПериодаОтбор)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ОстаткиПланируемыхПлатежей.ВидОперации КАК ВидОперации,
	...
	|	И ОстаткиПланируемыхПлатежей.Организация = &ОтборыПлатежныйКалендарь
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА &ОтображатьБолееРанниеПлатежи
	...
	|		ИНАЧЕ НАЧАЛОПЕРИОДА(ОстаткиПланируемыхПлатежей.Период, НЕДЕЛЯ)
	|	КОНЕЦ,
	|	ОстаткиПланируемыхПлатежей.ВидОперации,
	...
	|	Прогноз.ВидОперации КАК ВидОперации,
	...
	СписокОтборыПлатежныйКалендарь.Добавить("ВидОперации");
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И ОстаткиПланируемыхПлатежей.Организация = &ОтборыПлатежныйКалендарь",
		ОтборыПлатежныйКалендарь);
