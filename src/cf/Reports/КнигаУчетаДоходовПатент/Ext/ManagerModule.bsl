...
//   * НачалоПериода - Дата,
//   * КонецПериода - Дата,
//   * СписокСформированныхЛистов - СписокЗначений из ТабличныйДокумент
//  АдресХранилища - УникальныйИдентификатор, Строка - адрес во временном хранилище, куда будет
//  		помещён СписокЗначений из ТабличныхДокументов - результат формирования Книги учета доходов по патенту.
//
...
Процедура СформироватьТитульныйЛист(ПараметрыОтчета)
	...
	ПериодОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'на %1 год'"),
												Формат(Год(ПараметрыОтчета.КонецПериода), "ЧГ="));
	НомерПатента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыОтчета.Патент, "НомерПатента");
	ПериодПатента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'с %1 г. по %2 г.'"),
	...
	ДанныеТитульногоЛиста.Вставить("НаПериод", ПериодОтчета);
	...
	ДанныеТитульногоЛиста.Вставить("НомерПатента", НомерПатента);
	ДанныеТитульногоЛиста.Вставить("СрокПатента", ПериодПатента);
