...
Функция ВременныеТаблицыВоронкиПродажПоЗаказам(НастройкиКД, ОбъектВоронки) Экспорт 
	...
	Если ОбъектВоронки = "Лид" Тогда
		...
		|				ИЛИ ИсторияСостоянийЗаказов.Заказ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаявкаНаПродажу)
...
Функция СтрокаОтбораПоЛидам(ЭлементОтбора, ИмяПараметра)
	...
	ИначеЕсли КомпонентыПути.Найти("ВидЛида") <> Неопределено И КомпонентыПути.Найти("Заказ") = Неопределено Тогда
		
		Результат = "ИсторияСостоянийЛидов.Лид.Вид";
