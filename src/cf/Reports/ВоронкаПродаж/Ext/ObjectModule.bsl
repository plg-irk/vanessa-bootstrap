...
Процедура ЗаполнитьДетализациюГруппировки(ОбластьГруппировка, ВыборкаВоронкаПродаж, КоличествоЗаказов = Неопределено, ТаблицаОтмен = Неопределено)
	
	Если ТаблицаОтмен <> Неопределено Тогда
		
		ЗаказыПоОперации = ТаблицаОтмен.НайтиСтроки(Новый Структура("ГруппировкаВидОперации", ВыборкаВоронкаПродаж.ГруппировкаВидОперации));
		РазличныеЗаказы = Новый Массив;
		СуммаПоГруппировке = 0;
		Для Каждого Заказ Из ЗаказыПоОперации Цикл
			Если Не ЗначениеЗаполнено(Заказ.ВидЗаказа) тогда
				Продолжить;
			КонецЕсли;
			
			Если РазличныеЗаказы.Найти(Заказ.Заказ) <> Неопределено Тогда
				СуммаПоГруппировке = СуммаПоГруппировке + Заказ.СуммаДокумента;
				Продолжить;
			КонецЕсли;
			РазличныеЗаказы.Добавить(Заказ.Заказ);
			СуммаПоГруппировке = СуммаПоГруппировке + Заказ.СуммаДокумента;
		КонецЦикла;
		
		ОбластьГруппировка.Параметры.ПотериТовары = РазличныеЗаказы.Количество();
		ОбластьГруппировка.Параметры.СуммаПотерьТовары =  Формат(СуммаПоГруппировке, "ЧЦ=15; ЧДЦ=2");
...
Функция ДанныеПоОтменам(Знач ТаблицаОтмен)
	
	Результат = Новый Соответствие;
	Заказы = Новый Массив;
	
	Для Каждого ТекСтр Из ТаблицаОтмен Цикл
		...
		ЗаказУжеВСписке = Заказы.Найти(ТекСтр.Заказ) <> Неопределено;
		
		Результат[ТекСтр.ПолеГруппировки][ТекСтр.ВидЗаказа].Состояния[ТекСтр.СостояниеОтмены].Потери = 
			Результат[ТекСтр.ПолеГруппировки][ТекСтр.ВидЗаказа].Состояния[ТекСтр.СостояниеОтмены].Потери + ?(ЗаказУжеВСписке,0,1);
		Результат[ТекСтр.ПолеГруппировки][ТекСтр.ВидЗаказа].Состояния[ТекСтр.СостояниеОтмены].СуммаПотерь = 
			Результат[ТекСтр.ПолеГруппировки][ТекСтр.ВидЗаказа].Состояния[ТекСтр.СостояниеОтмены].СуммаПотерь + ТекСтр.СуммаДокумента;
		
		Если НЕ ЗаказУжеВСписке Тогда
			Заказы.Добавить(ТекСтр.Заказ);
		КонецЕсли;
		...
		Результат[ТекСтр.ПолеГруппировки][ТекСтр.ВидЗаказа].Состояния[ТекСтр.СостояниеОтмены].Причины[ТекСтр.ПричинаОтмены] = 
			Результат[ТекСтр.ПолеГруппировки][ТекСтр.ВидЗаказа].Состояния[ТекСтр.СостояниеОтмены].Причины[ТекСтр.ПричинаОтмены] + 1;
...
Функция ЦветаГрадиентаПричин(Знач ТаблицаПотерь, ОбъектВоронки, ТаблицаОтмен = Неопределено)
...
Процедура ЗаполнитьПотери(Знач ОбластьСостояние, Знач ВыборкаВоронкаПродажСостояние, Знач ДанныеПоПотерям, 
	Знач КоличествоСостоянийПоВидуЗаказа, Знач ЦветаГрадиента, ТипОтмены = "Потери")
	...
	ТекущиеПотери = ДанныеПоПотерям[ВыборкаВоронкаПродажСостояние.ПолеГруппировки][ВыборкаВоронкаПродажСостояние.ВидЗаказа];
	ПотериПоВидуЗаказаИСостоянию = ТекущиеПотери.Состояния[ВыборкаВоронкаПродажСостояние.Состояние];
	...
	ТекПозиция = НачалоОтображенияГрадиентаПотерь(ТипОтмены);
	...
	Если ТипОтмены = "Потери" Тогда
		
		ОбластьСостояние.Параметры.Потери = СтрШаблон("%1 (%2%%)",
		ПотериПоВидуЗаказаИСостоянию.Потери,
		ПроцентПотерь);
		
		ОбластьСостояние.Параметры.СуммаПотерь = Формат(ПотериПоВидуЗаказаИСостоянию.СуммаПотерь, "ЧЦ=15; ЧДЦ=2");
		
	Иначе
		
		ОбластьСостояние.Параметры.ПотериТовары = СтрШаблон("%1 (%2%%)",
		ПотериПоВидуЗаказаИСостоянию.Потери,
		ПроцентПотерь);
		
		ОбластьСостояние.Параметры.СуммаПотерьТовары =  Формат(ПотериПоВидуЗаказаИСостоянию.СуммаПотерь, "ЧЦ=15; ЧДЦ=2");
...
Функция НачалоОтображенияГрадиентаПотерь(ТипОтмены = "Потери")
...
Функция ИдентификаторЭлементаРасшифровки(ДанныеРасшифровки, Выборка, ЭтоПотери, ЭтоОтмены = Ложь, ДанныеОтмен = Неопределено)
	
	Если ДанныеОтмен = Неопределено Тогда
		ДанныеОтмен = Новый Массив;
	КонецЕсли;
	...
	ЗначениеЭтоОтмены = ЗначенияПолей.Добавить();
	ЗначениеЭтоОтмены.Поле = "ДанныеОтмен";
	ЗначениеЭтоОтмены.Значение = ЭтоОтмены;
	
	ЗначениеДанныеОтмен = ЗначенияПолей.Добавить();
	ЗначениеДанныеОтмен.Поле = "ОтменыПоЗаказам";
	ЗначениеДанныеОтмен.Значение = ДанныеОтмен;
...
Процедура ВывестиВоронкуПоЗаказам(ДокументРезультат, МакетОтчета, ВариантВоронки, ДанныеРасшифровки, ОбъектВоронки, ПолеГруппировки = Неопределено)
	...
	ТаблицаОтмен  = РезультатыЗапросаВоронкаПродаж[3].Выгрузить();
	
	ДанныеПоПотерям = ДанныеПоПотерям(ТаблицаПотерь);
	ДанныеПоОтменам = ДанныеПоОтменам(ТаблицаОтмен);
	
	ДанныеРасшифровки.Настройки.ДополнительныеСвойства.Вставить("ЦветаГрадиентаЗаказы", ЦветаГрадиентаПричин(ТаблицаПотерь, "Заказ", ТаблицаОтмен));
	...
	Пока ВыборкаВоронкаПродажГруппировка.Следующий() Цикл
		...
		Пока ВыборкаВоронкаПродажВидОперации.Следующий() Цикл
			...
			ЗаполнитьДетализациюГруппировки(ОбластьЗаголовок, ВыборкаВоронкаПродажВидОперации, , ТаблицаОтмен);
			...
			Пока ВыборкаВоронкаПродажВидЗаказа.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(ПредставлениеВидЗаказа) Тогда
					...
					ДокументРезультат.Вывести(ОбластьОтступ);
				КонецЕсли;
				ВывестиДетализациюСостояние(ДокументРезультат, ВыборкаВоронкаПродажВидЗаказа, ДанныеПоПотерям, ДанныеРасшифровки, МакетОтчета, ЦветСостояния, ДанныеПоОтменам, ТаблицаОтмен);
				Если ЗначениеЗаполнено(ПредставлениеВидЗаказа) Тогда
...
Процедура ВывестиДетализациюСостояние(Знач ДокументРезультат, Знач ВыборкаВоронкаПродажВидЗаказа, Знач ДанныеПоПотерям, Знач ДанныеРасшифровки, Знач МакетОтчета, Знач ЦветСостояния, Знач ДанныеПоОтменам, Знач ТаблицаОтмен)
	...
	Пока ВыборкаВоронкаПродажПорядок.Следующий() Цикл
		...
		Пока ВыборкаВоронкаПродажСостояние.Следующий() Цикл
			...
			ЗаказыРасшифровки = Новый Массив;
			РасшифровкаОтмены = ТаблицаОтмен.НайтиСтроки(Новый Структура("ВидЗаказа, СостояниеОтмены, ПолеГруппировки", 
				ВыборкаВоронкаПродажСостояние.ВидЗаказа, ВыборкаВоронкаПродажСостояние.Состояние, ВыборкаВоронкаПродажСостояние.ПолеГруппировки));
				
			Для Каждого Заказ ИЗ РасшифровкаОтмены Цикл 
				ЗаказыРасшифровки.Добавить(Заказ.Заказ);
			КонецЦикла; 
			...
			ОбластьСостояние.Параметры.РасшифровкаПотериТовары = ИдентификаторЭлементаРасшифровки(ДанныеРасшифровки, ВыборкаВоронкаПродажСостояние, Ложь, Истина, ЗаказыРасшифровки);
			...
			ЗаполнитьПотери(ОбластьСостояние, ВыборкаВоронкаПродажСостояние, 
				ДанныеПоПотерям, КоличествоСостоянийПоВидуЗаказа, 
				ДанныеРасшифровки.Настройки.ДополнительныеСвойства.ЦветаГрадиентаЗаказы, "Потери");
			ЗаполнитьПотери(ОбластьСостояние, ВыборкаВоронкаПродажСостояние, 
				ДанныеПоОтменам, КоличествоСостоянийПоВидуЗаказа, 
				ДанныеРасшифровки.Настройки.ДополнительныеСвойства.ЦветаГрадиентаЗаказы, "ОтменыТоваров");
...
Функция ТекстЗапросаВоронкиПоЗаказам(ОбъектВоронки, ПолеГруппировки, ВариантВоронки, НастройкиКД)
	...
	|	&ПолеГруппировкиЗаказы КАК ПолеГруппировки,
	|	ИсторияСостоянийЗаказовСрезПоследних.Заказ КАК Заказ,
	|	ИсторияСостоянийЗаказовСрезПоследних.Заказ.ВидОперации КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА (ИсторияСостоянийЗаказовСрезПоследних.Заказ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу)
	|					ИЛИ ИсторияСостоянийЗаказовСрезПоследних.Заказ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаявкаНаПродажу))
	|			ТОГДА ""ЗаказПокупателя""
	|		ИНАЧЕ ""ЗаказНаряд""
	|	КОНЕЦ КАК ГруппировкаВидОперации,
	|	ОтмененныеЗапасы.Ссылка.ВидЗаказа КАК ВидЗаказа,
	|	ОтмененныеЗапасы.СостояниеОтмены КАК СостояниеОтмены,
	|	ОтмененныеЗапасы.ПричинаОтмены КАК ПричинаОтмены,
	|	ЕСТЬNULL(ОтмененныеЗапасы.Цена * ОтмененныеЗапасы.Количество, 0) КАК СуммаДокумента,
	|	1 КАК Потеря
	|ИЗ
	|	РегистрСведений.ИсторияСостоянийЗаказов.СрезПоследних(
	|			,
	...
	|				ИЗ
	|					ВТ_Заказы)) КАК ИсторияСостоянийЗаказовСрезПоследних
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.ОтмененныеЗапасы КАК ОтмененныеЗапасы
	|		ПО ИсторияСостоянийЗаказовСрезПоследних.Заказ = ОтмененныеЗапасы.Ссылка";
