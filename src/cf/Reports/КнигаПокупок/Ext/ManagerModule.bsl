...
Процедура ЗаполнитьСтрокуКнигиПокупок(
	...
	Если ПараметрыЗаполнения.СводныйКомиссионный
	   И ТипЗнч(ПараметрыЗаполнения.СчетФактураДокумент) = Тип("ДокументСсылка.СчетФактура") Тогда
	...
	Для каждого ЭлементКоллекции Из МассивСвСФНаОтгрузку Цикл  
		Если ЗначениеЗаполнено(ЭлементКоллекции.НомерСчетаФактуры) Тогда 
			Если ЗначениеЗаполнено(ЭлементКоллекции.НомерИсправленияСчетаФактуры) Тогда
				СчетаФактурыНомерДата = СтрШаблон(НСтр("ru='%1 от %2, исправление № %3 от %4'"),
					ЭлементКоллекции.НомерСчетаФактуры, Формат(ЭлементКоллекции.ДатаСчетаФактуры, "ДФ=dd.MM.yyyy"),
					ЭлементКоллекции.НомерИсправленияСчетаФактуры, Формат(ЭлементКоллекции.ДатаИсправленияСчетаФактуры, "ДФ=dd.MM.yyyy"));
			Иначе
	...
	СчетаФактурыДокументы = ОбщегоНазначенияПереопределяемый.УдалитьПовторяющиесяЭлементыМассива(СчетаФактурыДокументы);
...
Функция ТекстЗапросаЗаписиРегистраНДСПокупки(СтруктураПараметров)
	...
	|        И НЕ (НДСЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.ПриходнаяНакладная
	|        И
	|            НДСЗаписиКнигиПокупок.СчетФактура.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя))
	...
	|                ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура КАК
	|                    Документ.КорректировкаПоступления).ПометкаУдаления
	|            КОГДА НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура ССЫЛКА Документ.СчетФактура
	|                ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура КАК Документ.СчетФактура).ПометкаУдаления
	|            КОГДА НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура ССЫЛКА Документ.КорректировкаРеализации
	|                ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура КАК
	|                    Документ.КорректировкаРеализации).ПометкаУдаления
	|            КОГДА НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура ССЫЛКА Документ.СчетФактураПолученный
	|                ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПокупок.ИсправленныйСчетФактура КАК
	|                    Документ.СчетФактураПолученный).ПометкаУдаления
	|            ИНАЧЕ ЛОЖЬ
	|        КОНЕЦ)
	...
	|            ТОГДА ВЫБОР
	|                КОГДА НДСЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.СчетФактура
	...
	|        КОГДА &Постановление1137_2019
	|        И НДСЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.ОтчетКомиссионера
	...
	|        ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК
	|            ДанныеПервичныхДокументовИсправленныйСчетФактура
	...
	|        ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК
	|            ДанныеПервичныхДокументовСчетФактураВыданныйДокументОснование
	|        ПО СчетФактураВыданный.Организация = ДанныеПервичныхДокументовСчетФактураВыданныйДокументОснование.Организация
	|        И
	|            СчетФактураВыданный.ДокументОснование = ДанныеПервичныхДокументовСчетФактураВыданныйДокументОснование.Документ
	...
	|        ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК
	|            ДанныеПервичныхДокументовСчетФактураПолученныйДокументОснование
	|        ПО СчетФактураПолученный.Организация = ДанныеПервичныхДокументовСчетФактураПолученныйДокументОснование.Организация
	|        И
	|            СчетФактураПолученный.ДокументОснование = ДанныеПервичныхДокументовСчетФактураПолученныйДокументОснование.Документ
	|ГДЕ
	...
	|    И
	|        СчетФактураВыданныйДокументыОснования.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.СводнаяСправка)
	...
	|        И СчетФактураВыданныйДокументыОснования.Ссылка.Проведен
	|ГДЕ
	|    ЗаписиКнигиПокупокПредварительная.СчетФактура ССЫЛКА Документ.ОтчетКомиссионера
	|    И
	|        СчетФактураВыданныйДокументыОснования.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.СводнаяСправка)
	|    И &Постановление1137_2019
	|
	...
	|    ЗаписиКнигиПокупокПредварительная.СчетФактураПолученныйНомерВходящегоДокумента КАК
	|        СчетФактураПолученныйНомерВходящегоДокумента,
	|    ЗаписиКнигиПокупокПредварительная.СчетФактураПолученныйДатаВходящегоДокумента КАК
	|        СчетФактураПолученныйДатаВходящегоДокумента,
	...
	|    ВЫБОР
	|        КОГДА ЗаписиКнигиПокупокПредварительная.СчетФактура ССЫЛКА Документ.ОтчетКомиссионера
	|        И ЗаписиКнигиПокупокПредварительная.ПосредническиеУслуги
	|            ТОГДА ЗНАЧЕНИЕ(Документ.СчетФактура.ПустаяСсылка)
	|        ИНАЧЕ ЕСТЬNULL(ВТ_СводныеСправки.СводнаяСправка, ЗНАЧЕНИЕ(Документ.СчетФактура.ПустаяСсылка))
	...
	Если НЕ СтруктураПараметров.СформироватьОтчетПоСтандартнойФорме 
	   И СтруктураПараметров.ОтбиратьПоКонтрагенту Тогда
		...
		|				ИЛИ Контрагенты.ГоловнойКонтрагент = &КонтрагентДляОтбора)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка";
...
Функция ТекстЗапросаПодготовленныеЗаписиКнигиПокупок(СтруктураПараметров)
	...
	|    ВЫБОР
	|        КОГДА &Постановление1137_2019
	|        И ЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.ОтчетКомиссионера
	|        И НЕ ЗаписиКнигиПокупок.СводнаяСправка = ЗНАЧЕНИЕ(Документ.СчетФактура.ПустаяСсылка)
	|        И НЕ ЗаписиКнигиПокупок.НаАванс
	...
	|    ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокумент, ЗаписиКнигиПокупок.СчетФактура) КАК
	|        СчетФактураДокумент,
	|    ВЫБОР
	|        КОГДА &Постановление1137_2019
	|        И ЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|        И ЗаписиКнигиПокупок.СводнаяСправка = ЗНАЧЕНИЕ(Документ.СчетФактура.ПустаяСсылка)
	|        И НЕ ЗаписиКнигиПокупок.НаАванс
	...
	|        И НЕ ЗаписиКнигиПокупок.НаАванс
	|        И ЗаписиКнигиПокупок.КодВидаОперации = ""17""
	|            ТОГДА ВЫРАЗИТЬ(ЗаписиКнигиПокупок.СчетФактура КАК Документ.ОтчетКомиссионера).Номер
	|        КОГДА ТаблицаСчетаФактурыДокументы.СчетФактураДокумент ЕСТЬ NULL
	|        И ЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.СчетФактураПолученный
	...
	|        И НЕ ЗаписиКнигиПокупок.НаАванс
	|        И ЗаписиКнигиПокупок.КодВидаОперации = ""17""
	|            ТОГДА ВЫРАЗИТЬ(ЗаписиКнигиПокупок.СчетФактура КАК Документ.ОтчетКомиссионера).Дата
	|        КОГДА ТаблицаСчетаФактурыДокументы.СчетФактураДокумент ЕСТЬ NULL
	|        И ЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.СчетФактураПолученный
	|            ТОГДА ЗаписиКнигиПокупок.СчетФактураПолученныйДатаВходящегоДокумента
	...
	|                ИНАЧЕ ВЫБОР
	|                    КОГДА ЗаписиКнигиПокупок.ВсегоПокупок < 0
	|                        ТОГДА -ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.ВсегоПокупокВВалюте,
	...
	|    ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокументРасшифровка, ЗаписиКнигиПокупок.СчетФактура) КАК
	|        СчетФактураДокументРасшифровка,
	...
	|            ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходнаяНакладная КАК ВозвратТоваровОтПокупателя
...
Функция ПолучитьДополнительнуюИнформациюПоСчетуФактуре(СтруктураПараметров)
	...
	УчетНДСПереопределяемый.ДополнитьИнформациейОСчетахФактурахНаОтгрузку(ДополнительнаяИнформация, 
