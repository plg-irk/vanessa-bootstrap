...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресРезультата) Экспорт
	...
	ИсходныеДанные = ТаблицаИсходныеДанные(
		...
		ПараметрыОтчета.Организация, ПараметрыОтчета.НомерОтчета);
...
Функция ТаблицаИсходныеДанные(Знач ДатаНачала, Знач ДатаОкончания, Знач СтруктурнаяЕдиница, Знач Организация, Знач НомерОтчета)
	...
	СхемаЗапроса.ПакетЗапросов[0].Операторы[0].Источники[0].Источник.Параметры[4].Выражение
		= Новый ВыражениеСхемыЗапроса("Организация = &Организация И СтруктурнаяЕдиница = &СтруктурнаяЕдиница");
	...
	Запрос.УстановитьПараметр("ВидЦен"					, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктурнаяЕдиница, 
		"РозничныйВидЦен"));
