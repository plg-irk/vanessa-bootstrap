...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РассылкаОтчетов") Тогда
		Если НЕ НастройкиОтчета.Свойство("ВариантСсылка") Тогда
			Если ТипЗнч(Параметры.Ключ) = Тип("СправочникСсылка.ВариантыОтчетов") Тогда
...
#Если МобильныйКлиент Тогда
	ПриНажатииНаВидОтчета(Элементы.ДекорацияДоходыРасходы);
#Иначе
	Если ЗначениеЗаполнено(АктивнаяГиперссылка) Тогда
		ПриНажатииНаВидОтчета(Элементы[АктивнаяГиперссылка]);
	Иначе
		ПриНажатииНаВидОтчета(Элементы.ДекорацияДоходыРасходы);
	КонецЕсли;
#КонецЕсли
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьКарточкиАнализаБизнеса" Тогда
		...
		СтрокиКарточки = ЭтотОбъект[ИмяТаблицы].НайтиСтроки(Новый Структура("Ссылка", Параметр.Ссылка));
...
Процедура Подключаемый_КарточкаНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		СтрокаЭлемента = ЭтотОбъект[ПараметрыЭлемента.ИмяТаблицы].Получить(ПараметрыЭлемента.ИндексСтроки);
...
&НаКлиенте
Процедура Подключаемый_УдалитьКарточкуНажатие(Элемент)
	...
	ТаблицаКУдалению = ЭтотОбъект[ПараметрыЭлемента.ИмяТаблицы];
...
&НаКлиенте
Процедура ДекорацияАссистентНажатие(Элемент)
	ПриНажатииНаВидОтчета(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияДоходыРасходыНажатие(Элемент)
	ПриНажатииНаВидОтчета(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияДенежныйПотокНажатие(Элемент)
	ПриНажатииНаВидОтчета(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияБалансНажатие(Элемент)
	ПриНажатииНаВидОтчета(Элемент);
КонецПроцедуры
...
Процедура РаскрытьНаВесьЭкран(Команда)
	...
	СохранитьНастройкиФормыИОтборов();
	ОткрытьФорму("Отчет.АнализБизнеса.Форма.Форма",, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Функция ШрифтГиперссылкиНавигации(Полужирный)
	Если Полужирный Тогда
		Возврат ШрифтыСтиля.ШрифтПодчеркнутыйПолужирныйАнализБизнеса;
	Иначе
		Возврат ШрифтыСтиля.ШрифтПодчеркнутыйАнализБизнеса;
	КонецЕсли;
КонецФункции

&НаКлиенте
Процедура ПриНажатииНаВидОтчета(Элемент)
	...
	Если Элемент.Имя = "ДекорацияАссистент" Тогда
		...
		Элемент.Шрифт = ШрифтГиперссылкиНавигации(Истина);
	...
	Иначе
		...
		Элементы.ДекорацияАссистент.Шрифт = ШрифтГиперссылкиНавигации(Ложь);
...
Процедура ПереключитьСтраницу(Элементы, ВидыОтчетов, АктивнаяСтраница)
	...
	ЖирныйШрифт = ШрифтГиперссылкиНавигации(Истина);
	ПростойШрифт = ШрифтГиперссылкиНавигации(Ложь);
	...
	Для каждого ВидОтчета Из ВидыОтчетов Цикл
		...
		Если ИмяДекорации = АктивнаяСтраница Тогда
			Элементы[ИмяДекорации].Шрифт = ЖирныйШрифт;
			Элементы.ГруппаВариантыОтчетов.ТекущаяСтраница = Элементы[ИмяГруппы];
		Иначе
			Элементы[ИмяДекорации].Шрифт = ПростойШрифт;
		КонецЕсли;
...
Процедура ДобавитьКарточкуНаФорму(ВидКарточки, Ссылка)
	...
	НомерСтроки = ЭтотОбъект[ИмяТаблицы].Количество();
	СтрокаКарточки = ЭтотОбъект[ИмяТаблицы].Добавить();
...
Функция ИнициализироватьДанныеОтчета()
	...
	ДанныеОтчета.Вставить("ВидОтчетаОтбор", Перечисления.ВидыФинансовыхОтчетов.ПустаяСсылка());
	ДанныеОтчета.Вставить("ПериодОтчета", Дата('00010101'));
	ДанныеОтчета.Вставить("ПланФакт", Перечисления.ПланФакт.ПустаяСсылка());
	ДанныеОтчета.Вставить("Периодичность", Перечисления.Периодичность.ПустаяСсылка());
	ДанныеОтчета.Вставить("СценарийПланирования", Справочники.СценарииПланирования.ПустаяСсылка());
...
Процедура ИнициализироватьДеревьяОтчета(ДанныеОтчета)
	...
	Для каждого ИмяДерева Из МассивИменДеревьев Цикл
		...
		Дерево.Колонки.Добавить("НеИспользоватьВИтогах");
...
Процедура ДиалогСозданияБюджетаПродолжение(Выбор, ДополнительныеПараметры) Экспорт
	...
	Если Выбор = КодВозвратаДиалога.Да Тогда
		ОткрытьФорму("Документ.Бюджет.Форма.ФормаДокумента", , ЭтотОбъект);
	КонецЕсли;
...
Процедура ОбработатьРасшифровкуЯчейки(Расшифровка, СтандартнаяОбработка)
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		ОткрытьФорму("Отчет.АнализБизнеса.Форма.ФормаРасшифровкиЗначения", ПараметрыЯчейки, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВключитьРежимРедактированияКарточек(ВидКарточки)
	...
	ТаблицаКарточек = ЭтотОбъект[ИмяТаблицы];
...
Процедура ОбновитьSVGНаСервере()
	...
	// Обновим цвета кнопки режима редактирования на сервере
	Если РежимРедактирования Тогда
		Элементы.ВключитьРежимРедактирования.ЦветФона = ЦветаСтиля.НедоступныеДанныеЦвет;
	Иначе
...
// Находит существующую или создает новую группу карточек для указанного вида карточек анализа бизнеса.
//
// Параметры:
//  ВидКарточки - ПеречислениеСсылка.ВидыКарточекАнализаБизнеса - вид карточки, для которого требуется найти или создать группу.
//
// Возвращаемое значение:
//  ГруппаФормы - найденная или созданная группа карточек.
//
&НаСервере
...
Функция ПолучитьАдресСохраняемогоОтчета()
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при записи табличного документа'", ОбщегоНазначения.КодОсновногоЯзыка()),
