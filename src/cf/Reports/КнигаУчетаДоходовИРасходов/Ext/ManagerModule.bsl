...
Процедура ПечатьТитульногоЛиста(ПараметрыОтчета, ПараметрыВывода)
	...
	ДанныеОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Организация,
		"ИНН, КПП, КодПоОКПО, НаименованиеПолное, ЮридическоеФизическоеЛицо");
	...
	ДанныеТитульногоЛиста.Вставить("КодОКПО", ДанныеОрганизации.КодПоОКПО);
	ДанныеТитульногоЛиста.Вставить("ПолноеНаименованиеОрганизации", ДанныеОрганизации.НаименованиеПолное);
	...
	Если ДанныеОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		ИмяОбластиИНН = "ПрИНН";
	Иначе
		ИмяОбластиИНН = "ИНН";
...
Процедура ПечатьРаздела1(ПараметрыОтчета, ПараметрыВывода)
	...
	РегламентированнаяОтчетностьУСН.ПолучитьВТПериодыСтавокНДСНаУСН(МенеджерВТ,
	...
	|	КнигаУчетаДоходовИРасходов.ДоходВсего КАК ДоходВсего,
	|	КнигаУчетаДоходовИРасходов.ДоходБаза КАК ДоходБаза,
	|	КнигаУчетаДоходовИРасходов.РасходВсего КАК РасходВсего,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТДанныеКУДиР.ДоходВсего КАК ПечДоходыБУ,
	|	ВТДанныеКУДиР.ДоходБаза КАК ПечДоходы,
	|	0 КАК ПечРасходыБУ,
	|	0 КАК ПечРасходы,
	|	0 КАК НДС,
	|	ВТДанныеКУДиР.Содержание КАК Содержание,
	|	ВТДанныеКУДиР.ДатаПервичногоДокумента КАК ДатаПервичногоДокумента,
	|	ВТДанныеКУДиР.НомерПервичногоДокумента КАК НомерПервичногоДокумента,
	|	ВТДанныеКУДиР.Регистратор КАК Регистратор,
	|	ВТДанныеКУДиР.ГруппировкаКварталов КАК ГруппировкаКварталов
	|ИЗ
	|	ВТДанныеКУДиР КАК ВТДанныеКУДиР
	|ГДЕ
	|	ВТДанныеКУДиР.ДоходБаза <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	0,
	|	0,
	|	ВТДанныеКУДиР.РасходВсего,
	|	ВТДанныеКУДиР.РасходБаза,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПериодыСтавокНДСНаУСН.НДСИсключитьРасходы, ЛОЖЬ)
	|		ТОГДА 0
	|	ИНАЧЕ ВТДанныеКУДиР.НДС КОНЕЦ,
	|	ВТДанныеКУДиР.Содержание КАК Содержание,
	|	ВТДанныеКУДиР.ДатаПервичногоДокумента КАК ДатаПервичногоДокумента,
	|	ВТДанныеКУДиР.НомерПервичногоДокумента КАК НомерПервичногоДокумента,
	|	ВТДанныеКУДиР.Регистратор КАК Регистратор,
	...
	|			И ВТДанныеКУДиР.Период МЕЖДУ ПериодыСтавокНДСНаУСН.ПериодНачало И ПериодыСтавокНДСНаУСН.ПериодОкончание
	|ГДЕ
	|	ВТДанныеКУДиР.РасходБаза <> 0
