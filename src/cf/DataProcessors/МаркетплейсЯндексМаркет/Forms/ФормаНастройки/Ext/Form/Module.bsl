...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТаблицаСкладов.Загрузить( Склады );
	...
	ЯндексМаркетДанныеАвторизацииПомещеныВХранилище = ЯндексМаркетПроверитьДанныеАвторизацииПомещеныВХранилище( Объект.Ссылка );
...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания( "ИдентификаторовКабинетовПроверитьЗаполнение", 0.1, Истина );	
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ЯндексАвторизация" Тогда
		Если ТипЗнч( Параметр ) = Тип( "Структура" ) И Параметр.Свойство( "access_token" ) Тогда
			...
			ДанныеПриложенияЯндексМаркет = Параметр;
...
Процедура ЗагрузкаТоваровНадписьНажатие(Элемент)
	...
	Если ТаблицаСкладов.Количество() > 0 Тогда
		ПараметрыФормы.ИдентификаторКампанииЯндексМаркет = ТаблицаСкладов[ 0 ].ИдентификаторКампанииМаркетплейса;
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСтатусыПубликацииТоваровЯндексМаркет
...
Процедура ТаблицаСкладовИдентификаторКампанииМаркетплейсаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПоказатьВыборИзСписка( Оповещение, СкладыИзЯндексМаркета );
...
Процедура КнопкаДалее2(Команда)
	...
	Если ЯндексМаркетДанныеАвторизацииПомещеныВХранилище <> Истина Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю( НСтр( "ru = 'Необходимо авторизоваться в приложении Яндекс Маркет.'" ) );
		Возврат;
	КонецЕсли;
	
	Результат = ЯндексМаркетДанныеАвторизацииПомещеныВХранилище;
	
	Если Результат = Истина Тогда

		ОбщегоНазначенияКлиент.СообщитьПользователю( НСтр( "ru = 'Авторизация пройдена успешно.'") );
	...
	Если Объект.НастройкаЗавершена Тогда
		...
		ТекущийЭлемент = Элементы.ТаблицаСкладов;
	КонецЕсли;
...
Процедура УдалитьИнтеграцию(Команда)
	...
	ПараметрыФормы = Новый Структура( "УчетнаяЗапись", Объект.Ссылка );
	ОткрытьФорму("Обработка.МаркетплейсЯндексМаркет.Форма.УдалениеНастройкиИнтеграции", ПараметрыФормы, 
...
&НаКлиенте
Процедура ТаблицаСкладовИдентификаторКампанииМаркетплейсаПослеВыбора( Параметр1, Параметр2 ) Экспорт
	...
	ТекущиеДанные.ИдентификаторКампанииМаркетплейса = Параметр1.Значение.ИдентификаторКампании;
	ТекущиеДанные.ИдентификаторКабинетаМаркетплейса = Параметр1.Значение.ИдентификаторКабинета;
	ТекущиеДанные.НаименованиеСкладаМаркетплейса = Параметр1.Значение.НаименованиеМагазина;
КонецПроцедуры
...
Функция НастрокиЯндексМаркетПередВыполнениемОбменаПроверить()
	...
	Если УчетнаяЗапись.Пустая() Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю( НСтр( "ru = 'Учетная запись не записана.'" ) );
		Возврат Ложь;
	КонецЕсли;

	Если Модифицированность Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю( НСтр( "ru = 'Перед обменом данными необходимо записать изменения настроек Яндекс маркет.'" ) );
		Возврат Ложь;
	КонецЕсли;
	
	Если ТаблицаСкладов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю( НСтр( "ru = 'Склады не заполнены.'" ) );
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОбновитьОтображениеНаСервере()
	...
	Заголовок = "Интеграция с Яндекс Маркет: " + ?( ПустаяСтрока( Объект.Наименование ), "Новая", Объект.Наименование );
...
Процедура ОповеститьНаКлиенте(ТипДействия, КоличествоОбработанных)
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(СтрокаСообщения, КоличествоОбработанных));
...
Процедура ОбработкаПодбораТоваровЗаполнитьИзХранилища(АдресЗапасовВХранилище)
	...
	Запрос.Параметры.Вставить("БазовыйПрайсЛист", ВидЦены);
...
Процедура ЗаписатьИзмененияНаСервере( ЗаписатьДанныеАвторизацииЯндексМаркет = Ложь, ЗаписатьТаблицуСкладов = Истина )
	...
	Если ЗаписатьДанныеАвторизацииЯндексМаркет = Истина Тогда
		ИнтеграцияСЯндексМаркетСервер.ЗаписатьНастройкиАвторизации( 
		УчетнаяЗапись, ДанныеПриложенияЯндексМаркет );
		
		ЯндексМаркетДанныеАвторизацииПомещеныВХранилище = ЯндексМаркетПроверитьДанныеАвторизацииПомещеныВХранилище( УчетнаяЗапись );
	КонецЕсли;
...
Функция ЗаписатьИзмененияНаКлиенте( ПроверитьТаблицуСкладов = Истина )
	...
	Если ПроверитьТаблицуСкладов = Истина И ТаблицаСкладовЗаполненаКорректно() <> Истина Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю( "Изменения не записаны." );
		Возврат Ложь;
	КонецЕсли;
...
Функция НастройкиИзКонстантыПеренестиВСправочник()
	...
	УчетнаяЗаписьОбъект.Организация = Справочники.Организации.ПустаяСсылка();
...
Функция ТаблицаСкладовЗаполненаКорректно()
	...
	Для Каждого ТаблицаСкладовСтрока Из ТаблицаСкладов Цикл
		...
		Если ПустаяСтрока( ТаблицаСкладовСтрока.ИдентификаторКампанииМаркетплейса )
			...
			Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр( "ru = 'В таблице складов заполнены не все обязательные поля.'" ));
			Возврат Ложь;
		КонецЕсли;
		...
		Если ТаблицаСкладовСтрокиУникальны <> Истина Тогда
			...
			СообщениеШаблон = НСтр( "ru = 'В таблице складов повторение значения ""%1"" в колонке ""%2"". Значения в разных строках должны быть уникальными.'" );
			СообщениеТекст = СтрШаблон( СообщениеШаблон, Значение, КолонкаЗаголовок ); 
			ОбщегоНазначенияКлиент.СообщитьПользователю( СообщениеТекст );
...
Процедура ПолучитьСведенияОСкладахИзЯндексМаркетНаСервере( УчетнаяЗапись )
    ...
    СкладыИзЯндексМаркета.Очистить();
    
	УчетнаяЗапись = Объект.Ссылка;
	Если УчетнаяЗапись.Пустая() Тогда
		ОбщегоНазначения.СообщитьПользователю( "Учетная запись не записана." );
		Возврат;
	КонецЕсли;
	...
	Для Каждого КампанияЭлемент Из Кампании Цикл
		...
		СкладыИзЯндексМаркета.Добавить( Значение, СкладПредставление );
	КонецЦикла;
...
Функция ЗаписатьУчетнуюЗаписьИДанныеВХранилище()
	...
	Попытка
		...
		УстановитьПривилегированныйРежим(Истина);
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище( УчетнаяЗапись, КлючAPI, "api_key" );
		УстановитьПривилегированныйРежим(Ложь);
		
		КлючAPI = "";// очистим значение так как уже записано в безопасное хранилище
...
Функция ПодключитьЯндексМаркет()
	...
	Если ПроверкаПройдена Тогда
		...
		ЯндексМаркетДанныеАвторизацииПомещеныВХранилище = Истина;
