...
Процедура ПриИзмененииСуммыПлатежа(пТекущиеДанные, пИдентификатор = Неопределено, ИзменятьСуммуРасчетовВместоКурса = Ложь, ЭтоЗачетРасшифровкиПлатежа = Ложь)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЭтоЗачетПредоплаты Тогда
			ТекущиеДанные.СуммаПлатежа = ТекущиеДанные.СуммаРасчетов;
...
Функция ПолучитьТекстЗапросаДляРаспределенияСуммыПлатежа()
	...
	|	ЕСТЬNULL(КурсыВалютРасчетовСрезПоследних.Курс, 1) КАК Курс,
	|	ЕСТЬNULL(КурсыВалютРасчетовСрезПоследних.Кратность, 1) КАК Кратность,
...
Функция ДобавитьСтрокуДерева(СтрокаДереваРодитель, ТекущаяВыборка, ПолеГруппировки, ПоследняяГруппировка) Экспорт
	...
	Если НЕ ПоследняяГруппировка ИЛИ ЗначениеЗаполнено(ТекущийОбъект) Тогда
		...
		Если ПолеГруппировки = "Договор" Тогда
			...
			Если ИмяПервойГруппировки = "Договоры" Тогда
				...
				Иначе
					...
					Если ТекущаяВыборка.ЗаказВалютаДокумента = ВалютаДокумента Тогда
						СуммаИтогЗаказы = СуммаИтогЗаказы + ТекущаяВыборка.СуммаВалОстатокПоЗаказу;
					Иначе	
		...
		// Сумма в валюте документа
		ДобавитьСтрокуДереваФрагментУстановитьКурсИКратность(ТекущаяВыборка, ТекущаяСтрокаДерева);
		
		//**************************************************************************
		ТекущаяСтрокаДерева.ВалютаЗаказаСовпадаетСВалютойДокумента =
			ТекущаяВыборка.ЗаказВалютаДокумента = ВалютаДокумента
			И ПолеГруппировки = "Заказ";
...
&НаСервере
Процедура ДобавитьСтрокуДереваФрагментУстановитьКурсИКратность(ТекущаяВыборка, ТекущаяСтрокаДерева)

	Если ТекущаяВыборка.СуммаРегОстаток <> 0 И (ТекущаяВыборка.СуммаРегОстаток / ТекущаяВыборка.СуммаВалОстаток > 0)
		И ТекущаяВыборка.СуммаВалОстаток <> 0 Тогда
		ТекущаяСтрокаДерева.Курс = ТекущаяВыборка.СуммаРегОстаток / ТекущаяВыборка.СуммаВалОстаток;
		ТекущаяСтрокаДерева.Кратность = 1;
	ИначеЕсли НациональнаяВалюта = ВалютаУчета И ТекущаяВыборка.СуммаОстаток <> 0
		И ТекущаяВыборка.СуммаОстаток / ТекущаяВыборка.СуммаВалОстаток > 0
		И ТекущаяВыборка.СуммаВалОстаток <> 0 Тогда
		ТекущаяСтрокаДерева.Курс = ТекущаяВыборка.СуммаОстаток / ТекущаяВыборка.СуммаВалОстаток;
		ТекущаяСтрокаДерева.Кратность = 1;
	Иначе
...
Процедура ПересчитатьОжидаетОплатыНаСервере(ТекущаяСтрока, ПересчитыватьСуммуПлатежа = Истина, ПолеГруппировки = "")
	...
	Иначе
		...
		Если ВалютаДенежныхСредств = ТекущаяСтрока.ВалютаРасчетов
			Или ТекущаяСтрока.ВалютаЗаказаСовпадаетСВалютойДокумента Тогда
			ТекущаяСтрока.ОжидаетОплаты = ТекущаяСтрока.ОстатокКОплатеВалРасчетов;
		Иначе
