...
#Область ОбработчикиСобытийЭлементовТаблицыФормыГруппыПроверок
...
Процедура ОткрытьЖурналРегистрации(Команда)
	...
	Отбор.Вставить("Сеанс");
	Отбор.Вставить("Уровень");
	Отбор.Пользователь = "";
	Отбор.ДатаНачала = НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()-86400);
	Отбор.ДатаОкончания = КонецДня(ОбщегоНазначенияКлиент.ДатаСеанса());
...
Функция ВыполнитьДиагностикуНастроек()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(ЭтотОбъект.УникальныйИдентификатор);
...
Процедура ОбновитьЭлементыДляСпискаПроверок(ОбновлениеТолькоГрупп = Ложь)
	...
	Для каждого РезультатПроверки Из РезультатыПроверок Цикл
		Если РезультатПроверки.УровеньПроверки = "Группа" Тогда
			//@skip-check module-self-reference
			ЭлементГруппыПроверок = ЭтотОбъект.Элементы.Найти("Группа" + СтрЗаменить(РезультатПроверки.ТипГруппыПроверок, " ",""));
			Если ЭлементГруппыПроверок <> Неопределено Тогда
				//@skip-check module-self-reference
				ЭтотОбъект.Элементы.Удалить(ЭлементГруппыПроверок);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
				...
				ИначеЕсли РезультатТекущий.УровеньПроверки = "Элемент" И РезультатОбновленный.УровеньПроверки = "Элемент"
					...
					Если РезультатОбновленный.ТипПроверки = ПредопределенноеЗначение("Перечисление.ТипыПроверокТелефонии.ПроверкаПолучениеПодпискиНаСобытияАТС") Тогда
						Если РезультатТекущий.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусПроверкиТелефонии.Успешно") Тогда
							ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Новая подписка на события АТС получена.'"));
						Иначе
							ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Ошибка при попытке создания новой подписки на события АТС.
...
Процедура ВыделитьСтрокуПервойОшибкеВСпискеПроверок()
	...
	//@skip-check module-self-reference
	ЭтотОбъект.ТекущийЭлемент = Элементы.ГруппыПроверок;
