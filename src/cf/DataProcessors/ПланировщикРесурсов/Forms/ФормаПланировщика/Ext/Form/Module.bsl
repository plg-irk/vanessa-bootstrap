...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ВыбранныеРесурсы.Количество() И Не ЭтоПодбор И Не ЗавершениеРаботы Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаПриЗакрытии", ЭтотОбъект, ПараметрыОповещения);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Не ВыбранноеЗначение = Неопределено Тогда 
		Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
			...
			ОткрытьФорму("Документ.ЗаказНаПроизводство.ФормаОбъекта", ПараметрыОткрытия, ЭтотОбъект);
		КонецЕсли;
		
		Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			ПараметрыОткрытия = Новый Структура("Ключ, ВыбранныеРесурсы, ОткрытИзПланировщика",ВыбранноеЗначение, КоллекцияРесурсов, Истина);
			ОткрытьФорму("Документ.ЗаказПокупателя.Форма.ФормаЗаказНаряда", ПараметрыОткрытия, ЭтотОбъект);
		КонецЕсли;
		
		Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.Событие") Тогда
			ПараметрыОткрытия = Новый Структура("Ключ, ВыбранныеРесурсы, ОткрытИзПланировщика",ВыбранноеЗначение, КоллекцияРесурсов, Истина);
			ОткрытьФорму("Документ.Событие.Форма.ФормаСобытияЗаписьКонтрагента", ПараметрыОткрытия, ЭтотОбъект);
		КонецЕсли;
...
Процедура ЗагрузкаРесурсовВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ТипЗнч(РасшифровкаЯчейки) = Тип("Структура")
		...
		Если Область.Лево = 1 Тогда
			...
			ОткрытьФорму("Справочник.КлючевыеРесурсы.ФормаОбъекта",ПараметрыОткрытия,ЭтотОбъект);
			Возврат
...
Процедура ОтборДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли ТолькоПоПодсистеме3 И Не ТолькоПоПодсистеме2 И Не ТолькоПоПодсистеме1 Тогда
		...
		ОткрытьФорму("Документ.Событие.ФормаСписка", ПараметрыОткрытия, Элемент,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Возврат;
	Иначе
	...
	Оповещение = Новый ОписаниеОповещения("ПослеВыбораДокументаИзМеню", ЭтотОбъект, ПараметрыОповещения);
...
Процедура КомандаПодобратьВКорзину(Команда)
	...
	Иначе
		...
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаПодборИзКонтекстногоМеню", ЭтотОбъект, ПараметрыОповещения);
...
Процедура ВывестиРасписаниеПоМесяцам(МакетПланированиеРесурсов, ОбластьМакетаЗаголовка, СтруктураЦветов,
	...
	Для Каждого СтрокаГод Из ТЗГодыПериода Цикл
		...
		Для Каждого СтрокаПериод Из НайденныеСтрокиМесяцы Цикл
			...
			СтруктураРасшифровки.Вставить("Ресурс", Неопределено);
			СтруктураРасшифровки.Вставить("Год", Неопределено);
			СтруктураРасшифровки.Вставить("Месяц", Неопределено);
			СтруктураРасшифровки.Вставить("ВидРасшифровки", Неопределено);
			...
			Для Каждого РесурсПредприятия Из ТаблицаРесурсовПредприятия Цикл
				...
				СтруктураРасшифровки.Ресурс = Ресурс;
				СтруктураРасшифровки.Год = СтрокаПериод.Год;
				СтруктураРасшифровки.Месяц = СтрокаПериод.Месяц;
				СтруктураРасшифровки.ВидРасшифровки = "ПоДням";
...
Процедура ВывестиРасписаниеКратноИнтервалам(МакетПланированиеРесурсов, ОбластьМакетаЗаголовка, СтруктураЦветов
	...
	ПараметрыРасшифровки.Вставить("ДокументыРасшифровки", Неопределено);
	ПараметрыРасшифровки.Вставить("ИнтервалыВыводаПоДокументам", Неопределено);
	ПараметрыРасшифровки.Вставить("СоответствиеДокументовРасшифровкиСтрокам", Неопределено);
	ПараметрыРасшифровки.Вставить("НачалоИнтервала", Неопределено);
	ПараметрыРасшифровки.Вставить("ОкончанияИнтервала", Неопределено);
	ПараметрыРасшифровки.Вставить("МассивДокументовСПересечениями", Неопределено);
	ПараметрыРасшифровки.Вставить("НомерСтрокиРасшифровки", Неопределено);
	ПараметрыРасшифровки.Вставить("КоличествоЯчеекДляОбъединения", Неопределено);
	ПараметрыРасшифровки.Вставить("ИнтервалВыводаОбласти", Неопределено);
	ПараметрыРасшифровки.Вставить("Ресурс", Неопределено);
	ПараметрыРасшифровки.Вставить("ОбластьРасшифровкаПоДокументам", Неопределено);
	ПараметрыРасшифровки.Вставить("ЗагрузкаРесурса", Неопределено);
	ПараметрыРасшифровки.Вставить("МатрицаИнтервалов", Неопределено);
	...
	Для Каждого РесурсПредприятия Из ТаблицаРесурсовПредприятия Цикл
		...
		СтруктураРасшифровки = Новый Структура();
		СтруктураРасшифровки.Вставить("Ресурс", Неопределено);
		СтруктураРасшифровки.Вставить("СсылкаНаДокумент", Неопределено);
		СтруктураРасшифровки.Вставить("ВидРасшифровки", Неопределено);
		СтруктураРасшифровки.Вставить("НомерСтрокиДокумента", Неопределено);
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			СтруктураРасшифровки.Ресурс = Ресурс;
			СтруктураРасшифровки.СсылкаНаДокумент = НайденнаяСтрока.Ссылка;
			СтруктураРасшифровки.ВидРасшифровки = "ПозиционированиеНаДокументе";
			СтруктураРасшифровки.НомерСтрокиДокумента = НайденнаяСтрока.НомерСтроки;
			...
			Пока Интервал <= КоличествоПериодовЗаДеньСписка Цикл
				...
				Если РабочийПериод И ВыводНеКратногоИнтервала = Ложь Тогда
					...
					Если КоличествоЯчеекДляОбъединения > 1 
						...
						ПараметрыРасшифровки.ДокументыРасшифровки = ДокументыРасшифровки;
						ПараметрыРасшифровки.ИнтервалыВыводаПоДокументам = ИнтервалыВыводаПоДокументам;
						ПараметрыРасшифровки.СоответствиеДокументовРасшифровкиСтрокам = СоответствиеДокументовРасшифровкиСтрокам;
						ПараметрыРасшифровки.НачалоИнтервала = НачалоИнтервала;
						ПараметрыРасшифровки.ОкончанияИнтервала = ОкончанияИнтервала;
						ПараметрыРасшифровки.МассивДокументовСПересечениями = МассивДокументовСПересечениями;
						ПараметрыРасшифровки.НомерСтрокиРасшифровки = НомерСтрокиРасшифровки;
						ПараметрыРасшифровки.КоличествоЯчеекДляОбъединения = КоличествоЯчеекДляОбъединения;
						ПараметрыРасшифровки.ИнтервалВыводаОбласти = ИнтервалВыводаОбласти;
						ПараметрыРасшифровки.Ресурс = Ресурс;
						ПараметрыРасшифровки.ОбластьРасшифровкаПоДокументам = ОбластьРасшифровкаПоДокументам;
						ПараметрыРасшифровки.ЗагрузкаРесурса = ЗагрузкаРесурса;
						ПараметрыРасшифровки.МатрицаИнтервалов = МатрицаИнтервалов;
						
						ВывестиДокументыРасшифровки(ПараметрыРасшифровки, ЗагрузкаРесурса);
				...
				Если КоличествоЯчеекДляОбъединения >=1 Тогда
					...
					ПараметрыРасшифровки.НомерСтрокиРасшифровки = НомерСтрокиРасшифровки;
					ПараметрыРасшифровки.КоличествоЯчеекДляОбъединения = КоличествоЯчеекДляОбъединения;
					ПараметрыРасшифровки.ИнтервалВыводаОбласти = ИнтервалВыводаОбласти-1;
					ПараметрыРасшифровки.Ресурс = Ресурс;
					ПараметрыРасшифровки.ОбластьРасшифровкаПоДокументам = ОбластьРасшифровкаПоДокументам;
					ПараметрыРасшифровки.ЗагрузкаРесурса = ЗагрузкаРесурса;
					ПараметрыРасшифровки.МатрицаИнтервалов = МатрицаИнтервалов;
...
&НаСервере
Процедура ВывестиДокументыРасшифровки(ПараметрыРасшифровки, ЗагрузкаРесурса)
	
	ДокументыРасшифровки = ПараметрыРасшифровки.ДокументыРасшифровки;
	ИнтервалыВыводаПоДокументам = ПараметрыРасшифровки.ИнтервалыВыводаПоДокументам;
	СоответствиеДокументовРасшифровкиСтрокам = ПараметрыРасшифровки.СоответствиеДокументовРасшифровкиСтрокам;
	НачалоИнтервала = ПараметрыРасшифровки.НачалоИнтервала;
	ОкончанияИнтервала = ПараметрыРасшифровки.ОкончанияИнтервала;
	МассивДокументовСПересечениями = ПараметрыРасшифровки.МассивДокументовСПересечениями;
	НомерСтрокиРасшифровки = ПараметрыРасшифровки.НомерСтрокиРасшифровки;
	КоличествоЯчеекДляОбъединения = ПараметрыРасшифровки.КоличествоЯчеекДляОбъединения;
	ИнтервалВыводаОбласти = ПараметрыРасшифровки.ИнтервалВыводаОбласти;
	Ресурс = ПараметрыРасшифровки.Ресурс;
	ОбластьРасшифровкаПоДокументам = ПараметрыРасшифровки.ОбластьРасшифровкаПоДокументам;
	МатрицаИнтервалов = ПараметрыРасшифровки.МатрицаИнтервалов;
...
Процедура ЗаполнитьСписокДнямиПоМесяцам()
	...
	ДатаКалендаря = ?(Не ЗначениеЗаполнено(ДатаПериода), ТекущаяДатаСеанса(), ДатаПериода);
...
Процедура ДобавитьВЗаказНаПроизводство()
	ПараметрыОткрытия = Новый Структура();
	ОткрытьФорму("Документ.ЗаказНаПроизводство.Форма.ФормаВыбора", ПараметрыОткрытия,ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВЗаказНаряд()
	ПараметрыОткрытия = Новый Структура("РежимВыбора", Истина);
	ОткрытьФорму("Документ.ЗаказПокупателя.Форма.ФормаСпискаЗаказНаряда", ПараметрыОткрытия, ЭтотОбъект);
КонецПроцедуры
...
Процедура ДобавитьВСобытие()
	ПараметрыОткрытия = Новый Структура("РежимВыбора, ТипСобытия", Истина, ПредопределенноеЗначение("Перечисление.ТипыСобытий.Запись"));
	ОткрытьФорму("Документ.Событие.ФормаСписка", ПараметрыОткрытия, ЭтотОбъект);
КонецПроцедуры
...
Процедура СпозиционироватьНаОбласти(ПредставлениеПериодаПоиска, СсылкаНаДокумент = Неопределено, Ресурс = Неопределено, НомерСтрокиДокумента = 0)
	...
	Если Не ЗначениеЗаполнено(СсылкаНаДокумент) Тогда
		...
		ТекущийЭлемент = Элементы.ЗагрузкаРесурсов;
...
Процедура ВосстановитьДанныеВариантовОтображения(НомерВарианта, ОткрытиеФормы = Ложь)
	...
	Если Не ЭтоПодбор Тогда
		...
		Для Каждого СтрокаКэш Из НайденныеСтроки Цикл
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтрокаКэш);
		КонецЦикла;
...
Процедура ПослеЗакрытияВопроса(Результат, ПараметрыВопроса) Экспорт
	...
	ВыбранноеДействие = ПараметрыВопроса.ВыбранноеДействие;
	
	Область = ПараметрыВопроса.Область;
	КонтрольГрафика = ПараметрыВопроса.КонтрольГрафика;
	
	Если ВыбранноеДействие = "Подобрать" Тогда
		ПодобратьВКорзину(ПараметрыВопроса.Ресурс, ПараметрыВопроса.Область, КонтрольГрафика);
		Возврат
	КонецЕсли;
	
	Если ВыбранноеДействие = "Создать Заказ на производство" Тогда
		ПодобратьВБуфер(ПараметрыВопроса.Ресурс, Область, КонтрольГрафика);
		СоздатьЗаказНаПроизводство();
		БуферДобавленияВДокумент.Очистить();
		Возврат
	КонецЕсли;
...
Процедура ПослеВыбораИзМеню(ВыбранныйЭлемент, ПараметрыВыбора) Экспорт
	...
	Область = ПараметрыВыбора.Область;
	
	Если Не ВыбранныйЭлемент.Значение = "Перейти" И Не РасписаниеРаботПереключатель = ИнтервалМесяц() Тогда
		
		Если ПараметрыВыбора.КонтрольГрафика И Не ТипЗнч(Область.Расшифровка) = Тип("Структура") 
			...
			ПараметрыОповещения.Вставить("Ресурс", ПараметрыВыбора.Ресурс);
			ПараметрыОповещения.Вставить("Область", Область);
			ПараметрыОповещения.Вставить("ВыбранноеДействие", ВыбранныйЭлемент.Значение);
			ПараметрыОповещения.Вставить("КонтрольГрафика", ПараметрыВыбора.КонтрольГрафика);
	...
	Если ВыбранныйЭлемент.Значение = "Подобрать" Тогда
		...
		ПодобратьВКорзину(ПараметрыВыбора.Ресурс, Область, ПараметрыВыбора.КонтрольГрафика);
		
	ИначеЕсли ВыбранныйЭлемент.Значение = "Перейти" И Не РасписаниеРаботПереключатель = ИнтервалМесяц() Тогда
		ВывестиКратноИнтервалам(ПараметрыВыбора.День);
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "Создать Заказ на производство" Тогда
		ПодобратьВБуфер(ПараметрыВыбора.Ресурс, Область, ПараметрыВыбора.КонтрольГрафика);
		СоздатьЗаказНаПроизводство();
		БуферДобавленияВДокумент.Очистить();
...
Процедура ОткрытьКорзину()
	...
	ПараметрыОткрытия = Новый Структура();
	ПараметрыОткрытия.Вставить("ВыбранныеРесурсы", ВыбранныеРесурсы);
	ПараметрыОткрытия.Вставить("ЭтоПодбор", ЭтоПодбор);
	ПараметрыОткрытия.Вставить("ТолькоПоПодсистеме1", ТолькоПоПодсистеме1);
	ПараметрыОткрытия.Вставить("ТолькоПоПодсистеме2", ТолькоПоПодсистеме2);
	ПараметрыОткрытия.Вставить("ТолькоПоПодсистеме3", ТолькоПоПодсистеме3);
	...
	ОткрытьФорму("Обработка.ПланировщикРесурсов.Форма.ФормаВыбранныеРесурсы",ПараметрыОткрытия,ЭтотОбъект,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработатьСобытиеВыбораИнтервала(Ресурс, Область, День, КонтрольГрафика)
	...
	ПараметрыОповещения = Новый Структура();
	ПараметрыОповещения.Вставить("Ресурс", Ресурс);
	ПараметрыОповещения.Вставить("Область", Область);
	ПараметрыОповещения.Вставить("День", День);
	ПараметрыОповещения.Вставить("КонтрольГрафика", КонтрольГрафика);
...
Процедура ЗаполнитьБуфер()
	...
	Для Каждого ВыделеннаяОбласть Из ВыделенныеОбласти Цикл
		...
		Для ИндексГраницы = ВерхняяГраницаОбласти По НижняяГраницаОбласти Цикл
			...
			ВремяОкончания = ?(Не ЗначениеЗаполнено(ВремяОкончания) И ТипЗнч(ВремяНачала) = Тип("Дата"), КонецДня(ВремяНачала), ВремяОкончания);
			ВремяОкончания = ?(ТипЗнч(ВремяНачала) = Тип("Дата") И ТипЗнч(ВремяОкончания) = Тип("Дата") 
								И ВремяОкончания = НачалоДня(ВремяОкончания), КонецДня(ВремяНачала), ВремяОкончания);
...
Процедура ПриИзмененииЦветаИнтервала()
	...
	Если ЦветРабочееВремя = ПустойЦвет Тогда
		ЦветРабочееВремя = WebЦвета.ЦветМорскойВолныНейтральный;
	КонецЕсли;
	Если ЦветНеРабочееВремя = ПустойЦвет Тогда
		ЦветНеРабочееВремя = WebЦвета.Серебряный;
	КонецЕсли;
	Если ЦветПолнойЗагрузки = ПустойЦвет Тогда
		ЦветПолнойЗагрузки = WebЦвета.Золотой;
	КонецЕсли;
	Если ЦветПревышенияЗагрузки = ПустойЦвет Тогда
		ЦветПревышенияЗагрузки = WebЦвета.Лосось;
	КонецЕсли;
	Если ЦветЧастичнойЗагрузкиДо = ПустойЦвет Тогда
		ЦветЧастичнойЗагрузкиДо = WebЦвета.ЦветМорскойВолныНейтральный;
	КонецЕсли;
	Если ЦветЧастичнойЗагрузкиПосле = ПустойЦвет Тогда
		ЦветЧастичнойЗагрузкиПосле = WebЦвета.ЦветМорскойВолныНейтральный;
	КонецЕсли;
	Если ЦветПодобранныхВКорзину = ПустойЦвет Тогда
		ЦветПодобранныхВКорзину = WebЦвета.КоролевскиГолубой;
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеВыбораДокументаИзМеню(ВыбранныйЭлемент, ПараметрыВыбора) Экспорт
	
	Если ВыбранныйЭлемент = Неопределено Тогда Возврат КонецЕсли;
	
	Если ВыбранныйЭлемент.Значение = "Заказ на производство" Тогда
		ПараметрыОткрытия = Новый Структура();
		ОткрытьФорму("Документ.ЗаказНаПроизводство.Форма.ФормаВыбора", ПараметрыОткрытия, ПараметрыВыбора.Элемент);
		Возврат;
	КонецЕсли;
	
	Если ВыбранныйЭлемент.Значение = "Заказ-Наряд" Тогда
		ПараметрыОткрытия = Новый Структура("РежимВыбора", Истина);
		ОткрытьФорму("Документ.ЗаказПокупателя.Форма.ФормаСпискаЗаказНаряда", ПараметрыОткрытия, ПараметрыВыбора.Элемент);
		Возврат;
	КонецЕсли;
	
	Если ВыбранныйЭлемент.Значение = "Событие ""Запись""" Тогда
		ПараметрыОткрытия = Новый Структура("РежимВыбора, ТипСобытия", Истина, ПредопределенноеЗначение("Перечисление.ТипыСобытий.Запись"));
		ОткрытьФорму("Документ.Событие.ФормаСписка", ПараметрыОткрытия, ПараметрыВыбора.Элемент);
		Возврат;
	КонецЕсли
