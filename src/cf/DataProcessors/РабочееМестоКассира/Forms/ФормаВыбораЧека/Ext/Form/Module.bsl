...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		Параметры, "ОтображатьНаименованиеДляПечати") Тогда
		ОтображатьНаименованиеДляПечати = Параметры.ОтображатьНаименованиеДляПечати;
	КонецЕсли;
	
	ОбщегоНазначенияРМКПереопределяемый.СформироватьЗапросТаблицаЧековПоУмолчанию(ЭтотОбъект);
	ОбщегоНазначенияРМКПереопределяемый.НастроитьОтображениеТоваровНаФормеВыбораЧека(ЭтотОбъект);
	...
	ИначеЕсли Параметры.Свойство("ВозвратПродажи") Или ПроверкаЧека Тогда
		...
		ЗаполнитьТаблицуЧековККМ();
		ПравоВозвратБезЧека = Истина;
		
		Если Параметры.Свойство("ПравоВозвратБезЧека") Тогда
			ПравоВозвратБезЧека = Параметры.ПравоВозвратБезЧека;
		КонецЕсли;
		
		Элементы.ВозвратБезЧека.Видимость = ПравоВозвратБезЧека;
		Элементы.КоррекцияБезОснования.Видимость = Ложь;
	...
	ИначеЕсли ЭтоЧекКоррекции Тогда
		...
		Элементы.ВозвратБезЧека.Видимость = Ложь;
		ЗаполнитьТаблицуЧековКоррекции();
	...
	Если Не (ПроверкаЧека И РежимКассыСамообслуживания) Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ТаблицаЧековККМ,
			"ИсключатьЧекиККМПредоплатыЗачтенные",
			ЭтоВозвратПродажи);
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ПоВсемМагазинамПриИзменении(Неопределено);
	ПериодПриИзменении();
	КассаККМПриИзменении(Неопределено);
	ТекущийЭлемент = Элементы.ТаблицаЧековККМ;
	
	Если ПоказатьСоставЧекаПриОткрытии Тогда
		Элементы.ГруппаСоставЧека.Показать();
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ПоказатьСоставЧекаПриОткрытии = Не Элементы.ГруппаСоставЧека.Скрыта();
...
&НаКлиенте
Процедура ТаблицаЧековККМПриАктивизацииСтроки(Элемент)
...
&НаКлиенте
Процедура КоррекцияБезОснования(Команда)
	
	СтруктураСтроки = Новый Структура;
	СтруктураСтроки.Вставить("КоррекцияБезОснования");
	Закрыть(СтруктураСтроки);
...
&НаСервере
Процедура ЗаполнитьТаблицуЧековКоррекции()
	ОбщегоНазначенияРМКПереопределяемый.ЗаполнитьТаблицуЧековКоррекции(ЭтотОбъект);
КонецПроцедуры
...
&НаКлиенте
Процедура ОбновитьСоставЧека()
	
	Если Не Элементы.ГруппаСоставЧека.Видимость Тогда
		Возврат;
	КонецЕсли;
	
	ТекущаяСтрока = Элементы.ТаблицаЧековККМ.ТекущиеДанные;
	
	СсылкаЧекаККМ = Неопределено;
	Если Не ТекущаяСтрока = Неопределено Тогда
		СсылкаЧекаККМ = ТекущаяСтрока.Ссылка;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ТоварыЧека,
		"ЧекККМ", СсылкаЧекаККМ);
