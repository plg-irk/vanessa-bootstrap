...
Процедура ВыполнитьЗакрытиеМесяца(СтруктураПараметров, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	//Если ведётся учет по компании в целом, делаем зачет ЕНС для каждого ИП в фоне
	Если Константы.УчетПоКомпании.Получить() Тогда
		ВыполнитьОперациюЗакрытияМесяцаЕНСДляИПнаЕНС(СтруктураПараметров, Операции, ДокументЗакрытияМесяца);
	КонецЕсли;  
	//Если ведётся учет по компании в целом, делаем зачет ЕНС для каждого ИП в фоне
...
Функция ДанныеПоЗакрытиюПериодов(Организация, МесяцРасчета)
	...
	//Если ведётся учет по компании в целом, делаем зачет ЕНС для каждого ИП в фоне
	Если Константы.УчетПоКомпании.Получить() Тогда 
		ЗапросОрганизацииЕНС = Новый Запрос;
		ЗапросОрганизацииЕНС.Текст = ТекстЗапросаОрганизацииИПЕдиныйНалоговыйСчет();  
		ЗапросОрганизацииЕНС.УстановитьПараметр("НижняяГраница", НижняяГраница);
		ЗапросОрганизацииЕНС.УстановитьПараметр("ВерхняяГраница", ВерхняяГраница);
		ЗапросОрганизацииЕНС.УстановитьПараметр("Компания", Компания);
		
		ОрганизацииЕНС = ЗапросОрганизацииЕНС.Выполнить().Выгрузить();
		
		СтрокаОрганизацииЕНС = ОрганизацииЕНС.Добавить();
		СтрокаОрганизацииЕНС.Организация = Компания;
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗакрытиеМесяца.Организация = &Организация", "ЗакрытиеМесяца.Организация В (&ОрганизацииЕНС)");
		
		Запрос.УстановитьПараметр("ОрганизацииЕНС", ОрганизацииЕНС);
	Иначе	
		Запрос.УстановитьПараметр("ОрганизацииЕНС", Компания);
	КонецЕсли;
	//Если ведётся учет по компании в целом, делаем зачет ЕНС для каждого ИП в фоне
...
Функция ТекстЗапросаОперацииЗакрытияМесяца()
	...
	|	ИСТИНА
	|ИЗ
	|	РегистрНакопления.РасчетыПоНалогамНаЕдиномНалоговомСчете.Обороты(&НижняяГраница, &ВерхняяГраница, Месяц, Организация В (&ОрганизацииЕНС)) КАК РасчетыПоНалогамНаЕдиномНалоговомСчетеОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаПолучениеДанных()
	...
	|	И (ЗапасыОстаткиИОбороты.КоличествоПриход <> 0 ИЛИ ЗапасыОстаткиИОбороты.КоличествоРасход <> 0)
	...
	Для каждого СтрокаМесяц Из РезультатРасчета.Строки Цикл
		...
		Если КоллекцияОписаний.Количество() > 0 Тогда
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьЭтапыПроизводства") Тогда
				ШаблонСообщения = НСтр("ru = 'В %1 возникли остатки себестоимости без количества по %2.
				|Следует выполнить повторное закрытие месяца для пересчета фактической себестоимости.'");
			Иначе
				ШаблонСообщения = НСтр("ru = 'В %1 возникли остатки себестоимости без количества по %2. 
				|Такая ситуация может возникать из-за ошибок в учете, в результате принятия к учету имущества, возвратов по поступлениям в закрытых периодах и так далее. 
				|Закрытие месяца может автоматически списывать такие остатки.'");
			КонецЕсли;
	...
	Пока ВыборкаМесяц.Следующий() Цикл
		...
		Пока ВыборкаКонтрагент.Следующий() Цикл
			...
			Пока ВыборкаДоговор.Следующий() Цикл
				ВыборкаЗаказ = ВыборкаДоговор.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				Пока ВыборкаЗаказ.Следующий() Цикл
					Если НЕ ЗначениеЗаполнено(ВыборкаЗаказ.АвансИзменение) И НЕ ЗначениеЗаполнено(ВыборкаЗаказ.ДолгИзменение) Тогда
						// Нет оборотов в месяце 
						Продолжить;
					КонецЕсли;
					Если ВыборкаЗаказ.АвансКонец > 0 И ВыборкаЗаказ.ДолгКонец > 0 Тогда
						СтруктураКонтрагента = Новый Структура;
						СтруктураКонтрагента.Вставить("Контрагент", ВыборкаЗаказ.Контрагент);
						СтруктураКонтрагента.Вставить("Договор", ВыборкаЗаказ.Договор);
						СтруктураКонтрагента.Вставить("Заказ", ВыборкаЗаказ.Заказ);
						СтруктураКонтрагента.Вставить("Аванс", ВыборкаЗаказ.АвансКонец);
						СтруктураКонтрагента.Вставить("Долг", ВыборкаЗаказ.ДолгКонец);
						СтруктураКонтрагента.Вставить("ОшибкиНаНачало", ВыборкаЗаказ.АвансНачало > 0 И ВыборкаЗаказ.ДолгНачало > 0);
						СтруктураКонтрагента.Вставить("КЗачету", Мин(ВыборкаЗаказ.АвансКонец, ВыборкаЗаказ.ДолгКонец));
						Если Ошибки.Получить(ВыборкаЗаказ.Месяц) = Неопределено Тогда
							Ошибки.Вставить(ВыборкаЗаказ.Месяц, Новый Массив);
						КонецЕсли; 
						Ошибки.Получить(ВыборкаЗаказ.Месяц).Добавить(СтруктураКонтрагента);
					КонецЕсли; 
				КонецЦикла; 
			КонецЦикла; 
		КонецЦикла; 
...
Процедура ПроверитьРасчетыСПоставщиками(РезультатРасчета, Организация, НижняяГраница, ВерхняяГраница)
	...
	|	РасчетыСПоставщикамиОстаткиИОбороты.Заказ КАК Заказ,
	|	ВЫБОР
...
Процедура СравнитьДанныеДокументов_ДоговорВТабЧасти(ОбъектПроверки, ТаблицаРасположенияДоговоров, Организация, МассивДоговоров, РезультатРасчета, Выборка, СтрокаТаблицыПравил)
	...
	ИначеЕсли РасположениеДоговораПодчиненный.ТипРеквизита = "ТабличнаяЧасть" Тогда
		...
		Если ИмяПодчиненногоДокумента = "АвансовыйОтчет" Тогда // АПК:1297 не локализуется
			Возврат;
		КонецЕсли;
...
Функция ОтменитьЗакрытиеМесяца(СтруктураПараметров, ОтменаЗакрытияЕНСДляИП = Ложь)
	...
	Запрос = Новый Запрос;
		...
		Если ОтменаЗакрытияЕНСДляИП Тогда 
	...
	Если Не ОтменаЗакрытияЕНСДляИП Тогда
...
Процедура ВыполнитьОперациюЗакрытияМесяца(СтруктураПараметров, Операция, ДокументЗакрытияМесяца)
	...
	Иначе
		...
		ДокОбъект.ЗачетАвансаПоЕдиномуНалоговомуСчету = Ложь;
...
Процедура ВыполнитьОперациюЗакрытияМесяцаЕНСДляИПнаЕНС(СтруктураПараметров, Операции, ДокументЗакрытияМесяца) 
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаОрганизацииИПЕдиныйНалоговыйСчет();
	ТекМесяц = СтруктураПараметров.ТекМесяц;
	ТекГод = СтруктураПараметров.ТекГод;
	МесяцРасчета = Дата(ТекГод, ТекМесяц, 1);
	НижняяГраница = НачалоГода(МесяцРасчета);
	ВерхняяГраница = КонецГода(МесяцРасчета);
	Компания = Константы.УчетПоКомпании.Компания(СтруктураПараметров.Организация);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СтруктураТекущихДокументов = ОтменитьЗакрытиеМесяца(СтруктураПараметров, Истина); 
		...
		Для Итератор = 0 По РазмерМассиваОпераций Цикл
			...
			Операция = Операции[Итератор];
			Если Операция = СекцияЗачетАвансаПоЕдиномуНалоговомуСчету() Тогда
				ДокументЗакрытияМесяца = ?(МассивДокументовЗМ.Количество() > 0, МассивДокументовЗМ[0], Неопределено); 				
				ВыполнитьОперациюЗакрытияМесяца(СтруктураПараметров, Операция, ДокументЗакрытияМесяца); 
			КонецЕсли;			
		КонецЦикла; 
	КонецЦикла;
...
Функция ТекстЗапросаОрганизацииИПЕдиныйНалоговыйСчет() 
	
	Возврат  
	"ВЫБРАТЬ
	|	РасчетыПоНалогамНаЕдиномНалоговомСчетеОбороты.Организация КАК Организация
	|ИЗ
	|	РегистрНакопления.РасчетыПоНалогамНаЕдиномНалоговомСчете.Обороты(&НижняяГраница, &ВерхняяГраница, Месяц, ) КАК РасчетыПоНалогамНаЕдиномНалоговомСчетеОбороты
	|ГДЕ
	|	НЕ РасчетыПоНалогамНаЕдиномНалоговомСчетеОбороты.Организация.Ссылка = &Компания
	|
	|СГРУППИРОВАТЬ ПО
