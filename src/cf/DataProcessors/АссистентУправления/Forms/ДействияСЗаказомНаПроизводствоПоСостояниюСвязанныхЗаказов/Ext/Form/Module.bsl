...
Процедура СостояниеЗаказаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ИндексБлока = ИндексБлока(Элемент.Имя);
	СтрокаНастроек = НастройкиЗадач.Получить(ИндексБлока);
...
&НаСервере
Процедура ЗаполнитьОписаниеЗадачиИзмененияПоЗаказамПокупателей()
	...
	Заголовок = НСтр("ru = 'Изменение состояния заказа на производство по состоянию заказов покупателей'");
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьОписаниеЗадачиСообщенияПоЗаказамПокупателей()
	...
	+ Символы.НПП;
	Элементы.ДопФункция_0.Заголовок = НСтр("ru = 'Заказы покупателей вернулись в более раннее состояние'") + Символы.НПП;
	
	Элементы.КартинкаЗадачи.Картинка = БиблиотекаКартинок.АссистентУправленияСообщениеПродажи;
	Заголовок = НСтр("ru = 'Сообщение заказу на производство по состоянию заказов покупателей'");
...
Процедура ОпределитьЗаголовкиПолей(ПолеТипПредмета, ДекорацияТекстОбсуждение)
	...
	ДекорацияТекстОбсуждение.Заголовок = НСтр("ru = 'напиши в обсуждение связанного с ними заказа на производство и'") + Символы.НПП;
	НадписьЗаказПокупателя = НСтр("ru = 'Когда заказы покупателей'");
