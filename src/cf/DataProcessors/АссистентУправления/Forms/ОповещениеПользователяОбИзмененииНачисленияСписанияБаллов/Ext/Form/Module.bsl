...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИдентификаторГруппы = "ОповещениеПользователяОбИзмененииБонусовПриПродаже";
...
Процедура ТекстОшибкиSMSНажатие(Элемент)
	ОткрытьФорму("ОбщаяФорма.НастройкаОтправкиSMS",,ЭтотОбъект,,,,Новый ОписаниеОповещения("ОтправкаSMSПроверкаНастроек", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ТипПредметаПриИзменении(Элемент)
	...
	Если СтрокаНастроек.ТипПредмета = "ЗаказПокупателя" Тогда
		...
		Если Не СтрокаНастроек.СобытиеИдентификатор = "НачислениеБонусовПриПродаже" Тогда
			ИмяПоляСобытие = "СобытиеИдентификатор_" + Индекс;
			ПутьКДаннымСобытия = СтрШаблон("НастройкиЗадач[%1].СобытиеИдентификатор", Индекс);
			СтрокаНастроек.СобытиеИдентификатор = "НачислениеБонусовПриПродаже";
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Начисленные бонусы в заказе покупателя отображаются справочно.
			|Фактически, начисления бонусов происходит во время проведения документов отгрузки по заказу. 
			|Событие отправки в настройке оповещения изменено на ""списание бонусов""'"),, ИмяПоляСобытие, ПутьКДаннымСобытия);
		КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораЭлементовФормы()
	...
	Для Итератор = 0 По НастройкиЗадач.Количество() - 1 Цикл
		...
		Элементы["ТипОбсуждения_" + Итератор].СписокВыбора.Добавить("Документ", НСтр("ru='привязанное к документу'"));
		...
		Элементы["ТипПредмета_" + Итератор].СписокВыбора.Добавить("Отгрузка", НСтр("ru='оптовой отгрузки'"));
		Если ВозможноИзменениеЧеков Тогда
			Элементы["ТипПредмета_" + Итератор].СписокВыбора.Добавить("ЧекККМ", НСтр("ru='розничной продажи'"));
		КонецЕсли;

		Элементы["СобытиеИдентификатор_" + Итератор].СписокВыбора.Очистить();
		Элементы["СобытиеИдентификатор_" + Итератор].СписокВыбора.Добавить("НачислениеБонусовПриПродаже", НСтр("ru='начисление бонусов'"));
		Если Не НастройкиЗадач[Итератор].ТипПредмета = "ЗаказПокупателя" Тогда
			Элементы["СобытиеИдентификатор_" + Итератор].СписокВыбора.Добавить("СписаниеБонусовПриПродаже", НСтр("ru='списание бонусов'"));
			Элементы["СобытиеИдентификатор_" + Итератор].СписокВыбора.Добавить("СписаниеНачислениеБонусовПриПродаже", НСтр("ru='начисление и списание бонусов'"));
		КонецЕсли;
...
Процедура ОбновитьЭлементыДействийАссистента()
	...
	Для Каждого СтрокаНастроек Из НастройкиЗадач Цикл
		...
		Если ИндексНастройки > 0 Тогда
			...
			ПолеТипПредмета_ = Элементы.Добавить("ТипПредмета_" + ИндексНастройки, Тип("ПолеФормы"), ГруппаВторойСтрокиДействия);
			ПолеТипПредмета_.ПутьКДанным = "НастройкиЗадач[" + ИндексНастройки + "].ТипПредмета";
			УстановитьОформлениеПоля(ПолеТипПредмета_, Элементы.ТипПредмета_0);
			ПолеТипПредмета_.УстановитьДействие("ПриИзменении","ТипПредметаПриИзменении");
...
Процедура УправлениеФормой()
	...
	ВРаботе = НЕ ЕстьОшибкиSMS И НЕ ЕстьОшибкиПодключенияБазы И ВРаботе;
...
Процедура ОбновитьБлокиНастроекАссистента()
	...
	Для каждого Задача Из ОтобранныеЗадачи Цикл
		...
		ДанныеЗадачи       = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Задача, 
			"Используется, ДействиеИдентификатор, ТипПредмета");
		...
		СтрокаНастроек.СобытиеИдентификатор = ИдентификаторСобытия(СобытиеИдентификатор);
...
Функция ИдентификаторСобытия(Событие)
	
	Результат = "СписаниеНачислениеБонусовПриПродаже";
	Если Событие = "ПриИзмененииСписанияБонусовЗаказПокупателя"
		ИЛИ Событие = "ПриИзмененииСписанияБонусовОтгрузка"
		ИЛИ Событие = "ПриИзмененииСписанияБонусовЧекККМ" Тогда
		Результат = "СписаниеБонусовПриПродаже";
	ИначеЕсли Событие = "ПриИзмененииНачисленияБонусовЗаказПокупателя"
		ИЛИ Событие = "ПриИзмененииНачисленияБонусовОтгрузка"
		ИЛИ Событие = "ПриИзмененииНачисленияБонусовЧекККМ" Тогда
		Результат = "НачислениеБонусовПриПродаже";
	КонецЕсли;
...
Процедура ДобавитьПустыеНастройкиЗадачи()
	...
	ОбщиеНастройки.Вставить("ТипПредмета",       "ЗаказПокупателя");
	ОбщиеНастройки.Вставить("ТипОповещения",     "Обсуждение");
	ОбщиеНастройки.Вставить("ТипОбсуждения",     "Документ");
...
Процедура СоздатьИзменитьЗадачиАссистента()
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(ГруппаЗадач) Тогда
			...
			ГруппаЗадач.Наименование = НСтр("ru = 'Оповестить пользователя об изменении начисленных/списанных бонусов в документе продажи'");
		...
		Для каждого СтрокаНастроек Из НастройкиЗадач Цикл
			...
			ЗадачаОбъект.МодульМенеджера = "РаботаСБонусами";
			...
			НовоеСобытие.СобытиеИдентификатор = СобытиеПоТипуПредмета(СтрокаНастроек.ТипПредмета, СтрокаНастроек.СобытиеИдентификатор);
...
Процедура ЗаполнитьЗначенияЗаполненияПоДействию(ЗадачаОбъект, СтрокаНастроек)
	...
	Если СтрНайти(ЗадачаОбъект.ДействиеИдентификатор, "Обсуждении") Тогда
		...
		Если СтрокаНастроек.ТипОбсуждения = "Документ" Тогда
...
Функция ДействиеПоТипуОповещения(ТипОповещения)
	...
	Если ТипОповещения = "Обсуждение" Тогда
		Возврат "ОповеститьСотрудникаОбИзмененииБонусовВОбсуждении";
	КонецЕсли;

	Если ТипОповещения = "Email" Тогда
		Возврат "ОповеститьСотрудникаОбИзмененииБонусовПоEmail";
	КонецЕсли;
	
	Если ТипОповещения = "SMS" Тогда
		Возврат "ОповеститьСотрудникаОбИзмененииБонусовПоSMS";
	КонецЕсли;
...
Процедура ЗначенияЗадачиПоДействию(СтрокаНастроек, ДанныеЗадачи, ЗначенияЗаполнения)
	...
	Если СтрНайти(ДанныеЗадачи.ДействиеИдентификатор, "Обсуждении") Тогда
		...
		Иначе
			СтрокаНастроек.ТипОбсуждения = "Документ";
		КонецЕсли;
...
&НаСервере
Функция СобытиеПоТипуПредмета(ТипПредмета, Событие)
	
	Если ТипПредмета = "ЗаказПокупателя" И Событие = "СписаниеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииСписанияБонусовЗаказПокупателя";
	КонецЕсли;

	Если ТипПредмета = "Отгрузка" И Событие = "СписаниеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииСписанияБонусовОтгрузка";
	КонецЕсли;
	
	Если ТипПредмета = "ЧекККМ" И Событие = "СписаниеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииСписанияБонусовЧекККМ";
	КонецЕсли;
	
	Если ТипПредмета = "ЗаказПокупателя" И Событие = "НачислениеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииНачисленияБонусовЗаказПокупателя";
	КонецЕсли;

	Если ТипПредмета = "Отгрузка" И Событие = "НачислениеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииНачисленияБонусовОтгрузка";
	КонецЕсли;
	
	Если ТипПредмета = "ЧекККМ" И Событие = "НачислениеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииНачисленияБонусовЧекККМ";
	КонецЕсли;
	
	Если ТипПредмета = "ЗаказПокупателя" И Событие = "СписаниеНачислениеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииСписанияНачисленияБонусовЗаказПокупателя";
	КонецЕсли;

	Если ТипПредмета = "Отгрузка" И Событие = "СписаниеНачислениеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииСписанияНачисленияБонусовОтгрузка";
	КонецЕсли;
	
	Если ТипПредмета = "ЧекККМ" И Событие = "СписаниеНачислениеБонусовПриПродаже" Тогда
		Возврат "ПриИзмененииСписанияНачисленияБонусовЧекККМ";
	КонецЕсли;
...
Функция НаименованиеПоТипуОповещения(ТипОповещения)
	...
	Если ТипОповещения = "Обсуждение" Тогда
		Возврат НСтр("ru = 'Оповещение пользователя в обсуждение об изменении бонусов'");
	КонецЕсли;
	
	Если ТипОповещения = "Email" Тогда
		Возврат НСтр("ru = 'Оповещение пользователя по почте об изменении бонусов'");
	КонецЕсли;
	
	Если ТипОповещения = "SMS" Тогда
		Возврат НСтр("ru = 'Оповещение пользователя по смс об изменении бонусов'");
	КонецЕсли;
...
Функция ИндексБлока(ЭлементИмя)
	...
	ИндексСтрокой = Прав(ЭлементИмя, СтрДлина(ЭлементИмя) - СтрНайти(ЭлементИмя, "_"));
	
	ОписаниеТипа = Новый ОписаниеТипов("Число");
	Возврат ОписаниеТипа.ПривестиЗначение(ИндексСтрокой);
...
Процедура ДобавитьНовыеНастройкиЗадачи()
	...
	ОбщиеНастройки.Вставить("ТипОбсуждения", "Документ");
	ОбщиеНастройки.Вставить("ТипОповещения", "Обсуждение");
	ОбщиеНастройки.Вставить("СобытиеИдентификатор", "СписаниеНачислениеБонусовПриПродаже");
	ОбщиеНастройки.Вставить("Модифицированность", Истина);
