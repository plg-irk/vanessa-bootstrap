...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИдентификаторГруппы = "ОповещениеПользователяОбИзмененииЗадачаСотрудника";
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого СтрокаНастроек Из НастройкиЗадач Цикл
		...
		Если (СтрокаНастроек.КалендарьСотрудникСотрудники = 1 ИЛИ СтрокаНастроек.КалендарьСотрудникСотрудники = 2)
				И НЕ ЗначениеЗаполнено(СтрокаНастроек.КонтролируемыйКалендарь) Тогда
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если СтрокаНастроек.КалендарьСотрудникСотрудники = 3 И СтрокаНастроек.КонтролируемыеСотрудники.Количество() = 0 Тогда
			ЕстьОшибки = Истина;
		КонецЕсли;        
		
		//Если СтрокаНастроек.КалендарьСотрудникСотрудники <> 1 И НЕ ЗначениеЗаполнено(СтрокаНастроек.ОтветственныйИлиАвтор)  Тогда
		//	ЕстьОшибки = Истина;
		//КонецЕсли;
...
Процедура Подключаемый_КалендарьСотрудникСотрудники_ОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	Если НастройкиЗадач[Число(Индекс)].КалендарьСотрудникСотрудники <> ВыбранноеЗначение Тогда
		...
		НастройкиЗадач[Число(Индекс)].ОтветственныйИлиАвтор = Неопределено; 
...
&НаКлиенте
Процедура Подключаемый_ОтветственныйИлиАвтор_ПриИзменении(Элемент)
	УстановитьМодифицированностьПриИзмененииПоля(Элемент.Имя);  
КонецПроцедуры                              
#КонецОбласти
...
Процедура ЗаполнитьСписокВыбораЭлементовФормы()
	...
	Для Итератор = 0 По НастройкиЗадач.Количество() - 1 Цикл
		...
		Элементы["КалендарьСотрудникСотрудники_" + Итератор].СписокВыбора.Добавить(0, НСтр("ru='по сотруднику'"));
		Элементы["КалендарьСотрудникСотрудники_" + Итератор].СписокВыбора.Добавить(1, НСтр("ru='по календарю'"));
		Элементы["КалендарьСотрудникСотрудники_" + Итератор].СписокВыбора.Добавить(2, НСтр("ru='по сотрудникам календаря'"));
		Элементы["КалендарьСотрудникСотрудники_" + Итератор].СписокВыбора.Добавить(3, НСтр("ru='по нескольким сотрудникам'"));

		Элементы["ОтветственныйИлиАвтор_" + Итератор].СписокВыбора.Очистить();
		Элементы["ОтветственныйИлиАвтор_" + Итератор].СписокВыбора.Добавить(0, НСтр("ru='ответственный'"));
		Элементы["ОтветственныйИлиАвтор_" + Итератор].СписокВыбора.Добавить(1, НСтр("ru='автор'"));
		Элементы["ОтветственныйИлиАвтор_" + Итератор].СписокВыбора.Добавить(2, НСтр("ru='ответственный или автор'"));
...
Процедура ОбновитьЭлементыДействийАссистента()
	...
	Для Каждого СтрокаНастроек Из НастройкиЗадач Цикл
		...
		Если ИндексНастройки > 0 Тогда
			...
			ДекорацияНадпись = Элементы.Добавить("НадписьГдеСотрудник_" + ИндексНастройки, Тип("ДекорацияФормы"), ГруппаВторойСтрокиДействия);
			ДекорацияНадпись.Вид = Элементы.НадписьГдеСотрудник_0.Вид;
			ДекорацияНадпись.Заголовок = Элементы.НадписьГдеСотрудник_0.Заголовок;
			ДекорацияНадпись.Ширина = Элементы.НадписьГдеСотрудник_0.Ширина;
			
			ПолеОтветственныйИлиАвтор = Элементы.Добавить("ОтветственныйИлиАвтор_" + ИндексНастройки, Тип("ПолеФормы"), ГруппаВторойСтрокиДействия);
			ПолеОтветственныйИлиАвтор.ПутьКДанным = "НастройкиЗадач[" + ИндексНастройки + "].ОтветственныйИлиАвтор";
			УстановитьОформлениеПоля(ПолеОтветственныйИлиАвтор, Элементы.ОтветственныйИлиАвтор_0);
			ПолеОтветственныйИлиАвтор.УстановитьДействие("ПриИзменении", "Подключаемый_ОтветственныйИлиАвтор_ПриИзменении");
...
Процедура УправлениеФормой()
	...
	Для Каждого СтрокаНастроек Из НастройкиЗадач Цикл
		...
		ПолеОтветственныйИлиАвтор    = Элементы["ОтветственныйИлиАвтор_"+ИндексНастройки];    
		НадписьГдеСотрудник          = Элементы["НадписьГдеСотрудник_"+ИндексНастройки];    
		...
		ПолеКонтролируемыйКалендарь.Видимость = (СтрокаНастроек.КалендарьСотрудникСотрудники = 1 ИЛИ СтрокаНастроек.КалендарьСотрудникСотрудники = 2); 
		ПолеКонтролируемыеСотрудники.Видимость = (СтрокаНастроек.КалендарьСотрудникСотрудники = 3); 
        НадписьГдеСотрудник.Видимость = (СтрокаНастроек.КалендарьСотрудникСотрудники <> 1);
       	ПолеОтветственныйИлиАвтор.Видимость = (СтрокаНастроек.КалендарьСотрудникСотрудники <> 1);	
...
Процедура УдалитьОдинаковыеДействияИзНастроек()
	...
	Для Каждого СтрокаНастроек Из НастройкиЗадач Цикл
		...
		ИначеЕсли СтрокаНастроек.КалендарьСотрудникСотрудники = 1 ИЛИ СтрокаНастроек.КалендарьСотрудникСотрудники = 2 Тогда 
...
Функция ЗадачиПоИдентификатору()
	...
	|ГДЕ
	|	ЗадачиАссистентаУправления.Идентификатор В(&Идентификатор)";
	
	ИдентификаторЗадач = Новый Массив;
	ИдентификаторЗадач.Добавить("ОповеститьСотрудникаОбИзмененииЗадачиСотрудникаВОбсуждении");
	ИдентификаторЗадач.Добавить("ОповеститьСотрудникаОбИзмененииЗадачиСотрудникаПоEmail");
...
Процедура ОбновитьБлокиНастроекАссистента()
	...
	Для каждого Задача Из СписокЗадач Цикл
		...
		СтруктураНастроекЗадачи = ДанныеЗадачи.ЗначенияЗаполнения[0].Значение.Получить();
		
		СтрокаНастроек.КонтролируемыйСотрудник	 = СтруктураНастроекЗадачи.КонтролируемыйСотрудник;
		СтрокаНастроек.КонтролируемыйКалендарь	 = СтруктураНастроекЗадачи.КонтролируемыйКалендарь;
		СтрокаНастроек.КалендарьОтправки	     = СтруктураНастроекЗадачи.КалендарьОтправки;
		...
		СтрокаНастроек.КалендарьСотрудникСотрудники = СтруктураНастроекЗадачи.КалендарьСотрудникСотрудники;
		СтрокаНастроек.ОтветственныйИлиАвтор = СтруктураНастроекЗадачи.ОтветственныйИлиАвтор;
		Для Каждого Сотрудник Из СтруктураНастроекЗадачи.МассивСотрудников Цикл  
			СтрокаНастроек.КонтролируемыеСотрудники.Добавить(Сотрудник);	
		КонецЦикла;
		ЗначенияЗадачиПоИдентификатору(СтрокаНастроек, ДанныеЗадачи, СтруктураНастроекЗадачи );
...
Процедура СоздатьИзменитьЗадачиАссистента()
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(ГруппаЗадач) Тогда
			...
			ГруппаЗадач.Наименование = НСтр("ru = 'Оповестить пользователя об изменениях в задаче'");
		...
		Для каждого СтрокаНастроек Из НастройкиЗадач Цикл
			...
			ЗадачаОбъект.ДействиеИдентификатор = ДействиеПоТипуОповещения(СтрокаНастроек.ТипОповещения);
			ЗадачаОбъект.МодульМенеджера = "ЗадачиСотрудника";
			ЗадачаОбъект.ТипПредмета = "ЗадачаСотрудника";
		
			ЗаполнитьСпособОповещенияПоПредставлению(СтрокаНастроек, ЗадачаОбъект);
			ЗадачаОбъект.Используется		= ВРаботе;  
			
			СтруктураНастроекЗадачи = Новый Структура;
			СтруктураНастроекЗадачи.Вставить("КонтролируемыйСотрудник", СтрокаНастроек.КонтролируемыйСотрудник);	
			СтруктураНастроекЗадачи.Вставить("КонтролируемыйКалендарь", СтрокаНастроек.КонтролируемыйКалендарь);	
			СтруктураНастроекЗадачи.Вставить("КалендарьОтправки", СтрокаНастроек.КалендарьОтправки);	
			СтруктураНастроекЗадачи.Вставить("КалендарьСотрудникСотрудники", СтрокаНастроек.КалендарьСотрудникСотрудники);	
			СтруктураНастроекЗадачи.Вставить("ТипОбсуждения", СтрокаНастроек.ТипОбсуждения);	
			СтруктураНастроекЗадачи.Вставить("УчетнаяЗапись", СтрокаНастроек.УчетнаяЗапись);	
			СтруктураНастроекЗадачи.Вставить("ПользовательДляОповещения", СтрокаНастроек.ПользовательДляОповещения);
			СтруктураНастроекЗадачи.Вставить("ПользовательДляПисем", СтрокаНастроек.ПользовательДляПисем);
			СтруктураНастроекЗадачи.Вставить("ОтветственныйИлиАвтор", СтрокаНастроек.ОтветственныйИлиАвтор);
			...
			КонецЦикла;
			СтруктураНастроекЗадачи.Вставить("МассивСотрудников", МассивСотрудников);	
			
			ЗаполнитьЗначенияЗадачиПоИдентификатору(ЗадачаОбъект, СтрокаНастроек, СтруктураНастроекЗадачи);   
			ЗадачаОбъект.ЗначенияЗаполнения.Очистить();
			СтрокаНастройки = ЗадачаОбъект.ЗначенияЗаполнения.Добавить();
			СтрокаНастройки.Параметр = "СтруктураНастроек";
			СтрокаНастройки.Значение = Новый ХранилищеЗначения(СтруктураНастроекЗадачи);
			
			ЗадачаОбъект.События.Очистить();
			НовоеСобытие = ЗадачаОбъект.События.Добавить();
			НовоеСобытие.СобытиеИдентификатор = "ПриИзмененииДанныхЗадачаСотрудника";
...
&НаСервере
Процедура ЗаполнитьЗначенияЗадачиПоИдентификатору(ЗадачаОбъект, СтрокаНастроек, СтруктураНастроекЗадачи)
	...
	Если СтрНайти(ЗадачаОбъект.Идентификатор, "Обсуждении") > 0 Тогда
		...
		СтруктураНастроекЗадачи.Вставить("ИдентификаторОбщегоОбсуждения",  СтрокаНастроек.ОбсуждениеИдентификатор);
	КонецЕсли;
...
Функция ИдентификаторПоТипуОповещения(ТипОповещения)
	...
	Если ТипОповещения = "Обсуждение" Тогда
		Возврат "ОповеститьСотрудникаОбИзмененииЗадачиСотрудникаВОбсуждении";
	КонецЕсли;
	
	Если ТипОповещения = "Email" Тогда
		Возврат "ОповеститьСотрудникаОбИзмененииЗадачиСотрудникаПоEmail";
	КонецЕсли;
	
КонецФункции

&НаСервере
Процедура ЗначенияЗадачиПоИдентификатору(СтрокаНастроек, ДанныеЗадачи, СтруктураНастроекЗадачи)
	...
	Если СтрНайти(ДанныеЗадачи.Идентификатор, "Обсуждении") Тогда
		...
		СтрокаНастроек.ОбсуждениеИдентификатор = СтруктураНастроекЗадачи.ИдентификаторОбщегоОбсуждения;
		
		Если ЗначениеЗаполнено(СтрокаНастроек.ОбсуждениеИдентификатор) И СтруктураНастроекЗадачи.ТипОбсуждения = "Групповое" Тогда
	...
	Если СтрНайти(ДанныеЗадачи.Идентификатор, "Email") Тогда
		...
		СтрокаНастроек.УчетнаяЗапись		= СтруктураНастроекЗадачи.УчетнаяЗапись;
...
&НаСервере
Функция ДействиеПоТипуОповещения(ТипОповещения)
