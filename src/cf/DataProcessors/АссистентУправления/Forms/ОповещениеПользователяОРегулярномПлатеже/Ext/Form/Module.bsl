...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыРегулярныхПлатежей =
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Задача, "ПараметрыРегулярныхПлатежей").Выгрузить();
	ПустойСправочникЗадач = ПараметрыРегулярныхПлатежей.Количество() = 0;
...
Процедура ОбновитьБлокиНастроекАссистента()
	...
	Если ОтображатьОдинДокумент Тогда
		СтрокаТЧ = ПараметрыРегулярныхПлатежей.Найти(ИсходныйДокументОтбор, "ИсходныйДокумент");
		НастройкаЗадачиПоНовомуДокументу = СтрокаТЧ = Неопределено;
	Иначе	
		НастройкаЗадачиПоНовомуДокументу = Ложь;
	КонецЕсли;

	Если ПараметрыРегулярныхПлатежей.Количество() = 0 Или НастройкаЗадачиПоНовомуДокументу Тогда
