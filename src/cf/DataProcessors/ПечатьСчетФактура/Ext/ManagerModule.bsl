...
Функция ПолныйПутьКМакету(ДатаДокумента, ЭтоУПД, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	НачалоИспользованияСФ2026 = '20260101';
	...
	ИначеЕсли ДатаДокумента < НачалоИспользованияСФ1096 Тогда
		...
		Если ЕстьПрослеживаемыеТовары И ДатаДокумента >= НачалоИспользованияСФ2023Кв4 Тогда
			ПолныйПутьКМакету = Тело + ?(ЭтоУПД, "ПФ_MXL_УниверсальныйПередаточныйДокумент2023Кв4Прослеж",
				"ПФ_MXL_СчетФактура2023Кв4Прослеж");
		Иначе
			ПолныйПутьКМакету = Тело + ?(ЭтоУПД, "ПФ_MXL_УниверсальныйПередаточныйДокумент534", "ПФ_MXL_СчетФактура534")
				+ ?(ЕстьПрослеживаемыеТовары, "Прослеж", "");
		КонецЕсли;
	ИначеЕсли ДатаДокумента < НачалоИспользованияСФ2026 Тогда

		ПолныйПутьКМакету = Тело + ?(ЭтоУПД, "ПФ_MXL_УниверсальныйПередаточныйДокумент1096", "ПФ_MXL_СчетФактура1096")
			+ ?(ЕстьПрослеживаемыеТовары, "Прослеж", "");

	Иначе

		ПолныйПутьКМакету = Тело + ?(ЭтоУПД, "ПФ_MXL_УниверсальныйПередаточныйДокумент2026", "ПФ_MXL_СчетФактура2026")
			+ ?(ЕстьПрослеживаемыеТовары, "Прослеж", "");
	КонецЕсли;
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектовПечати Цикл
		...
		Если ТипЗнч(ДанныеОбъекта.ПлатежныеДокументы) = Тип("ТаблицаЗначений") Тогда
			...
			Для Каждого ПлатежныйДокумент Из ДанныеОбъекта.ПлатежныеДокументы Цикл
				...
				Если ЗначениеЗаполнено(ПлатежныйДокумент.НомерСчетаФактурыНаАванс) Тогда
					АвансовыеСчетаФактурыНомерДата = СтрШаблон(НСтр("ru='№ %1 от %2, исправление № %3 от %4 '"),
						ПлатежныйДокумент.НомерСчетаФактурыНаАванс, Формат(ПлатежныйДокумент.ДатаСчетаФактурыНаАванс,
						"ДФ=dd.MM.yyyy"), ПлатежныйДокумент.НомерИсправленияСчетаФактурыНаАванс, Формат(
						ПлатежныйДокумент.ДатаИсправленияСчетаФактурыНаАванс, "ДФ=dd.MM.yyyy"));
					АвансовыеСчетаФактурыТекст = АвансовыеСчетаФактурыТекст + ?(Не ЗначениеЗаполнено(
						АвансовыеСчетаФактурыТекст), "", "; ") + АвансовыеСчетаФактурыНомерДата;
				КонецЕсли;
		...
		Если ПустаяСтрока(ПредставлениеПлатежныхДокументов) Тогда
			Если ДанныеОбъекта.ВидОперации = Перечисления.ВидыОперацийСчетФактура.Аванс Или ДанныеОбъекта.ВидОперации
				= Перечисления.ВидыОперацийСчетФактура.НаАвансКомитента Тогда
				ПредставлениеПлатежныхДокументов = "-- от --";
			Иначе
				ПредставлениеПлатежныхДокументов = "   от   ";
			КонецЕсли;
		КонецЕсли;
		...
		ДанныеПечати.Вставить("АвансовыеСчетаФактуры", АвансовыеСчетаФактурыТекст);
		...
		Если ОбластиМакета.ОбластьПодвалСФаксимиле <> Неопределено Тогда
			...
			Если ДанныеОбъекта.ЭтоФизическоеЛицо Тогда
				ДанныеПечати.Вставить("Свидетельство", ПечатьДокументовУНФ.СвидетельствоОРегистрации(
				СведенияОПоставщике));
			КонецЕсли;
...
Процедура УчестьОсобенностиДокументаСчетФактура(ТаблицаДанных, СчетФактура, ДополнительныеСвойства)
	...
	ОсобенностиЗаполнения = Новый Структура("ЭтоЗаказНаряд, ЭтоОтчетКомиссионера, ЭтоОтчетОПереработке, ЭтоПодтверждениеНулевойСтавкиНДС", Ложь, Ложь, Ложь, Ложь);
	...
	Если ОсобенностиЗаполнения.ЭтоПодтверждениеНулевойСтавкиНДС Тогда
		
		Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
			
			РеквизитыДокументаОтгрузки = Новый Структура("Организация, Дата", СтрокаТаблицы.Организация, СтрокаТаблицы.ДатаДокумента);
			УчетНДС.ПодготовитьТаблицуДокументаДляПолученияСуммСтавка0(СтрокаТаблицы, Истина, РеквизитыДокументаОтгрузки);
...
Процедура УчестьОсобенностиДокументаУПД(СтрокаОписаниеСФ, ДополнительныеСвойства)
	...
	Если СтрокаОписаниеСФ.ДанныеДокумента.Колонки.Найти("ПлатежныеДокументы") = Неопределено Тогда
	    ...
	    ВерсияПостановленияНДС1137 = УчетНДСПереопределяемый.ВерсияПостановленияНДС1137(СтрокаОписаниеСФ.ОснованиеУПД.Дата);
		...
		Иначе
			ТаблицаПД = Документы.СчетФактура.АрхивныеАлгоритмыПолучитьДатаНомерДокументовОплаты(
			СтрокаОписаниеСФ.ОснованиеУПД);
		КонецЕсли;
		ТаблицаПД = Документы.СчетФактура.ПолучитьДатаНомерДокументовОплаты(СтрокаОписаниеСФ.ОснованиеУПД);
		...
		Если СтруктураДанных <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ОписаниеСчетФактуры.СчетФактура) Тогда
				...
				Если ОписаниеСчетФактуры.СчетФактура.ВидОперации = Перечисления.ВидыОперацийСчетФактура.Аванс
					Или ОписаниеСчетФактуры.СчетФактура.ВидОперации = Перечисления.ВидыОперацийСчетФактура.НаАвансКомитента
...
Функция ТаблицаДанныхИсходящегоДокументаФормата1_6_12(СтрокаОписаниеСФ, ИспользоватьФаксимиле, ЭтоУПД, Ошибки)
	...
	ДокументыОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаОписаниеСФ.СчетФактура, "ДокументыОснования").Выгрузить();
	...
	Для Каждого СтрокаОснования Из ДокументыОснования Цикл
		...
		Если ВидОперации = Перечисления.ВидыОперацийСчетФактура.Продажа
			И ТипЗнч(СтрокаОснования.ДокументОснование) = Тип("ДокументСсылка.ПодтверждениеНулевойСтавкиНДС") Тогда
			
			СтрокаОснования.ДокументОснование = Документы.ПодтверждениеНулевойСтавкиНДС.ПолучитьДокументОтгрузкиПодтвержденияНулевойСтавки(СтрокаОснования.ДокументОснование, СтрокаОписаниеСФ.СчетФактура);
			СтрокаОписаниеСФ.ОписаниеДокумента = "ПодтверждениеНулевойСтавкиНДС";
		...
		Иначе
			...
			ДополнительныеСвойства.Вставить("ЭтоПодтверждениеНулевойСтавкиНДС", СтрокаОписаниеСФ.ОписаниеДокумента = "ПодтверждениеНулевойСтавкиНДС");
...
Функция РеквизитыСчетаФактурыДляПечати(СчетФактура)
	...
	Если ТипЗнч(СчетФактура) = Тип("ДокументСсылка.СчетФактура") Тогда
		...
		|		НаименованиеДокументаОтгрузки КАК НаименованиеДокументаОтгрузки) КАК ДокументыОснования,
		|	СчетФактура.СводныйКомиссионный КАК СводныйКомиссионный,
		|	ЕСТЬNULL(СчетФактура.ПодписьРуководителя.Должность, ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)) КАК
		|		ДолжностьРуководителя,
		|	ЕСТЬNULL(СчетФактура.ПодписьРуководителя.РасшифровкаПодписи, """") КАК РасшифровкаПодписиРуководителя,
		|	ЕСТЬNULL(СчетФактура.ПодписьРуководителя.Факсимиле,
		|		ЗНАЧЕНИЕ(Справочник.ФизическиеЛицаПрисоединенныеФайлы.ПустаяСсылка)) КАК ФаксимилеРуководителя,
		|	ЕСТЬNULL(СчетФактура.ПодписьГлавногоБухгалтера.РасшифровкаПодписи, """") КАК РасшифровкаПодписиГлавногоБухгалтера,
		|	ЕСТЬNULL(СчетФактура.ПодписьГлавногоБухгалтера.Факсимиле,
		|		ЗНАЧЕНИЕ(Справочник.ФизическиеЛицаПрисоединенныеФайлы.ПустаяСсылка)) КАК ФаксимилеГлавногоБухгалтера,
		|	ЕСТЬNULL(СчетФактура.ПодписьКладовщика.Должность, ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)) КАК
		|		ДолжностьКладовщика,
		|	ЕСТЬNULL(СчетФактура.ПодписьКладовщика.РасшифровкаПодписи, """") КАК РасшифровкаПодписиКладовщика,
		|	ЕСТЬNULL(СчетФактура.ПодписьКладовщика.Факсимиле,
		|		ЗНАЧЕНИЕ(Справочник.ФизическиеЛицаПрисоединенныеФайлы.ПустаяСсылка)) КАК ФаксимилеКладовщика,
		...
		|			ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
		|		КОНЕЦ КАК ДатаИсправленияСчетаФактурыНаАванс) КАК ПлатежныеДокументы
		|ИЗ
