...
Функция ЧекКоррекцииСторно(Контекст)
	...
	Если ТипЗнч(Контекст.ДокументОснование) = Тип("ДокументСсылка.ЧекККМКоррекции") Тогда
			...
			Иначе
				ЗаполнитьПозицииЧекаИзXML(ЧекОбъект.ПозицииЧека, ОбщиеПараметры.ПозицииЧека, ЧекОбъект.ДатаКоррекции);
			КонецЕсли;
...
Функция ЧекКоррекции(Контекст)
	...
	Если Не ПустаяСтрока(Контекст.АдресСайта) И Контекст.РасчетВИнтернете Тогда
		ЧекОбъект.МестоРасчетов = Контекст.АдресСайта
	Иначе
...
Процедура ЗаполнитьПозицииЧекаИзXML(ПозицииЧека, ПозицииЧекаXML, ДатаСтавкиНДС)
	...
	Для Каждого ПозицияЧека Из ПозицииЧекаXML Цикл
		...
		ИначеЕсли ПозицияЧека.СтавкаНДС = 18 ИЛИ ПозицияЧека.СтавкаНДС = 118 Тогда
			НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(Перечисления.ВидыСтавокНДС.Общая, ДатаСтавкиНДС);
		ИначеЕсли ПозицияЧека.СтавкаНДС = 20 ИЛИ ПозицияЧека.СтавкаНДС = 120 Тогда
			НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(Перечисления.ВидыСтавокНДС.Общая, ДатаСтавкиНДС);
		ИначеЕсли ПозицияЧека.СтавкаНДС = 22 ИЛИ ПозицияЧека.СтавкаНДС = 122 Тогда
			НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(Перечисления.ВидыСтавокНДС.Общая, ДатаСтавкиНДС);
		Иначе
