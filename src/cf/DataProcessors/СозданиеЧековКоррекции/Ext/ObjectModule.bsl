...
Процедура ЗаполнитьНаОсновании(Основание, Отказ) Экспорт
	...
	Если ФискальнаяОперация.ОбщиеПараметры = Неопределено Тогда
		...
		ОбщиеПараметры = ОборудованиеЧекопечатающиеУстройства.ЗагрузитьДанныеФискализацииИзXML(ДанныеXMLКорректируемогоЧека);
	Иначе
	...
	Если ТипЗнч(ФискальнаяОперация.ДокументОснование) = Тип("ДокументСсылка.ЧекККМКоррекции") Тогда
		...
		Иначе
			ЗаполнитьПозицииЧекаИзXML(ОбщиеПараметры.ПозицииЧека, ДатаКоррекции);
		КонецЕсли;
...
Процедура ЗаполнитьПозицииЧекаИзXML(ПозицииЧекаXML, ДатаСтавкиНДС)
