...
Процедура ПробитьЧекВыполнитьЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если ПробитыйДокумент = Объект.ЧекСторно  Тогда
			...
			ЧекСторноСтатус = НСтр("ru = 'Записан и пробит'");
		...
		ИначеЕсли ПробитыйДокумент = Объект.ЧекСкорректированный Тогда
			...
			ЧекСкорректированныйСтатус = НСтр("ru = 'Записан и пробит'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьЧекаКоррекции" И
		...
		Если Параметр.ПробитЧек Тогда
			Статус = НСтр("ru = 'Записан и пробит'");
		КонецЕсли;
...
Процедура ПозицииЧекаНаименованиеПредметаРасчетаПриИзменении(Элемент)
	...
	Если ТекущаяСтрока <> Неопределено 
		...
		РеквизитыНоменклатуры = РеквизитыПредметаРасчета(ТекущаяСтрока.НаименованиеПредметаРасчета,
			Объект.Организация, Объект.СтруктурнаяЕдиница, Объект.Дата);
...
Функция РеквизитыПредметаРасчета(Знач НаименованиеПредметаРасчета, Знач Организация, Знач Склад, Знач ДатаОбработки = Неопределено)
	
	СтруктураРеквизитов = Новый Структура;
	СтруктураРеквизитов.Вставить("ЕдиницаИзмерения");
	СтруктураРеквизитов.Вставить("ВидСтавкиНДС");
	
	РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НаименованиеПредметаРасчета, СтруктураРеквизитов);
	
	НалогообложениеОрганизации = НалогиУНФ.НастройкиНДСОрганизации(Организация, Склад, ДатаОбработки);
	...
	ПараметрыНДСНоменклатуры.НалогообложениеНДС = НалогообложениеОрганизации.ТипНалогообложения;
	ПараметрыНДСНоменклатуры.ВидСтавкиНДСПоУмолчанию = НалогообложениеОрганизации.ВидСтавкиНДС;
	ПараметрыНДСНоменклатуры.Номенклатура = НаименованиеПредметаРасчета;
	ПараметрыНДСНоменклатуры.НоменклатураВидСтавкиНДС = РеквизитыНоменклатуры.ВидСтавкиНДС;
...
Процедура СоздатьЧеки(Команда)
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Объект.НеприменениеККТ ИЛИ Объект.ТипКоррекции = 2 Тогда
