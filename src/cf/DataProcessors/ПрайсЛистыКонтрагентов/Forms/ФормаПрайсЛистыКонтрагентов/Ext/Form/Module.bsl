...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КэшЗначений.Вставить("ВопросСозданияПервогоПрайсЛиста",	ПерсональныеНастройкиРУНФ.Прочитать("ВыводитьВопросСозданииПервогоПрайсЛистаКонтрагента"));
	...
	ОткрыватьПрайсЛистКонтрагентовКакСписок = ПерсональныеНастройкиРУНФ.Прочитать("ОткрыватьПрайсЛистКонтрагентовКакСписок");
	
	Если НЕ Параметры.Свойство("РабочийПрайсЛистКонтрагентов", РабочийПрайсЛистКонтрагентов) Тогда
		
		РабочийПрайсЛистКонтрагентов = ПерсональныеНастройкиРУНФ.Прочитать("РабочийПрайсЛистКонтрагентов");
...
Процедура ОткрыватьПрайсЛистКонтрагентовКакОбработкуПриИзменении(Элемент)
	...
	ПерсональныеНастройкиРУНФВызовСервера.Установить(ОткрыватьПрайсЛистКонтрагентовКакСписок,
...
Процедура Декорация1Нажатие(Элемент)
	...
	ОткрытьФорму("Обработка.ПрайсЛистыКонтрагентов.Форма.ФормаПодбораВидовЦенКонтрагентов", ПараметрыОткрытия,
...
Процедура Декорация2Нажатие(Элемент)
	...
	ОткрытьФорму("Обработка.ПрайсЛистыКонтрагентов.Форма.ФормаПодбораНоменклатурыКомпоновщик", ПараметрыОткрытия,
		ЭтотОбъект, , , , ОписаниеОповещения);
...
Процедура Декорация3Нажатие(Элемент)
	...
	ОткрытьФорму("Справочник.ПрайсЛистыКонтрагентов.Форма.ФормаОДокументе", ПараметрыОткрытия, ЭтотОбъект, , , ,
		ОписаниеОповещения);
...
Процедура ПоказатьСоветПользователю(ЗаголовокФормы, ТекстСообщения, ОписаниеОповещения, ВидимостьБольшеНеПоказывать)
	...
	ОткрытьФорму("Обработка.ПрайсЛистыКонтрагентов.Форма.ФормаСообщения", ПараметрыСовета, ЭтотОбъект, Истина, , ,
...
Процедура ИзменитьОбъектФормыПоСсылке(НовыеДанныеЭлементаСправочника = Неопределено)
	...
	ПерсональныеНастройкиРУНФ.Установить(Объект.Ссылка, "РабочийПрайсЛистКонтрагентов");
...
Процедура ПослеВопросаОСохраненииНастроекПервогоПрайсЛиста(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") Тогда
		...
		Если РезультатЗакрытия.НажатаКнопкаОК Тогда
			...
			ЗаписатьЭлементСправочникаПрайсЛисты("Сохранить");
			ПерсональныеНастройкиРУНФВызовСервера.Установить(Объект.Ссылка, "РабочийПрайсЛистКонтрагентов");
...
Процедура ЗаписатьЭлементСправочникаПрайсЛисты(Действие)
	...
	ИначеЕсли Действие = "Скопировать" Тогда
		...
		ОткрытьФорму("Справочник.ПрайсЛистыКонтрагентов.Форма.ФормаЭлемента", ПараметрыОткрытия, ЭтотОбъект);
...
Процедура ОбновитьСуществующийПрайсЛист(СсылкаОбновляемого, НовыеДанныеЭлементаСправочника)
	...
	Попытка
		...
		ПерсональныеНастройкиРУНФ.Установить(СсылкаОбновляемого, "РабочийПрайсЛистКонтрагентов");
