...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеСостоянияСобытияКалендаря" Тогда
		Если Параметр = Объект.СобытиеКалендаря И Источник <> ЭтаФорма И Окно <> Неопределено И Не Окно.Основное Тогда
			Закрыть();
		КонецЕсли;
...
Процедура ВзносыПодлежащиеУплатеНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("УменьшеноРанее", УменьшениеСтраховыеВзносыТекущийПатент);
	...
	ОткрытьФорму("Обработка.ОбработкиНалоговИОтчетности.Форма.РасшифровкаФиксированныхВзносовПатент",
		ПараметрыФормы,
		ЭтотОбъект, , , , ,
...
Процедура ПолучитьДанныеОтчетности()
	...
	Если УпрощенныйЗачетФиксированныхВзносов Тогда
		...
		ДоляДоходовПоПатенту = 100 * РегламентированнаяОтчетностьУСН.ДоляДоходовПатентнойСистемыНалогообложения(
			Объект.Организация,
			ДатаДокументаОбработкиСобытия,
			Периодичность);
		...
		ДатаДокументаОбработкиСобытия,
		Истина); 
		ВзносыСДоходов = РегламентированнаяОтчетностьУСН.ВзносыСДоходовПодлежащиеУплате(Объект.Организация, КонецКвартала(ДатаДокументаОбработкиСобытия));
		...
		РасходыНаСтраховыеВзносы = ВсегоРасходыСтраховыеВзносы;
		
		ЗачтеноПоЗаявлениям = Цел(РегламентированнаяОтчетностьУСН.ЗарезервированныеСуммыВзносовЗаНалоговыйПериод(
			Объект.Организация, НачалоГода(ДатаДокументаОбработкиСобытия), ДатаДокументаОбработкиСобытия));
		...
		Объект.Организация, ДатаДокументаОбработкиСобытия, , Ложь);
