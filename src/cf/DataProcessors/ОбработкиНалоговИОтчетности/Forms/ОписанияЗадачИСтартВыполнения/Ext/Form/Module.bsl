...
Процедура Подключаемый_ПроверитьВыполнениеДлительнойОперацииНаКлиенте()
	...
	Если СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "РасчетПоСтраховымВзносам") > 0 
		...
		ИЛИ СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "ПерсСведенияОФизлицах") > 0
		ИЛИ (СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "Форма4ФСС") > 0 
		И ДатаОбработкиСобытия > '20230101') Тогда
	...
	Если СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "РасчетПоСтраховымВзносам") = 0 
		...
		И СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "ПерсСведенияОФизлицах") = 0
		И (СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "Форма4ФСС") = 0
		И ДатаОбработкиСобытия > '20230101') Тогда
...
Процедура ОбработатьПараметрСобытияКалендаря()
	...
	ДатаОбработкиСобытия = СобытиеКалендаряДанные.ДатаДокументаОбработкиСобытия;
	ИспользуютсяЗаписиКУДиР = РегламентированнаяОтчетностьУСНКлиентСервер.ЗадачаКалендаряИспользуетЗаписиКУДиР(
		СобытиеКалендаряДанные.Задача);
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогиСотрудников Тогда
		...
		Элементы.ДекорацияНадписьПериодовДокументовНалоговСотрудников.Заголовок = 
     	...
     	Элементы.ДекорацияТекстОписание24.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Элементы.ДекорацияТекстОписание24.Заголовок,
	    День(РегламентированнаяОтчетностьУСН.ОпределитьСрокПлатежаПоЕНП(СобытиеКалендаряДанные.ДатаОкончанияСобытия)));
...
Процедура ВыполнитьРасчетНажатие()	
	...
	СоздатьОбновитьКонтрагентаЕНПНаСервере();
	
	Если НЕ ВыполнитьКонтроль() Тогда
		Возврат;
	КонецЕсли; 	

	Если ИспользуютсяЗаписиКУДиР Тогда
		
		ДлительнаяОперация = РегламентированнаяОтчетностьУСНВызовСервера.ЗапуститьФормированиеЗаписейКУДиРВФоне(
			УникальныйИдентификатор, ДатаОбработкиСобытия);
		ОжидатьЗавершенияПодготовкиРасчета(ДлительнаяОперация);
		Возврат;
	...
	Если Результат.Свойство("ИмяФормыРезультата") Тогда
		...
		Если СтрНайти(ИмяФормыРезультата, "РасчетПоСтраховымВзносам") > 0
			...
			ИЛИ СтрНайти(ИмяФормыРезультата, "ПерсСведенияОФизлицах") > 0
			ИЛИ (СтрНайти(ИмяФормыРезультата, "Форма4ФСС") > 0 
			И ДатаОбработкиСобытия > '20230101') Тогда
		...
		Иначе
			...
			Оповестить("ИзменениеСостоянияСобытияКалендаря", СобытиеКалендаря, ЭтотОбъект);
...
Процедура СоздатьОбновитьКонтрагентаЕНПНаСервере()
	ДанныеГосударственныхОрганов.ДобавитьОбновитьПолучательКазначейство();
КонецПроцедуры  

&НаКлиенте
Процедура ОжидатьЗавершенияПодготовкиРасчета(ДлительнаяОперация)
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗавершенаОперацияПодготовкиРасчета", ЭтотОбъект);
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
	ПараметрыОжидания.ВыводитьОкноОжидания = Истина;
	ПараметрыОжидания.ВыводитьПрогрессВыполнения = Ложь;
	ПараметрыОжидания.ВыводитьСообщения = Ложь;
	ПараметрыОжидания.ТекстСообщения = "Обновляются записи Книги учета доходов и расходов.";
...
&НаКлиенте
Процедура ЗавершенаОперацияПодготовкиРасчета(Результат, ДополнительныеПараметры) Экспорт
	...
	РезультатОбработки = РегламентированнаяОтчетностьУСНКлиент.ОбработатьСобытиеКалендаря(
		Организация,
		СобытиеКалендаря);
	...
	Если РезультатОбработки.Свойство("ИмяФормыРезультата") Тогда
		ИмяФормыРезультата = РезультатОбработки.ИмяФормыРезультата;
		ПараметрыФормы = Новый Структура(
			"Организация, СобытиеКалендаря", 
			Организация,
			СобытиеКалендаря);
		...
		Оповестить("ИзменениеСостоянияСобытияКалендаря", СобытиеКалендаря, ЭтотОбъект);
		ОткрытьФорму(РезультатОбработки.ИмяФормыРезультата, ПараметрыФормы);
		Закрыть();
	КонецЕсли;
КонецПроцедуры
