...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьРеквизитыПоСерии(ЭтотОбъект, Серия);
	
	СообщениеОбОшибке = "";
	ЗаполнитьФлагиРаботыСМеткой(ЭтотОбъект, СообщениеОбОшибке);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен() И НЕ ТолькоПросмотр Тогда
		...
		ИначеЕсли ИмяСобытия = "RFID"
			...
			И RFIDКлиент.ДляОбработкиRFIDНуженСерверныйВызов(Параметр, ЭтотОбъект) Тогда
			ОбработатьСчитываниеRFID(Параметр);
		КонецЕсли;
		
		Если ЗавершитьПодбор
			И Не ЭтоПеремаркировкаТоваровГИСМ
			И ОбязательныеРеквизитыЗаполнены(НомерКиЗГИСМ, Ложь) Тогда
...
Процедура Далее(Команда)
	...
	Если Не ОбязательныеРеквизитыЗаполнены(НомерКиЗГИСМ) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьВRFID(Команда)
	...
	ПараметрыЗаписи.Вставить("ДанныеСерии",ДанныеСерии(ЭтотОбъект));
...
&НаСервереБезКонтекста
Функция ОбязательныеРеквизитыЗаполнены(Знач НомерКиЗГИСМ, ВыводитьСообщения = Истина)
...
Процедура ВернутьсяНаПредыдущийШаг()
	...
	СерияНоваяКешВвода = Новый ФиксированнаяСтруктура(ДанныеСерии(ЭтотОбъект));
...
Процедура ПерейтиНаСледующийШаг()
	...
	СерияСписываемаяКешВвода = Новый ФиксированнаяСтруктура(ДанныеСерии(ЭтотОбъект));
...
Функция РеквизитыВводаСерии()
	...
	РеквизитыВводаСерии = Новый Структура;
	РеквизитыВводаСерии.Вставить("Серия");
	РеквизитыВводаСерии.Вставить("EPCGTIN");
	РеквизитыВводаСерии.Вставить("RFIDEPC");
	РеквизитыВводаСерии.Вставить("RFIDTID");
	РеквизитыВводаСерии.Вставить("RFIDUser");
	РеквизитыВводаСерии.Вставить("ГоденДо");
	РеквизитыВводаСерии.Вставить("ЗаполненRFIDTID");
	РеквизитыВводаСерии.Вставить("НомерГИСМ");
	РеквизитыВводаСерии.Вставить("НомерКиЗГИСМ");
	РеквизитыВводаСерии.Вставить("НужноЗаписатьМетку");
	РеквизитыВводаСерии.Вставить("СтатусРаботыRFID");
...
Процедура ПодобратьСериюЗавершитьВвод()
	...
	ИначеЕсли НЕ ОбязательныеРеквизитыЗаполнены(НомерКиЗГИСМ)
...
Процедура НастроитьФормуПоШагу(Шаг)
	...
	Если Шаг = "Шаг1" Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, СерияСписываемаяКешВвода);
		...
		Заголовок = НСтр("ru = 'Укажите списываемую серию'");
		ИскомаяСерия = СерияСписываемая;
	Иначе
		
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, СерияНоваяКешВвода);
		...
		Заголовок = НСтр("ru = 'Укажите новую серию'");
...
Процедура ОбработатьСчитываниеRFID(ДанныеМеток)
	...
	Если ЗначениеЗаполнено(РезультатОбработки.ДанныеСерии) Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, РезультатОбработки.ДанныеСерии);
...
Процедура ЗаполнитьНомерГИСМ()
	...
	СерияНомераГИСМ = НайтиСерию();
	
	Если НЕ СерияНомераГИСМ.Пустая() Тогда
		
		НомерГИСМ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СерияНомераГИСМ, "НомерГИСМ");
