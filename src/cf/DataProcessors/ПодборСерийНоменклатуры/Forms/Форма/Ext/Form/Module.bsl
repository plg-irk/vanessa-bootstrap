...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТолькоПросмотр = СерииДоКорректировки;
	//@skip-check unknown-form-parameter-access
	Если Параметры.Свойство("ДокСсылка") Тогда
		ПоказыватьПроданныеПриПодборе = ЗначениеЗаполнено(Параметры.ДокСсылка)
			И ТипЗнч(Параметры.ДокСсылка) = Тип("ДокументСсылка.ОтчетКомитенту");
	КонецЕсли;
	...
	Если Параметры.Свойство("ДокСсылка") 
		...
		ДатаПроизводстваПриГенерацииСерий = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.ДокСсылка, "Дата");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Параметры.АдресВоВременномХранилище) Тогда
		...
		Иначе
			...
			Для Каждого СтрокаСерий Из Объект.СерииНоменклатуры Цикл
				...
				ДанныеСерииНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаСерий.Серия,
				 "Наименование,НомерКИЗГИСМ,RFIDTID,RFIDUser,RFIDEPC,EPCGTIN,НомерГИСМ,ГоденДо,ДатаПроизводства" );
	...
	Если КонтрольОстатковСерийНоменклатуры И Не ИгнорироватьКонтрольОстатков Тогда
		...
		СерииНоменклатурыОстатки.Параметры.УстановитьЗначениеПараметра("Номенклатура", Параметры.Запасы.Номенклатура);
		...
		Если Параметры.Свойство("СтруктурнаяЕдиница") Тогда
			...
			Если ЗначениеЗаполнено(Параметры.СтруктурнаяЕдиница) Тогда
				...
				Если ИспользоватьУникальныеСерии Тогда
					...
					Элементы.СерииОстаткиНаименование.Заголовок = СтрШаблон(НСтр("ru = 'В наличии на %1'"), Строка(Параметры.СтруктурнаяЕдиница));
				Иначе
					Элементы.СерииОстаткиСерия.Заголовок = Лев(СтрШаблон(НСтр("ru = 'В наличии на %1'"), Строка(Параметры.СтруктурнаяЕдиница)), 40);
					Элементы.СерииОстаткиНаименование.Заголовок = Лев(СтрШаблон(НСтр("ru = 'В наличии на %1'"), Строка(Параметры.СтруктурнаяЕдиница)), 40);
				КонецЕсли;
		...
		Если Параметры.Свойство("Ячейка") Тогда
			...
			Если ЗначениеЗаполнено(Параметры.Ячейка) Тогда
				...
				Элементы.СерииОстаткиНаименование.Заголовок = Элементы.СерииОстаткиНаименование.Заголовок + ", " + Параметры.Ячейка;
			КонецЕсли;
		Иначе
	...
	Иначе //без остатков
		...
		Элементы.СерииОстаткиНаименование.Заголовок = НСтр("ru = 'Серия'");
	...
	ДанныеНоменклатурыСерии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура,
		"ИспользоватьДатуПроизводстваСерии, ИспользоватьСрокГодностиСерии, ИспользоватьДатуПроизводстваСерии, ТочностьУказанияСрокаГодностиСерии, ЕдиницаИзмеренияСрокаГодности, СрокГодности, ПолитикаУчетаСерий");
	Элементы.СерииНоменклатурыНовыеДатаПроизводства.Видимость =
		ДанныеНоменклатурыСерии.ИспользоватьДатуПроизводстваСерии;
	Элементы.СерииНоменклатурыНовыеГоденДо.Видимость = ДанныеНоменклатурыСерии.ИспользоватьСрокГодностиСерии;
	
	Элементы.СерииОстаткиДатаПроизводства.Видимость = ДанныеНоменклатурыСерии.ИспользоватьДатуПроизводстваСерии;
	Элементы.СерииОстаткиГоденДо.Видимость = ДанныеНоменклатурыСерии.ИспользоватьСрокГодностиСерии;
	Элементы.СерииОстаткиНаименование.Видимость = ДанныеНоменклатурыСерии.ИспользоватьСрокГодностиСерии
		Или ДанныеНоменклатурыСерии.ИспользоватьДатуПроизводстваСерии;
	Элементы.СерииОстаткиСерия.Видимость = НЕ ДанныеНоменклатурыСерии.ИспользоватьСрокГодностиСерии
		И НЕ ДанныеНоменклатурыСерии.ИспользоватьДатуПроизводстваСерии;
	
	ТочностьУказанияСрокаГодностиСерии = ДанныеНоменклатурыСерии.ТочностьУказанияСрокаГодностиСерии;
	...
	ЕдиницаИзмеренияСрокаГодности = ДанныеНоменклатурыСерии.ЕдиницаИзмеренияСрокаГодности;
	СрокГодности = ДанныеНоменклатурыСерии.СрокГодности;
	
	СоздаватьШК = Ложь;
	Если ЗначениеЗаполнено(ДанныеНоменклатурыСерии.ПолитикаУчетаСерий) Тогда
		СоздаватьШК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеНоменклатурыСерии.ПолитикаУчетаСерий,
			"СоздаватьШК");
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		"ДекорацияИнформацияПоШтрихкодам", "Видимость", СоздаватьШК);
	
	УстановитьДоступностьЭлементовФормы();
	
	Если ТипЗнч(Номенклатура) = Тип("СправочникСсылка.КатегорииНоменклатуры") Тогда
		Элементы.Номенклатура.Заголовок = НСтр("ru = 'Категория'");
		ВладелецКатегория = Истина;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		...
		Если ИмяСобытия = "RFID" Тогда
			Если Не ИдетЗаписьМетки И RFIDКлиент.ДляОбработкиRFIDНуженСерверныйВызов(Параметр, ЭтотОбъект) Тогда
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСерииОстатки
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСерииНоменклатурыНовые
...
Процедура СерииНомерНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтруктураОтбора = Новый Структура("Отбор", Новый Структура("Владелец", Номенклатура));
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСерииНоменклатурыВыбранные
...
Процедура СерииВыбранныеНовыйНомерОткрытие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ОткрытьФорму("Справочник.СерииНоменклатуры.ФормаОбъекта", Новый Структура("Ключ",
		Элементы.СерииНоменклатурыВыбранные.ТекущиеДанные.Серия));
...
Процедура СерииВыбранныеПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Для каждого стр Из МассивСерийНоменклатуры Цикл
		...
		Если ДанныеСтроки<>Неопределено Тогда
			ДобавитьСерияВыбранный(ДанныеСтроки.Серия, ДанныеСтроки.Количество, ДанныеСтроки.ГоденДо, ДанныеСтроки.ДатаПроизводства, ДанныеСтроки.Наименование);
		КонецЕсли;
	КонецЦикла;
	...
	Если СохраненоУспешно Тогда
		...
		Если НЕ ОтключитьОбщееОповещение Тогда
			Оповестить("ПодборСерий", СтруктураВозврата, 
				?(УникальныйИдентификаторФормыВладельца =
					Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000"),
						Неопределено, УникальныйИдентификаторФормыВладельца));
		КонецЕсли;
...
Процедура СчитатьМетку(Команда)
	ОчиститьСообщения();
	RFIDКлиент.ОткрытьСессиюСчитывателяRFID(ЭтотОбъект);
...
Процедура ЗаписатьRFID(Команда)
	...
	ПараметрыЗаписи.Вставить("Форма", ЭтотОбъект);
...
Процедура ВыбратьНомерСерии()
	...
	Иначе
		ДобавитьСерияВыбранный(Элементы.СерииОстатки.ТекущиеДанные.Серия,
			Элементы.СерииОстатки.ТекущиеДанные.Количество,
			Элементы.СерииОстатки.ТекущиеДанные.ГоденДо,
			Элементы.СерииОстатки.ТекущиеДанные.ДатаПроизводства,
			Элементы.СерииОстатки.ТекущиеДанные.Наименование);
	КонецЕсли; 
...
Процедура ДобавитьСерияВыделенные(СерииНоменклатуры)
	...
	Для Каждого СерияНоменклатуры Из СерииНоменклатуры Цикл
		...
		ГоденДо = Элементы.СерииОстатки.ДанныеСтроки(СерияНоменклатуры).ГоденДо;
		ДатаПроизводства = Элементы.СерииОстатки.ДанныеСтроки(СерияНоменклатуры).ДатаПроизводства;
		НовыйНомер = Элементы.СерииОстатки.ДанныеСтроки(СерияНоменклатуры).Наименование;
		...
		НоваяСтрока.НовыйНомер = НовыйНомер;
...
Процедура НайтиЗарегистрированныеСерии()
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеСерии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаСерии.Серия,
			"Наименование, ГоденДо, ДатаПроизводства");
			
		СтрокаСерии.НовыйНомер = ДанныеСерии.Наименование;
		СтрокаСерии.ГоденДо = ДанныеСерии.ГоденДо;
		СтрокаСерии.ДатаПроизводства = ДанныеСерии.ДатаПроизводства;
...
Функция КонтролироватьОстатки(СтруктурнаяЕдиница, Организация)
	...
	ДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура,
		"ПолитикаУчетаСерий, ПолитикаУчетаСерий.ТипПолитики");
	
	ТипПолитикиУчетаСерий = ДанныеНоменклатуры.ПолитикаУчетаСерийТипПолитики;
						
	Если СтруктурнаяЕдиница = Неопределено Тогда

		Если ЗначениеЗаполнено(ДанныеНоменклатуры.ПолитикаУчетаСерий) Тогда
			Возврат ?(ТипПолитикиУчетаСерий = Перечисления.ТипыПолитикУказанияСерий.УправлениеОстаткамиСерий,
				Истина, Ложь);
		Иначе
	...
	Если Не Результат.Количество() Тогда
		Возврат ?(ТипПолитикиУчетаСерий = Перечисления.ТипыПолитикУказанияСерий.УправлениеОстаткамиСерий,
			Истина, Ложь);
	Иначе
		Возврат ?(Результат[0].ТипПолитики = Перечисления.ТипыПолитикУказанияСерий.УправлениеОстаткамиСерий,
			Истина, Ложь);
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьСерияВыбранный(Серия, КоличествоВСтроке = 1, ГоденДо = Неопределено,
	ДатаПроизводства = Неопределено, Наименование = "")
	...
	Если ИспользоватьУникальныеСерии Тогда
		...
		НоваяСтрока.ГоденДо = ГоденДо;
		НоваяСтрока.ДатаПроизводства = ДатаПроизводства;
		НоваяСтрока.НовыйНомер = Наименование;
...
Функция ТекстЗапросаСерииОстатки()
	...
	|	ВложенныйЗапросПереопределяемый.Серия.Наименование КАК Наименование,
	|	ВложенныйЗапросПереопределяемый.Серия.ДатаПроизводства КАК ДатаПроизводства,
	|	ВложенныйЗапросПереопределяемый.Серия.ГоденДо КАК ГоденДо
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	ВложенныйЗапросПереопределяемый.Серия.Наименование,
	|	ВложенныйЗапросПереопределяемый.Серия.ДатаПроизводства,
	|	ВложенныйЗапросПереопределяемый.Серия.ГоденДо";
...
Функция ТекстЗапросаСерииНоменклатуры()
	...
	|	1 КАК Количество,
	|	ВложенныйЗапрос.Серия.ГоденДо КАК ГоденДо,
	|	ВложенныйЗапрос.Серия.ДатаПроизводства КАК ДатаПроизводства,
	|	ВложенныйЗапрос.Серия.Наименование КАК Наименование
	|ИЗ
	|	(ВЫБРАТЬ
...
Процедура УстановитьУсловноеОформление()
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.СерииНоменклатуры.Серия", Справочники.СерииНоменклатуры.ПустаяСсылка(),
		ВидСравненияКомпоновкиДанных.НеРавно);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление,
		"СерииНоменклатурыНовыеДатаПроизводства,СерииНоменклатурыНовыеГоденДо");
...
Функция СохранитьВводСерийСервер()
	...
	АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(Объект.СерииНоменклатуры.Выгрузить(),
		УникальныйИдентификаторФормыВладельца);
...
Процедура СохранитьНастройки()
	...
	ХранилищеСистемныхНастроек.Сохранить(ИмяФормы, ИмяФормы + "_ШаблонСерииНоменклатуры", ШаблонСерииНоменклатуры);
...
Процедура ВосстановитьНастройки()
	...
	ШаблонСерииНоменклатуры = ХранилищеСистемныхНастроек.Загрузить(ИмяФормы, ИмяФормы
...
Процедура ЗаполнитьЗапасы(Запасы)
	...
	Если Запасы.Номенклатура.ИспользоватьСерииИзКатегории Тогда
		Номенклатура = Запасы.Номенклатура.КатегорияНоменклатуры;
	Иначе
...
Процедура ОтработатьТаймаутОжиданияСчитыванияМетки()
	...
	Если ОткрытаСессияСчитывателяRFID Тогда
		RFIDКлиент.ОтработатьТаймаутОжиданияСчитыванияМетки(ЭтотОбъект);
	КонецЕсли;
...
Функция ОбработатьСчитываниеRFID(ДанныеМеток, ИдентификаторТекущейСтроки)
	...
	РезультатОбработки = RFIDСервер.ОбработатьСчитываниеRFID(ДанныеМеток, ПараметрыОбработки,
		Объект.СерииНоменклатуры, ИдентификаторТекущейСтроки);
...
Функция ПроверитьМеткиНаНеобходимостьЗаписать()
	...
	Если ЭтоМаркировкаТоваровГИСМ
		...
		СтрокиСТребованиемЗаписи = Объект.СерииНоменклатуры.НайтиСтроки(Новый Структура("НужноЗаписатьМетку",
			Истина));
		
		Для Каждого Строка Из СтрокиСТребованиемЗаписи Цикл
			ТекстСообщения = СтрШаблон("%1 %2", 
				НСтр("ru='Необходимо записать данные в КИЗ с номером'"), Строка.НомерКИЗГИСМ);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , , , ЕстьНезаписанныеМетки);
		КонецЦикла;
			...
			Для Каждого ДанныеШтрихкода Из МассивДанныхШтрихкода Цикл
				...
				Если НЕ ЗначениеЗаполнено(ДанныеШтрихкода.ЕдиницаИзмерения) Тогда
					ДанныеШтрихкода.ЕдиницаИзмерения =
						ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеШтрихкода.Номенклатура, "ЕдиницаИзмерения");
				КонецЕсли;
				
				ТипНоменклатурыШК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеШтрихкода.Номенклатура,
					"ТипНоменклатуры");
				ДанныеШтрихкода.Вставить("ТипНоменклатуры", ТипНоменклатурыШК);
				...
				Если ЗначениеЗаполнено(ДанныеШтрихкода.ЕдиницаИзмерения)
					...
					КоэффициентЕдИзм = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеШтрихкода.ЕдиницаИзмерения,
						"Коэффициент");
					ДанныеШтрихкода.Вставить("Коэффициент", КоэффициентЕдИзм);
				Иначе
...
Процедура ЗаполнитьПоДаннымШтрихкодовЗавершение(СтруктураНоменклатуры, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДанныеШтрихкода) = Тип("Структура") И ДанныеШтрихкода.Свойство("ТипНоменклатуры") 
		...
		ШтрихкодыНекорректногоТипа.Добавить(Новый Структура("Штрихкод,Номенклатура,ТипНоменклатуры",
			ТекШтрихкод.Штрихкод, ДанныеШтрихкода.Номенклатура, ДанныеШтрихкода.ТипНоменклатуры));
...
Процедура ПолученыШтрихкоды(ДанныеШтрихкодов)
	...
	Для Каждого ТекШтрихкод Из МассивШтрихкодов Цикл
		ДополнительныеДанныеПоШтрихкоду = ШтрихкодированиеУНФКлиент.ДополнительныеДанныеПоШтрихкоду(
			ТекШтрихкод.Штрихкод, Неопределено);
	...
	Если ВладелецКатегория Тогда
		
		Если Не МассивШтрихкодов.Количество() > 0 Тогда
			Возврат;
		КонецЕсли;
		
		ТекШтрихкод = МассивШтрихкодов[0].Штрихкод;
		
		НайденнаяСерия = СерияКатегории(ТекШтрихкод);
		
		Если НайденнаяСерия = Неопределено Тогда
			
			ПараметрыОповещения = Новый Структура("Наименование", ТекШтрихкод);
			Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаСоздатьСериюКатегории", ЭтотОбъект, ПараметрыОповещения);
			
			Режим = РежимДиалогаВопрос.ДаНет;
			ПоказатьВопрос(Оповещение, НСтр("ru='Серия категории не найдена. Создать новую?'"), Режим, 0);
				
			Возврат
			
		КонецЕсли;
		
		УжеЕстьТакойЭлемент = Объект.СерииНоменклатуры.НайтиСтроки(Новый Структура("НовыйНомер", ТекШтрихкод));
		
		Если УжеЕстьТакойЭлемент.Количество() = 0 Тогда
			
			НоваяСтрока = Объект.СерииНоменклатуры.Добавить();
			НоваяСтрока.Серия = НайденнаяСерия;
			НоваяСтрока.НовыйНомер = ТекШтрихкод;
			НоваяСтрока.Количество = 1;
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаСоздатьСериюКатегории(Результат, ПараметрыОповещения) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		
		НоваяСерия = СоздатьСериюДляКатегории(ПараметрыОповещения.Наименование);
		
		Если НоваяСерия = Неопределено Тогда
			Возврат
		КонецЕсли;
		
		УжеЕстьТакойЭлемент = Объект.СерииНоменклатуры.НайтиСтроки(Новый Структура("НовыйНомер", ПараметрыОповещения.Наименование));
		
		Если УжеЕстьТакойЭлемент.Количество() = 0 Тогда
			
			НоваяСтрока = Объект.СерииНоменклатуры.Добавить();
			НоваяСтрока.Серия = НоваяСерия;
			НоваяСтрока.НовыйНомер = ПараметрыОповещения.Наименование;
			НоваяСтрока.Количество = 1;
...
&НаСервере
Функция СоздатьСериюДляКатегории(НаименованиеСерии)
			
	СправочникОбъект                = Справочники.СерииНоменклатуры.СоздатьЭлемент();
	СправочникОбъект.Владелец		= Номенклатура;
	СправочникОбъект.Наименование	= НаименованиеСерии;
	...
	Возврат ?(Не ОшибкаСоздания, СправочникОбъект.Ссылка, Неопределено);

КонецФункции

&НаСервере
Функция СерияКатегории(Штрихкод)
	...
	Возврат ?(НЕ Результат.Количество(), Неопределено, Результат[0].Серия);
