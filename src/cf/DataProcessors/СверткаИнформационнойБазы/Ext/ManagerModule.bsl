...
Функция НовыйВводНачальныхОстатков(ТекущийРегистр, ДатаСвертки, Автоформирование, Организация)
	...
	КонецЕсли;
	Если ИспользуетсяТаблицаЗакупкиКУДиР(НовыйВводНачальныхОстатков.РазделУчета) Тогда
		НовыйВводНачальныхОстатков.ЗакупкиКУДиРОтдельнымДокументом = Истина;
	КонецЕсли;
...
Функция ИспользуетсяТаблицаЗакупкиКУДиР(РазделУчета)
	
	Если РазделУчета = Перечисления.РазделыУчета.Запасы Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если РазделУчета = Перечисления.РазделыУчета.ЗакупкиТоваровДляУСН Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ЗаполнитьВводНачальныхОстатков(ДокументОбъект, ТекущийРегистр, ДатаСвертки, Организация)
	...
	ЗаполнитьВводНачальныхОстатковЗакупкиТоваровДляУСН(ДокументОбъект, ТекущийРегистр, ДатаСвертки, Организация);
КонецПроцедуры
...
Функция ТекстЗапросаЗакупкиТоваровДляУСН()
	...
	|ПОМЕСТИТЬ ВТОстатки
	|ИЗ
	|	РегистрНакопления.ПартииТоваровУСН.Остатки(&МоментВремени, Организация = &Организация
	|	И СтатусРасхода = ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеСписано)
	|	И УчитыватьВНУ
	|	И ЭтоТоварыКРеализации) КАК ПартииТоваровУСНОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ВТРеквизитыДокументовПартии
	|ИЗ
	|	ВТОстатки КАК ВТОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриходнаяНакладная КАК ПриходнаяНакладная
	|		ПО ВТОстатки.ДокументПартии = ПриходнаяНакладная.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ПО ВТОстатки.ДокументПартии = АвансовыйОтчет.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТОстатки.Сумма - ВТОстатки.СуммаНДС КАК Сумма,
	|	ВТОстатки.СуммаНДС КАК СуммаНДС,
	|	ЕСТЬNULL(ВТРеквизитыДокументовПартии.ДатаВходящегоДокумента, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаВходящегоДокумента,
	|	ЕСТЬNULL(ВТРеквизитыДокументовПартии.НомерВходящегоДокумента, """") КАК НомерВходящегоДокумента,
	|	ЕСТЬNULL(ВТРеквизитыДокументовПартии.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДокументПартииДата,
	|	ЕСТЬNULL(ВТРеквизитыДокументовПартии.Номер, """") КАК ДокументПартииНомер
	|ИЗ
	|	ВТОстатки КАК ВТОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРеквизитыДокументовПартии КАК ВТРеквизитыДокументовПартии
	|		ПО ВТОстатки.ДокументПартии = ВТРеквизитыДокументовПартии.Ссылка";
	
	Возврат Текст;
	
КонецФункции

Процедура ЗаполнитьВводНачальныхОстатковЗакупкиТоваровДляУСН(ВводНачальныхОстатковОбъект, ТекущийРегистр, ДатаСвертки,
	Организация)

	Если ТекущийРегистр <> Метаданные.РегистрыНакопления.ПартииТоваровУСН Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ УчетПартийУСН.ИспользуетсяРегистрПартииУСН(Организация, ДатаСвертки) Тогда
		Возврат;
	КонецЕсли;

	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаЗакупкиТоваровДляУСН();
	МоментВремени = Новый Граница(КонецДня(ДатаСвертки), ВидГраницы.Включая);
	Запрос.УстановитьПараметр("МоментВремени", МоментВремени);
	Запрос.УстановитьПараметр("ПериодЗаписи", ДатаСвертки);
	Запрос.УстановитьПараметр("Организация", Организация);
	...
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = ВводНачальныхОстатковОбъект.ЗакупкиДляКУДИР.Добавить();
		Если ЗначениеЗаполнено(Выборка.ДатаВходящегоДокумента) Тогда
			НоваяСтрока.ДатаДокумента = Выборка.ДатаВходящегоДокумента;
			НоваяСтрока.НомерДокумента = Выборка.НомерВходящегоДокумента;
		Иначе
			НоваяСтрока.ДатаДокумента = Выборка.ДокументПартииДата;
			НоваяСтрока.НомерДокумента = Выборка.ДокументПартииНомер;
		КонецЕсли;
		НоваяСтрока.Номенклатура = Выборка.Номенклатура;
		НоваяСтрока.Количество = Выборка.Количество;
		НоваяСтрока.Сумма = Выборка.Сумма;
