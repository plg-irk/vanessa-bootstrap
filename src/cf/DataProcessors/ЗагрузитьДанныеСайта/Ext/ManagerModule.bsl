...
Процедура ПрочитатьДанныеСайта(Параметры, ФоновоеЗаданиеАдресХранилища = "", ОжидаетсяЗагрузка = Ложь) Экспорт
	...
	Для каждого КаталогСайта Из СтруктураКаталогов Цикл
		...
		Для каждого пакетДанных Из ПакетыДанных Цикл
				...
				Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Свойства", Классификатор)
					...
					Для каждого СвойствоXDTO Из МассивСвойствXDTO Цикл
						...
						Если Параметры.ИспользоватьСопоставлениеПередЗагрузкой И МассивИдентификаторовКатегорий.Найти(СвойствоXDTOТипТовара) <> Неопределено
							...
							И (СвойствоXDTO.Общее = Истина Или НРег(СвойствоXDTO.Общее) = "истина" 
							Или НРег(СвойствоXDTO.Общее) = "true")) Тогда
							//@skip-check bsl-legacy-check-dynamic-feature-access
			...
			СформироватьЛогЗагрузки(Параметры.ШагиЗагрузки, НСтр("ru = 'Обработано объектов:'")
			+ " " + ПрогрессОбработки+"%", ПрогрессОбработки);
...
Процедура ЗагрузитьДанные(Параметры, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	СоздатьИлиНайтиСуществующий(СтруктураСайта, РегистрыСведений.ДанныеСайта, ДопРеквизиты);
...
Функция URLАдминзоны(Параметры, ИмяСайта, ОписаниеОшибки)
	...
	Если ОтветHTTP.КодСостояния<>200 Тогда
		ИнтеграцияСИнтернетМагазиномСервер.ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'Ошибка! Ответ HTTP:'")
...
Процедура ЗагрузитьЦены(ПредложениеXDTO, Номенклатура, Характеристика, Параметры)
	...
	Для каждого ЦенаXDTO Из МассивЦенXDTO Цикл
		...
		Если ЗначениеЗаполнено(ВидЦен) Тогда			
			...
			//@skip-check query-in-loop
			Если НЕ ЗначениеЗаполнено(РанееУстановленнаяВалютаВидаЦен)
			И ЭтоПерваяЦена(ВидЦен) Тогда
		...
		//@skip-check query-in-loop
		СоздатьИлиНайтиСуществующий(СтруктураОтбора, РегистрыСведений.ЦеныНоменклатуры, ДопРеквизиты);
...
Функция СоздатьИлиНайтиСуществующий(СтруктураОтбора, МенеджерОбъекта, ДопРеквизиты=Неопределено)
	...
	Если СтрНайти(ИмяСтрокой, "РегистрСведений") Тогда
		...
		ЗаполнитьЗначенияСвойств(МенеджерЗаписи, СтруктураОтбора);
		ЗаполнитьЗначенияСвойств(МенеджерЗаписи, ДопРеквизиты);
		МенеджерЗаписи.Записать();
	ИначеЕсли СтрНайти(ИмяСтрокой, "Справочник") 
	ИЛИ СтрНайти(ИмяСтрокой, "ПланВидовХарактеристик") Тогда
		...
		Если Результат.Пустой() Тогда
			...
			ЗаполнитьЗначенияСвойств(Объект, СтруктураОтбора);
			...
			Если Не СтруктураОтбора.Свойство("ЭтоГруппа") И СтрНайти(НРег(ИмяСтрокой), НРег("ВидыЦен")) <> 0 Тогда
				Если ЗначениеЗаполнено(Объект.ИдентификаторФормул) Тогда
					НовыйИдентификаторФормул = Объект.ИдентификаторФормул;
					ЦенообразованиеФормулыСервер.СформироватьНовыйИдентификаторВидаЦен(НовыйИдентификаторФормул, Объект.Наименование);
					Объект.ИдентификаторФормул = НовыйИдентификаторФормул;
				Иначе 
					НовыйИдентификаторФормул = Объект.Наименование;
					ЦенообразованиеФормулыСервер.СформироватьНовыйИдентификаторВидаЦен(НовыйИдентификаторФормул, Объект.Наименование);
					Объект.ИдентификаторФормул = НовыйИдентификаторФормул;
				КонецЕсли;
			КонецЕсли;
	...
	Пока НЕ КонецПередачи Цикл
		...
		Пока Истина Цикл
			...
			Иначе
				...
				СформироватьЛогЗагрузки(Параметры.ШагиЗагрузки, НСтр("ru = 'Прочитано объектов:'")
				+ " " + РазмерПакетаЗагрузки()*ПрочитаноПакетов);
				
			КонецЕсли;
			
		КонецЦикла;
		
		Параметры.ШагиЗагрузки.Вставить(2, НСтр("ru = 'Прочитано объектов:'")
		+ " " + РазмерПакетаЗагрузки()*ПрочитаноПакетов);
		СформироватьЛогЗагрузки(Параметры.ШагиЗагрузки);
