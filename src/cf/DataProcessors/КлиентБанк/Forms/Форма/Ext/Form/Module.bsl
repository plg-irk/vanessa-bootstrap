...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РасширениеРаботыСФайламиПодключено = Не ОбщегоНазначения.ЭтоВебКлиент();
	...
	ЦветОшибки = ЦветаСтиля.ПросроченноеЗадание;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеВыписки" И Источник <> ЭтотОбъект
		...
		Если НЕ ВыполняетсяПовторноеРаспознаваниеДанныхИзБанка И ДокументыКИмпорту.Количество() > 0 Тогда
			...
			ПодключитьОбработчикОжидания("Подключаемый_РаспознатьДанныеБанкаПовторно", 0.1, Истина);
		КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеВыгрузка() Экспорт
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПросроченноеЗадание);
...
Процедура ОбновитьСписокДокументовНаЭкспорт(Очищать = Истина)
	...
	Если НЕ ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.БанковскийСчет");
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонецПериода) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.КонецПериода");
		Возврат;
	КонецЕсли;
...
Функция ВыгрузитьДокументы(ВТекстовомФОрмате = Ложь)
	...
	СтруктураПараметов = Новый Структура();
	СтруктураПараметов.Вставить("ТаблицаДокументов", ТаблицаДокументов.Выгрузить());
	СтруктураПараметов.Вставить("ПлатежныеДокументы", Объект.ПлатежныеДокументы);
	СтруктураПараметов.Вставить("Кодировка", Объект.Кодировка);
	СтруктураПараметов.Вставить("Программа", Объект.Программа);
	СтруктураПараметов.Вставить("НачПериода",  Объект.НачалоПериода);
	СтруктураПараметов.Вставить("КонПериода", Объект.КонецПериода);
	СтруктураПараметов.Вставить("БанковскийСчет", Объект.БанковскийСчет);
	СтруктураПараметов.Вставить("ВерсияФормата", ВерсияФормата);
	СтруктураПараметов.Вставить("УникальныйИдентификатор", УникальныйИдентификатор);
	СтруктураПараметов.Вставить("ВТекстовомФормате", ВТекстовомФОрмате);

	Возврат Обработки.КлиентБанк.Выгрузить(СтруктураПараметов);
...
Процедура ЗаполнитьДокументыКИмпортуПоРаспознаннымДаннымИзБанка(ДокументыКИмпорту, РаспознанныеДанныеИзБанка, ВыводитьСообщения)
	...
	Если ВыводитьСообщения Тогда
		...
		Для Каждого ОшибкаГруппировкиВыписки Из РаспознанныеДанныеИзБанка.ПротоколГруппировкиВыписки Цикл
			ОбщегоНазначения.СообщитьПользователю(ОшибкаГруппировкиВыписки.ПолноеОписание);
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из РаспознанныеДанныеИзБанка.Выписки Цикл
		...
		Если ПредупреждениеНеНайденБанковскийСчет <> Неопределено Тогда
			Если ВыводитьСообщения Тогда
				ОбщегоНазначения.СообщитьПользователю(ПредупреждениеНеНайденБанковскийСчет.Текст);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеВСтрокеДокументыКИмпортуПоДаннымСтрокиСекции(СтрокаСекции, СтрокаТаблицы)
	...
	Если ТипЗнч(СтрокаТаблицы.Контрагент) = Тип("СправочникСсылка.Контрагенты")
	...
	СтрокаСекции.Загружать = Не ЗначениеЗаполнено(СтрокаТаблицы.Документ) И Не ЗначениеЗаполнено(СтрокаСекции.Документ);
...
Процедура СоздатьНенайденноеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ОшибкаСоздания Из Результат.МассивСообщений Цикл
		ОбщегоНазначенияКлиент.СообщитьПользователю(ОшибкаСоздания);
	КонецЦикла;
...
Функция ПрочитатьЭлектроннуюВыпискуБанкаНаСервере(ЭлектроннаяВыписка, СтекОповещений)
	...
	Исключение
		ОписаниеОшибки = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при удалении временного файла.'"), УровеньЖурналаРегистрации.Ошибка,
