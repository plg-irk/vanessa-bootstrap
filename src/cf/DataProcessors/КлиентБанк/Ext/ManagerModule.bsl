...
// Получает настройки программы клиента банка для организации и банковского счета.
//
// Параметры:
...
//  Структура - Настройки программы клиента банка:
//   * НастройкиЗагружены - Булево - Признак успешной загрузки настроек.
//   * ПрямойОбменТолькоЗагрузка - Булево - Признак того, что обмен с банком осуществляется только для загрузки данных.
//   * СоглашениеПрямогоОбменаСБанками - ПеречислениеСсылка.ПрограммыБанка, СправочникСсылка.НастройкиОбменСБанками, Неопределено - Программа банка для прямого обмена.
//   * БанкДляПрямогоОбмена - СправочникСсылка.Банки - Банк для прямого обмена.
//   * Кодировка - Неопределено - Кодировка обмена с банком.
//   * Программа - Строка - Программа обмена с банком.
//   * ФайлВыгрузки - Строка - Файл выгрузки данных для обмена с банком.
//   * ФайлЗагрузки - Строка - Файл загрузки данных для обмена с банком.
//   * КонтролироватьНекорректныеСимволыВНомере - Булево - Признак контроля некорректных символов в номере платежного документа.
//   * Платежное_Поручение - Булево - Признак выгрузки платежных поручений.
//   * Платежное_Требование - Булево - Признак выгрузки платежных требований.
//   * СоздаватьНенайденныеЭлементы - Булево - Признак создания ненайдененных элементов при обмене с банком.
//   * ПередЗагрузкойПоказыватьФормуОбменаСБанком - Булево - Признак показа формы обмена с банком перед загрузкой данных.
//   * КонтролироватьБезопасностьОбменаСБанком - Булево - Признак контроля безопасности обмена с банком.
//   * БанковскийСчет - СправочникСсылка.БанковскиеСчета - Банковский счет.
//   * Организация - Произвольный, Дата, Неопределено, СправочникСсылка.Организации - Организация.
//   * РежимИнтеграцииОграничиватьПериодВыписки - Булево - Признак ограничения периода выписки при интеграции с банком.
//   * АнализироватьИсториюВыбораЗначенийРеквизитов - Булево - Признак анализа истории выбора значений реквизитов.
//   * АвтоматическиПодставлятьДокументы - Булево - Признак автоматической подстановки документов.
//   * ИспользоватьПомощникСозданияСправочников - Булево - Признак использования помощника создания справочников.
//   * НеУдалятьДокументыКоторыхНетВВыписке - Булево - Признак запрета удаления документов, которых нет в выписке.
//   * СтатьяДДСВходящий - СправочникСсылка.СтатьиДвиженияДенежныхСредств - Статья движения денежных средств для входящих платежей.
//   * СтатьяДДСИсходящий - СправочникСсылка.СтатьиДвиженияДенежныхСредств - Статья движения денежных средств для исходящих платежей.
//   * СпособЗачета - ПеречислениеСсылка.СпособыЗачетаИРаспределенияПлатежей - Способ зачета платежей.
...
// 
// Возвращаемое значение:
//  Структура - Настройки загрузки:
// * СозданиеДокументов - Соответствие из КлючИЗначение:
//   ** Ключ - Тип - Тип загружаемого документа.
//   ** Значение - Булево - признак проведения загружаемых документов
// * ЗаполнениеНовыхЭлементов - Соответствие из КлючИЗначение:
//   ** Ключ - Тип - Тип создаваемого элемента справочника (Контрагенты).
//   ** Значение - Структура:
//   	*** Родитель - СправочникСсылка.Контрагенты - группа для новых контрагентов
// * Кодировка - Неопределено - Кодировка файла
...
// Функция возвращает таблицу значений с видами документов для клиента банка.
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица с колонками:
//    * Пометка        - Булево - признак выбора документа.
//    * Документ       - Строка - вид документа.
//    * Название       - Строка - название документа.
//    * Корреспондент  - Строка - корреспондент документа.
//    * ДокументКратко - Строка - краткое описание документа.
...
Функция Выгрузить(СтруктураПараметов) Экспорт
		
	ТаблицаДокументов = СтруктураПараметов.ТаблицаДокументов;
	ПлатежныеДокументы = СтруктураПараметов.ПлатежныеДокументы;
	Кодировка = СтруктураПараметов.Кодировка;
	Программа = СтруктураПараметов.Программа;
	НачПериода = СтруктураПараметов.НачПериода;
	КонПериода = СтруктураПараметов.КонПериода;
	БанковскийСчет = СтруктураПараметов.БанковскийСчет;
	ВерсияФормата = СтруктураПараметов.ВерсияФормата;
	КлючУникальности = СтруктураПараметов.УникальныйИдентификатор;
	ВТекстовомФормате = СтруктураПараметов.ВТекстовомФормате;
...
// Проверяет заполнение таблицы документов и выводит сообщения об ошибках или предупреждениях.
//
// Параметры:
//  Объект - Структура - Объект, содержащий таблицу документов.
//  Документ - ТаблицаЗначений - Таблица документов для проверки.
//  ЕстьСтрокиЭкспортаСОшибками - Булево - Флаг наличия строк с ошибками.
//
Процедура ПроверитьЗаполнениеТаблицыДокументов(Объект, Документ, ЕстьСтрокиЭкспортаСОшибками) Экспорт
	...
	Если ЕстьСтрокиЭкспортаСОшибками Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,, "Объект.ПлатежныеДокументы");
...
//
// Параметры:
//  ПолеОтчета - ПолеТабличногоДокумента - Поле табличного документа для вывода отчета.
//  Источник - ТаблицаЗначений - таблица значений с данными о выгруженных документах.
//  Организация - СправочникСсылка.Организации - Организация, по которой формируется отчет.
//  БанковскийСчет - СправочникСсылка.БанковскиеСчета - Банковский счет организации.
//  НачПериода - Дата - Начало периода для формирования отчета.
//  КонПериода - Дата - Конец периода для формирования отчета.
//
Процедура ПечатьОтчетаОВыгруженныхПлатежныхДокументах(ПолеОтчета, Источник, Организация, БанковскийСчет, НачПериода, КонПериода) Экспорт
	...
	НазвОрг.Параметры.НазваниеОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "Наименование");
	...
	Если НачПериода = КонПериода Тогда
		Шапка.Параметры.ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'по счету  %1 за %2'"), НомерСчета, Формат(НачПериода, "ДЛФ=D"));
	Иначе
		Шапка.Параметры.ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'по счету  %1 с %2 по %3'"), НомерСчета, Формат(НачПериода, "ДЛФ=D"), Формат(КонПериода, "ДЛФ=D"));
	КонецЕсли;
...
//
// Возвращаемое значение:
//  ДвоичныеДанные - Двоичные данные выгрузки документа.
//  Неопределено - Если документ не заполнен, не имеет счета организации или счет организации является валютным.
Функция ВыгрузитьДокумент(Документ) Экспорт
	...
	ДанныеДляЭкспортаПлатежногоПоручения = ДанныеДляЭкспортаПлатежногоПоручения(Документ);
...
// Процедура ограничивает период начала выписки банковского счета.
//
// Параметры:
//  СтруктураПериода - Структура:
//  * ДатаНачала - Дата - Ораганизация, по которой загружается выписка.
//  * ДатаОкончания - Дата - банковский счет, по которому загружается выписка.
//  НастройкиБанковскогоСчета - Структура - Настройки банковского счета с полями Организация, БанковскийСчет и ПрограммаБанка:
//  * Организация - СправочникСсылка.Организации - Ораганизация, по которой загружается выписка.
//  * БанковскийСчет - СправочникСсылка.БанковскиеСчета - банковский счет, по которому загружается выписка.
//  * ПрограммаБанка - ПеречислениеСсылка.ПрограммыБанка - программа банка.
//
...
Процедура ФоноваяЗагрузкаБанковскойВыпискиИзЖурнала(СтруктураПараметров, АдресХранилища,
	...
	НастройкиЗагрузки = НастройкиЗагрузки();
	...
	ДобавитьНастройкиПрограммыКлиентаБанкаВРаспознанныеВыписки(РаспознанныеДанныеИзБанка.РаспознанныеВыписки);
...
// Печатает отчет о загруженных платежных документах.
//
// Параметры:
//  ПолеОтчета - ПолеФормы - поле формы, в которое будет выведен отчет.
//  ДанныеФормы - Структура - данные формы, содержащие информацию о документах к импорту, организации, банковском счете и флаге создания несуществующих элементов.
//  АдресХранилищаРаспознанныеДанныеИзБанка - Строка - адрес временного хранилища, содержащий распознанные данные из банка.
//
...
Процедура ЗаполнитьДокументыНаЭкспортВФоне(ПараметрыЗадания, АдресХранилища) Экспорт
	...
	ЗаполнитьДокументыНаЭкспорт(ПараметрыЗадания);
	
	ПоместитьВоВременноеХранилище(ПараметрыЗадания.ПлатежныеДокументы, АдресХранилища);
	
КонецПроцедуры

Процедура ЗаполнитьДокументыНаЭкспорт(ПараметрыЗадания)
	
	Организация = ПараметрыЗадания.Организация;
	ОбъектПлатежныеДокументы = ПараметрыЗадания.ПлатежныеДокументы;
	НачПериода = ПараметрыЗадания.НачалоПериода;
	КонПериода = ПараметрыЗадания.КонецПериода;
	БанковскийСчет = ПараметрыЗадания.БанковскийСчет;
	ВыгружатьПлатПоручения = ПараметрыЗадания.ВыгружатьПлатПоручения;
	ВыгружатьПлатТребования = ПараметрыЗадания.ВыгружатьПлатТребования;
	КонтролироватьНекорректныеСимволыВНомере = ПараметрыЗадания.КонтролироватьНекорректныеСимволыВНомере;
	СоглашениеПрямогоОбменаСБанками = ПараметрыЗадания.СоглашениеПрямогоОбменаСБанками;
	СписокДокументов = ПараметрыЗадания.СписокДокументов;
	...
	ОбъектПлатежныеДокументы.Колонки.Добавить("СчетКонтрагентаВладелец", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
...
//
// Параметры:
//  ПлатежныеДокументы - ТаблицаЗначений - таблица обработки, содержащая список документов.
//
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - ДокументСсылка - ссылка на документ.
//  * Значение - Булево - флаг выгрузки.
//
...
Функция ПолучитьТаблицуДокументовДляЭкспорта(ПлатежныеПоручения, ПлатежныеТребования) Экспорт
	...
	|	ПлатежноеПоручение.СчетКонтрагента.Владелец КАК СчетКонтрагентаВладелец,
...
Процедура ПроверитьПлатежныеРеквизитыГосОрганов(ПлатежныеДокументы, Организация)
	...
	Для Каждого ПлатежныйДокумент Из ПлатежныеДокументы Цикл
		...
		Если ПлатежныйДокумент.ПеречислениеВБюджет И ПлатежныйДокумент.Готовность = 0 Тогда
			...
			ПлатежныеРеквизиты.Вставить("СчетКонтрагентаВладелец", ПлатежныйДокумент.СчетКонтрагентаВладелец);
	...
	Для Каждого СчетДляПроверки Из СчетаДляПроверки Цикл
		...
		Контрагент = СчетДляПроверки.Значение.СчетКонтрагентаВладелец;
...
// Процедура заполняет экспортные значения для выгрузки платежных документов.
//
// Параметры:
//  СтрокаЭкспорта - Структура - структура, в которую будут помещены заполненные значения.
//  ВыборкаДляЭкспорта - ВыборкаИзРезультатаЗапроса - выборка из результата запроса, содержащая данные для заполнения.
//
...
// Процедура добавляет замечание к строке документа.
//
// Параметры:
//  СтрокаДокумента - СтрокаТаблицыЗначений - строка таблицы значений, к которой добавляется замечание.
//  НоваяГотовность - Число - новое значение готовности.
//  ТекстЗамечания - Строка - текст замечания.
//
...
Функция СоздаватьНенайденныеЭлементы(РаспознанныеВыписки)
	...
	Для Каждого КлючИЗначение Из РаспознанныеВыписки Цикл
		...
		Если НЕ РаспознаннаяВыписка.НастройкиСчета.СоздаватьНенайденныеЭлементы Тогда
...
Функция ПередЗагрузкойПоказыватьФормуОбменаСБанком(РаспознанныеВыписки)
	...
	Для Каждого КлючИЗначение Из РаспознанныеВыписки Цикл
		...
		Если РаспознаннаяВыписка.НастройкиСчета.ПередЗагрузкойПоказыватьФормуОбменаСБанком
			И Не АвтоматическоеПолучениеВыпискиАктивно Тогда
...
// Проверяет корректность данных для выгрузки платежного документа.
//
// Параметры:
//  СтрокаДокумента - Структура - Строка таблицы значений с данными платежного документа.
	...
	Иначе
		...
		ИначеЕсли СтрНайти("ТП ,ЗД ", П106) > 0 Тогда
			Если СтрЗаменить(П107, "0", "") <> "" Тогда
				...
				ГГГГ = Сред((П107), 7, 4);
				Если НЕ ММММ = "" Тогда
					ММММ = Число(Сред((П107), 4, 2));
				Иначе
					ММММ = 0;
				КонецЕсли;
				Если НЕ ГГГГ = "" Тогда
					ГГГГ = Число(Сред((П107), 7, 4));
				Иначе
					ГГГГ = 0;
				КонецЕсли;
				Если (СтрНайти("Д1, Д2, Д3, МС", ДДДД) > 0) Тогда
					Если (ММММ < 1)
					 ИЛИ (ММММ > 12)
					 ИЛИ (ГГГГ < 2000)
					 ИЛИ (СтрДлина(П107) - СтрДлина(СтрЗаменить(П107, ".", "")) <> 2) Тогда
						...
						УстановитьГотовность(СтрокаЭкспорта.Готовность, 4);
					КонецЕсли;
				ИначеЕсли (СтрНайти("КВ", ДДДД) > 0) Тогда
					Если (ММММ < 1)
					 ИЛИ (ММММ > 4)
					 ИЛИ (ГГГГ < 2000)
					 ИЛИ (СтрДлина(П107) - СтрДлина(СтрЗаменить(П107, ".", "")) <> 2) Тогда
						...
						УстановитьГотовность(СтрокаЭкспорта.Готовность, 4);
					КонецЕсли;
				ИначеЕсли (СтрНайти("ПЛ", ДДДД) > 0) Тогда
					Если (ММММ < 1)
					 ИЛИ (ММММ > 2)
					 ИЛИ (ГГГГ < 2000)
					 ИЛИ (СтрДлина(П107) - СтрДлина(СтрЗаменить(П107, ".", "")) <> 2) Тогда
						...
						УстановитьГотовность(СтрокаЭкспорта.Готовность, 4);
					КонецЕсли;
				ИначеЕсли (СтрНайти("ГД", ДДДД) > 0) Тогда
					Если (ММММ <> 0)
					 ИЛИ (ГГГГ < 2000)
					 ИЛИ (СтрДлина(П107) - СтрДлина(СтрЗаменить(П107, ".", "")) <> 2) Тогда
						ДобавитьЗамечание(
...
Процедура ДобавитьНастройкиПрограммыКлиентаБанкаВРаспознанныеВыписки(РаспознанныеВыписки)
	...
	Для Каждого КлючИЗначение Из РаспознанныеВыписки Цикл
		...
		РаспознаннаяВыписка = КлючИЗначение.Значение;
		ВладелецСчета = РаспознаннаяВыписка.ВладелецСчета;	
		Счета.Добавить(ВладелецСчета.БанковскийСчет);
	...
	|ПОМЕСТИТЬ ВременнаяТаблицаБанковскиеСчета
	|ИЗ
	|	Справочник.БанковскиеСчета КАК БанковскиеСчета
	|ГДЕ
	|	БанковскиеСчета.Ссылка В (&Счета)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НастройкиОбменаСКлиентомБанка.БанковскийСчет КАК БанковскийСчет,
	|	НЕ НастройкиОбменаСКлиентомБанка.ОтключитьАвтоматическоеСозданиеНенайденныхЭлементов КАК
	|		СоздаватьНенайденныеЭлементы,
	|	НастройкиОбменаСКлиентомБанка.ПередЗагрузкойПоказыватьФормуОбменаСБанком КАК
	|		ПередЗагрузкойПоказыватьФормуОбменаСБанком
	|ПОМЕСТИТЬ ВременнаяТаблицаНастройкиОбмена
	|ИЗ
	|	РегистрСведений.НастройкиОбменаСКлиентомБанка КАК НастройкиОбменаСКлиентомБанка
	|ГДЕ
	|	НастройкиОбменаСКлиентомБанка.БанковскийСчет В (&Счета)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблицаБанковскиеСчета.БанковскийСчет,
	|	ЕСТЬNULL(ВременнаяТаблицаНастройкиОбмена.СоздаватьНенайденныеЭлементы, ИСТИНА) КАК СоздаватьНенайденныеЭлементы,
	|	ЕСТЬNULL(ВременнаяТаблицаНастройкиОбмена.ПередЗагрузкойПоказыватьФормуОбменаСБанком, ЛОЖЬ) КАК
	|		ПередЗагрузкойПоказыватьФормуОбменаСБанком
	|ИЗ
	|	ВременнаяТаблицаБанковскиеСчета КАК ВременнаяТаблицаБанковскиеСчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаНастройкиОбмена КАК ВременнаяТаблицаНастройкиОбмена
	|		ПО ВременнаяТаблицаБанковскиеСчета.БанковскийСчет = ВременнаяТаблицаНастройкиОбмена.БанковскийСчет";
	...
	Для Каждого КлючИЗначение Из РаспознанныеВыписки Цикл
		...
		СтрокаТаблицыНастроек = ТаблицаНастроек.Найти(БанковскийСчет, "БанковскийСчет");
		Если СтрокаТаблицыНастроек <> Неопределено Тогда
			РаспознаннаяВыписка.НастройкиСчета.Вставить("СоздаватьНенайденныеЭлементы",
				СтрокаТаблицыНастроек.СоздаватьНенайденныеЭлементы);
			РаспознаннаяВыписка.НастройкиСчета.Вставить("ПередЗагрузкойПоказыватьФормуОбменаСБанком",
				СтрокаТаблицыНастроек.ПередЗагрузкойПоказыватьФормуОбменаСБанком
				Или ИнтеграцияСБанкамиПовтИсп.ИнтеграцияВключена(БанковскийСчет));
		КонецЕсли;
		...
		Если ТипЗнч(СтрокаСекции.Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ СтрокаСекции.Контрагент.Пустая() Тогда
			...
			Если СтрокаСекции.ВидОперации <> Перечисления.ВидыОперацийРасходСоСчета.РасчетыПоКредитам 
				...
				Иначе
					...
					Договор = СоздатьДоговорПоСтруктуреДанных(СтруктураДанных);
...
Функция СоздатьНовогоКонтрагентаПриЗагрузке(ТаблицаОбъектовСозданныхПриЗагрузке, ДанныеСтроки, ДанныеОбъекта)
	...
	СтруктураПоиска.Вставить("ТипСтроки", 1);
	...
	Иначе
		...
		Если Константы.ОграничиватьДоступНаУровнеЗаписей.Получить() Тогда
			СтруктураДанных.Вставить("ГруппаДоступа", ПерсональныеНастройкиРУНФ.Прочитать(
		...
		ДобавитьИнформациюОбОтношенияхСКонтрагентомПоВидуОперации(СтруктураДанных, ВидОперации);
		
		КонтрагентСоздан = СоздатьКонтрагентаПоСтруктуреДанных(СтруктураДанных);
...
Функция ПолучитьНаименованиеКонтрагентаПоПолномуНаименованию(НаименованиеПолное)
	
	КлассификаторОПФ = РегламентированныеДанныеПовтИсп.КлассификаторОрганизационноПравовыхФорм();
	ОрганизационноПравоваяФорма = УправлениеНебольшойФирмойКлиентСервер.ВыделитьИзНаименованияОПФ(КлассификаторОПФ, НаименованиеПолное);
...
// Определяет группу контрагентов, в которую будет добавлен новый контрагент.
//
// Параметры:
//  ДеньгиОтКонтрагентаПоступили - Булево - Признак поступления денег от контрагента.
//
// Возвращаемое значение:
//  СправочникСсылка.Контрагенты - Группа контрагентов.
//
...
// Функция находит договор контрагента по заданным параметрам.
//
// Параметры:
//  ВладелецДоговора - СправочникСсылка.Контрагенты - Контрагент, которому принадлежит договор.
//  ОрганизацияДоговора - СправочникСсылка.Организации - Организация, с которой заключен договор.
//  СписокВидовДоговора - СписокЗначений из ПеречислениеСсылка.ВидыДоговоров - Виды договоров.
//
// Возвращаемое значение:
//  СправочникСсылка.ДоговорыКонтрагентов - Найденный договор контрагента или строка "Не найден", если договор не найден.
//
...
//
// Параметры:
//  Контрагент - СправочникСсылка.Контрагенты - Контрагент, для которого ищем счет.
//  НомерСчета - Строка - Номер счета контрагента.
//
// Возвращаемое значение:
//  СправочникСсылка.БанковскиеСчета - Найденный счет контрагента или пустая ссылка, если счет не найден.
//
...
Процедура ЗаполнитьДоговорПоСтруктуреДанных(СтруктураДанных, ДоговорОбъект) Экспорт
...
Функция СоздатьКонтрагентаПоСтруктуреДанных(СтруктураДанных) Экспорт
	...
	Попытка
		...
		ИначеЕсли СтруктураДанных.Свойство("Организация") И ЗначениеЗаполнено(СтруктураДанных.Организация) И
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорПоУмолчанию, "Организация") <> СтруктураДанных.Организация Тогда
