...
Функция ПодготовитьДанныеДляПечати(НастройкиПечати, ОписаниеШаблона = Неопределено) Экспорт
	...
	РасшифровкаПолейДата = Новый Соответствие;
	...
	Для Каждого ОбязательноеПоле Из НастройкиПечати.ОбязательныеПоля Цикл
		Если ТипЗнч(ОбязательноеПоле) = Тип("Структура") И ОбязательноеПоле.Значение <> "" Тогда
			...
			Если ПолеСКД <> Неопределено Тогда
				...
				ЗаполнитьРасшифровкуПоляДата(РасшифровкаПолейДата, ОбязательноеПоле.Значение);
			КонецЕсли;
		Иначе
			ПолеСКД = ПечатьЭтикетокИСМПУНФ.НайтиПолеСКДПоПолномуИмени(Компоновщик.Настройки.Выбор.ДоступныеПоляВыбора.Элементы, ОбязательноеПоле);
			Если ПолеСКД <> Неопределено Тогда
				ВыбранноеПоле = Компоновщик.Настройки.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
				ВыбранноеПоле.Поле = ПолеСКД.Поле;
				ЗаполнитьРасшифровкуПоляДата(РасшифровкаПолейДата, ОбязательноеПоле);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	ДобавитьВЗапросРасшифровкуПолейДата(Запрос.Текст, Результат.СоответствиеПолейСКДКолонкамТаблицыТоваров,
		РасшифровкаПолейДата);
...
#Область РасшифровкаДаты

Процедура ЗаполнитьРасшифровкуПоляДата(РасшифровкаПолейДата, Знач ПутьКДанным)
	
	ПозицияРасшифровкиДаты = СтрНайти(ПутьКДанным, ".ДатыНачала.");
	Если ПозицияРасшифровкиДаты = 0 Тогда
		ПозицияРасшифровкиДаты = СтрНайти(ПутьКДанным, ".ДатыКонца.");
	КонецЕсли;
	Если ПозицияРасшифровкиДаты = 0 Тогда
		ПозицияРасшифровкиДаты = СтрНайти(ПутьКДанным, ".ЧастиДат.");
	КонецЕсли;
	...
	КоличествоСимволовВПутиКДанным = СтрДлина(ПутьКДанным);
	
	ПолеДата = Лев(ПутьКДанным, ПозицияРасшифровкиДаты - 1);
	РасшифровкаПоляДата = Прав(ПутьКДанным, КоличествоСимволовВПутиКДанным - ПозицияРасшифровкиДаты);
	
	СписокРасшифровокПоляДата = РасшифровкаПолейДата.Получить(ПолеДата);
	Если СписокРасшифровокПоляДата = Неопределено Тогда
		СписокРасшифровокПоляДата = Новый Массив;
		СписокРасшифровокПоляДата.Добавить(РасшифровкаПоляДата);
	Иначе
		СписокРасшифровокПоляДата.Добавить(РасшифровкаПоляДата);
	КонецЕсли;
	
	РасшифровкаПолейДата.Вставить(ПолеДата, СписокРасшифровокПоляДата);
	
КонецПроцедуры

Процедура ДобавитьВЗапросРасшифровкуПолейДата(ТекстЗапроса, СоответствиеПолейСКДКолонкамТаблицыТоваров,
	Знач РасшифровкаПолейДата)
	
	Для Каждого РасшифровкиПоляДата Из РасшифровкаПолейДата Цикл
		
		ПолеВЗапросе = СоответствиеПолейСКДКолонкамТаблицыТоваров.Получить(РасшифровкиПоляДата.Ключ);
		Если ПолеВЗапросе = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ПозицияПоляДатаВЗапросе = СтрНайти(ТекстЗапроса, "КАК " + ПолеВЗапросе);
		Если ПозицияПоляДатаВЗапросе = 0 Тогда
			Продолжить;
		КонецЕсли;
		ПозицияПредыдущегоПоляВЗапросе = СтрНайти(ТекстЗапроса, "," + Символы.ПС,
			НаправлениеПоиска.СКонца, ПозицияПоляДатаВЗапросе);
		ПозицияНачалаИтоговогоЗапроса = СтрНайти(ТекстЗапроса, "ВЫБРАТЬ",
			НаправлениеПоиска.СКонца, ПозицияПоляДатаВЗапросе);
		СмещениеПозиции = 0;
		Если ПозицияНачалаИтоговогоЗапроса > ПозицияПредыдущегоПоляВЗапросе Тогда
			СмещениеПозиции = 7;
			НачальнаяПозицияКодаПолученияДаты = ПозицияНачалаИтоговогоЗапроса;
		Иначе
			СмещениеПозиции = 2;
			НачальнаяПозицияКодаПолученияДаты = ПозицияПредыдущегоПоляВЗапросе;
		КонецЕсли;
		НачальнаяПозицияКодаПолученияДаты = НачальнаяПозицияКодаПолученияДаты + СмещениеПозиции;
		ТекстПолученияДатыИОтступ = Сред(ТекстЗапроса, НачальнаяПозицияКодаПолученияДаты,
			ПозицияПоляДатаВЗапросе - НачальнаяПозицияКодаПолученияДаты);
		ТекстПолученияДаты = СокрЛ(ТекстПолученияДатыИОтступ);
		ТекстОтступ = СтрЗаменить(ТекстПолученияДатыИОтступ, ТекстПолученияДаты, "");
		ТекстПолученияДаты = СокрП(ТекстПолученияДаты);
		
		ТекстПолученияРасшифровокДаты = "";
		Для Каждого РасшифровкаПоляДата Из РасшифровкиПоляДата.Значение Цикл
			ИмяРасшифровкиПоляДата = РасшифровкаПоляДата;
			ТекстПолученияРасшифровкиДаты = ТекстЗапросаПолученияРасшифровкиДаты(ИмяРасшифровкиПоляДата,
				ТекстПолученияДаты, ПолеВЗапросе, ТекстОтступ);
			Если ПустаяСтрока(ТекстПолученияРасшифровкиДаты) Тогда
				Продолжить;
			КонецЕсли;
			ТекстПолученияРасшифровокДаты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1%2",
				ТекстПолученияРасшифровокДаты, ТекстПолученияРасшифровкиДаты);
			ПолныйПутьРасшифровкиПоляДата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2",
				РасшифровкиПоляДата.Ключ, РасшифровкаПоляДата);
			СоответствиеПолейСКДКолонкамТаблицыТоваров.Вставить(ПолныйПутьРасшифровкиПоляДата, ИмяРасшифровкиПоляДата);
		КонецЦикла;
		
		Если ПустаяСтрока(ТекстПолученияРасшифровокДаты) Тогда
			Продолжить;
		КонецЕсли;
		ТекстПолученияДатыИРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1%2",
			ТекстПолученияРасшифровокДаты, ТекстПолученияДаты);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ТекстПолученияДаты, ТекстПолученияДатыИРасшифровки);
		
	КонецЦикла;
	
КонецПроцедуры

Функция ТекстЗапросаПолученияРасшифровкиДаты(РасшифровкаПоляДата, Знач ТекстПолученияДаты,
	Знач ПолеВЗапросе, Знач ТекстОтступ)
	
	Если СтрНайти(РасшифровкаПоляДата, "ДатыНачала.") Тогда
		РасшифровкаПоляДатыНачала = СтрЗаменить(РасшифровкаПоляДата, "ДатыНачала.", "");
		ТекстЗапросаПолученияРасшифровкиДаты = ТекстЗапросаОперацииСДатой(ТекстПолученияДаты,
			РасшифровкаПоляДатыНачала, "НАЧАЛОПЕРИОДА");
	ИначеЕсли СтрНайти(РасшифровкаПоляДата, "ДатыКонца.") Тогда
		РасшифровкаПоляДатыКонца = СтрЗаменить(РасшифровкаПоляДата, "ДатыКонца.", "");
		ТекстЗапросаПолученияРасшифровкиДаты = ТекстЗапросаОперацииСДатой(ТекстПолученияДаты,
			РасшифровкаПоляДатыКонца, "КОНЕЦПЕРИОДА");
	ИначеЕсли СтрНайти(РасшифровкаПоляДата, "ЧастиДат.") Тогда
		РасшифровкаПоляЧастиДат = СтрЗаменить(РасшифровкаПоляДата, "ЧастиДат.", "");
		ТекстЗапросаПолученияРасшифровкиДаты = ТекстЗапросаПолученияЧастиДат(ТекстПолученияДаты,
			РасшифровкаПоляЧастиДат);
	Иначе
		ТекстЗапросаПолученияРасшифровкиДаты = "";
	КонецЕсли;
	
	Если ПустаяСтрока(ТекстЗапросаПолученияРасшифровкиДаты) Тогда
		Возврат ТекстЗапросаПолученияРасшифровкиДаты;
	КонецЕсли;
	
	РасшифровкаПоляДата = СтрЗаменить(РасшифровкаПоляДата, ".", "");
	РасшифровкаПоляДата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1%2",
		ПолеВЗапросе, РасшифровкаПоляДата);
	
	ШаблонТекста = "%1 КАК %2,%3%4";
	ТекстЗапросаПолученияРасшифровкиДаты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста,
		ТекстЗапросаПолученияРасшифровкиДаты, РасшифровкаПоляДата, Символы.ПС, ТекстОтступ);
	
	Возврат ТекстЗапросаПолученияРасшифровкиДаты;
	
КонецФункции

Функция ТекстЗапросаОперацииСДатой(Знач ТекстПолученияДаты, Знач РасшифровкаПоляДата, Знач Операция)
	
	Если РасшифровкаПоляДата = "НачалоДня" Тогда
		ПериодДаты = "ДЕНЬ";
	ИначеЕсли РасшифровкаПоляДата = "НачалоНедели" Тогда
		ПериодДаты = "НЕДЕЛЯ";
	ИначеЕсли РасшифровкаПоляДата = "НачалоДекады" Тогда
		ПериодДаты = "ДЕКАДА";
	ИначеЕсли РасшифровкаПоляДата = "НачалоМесяца" Тогда
		ПериодДаты = "МЕСЯЦ";
	ИначеЕсли РасшифровкаПоляДата = "НачалоКвартала" Тогда
		ПериодДаты = "КВАРТАЛ";
	ИначеЕсли РасшифровкаПоляДата = "НачалоПолугодия" Тогда
		ПериодДаты = "ПОЛУГОДИЕ";
	ИначеЕсли РасшифровкаПоляДата = "НачалоГода" Тогда
		ПериодДаты = "ГОД";
	ИначеЕсли РасшифровкаПоляДата = "НачалоМинуты" Тогда
		ПериодДаты = "МИНУТА";
	ИначеЕсли РасшифровкаПоляДата = "НачалоЧаса" Тогда
		ПериодДаты = "ЧАС";
	Иначе
		ПериодДаты = "";
	КонецЕсли;
	
	Если ПустаяСтрока(ПериодДаты) Тогда
		ТекстПолученияРасшифровкиДаты = "";
	Иначе
		ТекстПолученияРасшифровкиДаты = "%1(%2, %3)";
		ТекстПолученияРасшифровкиДаты = СтрШаблон(ТекстПолученияРасшифровкиДаты,
			Операция, ТекстПолученияДаты , ПериодДаты);
	КонецЕсли;
	
	Возврат ТекстПолученияРасшифровкиДаты;
	
КонецФункции

Функция ТекстЗапросаПолученияЧастиДат(Знач ТекстПолученияДаты, Знач РасшифровкаПоляДата)
	
	Если РасшифровкаПоляДата = "День" Тогда
		ПериодДаты = "ДЕНЬ";
	ИначеЕсли РасшифровкаПоляДата = "ДеньНедели"
		ИЛИ РасшифровкаПоляДата = "НазваниеДняНедели" Тогда
		ПериодДаты = "ДЕНЬНЕДЕЛИ";
	ИначеЕсли РасшифровкаПоляДата = "ДеньГода" Тогда
		ПериодДаты = "ДЕНЬГОДА";
	ИначеЕсли РасшифровкаПоляДата = "НеделяГода" Тогда
		ПериодДаты = "НЕДЕЛЯ";
	ИначеЕсли РасшифровкаПоляДата = "Месяц"
		ИЛИ РасшифровкаПоляДата = "НазваниеМесяца" Тогда
		ПериодДаты = "МЕСЯЦ";
	ИначеЕсли РасшифровкаПоляДата = "Квартал" Тогда
		ПериодДаты = "КВАРТАЛ";
	ИначеЕсли РасшифровкаПоляДата = "Год" Тогда
		ПериодДаты = "ГОД";
	ИначеЕсли РасшифровкаПоляДата = "Минута" Тогда
		ПериодДаты = "МИНУТА";
	ИначеЕсли РасшифровкаПоляДата = "Час" Тогда
		ПериодДаты = "ЧАС";
	Иначе
		ПериодДаты = "";
	КонецЕсли;
	...
	ТекстПолученияРасшифровкиДаты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1(%2)",
		ПериодДаты, ТекстПолученияДаты);
	
	Если РасшифровкаПоляДата = "НазваниеДняНедели" Тогда
		Понедельник = НСтр("ru = 'Понедельник'");
		Вторник     = НСтр("ru = 'Вторник'");
		Среда       = НСтр("ru = 'Среда'");
		Четверг     = НСтр("ru = 'Четверг'");
		Пятница     = НСтр("ru = 'Пятница'");
		Суббота     = НСтр("ru = 'Суббота'");
		Воскресенье = НСтр("ru = 'Воскресенье'");
		
		ШаблонНазванияДняНедели = "ВЫБОР
		|		КОГДА &ДеньНедели = 1
		|			ТОГДА ""%1""
		|		КОГДА &ДеньНедели = 2
		|			ТОГДА ""%2""
		...
		|		ИНАЧЕ """"
		|	КОНЕЦ";
		ШаблонНазванияДняНедели = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонНазванияДняНедели,
			Понедельник, Вторник, Среда, Четверг, Пятница, Суббота, Воскресенье);
		
		ТекстПолученияРасшифровкиДаты = СтрЗаменить(ШаблонНазванияДняНедели,
			"&ДеньНедели", ТекстПолученияРасшифровкиДаты);
	КонецЕсли;
	
	Если РасшифровкаПоляДата = "НазваниеМесяца" Тогда
		ПараметрыШаблонаНазванияМесяца = Новый  Массив;
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Январь'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Февраль'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Март'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Апрель'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Май'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Июнь'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Июль'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Август'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Сентябрь'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Октябрь'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Ноябрь'"));
		ПараметрыШаблонаНазванияМесяца.Добавить(НСтр("ru = 'Декабрь'"));
		
		ШаблонНазванияМесяца = "ВЫБОР
		|		КОГДА &Месяц = 1
		|			ТОГДА ""%1""
		|		КОГДА &Месяц = 2
		|			ТОГДА ""%2""
		...
		|		ИНАЧЕ """"
		|	КОНЕЦ";
		ШаблонНазванияМесяца = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтрокуИзМассива(ШаблонНазванияМесяца,
			ПараметрыШаблонаНазванияМесяца);
		
		ТекстПолученияРасшифровкиДаты = СтрЗаменить(ШаблонНазванияМесяца,
			"&Месяц", ТекстПолученияРасшифровкиДаты);
	КонецЕсли;
