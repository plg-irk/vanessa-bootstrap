	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		Если ИмяСобытия = "ScanData" Тогда
			...
			Если Не ЗначениеЗаполнено(Объект.ВидЦен) Тогда
				Объект.ВидЦен = ПерсональныеНастройкиРУНФВызовСервера.Прочитать("ОсновнойВидЦенПродажи");
			КонецЕсли;
...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ИнструкцияПереносаШаблонов" Тогда
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("ОбщаяФорма.ПомощникПереносаШаблоновЭтикетокИЦенников", , , , , , ,
...
Процедура ТоварыШаблонЦенникаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработчикОповещения = Новый ОписаниеОповещения("ТоварыШаблонЦенникаНачалоВыбораЗавершение", ЭтотОбъект);
	...
	ОткрытьФорму("Справочник.ХранилищеШаблонов.ФормаВыбора", ПараметрыОткрытия, ЭтотОбъект, , , , ОбработчикОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ТоварыШаблонЭтикеткиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработчикОповещения = Новый ОписаниеОповещения("ТоварыШаблонЭтикеткиНачалоВыбораЗавершение", ЭтотОбъект);
...
Процедура УстановитьОбъектОрганизация()
	...
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Объект.Организация = ПерсональныеНастройкиРУНФВызовСервера.Прочитать("ОсновнаяОрганизация");
...
Процедура Подбор(Команда)
	...
	ПодборНоменклатурыВДокументахКлиент.ОткрытьФормуПодбораНоменклатуры(ЭтотОбъект, , ЗаполнитьПараметрыПодбора(),
...
Функция ЗаполнитьПараметрыПодбора()
	...
	ОсновнаяОрганизация = ПерсональныеНастройкиРУНФВызовСервера.Прочитать("ОсновнаяОрганизация");
	ПараметрыПодбора.Вставить("Организация", ?(ЗначениеЗаполнено(Объект.Организация), Объект.Организация,
		ОсновнаяОрганизация));
	ПараметрыПодбора.Вставить("Дата", ОбщегоНазначенияКлиент.ДатаСеанса());
