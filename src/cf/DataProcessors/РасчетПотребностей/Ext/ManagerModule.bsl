...
Процедура ПолучитьДанные(Параметры, ВременноеХранилищеРезультата) Экспорт
	...
	ЗаданГрафикРаботы 	= ЗначениеЗаполнено(Параметры.ГрафикРаботы) И Не ЗначениеЗаполнено(Параметры.Склад);
	...
	УстановитьПараметр(Параметры.Настройки, "ГрафикРаботы", 			Параметры.ГрафикРаботы);
	УстановитьПараметр(Параметры.Настройки, "Склад", 					Параметры.Склад);

	УстановитьПараметр(Параметры.Настройки, "ПланироватьПеремещения", 	ЗначениеЗаполнено(Параметры.Склад));
	УстановитьПараметр(Параметры.Настройки, "ЗаданГрафикРаботы", 		ЗаданГрафикРаботы);
...
Процедура ЗаполнитьПараметрыПоступления(Дерево, Параметры)
	...
	|	ЕСТЬNULL(ЦеныНоменклатурыКонтрагентов.Валюта, НоменклатураИсточники.Договор.ВалютаРасчетов) КАК Валюта,
...
Процедура ДобавитьВременныеТаблицыГрафиковРаботы(Запрос)
	...
	|				И НЕ &ЗаданГрафикРаботы) КАК ГрафикиРаботыСтруктурныхЕдиницСрезПоследних
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
...
Функция ТаблицаПрогнозаПоДням(Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Параметры.Склад) 
		ИЛИ ЗначениеЗаполнено(Параметры.ГрафикРаботы) Тогда
	...
	Если Не ГрафикРаботыЗадан Тогда
		...
		РабочиеДни.Колонки.Добавить("Склад", 		Новый ОписаниеТипов("СправочникСсылка.СтруктурныеЕдиницы"));
		РабочиеДни.Колонки.Добавить("День",  		ОписаниеТиповДаты());
		РабочиеДни.Колонки.Добавить("РабочихДней", 	ОписаниеТиповЧисла(1));
	...
	Если ГрафикРаботыЗадан Тогда
		...
		Для Каждого ПараметрЗапроса Из Запрос.Параметры Цикл
			ЗапросДатыПрогноза.УстановитьПараметр(ПараметрЗапроса.Ключ, ПараметрЗапроса.Значение);
		КонецЦикла;
