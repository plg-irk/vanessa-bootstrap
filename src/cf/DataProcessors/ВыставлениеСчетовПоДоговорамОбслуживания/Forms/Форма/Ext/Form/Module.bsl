...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокСчетаНаОплату.Параметры.УстановитьЗначениеПараметра("ОплаченПолностью", НСтр("ru = 'Оплачен полностью'"));
	СписокСчетаНаОплату.Параметры.УстановитьЗначениеПараметра("ОплаченЧастично", НСтр("ru = 'Оплачен частично'"));
	СписокСчетаНаОплату.Параметры.УстановитьЗначениеПараметра("БезОплаты", НСтр("ru = 'Без оплаты'"));
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокДоговорыОбслуживания
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокРасшифровкаДоговораОбслуживания
...
Процедура СформироватьДокументы(Команда)
	...
	Если ФормированиеСчетовОтправлятьПоПочте ИЛИ (ФормированиеАктов И ФормированиеАктовОтправлятьПоПочте) Тогда
		УчетнаяЗаписьПоУмолчанию = ПерсональныеНастройкиРУНФВызовСервера.Прочитать(
...
Процедура ОбработатьЗавершениеФоновогоЗадания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗаданиеИмя = "ФоновоеЗаданиеСозданиеПакетаДокументов" Тогда
		...
		Если Результат.Свойство("АктыВыполненныхРабот") Тогда
			...
			КонецЕсли;
			ОповеститьОбИзменении(Тип("ДокументСсылка.АктВыполненныхРабот"));
...
Процедура ВосстановитьНастройкиФормы()
	...
	Для каждого Настройка Из Настройки Цикл
		...
		ЗначениеНастройки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, Настройка.Ключ,
			Настройка.Значение);
		ЭтотОбъект[Настройка.Ключ] = ЗначениеНастройки;
...
Процедура СохранитьНастройкиФормы()
	...
	Для каждого КлючНастройки Из Настройки Цикл
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъекта, КлючНастройки, ЭтотОбъект[КлючНастройки]);
...
Процедура ЗаполнитьСписокПечатныхФорм()
	...
	Если НастройкаПечатныхФорм <> Неопределено Тогда
		Если ТипЗнч(НастройкаПечатныхФорм) = Тип("Массив") Тогда
			// Для совместимости со старыми версиями - изменился формат хранения настроек.
			ВыбранныеПечатныеФормы = НастройкаПечатныхФорм; 
		ИначеЕсли ТипЗнч(НастройкаПечатныхФорм) = Тип("Структура")
			И НастройкаПечатныхФорм.Свойство("ПечатныеФормы") Тогда
			ВыбранныеПечатныеФормы = НастройкаПечатныхФорм.ПечатныеФормы;
		Иначе
	...
	Если НастройкаПечатныхФорм <> Неопределено Тогда
		...
		КонецЦикла;
	Иначе
		ПечатнаяФормаПоУмолчанию = "АктОбОказанииУслуг";
		Команда = КомандыПечати.Найти(ПечатнаяФормаПоУмолчанию, "Идентификатор");
		Если Команда <> Неопределено Тогда
