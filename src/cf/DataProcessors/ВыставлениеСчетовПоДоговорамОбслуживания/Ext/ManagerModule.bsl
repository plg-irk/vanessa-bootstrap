...
//					* Периодичности                         - СписокЗначений из ПеречислениеСсылка.БиллингПериодичностьДоговораОбслуживания
		...
		Если ТипЗнч(НоваяСтрока.ОбъектДоговораОбслуживания) = Тип("СправочникСсылка.Номенклатура") Тогда
			Если КоличествоОказаноБезЦены <> 0 И СуммаОказаноБезЦены = 0
				...
				Если ЗначениеЗаполнено(НоваяСТрока.Договор) Тогда
					ДанныеДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НоваяСТрока.Договор, "Владелец,ВидЦен");
					ПредставлениеВладельца = Строка(ДанныеДоговора.Владелец);
					ПредставлениеВидаЦен = Строка(ДанныеДоговора.ВидЦен);
				Иначе
					ПредставлениеВладельца = "";
					ПредставлениеВидаЦен = "";
				КонецЕсли;
...
//  ОборотыПоДоговору - Массив из СтрокаТаблицыЗначений - Обороты по договору:
//   * Договор - СправочникСсылка.ДоговорыКонтрагентов
// 
// Возвращаемое значение:
//  Неопределено, ДокументОбъект.СчетНаОплату - Создать счет на оплату
...
Функция СоздатьСчетФактуруПоАктуВыполненныхРабот(Дата, АктВыполненныхРабот)
	...
	Перем СчетФактура;
	Если Не ЗначениеЗаполнено(АктВыполненныхРабот) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	РаботыИУслуги = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(АктВыполненныхРабот, "РаботыИУслуги").Выгрузить();
	Если РаботыИУслуги.Итог("СуммаНДС") > 0 Тогда
...
Функция СоздатьЭлектронноеПисьмо(ШаблонСообщения, Договор, ДокументыКОтправке)
	...
	ЭлектронноеПисьмо.УчетнаяЗапись  = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнаяУчетнаяЗаписьЭлектроннойПочты");
...
Функция ПолучитьДанныеЗаполненияЭлектронногоПисьма(ШаблонСообщения, Договор, ДокументыКОтправке)
	...
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Отправитель");
	ДанныеЗаполнения.Вставить("Тема");
	ДанныеЗаполнения.Вставить("Содержание");
	ДанныеЗаполнения.Вставить("СодержаниеHTML");
	ДанныеЗаполнения.Вставить("КартинкиHTML");
	ДанныеЗаполнения.Вставить("Вложения");
	...
	ФорматированныйДокумент.УстановитьHTML(СформированноеСообщение.Текст, Новый Структура);
	ДанныеЗаполнения.Тема           = СформированноеСообщение.Тема;
	ДанныеЗаполнения.Содержание     = ФорматированныйДокумент.ПолучитьТекст();
