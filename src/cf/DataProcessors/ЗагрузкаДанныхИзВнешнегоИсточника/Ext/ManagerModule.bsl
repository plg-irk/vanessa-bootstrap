...
// Добавить колонки таблицы сопоставлений.
// 
// Параметры:
//  ЭтотОбъект Этот объект
//  ТаблицаСопоставленияДанных Таблица сопоставления данных
//  НастройкиЗагрузкиДанных Настройки загрузки данных
//  ПакетнаяЗагрузка - Булево - Пакетная загрузка
...
// Создать дерево полей загрузки доступных пользователю.
// 
// Параметры:
//  ДеревоПолей Дерево полей
//  НастройкиЗагрузкиДанных Настройки загрузки данных
...
// Добавить дополнительный реквизит поиска.
// 
// Параметры:
//  ЭтотОбъект Этот объект
//  ДополнительныйРеквизитПоиска Дополнительный реквизит поиска
...
Процедура ПроверитьЗаполнениеТабличногоДокументаИЗаполнитьТаблицуФормы(ПараметрыВыполнения)
	...
	ИскатьПоДополнительномуРеквизиту = ПараметрыВыполнения.НастройкиЗагрузкиДанных.НастройкиПоиска.ИскатьПоДополнительномуРеквизиту;
			...
			Если ПустаяСтрока(ГруппаИлиПоле.ИмяГруппыПолей) Тогда
				...
				Если ГруппаИлиПоле.ИмяПоля
					...
					Для Каждого ПолеИзГруппыПолейДополнительныхРеквизитов Из ГруппаИлиПоле.Строки Цикл
						...
						ЭтоДопРеквизитПоиска = Ложь;
						
						ПриведенноеИмяПоля = ИдентификаторДляФормул(ПолеИзГруппыПолейДополнительныхРеквизитов.ИмяПоля);
						
						Если ВРег(ПриведенноеИмяПоля) = Врег(ДополнительныйРеквизитПоискаИдентификатор) Тогда
							ЭтоДопРеквизитПоиска = Истина;	
						КонецЕсли;
						...
						// заполним название колонки дополнительного реквизита поиска
						Если ЗначениеЗаполнено(ДополнительныйРеквизитПоискаИдентификатор) 
							И ЭтоДопРеквизитПоиска 
							И ИскатьПоДополнительномуРеквизиту Тогда
							
							НоваяСтрокаДанных[ИмяДопРеквизитаПоиска] = ПолеИзГруппыПолейДополнительныхРеквизитов.ИмяПоля;
			...
			Иначе
				...
				Для Каждого ПолеИзГруппыПолей Из ГруппаИлиПоле.Строки Цикл
					...
					Если ВРег(ПриведенноеИмяПоля) = ВРег(ДополнительныйРеквизитПоискаИдентификатор) Тогда
						...
						НоваяСтрокаДанных[ИмяДопРеквизитаПоиска] = ПриведенноеИмяПоля;   
						
						Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрокаДанных, ПриведенноеИмяПоля) Тогда
							НоваяСтрокаДанных[ПриведенноеИмяПоля] = ЗначениеЯчейки;								 
						КонецЕсли;       
...
Процедура ДобавитьРеквизитыИзПользовательскойГруппыПолей(ЭтотОбъект, ГруппаПолей, ПутьРеквизита, БлокируемыеРеквизиты = Неопределено)
	...
	ПолноеИмяОбъектаЗаполнения = ЭтотОбъект.Параметры.НастройкиЗагрузкиДанных.ПолноеИмяОбъектазаполнения;
	...
	Для Каждого СтрокаГруппы Из ГруппаПолей.Строки Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаГруппы.ИмяПоля) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ПолноеИмяОбъектаЗаполнения = "РегистрСведений.ЦеныНоменклатуры" 
			И ГруппаПолей.ИмяГруппыПолей = "Номенклатура" 
			И ДобавленныеРеквизиты.Получить(СтрокаГруппы.ИмяПоля) <> Неопределено Тогда
			
			Продолжить;
		КонецЕсли;
		
		НовыйРеквизит = Новый РеквизитФормы(СтрокаГруппы.ИмяПоля, СтрокаГруппы.ТипВходящихДанных, 
			ПутьРеквизита, ГруппаПолей.ПредставлениеПоля); 
		
		ДобавленныеРеквизиты.Вставить(СтрокаГруппы.ИмяПоля, СчетчикРеквизитов);	
		МассивРеквизитов.Добавить(НовыйРеквизит);
