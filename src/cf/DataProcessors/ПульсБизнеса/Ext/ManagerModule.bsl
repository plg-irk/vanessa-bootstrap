...
Функция РассчитатьПоказатели(Параметры)
	...
	Для каждого ОписаниеПоказателя Из Параметры.Показатели Цикл
		...
		// В связи с изменением работы метода ВыполнитьВБезопасномРежиме, 
		// для сохранения данных, рассчитанных в МВТ необходимо выполнять расчет в транзакции.
		НачатьТранзакцию();    
		Попытка			
		...
		Исключение   
			ОтменитьТранзакцию();     
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Пульс бизнеса: ошибка расчета данных показателей'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки());  
			Возврат Неопределено;
		КонецПопытки;
		Если Индекс%50=0 И НЕ ПустаяСтрока(Запрос.Текст) Тогда
			//@skip-check query-in-loop
			// Особеннность реализации
...
Процедура ВыполнитьЗапросПоказатели(Запрос, Параметры, Результат)
	...
	ТекстИсходногоЗапроса = Запрос.Текст;
	Для Каждого ЭлементРезультата Из Результат Цикл  
		
		ИндексСтроки = ЭлементРезультата.Ключ;
		СтруктураРезультата = ЭлементРезультата.Значение;
		
		ШаблонИмяИтоговойТаблицы = "втИтоговая%1 ";
		ИмяИтоговойТаблицы = СтрШаблон(ШаблонИмяИтоговойТаблицы, ИндексСтроки);
		
		ЕстьИтоговаяВременнаяТаблица = СтрНайти(ТекстИсходногоЗапроса, ИмяИтоговойТаблицы) <> 0;
		Если ЕстьИтоговаяВременнаяТаблица = Истина И ТипЗнч(ИндексСтроки) = Тип("Число") Тогда   
			
			ШаблонЗапрос = "ВЫБРАТЬ РАЗЛИЧНЫЕ Ссылка ИЗ втИтоговая%1";
			ТекстЗапросаМассивСсылок = СтрШаблон(ШаблонЗапрос, ИндексСтроки);
			Запрос.Текст = ТекстЗапросаМассивСсылок;
			Попытка
				//@skip-check query-in-loop
				// Особеннность реализации
			...
			Исключение
				...
				Продолжить;
			КонецПопытки;
			СтруктураРезультата.Вставить("МассивСсылок", МассивСсылок);    
		КонецЕсли; 
	КонецЦикла;  
...
Функция РассчитатьДиаграммы(Параметры)
	...
	Для каждого ОписаниеДиаграммы Из Параметры.Диаграммы Цикл
		...
		Иначе
			...
			Исключение   
				ОтменитьТранзакцию();     
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Пульс бизнеса: ошибка расчета данных диаграммы'", ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки());  
				Возврат Неопределено;
			КонецПопытки;
		КонецЕсли; 
...
// АПК:299-выкл
// АПК:581-выкл Все указанные ниже процедуры и функции должны быть экспортными, идет неявный вызов
...
Процедура Подключаемый_CRMРасчет(Запрос, Стр, Индекс, Параметры) Экспорт   
	
	Если СтрНайти(Стр.Ресурс,"Всего") <> 0 Тогда
		Если СтрНайти(Стр.Ресурс,"Количество") <> 0 Тогда
			Подключаемый_CRMКоличествоРасчет(Запрос, Стр, Индекс, Параметры);
		КонецЕсли; 
	ИначеЕсли СтрНайти(Стр.Ресурс,"ЗаПериод") <> 0 Тогда
		Если СтрНайти(Стр.Ресурс,"Количество") <> 0 Тогда
			Подключаемый_CRMКоличествоОборотРасчет(Запрос, Стр, Индекс, Параметры);
		КонецЕсли; 
	ИначеЕсли СтрНайти(Стр.Ресурс,"ВСегменте") <> 0 Тогда
		Подключаемый_CRMСегментыРасчет(Запрос, Стр, Индекс, Параметры);	
	ИначеЕсли СтрНайти(Стр.Ресурс,"Расчет") <> 0 Тогда 
		Если СтрНайти(Стр.Ресурс,"Конверсия") <> 0 Тогда		
			Подключаемый_CRMКонверсионныеПоказателиОборотРасчет(Запрос, Стр, Индекс, Параметры);	
		Иначе
			Подключаемый_CRMРасчетныеПоказателиОборотРасчет(Запрос, Стр, Индекс, Параметры);	
		КонецЕсли; 
	КонецЕсли; 
	
КонецПроцедуры	

Процедура Подключаемый_CRMКоличествоРасчет(Запрос, Стр, Индекс, Параметры) Экспорт
	
	МассивРесурсов = Новый Массив;
	МассивРесурсов.Добавить("КоличествоЛидовВсего");
	МассивРесурсов.Добавить("КоличествоНекачественныхЛидовВсего");
	МассивРесурсов.Добавить("КоличествоКонтрагентовВсего");
	МассивРесурсов.Добавить("КоличествоСобытийВсего");
	МассивРесурсов.Добавить("КоличествоЗадачВсего");
	МассивРесурсов.Добавить("КоличествоЗадачПросроченоВсего");
	
	Если МассивРесурсов.Найти(Стр.Ресурс) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекстРесурса = 	"КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Рег.Ссылка)";   
	
	// Всего лидов (на дату)
	Если Стр.Ресурс="КоличествоЛидовВсего" Тогда   
		
		ТекстЭлемента = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РегистрИсторияСостоянийЛидов.Лид КАК Ссылка
		|ПОМЕСТИТЬ втИтоговаяСуффиксИндекс
		|ИЗ
		|	РегистрСведений.ИсторияСостоянийЛидов.СрезПоследних(&Период, Лид.ПометкаУдаления = ЛОЖЬ) КАК РегистрИсторияСостоянийЛидов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Лиды КАК Рег
		|		ПО РегистрИсторияСостоянийЛидов.Лид = Рег.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Лиды.Теги КАК ТЧТеги
		|		ПО (ТЧТеги.Ссылка = Рег.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиКонтрагентЛид КАК РегистрСвязиКонтрагентЛид
		|		ПО РегистрИсторияСостоянийЛидов.Лид = РегистрСвязиКонтрагентЛид.Лид
		|ГДЕ
		|	Рег.ПометкаУдаления = ЛОЖЬ
		|	И Рег.ЭтоГруппа = ЛОЖЬ
		|	И &Отборы
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|	&Ресурс КАК Значение
		|ИЗ
		|	втИтоговаяСуффиксИндекс КАК Рег"; 
		
	// Всего контрагентов (на дату)		
	ИначеЕсли Стр.Ресурс="КоличествоКонтрагентовВсего" Тогда   
		
		ТекстЭлемента = "ВЫБРАТЬ РАЗЛИЧНЫЕ 
		|	Рег.Ссылка КАК Ссылка    
		|ПОМЕСТИТЬ втИтоговаяСуффиксИндекс		
		|ИЗ
		|	Справочник.Контрагенты КАК Рег
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.Теги КАК ТЧТеги
		|		ПО (ТЧТеги.Ссылка = Рег.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоставСегментаКонтрагентов КАК РегССК
		|		ПО (РегССК.Контрагент = Рег.Ссылка)
		|ГДЕ      
		|	Рег.ПометкаУдаления = ЛОЖЬ   
		|	И Рег.ЭтоГруппа = ЛОЖЬ		
		|	И Рег.ДатаСоздания <= &Период 
		|	И &Отборы
		|;		
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	// Всего Событий (на дату)		
	ИначеЕсли Стр.Ресурс="КоличествоСобытийВсего" Тогда
		
		ТекстЭлемента = "ВЫБРАТЬ РАЗЛИЧНЫЕ 
		|	Рег.Ссылка КАК Ссылка    
		|ПОМЕСТИТЬ втИтоговаяСуффиксИндекс		
		|ИЗ
		|	Документ.Событие КАК Рег
		|ГДЕ
		|	Рег.Дата <= &Период 
		|	И Рег.ПометкаУдаления = ЛОЖЬ
		|	И &Отборы
		|;		
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
  	...
  	// Всего Задач (на дату)		
	ИначеЕсли Стр.Ресурс="КоличествоЗадачВсего" Тогда
		
		ТекстЭлемента = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Рег.Ссылка КАК Ссылка,
		|	Рег.Ответственный КАК Ответственный,
		|	Рег.Выполнена КАК Выполнена,
		|	Рег.ДатаОкончания <= &Период И Рег.Выполнена = ЛОЖЬ КАК Просрочена,
		|	Рег.ВесьДень КАК ВесьДень
		|ПОМЕСТИТЬ втРасчетнаяСуффиксИндекс
		|ИЗ
		|	Документ.ЗадачаСотрудника КАК Рег
		|ГДЕ
		|	Рег.ДатаНачала <= &Период
		|	И Рег.ПометкаУдаления = ЛОЖЬ
		|	
		|;
		|
		...
		|ПОМЕСТИТЬ втИтоговаяСуффиксИндекс
		|ИЗ
		|	втРасчетнаяСуффиксИндекс КАК Рег
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗадачаСотрудника.Теги КАК ТЧТеги
		|		ПО (ТЧТеги.Ссылка = Рег.Ссылка)
		|ГДЕ
		|	&Отборы
		|;
...
Процедура Подключаемый_CRMКоличествоОборотРасчет(Запрос, Стр, Индекс, Параметры) Экспорт
	
	МассивРесурсов = Новый Массив;
	МассивРесурсов.Добавить("КоличествоЛидовЗаПериод");
	МассивРесурсов.Добавить("КоличествоНекачественныхЛидовЗаПериод");
	МассивРесурсов.Добавить("КоличествоКонтрагентовЗаПериод");
	МассивРесурсов.Добавить("КоличествоСобытийЗаПериод");
	МассивРесурсов.Добавить("КоличествоЗадачЗаПериод");
	...
	// Лидов (за период)
	Если Стр.Ресурс="КоличествоЛидовЗаПериод" Тогда
		
		ТекстЭлемента =	"ВЫБРАТЬ
		|	Рег.Лид КАК Лид,
		|	МИНИМУМ(Рег.Период) КАК Период
		|ПОМЕСТИТЬ втПериодыСуффиксИндекс
		|ИЗ
		|	РегистрСведений.ИсторияСостоянийЛидов КАК Рег
		|
		|СГРУППИРОВАТЬ ПО
		|	Рег.Лид
		|
		|ИМЕЮЩИЕ
		|	МИНИМУМ(Рег.Период) МЕЖДУ &ДатаНачала И &ДатаОкончания
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ПОМЕСТИТЬ втИтоговаяСуффиксИндекс
		|ИЗ
		|	втПериодыСуффиксИндекс КАК втПериоды
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияСостоянийЛидов.СрезПоследних(
		|		,
		|		Лид В
		|			(ВЫБРАТЬ
		|				ПериодыСуффиксИндекс.Лид
		|			ИЗ
		|				втПериодыСуффиксИндекс КАК ПериодыСуффиксИндекс)) КАК РегистрИсторияСостоянийЛидов
		|		ПО (РегистрИсторияСостоянийЛидов.Лид = втПериоды.Лид)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Лиды КАК Рег
		|		ПО (РегистрИсторияСостоянийЛидов.Лид = Рег.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Лиды.Теги КАК ТЧТеги
		|		ПО (ТЧТеги.Ссылка = Рег.Ссылка)
		...
		|		ПО (РегистрИсторияСостоянийЛидов.Лид = РегистрСвязиКонтрагентЛид.Лид)
		|ГДЕ
		|	РегистрИсторияСостоянийЛидов.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
		|	И Рег.ПометкаУдаления = ЛОЖЬ    
		|	И Рег.ЭтоГруппа = ЛОЖЬ		
		|	И &Отборы
		|;
	...
	// Контрагентов (за период)
	ИначеЕсли Стр.Ресурс="КоличествоКонтрагентовЗаПериод" Тогда
		...
		|		ПО (РегССК.Контрагент = Рег.Ссылка)
		|ГДЕ
		|	Рег.ДатаСоздания МЕЖДУ &ДатаНачала И &ДатаОкончания   
		|	И Рег.ПометкаУдаления = ЛОЖЬ	
		|	И Рег.ЭтоГруппа = ЛОЖЬ		
		|	И &Отборы
		|;
	...
	// Событий (за период)		
	ИначеЕсли Стр.Ресурс="КоличествоСобытийЗаПериод" Тогда
		...
		|	Документ.Событие КАК Рег
		|ГДЕ
		|	Рег.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания 
		|	И Рег.ПометкаУдаления = ЛОЖЬ
		|	И &Отборы
		|;
		|
  	...
  	// Задач (за период)		
	ИначеЕсли Стр.Ресурс="КоличествоЗадачЗаПериод" Тогда
		...
		|		ПО (ТЧТеги.Ссылка = Рег.Ссылка)
		|ГДЕ
		|	Рег.ДатаНачала МЕЖДУ &ДатаНачала И &ДатаОкончания
		|	И Рег.ПометкаУдаления = ЛОЖЬ
		|	И &Отборы
		|;
		|
...
Процедура Подключаемый_CRMСегментыРасчет(Запрос, Стр, Индекс, Параметры) Экспорт

	Если СтрНайти(Стр.Ресурс,"КоличествоВСегменте") <> 0 Тогда
		ТекстРесурса = 	"КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Рег.Контрагент)";  
 	Иначе
		Возврат;
	КонецЕсли; 
	
	// Количество в сегменте		
	Если СтрНайти(Стр.Ресурс,"КоличествоВСегменте") <> 0  Тогда   
		
		Если НЕ ЗначениеЗаполнено(Стр.СсылкаСвязи) Тогда
			Возврат;
		Иначе
			Сегмент = Стр.СсылкаСвязи;	
		КонецЕсли;
		...
		|	&Ресурс  КАК Значение
		|ИЗ
		|	РегистрСведений.СоставСегментаКонтрагентов КАК Рег
		|ГДЕ
		|	Рег.Сегмент = &Сегмент" + УИД ;
		
		Запрос.УстановитьПараметр("Сегмент" + УИД, Сегмент);
...
Процедура Подключаемый_CRMКонверсионныеПоказателиОборотРасчет(Запрос, Стр, Индекс, Параметры) Экспорт

	МассивРесурсов = Новый Массив;
	МассивРесурсов.Добавить("КонверсияЛидовПоИсточникамПривлеченияРасчет");
	МассивРесурсов.Добавить("КонверсияПокупателейПоИсточникамПривлеченияРасчет");
	МассивРесурсов.Добавить("КонверсияЗаказовПоИсточникамПривлеченияРасчет");

	Если МассивРесурсов.Найти(Стр.Ресурс) = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ТекстРесурса = 	"ВЫБОР КОГДА СУММА(втСуммыСуффиксИндекс.ИтогВсего) <> 0 ТОГДА СУММА(втСуммыСуффиксИндекс.ИтогУспешные) / СУММА(втСуммыСуффиксИндекс.ИтогВсего) * 100 ИНАЧЕ 0 КОНЕЦ";  
	
	Если ЗначениеЗаполнено(Стр.СсылкаСвязи) Тогда
		Источник = Стр.СсылкаСвязи;	
		УИД = СтрЗаменить(Строка(Новый УникальныйИдентификатор),"-","");
	КонецЕсли;  
	
	Запрос.УстановитьПараметр("ИсточникПривлеченияПустаяСсылка", Справочники.ИсточникиПривлеченияПокупателей.ПустаяСсылка());
	
	Если Стр.Ресурс="КонверсияЛидовПоИсточникамПривлеченияРасчет" Тогда  
		
		ТекстЭлемента = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ИсторияСостоянийЛидов.Лид КАК Лид,
		|	ИсторияСостоянийЛидов.Лид.Ответственный КАК Ответственный,
		...
		|ПОМЕСТИТЬ ВТ_ЛидыСуффиксИндекс
		|ИЗ
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Лиды КАК Рег
		|		ПО ИсторияСостоянийЛидов.Лид = Рег.Ссылка
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
		|				ТОГДА ИсторияСостоянийЛидов.Период >= &ДатаНачала
		|			ИНАЧЕ ИсторияСостоянийЛидов.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
		|		КОНЕЦ
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Рег.Лид КАК Лид,
		|	Рег.Лид.ВариантЗавершения КАК ВариантЗавершения,
		|	Рег.ИсточникПривлечения КАК ИсточникПривлечения,
		|	СостоянияЛидов.РеквизитДопУпорядочивания КАК Порядок,
		|	СостоянияЛидов.Ссылка КАК Состояние
		|ПОМЕСТИТЬ ВТ_СостоянияЛидыСуффиксИндекс
		|ИЗ
		|	ВТ_ЛидыСуффиксИндекс КАК Рег
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СостоянияЛидов КАК СостоянияЛидов
		|		ПО (Рег.Состояние <> СостоянияЛидов.Ссылка
		|				ИЛИ Рег.Состояние = СостоянияЛидов.Ссылка) 
		|ГДЕ
		|	&Отборы		
        |
		|;
		...
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ИтогВсего
		|ПОМЕСТИТЬ втИтогСуффиксИндекс
		|ИЗ
		|	ВТ_СостоянияЛидыСуффиксИндекс КАК ВТ_СостоянияЛиды
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(втИтог.ИтогУспешные) КАК ИтогУспешные,
		|	СУММА(втИтог.ИтогВсего) КАК ИтогВсего,
		|	втИтог.ИсточникПривлечения КАК ИсточникПривлечения
		|ПОМЕСТИТЬ втСуммыСуффиксИндекс
		|ИЗ
		|	втИтогСуффиксИндекс КАК втИтог    
		|   
		|ГДЕ
        |	втИтог.ИсточникПривлечения <> &ИсточникПривлеченияПустаяСсылка
		|СГРУППИРОВАТЬ ПО
		|	втИтог.ИсточникПривлечения
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	&Ресурс КАК Значение
		|ИЗ
		|	втСуммыСуффиксИндекс КАК втСуммыСуффиксИндекс ";
		
		Если ЗначениеЗаполнено(Источник) Тогда 
			ТекстЭлемента = ТекстЭлемента + " ГДЕ
			|	втСуммыСуффиксИндекс.ИсточникПривлечения = &Источник" + УИД;
			Запрос.УстановитьПараметр("Источник" + УИД, Источник);   
		КонецЕсли;            
		
	ИначеЕсли Стр.Ресурс="КонверсияПокупателейПоИсточникамПривлеченияРасчет" Тогда
		
		ТекстЭлемента = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Рег.ЭтоГруппа КАК ПолеГруппировки,
		|	Рег.Ссылка КАК Покупатель,
		|	Рег.Ссылка КАК Контрагент,
		|	Рег.Ответственный КАК Ответственный,
		|	Рег.ИсточникПривлеченияПокупателя КАК ИсточникПривлечения
		|ПОМЕСТИТЬ втПокупателиОтборыСуффиксИндекс 
		|ИЗ
		|	Справочник.Контрагенты КАК Рег
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиКонтрагентЛид КАК СвязиКонтрагентЛид
		|		ПО (СвязиКонтрагентЛид.Контрагент = Рег.Ссылка)
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
		|				ТОГДА Рег.ДатаСоздания >= &ДатаНачала
		|			ИНАЧЕ Рег.ДатаСоздания МЕЖДУ &ДатаНачала И &ДатаОкончания
		|		КОНЕЦ
		|	И Рег.ЭтоГруппа = ЛОЖЬ
		|	И Рег.Покупатель = ИСТИНА
		|;     
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Рег.ПолеГруппировки КАК ПолеГруппировки,
		|	Рег.Покупатель КАК Покупатель,
		|	Рег.Контрагент КАК Контрагент,
		|	Рег.Ответственный КАК Ответственный,
		|	Рег.ИсточникПривлечения КАК ИсточникПривлечения
		|ПОМЕСТИТЬ ВТ_ПокупателиСуффиксИндекс
		|ИЗ
		|	втПокупателиОтборыСуффиксИндекс КАК Рег	
		|ГДЕ &Отборы
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ВТ_ДанныеВоронкиПоПокупателямСуффиксИндекс
		|ИЗ
		|	ВТ_ПокупателиСуффиксИндекс КАК ВТ_Покупатели
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Продажи.Обороты(, , , Организация.ПометкаУдаления = ЛОЖЬ) КАК ПродажиОбороты
		...
		|ПОМЕСТИТЬ втАналитикаСуффиксИндекс
		|ИЗ
		|	ВТ_ДанныеВоронкиПоПокупателямСуффиксИндекс КАК ВТ_ДанныеВоронкиПоПокупателям
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втАналитика.ИсточникПривлечения КАК ИсточникПривлечения,
		|	СУММА(втАналитика.ИтогВсего) КАК ИтогВсего,
		|	СУММА(втАналитика.ИтогУспешные) КАК ИтогУспешные
		|ПОМЕСТИТЬ втСуммыСуффиксИндекс
		|ИЗ
		|	втАналитикаСуффиксИндекс КАК втАналитика   
		|ГДЕ
        |	втАналитика.ИсточникПривлечения <> &ИсточникПривлеченияПустаяСсылка				
		|
		|СГРУППИРОВАТЬ ПО
		|	втАналитика.ИсточникПривлечения   
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	ИначеЕсли Стр.Ресурс="КонверсияЗаказовПоИсточникамПривлеченияРасчет" Тогда
		
		ТекстЭлемента = "ВЫБРАТЬ
		|	ИсторияСостоянийЗаказов.Период КАК ПериодСостояния,
		|	ИсторияСостоянийЗаказов.Заказ КАК Заказ,
		|	Рег.ВидЗаказа КАК ВидЗаказа,
		|	Рег.ВариантЗавершения КАК ВариантЗавершения,
		|	Рег.ИсточникПривлечения КАК ИсточникПривлечения,
		|	Рег.ВидОперации КАК ВидОперации,     
		|	Рег.Организация КАК Организация,
		|	Рег.Контрагент КАК Контрагент,
		|	Рег.Ответственный КАК Ответственный,
		...
		|ПОМЕСТИТЬ втЗаказыОтборСуффиксИндекс
		|ИЗ
		|	РегистрСведений.ИсторияСостоянийЗаказов КАК ИсторияСостоянийЗаказов    
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК Рег
		|		ПО ИсторияСостоянийЗаказов.Заказ = Рег.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиКонтрагентЛид КАК СвязиКонтрагентЛид
		|		ПО (СвязиКонтрагентЛид.Контрагент = Рег.Контрагент)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗаказовПокупателей.ПорядокСостояний КАК ВидыЗаказовПокупателейПорядокСостояний
		|		ПО Рег.ВидЗаказа = ВидыЗаказовПокупателейПорядокСостояний.Ссылка
		...
		|		ПО Рег.ВидЗаказа = ВидыЗаказНарядовПорядокСостояний.Ссылка
		|			И ИсторияСостоянийЗаказов.Состояние = ВидыЗаказНарядовПорядокСостояний.Состояние
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
		|				ТОГДА ИсторияСостоянийЗаказов.Период >= &ДатаНачала
		|			ИНАЧЕ ИсторияСостоянийЗаказов.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
		|		КОНЕЦ
		|	И НЕ Рег.ВидЗаказа ЕСТЬ NULL     
		|;  
		|ВЫБРАТЬ
		|	Рег.ПериодСостояния КАК ПериодСостояния,
		|	Рег.Заказ КАК Заказ,
		|	Рег.ВидЗаказа КАК ВидЗаказа,
		|	Рег.ВариантЗавершения КАК ВариантЗавершения,
		|	Рег.ИсточникПривлечения КАК ИсточникПривлечения,
		...
		|	Рег.Порядок КАК Порядок
		|ПОМЕСТИТЬ ВТ_ЗаказыСуффиксИндекс
		|ИЗ
		|	втЗаказыОтборСуффиксИндекс КАК Рег
		|	ГДЕ &Отборы				
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|ПОМЕСТИТЬ ВТ_ПериодЗаказыСуффиксИндекс
		|ИЗ
		|	ВТ_ЗаказыСуффиксИндекс КАК ВТ_Заказы
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗаказовПокупателей.ПорядокСостояний КАК ВидыЗаказовПокупателейПорядокСостояний
		...
		|ПОМЕСТИТЬ ВТ_СостоянияСуффиксИндекс
		|ИЗ
		|	ВТ_ЗаказыСуффиксИндекс КАК ВТ_Заказы
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗаказовПокупателей.ПорядокСостояний КАК ВидыЗаказовПокупателейПорядокСостояний
		...
		|ПОМЕСТИТЬ ВТ_СрезПоследнихСуффиксИндекс
		|ИЗ
		|	ВТ_ЗаказыСуффиксИндекс КАК ВТ_Заказы
		...
		|				Заказ В
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|						ВТ_ЗаказыСуффиксИндекс.Заказ
		|					ИЗ
		|						ВТ_ЗаказыСуффиксИндекс)) КАК ИсторияСостоянийЗаказовСрезПоследних
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗаказНарядов.ПорядокСостояний КАК ВидыЗаказНарядовПорядокСостояний
		...
		|	ИсторияСостоянийЗаказовСрезПоследних.Период КАК ПоследнийПериод,
		|	ЕСТЬNULL(ВидыЗаказовПокупателейПорядокСостояний.НомерСтроки, ВидыЗаказНарядовПорядокСостояний.НомерСтроки) КАК ПоследнийПорядок
		|ПОМЕСТИТЬ ВТ_СрезПоследнихДоОтменыСуффиксИндекс
		|ИЗ
		|	ВТ_ЗаказыСуффиксИндекс КАК ВТ_Заказы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияСостоянийЗаказов.СрезПоследних(
		|				,
		...
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК Успешные
		|ПОМЕСТИТЬ ВТ_ДанныеВоронкиПродажСуффиксИндекс
		|ИЗ
		|	ВТ_СостоянияСуффиксИндекс КАК ВТ_Состояния
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПериодЗаказыСуффиксИндекс КАК ВТ_ПериодЗаказы
		|		ПО ВТ_Состояния.Заказ = ВТ_ПериодЗаказы.Заказ
		|			И ВТ_Состояния.Состояние = ВТ_ПериодЗаказы.Состояние
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПериодЗаказыСуффиксИндекс КАК ВТ_ПериодЗаказы1
		|		ПО ВТ_Состояния.Заказ = ВТ_ПериодЗаказы1.Заказ
		|			И ВТ_Состояния.СледующееСостояние = ВТ_ПериодЗаказы1.Состояние
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СрезПоследнихСуффиксИндекс КАК ВТ_СрезПоследних
		|		ПО ВТ_Состояния.Заказ = ВТ_СрезПоследних.Заказ
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СрезПоследнихДоОтменыСуффиксИндекс КАК ВТ_СрезПоследнихДоОтмены
		...
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ИтогВсего,
		|	ВТ_ДанныеВоронкиПродаж.Заказ.ИсточникПривлечения КАК ЗаказИсточникПривлечения
		|ПОМЕСТИТЬ втИтоговаяДВПСуффиксИндекс
		|ИЗ
		|	ВТ_ДанныеВоронкиПродажСуффиксИндекс КАК ВТ_ДанныеВоронкиПродаж
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(втИтоговаяДВП.ИтогУспешные) КАК ИтогУспешные,
		|	СУММА(втИтоговаяДВП.ИтогВсего) КАК ИтогВсего,
		|	втИтоговаяДВП.ЗаказИсточникПривлечения КАК ИсточникПривлечения
		|ПОМЕСТИТЬ втСуммыСуффиксИндекс
		|ИЗ
		|	втИтоговаяДВПСуффиксИндекс КАК втИтоговаяДВП
		|ГДЕ
        |	втИтоговаяДВП.ЗаказИсточникПривлечения <> &ИсточникПривлеченияПустаяСсылка						
		|
		|СГРУППИРОВАТЬ ПО
		|	втИтоговаяДВП.ЗаказИсточникПривлечения
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура Подключаемый_CRMРасчетныеПоказателиОборотРасчет(Запрос, Стр, Индекс, Параметры) Экспорт
	
	МассивРесурсов = Новый Массив;
	МассивРесурсов.Добавить("СреднееВремяЗаказаРасчет");

	Если МассивРесурсов.Найти(Стр.Ресурс) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
  	Если Стр.Ресурс="СреднееВремяЗаказаРасчет" Тогда  
		ТекстРесурса = 	"СРЕДНЕЕ(ВЫБОР КОГДА ВТ_ДанныеВоронкиПродаж.Длительность <> 0 ТОГДА ВТ_ДанныеВоронкиПродаж.Длительность / 3600 ИНАЧЕ NULL КОНЕЦ)";
	Иначе
		Возврат;
	КонецЕсли;
	...
	Если Стр.Ресурс="СреднееВремяЗаказаРасчет" Тогда  
		ТекстЭлемента = "ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА Рег.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу)
		|				ИЛИ Рег.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаявкаНаПродажу)
		|				ИЛИ Рег.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаПереработку)
		|			ТОГДА ""ЗаказПокупателя""
		|		ИНАЧЕ ""ЗаказНаряд""
		|	КОНЕЦ КАК ГруппировкаВидОперации,
		|	ИСТИНА КАК ПолеГруппировки,
		|	ИсторияСостоянийЗаказов.Период КАК ПериодСостояния,
		|	ИсторияСостоянийЗаказов.Заказ КАК Заказ,
		|	Рег.ВидЗаказа КАК ВидЗаказа,
		|	Рег.ВариантЗавершения КАК ВариантЗавершения,
		...
		|	ИсторияСостоянийЗаказов.Состояние КАК Состояние,
		|	ЕСТЬNULL(ВидыЗаказовПокупателейПорядокСостояний.НомерСтроки, ВидыЗаказНарядовПорядокСостояний.НомерСтроки) КАК Порядок
		|ПОМЕСТИТЬ втОборыЗаказыСуффиксИндекс
		|ИЗ
		|	РегистрСведений.ИсторияСостоянийЗаказов КАК ИсторияСостоянийЗаказов         
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК Рег
		|		ПО ИсторияСостоянийЗаказов.Заказ = Рег.Ссылка		
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Рег.ГруппировкаВидОперации КАК ГруппировкаВидОперации,
		|	Рег.ПолеГруппировки КАК ПолеГруппировки,
		|	Рег.ПериодСостояния КАК ПериодСостояния,
		|	Рег.Заказ КАК Заказ,
		|	Рег.ВидЗаказа КАК ВидЗаказа,
		...
		|ПОМЕСТИТЬ ВТ_ЗаказыСуффиксИндекс
		|ИЗ
		|	втОборыЗаказыСуффиксИндекс КАК Рег	
		|	ГДЕ &Отборы						
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура Подключаемый_ДинамикаАктивовПассивовДиаграмма(Результат, Стр, Индекс, Актив = Истина) Экспорт
	...
	Для каждого СтрПериод Из Дерево.Строки Цикл
		...
		КонецЕсли;
		Если Актив Тогда
			СтруктураЗначения = Новый Структура("Серия1, Серия2, Серия3", 0, 0, 0);
			СтруктураЗначения.Вставить("Серия4", 0);
...
Процедура Подключаемый_ЛидыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	
	Если Стр.Серия = "Состояние" Тогда
		ТекстСерии1 = "РегистрИсторияСостоянийЛидов.Состояние";
	ИначеЕсли Стр.Серия = "Тег" Тогда
		ТекстСерии1 = "ТЧТеги.Тег";
	ИначеЕсли Стр.Серия = "ИсточникПривлечения" Тогда
		ТекстСерии1 = "Рег.ИсточникПривлечения";
	ИначеЕсли Стр.Серия = "Ответственный" Тогда
		ТекстСерии1 = "Рег.Ответственный";
	ИначеЕсли Стр.Серия = "ПервичноеПовторное" Тогда
		ТекстСерии1 = "Рег.Вид";
	ИначеЕсли Стр.Серия = "Результат" Тогда
		ТекстСерии1 = "Рег.ВариантЗавершения";
	Иначе
		Возврат;
	КонецЕсли;
	
	ТекстТочки = "КОЛИЧЕСТВО(РАЗЛИЧНЫЕ РегистрИсторияСостоянийЛидов.Лид)";
...
Процедура Подключаемый_КонтрагентыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	
	Если Стр.Серия = "Тег" Тогда
		ТекстСерии1 = "ТЧТеги.Тег";
	ИначеЕсли Стр.Серия = "ИсточникПривлечения" Тогда
		ТекстСерии1 = "Рег.ИсточникПривлеченияПокупателя";
	ИначеЕсли Стр.Серия = "Ответственный" Тогда
		ТекстСерии1 = "Рег.Ответственный";
	Иначе
		Возврат;
	...
	ТекстТочки = "КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Рег.Ссылка)";
	...
	|		ПО (РегССК.Контрагент = Рег.Ссылка)
	|ГДЕ 
	|	Рег.ДатаСоздания МЕЖДУ &ПустаяДата И &Период   
	|	И Рег.ПометкаУдаления = ЛОЖЬ   
	|	И Рег.ЭтоГруппа = ЛОЖЬ		
	|	И &Отборы
	|СГРУППИРОВАТЬ ПО
	...
	Если Стр.ВходитВГруппу = "CRMОбороты" Тогда
		ТекстЭлемента = СтрЗаменить(ТекстЭлемента, "&ПустаяДата", "&ДатаНачала");
		ТекстЭлемента = СтрЗаменить(ТекстЭлемента, "&Период", "&ДатаОкончания");
	Иначе
		Запрос.УстановитьПараметр("ПустаяДата", Дата(1,1,1));		
	КонецЕсли;
...
Процедура Подключаемый_ЗаказыПокупателяДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	
	Если Стр.Серия = "ВидЗаказа" Тогда
		ТекстСерии1 = "Рег.ВидЗаказа";
	ИначеЕсли Стр.Серия = "Состояние" Тогда
		ТекстСерии1 = "РегИСЗ.Состояние";
	ИначеЕсли Стр.Серия = "ВариантЗавершения" Тогда
		ТекстСерии1 = "Рег.ВариантЗавершения";
	ИначеЕсли Стр.Серия = "Ответственный" Тогда
		ТекстСерии1 = "Рег.Ответственный";
	ИначеЕсли Стр.Серия = "Организация" Тогда
		ТекстСерии1 = "Рег.Организация";
	Иначе
		Возврат;
	КонецЕсли;

	Если Стр.Точка = "Количество" Тогда
		ТекстТочки = "КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВЫБОР КОГДА РегистрОплатаСчетовИЗаказов.СуммаОборот >0 ТОГДА РегистрОплатаСчетовИЗаказов.СчетНаОплату ИНАЧЕ NULL КОНЕЦ)";
	ИначеЕсли Стр.Точка = "Сумма" Тогда  
		ТекстТочки = "СУММА(ВЫРАЗИТЬ(РегистрОплатаСчетовИЗаказов.СуммаОборот*РегистрОплатаСчетовИЗаказов.СчетНаОплату.Курс/РегистрОплатаСчетовИЗаказов.СчетНаОплату.Кратность/КурсыВалют.Курс*КурсыВалют.Кратность КАК Число(15,2)))";
	КонецЕсли;
	...
	Если Стр.Точка ="Количество" Тогда	
		...
		|	&Точка КАК Точка
		|ИЗ
		|	РегистрНакопления.ОплатаСчетовИЗаказов.Обороты(&ПустаяДата, &Период, Авто, СчетНаОплату ССЫЛКА Документ.ЗаказПокупателя) КАК РегистрОплатаСчетовИЗаказов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК Рег
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияСостоянийЗаказов.СрезПоследних(,Заказ.ПометкаУдаления = ЛОЖЬ) КАК РегИСЗ
		|			ПО (РегИСЗ.Заказ = Рег.Ссылка)
		|		ПО РегистрОплатаСчетовИЗаказов.СчетНаОплату = Рег.Ссылка
		|ГДЕ
		|	&Отборы
		|СГРУППИРОВАТЬ ПО
		|	&Серия1 
	...
	ИначеЕсли Стр.Точка ="Сумма" Тогда
		...
		|	ОплатаСчетовИЗаказовОбороты.Регистратор КАК СчетЗаказ,
		|	МАКСИМУМ(ЕСТЬNULL(КурсыВалют.Период, ДАТАВРЕМЯ(1, 1, 1))) КАК ПериодКурса
		|ПОМЕСТИТЬ втВложенныйСуффиксИндекс
		|ИЗ
		|	РегистрНакопления.ОплатаСчетовИЗаказов.Обороты(&ПустаяДата, &Период, Регистратор, СчетНаОплату ССЫЛКА Документ.ЗаказПокупателя) КАК ОплатаСчетовИЗаказовОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
		|		ПО ОплатаСчетовИЗаказовОбороты.Период >= КурсыВалют.Период
		|			И (КурсыВалют.Валюта В
		|				(ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.ОплатаСчетовИЗаказов.Обороты(&ПустаяДата, &Период, Авто, СчетНаОплату ССЫЛКА Документ.ЗаказПокупателя) КАК РегистрОплатаСчетовИЗаказов
		|		ЛЕВОЕ СОЕДИНЕНИЕ втВложенныйСуффиксИндекс КАК ВложенныйЗапрос
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
		|			ПО ВложенныйЗапрос.ПериодКурса = КурсыВалют.Период
		|				И (КурсыВалют.Валюта В
		|					(ВЫБРАТЬ
		...
		|					ИЗ
		|						Константа.ВалютаУчета))
		|		ПО РегистрОплатаСчетовИЗаказов.СчетНаОплату = ВложенныйЗапрос.СчетЗаказ
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК Рег
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияСостоянийЗаказов.СрезПоследних(,Заказ.ПометкаУдаления = ЛОЖЬ) КАК РегИСЗ
		|			ПО (РегИСЗ.Заказ = Рег.Ссылка)
		|		ПО РегистрОплатаСчетовИЗаказов.СчетНаОплату = Рег.Ссылка
		|ГДЕ
		|	РегистрОплатаСчетовИЗаказов.СуммаОборот > 0
		|	И &Отборы
		|СГРУППИРОВАТЬ ПО
		|	&Серия1 
		|
...
Процедура Подключаемый_СобытияДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	
	Если Стр.Серия = "ТипСобытия" Тогда
		ТекстСерии1 = "Рег.ТипСобытия";
	ИначеЕсли Стр.Серия = "Состояние" Тогда
		ТекстСерии1 = "Рег.Состояние";
	ИначеЕсли Стр.Серия = "Важность" Тогда
		ТекстСерии1 = "Рег.Важность";
	ИначеЕсли Стр.Серия = "Ответственный" Тогда
		ТекстСерии1 = "Рег.Ответственный";
	Иначе
		Возврат;
	...
	|	Документ.Событие КАК Рег
	|ГДЕ
	|	Рег.Дата МЕЖДУ &ПустаяДата И &Период
	|	И Рег.ПометкаУдаления = ЛОЖЬ
	|	И &Отборы 
	|СГРУППИРОВАТЬ ПО
	|	&Серия1 
...
Процедура Подключаемый_ЗадачиДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	
	Если Стр.Серия = "Ответственный" Тогда
		ТекстСерии1 = "Рег.Ответственный";
	ИначеЕсли Стр.Серия = "Выполнена" Тогда
		ТекстСерии1 = "Рег.Выполнена";
	ИначеЕсли Стр.Серия = "Тег" Тогда
		ТекстСерии1 = "ТЧТеги.Тег";
	Иначе
		Возврат;
	...
	|		ПО (ТЧТеги.Ссылка = Рег.Ссылка)
	|ГДЕ
	|	Рег.ДатаНачала МЕЖДУ &ПустаяДата И &Период
	|	И Рег.ПометкаУдаления = ЛОЖЬ
	|	И &Отборы
	|СГРУППИРОВАТЬ ПО
	|	&Серия1 
...
Процедура Подключаемый_ЛидыОборотыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	...
	|	&Точка КАК Точка
	|ИЗ
	|	втПериодыСуффиксИндекс КАК ЗапросОтбор
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияСостоянийЛидов.СрезПоследних(,Лид.ПометкаУдаления = ЛОЖЬ) КАК РегистрИсторияСостоянийЛидов
	|		ПО (РегистрИсторияСостоянийЛидов.Лид = ЗапросОтбор.Лид)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Лиды КАК Рег
	|		ПО (РегистрИсторияСостоянийЛидов.Лид = Рег.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Лиды.Теги КАК ТЧТеги
	|		ПО (ТЧТеги.Ссылка = Рег.Ссылка)
...
Процедура Подключаемый_КонтрагентыОборотыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	
	Подключаемый_КонтрагентыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс);
	
КонецПроцедуры

Процедура Подключаемый_ЗаказыПокупателяОборотыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	
	Подключаемый_ЗаказыПокупателяДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс);
	
КонецПроцедуры  

Процедура Подключаемый_ЗадачиОборотыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт
	
	Подключаемый_ЗадачиДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс);
	
КонецПроцедуры

Процедура Подключаемый_СобытияОборотыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт

	Подключаемый_СобытияДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс);	
                                
КонецПроцедуры

Процедура Подключаемый_ПокупателиПоСостояниямОборотыДиаграмма(Запрос, ПараметрыЗапросов, Стр, Индекс) Экспорт 
	
	Если Стр.Серия = "Состояние" Тогда
		ТекстСерии1 = "втГруппировкаСостояния.Состояние";
	Иначе
		Возврат;
	КонецЕсли;
	
	ТекстТочки = "СУММА(втГруппировкаСостояния.ИтогГруппировка)";
	...
	|ПОМЕСТИТЬ ВТ_ЗаказыСуффиксИндекс
	|ИЗ
	|	РегистрСведений.ИсторияСостоянийЗаказов.СрезПоследних(,Заказ.ПометкаУдаления = ЛОЖЬ) КАК ИсторияСостоянийЗаказов     
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК Рег
	|		ПО ИсторияСостоянийЗаказов.Заказ = Рег.Ссылка
	|ГДЕ
	...
	|	ВЫРАЗИТЬ(ВТ_Заказы.СуммаДокумента КАК ЧИСЛО(15, 2)) КАК СуммаДокумента
	|ПОМЕСТИТЬ ВТ_ПокупателиСуффиксИндекс
	|ИЗ
	|	Справочник.Контрагенты КАК Рег
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиКонтрагентЛид КАК СвязиКонтрагентЛид
	|		ПО (СвязиКонтрагентЛид.Контрагент = Рег.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗаказыСуффиксИндекс КАК ВТ_Заказы
	|		ПО Рег.Ссылка = ВТ_Заказы.Заказ.Контрагент
	|ГДЕ
	|	ВЫБОР
	|			КОГДА &ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА Рег.ДатаСоздания >= &ДатаНачала
	...
	|ПОМЕСТИТЬ втДанныеВоронкиСуффиксИндекс
	|ИЗ
	|	ВТ_ДанныеВоронкиПоПокупателямСуффиксИндекс КАК ВТ_ДанныеВоронкиПоПокупателям
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втДанныеВоронки.ИтогУспешные КАК ИтогГруппировка,
	|	ВЫРАЗИТЬ(&ПараметрСовершенаПродажа КАК СТРОКА(25)) КАК Состояние
	|ПОМЕСТИТЬ втГруппировкаСостоянияСуффиксИндекс
	|ИЗ
	|	втДанныеВоронкиСуффиксИндекс КАК втДанныеВоронки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	втДанныеВоронки.ВедетсяРабота,
	|	ВЫРАЗИТЬ(&ПараметрВедетсяРабота КАК СТРОКА(25))
	|ИЗ
	|	втДанныеВоронкиСуффиксИндекс КАК втДанныеВоронки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	втДанныеВоронки.ОтменилиЗаказ,
	|	ВЫРАЗИТЬ(&ПараметрОтменилиЗаказ КАК СТРОКА(25))
	|ИЗ
	|	втДанныеВоронкиСуффиксИндекс КАК втДанныеВоронки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	втДанныеВоронки.НетПродажИЗаказов,
	|	ВЫРАЗИТЬ(&ПараметрНетПродажИЗаказов КАК СТРОКА(25))
	|ИЗ
	|	втДанныеВоронкиСуффиксИндекс КАК втДанныеВоронки
	|;
	|
	...
	|	&Точка КАК Точка
	|ИЗ
	|	втГруппировкаСостоянияСуффиксИндекс КАК втГруппировкаСостояния
	|СГРУППИРОВАТЬ ПО
	|	&Серия1 
	|УПОРЯДОЧИТЬ ПО
	|	Точка";
	...
	Запрос.УстановитьПараметр("ПараметрНетПродажИЗаказов",НСтр("ru = 'Нет продаж и заказов'"));
	Запрос.УстановитьПараметр("ПараметрОтменилиЗаказ",НСтр("ru = 'Отменили заказ'"));
	Запрос.УстановитьПараметр("ПараметрВедетсяРабота",НСтр("ru = 'Ведется работа по заказам'"));
	Запрос.УстановитьПараметр("ПараметрСовершенаПродажа",НСтр("ru = 'Совершена продажа'"));
...
Процедура ОбработатьЭлементЗапросаПоказатели(Стр, Запрос, Элемент, Индекс, Суффикс = "")
	...
	ТекстВТ = "";
	ПоследняяВТ = СтрНайти(Элемент,"ПОМЕСТИТЬ",НаправлениеПоиска.СКонца);
	Если ПоследняяВТ <> 0 Тогда
		Элемент = СтрЗаменить(Элемент, "СуффиксИндекс", Суффикс + Индекс);	
		ТекстВТ = Лев(Элемент,СтрНайти(Элемент,";",НаправлениеПоиска.СКонца)) + Символы.ПС;
		Элемент = Сред(Элемент,СтрНайти(Элемент,";",НаправлениеПоиска.СКонца) + 1); 
	КонецЕсли;
	...
	Запрос.Текст = ТекстВТ + Запрос.Текст; 
...
Процедура ОбработатьЭлементЗапросаДиаграммы(Стр, Запрос, ПараметрыЗапросов, Элемент, Индекс, ДляСравнения = Ложь)
	...
	СтруктураПараметров.Вставить("ЗапросСИтогами", СтрНайти(Элемент, "ИТОГИ ") > 0);
	...
	Если ПоследняяВТ <> 0 Тогда                            
		...
		ЗапросМВТ.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
		ЗапросМВТ.Текст = ТекстВТ;           
		Для Каждого Параметр Из Запрос.Параметры Цикл   
			ЗапросМВТ.УстановитьПараметр(Параметр.Ключ, Параметр.Значение);	
		КонецЦикла;  
...
Процедура ДобавитьФильтрПоОрганизации(Параметры, Коллекция, ПроверятьЗначениеРеквизитаНалоги)
	...
	Для каждого ОписаниеЭлемента Из Коллекция Цикл
		...
		Если ИгнорироватьОтборПоОрганизации(Коллекция,ОписаниеЭлемента) = Истина Тогда
			Продолжить;
		КонецЕсли;			
...
Функция СтрокаОтбора(Фильтр, ИмяПараметра)
	...
	Если СтрНайти(Фильтр.Поле, "DataParameters.")>0 Тогда
		Возврат "";
	КонецЕсли;
	...
	ИначеЕсли СтрНайти(Фильтр.Поле, "[")>0 Тогда 
		
		Позиция = СтрНайти(Фильтр.Поле, "[");
		ПозицияКонца = СтрНайти(Фильтр.Поле, "]");
	...
	Иначе
		...
		ЭтоСвойство = СтрНайти(Фильтр.Поле, "Свойство")>0;
		
		ИмяПоля = "Рег."+Фильтр.Поле; 

		ПозицияТочка = СтрНайти(Фильтр.Поле, "."); 
		Если ПозицияТочка <> 0 Тогда                
			ОбработатьОтборСТочкой(Фильтр, ИмяПоля, ПозицияТочка);			
		КонецЕсли;
...
Процедура ОбработатьОтборСТочкой(Фильтр,ИмяПоля,ПозицияТочка)
	
	МассивВнешнихТаблиц = Новый Массив;
	МассивВнешнихТаблиц.Добавить("ТЧТеги");
	МассивВнешнихТаблиц.Добавить("РегистрИсторияСостоянийЛидов");
	МассивВнешнихТаблиц.Добавить("РегистрСвязиКонтрагентЛид");
	МассивВнешнихТаблиц.Добавить("РегистрОплатаСчетовИЗаказов");      
	
	ЧастьДоТочки = Лев(Фильтр.Поле, ПозицияТочка - 1);
	Если МассивВнешнихТаблиц.Найти(ЧастьДоТочки) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТипФильтра = ТипЗнч(Фильтр.Значение);     
	Если ТипЗнч(Фильтр.Значение) = Тип("СписокЗначений") И Фильтр.Количество() > 0 Тогда
		ТипФильтра = ТипЗнч(Фильтр.Значение[0].Значение);     		
	КонецЕсли;	      
	
	ЭтоСсылка = ОбщегоНазначения.ЭтоСсылка(ТипФильтра);     
	Если ЭтоСсылка Тогда
		ИмяОбъектаМетаданного = Метаданные.НайтиПоТипу(ТипФильтра).ПолноеИмя();  
		ШаблонПустоеЗначение = "Значение(%1.ПустаяСсылка)";
		ТекстПустоеЗначение = СтрШаблон(ШаблонПустоеЗначение, ИмяОбъектаМетаданного);
	Иначе
		ТекстПустоеЗначение = 0;	
	КонецЕсли;	
	
	ШаблонИмяПоля = "ВЫБОР КОГДА %1 IS NULL ТОГДА %2 ИНАЧЕ %1 КОНЕЦ "; 
	ИмяПоля = СтрШаблон(ШаблонИмяПоля, Фильтр.Поле, ТекстПустоеЗначение);
	
КонецПроцедуры

Функция ИгнорироватьОтборПоОрганизации(Коллекция,ОписаниеЭлемента)    

	Если Коллекция.Колонки.Найти("Показатель") <> Неопределено И СтрНайти(ОписаниеЭлемента.Показатель,"CRM") <> 0 Тогда
		Возврат Истина;
	КонецЕсли; 
	
	Если Коллекция.Колонки.Найти("ВходитВГруппу") <> Неопределено И СтрНайти(ОписаниеЭлемента.ВходитВГруппу,"CRM") <> 0 Тогда
		Возврат Истина;
	КонецЕсли; 
