...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("Фильтры", Фильтры);
 	Параметры.Свойство("СсылкаСвязи", СсылкаСвязи);
 	Параметры.Свойство("ПредставлениеСсылкиСвязи", ПредставлениеСсылкиСвязи);
...
&НаКлиенте
Процедура СсылкаСвязиПриИзменении(Элемент)
...
Процедура СохранитьНастройку(Команда)
	...
	Если ИспользуетсяСсылкаСвязи() = Истина И НЕ ЗначениеЗаполнено(СсылкаСвязи) И НЕ ТипЗнч(СсылкаСвязи) = Тип("СправочникСсылка.ИсточникиПривлеченияПокупателей") Тогда 
		ШаблонСообщения = НСтр("ru='Не указан параметр %1'");  
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПолучитьСтруктуруСсылкиСвязи().Представление);  
	...
	Если ТипЗнч(Настройки)=Тип("ФиксированныйМассив") Тогда
		Для каждого Элемент Из Настройки Цикл
			Элемент.Значение = ЭтотОбъект[Элемент.Имя];
		КонецЦикла;      
	КонецЕсли;           
	
	Результат.Вставить("СсылкаСвязи", СсылкаСвязи);      
...
Процедура ДобавитьФильтр(Команда)
	...
	ОткрытьФорму("Обработка.ПульсБизнеса.Форма.ФормаДобавленияФильтра", СтруктураОткрытия, ЭтотОбъект);
...
Процедура УстановитьВидимостьПолейНастроек()
	...
	Элементы.СсылкаСвязи.Видимость = ИспользуетсяСсылкаСвязи();  
...
Процедура СформироватьПредставление()
	...
	Если ЗначениеЗаполнено(СсылкаСвязи) Тогда
		ТекстПредставления = Представление;
		Если СтрНайти(Ресурс,"КоличествоВСегменте") <> 0 Тогда 
			ТекстПредставления = НСтр("ru='Сегмент %1'");
			ТекстПредставления = СтрШаблон(ТекстПредставления, УправлениеНебольшойФирмойВызовСервера.ЗначениеРеквизитаОбъекта(СсылкаСвязи,"Наименование"));
		ИначеЕсли Ресурс = "КонверсияЛидовПоИсточникамПривлеченияРасчет" Тогда
			ТекстПредставления = НСтр("ru='Конверсия лидов (источник - %1)'");
			ТекстПредставления = СтрШаблон(ТекстПредставления, СсылкаСвязи);
		ИначеЕсли Ресурс = "КонверсияПокупателейПоИсточникамПривлеченияРасчет" Тогда
			ТекстПредставления = НСтр("ru='Конверсия покупателей (источник - %1)'");
			ТекстПредставления = СтрШаблон(ТекстПредставления, СсылкаСвязи);  
		ИначеЕсли Ресурс = "КонверсияЗаказовПоИсточникамПривлеченияРасчет" Тогда
			ТекстПредставления = НСтр("ru='Конверсия Заказов покупателя (источник - %1)'");
			ТекстПредставления = СтрШаблон(ТекстПредставления, СсылкаСвязи);  
		КонецЕсли;       
		Если Представление <> ТекстПредставления Тогда  
...
&НаСервере
Функция ИспользуетсяСсылкаСвязи()

	Строки = НастройкиПоказателей.НайтиСтроки(Новый Структура("Показатель,Ресурс",Показатель,Ресурс));
	Если Строки.Количество() = 1 Тогда
		Возврат Строки[0].СсылкаСвязи <> Неопределено;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьСтруктуруСсылкиСвязи()
	...
	Если Строки.Количество() = 1 Тогда         
		Строка = Строки[0];
		Структура.Значение = Строка.СсылкаСвязи;
		Структура.Тип = ТипЗнч(Строка.СсылкаСвязи);
		Структура.Представление = Строка.ПредставлениеСсылкиСвязи;
	КонецЕсли;
...
Процедура ОбновитьОтображениеНастроек()
	...
	СтруктураСсылкиСвязи = ПолучитьСтруктуруСсылкиСвязи();
	Если СтруктураСсылкиСвязи.Тип <> Неопределено Тогда
		Если ТипЗнч(СсылкаСвязи) <> СтруктураСсылкиСвязи.Тип Тогда
			СсылкаСвязи = СтруктураСсылкиСвязи.Значение;
		КонецЕсли;   
		Если ТипЗнч(СсылкаСвязи) = Тип("СправочникСсылка.ИсточникиПривлеченияПокупателей") Тогда
			Элементы.СсылкаСвязи.ПодсказкаВвода = НСтр("ru='Все источники привлечения'");
		Иначе  
			Элементы.СсылкаСвязи.ПодсказкаВвода = "";
		КонецЕсли;
		Массив = Новый Массив(); 
		Массив.Добавить(СтруктураСсылкиСвязи.Тип); 
		ОписаниеТипов = Новый ОписаниеТипов(Массив);
		Элементы.СсылкаСвязи.ОграничениеТипа = ОписаниеТипов;  
		Элементы.СсылкаСвязи.Заголовок = СтруктураСсылкиСвязи.Представление;  
		Элементы.СсылкаСвязи.ВыборГруппИЭлементов = ГруппыИЭлементы.Элементы;
	КонецЕсли;     
	...
	Для каждого Элемент Из Настройки Цикл
		...
		ЭтотОбъект[Элемент.Имя] = Элемент.Значение;
	КонецЦикла; 
