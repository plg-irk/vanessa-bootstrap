...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ВходитВГруппу", ГруппаДиаграмм);    
	Параметры.Свойство("ПредставлениеГруппыДиаграмм", ПредставлениеГруппыДиаграмм);    
	Параметры.Свойство("Серия", Серия);
	Параметры.Свойство("Точка", Точка);
	Параметры.Свойство("Представление", Представление);
	
	Если ГруппаДиаграмм <> "" Тогда 
		ЭтоГруппа = Истина;
	КонецЕсли;
	Если ЭтоГруппа = Истина Тогда 
		Элементы.Диаграмма.СписокВыбора.Добавить(Диаграмма, Представление);	
		Элементы.ГруппаДиаграмм.СписокВыбора.Добавить(ГруппаДиаграмм, ПредставлениеГруппыДиаграмм);				
	Иначе                                                                       
		ГруппаДиаграмм = Диаграмма;
		Элементы.ГруппаДиаграмм.СписокВыбора.Добавить(Диаграмма, Представление);		
	КонецЕсли; 	      
	...
	Если Диаграмма = "" Тогда 
		ЭтоГруппа = ЭтоГруппа(ГруппаДиаграмм);	
		Таб = НастройкиДиаграмм.Выгрузить(Новый Структура("ВходитВГруппу",""), "Диаграмма, Представление");
		Таб.Свернуть("Диаграмма, Представление");
		Элементы.ГруппаДиаграмм.СписокВыбора.Очистить();
		Для Каждого Стр Из Таб Цикл     
			Элементы.ГруппаДиаграмм.СписокВыбора.Добавить(Стр.Диаграмма, Стр.Представление);
		КонецЦикла;    
	КонецЕсли; 
...
&НаКлиенте
Процедура ГруппаДиаграммПриИзменении(Элемент)
	
	ЭтоГруппа = ЭтоГруппа(ГруппаДиаграмм);
	Если ЭтоГруппа = Истина Тогда  
		Диаграмма = "";	
		ЗаполнитьСписокПодчиненныеДиаграммы();
		ПриИзмененииДиаграммыСерииТочкиСервер();
	Иначе   
		Диаграмма = ГруппаДиаграмм;
		//@skip-check invocation-form-event-handler
		ДиаграммаПриИзменении(Элементы.Диаграмма);	
	КонецЕсли;	
...
Процедура СохранитьНастройку(Команда)
	...
	Если ГруппаДиаграмм <> Диаграмма Тогда
		Результат.Вставить("ВходитВГруппу", ГруппаДиаграмм);    
	КонецЕсли;
...
Процедура УстановитьВидимостьНастроек()
	...
	Элементы.Диаграмма.Видимость = ЭтоГруппа; 
	Элементы.ГруппаДиаграмм.Доступность = ИдентификаторСтроки<0;
	Если ЭтоГруппа Тогда 
		Элементы.Диаграмма.Заголовок = НСтр("ru = 'Диаграмма'");
	Иначе
		Элементы.Диаграмма.Заголовок = НСтр("ru = 'Раздел учета'");
	КонецЕсли;
...
&НаСервере 
Процедура ЗаполнитьСписокПодчиненныеДиаграммы()
	...
	ТабПодчиненные = НастройкиДиаграмм.Выгрузить(Новый Структура("ВходитВГруппу",ГруппаДиаграмм), "Диаграмма, Представление");
	Для каждого Стр Из ТабПодчиненные Цикл     
...
Процедура ВыборПериодаНачало(Элемент)
	...
	Если РежимОстатка Тогда
		...
		ИначеЕсли Элемент=Элементы.ПериодСравненияПредставление Тогда
			...
			КонецЕсли;
			ДатаРасчета = ?(Период.Вариант=ВариантСтандартнойДатыНачала.НачалоСледующегоДня, КонецДня(ОбщегоНазначенияКлиент.ДатаСеанса()), НачалоДня(Период.Дата));
