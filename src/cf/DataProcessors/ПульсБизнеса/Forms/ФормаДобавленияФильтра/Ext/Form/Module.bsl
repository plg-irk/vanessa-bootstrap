...
Процедура ПолеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение="..." Тогда
		...
		ОткрытьФорму("ОбщаяФорма.ФормаСпискаПолей", СтруктураОткрытия, ЭтотОбъект);
	КонецЕсли; 
...
Процедура ПолеПриИзмененииСервер()
	...
	МассивОтключитьОтрицательныеОтборы = Новый Массив;
	МассивОтключитьОтрицательныеОтборы.Добавить("ТЧТеги.Тег");
	...
	Для каждого Тип Из ОписаниеТипов.Типы() Цикл
		...
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(Тип) Тогда
			...
			Если (ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) 
                ...
                И МетаданныеОбъекта.Иерархический И СтрНайти(Поле, ".") = 0 Тогда 
			...
			Если КоличествоЗначений>2 Тогда
				...
				Если МассивОтключитьОтрицательныеОтборы.Найти(Поле) = Неопределено Тогда		
	...
	Для каждого ОписаниеФильтра Из Фильтры Цикл
		...
		ИмяВидаСравнения = Сред(ИмяВидаСравнения, СтрНайти(ИмяВидаСравнения, ".")+1);
...
Процедура ОбновитьТипЗначения()
	...
	ИначеЕсли СтрНайти(Поле, "[")>0 Тогда
		// Это доп. реквизит
		Позиция = СтрНайти(Поле, "[");
		ПозицияКонца = СтрНайти(Поле, "]");
		...
		КонецЕсли; 
	ИначеЕсли Поле="ВариантЗавершения" Тогда
		
		Элементы.Значение.РежимВыбораИзСписка = Истина;
		Элементы.Значение.АвтоОтметкаНезаполненного = Ложь;
		Элементы.Значение.ОтметкаНезаполненного = Ложь;
		
		Элементы.Значение.СписокВыбора.Очистить();
		Элементы.Значение.СписокВыбора.Добавить(Перечисления.ВариантЗавершенияРаботыСЛидом.ПустаяСсылка(), НСтр("ru = 'В работе'"));  
		Для Каждого ЗначениеПеречисления Из Перечисления.ВариантЗавершенияРаботыСЛидом Цикл
        	Элементы.Значение.СписокВыбора.Добавить(ЗначениеПеречисления, Строка(ЗначениеПеречисления));
		КонецЦикла;		
	Иначе
		Элементы.Значение.ПараметрыВыбора = Новый ФиксированныйМассив(Новый Массив);
	КонецЕсли;
