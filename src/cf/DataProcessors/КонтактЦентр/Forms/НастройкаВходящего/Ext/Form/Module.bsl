...
&НаКлиенте
Процедура КартинкаТелефонияНажатие(Элемент)
...
&НаКлиенте
Процедура КартинкаТелеграмНажатие(Элемент)
...
&НаКлиенте
Процедура КартинкаВКонтактеНажатие(Элемент)
...
&НаКлиенте
Процедура КартинкаОбменССайтомНажатие(Элемент)
	
	НастроитьОбменССайтом();
	
КонецПроцедуры

&НаКлиенте
Процедура КартинкаКабинетКлиентаНажатие(Элемент)
	
	НастроитьКабинетКлиента();
	
КонецПроцедуры

#КонецОбласти

#Область ОбработчикиСобытийЭлементовТаблицыФормыКолонкиТекущейДоски
...
Процедура НастроитьТелефонию()
	...
	ОткрытьФорму("Обработка.НастройкаПрограммы.Форма.НастройкаПрограммы", ПараметрыНастроек, , , , ,
		ОповещениеОЗакрытии);
...
Процедура НастроитьПочту()
	...
	ПараметрыНастроек = Новый Структура("СтрокаПоиска", НСтр("ru = 'Учетные записи почты'"));
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("НастройкаКаналовЗавершение", ЭтотОбъект);
	ОткрытьФорму("Обработка.НастройкаПрограммы.Форма.НастройкаПрограммы", ПараметрыНастроек, , , , ,
		ОповещениеОЗакрытии);	
...
&НаКлиенте
Процедура НастроитьОбменССайтом()
...
&НаКлиенте
Процедура НастроитьКабинетКлиента()
...
Функция ИзменитьПорядокЗаписей(Запись, Порядок, НаправлениеИзменения)
	...
	ПорядокПредыдущейКолонки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПредыдущаяКолонка, "Порядок");
	Если ПорядокПредыдущейКолонки = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	КонецЕсли;
	Элементы.КабинетКлиента.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуМобильныйЛичныйКабинет");
	...
	Подключено_ОбменССайтом = КонтактЦентр.ПодключенСайт();
