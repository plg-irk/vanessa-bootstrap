...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Справочник.КалендариСотрудников.Форма.ФормаЭлемента" Тогда
...
Процедура ДекорацияГиперссылкаНетКаналовНажатие(Элемент)
	...
	ОткрытьФормуНастройкиВходящего();
...
Процедура ПланировщикПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Дата, Значения)
	...
	ПараметрыЗадачи.Вставить("ОткрытьМодально", Истина);
...
Процедура ВходящееПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	ТипыВнешнихСистем = ТипыВнешнихСистем();	
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Если ТипЗнч(КлючИЗначение.Ключ.Источник) = Тип("Строка") Тогда
			...
			ТипПоля = ТипыВнешнихСистем.Получить(КлючИЗначение.Значение.Данные.ТипВнешнейСистемы);
			Если ТипПоля = Неопределено Тогда
				ТипПоля = 6;
			КонецЕсли;
		
			КлючИЗначение.Значение.Данные.ТипПоля = ТипПоля;
...
Процедура ВРаботеПриАктивизацииЯчейки(Элемент)
	...
	Если Элемент.ТекущаяСтрока <> Неопределено И Элемент.ТекущиеДанные.РедактированиеЗапрещено Тогда
		Элементы.ВРаботеКонтекстноеУстановитьСостояние.Видимость = Ложь;
...
Процедура ЗадачиНаСегодняПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	СтандартнаяОбработка = Ложь;
	ИзменитьСтандартнуюОбработкуСписковКанбан(СтандартнаяОбработка);
...
&НаКлиенте
Процедура ЗадачиЗавершеныНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
...
&НаКлиенте
Процедура ЗадачиЗавершеныПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
...
Процедура ЗавершитьЗадачуВРаботе(Команда)
	...
	ЗадачиДляЗавершения = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Задача);
	
	СостоянияДокументовКлиент.УстановитьОтметкуВыполненияЗадачи(ЗадачиДляЗавершения);
...
Процедура Подключаемый_ИзменитьСостояние(Команда)
	...
	Если ВидыКонтактЦентра = 0 Тогда 
		...
		ТекущиеДанные = Элементы.ВРаботе.ТекущиеДанные;
		Если ТекущиеДанные = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		ОбрабатываемыеОбъекты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущиеДанные.Ссылка);
	...
	ИначеЕсли ВидыКонтактЦентра = 1 Тогда 
		...
		ОбрабатываемыйОбъект = ДокументАктивнойСтрокиСпискаКанбан(ИмяТекущегоЭлемента);
		Если ОбрабатываемыйОбъект = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		ОбрабатываемыеОбъекты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОбрабатываемыйОбъект);		
		СостоянияДокументовКлиент.УстановитьСостояние(Команда.Имя, ДоступныеСостояния, ОбрабатываемыеОбъекты, ОбработанныеТипыДокументов);	
		
		ОбновитьДанныеВидКанбан();

	// Открыт вид "Календарь"
	ИначеЕсли ВидыКонтактЦентра = 2 Тогда
		
		// Команда вызвана из таблицы "Задачи на весь день" или из Планировщика
		Если ТекущийЭлемент = Элементы.ЗадачиНаВесьДеньКалендарь Тогда
...
Процедура ЗавершитьЗадачуНаВесьДень(Команда)
	...
	ОбрабатываемыеОбъекты = СостоянияДокументовКлиент.ДокументыВыбранныхСобытийЗадачиНаВесьДень(
	Элементы.ЗадачиНаВесьДеньКалендарь, ЗадачиНаВесьДеньКалендарь);
...
&НаКлиенте
Асинх Процедура ОчиститьВходящее(Команда)
	
	Если ПользователиКлиент.ЭтоПолноправныйПользователь() Тогда
		ТекстВопроса = НСтр("ru='Отображены события всех сотрудников.
			|Будут очищены только события текущего пользователя. Продолжить?'");
	Иначе
		ТекстВопроса = НСтр("ru='Очистить входящее?'");
	КонецЕсли;
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ОчиститьВходящееНаСервере(СотрудникТекущегоПользователя);
		Элементы.Входящее.Обновить();
		
		ТекстОповещения = НСтр("ru='Очистка входящих.'");
		ТекстПояснения = НСтр("ru='Операция завершена.'");
		ПоказатьОповещениеПользователя(ТекстОповещения, , ТекстПояснения, БиблиотекаКартинок.Информация32);
	
		ОбработаноВсеВходящее  = ОбработаноВсеВходящее(СотрудникТекущегоПользователя);
...
&НаКлиенте
Процедура СоздатьДело_БезСрока(Команда)
	
	СоздатьДелоНаЗаданнуюДату(Команда.Имя);
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьДело_Завтра(Команда)
...
&НаКлиенте
Процедура СоздатьДело_НаНеделе(Команда)
...
&НаКлиенте
Процедура СоздатьДело_Позже(Команда)
...
&НаКлиенте
Процедура СоздатьДело_Сегодня(Команда)
...
&НаКлиенте
Процедура СоздатьДелоНаЗаданнуюДату(ИмяКоманды)
	
	СтрокаДаты = Прав(ИмяКоманды, СтрДлина(ИмяКоманды) - СтрНайти(ИмяКоманды, "_"));
...
Процедура ЗаполнитьЗадачиНаВесьДеньКалендарь()
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.РедактированиеЗапрещено = Выборка.РедактированиеЗапрещено;
...
&НаКлиенте
Процедура ОткрытьФормуНастройкиВходящего()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияФормыНастройкиВходящего", ЭтотОбъект);
	ОткрытьФорму("Обработка.КонтактЦентр.Форма.НастройкаВходящего", Настройки, ЭтотОбъект, , , , ОписаниеОповещения);
		
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияФормыНастройкиВходящего(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(НастройкиОтображения, Результат);
		СохранитьНастройкиИОбновитьДанныеПланировщикаСервер();
	КонецЕсли;
...
Процедура ОткрытьФормуТекущегоЭлементаСписка(ВыбраннаяСтрока)
	...
	ИначеЕсли ЗначениеЗаполнено(ВыбраннаяСтрока.Ссылка)
		И ТипЗнч(ВыбраннаяСтрока.Ссылка) = Тип("ДокументСсылка.ЗадачаСотрудника") Тогда
		
		ПарамерыОткрытия = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ПараметрыФормы);
		//@skip-check bsl-legacy-check-expression-type
		ПарамерыОткрытия.Вставить("ОткрытьМодально", Истина);
		
		ОткрытьФорму(ИмяФормыЗадачиПоТипу(ВыбраннаяСтрока.Ссылка), ПарамерыОткрытия);
...
Функция ИмяСпискаПоДатеЗначения(ВыбранноеЗначение, ЗаписьКалендаря)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ЗадачаСотрудника") Тогда
		ДатыЗадачиСотрудника = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВыбранноеЗначение,
			"ДатаНачала, ДатаОкончания");
		ДатаНачала = ДатыЗадачиСотрудника.ДатаНачала;
		ДатаОкончания = ДатыЗадачиСотрудника.ДатаОкончания;
	Иначе
		ДатыЗаписиКалендаря = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаписьКалендаря,
			"ДатаНачала, ДатаОкончания");
		ДатаНачала = ДатыЗаписиКалендаря.Начало;
		ДатаОкончания = ДатыЗаписиКалендаря.Окончание;
	КонецЕсли;
...
Функция ИмяСпискаПоКолонкеЗначения(ВыбранноеЗначение, ЗаписьКалендаря)
	...
	Колонка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник, "КолонкаКалендаря");
...
Процедура ВосстановитьНастройки()
	...
	СотрудникТекущегоПользователя = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойОтветственный");
	ОсновнойКалендарьСотрудника   = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойКалендарь");
	
	СохраненноеЗначение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиКонтактЦентра",
		"КонтактЦентра_ОтображатьПодсказкуДоски");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(Выборка.ЗаписьКалендаря) = Тип("ДокументСсылка.ЗадачаСотрудника") Тогда
			...
			Если Выборка.Выполнена Тогда 
				ЭлементПланировщика.ЦветТекста = КалендариСотрудников.ЦветТекстаВыполненнойЗадачи(
...
&НаКлиенте
Процедура ИзменитьСтандартнуюОбработкуСписковКанбан(СтандартнаяОбработка)
	
	Если ТекущийЭлемент.Имя = "ЗадачиЗавершены" Тогда
...
&НаСервереБезКонтекста
Функция ТипыВнешнихСистем()
	
	ТипыВнешнихСистем = Новый Соответствие;
	ТипыВнешнихСистем.Вставить("Telegram", 			0);
	ТипыВнешнихСистем.Вставить("VK", 				1);
	ТипыВнешнихСистем.Вставить("WhatsApp Devino", 	5);
	ТипыВнешнихСистем.Вставить("WebChat", 			6);
	ТипыВнешнихСистем.Вставить("Webhook", 			7);
...
&НаСервереБезКонтекста
Функция ОбработаноВсеВходящее(СотрудникТекущегоПользователя)

	Возврат КонтактЦентр.ОбработаноВсеВходящее(СотрудникТекущегоПользователя);
	
КонецФункции

&НаСервереБезКонтекста
Процедура ОчиститьВходящееНаСервере(СотрудникТекущегоПользователя)
	
	КонтактЦентр.ОчиститьВходящееСотрудника(СотрудникТекущегоПользователя);
...
Процедура ЗаполнитьКомандыИзмененияСостояния()
	...
	// Страница "В работе"
	СостоянияДокументов.ЗаполнитьКомандыИзмененияСостояний(ЭтотОбъект, Элементы.ВРаботеКонтекстноеУстановитьСостояние);
	
	// Страница "Календарь"
	СостоянияДокументов.ЗаполнитьКомандыИзмененияСостояний(ЭтотОбъект, Элементы.СписокКонтекстноеМенюУстановитьСостояние);
	
	// Список ЗадачиПросрочены
	СостоянияДокументов.ЗаполнитьКомандыИзмененияСостояний(ЭтотОбъект, Элементы.ЗадачиПросроченыКонтекстноеМенюГруппаУстановитьСостояние);
	
	// Список ЗадачиНаСегодня
	СостоянияДокументов.ЗаполнитьКомандыИзмененияСостояний(ЭтотОбъект, Элементы.ЗадачиНаСегодняКонтекстноеМенюГруппаУстановитьСостояние);
	
	// Список ЗадачиНаЗавтра
	СостоянияДокументов.ЗаполнитьКомандыИзмененияСостояний(ЭтотОбъект, Элементы.ЗадачиНаЗавтраКонтекстноеМенюГруппаУстановитьСостояние);
	
	// Список ЗадачиБезСрока
	СостоянияДокументов.ЗаполнитьКомандыИзмененияСостояний(ЭтотОбъект, Элементы.ЗадачиБезСрокаКонтекстноеМенюГруппаУстановитьСостояние);
	
	// Список ЗадачиНаНеделе
	СостоянияДокументов.ЗаполнитьКомандыИзмененияСостояний(ЭтотОбъект, Элементы.ЗадачиНаНеделеКонтекстноеМенюГруппаУстановитьСостояние);
	
	// Список ЗадачиПозже
	СостоянияДокументов.ЗаполнитьКомандыИзмененияСостояний(ЭтотОбъект, Элементы.ЗадачиПозжеКонтекстноеМенюГруппаУстановитьСостояние);
	
	// Список ЗадачиЗавершены. Добавление задачи не требуется
	
КонецПроцедуры

&НаКлиенте
Функция ДокументАктивнойСтрокиСпискаКанбан(ИмяТекущегоЭлемента)
	
	Результат = Неопределено;
	
	ИменаЭлементовДинамическихСписков = Новый Массив;
	ИменаЭлементовДинамическихСписков.Добавить("ЗадачиПросрочены");
	ИменаЭлементовДинамическихСписков.Добавить("ЗадачиНаСегодня");
	ИменаЭлементовДинамическихСписков.Добавить("ЗадачиНаЗавтра");
	ИменаЭлементовДинамическихСписков.Добавить("ЗадачиБезСрока");
	ИменаЭлементовДинамическихСписков.Добавить("ЗадачиНаНеделе");
	ИменаЭлементовДинамическихСписков.Добавить("ЗадачиПозже");
	ИменаЭлементовДинамическихСписков.Добавить("ЗадачиЗавершены");

	Если ИменаЭлементовДинамическихСписков.Найти(ИмяТекущегоЭлемента) = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
...
Функция КартинкаПоТипуЗадачи(Задача)
	...
	Если ТипЗнч(Задача) = Тип("ДокументСсылка.Событие") 
		И ТипСобытияЗадачи(Задача) = Перечисления.ТипыСобытий.ЛичнаяВстреча Тогда
		Возврат 0;
	ИначеЕсли ТипЗнч(Задача) = Тип("ДокументСсылка.Событие") 
		И ТипСобытияЗадачи(Задача) = Перечисления.ТипыСобытий.ТелефонныйЗвонок Тогда
		Возврат 1;
	ИначеЕсли ТипЗнч(Задача) = Тип("ДокументСсылка.Событие") 
		И ТипСобытияЗадачи(Задача) = Перечисления.ТипыСобытий.Прочее Тогда
		Возврат 2;
	ИначеЕсли ТипЗнч(Задача) = Тип("ДокументСсылка.Событие") 
		И ТипСобытияЗадачи(Задача) =  Перечисления.ТипыСобытий.Запись Тогда
		Возврат 3;
	ИначеЕсли ТипЗнч(Задача) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		Возврат 4;
	ИначеЕсли ТипЗнч(Задача) = Тип("ДокументСсылка.ЗаданиеНаРаботу") 
		ИЛИ ТипЗнч(Задача) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		Возврат 5;
...
&НаСервереБезКонтекста
Функция ТипСобытияЗадачи(Задача)

	//@skip-check bsl-legacy-check-string-literal
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Задача, "ТипСобытия");
...
Процедура ВыборОтветственногоСервер(Результат, Источник, ДополнительныеПараметры)
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(СообщениеПользователю, Источник);
	КонецПопытки;
...
Процедура ОткрытьФормуЗадачи(ПараметрыПеретаскивания, Список = Неопределено, Создание = Ложь)
	...
	Если Создание Тогда
		...
		ПараметрыФормы.Вставить("ОткрытьМодально", Истина);
	Иначе
...
Процедура ОбновитьЭлементыТекущейДоски()
	...
	Для Каждого ДанныеКолонкиДоски Из КолонкиКалендаряСотрудника Цикл
		...
		КалендарьТекущейДоскиНедействителен = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяДоска,
			"Недействителен");
		
		Если ИндексКолонки = 0 Или КалендарьТекущейДоскиНедействителен Тогда
			ЦветФона = Новый Цвет(245, 245, 245);
		Иначе
			ЦветФона = Новый Цвет(233, 242, 204);
		КонецЕсли;
		
		ГруппаСписка = Элементы.Добавить("ГруппаСписка_" + Строка(ИндексКолонки), Тип("ГруппаФормы"),
			Элементы.ДоскаСписки);
		...
		РеквизитДинамическийСписок = ЭтотОбъект["СписокДел_" + Строка(ИндексКолонки)];
...
Процедура ДобавитьКомандыКонтекстногоМеню(ИндексКолонки, ТаблицаФормы)
	...
	КомандаИзменить = Команды.Добавить("КонтекстноеМенюИзменитьДелоСписка_" + Строка(ИндексКолонки));
...
Процедура ДобавитьЗаголовокКолонки(Колонка, ИндексКолонки)
	...
	КомандаСоздатьДело = Команды.Добавить("КомандаСоздатьДело_" + ИндексКолонки);
...
Процедура Подключаемый_Выбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбраннаяСтрока.Ссылка) = Тип("ДокументСсылка.ЗадачаСотрудника") Тогда
		ПараметрыФормы.Вставить("ОткрытьМодально", Истина);
	КонецЕсли;
