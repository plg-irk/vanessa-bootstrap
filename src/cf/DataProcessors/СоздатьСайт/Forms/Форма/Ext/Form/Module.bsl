...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступнаЗаписьНаУслуги = ВозможностиПриложения.КонстантаДоступнаВПриложении("ФункциональнаяОпцияИспользоватьПодсистемуРаботы");
	Элементы.ЗаписьНаУслуги.Видимость = ДоступнаЗаписьНаУслуги;
...
Процедура Декорация2Нажатие(Элемент)
	...
	ТекущийЭлемент = Элементы.EmailРегистрацииСайта;
КонецПроцедуры
...
Процедура СоздатьСайтПродолжить()
	...
	ИначеЕсли ДанныеСайтаСтруктура.Свойство("СайтСоздан") И ДанныеСайтаСтруктура.СайтСоздан = Ложь Тогда
		...
		Если (ОбщегоНазначенияКлиент.ДатаСеанса() - СчетчикОжиданияСозданияСайта) > 1800 Тогда // ждем создания сайта не более 30 мин, потом выдаем ошибку
			ЗакрытьФормуДлительнойОперации(ФормаДлительнойОперации);
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Время ответа от сайта превышено, попробуйте повторить операцию позже'"));
			Возврат;
		Иначе	
			ПодключитьОбработчикОжидания("СоздатьСайтПродолжить", 2, Истина);
		КонецЕсли;
...
Функция ПолучитьДанныеЗапросом(HTTPМетод, ТелоЗапроса, АдресЗапроса, ЗаголовкиЗапроса, 
	...
	Если ОтветHTTP.КодСостояния<>200 Тогда
		ОбщегоНазначения.СообщитьПользователю("Ответ сервера: " + ОтветHTTP.КодСостояния);
	КонецЕсли;
...
Функция ПроверитьИмяСайтаТекст(Текст)
	...
	Пока Номер <= СтрДлина(Текст) Цикл
		ПроверяемыйСимвол = Сред(Текст, Номер, 1);
...
Процедура УстановитьПометку(Пометка, ИмяТЧ)
	...
	НоменклатураВДокументахКлиентСервер.УстановитьФлагУПодчиненных(ЭтотОбъект[ИмяТЧ].ПолучитьЭлементы(), "Флаг", Пометка);
	
	ТекущийЭлемент = Элементы[ИмяТЧ];
...
Функция СоответствиеВТЗ(МассивЗначений) 
	...
	Для каждого элСоответствия Из МассивЗначений[0] Цикл
		ТаблицаЗначений.Колонки.Добавить(элСоответствия.Ключ);
	КонецЦикла; 
...
Функция ПолучитьИмяСайтаИзИмениОрганизации()
	...
	ИмяСайтаТекст = СтроковыеФункции.СтрокаЛатиницей(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнаяОрганизация, "Наименование"));
...
Процедура ПодгрузитьКартинку(ВыбранноеЗначение)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'СоздатьСайт.ЗагрузкаКартинки - удаление временного файла'"), УровеньЖурналаРегистрации.Ошибка, , , ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
