...
Функция ИдентификаторПечатнойФормы(ВключаяУслуги = Ложь, ИспользоватьФаксимиле = Ложь) Экспорт
	...
	Возврат ?(ВключаяУслуги, "ТОРГ12СУслугами", "ТОРГ12");
...
Функция СформироватьПФ(ОписаниеПечатнойФормы, ДанныеОбъектовПечати, ОбъектыПечати, ВключаяРаботыУслуги,
	ВыводитьОбластьФаксимиле = Ложь) Экспорт
	...
	ТекстСообщенияМакетИзменен = ПечатьДокументовУНФ.ТекстСообщенияМакетИзменен(НСтр("ru = 'ТОРГ-12'"));
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектовПечати Цикл
		...
		Если ОбластиМакета.ОбластьМакетаШапка <> Неопределено Тогда
			...
			ДанныеПечати.Вставить("ПредставлениеПоставщика", ПечатьДокументовУНФ.ОписаниеОрганизации(
				СведенияОПоставщике,
				"НаименованиеДляПечати,ИНН,Свидетельство,ЮридическийАдрес,Телефоны,Факс,НомерСчета,Банк,БИК,КоррСчет"));
			...
			ДанныеПечати.Вставить("ПредставлениеГрузоотправителя", ПечатьДокументовУНФ.ОписаниеОрганизации(
				...
				"НаименованиеДляПечати,ИНН,ФактическийАдрес,Телефоны,Факс,НомерСчета,Банк,БИК,КоррСчет"));
		...
		ОбластьМакета = ОбластиМакета.ОбластьМакетаПодвалСФаксимиле;
		Если ОбластьМакета <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ДанныеОбъекта.ФаксимилеРуководителя)
				...
				Или ЗначениеЗаполнено(ДанныеОбъекта.ФаксимилеКладовщика)
			...
			Если Не ЕстьФаксимиле И МакетИзменен Тогда
				ОбластиМакета.ОбластьМакетаПодвалБезФаксимиле.Параметры.Заполнить(ДанныеПечати);
				ОбластьПодписи = ТабличныйДокумент.Вывести(ОбластиМакета.ОбластьМакетаПодвалБезФаксимиле);
			...
			Иначе
				ОбластиМакета.ОбластьМакетаПодвалСФаксимиле.Параметры.Заполнить(ДанныеПечати);
				ОбластьПодписи = ТабличныйДокумент.Вывести(ОбластиМакета.ОбластьМакетаПодвалСФаксимиле);
			КонецЕсли;
...
Функция СтрокаКорректноРазмещаетсяНаСтранице(ТабличныйДокумент, СтруктураОбластейМакета, Итоги,
	...
	Если Итоги.НомерСтроки = Итоги.КоличествоСтрок - 1 Тогда
		...
		Если ВыводитьОбластьФаксимиле Тогда
			ИмяСекции = "ОбластьМакетаПодвалСФаксимиле";
		Иначе
			ИмяСекции = "ОбластьМакетаПодвалБезФаксимиле";
		КонецЕсли;
