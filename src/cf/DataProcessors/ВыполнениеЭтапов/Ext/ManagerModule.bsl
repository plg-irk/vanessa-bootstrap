	...
	Для каждого СтрокаЗаказ Из Заказы.Строки Цикл
		Для каждого СтрокаПродукция Из СтрокаЗаказ.Строки Цикл
			Для ии = 1 По КоличествоЭтапов Цикл
				...
				КоличествоВДокумент = СтрокаПродукция["ЭтапКПроизводству"+ии];
				Если НЕ ЗначениеЗаполнено(СтрокаПродукция.ВидПроизводства) Тогда
					Этап = Справочники.ЭтапыПроизводства.ЗавершениеПроизводства;
				КонецЕсли; 
				...
				Если ЗначениеЗаполнено(СтрокаПродукция.СтруктурнаяЕдиница) Тогда
					ЗначенияПолей.Вставить("ПодразделениеЗавершающегоЭтапа", СтрокаПродукция.СтруктурнаяЕдиница);
				Иначе
					ЗначенияПолей.Вставить("ПодразделениеЗавершающегоЭтапа", ПроизводствоСервер.ПодразделениеЭтапа(Этап));
				КонецЕсли;
				...
				ЗначенияПолей.Вставить("КоличествоПродукции", КоличествоВДокумент);
...
Функция ВыполнитьЭтапСборкаЗапасов(ДокументОбъект, ЗначенияПолей, Ошибки)
	...
	СтрокаПродукция = НайтиСтрокуПродукции(ЗначенияПолей, ДокументОбъект);
	
	Если Не ЗначениеЗаполнено(СтрокаПродукция) Тогда
		
		СтрокаПродукция = ДокументОбъект.Продукция.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаПродукция, ЗначенияПолей, "ЗаказПокупателя, Номенклатура, Характеристика, Партия, Спецификация");
		Если ЗначениеЗаполнено(ЗначенияПолей.ВидПроизводства) Тогда
			СтрокаПродукция.ПодразделениеЗавершающегоЭтапа = ЗначенияПолей.ПодразделениеЗавершающегоЭтапа;
		КонецЕсли; 
		СтрокаПродукция.ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗначенияПолей.Номенклатура, "ЕдиницаИзмерения");
		СтрокаПродукция.Количество = ЗначенияПолей.КоличествоПродукции;
		ТабличныеЧастиУНФКлиентСервер.ЗаполнитьКлючСвязи(ДокументОбъект.Продукция, СтрокаПродукция, "КлючСвязи");
		ЗначенияПолей.Вставить("КлючСвязи", СтрокаПродукция.КлючСвязи);
		
		Если ЗначениеЗаполнено(ЗначенияПолей.ВидПроизводства) Тогда
			НоваяСтрокаВыполненныйЭтап = ДокументОбъект.ВыполненныеЭтапы.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаВыполненныйЭтап, ЗначенияПолей);
		КонецЕсли;
		
	Иначе
		
		СтрокаПродукция.Количество = СтрокаПродукция.Количество + ЗначенияПолей.КоличествоПродукции;
	...
	Для каждого СтрокаТаблицы Из ТаблицаСостава Цикл
		...
		НоваяСтрока.КлючСвязиПродукция = СтрокаПродукция.КлючСвязи;
...
Функция НайтиСтрокуПродукции(ЗначенияПолей,ДокументОбъект)
	...
	Для Каждого СтрокаПродукция Из СтрокиПродукция Цикл
		...
		СтруктураПоискаЭтапа = Новый Структура;
		СтруктураПоискаЭтапа.Вставить("КлючСвязи",СтрокаПродукция.КлючСвязи);
		СтруктураПоискаЭтапа.Вставить("Этап",ЗначенияПолей.Этап);
		
		СтрокиВыполненныеЭтапы = ДокументОбъект.ВыполненныеЭтапы.НайтиСтроки(СтруктураПоискаЭтапа);
		
		Если СтрокиВыполненныеЭтапы.Количество() = 1 Или Не ЗначениеЗаполнено(ЗначенияПолей.ВидПроизводства) Тогда
			
			Возврат СтрокаПродукция;
...
Функция ВыполнитьЭтапСдельныйНаряд(ДокументОбъект, ЗначенияПолей, Ошибки)
	...
	Для каждого СтрокаОперации Из ТаблицаОпераций Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрокаОперация, ЗначенияПолей, , "ПодразделениеЗавершающегоЭтапа,КлючСвязи");
		Если ЗначениеЗаполнено(ЗначенияПолей.ВидПроизводства) Тогда
...
Функция СоставЭтапа(ЗначенияПолей)
	...
	Если ЗначениеЗаполнено(ЗначенияПолей.ЗаказНаПроизводство) Тогда
		...
		|	ЗаказНаПроизводствоРаспределениеЗапасов.Количество / ЗаказНаПроизводствоПродукция.Количество *
		|		&КоличествоПродукции КАК Количество,
