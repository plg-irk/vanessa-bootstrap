...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияЗаголовок.Заголовок = 
	СтрШаблон(НСтр("ru = 'Необходимо выбрать реквизит 1С для атрибута маркетплейса <%1>.'"), Параметры.Атрибут);
...
Функция ПолучитьДоступноеПоле(ПутьКДанным, Форма)

	КоллекцияПолей = КоллекцияПолей(Форма);
	...
	СообщениеОДубле = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ПроверитьДублированиеДополнительногоРеквизита(ПутьКДанным, ДоступноеПоле.Заголовок,
		Форма.КомпоновщикНастроек.Настройки[Форма.ИмяКоллекцииПолей]);
