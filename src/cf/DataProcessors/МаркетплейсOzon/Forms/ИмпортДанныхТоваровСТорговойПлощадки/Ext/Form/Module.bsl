...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИдентификаторыТоваровДляИмпорта = Параметры.ИдентификаторыТоваровДляИмпорта;
    ИмпортируемыеТоварыИдентификаторыТоваровДляИмпортаПоЗаполнить( ИдентификаторыТоваровДляИмпорта );
	...
	Заголовок = НСтр("ru = 'Импорт описаний товаров с торговой площадки Ozon'") + ", " + СокрЛП( УчетнаяЗаписьМаркетплейса );
...
Процедура ИмпортируемыеТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ТекущиеДанные.ВариантЗагрузки <> 2 
		...
		Если Поле.Имя <> "ИмпортируемыеТоварыНоменклатура" 
			 ...
			 И Поле.Имя <> "ИмпортируемыеТоварыИсточникКатегорииМаркетплейса"
...
Процедура ФормаПараметрыИмпортаДанныхОписаниеОбновить()
	...
	Если ВариантЗагрузкиДанных = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ВариантЗагрузкиЗагрузитьНесопоставленныеКарточкиТоваров() Тогда
		Если СоздатьИерархиюНоменклатуры Тогда
			ПараметрыИмпортаДанныхЗаголовок = ПараметрыИмпортаДанныхЗаголовок
...
Процедура УстановитьВидимостьДоступность()
	...
	Если ВариантЗагрузкиДанных = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ВариантЗагрузкиЗагрузитьНесопоставленныеКарточкиТоваров()
	ИЛИ ВариантЗагрузкиДанных = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ВариантЗагрузкиЗагрузитьНесопоставленныеКарточкиТоваровПоИдентификаторам()
	Тогда 
...
Процедура ФормаРежимОтображенияЭлементыОбновить( ПараметрФормаРежимОтображения )
	...
	Если ФормаРежимОтображенияТекущий = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ФормаРежимОтображенияОткрытие() Тогда

		Элементы.ГруппаНазадВперед.Видимость = Ложь;

		Если ВариантЗагрузкиДанных = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ВариантЗагрузкиОбновитьСопоставленныеКарточкиТоваров() Тогда 
		...
		ИначеЕсли ВариантЗагрузкиДанных = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ВариантЗагрузкиЗагрузитьНесопоставленныеКарточкиТоваров() Тогда 
		
			Элементы.ГруппаДополнительныеДействия.Видимость = Истина;
			Элементы.ГруппаИмпортДанных.Видимость = Ложь;
			Элементы.ГруппаДетальныеНастройкиИмпорта.Видимость = Истина;
			
		ИначеЕсли ВариантЗагрузкиДанных = 
		ИнтеграцияСМаркетплейсомOzonКлиентСервер.ВариантЗагрузкиЗагрузитьНесопоставленныеКарточкиТоваровПоИдентификаторам() Тогда 
	...
	ИначеЕсли ФормаРежимОтображенияТекущий = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ФормаРежимОтображенияЗагрузитьВсеОписания() Тогда
	...
	ИначеЕсли ФормаРежимОтображенияТекущий = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ФормаРежимОтображенияЗагрузитьСДетальнымиНастройками() Тогда
...
&НаСервере
Процедура ИмпортируемыеТоварыИдентификаторыТоваровДляИмпортаПоЗаполнить( ИдентификаторыТоваровДляИмпорта )
	
	Если ТипЗнч( ИдентификаторыТоваровДляИмпорта ) <> Тип( "Массив" ) Тогда
		Возврат;
	КонецЕсли;
	
	Если ИдентификаторыТоваровДляИмпорта.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	СтрокаДереваГруппаОбновить = ИмпортируемыеТовары.ПолучитьЭлементы().Добавить();
	СтрокаДереваГруппаОбновить.ВариантЗагрузки = 2;
	СтрокаДереваГруппаОбновить.ИндексСтатуса = -2;

	Для Каждого ИдентификаторыТоваровДляИмпортаЭлемент Из ИдентификаторыТоваровДляИмпорта Цикл
		
		СтрокаДереваВторогоУровня = СтрокаДереваГруппаОбновить.ПолучитьЭлементы().Добавить();
		
		СтрокаДереваВторогоУровня.ПредставлениеОбъектаМаркетплейса = ИдентификаторыТоваровДляИмпортаЭлемент.Наименование;
		СтрокаДереваВторогоУровня.ИдентификаторПубликации = ИдентификаторыТоваровДляИмпортаЭлемент.Артикул; // offer_id
		СтрокаДереваВторогоУровня.ИдентификаторFBOSKU = ИдентификаторыТоваровДляИмпортаЭлемент.Идентификатор; 
