...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ОперацияНалоговогоУчетаПоНДФЛ) Тогда
		Элементы.ОперацииУчетаНДФЛ.Видимость = Истина;
		Элементы.ПерерасчетНДФЛ.Видимость = Истина;
		Элементы.УточнениеНДФЛ_РКСН.Видимость = Истина;
	Иначе
		Элементы.ОперацииУчетаНДФЛ.Видимость = Ложь;
		Элементы.ПерерасчетНДФЛ.Видимость = Ложь;
		Элементы.УточнениеНДФЛ_РКСН.Видимость = Ложь;
	КонецЕсли;

	Если ПравоДоступа("Чтение", Метаданные.Документы.ПерерасчетНДФЛ) Тогда
		Элементы.ПерерасчетНДФЛ.Видимость = Истина;
	Иначе
		Элементы.ПерерасчетНДФЛ.Видимость = Ложь;
	КонецЕсли;
...
Процедура Отчеты(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.ФормаСпискаОтчетов", ФормаПараметры, , ПутьКПодсистеме);
...
&НаКлиенте
Процедура ПерерасчетНДФЛ(Команда)
	ОткрытьФорму("Документ.ПерерасчетНДФЛ.ФормаСписка", , , УникальныйИдентификатор);
КонецПроцедуры

&НаКлиенте
Процедура ОперацииУчетаНДФЛ(Команда)
	ОткрытьФорму("Документ.ОперацияНалоговогоУчетаПоНДФЛ.Форма.ФормаСписка", , , УникальныйИдентификатор);
КонецПроцедуры

&НаКлиенте
Процедура УточнениеНДФЛ_РКСН(Команда)
	ОткрытьФорму("Обработка.УточнениеУчетаНДФЛПоНалоговымБазамПриОтменеДолейРКиСН.Форма");
КонецПроцедуры
#КонецОбласти