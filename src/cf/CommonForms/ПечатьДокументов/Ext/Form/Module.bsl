...
Процедура СохранитьНаКомпьютер(Команда)
	...
	СтатистикаПоПоказателямКлиентСервер.ДобавитьСобытие("ПечатьДокументов.КнопкаСохранить"); 
...
Процедура Отправить(Команда)
	...
	СтатистикаПоПоказателямКлиентСервер.ДобавитьСобытие("ПечатьДокументов.КнопкаОтправитьПоПочте"); 
КонецПроцедуры
...
Процедура УстановитьЗаголовокФормы()
	...
	ЗаголовокФормы = ЗаголовокДляОдногоДокумента(ЗаголовокФормы); // УНФ
...
Функция ПереформироватьПечатнуюФорму(ИмяМакета, ИмяРеквизита)
	...
	ПечатьДокументовУНФ.ЗаписатьНастройкуПользователяЗаполнятьФаксимиле(ПодписьИПечать, ИмяМакета);
...
Процедура ДобавитьПодписьИПечать()
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		НастроитьВидимостьОбластейФаксимиле(ТабличныйДокумент, Истина); // УНФ
...
Процедура УбратьПодписьИПечать()
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		НастроитьВидимостьОбластейФаксимиле(ТабличныйДокумент, Ложь);
...
// Формирует заголовок формы для случаев, когда все значения списка объектов - один и тот же документ.
// Параметры:
//  ЗаголовокФормы - Строка
//
&НаСервере
Функция ЗаголовокДляОдногоДокумента(ЗаголовокФормы)
	
	Если Не ЗаголовокФормы = НСтр("ru = 'Печать документов'") Тогда
		Возврат ЗаголовокФормы;
	КонецЕсли;
	
	Результат = ЗаголовокФормы;
	
	Если ОбъектыПечати.Количество() > 1 И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ОбъектыПечати[0].Значение))
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектыПечати[0].Значение, "Ссылка", Истина) <> Неопределено Тогда
			МассивОбъектов = ОбъектыПечати.ВыгрузитьЗначения();
			СвернутыйМассивОбъектов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивОбъектов);
			
			Если СвернутыйМассивОбъектов.Количество() = 1 Тогда
				Результат = Строка(ОбъектыПечати[0].Значение);
			КонецЕсли;
...
&НаСервере
Процедура НастроитьВидимостьОбластейФаксимиле(ТабличныйДокумент, ПоказатьФаксимиле)
	
	ЕстьПодписьИПечать = ЕстьПодписиИПечатиДляОбъектовПечати();
	
	Для Каждого Область Из ТабличныйДокумент.Области Цикл
		Если СтрНайти(Область.Имя, "СкрываемаяБезПодписейИПечати") <> 0 Тогда
			Область.Видимость = ПоказатьФаксимиле И ЕстьПодписьИПечать;
		КонецЕсли;
	КонецЦикла;
