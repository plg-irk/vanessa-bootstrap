...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Контекст.Вставить("РежимНастройки", Параметры.РежимНастройки);
	
	Если Не Контекст.РежимНастройки И (Параметры.Настройки = Неопределено Или Параметры.АдресСхемы = Неопределено) Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Не Параметры.Настройки = Неопределено И Не Параметры.АдресСхемы = Неопределено Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(Контекст.ПолноеИмя) Тогда
		...
		Если НЕ ОтчетОбъект=Неопределено Тогда
			...
			Если НЕ МетаданныеОтчета.ОсновнаяСхемаКомпоновкиДанных=Неопределено Тогда
				...
				Если СтрНайти(МетаданныеОтчета.ПолноеИмя(), "ВнешнийОтчет.")>0 Тогда
					Если Не Параметры.Настройки = Неопределено Тогда
						Настройки = Параметры.Настройки;
...
Процедура Подключаемый_ПривязкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтрокаПривязки = ПривязкиПолей.НайтиПоИдентификатору(Число(ПривязкаИД));
...
&НаКлиенте
Процедура ДоступенДополнительныйОтчетПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВариантНаименованиеПриИзменении(Элемент)
...
Функция СформироватьИнформациюОбОбДополнительномОтчете(Отчет)
	...
	Результат.Вставить("Отчет");
	Результат.Вставить("ОтчетИмя");
	Результат.Вставить("ТипОтчета");
...
Процедура ВариантыОтчетаОбработчикАктивизацииСтроки()
	...
	КонецЕсли;
	УстановитьЧтоБудетДальше(ЭтотОбъект, ЗначениеЗаполнено(ВариантСсылка));
	УстановитьДоступностьАвтора(ЭтотОбъект);
...
Функция ПредставлениеПривязки(Идентификатор, Синоним = "")
	...
	Если ПустаяСтрока(Синоним) Тогда
		Синоним = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Стр.Объект, "Синоним");
	КонецЕсли; 
	ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(Стр.Объект);
	Если Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
		Представление = СтрШаблон(НСтр("ru = 'Из справочника %1'"), Синоним);
	ИначеЕсли Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
		Представление = СтрШаблон(НСтр("ru = 'Из документа %1'"), Синоним);
	Иначе
		Представление = СтрШаблон(НСтр("ru = 'Из %1'"), Синоним);
	КонецЕсли; 
...
Функция ПредставлениеОтметки(НаименованиеТега, ПометкаУдаления = Ложь, НавигационнаяСсылкаФС = "" , БезПривязки = Ложь)
	...
	Шрифт = ?(ПометкаУдаления, ШрифтыСтиля.ВажнаяНадписьШрифт, ШрифтыСтиля.ШрифтПолужирныйЗачеркнутыйУНФ); 
