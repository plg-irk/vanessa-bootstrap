...
Процедура Кнопка4Нажатие(Элемент)
	...
	// Сбор статистики
	СборСтатистикиМПКлиентСерверПереопределяемый.ОтправитьДействиеВGA(ИмяФормы + "." + Элемент.Имя + ".Нажатие");
	// Конец Сбор статистики
...
Процедура ПолучитьЗначенияПараметровФормы()
	...
	Иначе
		Заголовок = НСтр("ru='Прием оплаты'");
	КонецЕсли;
...
Процедура ЗаблокироватьФорму()
	...
	ОбщегоНазначенияМПКлиентСервер.УстановитьСвойствоЭлементовФормы(ЭтотОбъект,
...
Процедура Подключаемый_ПоказатьРаспечататьСлипЧек()
	...
	Иначе
		...
		ОткрытьФорму("ОбщаяФорма.СлипЧекПлатежнойСистемыМП", ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения);
	КонецЕсли;
