...
Процедура ЗаполнитьНастройкиДинамическогоСписка(Знач ИмяТаблицы, Знач ИмяКоманды)
	...
	Если ПустаяСтрока(Заголовок) Тогда
		ЗаголовокФормы = НСтр("ru = 'Документы по кассе'");
	Иначе
...
Функция ЗаголовкиПолейДляРеестраДокументов()
	...
	ЗаголовокКонтрагента = Элементы.СписокКонтрагентПодотчетник.Заголовок;
...
Функция СписокПолейДляЗамены()
	...
	Результат.Вставить("Организация", "ОрганизацияДляОтбора");
	Результат.Вставить("СуммаДокумента", "Сумма");
...
Процедура Подключаемый_ВыполнитьКоманду(Команда)
	...
	Если СтрНайти(ИмяКоманды, "РеестрДокументов") <> 0 Тогда
		ЗаполнитьНастройкиДинамическогоСписка("ДокументыПоКассе", ИмяКоманды);
	КонецЕсли;
	
	ПодключаемыеКомандыКлиент.НачатьВыполнениеКоманды(ЭтотОбъект, Команда, Элементы.ДокументыПоКассе);
КонецПроцедуры
...
Процедура ВыполнитьКомандуНаСервере(ПараметрыВыполнения)
	ПодключаемыеКоманды.ВыполнитьКоманду(ЭтотОбъект, ПараметрыВыполнения, Элементы.ДокументыПоКассе);
КонецПроцедуры

//@skip-check module-unused-method
&НаКлиенте
Процедура Подключаемый_ОбновитьКоманды()
	ПодключаемыеКомандыКлиентСервер.ОбновитьКоманды(ЭтотОбъект, Элементы.ДокументыПоКассе);
КонецПроцедуры
