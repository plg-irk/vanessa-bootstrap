...
Процедура ПередФормированиеСпискаОтложенныхОбработчиков(ИтерацииОбновления) Экспорт
	...
	Для Каждого ИтерацияОбновления Из ИтерацииОбновления Цикл
		Если ИтерацияОбновления.Подсистема = "ЗарплатаКадрыБазовая" Тогда
			ИнтеграцияБЗКБУНФ.КоррекцияОбработчиковОбновления(ИтерацияОбновления.Обработчики);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриЗаполненииОбъектовПланируемыхКУдалению(Объекты) Экспорт
	...
	ОбновлениеИнформационнойБазыУНФ.ПриЗаполненииОбъектовПланируемыхКУдалению(Объекты);
