...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ТТНИсходящаяЕГАИС.Форма.ФормаДокумента" Тогда
		...
		ИмяКоманды = "КомандаУНФЗаполнитьЦеныПоВидуЦен";
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Заголовок = НСтр("ru = 'Перезаполнить цены по виду цен'");
		Команда.Действие  = "Подключаемый_ВыполнитьПереопределяемуюКоманду";
		Команда.Подсказка = НСтр("ru = 'Перезаполнить цены по виду цен'");
		
		НовыйЭлемент = Форма.Элементы.Добавить(
			ИмяКоманды,
			Тип("КнопкаФормы"),
			Форма.Элементы.ТоварыГруппаДополнительныеКоманды);
