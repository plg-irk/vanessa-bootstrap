...
Процедура ЗаполнитьОтчетРеестрДокументовПодтверждающихЛьготыФорма2020_1_2021_1(ПараметрыОтчета, Контейнер) Экспорт
	...
	Организация   = ПрослеживаемостьПереопределяемый.ВсяОрганизация(ПараметрыОтчета.Организация);
...
#Область Раздел2
	...
	Если Контейнер.Свойство("Раздел2") Тогда
		...
		Для Каждого Показатель Из СтраницаРаздел2 Цикл
			СтраницаРаздел2[Показатель.Ключ] = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(ТипЗнч(Показатель.Значение));
		КонецЦикла;
			...
			Если НЕ ПерваяСтраница Тогда
				...
				Для Каждого Показатель Из СтраницаРаздел2 Цикл
					СтруктураНовойТаблицы.Вставить(
						Показатель.Ключ, РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(ТипЗнч(Показатель.Значение)));
				КонецЦикла;
...
#Область Раздел12
	...
	Если Контейнер.Свойство("Раздел12") Тогда
		...
		|	СчетФактураВыданный.СуммаНДСДокументаКомиссия КАК СуммаНДСДокументаКомиссия,
		...
		|ГДЕ
		|	НЕ СчетФактураВыданный.ВидОперации В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Корректировка), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.НаАвансКомитента))
		...
		|	ВЫБОР
		|		КОГДА СчетФактураВыданный.Контрагент.КПП ПОДОБНО """"
		|			ТОГДА ЕСТЬNULL(СведенияОСсылкахНаДаты.КПП, """")
		|		ИНАЧЕ СчетФактураВыданный.Контрагент.КПП
		...
		|	СчетФактураВыданный.СуммаДокумента + (СчетФактураВыданный.СуммаНДСДокумента + СчетФактураВыданный.СуммаНДСДокументаКомиссия) КАК СтоимТовСНалВс,
		|	СчетФактураВыданный.СуммаДокумента КАК СтоимТовБНалВс,
...
Функция ТекстЗапросаРаздел3Начисления()
    ...
    |            ТОГДА НДСЗаписиКнигиПродажОборотыДетальные.СуммаБезНДСОборот +
    |                НДСЗаписиКнигиПродажОборотыДетальные.НДСОборот
    |        ИНАЧЕ 0
    ...
    |    ПредварительныеНачисленияРаздел3.П000300003003 КАК П000300003003,
    ...
    |    ПредварительныеНачисленияРаздел3.П000300004003 КАК П000300004003,
...
Процедура ЗаполнитьРеестрДляПодтвержденияСтавки0(Контейнер, ПараметрыОтчета)
	...
	Запрос.УстановитьПараметр("Организация", ПрослеживаемостьПереопределяемый.ВсяОрганизация(ПараметрыОтчета.Организация));
...
Функция СведенияОРуководителе(ВыгружаемыйДокумент) 	
	...
	ОрганизацияДокумента = ВыгружаемыйДокумент.Организация;
	Если НЕ ЗначениеЗаполнено(ОрганизацияДокумента) Тогда
		Возврат Результат;
	КонецЕсли;
	
	РеквизитФизЛицо = "ФизическоеЛицо";
	РеквизитыОрганизации = СтрШаблон("%1, %2", РеквизитФизЛицо, "ПодписьРуководителя, ЮридическоеФизическоеЛицо");
	ДанныеОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОрганизацияДокумента, РеквизитыОрганизации);
	
	ФизЛицо = Неопределено;	
	Если ЗначениеЗаполнено(ДанныеОрганизации.ПодписьРуководителя) Тогда
		ФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеОрганизации.ПодписьРуководителя, РеквизитФизЛицо);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ФизЛицо)
		И ДанныеОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		ФизЛицо = ДанныеОрганизации.ФизическоеЛицо;
	КонецЕсли;
	...
	ДанныеФЛ = РегистрыСведений.ФИОФизическихЛиц.СрезПоследних(ВыгружаемыйДокумент.Дата, Новый Структура(РеквизитФизЛицо, ФизЛицо));
...
Функция СведенияОбИП(ВыгружаемыйДокумент)
	...
	Если НЕ ЗначениеЗаполнено(ВыгружаемыйДокумент.Организация) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ДанныеОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВыгружаемыйДокумент.Организация, 
		"ИндивидуальныйПредприниматель, ФизическоеЛицо, НаименованиеПолное, Наименование");
		
	ФЛ = ДанныеОрганизации.ИндивидуальныйПредприниматель;
	Если НЕ ЗначениеЗаполнено(ФЛ) Тогда
		ФЛ = ДанныеОрганизации.ФизическоеЛицо;
	КонецЕсли;
	
	Если ФЛ = Справочники.ФизическиеЛица.ПустаяСсылка() Тогда
		ФИОИП = ДанныеОрганизации.НаименованиеПолное;
		ЧастиИмени = Справочники.ФизическиеЛица.ПолучитьСтруктуруФИОПоИмени(ФИОИП);
		Для Каждого Элемент Из ЧастиИмени Цикл
			Если ЗначениеЗаполнено(Элемент.Значение) Тогда
				Результат[Элемент.Ключ] = СокрЛП(Элемент.Значение);
			КонецЕсли;
		КонецЦикла;
	Иначе
