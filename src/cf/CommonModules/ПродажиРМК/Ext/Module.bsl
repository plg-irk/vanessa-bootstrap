...
Функция СтрокаТовара(Знач ТекущийВыбор, ДополнительныеИсходящиеДанные = Неопределено) Экспорт
	...
	ДополнитьСтрокуТовараВычисляемымиДанными(НоваяСтрока, ЭтотОбъект);
	...
	Если НоваяСтрока.КоличествоВПотребительскойУпаковке = НоваяСтрока.КоэффициентУпаковки Тогда
		НоваяСтрока.ПоштучноеВыбытие = Ложь;
	КонецЕсли;
	
	НоваяСтрока.ИспользоватьХарактеристики =
		ОбщегоНазначенияРМК.ТребуетсяВводХарактеристикиДляНоменклатуры(ТекущийВыбор.Номенклатура);
		
	Если ТекущийВыбор.Свойство("ПолучитьЦену") Тогда
		ПараметрыРасчетаЦены = ОбщегоНазначенияРМК.ПараметрыРасчетаЦены();
		ПараметрыРасчетаЦены.ВидЦены = ЭтотОбъект.РозничныйВидЦены;
		ПараметрыРасчетаЦены.ТорговыйОбъект = Объект.ТорговыйОбъект;
		ПараметрыРасчетаЦены.Организация = Объект.Организация;
		ПараметрыРасчетаЦены.Номенклатура = ТекущийВыбор.Номенклатура;
		
		ОбщегоНазначенияРМКПереопределяемый.РассчитатьЦенуВыбранногоТовара(ПараметрыРасчетаЦены);
		НоваяСтрока.Цена = ПараметрыРасчетаЦены.Цена;
	КонецЕсли;
	
	Если ЭтотОбъект.Свойство("ПараметрыДляОбработкиШтрихкода") Тогда
		Если ТекущийВыбор.Маркируемый Тогда
			ПараметрыДляОбработкиШтрихкода = ЭтотОбъект.ПараметрыДляОбработкиШтрихкода;
			...
			Если ЭтоКодМарки Тогда
				...
				ШтрихкодированиеРМК.РазобратьМаркуИЗаполнитьДанные(СтрокаТовары, ТекущийШтрихкод, СтрокаТовары.ОсобенностьУчета,
					ПараметрыДляОбработкиШтрихкода);
					
				Если Не ПараметрыДляОбработкиШтрихкода.Свойство("ДозаполнитьПоВосстановленнойМарке")
						И ПараметрыДляОбработкиШтрихкода.Свойство("СвойстваДляОбновленияВСтроке") Тогда
					ЗаполнитьЗначенияСвойств(ТекущийВыбор, НоваяСтрока,
						"Цена, Упаковка, Количество, КоличествоУпаковок, Весовой");
				КонецЕсли;
			КонецЕсли;
			
			Если ЭтоКодМарки И СтрокаТовары.ЭтоАлкогольнаяПродукция Тогда
				...
				Если НужнаПроверкаСтатусаМарки Тогда
					...
					УдалитьПараметрОбъект = Ложь;
					Если Не ПараметрыДляОбработкиШтрихкода.Свойство("Объект") Тогда
						ПараметрыДляОбработкиШтрихкода.Вставить("Объект", Объект);
						УдалитьПараметрОбъект = Истина;
					КонецЕсли;
					ШтрихкодированиеРМКПереопределяемый.ПроверитьСтатусМарки(ПараметрыДляОбработкиШтрихкода);
					ПараметрыДляОбработкиШтрихкода.Удалить("СтрокаТовары");
					Если УдалитьПараметрОбъект Тогда
						ПараметрыДляОбработкиШтрихкода.Удалить("Объект");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура ДополнитьСтрокуТовараВычисляемымиДанными(СтрокаТовара, Форма)
	...
	Если СтрокаТовара.Маркируемый И Не ЗначениеЗаполнено(СтрокаТовара.КодМаркировки)
		И СтрокаТовара.ПоштучноеВыбытие
		И ОбщегоНазначенияРМК.ЭтоВидОперацииПродажа(Объект.ВидОперации) Тогда
		ЗаполнитьДанныеКодовМаркировкиДляВыбора(СтрокаТовара, Форма);
		
		Если СтрокаТовара.Свойство("КодыМаркировкиПоДаннымИБ") Тогда
			КодыМаркировкиДоступныеДляВыбора = КодыМаркировкиДоступныеДляВыбора(
				СтрокаТовара.КодыМаркировкиПоДаннымИБ, СтрокаТовара.ПредставлениеЕдиницыИзмерения);
...
Процедура ЗаполнитьДанныеКодовМаркировкиДляВыбора(СтрокаТовара, Форма)
	
	МассивКодовМаркировки = Новый Массив;
	КодыМаркировкиПоДаннымИБ = ОбщегоНазначенияРМК.КодыМаркировкиПоДаннымИБ(Форма, СтрокаТовара);
	Для Каждого КодМаркировкиПоДаннымИБ Из КодыМаркировкиПоДаннымИБ Цикл
		...
		ИнтерфейсРМКСлужебныйКлиентСервер.ЗаменитьСимволGS1(КодМаркировки);
...
Функция КодыМаркировкиДоступныеДляВыбора(КодыМаркировки, ПедставлениеЕдиницыИзмерения)
	...
	Для Каждого ЭлементСписка Из КодыМаркировки Цикл
		Если ВыводитьОстатокВПредставлении Тогда
			ПредставлениеКодаМаркировки = СтрШаблон(НСтр("ru = '%1 (Остаток %2 %3)'"), ЭлементСписка.КодМаркировкиПредставление, ЭлементСписка.ОстатокПоКодуМаркировки,
											ПедставлениеЕдиницыИзмерения);
		Иначе
