...
Процедура КоэффициентВесОбъемУпаковок(ДанныеУпаковок, ВесОбъемУпаковкиВЕдиницеКонстантИС = Истина) Экспорт
	...
	ЕдиницаИзмеренияЛитр = Неопределено;
	ЕдиницаИзмеренияКилограмм = Неопределено;
	ЕдиницаИзмеренияЛитр(ЕдиницаИзмеренияЛитр);
	ЕдиницаИзмеренияКилограмм(ЕдиницаИзмеренияКилограмм);
	
	Для Каждого СтрокаДанныеУпаковок Из ДанныеУпаковок Цикл
		КоэффициентУпаковки(СтрокаДанныеУпаковок.Коэффициент, СтрокаДанныеУпаковок.Упаковка, СтрокаДанныеУпаковок.Номенклатура, Неопределено);
		ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДанныеУпаковок.Номенклатура, "ЕдиницаИзмерения");
		Если ЕдиницаИзмерения = ЕдиницаИзмеренияКилограмм Тогда
			СтрокаДанныеУпаковок.ТипИзмеряемойВеличиныВес = Истина;
			СтрокаДанныеУпаковок.Вес = СтрокаДанныеУпаковок.Коэффициент;
		ИначеЕсли ЕдиницаИзмерения = ЕдиницаИзмеренияЛитр Тогда
			СтрокаДанныеУпаковок.ТипИзмеряемойВеличиныОбъем = Истина;
			СтрокаДанныеУпаковок.Объем = СтрокаДанныеУпаковок.Коэффициент;
		КонецЕсли;
	КонецЦикла;
