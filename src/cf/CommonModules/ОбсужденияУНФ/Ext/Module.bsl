...
// Проверяет, заполнено ли имя у авторизованного текущего пользователя
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоСеансАвторизованногоПользователя() Экспорт
...
//
// Параметры:
//  ПользовательСВ - ИдентификаторПользователяСистемыВзаимодействия - Пользователь системы взаимодействия.
// 
// Возвращаемое значение:
...
Процедура ПриСозданииФайла(Файл) Экспорт
	...
	ДобавитьСистемноеСообщение(ТекстСообщения, ДанныеФайла.ВладелецФайла);
...
// Возвращает интеграции системы взаимодействия в привилегированном режиме
// 
// Возвращаемое значение:
//  ФиксированныйМассив из ИнтеграцияСистемыВзаимодействия
Функция ИнтеграцииСистемыВзаимодействия() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Возврат СистемаВзаимодействия.ПолучитьИнтеграции();
	
КонецФункции

#Область СобытияФорм

// Вызывается из соответствующего метода формы, после записи на сервере.
// 
// Параметры:
//  ТекущийОбъект - ДокументОбъект, СправочникОбъект - Текущий объект
Процедура ПослеЗаписиНаСервере(ТекущийОбъект) Экспорт
	...
	Если ТекущийОбъект.ДополнительныеСвойства.ДляОбсуждений.Свойство("АссистентЗапущен") Тогда
		Возврат;
	КонецЕсли;
...
Функция ПолучитьСсылкуИзНавигационной(НС) Экспорт
	...
	Исключение
		Возврат СтрШаблон("%1 %2", НСтр("ru = 'Ошибка преобразования:'"), НС);
	КонецПопытки; 
...
//   * ОтключитьУведомление    - Булево - Установить режим наблюдения при отправке сообщения в Ложь
// 
Функция НовыйДанныеСообщения() Экспорт
	...
	ДанныеСообщения.Вставить("ОтключитьУведомление", Ложь);
...
//  Пользователь - СправочникСсылка.Пользователи - пользователь, сообщения по которому необходимо отправить
Процедура ЗаписатьОтложенныеСообщенияВСистемуВзаимодействияСейчас(Предмет = Неопределено, Пользователь = Неопределено) Экспорт
	
	Если Не ЭтоСеансАвторизованногоПользователя() Тогда
		Возврат;
	КонецЕсли;
	
	ЗаписатьОтложенныеСообщенияВСистемуВзаимодействия(Предмет, Пользователь);
...
Процедура ЗаписатьОтложенныеСообщенияВСистемуВзаимодействия(ОбсуждениеКонтекст = Неопределено, Автор = Неопределено) Экспорт
	...
	// Даем возможность отключить отправку, если что-то пойдет не так
	КонстантаЗаписьВОбсуждения = Константы.ЗаписьИзмененийОбъектовВОбсуждения.Получить();
	Если КонстантаЗаписьВОбсуждения = Перечисления.ЗаписьИзмененийОбъектовВОбсуждения.НеЗаписывать Тогда  
		ЗаписьЖурналаРегистрации("ЗаписьНеотправленныхСообщенийУНФ",УровеньЖурналаРегистрации.Информация,,, "Отключена константа ЗаписьИзмененийОбъектовВОбсуждения, отправка сообщений не будет выполнена");	
		Возврат;
	КонецЕсли;		
	
	Запрос = Новый Запрос;
	ТекстЗапроса =   
	// Ограничиваем выборку, для Ассистента - последние 5 сообщений,
	// для регламентного - перые 100, с ограничением даты в неделю, а так же ограничиваем количество попыток отправки 
	// до 4 с одной и той же ошибкой
	"ВЫБРАТЬ ПЕРВЫЕ 100
	...
	|	ИЛИ (&НеВыполнятьОтборПоАвтору
	|				ИЛИ НеотправленныеСообщенияСистемыВзаимодействия.Автор = &Автор))
	|	И НеотправленныеСообщенияСистемыВзаимодействия.Дата >= &ГраницаДатыСообщения
	|	И НеотправленныеСообщенияСистемыВзаимодействия.КоличествоПопыток < &КоличествоПопыток
	|	И НеотправленныеСообщенияСистемыВзаимодействия.ПометкаУдаления = ЛОЖЬ
	|
	|УПОРЯДОЧИТЬ ПО
	|	НеотправленныеСообщенияСистемыВзаимодействия.Дата ВОЗР";
	...
	Запрос.УстановитьПараметр("НеВыполнятьОтборПоАвтору", Автор = Неопределено);
	Запрос.УстановитьПараметр("Автор", Автор);	
	Запрос.УстановитьПараметр("КоличествоПопыток", ГраницаНеудачныхПопытокОтправки());
	
	ГраницаДатыСообщения = ТекущаяДатаСеанса() - ПериодНеотправленныхСообщений();
	Запрос.УстановитьПараметр("ГраницаДатыСообщения", ГраницаДатыСообщения);
		
	Если ЗначениеЗаполнено(Автор) ИЛИ ЗначениеЗаполнено(ОбсуждениеКонтекст) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВОЗР", "УБЫВ");	
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "100", "20");	
	КонецЕсли;   
	...
	Пока НеотправленныеСообщения.Следующий() Цикл
		...
		Попытка
			...
			Если НужноПопытатьсяОтправитьСообщение Тогда
				...
				ПричинаНеотправки = "НеотслеживаемаяОшибка";	
				СообщениеОтправлено = Ложь;			
				СтруктураОтправки = СоздатьИОтправитьСообщениеСистемыВзаимодействия(ДанныеСообщения, Ложь);  
				Если ТипЗнч(СтруктураОтправки) = Тип("Структура") Тогда		
					Если СтруктураОтправки.Свойство("СообщениеОтправлено") Тогда
						СообщениеОтправлено = СтруктураОтправки.СообщениеОтправлено; 
					Иначе
						СообщениеОтправлено = Ложь;
					КонецЕсли;      				
					Если СообщениеОтправлено <> Истина Тогда
						Если СтруктураОтправки.Свойство("ТекстОшибки") Тогда
							ПричинаНеотправки = СтруктураОтправки.ТекстОшибки; 
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Иначе
				ХешТекстОшибки = ОбщегоНазначения.КонтрольнаяСуммаСтрокой(ПричинаНеотправки); 
				Если ХешТекстОшибки <> НеотправленноеСообщение.ХешТекстОшибки Тогда
					НеотправленноеСообщение.ХешТекстОшибки = ХешТекстОшибки;
					НеотправленноеСообщение.КоличествоПопыток = 1;         
					НеотправленноеСообщение.ТекстОшибки = ПричинаНеотправки;			
				Иначе				
					НеотправленноеСообщение.КоличествоПопыток = НеотправленноеСообщение.КоличествоПопыток + 1;
				КонецЕсли;            
...
// 
// Параметры:
//  ПользовательДляОтправкиПриема - СправочникСсылка.Пользователи, Неопределено - Пользователь для отправки приема
Процедура ЗаписьОтложенныхСообщенийВСистемуВзаимодействия(ПользовательДляОтправкиПриема = Неопределено) Экспорт
	...
	Если Не ПроверитьРегистрациюБазыВСистемеВзаимодействия() Или Не ЕстьСообщенияДляОтправки() Тогда
		ИспользованиеРеглЗаданияЗаписьСообщенийВСистемуВзаимодействия(Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ПопытатьсяСоздатьСообщениеСистемыВзаимодействия(ДанныеСообщения)
	...
	СтруктураСообщения = СоздатьИОтправитьСообщениеСистемыВзаимодействия(ДанныеСообщения); 
	Если ТипЗнч(СтруктураСообщения) = Тип("Структура") И СтруктураСообщения.Свойство("СообщениеОтправлено") Тогда
		СообщениеОтправлено = СтруктураСообщения.СообщениеОтправлено;
	Иначе     
		СообщениеОтправлено = Ложь;		
	КонецЕсли;		
...
Процедура ДобавитьСообщениеВОчередьОтправки(ДанныеСообщения)
	...
	НеотправленноеСообщение.ОтключитьУведомление = ДанныеСообщения.ОтключитьУведомление;
...
Функция ДанныеНеотправленногоСообщения(НеотправленноеСообщениеОбъект)
	...
	ДанныеСообщения.ОтключитьУведомление = НеотправленноеСообщениеОбъект.ОтключитьУведомление;
...
// 
// Возвращаемое значение:
 //  Структура - структура, содержащая в себе признак отправленности и текст ошибки.
//
Функция СоздатьИОтправитьСообщениеСистемыВзаимодействия(ДанныеСообщения, ВызыватьИсключение = Истина)
	...
	СтруктураВозврата.Вставить("СообщениеОтправлено", Ложь);
	...
	Если Не ЗначениеЗаполнено(СообщениеСистемыВзаимодействия.Автор) Тогда
		...
		СтруктураВозврата.СообщениеОтправлено = Неопределено;
		СтруктураВозврата.ТекстОшибки = ТекстСообщения;	
		Возврат СтруктураВозврата; 
	КонецЕсли;
	
	Попытка
		Если ОбсуждениеСистемыВзаимодействия.КонтекстОбсуждения <> Неопределено
			Или ДанныеСообщения.ОтключитьУведомление Тогда
		...
		Если ОбсуждениеСистемыВзаимодействия.КонтекстОбсуждения <> Неопределено  Или ДанныеСообщения.ОтключитьУведомление Тогда
	...
	Если СообщениеОтправлено = Истина Тогда
		СтруктураВозврата.СообщениеОтправлено = Истина;			
	КонецЕсли;        
...
Функция ПользовательПодходитДляОтправкиПриемаСообщения(ПользовательСправочника) Экспорт
	...
	ДанныеАвтора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПользовательСправочника,
		"Недействителен, ПометкаУдаления, ИдентификаторПользователяИБ");
	
	Результат = Не ДанныеАвтора.Недействителен И Не ДанныеАвтора.ПометкаУдаления
		И ЗначениеЗаполнено(ДанныеАвтора.ИдентификаторПользователяИБ);
...
// Максимальное количество неудачных попыток отправки. Используется в качестве критерия выбора сообщений
// для отправки.
// 
// Возвращаемое значение:
//  Число - Граница неудачных попыток отправки
Функция ГраницаНеудачныхПопытокОтправки()

	Возврат 4;
		
КонецФункции

// Период анализа неотправленных сообщений. Используется в качестве критерия выбора сообщений
// для отправки.
// 
// Возвращаемое значение:
//  Число - Количество секунд
Функция ПериодНеотправленныхСообщений()
	...
	КоличествоСекунд =  ЧасовВСутках * СекундВЧасе * КоличествоДней;
...
Функция ЕстьСообщенияДляОтправки()
	...
	|	Справочник.НеотправленныеСообщенияСистемыВзаимодействия КАК НеотправленныеСообщенияСистемыВзаимодействия
	|ГДЕ
	|	НеотправленныеСообщенияСистемыВзаимодействия.Дата >= &ГраницаДатыСообщения
	|	И НеотправленныеСообщенияСистемыВзаимодействия.КоличествоПопыток < &КоличествоПопыток
	|	И НЕ НеотправленныеСообщенияСистемыВзаимодействия.ПометкаУдаления";
