...
//  	куда будет помещены временные таблицы, содержит вр. таблицу ВТРасширенныеСведенияОДоходах с полями
//			ФизическоеЛицо
//			Организация
//			Период - дата, начало месяца получения дохода
...
//
//		ОписаниеПараметровСреза - Структура - структура полученная функцией ПараметрыСрезаПоследних
...
//
//			
//		Отбор - Массив из Структура - коллекция условий отбора, соединяемых по "И" и применяемых к регистру сведений
...
// Создает временную таблицу, содержащую коды видов тарифов страховых взносов с годами их применения
// 
// Параметры:
...
//  МенеджерВременныхТаблиц 	- МенеджерВременныхТаблиц - сюда будут помещены сформированные временные таблицы,
...
//	ЕстьОтборПоФизическимЛицам 	- Булево
//	ИсключатьДоходыРегистратора - Булево
//	Регистратор					- Неопределено - доходы регистратора не исключаются
//								- ДокументСсылка.НачислениеЗарплатыУНФ - начисление зарплаты, которое нужно исключить 
...
Процедура СформироватьВТРасширенныхСведенийОДоходах(НачалоПериода, ОкончаниеПериода, Организация, МенеджерВременныхТаблиц, ЕстьОтборПоФизическимЛицам = Ложь, ИсключатьДоходыРегистратора = Ложь, Регистратор = Неопределено) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	ОсновнойТариф = РегламентированнаяОтчетностьПереопределяемый.СсылкаНаОбъектПоИдентификатору(
		"Справочник.ВидыТарифовСтраховыхВзносовУНФ", "ОбщийНалоговыйРежим");
	ОтраслиПострадавшихОтКоронавируснойИнфекции
		= РегламентированнаяОтчетностьПереопределяемый.СсылкаНаОбъектПоИдентификатору(
		"Справочник.ВидыТарифовСтраховыхВзносовУНФ", "ДляОтраслейПострадавшихОтКоронавируснойИнфекции");
	Патент = РегламентированнаяОтчетностьПереопределяемый.СсылкаНаОбъектПоИдентификатору(
		"Справочник.ВидыТарифовСтраховыхВзносовУНФ", "ИндивидуальныйПредпринимательПрименяющийПатент");
	...
	Запрос.УстановитьПараметр("ДляОтраслейПострадавшихОтКоронавируснойИнфекции",
		ОтраслиПострадавшихОтКоронавируснойИнфекции); 
	...
	|	ТарифыСтраховыхВзносов.СтавкаСПревышенияПредела КАК СтавкаСПревышенияПредела,
	|	ВЫБОР
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблица.РасчетныйПериод КАК РасчетныйПериод,
	...
	|	ВременнаяТаблица.НачислятьНакопительнуюЧасть КАК НачислятьНакопительнуюЧасть,
	|	ВременнаяТаблица.ФизическоеЛицоНаименование КАК ФизическоеЛицоНаименование,
	|	ВременнаяТаблица.ВидТарифаСтраховыхВзносов КАК ВидТарифаСтраховыхВзносов,
	|	ВременнаяТаблица.ВидДохода КАК ВидДохода,
	|	ВременнаяТаблица.ВходитВБазуПФР КАК ВходитВБазуПФР,
	|	ВременнаяТаблица.ВходитВБазуФСС КАК ВходитВБазуФСС,
	|	ВременнаяТаблица.ВходитВБазуФОМС КАК ВходитВБазуФОМС,
	|	ВременнаяТаблица.ЭтоТрудовойДоговор КАК ЭтоТрудовойДоговор,
	|	ВременнаяТаблица.ТипДоговора КАК ТипДоговора,
	...
	|ИЗ
	|	ВТСведенияОДоходах КАК СведенияОДоходах
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыТарифовСтраховыхВзносовУНФ КАК ВидыТарифовСтраховыхВзносов
	|		ПО СведенияОДоходах.ВидТарифаСтраховыхВзносов = ВидыТарифовСтраховыхВзносов.Ссылка
	|ГДЕ
	|	ВидыТарифовСтраховыхВзносов.ЯвляетсяТарифомДляМСП <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|	И СведенияОДоходах.Период >= ВидыТарифовСтраховыхВзносов.ЯвляетсяТарифомДляМСП";
	Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
		...
		ТаблицаПовышающихКоэффициентов = Новый ТаблицаЗначений;	
		ТаблицаПовышающихКоэффициентов.Колонки.Добавить("РасчетныйПериод",
			Новый ОписаниеТипов("Дата", , , Новый КвалификаторыДаты()));	
		ТаблицаПовышающихКоэффициентов.Колонки.Добавить("ПовышающийКоэффициент",
		...
		НоваяСтрокаПовышающегоКоэффициента(ТаблицаПовышающихКоэффициентов, НачалоГода);
		...
		Запрос.УстановитьПараметр("ТаблицаПовышающихКоэффициентов", ТаблицаПовышающихКоэффициентов);
		
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаПовышающихКоэффициентов.РасчетныйПериод КАК РасчетныйПериод,
		|	ТаблицаПовышающихКоэффициентов.ПовышающийКоэффициент КАК ПовышающийКоэффициент
		|ПОМЕСТИТЬ ВТТаблицаПовышающихКоэффициентов
		|ИЗ
		|	&ТаблицаПовышающихКоэффициентов КАК ТаблицаПовышающихКоэффициентов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МинимальнаяОплатаТрудаРФСрезПоследних.Период КАК Период,
		|	МинимальнаяОплатаТрудаРФСрезПоследних.Значение * ЕСТЬNULL(ТаблицаПовышающихКоэффициентов.ПовышающийКоэффициент, 1) КАК Значение
		|ПОМЕСТИТЬ ВТМинимальнаяОплатаТрудаРФСКоэффициентом
		|ИЗ
		|	ВТМРОТСрезПоследних КАК МинимальнаяОплатаТрудаРФСрезПоследних
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТТаблицаПовышающихКоэффициентов КАК ТаблицаПовышающихКоэффициентов
		|		ПО МинимальнаяОплатаТрудаРФСрезПоследних.Период = ТаблицаПовышающихКоэффициентов.РасчетныйПериод
		|;
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СведенияОДоходах.РасчетныйПериод КАК РасчетныйПериод,
		|	СведенияОДоходах.Период КАК Период,
		...
		|	СведенияОДоходах.Организация КАК Организация,
		|	СведенияОДоходах.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ЕСТЬNULL(МинимальнаяОплатаТрудаРФ.Значение, 0) КАК РазмерМРОТ,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТМинимальнаяОплатаТрудаРФСКоэффициентом КАК МинимальнаяОплатаТрудаРФ
		...
		|	СведенияОДоходах.БазаФСС_НС КАК БазаФСС_НС,
		|	СведенияОДоходах.ЭтоТрудовойДоговор КАК ЭтоТрудовойДоговор,
		|	ЕСТЬNULL(ДанныеДляВыделенияМРОТ.РазмерМРОТ, 0) КАК РазмерМРОТ,
		...
		ТаблицаРаспределение = Результат.СкопироватьКолонки();
		Для Каждого СтрокаТаблицы Из Результат Цикл
			ВыделитьБазуМРОТНаОсновнойТариф(СтрокаТаблицы, ТаблицаРаспределение, ОсновнойТариф);			
		КонецЦикла;
		Результат = Неопределено;
		
		Запрос.УстановитьПараметр("ДанныеСВыделеннымМРОТ", ТаблицаРаспределение);
		Запрос.Текст =
		"ВЫБРАТЬ
		|	СведенияОДоходах.Период КАК Период,
		|	СведенияОДоходах.РасчетныйПериод КАК РасчетныйПериод,
...
//
// Возвращаемое значение:
//	- Массив из СправочникСсылка.ВидыТарифовСтраховыхВзносовУНФ
//
...
//
// Параметры:
//		ДоступныеТарифы - Массив из СправочникСсылка.ВидыТарифовСтраховыхВзносовУНФ
//
...
Процедура НоваяСтрокаПовышающегоКоэффициента(ТаблицаЗначений, РасчетныйПериод)
	
	НоваяСтрока = ТаблицаЗначений.Добавить();
	НоваяСтрока.РасчетныйПериод = РасчетныйПериод;
	НоваяСтрока.ПовышающийКоэффициент = ?(РасчетныйПериод < ДатаПовышенияПределаДляМСП(), 1, 1.5);
	
КонецПроцедуры

Процедура ВыделитьМРОТ(СтрокаМесяца, Показатель, СтрокиДеления, ПоказательПредел = "")
	
	ЗначениеПоказателя = ?(СтрокаМесяца[Показатель] = Неопределено, 0, СтрокаМесяца[Показатель]);
	
	СтрокиДеления.ОсновнойТариф[Показатель] = Мин(СтрокаМесяца.РазмерМРОТ, ЗначениеПоказателя);
	СтрокиДеления.ИсходныйТариф[Показатель] = ЗначениеПоказателя - СтрокиДеления.ОсновнойТариф[Показатель];
	
	Если НЕ ПустаяСтрока(ПоказательПредел) Тогда
		СтрокиДеления.ИсходныйТариф[ПоказательПредел]
			= Мин(СтрокиДеления.ИсходныйТариф[Показатель], СтрокаМесяца[ПоказательПредел]);
		СтрокиДеления.ОсновнойТариф[ПоказательПредел]
			= СтрокаМесяца[ПоказательПредел] - СтрокиДеления.ИсходныйТариф[ПоказательПредел];
	КонецЕсли;	
КонецПроцедуры

Функция ДатаПовышенияПределаДляМСП()
	
	Возврат '20250101';
	
КонецФункции

Процедура ВыделитьБазуМРОТНаОсновнойТариф(СтрокаМесяца, Результат, ОсновнойТариф)
	
	Если СтрокаМесяца.БазаПФР = 0 И СтрокаМесяца.БазаФОМС = 0 И СтрокаМесяца.БазаФСС = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Показатели = "Сумма, Скидка, БазаПФР, БазаФОМС, БазаФСС, СуммаПревысившаяПределПФР, СуммаПревысившаяПределФСС, СуммаПревысившаяПределФОМС";
	
	ТребуетсяВыделятьМРОТ_ПФР = СтрокаМесяца.ТребуетсяВыделятьМРОТ_ПФР;
	ТребуетсяВыделятьМРОТ_ФОМС = СтрокаМесяца.ТребуетсяВыделятьМРОТ_ФОМС;
	ТребуетсяВыделятьМРОТ_ФСС = СтрокаМесяца.ТребуетсяВыделятьМРОТ_ФСС;

	СтрокаОсновной = Результат.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаОсновной, СтрокаМесяца, , Показатели);
	СтрокаОсновной.ВидТарифаСтраховыхВзносов = ОсновнойТариф;
	СтрокаОсновной.БазаФСС_НС = 0;
	
	СтрокаИсходный = Результат.Добавить();	
	ЗаполнитьЗначенияСвойств(СтрокаИсходный, СтрокаМесяца, , Показатели);
	СтрокаИсходный.БазаФСС_НС = СтрокаМесяца.БазаФСС_НС;
	
	СтрокиДеления = Новый Структура;
	СтрокиДеления.Вставить("ОсновнойТариф", СтрокаОсновной);
	СтрокиДеления.Вставить("ИсходныйТариф", СтрокаИсходный);

	ВыделитьМРОТ(СтрокаМесяца, "Сумма", СтрокиДеления);
	Если СтрокаМесяца.Скидка > 0 И ЗначениеЗаполнено(СтрокаМесяца.Сумма) Тогда
		ПроцентСкидки = СтрокаМесяца.Скидка / СтрокаМесяца.Сумма;
		СтрокаОсновной.Скидка = ПроцентСкидки * СтрокаОсновной.Сумма;
		СтрокаИсходный.Скидка = ПроцентСкидки * СтрокаИсходный.Сумма;
	Иначе
		СтрокаОсновной.Скидка = 0;
		СтрокаИсходный.Скидка = 0;
	КонецЕсли;
	Если ТребуетсяВыделятьМРОТ_ПФР Тогда
		ВыделитьМРОТ(СтрокаМесяца, "БазаПФР", СтрокиДеления, "СуммаПревысившаяПределПФР");
	Иначе
		СтрокаОсновной.БазаПФР = СтрокаМесяца.БазаПФР;
		СтрокаОсновной.СуммаПревысившаяПределПФР = СтрокаМесяца.СуммаПревысившаяПределПФР;
		СтрокаИсходный.БазаПФР = 0;
		СтрокаИсходный.СуммаПревысившаяПределПФР = 0;
	КонецЕсли;
	
	Если ТребуетсяВыделятьМРОТ_ФСС Тогда
		ВыделитьМРОТ(СтрокаМесяца, "БазаФСС", СтрокиДеления, "СуммаПревысившаяПределФСС");
	Иначе
		СтрокаОсновной.БазаФСС = СтрокаМесяца.БазаФСС;
		СтрокаОсновной.СуммаПревысившаяПределФСС = СтрокаМесяца.СуммаПревысившаяПределФСС;
		СтрокаИсходный.БазаФСС = 0;
		СтрокаИсходный.СуммаПревысившаяПределФСС = 0;
	КонецЕсли;
	
	Если ТребуетсяВыделятьМРОТ_ФОМС Тогда
		ВыделитьМРОТ(СтрокаМесяца, "БазаФОМС", СтрокиДеления, "СуммаПревысившаяПределФОМС");
	Иначе
		СтрокаОсновной.БазаФОМС = СтрокаМесяца.БазаФОМС;
		СтрокаОсновной.СуммаПревысившаяПределФОМС = СтрокаМесяца.СуммаПревысившаяПределФОМС;
		СтрокаИсходный.БазаФОМС = 0;
		СтрокаИсходный.СуммаПревысившаяПределФОМС = 0;
	КонецЕсли;
