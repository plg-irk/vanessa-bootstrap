...
//
// Параметры:
//	НастройкаИнтеграции - СправочникСсылка.НастройкиИнтеграцииСИнтернетМагазином 
//
// Возвращаемое значение:
//	СправочникСсылка.КлючиИнтеграцийСИнтернетМагазином - найденная валюта.
//
...
// 
// Параметры:
//  ТипСсылкиНаДокументСтрокой - Произвольный - Тип ссылки на документ строкой
// 
// Возвращаемое значение:
//  Булево - Это документ
...
// Получить узел обмена по настройке.
// 
// Параметры:
//  НастройкаИнтеграции - СправочникСсылка.НастройкиИнтеграцииСИнтернетМагазином
// 
// Возвращаемое значение:
//  Произвольный - Получить узел обмена по настройке
...
//
Функция ПолучитьПоСтавкеНДСЗначениеДляВыгрузки(СтавкаНДС, Знач Период = Неопределено) Экспорт
	
	СтавкаНДСЗначения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта( СтавкаНДС, "НеОблагается, Ставка, Расчетная" );
	
	Ставка = СтавкаНДСЗначения.Ставка;
	Если СтавкаНДС.ВидСтавкиНДС = Перечисления.ВидыСтавокНДС.Общая Тогда
		Если Период = Неопределено Тогда
			Период = ТекущаяДатаСеанса();
		КонецЕсли;
		ОбщаяСтавкаНДС = УчетНДСКлиентСервер.ОбщаяСтавкаНДС( Период );
		Ставка = УчетНДСПереопределяемый.ПолучитьСтавкуНДС( ОбщаяСтавкаНДС );
	КонецЕсли;
	
	Если СтавкаНДСЗначения.НеОблагается Тогда 
		ЗначениеНалога = НСтр("ru = 'Без НДС'");
	ИначеЕсли Ставка = 0 Тогда 
		ЗначениеНалога = "0";
	ИначеЕсли Ставка = 5 Тогда 
		ЗначениеНалога = "5" + ?(СтавкаНДСЗначения.Расчетная, "/105", "");
	ИначеЕсли Ставка = 7 Тогда 
		ЗначениеНалога = "7" + ?(СтавкаНДСЗначения.Расчетная, "/107", "");
	ИначеЕсли Ставка = 10 Тогда 
		ЗначениеНалога = "10" + ?(СтавкаНДСЗначения.Расчетная, "/110", "");
	ИначеЕсли Ставка = 18 Тогда 
		ЗначениеНалога = "18" + ?(СтавкаНДСЗначения.Расчетная, "/118", "");
	ИначеЕсли Ставка = 20 Тогда 
		ЗначениеНалога = "20" + ?(СтавкаНДСЗначения.Расчетная, "/120", "");
	ИначеЕсли Ставка = 22 Тогда 
		ЗначениеНалога = "22" + ?(СтавкаНДСЗначения.Расчетная, "/122", "");
	Иначе
		ЗначениеНалога = Строка(Ставка);
	КонецЕсли;
...
// Получить по ставке НДСЗначение для выгрузки число.
// 
// Параметры:
...
// 
// Параметры:
//  НаименованиеСкидки - Строка - Наименование скидки
//  Процент - Число - Процент
// 
// Возвращаемое значение:
//  СправочникСсылка.ВидыСкидокНаценок - Вид скидки
...
// 
// Возвращаемое значение:
// 	Булево - Истина - реквизит имеет тип Строка неограниченной длины, иначе Ложь
//
...
// 
// Возвращаемое значение:
//  Произвольный, Булево - Использовать интеграцию с интернет магазином
