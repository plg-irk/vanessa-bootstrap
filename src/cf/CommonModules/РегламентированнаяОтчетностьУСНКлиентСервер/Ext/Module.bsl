...
// Задача календаря использует записи КУДиР.
// 
// Параметры:
//  ЗадачаКалендаря - СправочникСсылка.ЗадачиКалендаряПодготовкиОтчетности
// 
// Возвращаемое значение:
//  -Булево - Для выполнения задачи требуются актуальные записи КУДиР
//
Функция ЗадачаКалендаряИспользуетЗаписиКУДиР(ЗадачаКалендаря) Экспорт
	
	Если ЗадачаКалендаря = ПредопределенноеЗначение("Справочник.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоУСН")
	Тогда
		Возврат Истина;
	КонецЕсли;
...
// 
// Возвращаемое значение:
//   - Массив из ПеречислениеСсылка - виды операций денежных документов
//
...
// Возвращает массив операций денежных документов, которые отражаются в доходах по УСН
// 
// Возвращаемое значение:
//   - Массив из ПеречислениеСсылка - виды операций денежных документов
//
Функция ВидыОперацийДоходыУСН() Экспорт
	
	Результат = Новый Массив;
	
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ОтПокупателя"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.Прочее"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ПрочиеРасчеты"));
	
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтПокупателя"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.Прочее"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПрочиеРасчеты"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКредитам"));
	
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходИзКассы.Покупателю"));
	
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Покупателю"));
	Результат.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты"));
...
// Возвращает дату, с которой отменен ЕНВД
//
// Возвращаемое значение:
//  Дата
//
