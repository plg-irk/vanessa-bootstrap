...
Процедура ПроверитьВозможностьУстановкиПризнакаНедействителен(Ссылка) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	МассивИмен = Новый Массив;
	
	ПерсональныеНастройкиВыборка = ПерсональныеНастройкиРУНФ.ВсеНастройки();
	Пока ПерсональныеНастройкиВыборка.Следующий() Цикл
		Если ПерсональныеНастройкиВыборка.Настройки = Ссылка Тогда
			МассивИмен.Добавить(ПерсональныеНастройкиВыборка.Пользователь);
		КонецЕсли;
	КонецЦикла;
	...
	"ru = 'Текущий элемент нельзя вывести из оборота (сделать недействительным).
	|Прайс-лист указан как основной в настройках пользователя:
	|
	|%1'");
	...
	ВызватьИсключение СтрШаблон(ШаблонСообщения, ИменаПользователей);
	
КонецПроцедуры

Процедура НастройкиПрайсЛистовПриСозданииНовогоПользователя(Пользователь, ОбновитьЗначениеНастройки = Ложь) Экспорт
	
	ВыводитьВопросПоОрганизации = ПерсональныеНастройкиРУНФ.Прочитать(
		"ВыводитьВопросСозданииПервогоПрайсЛистаОрганизации", , Пользователь);
	Если (ЗначениеЗаполнено(ВыводитьВопросПоОрганизации)
		...
		ПерсональныеНастройкиРУНФ.Установить(Истина, "ВыводитьВопросСозданииПервогоПрайсЛистаОрганизации", Пользователь);
		
	КонецЕсли;
	
	ВыводитьВопросПоКонтрагентам = ПерсональныеНастройкиРУНФ.Прочитать(
		"ВыводитьВопросСозданииПервогоПрайсЛистаКонтрагента", , Пользователь);
	Если (ЗначениеЗаполнено(ВыводитьВопросПоКонтрагентам)
			И ОбновитьЗначениеНастройки)
		ИЛИ НЕ ЗначениеЗаполнено(ВыводитьВопросПоКонтрагентам) Тогда
		
		ПерсональныеНастройкиРУНФ.Установить(Истина, "ВыводитьВопросСозданииПервогоПрайсЛистаКонтрагента", Пользователь);
