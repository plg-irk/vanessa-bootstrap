...
Процедура ПриЗаписиПользователя(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.ЭтоНовыйОбъект Тогда
		УстановитьНастройкиНовогоПользователя(Пользователь);
	КонецЕсли;
		...
		Исключение
			...
			СообщениеОбОшибке = СтрШаблон(
				НСтр("ru = 'Не удалось добавить пользователя ""%1"" в группу доступа ""%2"" по причине:
					 |%3'"), Пользователь, Выборка.Ссылка, ПодробноеПредставлениеОшибки);
...
Процедура ПредоставитьДоступПоУмолчаниюВНеполноправномРежиме(Пользователь)
	...
	СвойстваПользователя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Пользователь, "Недействителен, Служебный");
	
	Если СвойстваПользователя.Недействителен Или СвойстваПользователя.Служебный Тогда
		Возврат;
	КонецЕсли;
...
Процедура СвязатьПользователяССотрудником(Пользователь)
	...
	СвойстваПользователя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Пользователь,
		"Наименование, Недействителен, Служебный");
	...
	Если СвязанныеПользователи.Количество() = 0 Тогда
		...
		НайденныйПользователь = Справочники.Сотрудники.НайтиПоНаименованию(СвойстваПользователя.Наименование, Истина);
		...
		Иначе
			...
			ДанныеНовогоСотрудника.Вставить("Наименование", СвойстваПользователя.Наименование);
...
//
Процедура УстановитьНастройкиНовогоПользователя(Пользователь)
	
	Если Не ЗначениеЗаполнено(Пользователь) Тогда
		Возврат;
	КонецЕсли;
	
	ПодборНоменклатурыВДокументах.УстановитьСтандартныеНастройкиПодбора(Пользователь);
	ПерсональныеНастройкиРУНФ.Установить(Истина, "ПоказыватьКолонкуСодержаниеВАктеВыполненныхРабот",
		Пользователь);
	ПерсональныеНастройкиРУНФ.Установить(Истина, "ПоказыватьПредпросмотрЗаказНарядов",
		Пользователь);
	ПерсональныеНастройкиРУНФ.Установить(Истина, "ПоказыватьПредпросмотрЗаказовПокупателей",
		Пользователь);
		
	ПерсональныеНастройкиРУНФ.Установить(Истина, "ИспользоватьБыструюФормуКонтрагентовВДокументах",
		Пользователь);
	ПерсональныеНастройкиРУНФ.Установить(Истина, "ИспользоватьБыструюФормуНоменклатурыВДокументах",
		Пользователь);
	...
	Если ПользовательИБ <> Неопределено Тогда
		...
		ПечатьДокументовУНФ.ВыключитьИнформированиеНовоеФаксимиле(ИмяПользователя);
	КонецЕсли;
...
Функция КоличествоЗначенийНастройки(НазваниеНастройки)
	...
	Если ПолучитьФункциональнуюОпцию("МиграцияПерсональныхНастроекЗавершена") Тогда
		Результат = ПерсональныеНастройкиРУНФ.КоличествоЗначенийНастройки(НазваниеНастройки);
		Возврат Результат;
	КонецЕсли;
	
	Настройка = ПланыВидовХарактеристик.НастройкиПользователей[НазваниеНастройки];
...
Процедура ЗаписатьСтатистикуПоНастройке(НазваниеНастройки)
	...
	ШаблонКлюча = "СтатистикаУНФ.Настройки.Пользовательские.%1.%2";
	Для Каждого Элемент Из ЗначенияНастройки Цикл
		Ключ = Строка(Элемент.Ключ);
		Ключ = СтрЗаменить(Ключ, ".","");
		Имя = СтрШаблон(ШаблонКлюча, Строка(НазваниеНастройки), Ключ);
