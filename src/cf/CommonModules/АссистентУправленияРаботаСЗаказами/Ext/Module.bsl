...
Процедура ПроверитьИзменениеСостоянияЗаказаПокупателяПриЗаписи(Источник)
	...
	Если СостояниеЗаказаТекущее <> СостояниеЗаказаДо Тогда
		Если РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(Источник.ВидОперации) Тогда
...
Функция СоздатьEmail(ДокументОснование, УчетнаяЗапись) Экспорт
...
Процедура ЗаполнитьУчастниковСобытия(Событие, Получатель) Экспорт
...
Функция ЗаполненоКакСвязатьсяВСобытии(Событие) Экспорт
...
Процедура ОтправитьСобытие(Событие) Экспорт
