...
Процедура ПриОпределенииПараметровСозданияЗаказа(
	...
	ОнлайнЗаказыУНФ.ПриОпределенииПараметровСозданияЗаказа(ДокументЗаказа, Результат);
...
Процедура ПриОпределенииПечатныхФормЗаказа(
	...
	ОнлайнЗаказыУНФ.ПриОпределенииПечатныхФормЗаказа(ДокументЗаказа, ПечатныеФормы);
...
Процедура ПриНастройкеТаблицыТоваров(
	...
	ОнлайнЗаказыУНФ.ПриНастройкеТаблицыТоваров(ДокументЗаказа, ОписаниеПредставленияТоваров);
...
Процедура ПриФормированииЗаказаНаОплату(ДокументЗаказа, ЗаказНаОплату) Экспорт
	...
	ОнлайнЗаказыУНФ.ПриФормированииЗаказаНаОплату(ДокументЗаказа, ЗаказНаОплату);
...
Процедура ПриЗагрузкеСтатусаЗаказа(
	...
	ОбязательныеПараметры.Вставить("СБПc2b");
	ОбязательныеПараметры.Вставить("СБПb2b");
	ОбязательныеПараметры.Вставить("ИнтернетЭквайринг");
	
	ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(ОбязательныеПараметры, ПараметрыОплаты, Истина);
	
	СБП = Неопределено;
	
	Если ОбязательныеПараметры.Получить("СБПc2b") <> Неопределено Тогда
		СБП = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ОбязательныеПараметры.Получить("СБПc2b"));
	ИначеЕсли ОбязательныеПараметры.Получить("СБПb2b") <> Неопределено Тогда
		СБП = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ОбязательныеПараметры.Получить("СБПb2b"));
	ИначеЕсли ОбязательныеПараметры.Получить("ИнтернетЭквайринг") <> Неопределено Тогда
		СБП = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ОбязательныеПараметры.Получить("ИнтернетЭквайринг"));
	Иначе
		Возврат;
	КонецЕсли;
	
	РезультатОбработки = Новый Структура("ПараметрыОперации, СтатусОперации, СообщениеОбОшибке");
	
	ЗаполнитьЗначенияСвойств(РезультатОбработки, СБП.РезультатОбработки);

	Если ТипЗнч(РезультатОбработки.СтатусОперации) = Тип("Структура") Тогда
		ЗаполнитьЗначенияСвойств(РезультатОбработки, РезультатОбработки.СтатусОперации);
	КонецЕсли;
	
	ИнтеграцияСПлатежнымиСистемамиУНФ.ПриЗагрузкеСтатусаОперации(
		ДокументЗаказа,
		СБП.НастройкаПодключения,
		РезультатОбработки,
		Обработан,
		,
		ПараметрыФискализации);
...
Процедура ПриПроверкеИспользованияШаблоновСообщенийОнлайнЗаказов(Используется) Экспорт
	...
	ОнлайнЗаказыУНФ.ПриПроверкеИспользованияШаблоновСообщенийОнлайнЗаказов(Используется);
...
Процедура ПриОпределенииПредопределенныхШаблоновСообщенийОнлайнЗаказовПоТипам(Шаблоны) Экспорт
	...
	ОнлайнЗаказыУНФ.ПриОпределенииПредопределенныхШаблоновСообщенийОнлайнЗаказовПоТипам(Шаблоны);
...
Процедура ПриОпределенииПараметровОтправкиСообщений(
	...
	ОнлайнЗаказыУНФ.ПриОпределенииПараметровОтправкиСообщений(ПараметрыОтправкиСообщений);
