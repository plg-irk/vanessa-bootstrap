...
Функция ПолучитьТипыМетаданныхСодержащиеПартнерскиеEmail() Экспорт
	...
	СписокТиповМетаданныхСодержащиеEmail.Добавить(Новый ОписаниеТипов("СправочникСсылка.Лиды"));
...
Функция ПодготовитьЭлектронныеАдресаПолучателей(знач Получатели, ВключаяПодчиненные = Истина) Экспорт
	...
	Для каждого Получатель Из Получатели Цикл
		...
		Если ВключаяПодчиненные Тогда
			...
			Если ТипЗнч(ЗначениеЭлементаСпискаЗначений) = Тип("СправочникСсылка.Контрагенты") Тогда
				ПодготовитьЭлектронныеАдресаПолучателейКонтрагента(ЭлектронныеАдресаПолучателей,
					ЗначениеЭлементаСпискаЗначений);
			КонецЕсли;
			
			Если ТипЗнч(ЗначениеЭлементаСпискаЗначений) = Тип("СправочникСсылка.Лиды") Тогда
				ПодготовитьЭлектронныеАдресаПолучателейЛида(ЭлектронныеАдресаПолучателей,
					ЗначениеЭлементаСпискаЗначений);			
			КонецЕсли;
...
Процедура ПодготовитьЭлектронныеАдресаПолучателейКонтрагента(ЭлектронныеАдресаПолучателей,
	ЗначениеЭлементаСпискаЗначений)
	...
	ЭлектронныеАдресаКонтактныхЛиц = ПодготовитьЭлектронныеАдресаПолучателей(СписокКонтактов, Ложь);
...
Процедура ПодготовитьЭлектронныеАдресаПолучателейЛида(ЭлектронныеАдресаПолучателей, ЗначениеЭлементаСпискаЗначений)
				
	СписокКонтактов = Новый СписокЗначений;

	ЭтоПервичноеОбращениеПоЛиду =
		Справочники.Лиды.ЭтоПервичноеОбращениеПоЛиду(ЗначениеЭлементаСпискаЗначений);
	
	Если ЭтоПервичноеОбращениеПоЛиду Тогда
		
		МассивКонтактов = Справочники.КонтактыЛидов.КонтактыЛида(ЗначениеЭлементаСпискаЗначений);	
		Для Каждого Контакт Из МассивКонтактов Цикл
	...
	Иначе
		...
		Связанныйконтрагент = Справочники.Лиды.СвязанныйКонтрагент(ЗначениеЭлементаСпискаЗначений);
		СписокКонтактов.Добавить(Связанныйконтрагент);
		
	КонецЕсли;

	ДобавлятьКонтактныеЛица = ?(ЭтоПервичноеОбращениеПоЛиду, Ложь, Истина);
	
	ЭлектронныеАдресаКонтактныхЛиц = ПодготовитьЭлектронныеАдресаПолучателей(СписокКонтактов, ДобавлятьКонтактныеЛица);
