...
Процедура ПриОбработкеНавигационнойСсылкиДополнительнойИнформации(
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "УстановитьСрокДействияQRКода" Тогда
		
		Форма 					= ИнтеграцияСПлатежнымиСистемамиУНФКлиентСервер.ПолучитьФормуПоЭлементу(Элемент);
		ДополнительныеПараметры = Новый Структура();
		ДополнительныеПараметры.Вставить("Форма", 				Форма);
		ДополнительныеПараметры.Вставить("ДанныеФормы", 		ДанныеФормы);
		ДополнительныеПараметры.Вставить("ЭлементДекорации", 	Элемент);

		ПроверкаНаОбъектФормы	= Новый Структура("Объект, ВариантНастройки");
		ЗаполнитьЗначенияСвойств(ПроверкаНаОбъектФормы, Форма);
		
		ВариантНастройки		= ?(ПроверкаНаОбъектФормы.Объект = Неопределено,
			ПроверкаНаОбъектФормы.ВариантНастройки,
			ПроверкаНаОбъектФормы.Объект.ВариантНастройки);
		
		ПараметрыФормы			= Новый Структура();
		ПараметрыФормы.Вставить("СрокДействияQRКода", 			Форма.СрокДействияQRКода);
		ПараметрыФормы.Вставить("ВариантНастройки", 			ВариантНастройки);
		
		ПродолжениеНастройки    = Новый ОписаниеОповещения(
			"ОбработкаДополнительнойИнформацииЗавершение", 
			ЭтотОбъект, 
			ДополнительныеПараметры);

		ОткрытьФорму(
			"ОбщаяФорма.УстановкаСрокаДействияQRКода", 
			ПараметрыФормы, 
			, 
			, 
			, 
...
Процедура ОбработкаДополнительнойИнформацииЗавершение(СрокДействияQRКода, ДополнительныеПараметры) Экспорт
	
	Если СрокДействияQRКода <> Неопределено Тогда  
		
		Форма       					= ДополнительныеПараметры.Форма;
		ДанныеФормы 					= ДополнительныеПараметры.ДанныеФормы;
		Элемент							= ДополнительныеПараметры.ЭлементДекорации;
		
		Форма.СрокДействияQRКода		= СрокДействияQRКода;
		Форма.Модифицированность  		= Истина;
		
		ИнтеграцияСПлатежнымиСистемамиУНФКлиентСервер.НастроитьДополнительнуюИнформациюНастройкиПодключенияСБП(Элемент);
