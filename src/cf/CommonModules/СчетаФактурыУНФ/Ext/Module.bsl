...
//  Полученный - определяет вид документа (СчетФактура или СчетФактураПолученный)
//  Корректировочный - определяет вид операции счета-фаткуры
//  ДополнительныеПараметры - структура дополнительных параметров поиска счета-фактуры
//
// Возвращаемое значение:
//  Структура - структура с данными счет фактуры
Функция ПолучитьПодчиненныйСчетФактуру(ДокументОснование, Полученный = Ложь, Корректировочный = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ЭтоСчетФактураНаАвансКорректировкаПоступления = НЕ ДополнительныеПараметры = Неопределено
													И ДополнительныеПараметры.Свойство("ЭтоСчетФактураНаАвансКорректировкаПоступления");
	...
	Если Полученный И Не Корректировочный Тогда
		...
		ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления")
			И ЭтоСчетФактураНаАвансКорректировкаПоступления Тогда
			...
			|ГДЕ
			|	СчетФактураДокументыОснования.ДокументОснование = &ДокументОснование
			|	И СчетФактураДокументыОснования.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактураПолученный.Аванс)";			
...
//  СчетФактураДата - Дата - дата счет-фактуры
//  СчетФактураНомер - Строка - номер счет-фактуры
//  СчетФактураНомерИсправления - Число - номер исправления для полученных счетов-фактур
// Возвращаемое значение:
...
Функция СоздатьСчетФактуру(ДокументОснование, СчетФактураДата, СчетФактураНомер = Неопределено, СчетФактураНомерИсправления = Неопределено) Экспорт
	...
	Если Полученный И Не Корректировочный Тогда
		...
		Если СчетФактура.Исправление И ЗначениеЗаполнено(СчетФактураНомерИсправления) Тогда
			СчетФактура.НомерИсправления = СчетФактураНомерИсправления;
			Если НЕ ЗначениеЗаполнено(СчетФактура.ДатаВходящегоДокумента) Тогда
	...
	Иначе
		...
		ДатаНомер = ДатаНомерДляПечатиСчетаФактуры(СчетФактура.Ссылка);
		
		СтруктураВозврата.Вставить("Дата", ДатаНомер.Дата);
		СтруктураВозврата.Вставить("Номер", ДатаНомер.Номер);
