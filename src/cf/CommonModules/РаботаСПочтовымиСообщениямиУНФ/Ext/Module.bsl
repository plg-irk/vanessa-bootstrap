...
//  Структура - Синхронизировать содержимое почтовых ящиков:
// * ЗагруженныеСообщения - ТаблицаЗначений - :
// ** ИнтернетПочтовоеСообщение - ИнтернетПочтовоеСообщение - 
// ** ПочтовыйЯщик - Строка
// ** ФлагиСообщения - Соответствие из КлючИЗначение
// ** СодержаниеПолучено - Булево
// * ФлагиСообщений - Соответствие из КлючИЗначение - 
Функция СинхронизироватьСодержимоеПочтовыхЯщиков(УчетнаяЗапись, ДополнительныеПараметры = Неопределено) Экспорт
	
	ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог("Начало синхронизации почты", УчетнаяЗапись);
	...
	Попытка
		...
		ПредставлениеДанных = ОбменСВнешнимиСистемамиЛогирование.ОбъектПредставление(ПочтовыеЯщики);
		ТекстСообщения = СтрШаблон("Все почтовые ящики:
		...
		Для каждого ОписаниеПочтовогоЯщика Из ОписаниеПочтовыхЯщиков Цикл
			...
			ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог(
			СтрШаблон("Начало синхронизации почтового ящика ""%1""", ОписаниеПочтовогоЯщика.ПочтовыйЯщик), УчетнаяЗапись);
			...
			ТекстСообщения = СтрШаблон("Начало перемещения писем по почтовому ящику ""%1""",
			ОписаниеПочтовогоЯщика.ПочтовыйЯщик);
			ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог(ТекстСообщения, УчетнаяЗапись);
			...
			КоличествоПеремещенныхПисем = 0;
			КоличествоНеперемещенныхПисем = 0;
			Для каждого КлючЗначение Из МассивыИдентификаторов Цикл
				
				Если ТипЗнч(КлючЗначение.Значение) = Тип("Массив")
					И КлючЗначение.Значение.Количество() <> 0
					И ПочтовыеЯщики.Найти(КлючЗначение.Ключ) <> Неопределено Тогда
					
					Соединение.ПереместитьВПочтовыйЯщик(КлючЗначение.Значение, КлючЗначение.Ключ);
					КоличествоПеремещенныхПисем = КоличествоПеремещенныхПисем + 1;
					
				Иначе
					КоличествоНеперемещенныхПисем = КоличествоНеперемещенныхПисем + 1;
				КонецЕсли;
			...
			ТекстСообщения = СтрШаблон("Конец перемещения писем по почтовому ящику ""%1"".
			|Количество перемещенных писем - %2
			|Количество неперемещенных писем - %3",
			ОписаниеПочтовогоЯщика.ПочтовыйЯщик,
			КоличествоПеремещенныхПисем,
			КоличествоНеперемещенныхПисем);
			ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог(ТекстСообщения, УчетнаяЗапись);
			...
			ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог(
			СтрШаблон("Конец обработки флагов по почтовому ящику ""%1""", ОписаниеПочтовогоЯщика.ПочтовыйЯщик), УчетнаяЗапись);
			...
			КоличествоИдентификаторов = ИдентификаторыПисемДляЗагрузки.Количество();
			
			ТекстСообщения = СтрШаблон("Количество загруженных идентификаторов по почтовому ящику ""%1"" - %2",
			ОписаниеПочтовогоЯщика.ПочтовыйЯщик,
			КоличествоИдентификаторов);
			ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог(ТекстСообщения, УчетнаяЗапись);
			...
			ТекстСообщения = СтрШаблон("Количество загруженных писем по почтовому ящику ""%1"" - %2",
			ОписаниеПочтовогоЯщика.ПочтовыйЯщик,
			НовыеСообщения.Количество());
			ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог(ТекстСообщения, УчетнаяЗапись);
		...
		ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог(
		СтрШаблон("Конец синхронизации почтового ящика ""%1""", ОписаниеПочтовогоЯщика.ПочтовыйЯщик), УчетнаяЗапись);
	...
	ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог("Конец синхронизации почты", УчетнаяЗапись);
...
//  ДляПолучения - Булево - Для получения
// 
// Возвращаемое значение:
...
Процедура ЗагрузитьСодержаниеСообщенияИнтернетПочты(Идентификатор, ВходящееИсходящееСобытие, УчетнаяЗапись, АдресХранилища) Экспорт
	...
	Для Каждого ТекИнтернетПочтовоеСообщение Из ИнтернетПочтовыеСообщения Цикл
		Для Индекс = 0 По ТекИнтернетПочтовоеСообщение.Идентификатор.Количество() - 1 Цикл
...
//  Соединение - ИнтернетПочта - существующее соединение с почтовым сервером.
...
// Протокол интернет почты IMAP.
// 
// Параметры:
//  УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты - Учетная запись
// 
// Возвращаемое значение:
//  ПротоколИнтернетПочты - Протокол интернет почты IMAP
