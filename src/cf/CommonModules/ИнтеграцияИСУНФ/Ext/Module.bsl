...
Процедура ИспользованиеСерий(Использование, Номенклатура) Экспорт
	...
	Иначе
		ДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "ВидПродукцииИС, ПодконтрольнаяПродукцияВЕТИС, ИспользоватьПартии, ИспользоватьСерииНоменклатуры");
		Если ДанныеНоменклатуры.ПодконтрольнаяПродукцияВЕТИС
			И (Не ЗначениеЗаполнено(ДанныеНоменклатуры.ВидПродукцииИС) Или ДанныеНоменклатуры.ВидПродукцииИС = Перечисления.ВидыПродукцииИС.ПодконтрольнаяПродукцияВЕТИС) Тогда
			Использование = ДанныеНоменклатуры.ИспользоватьПартии;
		Иначе
			Использование = ДанныеНоменклатуры.ИспользоватьСерииНоменклатуры;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОпределенииПараметровИнтеграцииФормыПрикладногоОбъекта(Форма, МодулиИнтеграции) Экспорт
	...
	Если ПоддерживаетсяСканированиеМаркировки(ИмяФормы)
		Или ИмяФормы = "Документ.РасходнаяНакладная.Форма.ФормаДокумента"
		Или ИмяФормы = "Документ.ПриходнаяНакладная.Форма.ФормаДокумента"
		Или ИмяФормы = "Документ.ПередачаТоваровМеждуОрганизациями.Форма.ФормаДокумента"
		Или ИмяФормы = "Документ.ОтчетПереработчика.Форма.ФормаДокумента"
		Или ИмяФормы = "Документ.ОтчетОПереработке.Форма.ФормаДокумента"
...
Процедура УстановитьСвязиПараметровВыбораСНоменклатурой(Форма, ИмяПоляВвода, ПутьКДаннымНоменклатуры) Экспорт
	...
	Если ИмяПоляВвода = "ТоварыХарактеристика"
		Или ИмяПоляВвода = "Характеристика"
		Или ИмяПоляВвода = "ОписаниеGTINХарактеристика" Тогда
...
Процедура ПриСозданииЧтенииНаСервере(Форма) Экспорт
	...
	ИначеЕсли ПоддерживаетсяСканированиеМаркировки(Форма.ИмяФормы) Тогда
		
		ИнициализироватьФормуДляПоддержкиСканированияМаркировки(Форма);
...
Процедура ПроверитьЗаполнениеАкцизныхМарок(Объект, Отказ, НаДату = Неопределено) Экспорт
	...
	Если Не Отказ И ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РасходнаяНакладная")
		И УправлениеНебольшойФирмойЭлектронныеДокументыСервер.ЕстьТоварыОСУ(Объект.Ссылка) Тогда
		
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Пробитие чека с кодами ОСУ не поддерживается. Введите коды маркировки'")
			,,,, Отказ);
		Возврат;
...
Процедура МодифицироватьИнициализироватьФормуОптовогоДокумента(Форма) Экспорт
	...
	Если ИмяФормы = "Документ.РасходнаяНакладная.Форма.ФормаДокумента"
		...
		ИЛИ ИмяФормы = "Документ.ОтчетОПереработке.Форма.ФормаДокумента"
		ИЛИ ИмяФормы = "Документ.ОтчетПереработчика.Форма.ФормаДокумента"
		ИЛИ ПоддерживаетсяСканированиеМаркировки(Форма.ИмяФормы) Тогда
...
// Возвращает структуру данных по данным штрихкода.
//
// Параметры:
...
//   * Серия - СправочникСсылка.СерииНоменклатуры,СправочникСсылка.ПартииНоменклатуры
//  ДополнитьДаннымиНоменклатуры - Булево - Признак добавления данных по номенклатуре.
// 
// Возвращаемое значение:
//  Структура - Описание:
//   * Номенклатура - СправочникСсылка.Номенклатура
...
//   * ЕдиницаИзмеренияКоэффициент - Число
...
//
Функция ДанныеПоШтрихкодуГосИС(ДанныеШтрихкода, ДополнитьДаннымиНоменклатуры = Истина) Экспорт
	
	ДанныеПоШтрихкоду = Новый Структура;
	ДанныеПоШтрихкоду.Вставить("Номенклатура", ДанныеШтрихкода.Номенклатура);
	ДанныеПоШтрихкоду.Вставить("Характеристика", ДанныеШтрихкода.Характеристика);
	Если ЗначениеЗаполнено(ДанныеШтрихкода.Упаковка)
		И ТипЗнч(ДанныеШтрихкода.Упаковка) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
		ДанныеПоШтрихкоду.Вставить("ЕдиницаИзмерения", ДанныеШтрихкода.Упаковка);
		ЕдиницаИзмеренияКоэффициент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеШтрихкода.Упаковка,
			"Коэффициент");
	Иначе
		ДанныеПоШтрихкоду.Вставить("ЕдиницаИзмерения", Справочники.ЕдиницыИзмерения.ПустаяСсылка());
		ЕдиницаИзмеренияКоэффициент = 1;
	КонецЕсли;
	ДанныеПоШтрихкоду.Вставить("ЕдиницаИзмеренияКоэффициент", ЕдиницаИзмеренияКоэффициент);
	Если ЗначениеЗаполнено(ДанныеШтрихкода.Серия)
		И ТипЗнч(ДанныеШтрихкода.Серия) = Тип("СправочникСсылка.ПартииНоменклатуры") Тогда
		ДанныеПоШтрихкоду.Вставить("Партия", ДанныеШтрихкода.Серия);
	Иначе
		ДанныеПоШтрихкоду.Вставить("Партия", Справочники.ПартииНоменклатуры.ПустаяСсылка());
	КонецЕсли;
	...
	Если ИспользоватьСерииНоменклатуры <> Неопределено Тогда
		Если ЗначениеЗаполнено(ДанныеШтрихкода.Серия)
			И ТипЗнч(ДанныеШтрихкода.Серия) = Тип("СправочникСсылка.СерииНоменклатуры") Тогда
			ДанныеПоШтрихкоду.Вставить("Серия", ДанныеШтрихкода.Серия);
		Иначе
			ДанныеПоШтрихкоду.Вставить("Серия", Справочники.СерииНоменклатуры.ПустаяСсылка());
		КонецЕсли;
	КонецЕсли;
	
	Если ДополнитьДаннымиНоменклатуры Тогда
		Если ЗначениеЗаполнено(ДанныеШтрихкода.Номенклатура) Тогда
			ДополнительныеДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеШтрихкода.Номенклатура,
				"ТипНоменклатуры, СтранаПроисхождения");
			ДанныеПоШтрихкоду.Вставить("ТипНоменклатуры", ДополнительныеДанныеНоменклатуры.ТипНоменклатуры);
			ДанныеПоШтрихкоду.Вставить("СтранаПроисхождения", ДополнительныеДанныеНоменклатуры.СтранаПроисхождения);
		Иначе
			ДанныеПоШтрихкоду.Вставить("ТипНоменклатуры", Перечисления.ТипыНоменклатуры.ПустаяСсылка());
			ДанныеПоШтрихкоду.Вставить("СтранаПроисхождения", Справочники.СтраныМира.ПустаяСсылка());
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстЗапросаТоварыСерии(ТаблицаЗапасы) Экспорт
	...
	|	ТаблицаЗапасы.ЭтоВетТребование КАК ЭтоВетТребование
	|ПОМЕСТИТЬ Запасы
	|ИЗ
	...
	|		ИНАЧЕ (Запасы.СуммаАвтоматическойСкидки + Запасы.СуммаСкидкиНаценки + Запасы.СуммаСкидкиОплатыБонусом) /
	|			Запасы.Количество * Серии.Количество
	...
	|	Запасы.ЭтоВетТребование КАК ЭтоВетТребование
	|ИЗ
	|	Запасы КАК Запасы
	...
	ОпциональныеПоля.Вставить("ЭтоВетТребование", "Ложь");
...
Функция ПоддерживаетсяСканированиеМаркировки(ИмяФормы)
	
	ИменаФорм = Новый СписокЗначений;
	ИменаФорм.Добавить("Обработка.ПомощникВводаНачальныхОстатков.Форма.ФормаВводаОстатков");
	ИменаФорм.Добавить("Документ.СчетНаОплатуПоставщика.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.СчетНаОплату.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.СписаниеЗапасов.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.СборкаЗапасов.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.РезервированиеЗапасов.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.РасходныйОрдер.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.РаспределениеЗатрат.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ПриходныйОрдер.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ПриемИПередачаВРемонт.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ПересортицаЗапасов.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ПереоценкаТоваровНаКомиссии.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ПереоценкаВРозницеСуммовойУчет.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ПеремещениеПоЯчейкам.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ПеремещениеЗапасов.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ОтчетОРозничныхПродажах.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ОтчетКомитенту.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ОтчетКомиссионераОСписании.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ОтчетКомиссионера.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ОприходованиеЗапасов.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.КомплектацияЗапасов.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ИнвентаризацияЗапасов.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ЗаказНаПроизводство.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ЗаказНаПеремещение.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ВводНачальныхОстатков.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.АвансовыйОтчет.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.ЗаказПокупателя.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Документ.УстановкаЦенНоменклатуры.Форма.ФормаДокумента");
	ИменаФорм.Добавить("Справочник.Номенклатура.Форма.ФормаСписка");
	
	Возврат ИменаФорм.НайтиПоЗначению(ИмяФормы) <> Неопределено;
	
КонецФункции

Процедура ИнициализироватьФормуДляПоддержкиСканированияМаркировки(Форма)
	
	Если НЕ ИнициализацияФормыВыполнена(Форма) Тогда
		ИмяТабличнойЧасти = "Запасы";
		Форма.КэшДанныхГОСИС.Вставить("ИмяТабличнойЧасти", ИмяТабличнойЧасти);
		
		ПараметрыИнтеграцииЕГАИС = Форма.ПараметрыИнтеграцииГосИС.Получить("ЕГАИС.ДокументОснование");
		Если (Форма.ИмяФормы = "Справочник.Номенклатура.Форма.ФормаСписка"
			ИЛИ Форма.ИмяФормы = "Обработка.ПомощникВводаНачальныхОстатков.Форма.ФормаВводаОстатков")
			И ЗначениеЗаполнено(ПараметрыИнтеграцииЕГАИС) Тогда
			
			ПараметрыИнтеграцииГосИС = Новый Соответствие(Форма.ПараметрыИнтеграцииГосИС);
			ПараметрыИнтеграцииГосИС.Удалить("ЕГАИС.ДокументОснование");
			Форма.ПараметрыИнтеграцииГосИС = Новый ФиксированноеСоответствие(ПараметрыИнтеграцииГосИС);
		КонецЕсли;
...
Функция ПараметрыФормированияЗапросаЗаполнения(МетаданныеДокументаОснования, МетаданныеДокументаИСМП) Экспорт
	...
	ПараметрыФормирования.Вставить("ПотребительскиеУпаковки",          Ложь);
...
Функция ТекстЗапросаТоварыДокументаОснования(
	...
	ОпциональныеПоля = Новый Соответствие;
	ОпциональныеПоля.Вставить("Цена",      "0");
...
Процедура ЗаполнитьТабличныеЧастиДокументаИС(ДокументОбъект, ДанныеЗаполнения, ПараметрыФормированияЗапроса) Экспорт
	...
	СвойстваТаблицыИС = МетаданныеДокументаИСМП.ТабличныеЧасти[ИмяТЧТовары].Реквизиты;
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СвойстваТаблицыИС, "Цена") Тогда
		ТекстЗапросаТоварыДокументаИС = СтрЗаменить(ТекстЗапросаТоварыДокументаИС, "ТоварыИС.Цена", "0");
	КонецЕсли;
	...
	|	СправочникНоменклатура.Крепость КАК СодержаниеЭтиловогоСпирта,
	...
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ / &КоэффициентПересчетаКоличества КАК Количество,
	...
	Если ПараметрыФормированияЗапроса.ПотребительскиеУпаковки Тогда
		ТекстКоэффициентПересчетаКоличества =
		"ЕСТЬNULL(КоэффициентыПересчета.КоэффициентПересчета, 1)";
	Иначе
		ТекстКоэффициентПересчетаКоличества = "1";
	КонецЕсли;
	ТекстЗапросаОбъединения = СтрЗаменить(ТекстЗапросаОбъединения,
		"&КоэффициентПересчетаКоличества",
		ТекстКоэффициентПересчетаКоличества);
...
Процедура ЗаполнитьШапкуДокумента(ДокументОбъект, ДанныеЗаполнения, Запрос) Экспорт
	...
	Если ТипЗнч(РеквизитыДокумента.ДокументОснование) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(РеквизитыДокумента.ВидОперации) Тогда
			ВызватьИсключение РаботаСЗаявкамиКлиентСервер.СообщениеОНеобходимостиПереводаЗаявкиВЗаказ(
...
Функция ДанныеШтрихкодаДляОбработки() Экспорт
	...
	СтруктураПараметров.Вставить("РазрешительныйРежимИдентификаторЭкземпляраПО");
	СтруктураПараметров.Вставить("РазрешительныйРежимВерсияБазы");
...
Процедура ОбработатьДанныеШтрихкодаПотребительскойУпаковки(ДанныеШтрихкодаДляОбработки, ПараметрыШтрихкодаДляОбработки, Форма = Неопределено) Экспорт
	...
	Если НЕ СтрокаДляОбработки = Неопределено Тогда
		...
		Иначе
			...
			Если РазрешеноЗапрашиватьКодМаркировки
				И НЕ ДанныеШтрихкодаДляОбработки.ОбработатьБезМаркировки Тогда
				ВсегоВКеше = 0;
		...
		Если КоличествоМаркируемойПродукции > 0 Тогда
			...
			Иначе
				...
				Если ДанныеШтрихкодаДляОбработки.Количество = ДанныеШтрихкодаДляОбработки.КоличествоВБазовыхЕдиницахГосИС Тогда
	...
	Если ЗначениеЗаполнено(ДанныеШтрихкодаДляОбработки.Серия)
		...
		Иначе
			НайденныеСтрокиСерии[0].Количество = НайденныеСтрокиСерии[0].Количество + ДанныеШтрихкодаДляОбработки.Количество;
		КонецЕсли;
		
	КонецЕсли;
	
	Если ДанныеШтрихкодаДляОбработки.ОбработатьБезМаркировки Тогда
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДляОбработки,
			"МаркируемаяПродукция") Тогда
