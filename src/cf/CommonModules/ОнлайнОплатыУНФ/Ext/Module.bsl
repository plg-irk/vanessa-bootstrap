...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки,
	...
	Команда.Вид = "ЮКасса";
	Команда.Идентификатор 				= "ЗагрузитьОплатыПоЮKassa";
	Команда.Представление 				= НСтр("ru = 'Загрузить оплаты ЮKassa'");
	...
	Команда.ТипПараметра 				= ТипыПараметровСпискаЮKassa();
	...
	Команда.Обработчик 					= "ОнлайнОплатыУНФКлиент.ЗагрузитьОплатыПоЮкаssа";
	
	Если Пользователи.РолиДоступны("ПолучениеПлатежнойСсылкиДляОнлайнОплаты") Тогда
...
Процедура ПриОпределенииДополнительныхНастроекОнлайнОплаты(ДополнительныеНастройки) Экспорт
	...
	ТипЧисло  		= Новый ОписаниеТипов("Число", ,, Новый КвалификаторыЧисла(10, 1, ДопустимыйЗнак.Неотрицательный));
	...
	СтрокаНастройки = ДополнительныеНастройки.Добавить();
	СтрокаНастройки.Настройка = "СрокЖизниСсылки";
	СтрокаНастройки.Представление = НСтр("ru = 'Срок действия платежной ссылки'");
	СтрокаНастройки.ТипЗначения = ТипЧисло;
...
Процедура ПриСозданииФормыОнлайнОплаты(Форма, Группа, Префикс, ДополнительныеНастройки) Экспорт
	...
	Реквизит = Новый РеквизитФормы(
		Префикс + "СрокЖизниСсылки",
		Новый ОписаниеТипов("Число", ,, новый КвалификаторыЧисла(10, 1, ДопустимыйЗнак.Неотрицательный)),
		,
		НСтр("ru = 'Срок действия платежной ссылки'"),
		Истина);
	ДобавляемыеРеквизиты.Добавить(Реквизит);
	ДополнительныеНастройки.Вставить("СрокЖизниСсылки", Реквизит.Имя);
	...
	ИмяРеквизита = Префикс + "СрокЖизниСсылки";
	Поле = Элементы.Добавить(ИмяРеквизита, ТипПолеФормы, ГруппаНастройкиУчета);
	Поле.Вид = ВидПоляФормы.ПолеВвода;
	Поле.ПутьКДанным = ИмяРеквизита;
	Поле.Подсказка = НСтр("ru = 'Устанавливается в часах и показывает время действия QR-кода при отправке платежной ссылки из заказов, счетов, документов реализации и др.'");
	Поле.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
	Поле.МинимальноеЗначение = 0.5;                             
	Поле.МаксимальноеЗначение = 2160;                             
	Поле.АвтоОтметкаНезаполненного = Истина;                             
...
Процедура ПередНачаломРедактированияДополнительныхНастроекОнлайнОплаты(Контекст, Отказ = Ложь) Экспорт
	...
	ПолеСрокЖизниСсылки  = Контекст.Префикс + "СрокЖизниСсылки";
	...
	БанковскийСчетПоУмолчанию			= ?(ЗначениеЗаполнено(Контекст.Организация),
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контекст.Организация, "БанковскийСчетПоУмолчанию"),
		Справочники.БанковскиеСчета.ПустаяСсылка());
	
	Контекст.Форма[ПолеБанковскийСчет] 	= БанковскийСчетПоУмолчанию;
	...
	Контекст.Форма[ПолеСрокЖизниСсылки] = 30 * 24; // 30 дней
...
Процедура ПередОкончаниемРедактированияДополнительныхНастроекОнлайнОплат(Контекст, Отказ = Ложь) Экспорт
	...
	ИмяРеквизита = Контекст.Префикс + "СрокЖизниСсылки";
	Если Не ЗначениеЗаполнено(Контекст.Форма[ИмяРеквизита]) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не заполнен срок действия платежной ссылки'"),, ИмяРеквизита);
		Отказ = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеОснованияПлатежа(Знач ОснованиеПлатежа, ДанныеОснованияПлатежа) Экспорт
	...
	Если ВыборкаШапка.Следующий() Тогда
		...
		ОтборПоОрганизации 	= Новый Структура("Организация", ВыборкаШапка.Организация);
		ВыборкаНастроек 	= Справочники.НастройкиОнлайнОплат.Выбрать(,, ОтборПоОрганизации);
		
		Пока ВыборкаНастроек.Следующий() Цикл
			
			Если Не ВыборкаНастроек.ПометкаУдаления Тогда
				
				СрокЖизниСсылки = ВыборкаНастроек.ДополнительныеНастройки.Найти("СрокЖизниСсылки", "Настройка");
				СрокЖизниСсылки = ?(СрокЖизниСсылки <> Неопределено, СрокЖизниСсылки.Значение, 0);
		...
		ДанныеОснованияПлатежа.СрокЖизниСсылки	= ?(СрокЖизниСсылки = 0, 30 * 24 * 60, окр(СрокЖизниСсылки * 60)); // 30 дней в минутах
...
Функция НайтиИлиСоздатьЭквайринговыйТерминалПоУмолчанию(СтруктураПараметров) Экспорт
	...
	БанковскийСчетПоУмолчанию			= ?(ЗначениеЗаполнено(СтруктураПараметров.Организация),
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметров.Организация, "БанковскийСчетПоУмолчанию"),
		Справочники.БанковскиеСчета.ПустаяСсылка());

	КассаПоУмолчанию					= ?(ЗначениеЗаполнено(СтруктураПараметров.Организация),
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметров.Организация, "КассаПоУмолчанию"),
	...
	НовыйТерминал.БанковскийСчетЭквайринг = БанковскийСчетПоУмолчанию;
	...
	Иначе	
		НовыйТерминал.Касса = КассаПоУмолчанию;
	КонецЕсли; 
...
Функция ТипыПараметровСпискаЮKassa()
	
	ИменаДокументов = Новый Массив;
	ИменаДокументов.Добавить("Документ.АктВыполненныхРабот");
	ИменаДокументов.Добавить("Документ.ЗаказПокупателя");
	ИменаДокументов.Добавить("Документ.ОперацияПоПлатежнымКартам");
	ИменаДокументов.Добавить("Документ.ПриемИПередачаВРемонт");
	ИменаДокументов.Добавить("Документ.РасходнаяНакладная");
	ИменаДокументов.Добавить("Документ.СчетНаОплату");
	
	ТипыОбъектов = Новый Массив;
	Для Каждого ПолноеИмя Из ИменаДокументов Цикл
		Менеджер = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
		ТипыОбъектов.Добавить(ТипЗнч(Менеджер.ПустаяСсылка()));
	КонецЦикла;
	
	Возврат Новый ОписаниеТипов(ТипыОбъектов);
...
Функция КодСистемыНалогообложения(Организация, Период)
	...
	ИначеЕсли УПОрганизации.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная
		Или УПОрганизации.СистемаНалогообложения = Перечисления.СистемыНалогообложения.АУСН Тогда
