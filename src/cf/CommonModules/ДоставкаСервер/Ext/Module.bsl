...
Процедура ЗаполнитьПараметрыЗаказаНаДоставку(Параметры) Экспорт
	...
	ДокументОснование = ДокументыОснования[0].Значение;
	Параметры.ДокументОснование = ДокументОснование;
	...
	Если Параметры.ТипГрузоперевозки = СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СКурьерика() Тогда
		Параметры.СуммаДокумента = ДанныеПоДокументу.СуммаДокумента;
		СуммаНаложенногоПлатежа = ДанныеПоДокументу.СуммаДокумента - ДанныеПоДокументу.СуммаАванса - ДанныеПоДокументу.СуммаОплаты;
		Параметры.СуммаНаложенногоПлатежа = ?( СуммаНаложенногоПлатежа > 0, СуммаНаложенногоПлатежа, 0 );
		Если Параметры.СуммаНаложенногоПлатежа >= Параметры.СуммаДокумента Тогда
			Параметры.Оплачен = Истина;
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

Функция СервисДоставкиПоТипуГрузоперевозки( ТипГрузоперевозки ) Экспорт
	
	Если ТипГрузоперевозки = СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СДоставка() Тогда
		Возврат Перечисления.СервисыДоставки.Сервис1СДоставка;
	ИначеЕсли ТипГрузоперевозки = СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СКурьер() Тогда
		Возврат Перечисления.СервисыДоставки.Сервис1СКурьер;
	ИначеЕсли ТипГрузоперевозки = СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СКурьерика() Тогда
		Возврат Перечисления.СервисыДоставки.Сервис1СКурьерика;
	Иначе
		Возврат Перечисления.СервисыДоставки.БезПомощиСервиса;
	КонецЕсли;
	
КонецФункции

Функция ТипГрузоперевозкиПоСервисуДоставки( ПараметрСервисДоставки ) Экспорт
	
	Если ПараметрСервисДоставки = Перечисления.СервисыДоставки.Сервис1СДоставка Тогда
		Возврат СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СДоставка();
	ИначеЕсли ПараметрСервисДоставки = Перечисления.СервисыДоставки.Сервис1СКурьер Тогда
		Возврат СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СКурьер();
	ИначеЕсли ПараметрСервисДоставки = Перечисления.СервисыДоставки.Сервис1СКурьерика Тогда
		Возврат СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СКурьерика();
	Иначе
		Возврат 0;
	КонецЕсли;
...
Процедура ОбновитьДанныеПоДоставкеИзСервиса(ДокументОснование, ИмяГруппыДоставка, Организация, Форма,
    ...
    ТипГрузоперевозкиИзДанныеЗаказовПоДокументуОснованиеЧисломПолучить( ДокументОснование );
	
	ПараметрыЗаказаНаДоставку = ПолучитьЗаказНаДоставкуИзСервиса(Организация, ДокументОснование, 
	ЗаказНаДоставку.ИдентификаторЗаказа, ТипГрузоперевозкиИзДанныхЗаказовЧислом);
...
// 
// Возвращаемое значение:
//   Структура, Строка- данные по заказу из сервиса
//
Функция ПолучитьЗаказНаДоставкуИзСервиса(Организация, ДокументОснование, ИдентификаторЗаказа, ТипГрузоперевозки) Экспорт
...
Функция ТекстЗапросаДляЗаказаНаДоставкуПоЗаказуПокупателя(Параметры)
	...
	|	ЕстьNull( ФактОплатыЗаказов.СуммаАванса, 0 ) Как СуммаАванса,
	|	ЕстьNull( ФактОплатыЗаказов.СуммаОплаты, 0 ) Как СуммаОплаты
	|ИЗ
	...
	|		ПО (ФактОплатыЗаказов.СчетНаОплату = ШапкаДокумента.Ссылка)
	|ГДЕ
...
Функция ТекстЗапросаДляЗаказаНаДоставкуПоЧекуККМ()
	...
	|	ШапкаДокумента.СуммаДокумента КАК СуммаДокумента,
	|	0 КАК СуммаАванса,
	|	ШапкаДокумента.СуммаДокумента КАК СуммаОплаты
	|ИЗ
...
Функция ТекстЗапросаДляЗаказаНаДоставкуПоРасходнойНакладной()
	...
	|	"""" КАК ГрузОписание,
	|	ШапкаДокумента.СуммаДокумента КАК СуммаДокумента,
	|	ЕСТЬNULL(ФактОплатыДокументов.СуммаАванса, 0) КАК СуммаАванса,
	|	ЕСТЬNULL(ФактОплатыДокументов.СуммаОплаты, 0) КАК СуммаОплаты
	|ИЗ
	...
	|		ПО (ФактОплатыДокументов.Документ = ШапкаДокумента.Ссылка)
	|ГДЕ
	|	ШапкаДокумента.Ссылка = &Ссылка
	|;
	|
...
Функция ТипГрузоперевозкиИзДанныеЗаказовПоДокументуОснованиеПолучить( ДокументОснование )

	Результат = Перечисления.ТипыГрузоперевозки.ПустаяСсылка();
	
	ЗапросТекст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ДанныеЗаказовСервисДоставки.ТипГрузоперевозки КАК ТипГрузоперевозки
	|ИЗ
	|	РегистрСведений.ДанныеЗаказовСервисДоставки КАК ДанныеЗаказовСервисДоставки
	|ГДЕ
	|	ДанныеЗаказовСервисДоставки.ДокументОснование = &ДокументОснование";
	
	ЗапросОбъект = Новый Запрос( ЗапросТекст );
	ЗапросОбъект.УстановитьПараметр( "ДокументОснование", ДокументОснование );
	...
	Если ЗапросВыборка.Следующий() Тогда
		Результат = ЗапросВыборка.ТипГрузоперевозки;	
	КонецЕсли;
	
	Возврат Результат;
КонецФункции

Функция ТипГрузоперевозкиИзДанныеЗаказовПоДокументуОснованиеЧисломПолучить( ДокументОснование ) Экспорт
	
	Результат = 0;
	
    ТипГрузоперевозки = ТипГрузоперевозкиИзДанныеЗаказовПоДокументуОснованиеПолучить( ДокументОснование );
    Если Не ЗначениеЗаполнено( ТипГрузоперевозки ) Тогда
    	Возврат Результат;
    КонецЕсли;
    
    Результат = Перечисления.ТипыГрузоперевозки.Индекс(ТипГрузоперевозки) + 1;	
