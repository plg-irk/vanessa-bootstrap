...
Функция СостояниеРегистрацииККМ(Ссылка) Экспорт
	...
	|ИЗ
	|	РегистрСведений.УдалитьНастройкиКассыШтрихМ КАК КассовыеАппараты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УдалитьЗарегистрированныеКассы КАК ЗарегистрированныеКассы
...
Процедура ЗаписатьКассовыйДокумент(ИнформацияПоДокументу, ДатаУспешногоЧека, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	ПротоколЗагрузки = Справочники.УдалитьДанныеОбменаШтрихМПЭЙ.НайтиПоРеквизиту(
		"ИдентификаторДокументаШтрихМ",
		Новый УникальныйИдентификатор(ИдентификаторДокумента));
		
	Если ПротоколЗагрузки.Пустая() Тогда
		ПротоколЗагрузки = Справочники.УдалитьДанныеОбменаШтрихМПЭЙ.СоздатьЭлемент();
...
Процедура ЗавершитьЗагрузкуЧеков(РегистрационныйНомер) Экспорт
	...
	Касса = РегистрыСведений.УдалитьНастройкиКассыШтрихМ.СоздатьМенеджерЗаписи();
...
Процедура ОбработатьРезультатПодключенияКассы(ИнформацияОПодключении, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	Если ЗначениеЗаполнено(КассовыйАппарат) Тогда
		Попытка
			МенеджерЗаписиКассы = РегистрыСведений.УдалитьНастройкиКассыШтрихМ.СоздатьМенеджерЗаписи();
...
Процедура ОбменТоварамиССерверомШтрихМ() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбменТоваромССерверомШтрихМ);
...
Процедура ОбработатьРезультатОчисткиТоваров(ИнформацияОПодключении, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	Если ЗначениеЗаполнено(КассовыйАппарат) Тогда
		Попытка
			КассаОбъект = РегистрыСведений.УдалитьНастройкиКассыШтрихМ.СоздатьМенеджерЗаписи();//КассовыйАппарат.ПолучитьОбъект();
