...
//  ОтображатьНаименованиеДляПечати - Булево - Признак использования наименования для печати.
//
Процедура ЗаполнитьИмяЭлементаВСтрокеСоставаПалитры(СтрокаСостава, ОтображатьНаименованиеДляПечати = Ложь) Экспорт
	
	Если Не СтрокаСостава = Неопределено Тогда
		ПредставлениеНоменклатуры = "";
		ПредставлениеХарактеристики = "";
		Если ОтображатьНаименованиеДляПечати Тогда
			ОбщегоНазначенияРМККлиентПереопределяемый.ЗаполнитьПредставленияНоменклатурыИХарактеристики(
				ПредставлениеНоменклатуры, ПредставлениеХарактеристики,
				СтрокаСостава.Номенклатура, СтрокаСостава.Характеристика);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ПредставлениеНоменклатуры) Тогда
			ПредставлениеНоменклатуры = Строка(СтрокаСостава.Номенклатура);
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаСостава.Характеристика)
			И НЕ ЗначениеЗаполнено(ПредставлениеХарактеристики) Тогда
			ПредставлениеХарактеристики = Строка(СтрокаСостава.Характеристика);
		КонецЕсли;
		
		ИмяЭлемента = СтрШаблон(НСтр("ru = '%1 %2 %3'"),
			ПредставлениеНоменклатуры, ПредставлениеХарактеристики, СтрокаСостава.Упаковка);
...
Процедура ПередНачаломРаботыСистемы() Экспорт
	...
	КлючЗапускаКСО = СтрНачинаетсяС(ПараметрЗапуска, "BRMK_KSO");
	...
	БРМК_ЗапуститьРМКПриСтарте = ПараметрыРаботыКлиента.Свойство("БРМК_ЗапуститьРМКПриСтарте")
		И ПараметрыРаботыКлиента.БРМК_ЗапуститьРМКПриСтарте;
	
	Если БРМК_РежимКассыСамообслуживания ИЛИ БРМК_ЗапуститьНовыйРМК И НЕ БРМК_ЗапуститьРМКПриСтарте Тогда
...
// Выполняется при интерактивным началом работы пользователя с областью данных или в локальном режиме.
...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	// добавлением в процедуру ОбщегоНазначенияРМК.ЗаполнитьПараметрыРаботыКлиентаПриЗапуске(Параметры);
	ПараметрыРаботыКлиента = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиентаПриЗапуске();
	БРМК_ЗапуститьНовыйРМК = ПараметрыРаботыКлиента.Свойство("БРМК_ЗапуститьРМКПриСтарте")
		И ПараметрыРаботыКлиента.БРМК_ЗапуститьРМКПриСтарте;
	БРМК_ПолноэкранныйРежимЗапуска = ПараметрыРаботыКлиента.Свойство("БРМК_ПолноэкранныйРежимЗапуска")
		И ПараметрыРаботыКлиента.БРМК_ПолноэкранныйРежимЗапуска;
		
	Если БРМК_ЗапуститьНовыйРМК И НЕ БРМК_ПолноэкранныйРежимЗапуска Тогда
		ОткрытьРабочееМестоКассира();
	КонецЕсли;
...
Процедура ОткрытьРабочееМестоКассира() Экспорт
	...
	ОткрытьФорму("Обработка.РабочееМестоКассира.Форма", , , , , , ОбработчикОповещения);
...
Процедура ОткрытьСтандартныйВопросОбработкиАвтономнойККТ(Отказ, СтандартнаяОбработка, ПараметрыОперации, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыОперации.Команда = "CheckFiscalization" Тогда 
		...
		ПараметрыОперации.Форма.Элементы.ОтветДаАвтономнаяКасса.Заголовок = НСтр("ru='Пробит'");
...
Функция ИменаРеквизитовФормыРМКДляДобавленияСтроки() Экспорт
	...
	ИменаРеквизитов = "ОсновнаяОрганизация, ОтображатьОстаток, ИмяФормы, РозничныйВидЦены, ИспользоватьКонтрольПродаж,
		...
		|ИспользоватьУточненияПринадлежностиЗапасов";
	ОбщегоНазначенияРМККлиентПереопределяемый.
...
// Возвращает строку имен реквизитов формы для формирования структуры реквизитов аналогичной форме авторизации.
// Структура требуется для обработки поиска кассира.
//
// Возвращаемое значение:
//  Строка - Имена реквизитов через запятую.
//
Функция ИменаРеквизитовФормыАвторизацииРМК() Экспорт
	
	ИменаРеквизитов = "ИмяФормы, ФильтрПоискаПоШтрихкоду, РежимКассыСамообслуживания";
	ОбщегоНазначенияРМККлиентПереопределяемый.
		ДополнитьИменаРеквизитовФормыАвторизацииРМК(ИменаРеквизитов);
	Возврат ИменаРеквизитов;
...
Процедура ЗаполнитьДополнительныеПараметрыПроверкиКодаАвторизаци(Форма, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияРМККлиентПереопределяемый.ЗаполнитьДополнительныеПараметрыПроверкиКодаАвторизаци(Форма,
		ДополнительныеПараметры);
...
//
// Возвращаемое значение:
//  Структура - имена событий оповещения проверки марки на ККТ.
//
...
// Возвращает выполняемые операции проверки средствами ККТ
// 
// Возвращаемое значение:
//  Структура - содержит строки операций
//
...
Процедура СверитьСДаннымиОФДЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Статус = "Выполнено" Тогда
		...
		Если ТипЗнч(РезультатОперации) = Тип("Структура") Тогда
			...
			Если Не РезультатОперации.Отказ Тогда
				Результат.ЕстьРасхожденияВСверке = Истина;
			Иначе
				Результат.Результат = Ложь;
			КонецЕсли;
			Результат.Отказ = РезультатОперации.Отказ;
			Результат.ТекстОшибки = РезультатОперации.ИнформацияОбОшибке;
		КонецЕсли;
