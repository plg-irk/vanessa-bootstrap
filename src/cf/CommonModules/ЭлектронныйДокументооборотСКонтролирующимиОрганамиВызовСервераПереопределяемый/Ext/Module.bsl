...
Функция ВыгрузитьДокумент(Ссылка, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЖурналУчетаСчетовФактур") Тогда
		...
		ДанныеФайлов = Документы.ЖурналУчетаСчетовФактур.СформироватьЭлектронныеДокументы(СписокДокументов, УникальныйИдентификатор);
		Если ДанныеФайлов = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
		
		ИнформацияОФайлах = ДанныеФайлов.Получить(Ссылка);
		Если ИнформацияОФайлах = Неопределено ИЛИ ИнформацияОФайлах.Количество() = 0 Тогда
			Возврат Неопределено;
		КонецЕсли;
		ИнформацияОФайле = ИнформацияОФайлах[0];
		ФайлДляОтправки.ИмяФайлаВыгрузки = ИнформацияОФайле.ИмяФайла;
		ФайлДляОтправки.АдресФайлаВыгрузки = ИнформацияОФайле.АдресВременногоХранилища;
		ФайлДляОтправки.ТипФайлаВыгрузки = ИнформацияОФайле.ТипФайла;
		ФайлДляОтправки.КодировкаФайлаВыгрузки =  КодировкаТекста.ANSI;
		Возврат ФайлДляОтправки;
	Иначе 
...
Функция ПолучитьДанныеИсполнителя(СсылкаФизЛицо, ОрганизацияСсылка) Экспорт
	...
	Структура.Вставить("АдресПоПрописке_JSON", "");
	Структура.Вставить("АдресМестаПроживания_JSON", "");
