...
Процедура СформироватьТабличнуюЧастьСотрудники(Объект) Экспорт
	...
	ПоляВыборкиНДФЛ = Новый Массив;
	ПоляСуммированияНДФЛ = Новый Массив;
	Для каждого ИмяКолонки Из УчетНДФЛ.РесурсыИсчисленногоНалогаВМассиве("Налог") Цикл
		ПоляВыборкиНДФЛ.Добавить(СтрШаблон("НачислениеЗарплатыНДФЛ.%1 КАК %1", ИмяКолонки));
		ПоляСуммированияНДФЛ.Добавить(СтрШаблон("НачислениеЗарплатыНДФЛ.%1", ИмяКолонки));
	КонецЦикла;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "НачислениеЗарплатыНДФЛ.Налог КАК Налог",
		СтрСоединить(ПоляВыборкиНДФЛ, "," + Символы.ПС));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "СУММА(НачислениеЗарплатыНДФЛ.Налог) КАК НДФЛ",
		СтрШаблон("СУММА(%1) КАК НДФЛ", СтрСоединить(ПоляСуммированияНДФЛ, " + ")));
...
Процедура ЗаполнитьРезультатУдержаний(Объект, ДобавляемыеНачисления = Неопределено) Экспорт
	...
	Если ДобавляемыеНачисления = Неопределено Тогда
		...
		Если Объект.НачисленияПоДоговорам.Количество() > 0 Тогда
			
			Начисления.Колонки.Начисление.Имя = "НачислениеПВР";
			Начисления.Колонки.Добавить("Начисление", Новый ОписаниеТипов("ПланВидовРасчетаСсылка.Начисления,ПеречислениеСсылка.ВидыОсобыхНачисленийИУдержаний"));
			Для Каждого СтрокаНачислений Из Начисления Цикл
				СтрокаНачислений.Начисление = СтрокаНачислений.НачислениеПВР;
			КонецЦикла;
			Для Каждого СрокаДоговра Из Объект.НачисленияПоДоговорам Цикл
				СтрокаНачисления = Начисления.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаНачисления, СрокаДоговра);
				СтрокаНачисления.Начисление = Перечисления.ВидыОсобыхНачисленийИУдержаний.ДоговорРаботыУслуги;
			КонецЦикла;
		КонецЕсли;
	Иначе
...
Процедура ЗаполнитьРезультатНачисления(Объект, Начисления) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "МесяцНачисления") Тогда
		МесяцНачисления = Объект.МесяцНачисления;
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ПериодРегистрации") Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ПерваяПоловинаМесяца") Тогда
		Если Объект.ПерваяПоловинаМесяца Тогда
			ОкончаниеПериода = РасчетЗарплатыДляНебольшихОрганизацийПовтИсп.ДатаРасчетаАванса(МесяцНачисления);
			ОкончаниеПериода = РасчетЗарплатыДляНебольшихОрганизаций.ДатаСобытияСУчетомНерабочихДней(ОкончаниеПериода, Объект.Организация);
	...
	Запрос.Текст =
		...
		|			И НЕ НачисленияУдержанияПоСотрудникам.НачислениеУдержание.КатегорияНачисленияИлиНеоплаченногоВремени В (&СписокКатегорийИсключений)";
	...
	Для каждого СтрокаВытеснения Из ТаблицаВытеснений Цикл
		...
		ДатаОкончания = СтрокаВытеснения.ДатаОкончания;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ПерваяПоловинаМесяца") Тогда
			Если Объект.ПерваяПоловинаМесяца Тогда
				ДатаОкончания = Мин(ДатаОкончания, РасчетЗарплатыДляНебольшихОрганизацийПовтИсп.ДатаРасчетаАванса(МесяцНачисления));
			КонецЕсли;
		КонецЕсли;
		
		РабочихДнейЧасов = РасчетЗарплатыБазовый.РабочихДнейЧасовВПериоде(КалендарьОрганизации, СтрокаВытеснения.ДатаНачала, ДатаОкончания);
				...
				Если ЗначениеЗаполнено(СтрокаНачислений.ОтработаноДней) И СтрокаНачислений.ОтработаноДней > 0 Тогда
					...
					Если ДанныеСотрудника <> Неопределено Тогда
						...
						Если ВытеснениеСотрудника <> Неопределено Тогда
							...
							СтрокаНачисленийДатаНачала = Макс(ДанныеСотрудника.ДатаПриема, МесяцНачисления);
							Если ЗначениеЗаполнено(ДанныеСотрудника.ДатаЗавершенияРаботы) Тогда
								СтрокаНачисленийДатаОкончания = Мин(ДанныеСотрудника.ДатаЗавершенияРаботы, КонецМесяца(МесяцНачисления));
							Иначе
								СтрокаНачисленийДатаОкончания = КонецМесяца(МесяцНачисления);
							КонецЕсли;
...
Функция ДанныеОЗаработкеДляРасчетаСреднегоФСС(Сотрудник, Организация, ДатаНачалаСобытия, РасчетныеГоды) Экспорт
	...
	Запрос.Текст =
		...
		|	СУММА(СведенияОДоходах.БазаДляРасчетаПособий) КАК Сумма
		|ИЗ
	...
	Пока Выборка.СледующийПоЗначениюПоля("Год") Цикл
		...
		Пока Выборка.СледующийПоЗначениюПоля("Месяц") Цикл
			...
			СведенияМесяца = СведенияГода.Получить(Выборка.Месяц);
			Если СведенияМесяца = Неопределено Тогда
				СведенияМесяца = Новый Структура("Заработок,ДнейБолезниУходаЗаДетьми", Новый Массив, 0);
				СведенияГода.Вставить(Выборка.Месяц, СведенияМесяца);
			КонецЕсли; 
			
			Пока Выборка.Следующий() Цикл
				СведенияМесяца.Заработок.Добавить(
					Новый Структура("ФизическоеЛицо,Период,Начисление,Сумма",
						Выборка.ФизическоеЛицо,
						Дата(Выборка.Год, Выборка.Месяц, 1),
						Выборка.Начисление,
						Выборка.Сумма));
			КонецЦикла;
		КонецЦикла;
...
Функция ДанныеРасчетаСреднегоЗаработкаОбщего(ПараметрыПолученияДанныхСреднего) Экспорт
	...
	Заработок.Колонки.Добавить("СуммаРК", 						Новый ОписаниеТипов("Число"));
	Заработок.Колонки.Добавить("СуммаСН",						Новый ОписаниеТипов("Число"));
	Заработок.Колонки.Добавить("Учтено", 						Новый ОписаниеТипов("Число"));
	Заработок.Колонки.Добавить("УчтеноРК", 						Новый ОписаниеТипов("Число"));
	Заработок.Колонки.Добавить("УчтеноСН", 						Новый ОписаниеТипов("Число"));
	...
	Для Каждого СтрокаНачислений Из ДанныеОНачислениях Цикл
		...
		СтрокаЗаработка.УчтеноРК 	= Окр(СтрокаЗаработка.СуммаРК, 2);
		СтрокаЗаработка.УчтеноСН 	= Окр(СтрокаЗаработка.СуммаСН, 2);
	...
	Итоги.Вставить("ВсегоЗаработкаРК",				Заработок.Итог("УчтеноРК"));
	Итоги.Вставить("ВсегоЗаработкаСН",				Заработок.Итог("УчтеноСН"));
	...
	ВсегоЗаработок = ?(Итоги.ВсегоДнейЧасов = 0, 0, Итоги.ВсегоЗаработкаРК / Итоги.ВсегоДнейЧасов);
	Итоги.Вставить("СреднедневнойЗаработокРК", 		Окр(ВсегоЗаработок, 2));
	ВсегоЗаработок = ?(Итоги.ВсегоДнейЧасов = 0, 0, Итоги.ВсегоЗаработкаСН / Итоги.ВсегоДнейЧасов);
	Итоги.Вставить("СреднедневнойЗаработокСН", 		Окр(ВсегоЗаработок, 2));
...
//		Структура
//
Функция ДанныеДляРасчетаОбщегоСреднегоЗаработкаСотрудника(Сотрудник, Организация, НачалоПериодаРасчета, ОкончаниеПериодаРасчета, РазворачиватьПоНачислениям = Ложь) Экспорт
	...
	Запрос.Текст =
		...
		|	КОНЕЦ КАК ДатаОкончания,
		|	ВЫБОР
		|		КОГДА Отсутствия.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаОтпуска)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ОплачиваемыйОтпуск
		...
		|	ВЫБОР
		|		КОГДА ОтсутствияСледующийМесяц.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаОтпуска)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ
		|ИЗ
		...
		|ПОМЕСТИТЬ ВТРабочиеПраздничныеДни
		|ИЗ
		...
		|	РегистрСведений.ДанныеПроизводственногоКалендаря КАК ДанныеПроизводственногоКалендаря
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОтсутствия КАК Отсутствия
		|		ПО (ДанныеПроизводственногоКалендаря.ДатаПереноса МЕЖДУ Отсутствия.ДатаНачала И Отсутствия.ДатаОкончания)
		|ГДЕ
		|	ДанныеПроизводственногоКалендаря.ПроизводственныйКалендарь = &ПроизводственныйКалендарь
		|	И ДанныеПроизводственногоКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Праздник)
		|	И ДанныеПроизводственногоКалендаря.ДатаПереноса <> ДАТАВРЕМЯ(1, 1, 1)
		...
		|ВЫБРАТЬ
		|	Отсутствия.ДатаНачала КАК ДатаНачала,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ РабочиеПраздничныеДни.Дата) КАК ДнейПраздников
		|ПОМЕСТИТЬ ВТПраздничныеДниОтсутствий
		|ИЗ
		|	ВТРабочиеПраздничныеДни КАК РабочиеПраздничныеДни
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОтсутствия КАК Отсутствия
		|		ПО РабочиеПраздничныеДни.ДатаНачала = Отсутствия.ДатаНачала
		|
		|СГРУППИРОВАТЬ ПО
		|	Отсутствия.ДатаНачала
		|;
		...
		|	Отсутствия.Сотрудник КАК Сотрудник,
		|	СУММА(РАЗНОСТЬДАТ(Отсутствия.ДатаНачала, Отсутствия.ДатаОкончания, ДЕНЬ) + 1) - СУММА(ЕСТЬNULL(ВЫБОР
		|				КОГДА Отсутствия.ОплачиваемыйОтпуск
		|					ТОГДА ПраздничныеДниОтсутствий.ДнейПраздников
		|				ИНАЧЕ 0
		|			КОНЕЦ, 0)) КАК НеОтработанныеДни
		...
		|	СУММА(ЕСТЬNULL(Начисления.Сумма, 0)) КАК Сумма,
		...
		|	РАЗНОСТЬДАТ(Начисления.ДатаНачала, Начисления.ДатаОкончания, ДЕНЬ) + 1,
		|	ЕСТЬNULL(НеОтработанныеКалендарныеДни.НеОтработанныеДни, 0)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если ЗарплатаКадры.ВТСодержитСтроки(Запрос.МенеджерВременныхТаблиц, "ВТНедостающиеМесяцы") Тогда
		...
		Запрос.Текст =
			...
			|	ОтпускСреднийЗаработокОбщий.Период КАК Период,
			|	ОтпускСреднийЗаработокОбщий.Начисление КАК Начисление,
			|	ОтпускСреднийЗаработокОбщий.Сумма КАК Сумма
			|ИЗ
			|	Документ.Отпуск.СреднийЗаработокОбщий КАК ОтпускСреднийЗаработокОбщий
	...
	Если Не РазворачиватьПоНачислениям Тогда
		ДанныеДляРасчета.ДанныеОНачислениях.Свернуть("Сотрудник,Период", "Сумма");
	КонецЕсли;
...
Функция ДатаСобытияСУчетомНерабочихДней(ДатаСобытия, Организация, ДатаНакануне = Истина) Экспорт

	ДатаСобытияСУчетомНерабочихДней = ДатаСобытия;

	Даты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДатаСобытия);
	КалендарьОрганизации = РасчетЗарплатыДляНебольшихОрганизацийПовтИсп.КалендарьОрганизации(Организация);
		
	Если ТипЗнч(КалендарьОрганизации) <> Тип("СправочникСсылка.ПроизводственныеКалендари") Тогда
		Параметры = ГрафикиРаботы.ПараметрыПолученияБлижайшихДатПоГрафику();
		Параметры.ПолучатьПредшествующие = ДатаНакануне;
		ДатыРабочихДней = ГрафикиРаботы.БлижайшиеДатыВключенныеВГрафик(КалендарьОрганизации, Даты, Параметры);
	Иначе
		Параметры = КалендарныеГрафики.ПараметрыПолученияБлижайшихРабочихДат();
		Параметры.ПолучатьПредшествующие              = ДатаНакануне;
		Параметры.ПолучатьДатыЕслиКалендарьНеЗаполнен = Истина;
		ДатыРабочихДней = КалендарныеГрафики.БлижайшиеРабочиеДаты(КалендарьОрганизации, Даты, Параметры);
	КонецЕсли;
	
	Если ДатыРабочихДней <> Неопределено Тогда
		ДатаСобытияСУчетомНерабочихДней = ДатыРабочихДней.Получить(НачалоДня(ДатаСобытия));
	КонецЕсли;
...
Функция КоличествоДнейКомпенсацииУдержанияОтпуска(Организация, Сотрудник, Период) Экспорт
	
	СведенияПоОтпуску = Новый Структура();
	СведенияПоОтпуску.Вставить("ДнейКомпенсацииУдержанияОтпуска", 0);
	СведенияПоОтпуску.Вставить("РабочийГодС",                     Дата(1,1,1));
	СведенияПоОтпуску.Вставить("РабочийГодПо",                    Дата(1,1,1));
	
	РасчетЗарплатыДляНебольшихОрганизацийПереопределяемый.РассчитатьКоличествоДнейКомпенсацииУдержанияОтпуска(Организация, Сотрудник, Период, СведенияПоОтпуску);
	
	Возврат СведенияПоОтпуску;
	
КонецФункции

Функция КоличествоПраздничныхДнейКалендаря(Календарь, ДатаНачала, ДатаОкончания) Экспорт
	...
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Дата").Количество();
	
КонецФункции

Функция КалендарныеДниМесяцаБезПраздников(Календарь, Период) Экспорт
	
	КоличествоПраздничныхДней = КоличествоПраздничныхДнейКалендаря(Календарь, НачалоМесяца(Период), КонецМесяца(Период));
	КалендарныеДниМесяцаБезПраздников = День(КонецМесяца(Период)) - КоличествоПраздничныхДней;
	
	Возврат КалендарныеДниМесяцаБезПраздников;
...
// Функция вычисляет итоговое значение колонки переданной коллекции
//
...
Процедура СоздатьВТНачисленияЗарплаты(МенеджерВременныхТаблиц, ИмяВТНачисленияЗарплаты = "ВТНачисленияЗарплаты") Экспорт
	...
	СписокКатегорий = КатегорииОтсутствий();
...
Процедура СоздатьВТПериодыОтсутствий(МенеджерВременныхТаблиц, ИмяВТПериодыОтсутствий = "ВТПериодыОтсутствий") Экспорт
	...
	Запрос.Текст =
		...
		|	И НЕ ОтработанноеВремяПоСотрудникам.Регистратор ССЫЛКА Документ.Увольнение";
...
Функция СоздатьВТНачисленияПоОтсутствиям(МенеджерВременныхТаблиц, ТолькоРазрешенные, МесяцНачисления, Организация, Подразделение = Неопределено, СписокСотрудников = Неопределено, ИсключаемыйРегистратор = Неопределено, ИмяВТНачисленияПоОтсутствиям = "ВТНачисленияПоОтсутствиям")
	...
	КатегорииОтсутствий = КатегорииОтсутствий();
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.31.57";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("76e8bf53-0ea2-4f08-a8d6-116e3873739b");
	Обработчик.Процедура = "РасчетЗарплатыДляНебольшихОрганизаций.СоздатьНачислениеКомпенсацияОтпуска";
	Обработчик.Комментарий = НСтр("ru='Создает начисление ""Компенсация отпуска""'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.34.169";
	Обработчик.Процедура = "РасчетЗарплатыДляНебольшихОрганизаций.ЗаполнитьОсновнойВидРасчетаНачисленийОплатыДолейРКСН";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ОсновнойВидРасчета в начислениях оплаты долей РКСН'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.34.170";
	Обработчик.Процедура = "РасчетЗарплатыДляНебольшихОрганизаций.СоздатьНачисленияОплатыДолейРКиСН";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("ebf4a1fa-1c0c-4ebe-bd9e-b707659a931c");
	Обработчик.Комментарий = НСтр("ru = 'Создание начислений оплаты долей РКСН.'");
...
Процедура СоздатьНачислениеКомпенсацияОтпуска(ПараметрыОбновления = Неопределено) Экспорт
	
	ПланыВидовРасчета.Начисления.СоздатьНачислениеКомпенсацияОтпуска();
	ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.УстановитьПараметрОбновления(ПараметрыОбновления, "ОбработкаЗавершена", Истина);
...
Процедура ПодготовитьНачисленияАУСН() Экспорт
	...
	КатегорииДохода.Добавить(Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки);
...
Функция КатегорииДоходовНДФЛПриАУСН(Организация, Период)
	...
	КатегорииДоходовНДФЛ.Добавить(Перечисления.КатегорииДоходовНДФЛ.ПустаяСсылка());
	Если ОрганизацияПрименяетАУСН(Организация, Период) Тогда
		...
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(
			КатегорииДоходовНДФЛ, Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(
		...
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(
			КатегорииДоходовНДФЛ, Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН);
	КонецЕсли;
...
Функция ЭтоКонтролируемаяКатегорияДоходаПриАУСН(КатегорияДохода)
	...
	Возврат КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.ОплатаТруда
		...
		Или КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки
		Или КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН;
	
КонецФункции

Функция НовоеНачислениеПриАУСН(Источник)
	
	Если Источник.КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиРайонногоКоэффициентаВСреднемЗаработке Или
		Источник.КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиСевернойНадбавкиВСреднемЗаработке Тогда
		Возврат ПланыВидовРасчета.Начисления.ПустаяСсылка();
	КонецЕсли;
	...
	ИначеЕсли НачислениеАУСН.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки Тогда
		
		НачислениеАУСН.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН;
	ИначеЕсли НачислениеАУСН.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН Тогда
		
		НачислениеАУСН.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки;
	...
	Если НачислениеАУСН.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.ОплатаТруда
		...
		Или НачислениеАУСН.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.ПрочиеДоходыВДенежнойФормеОтТрудовойДеятельности
		Или НачислениеАУСН.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки Тогда
...
Процедура ЗаполнитьНачисленияДокументаПоОрганизации(Начисления, Объект)
	...
	НачисленияОрганизации = НачисленияОрганизацииПриАУСН(Объект.Организация, МесяцНачисления);
...
Функция КатегорииДоходаНадбавок(Организация, Период) Экспорт
	
	СписокКатегорий = Новый Массив;
	Если ОрганизацияПрименяетАУСН(Организация, Период) Тогда
		СписокКатегорий.Добавить(Перечисления.КатегорииДоходовНДФЛ.ОплатаТрудаАУСН);
		СписокКатегорий.Добавить(Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН);
	Иначе
...
Процедура СоздатьНачисленияОплатыДолейРКиСН(ПараметрыОбновления = Неопределено) Экспорт
	
	ПланыВидовРасчета.Начисления.СоздатьНачисленияОплатыДолейРКиСН();
	ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.УстановитьПараметрОбновления(ПараметрыОбновления, "ОбработкаЗавершена", Истина);
	
КонецПроцедуры

Процедура ЗаполнитьОсновнойВидРасчетаНачисленийОплатыДолейРКСН(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	Начисления.ВидОперацииПоЗарплате КАК ВидОперацииПоЗарплате,
		|	ВЫБОР
		|		КОГДА Начисления.КатегорияДохода В (&КатегорииАУСН)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЭтоАУСН
		...
		|	ПланВидовРасчета.Начисления КАК Начисления
		|ГДЕ
		|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени В(&КатегорииНачисленийОплатыДолейРКиСН)
		|	И Начисления.ВидОперацииПоЗарплате В(&ВидыОперацийПоЗарплате)
		...
		|	ПланВидовРасчета.Начисления КАК Начисления
		|ГДЕ
		|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени В(&КатегорииНачисленийДляВыделенияРКиСН)
		|	И Начисления.ВидОперацииПоЗарплате В(&ВидыОперацийПоЗарплате)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТНачисленияДолейРКСН.Ссылка КАК НачислениеОплатыДолиРКСН,
		|	ВТОсновныеВидыРасчета.Ссылка КАК ОсновнойВидРасчета
		|ИЗ
		|	ВТНачисленияДолейРКСН КАК ВТНачисленияДолейРКСН
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОсновныеВидыРасчета КАК ВТОсновныеВидыРасчета
		|		ПО ВТНачисленияДолейРКСН.ВидОперацииПоЗарплате = ВТОсновныеВидыРасчета.ВидОперацииПоЗарплате
		|			И ВТНачисленияДолейРКСН.ЭтоАУСН = ВТОсновныеВидыРасчета.ЭтоАУСН";
	
	Запрос.УстановитьПараметр("ВидыОперацийПоЗарплате", ВидыОперацийПоЗарплате);
	Запрос.УстановитьПараметр("КатегорииНачисленийОплатыДолейРКиСН", ПланыВидовРасчета.Начисления.КатегорииНачисленийОплатыДолейРКиСН());
	Запрос.УстановитьПараметр("КатегорииАУСН", Перечисления.КатегорииДоходовНДФЛ.ДоходыАУСН());
	Запрос.УстановитьПараметр("КатегорииНачисленийДляВыделенияРКиСН", ПланыВидовРасчета.Начисления.КатегорииНачисленийДляВыделенияРКиСН());
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ОсновнойВидРасчета) Тогда
			НачислениеОбъект = Выборка.НачислениеОплатыДолиРКСН.ПолучитьОбъект();
			НачислениеОбъект.ОсновнойВидРасчета = Выборка.ОсновнойВидРасчета;
