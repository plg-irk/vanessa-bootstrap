...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьРегулярныеПлатежи() Экспорт
	...
	ПараметрыРегулярныхПлатежей = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		Справочники.ЗадачиАссистентаПоРегулярнымПлатежам.ЗадачаПоРегулярнымПлатежамОсновная,
		"ПараметрыРегулярныхПлатежей");
		
	КоличествоЗаписей =	ПараметрыРегулярныхПлатежей.Количество();
...
// Проверка на возможность отключения опции ИспользоватьВыплатыСамозанятым.
//
// Возвращаемое значение:
// 	Строка - текст сообщения если изменение настройки невозможно.
Функция ОтказСнятьИспользоватьВыплатыСамозанятым() Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(ВыплатыСамозанятым.Ссылка) КАК КоличествоДокументов
	|ИЗ
	|	Документ.ВыплатыСамозанятым КАК ВыплатыСамозанятым";

	ВыборкаЗапроса = Запрос.Выполнить().Выбрать();

	Если ВыборкаЗапроса.Следующий() И ВыборкаЗапроса.КоличествоДокументов > 0 Тогда
...
КонецФункции // ОтказСнятьИспользоватьВыплатыСамозанятым()

// Проверка на возможность отключения опции ИспользоватьДокументРасходыПредпринимателя.
//
// Возвращаемое значение:
// 	Строка - текст сообщения если изменение настройки невозможно.
Функция ОтказСнятьИспользоватьДокументРасходыПредпринимателя() Экспорт

	Запрос = Новый Запрос;

	Запрос.Текст =
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ РасходыПредпринимателя.Ссылка) КАК КоличествоИП
	|ИЗ
	|	Документ.РасходыПредпринимателя КАК РасходыПредпринимателя";

	ВыборкаЗапроса = Запрос.Выполнить().Выбрать();

	Если ВыборкаЗапроса.Следующий() И ВыборкаЗапроса.КоличествоИП > 0 Тогда

		ТекстСообщения = НСтр(
			"ru = 'В базе присутствуют документы ""Расходы предпринимателя"".
			|Изменение настройки невозможно.'");
	КонецЕсли;

	Возврат ТекстСообщения;

КонецФункции // ОтказСнятьИспользоватьДокументРасходыПредпринимателя()

// Проверка на возможность отключения опции ИспользоватьДокументРасходыПредпринимателя.
//
// Возвращаемое значение:
// 	Булево - признак присутствия хотя бы одной организации ИП.
Функция ЕстьХотяБыОднаОрганизацияИП() Экспорт

	Запрос = Новый Запрос;

	Запрос.Текст =
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Организации.Ссылка) КАК КоличествоДокументов
	|ИЗ
	|	Справочник.Организации КАК Организации
	|ГДЕ
