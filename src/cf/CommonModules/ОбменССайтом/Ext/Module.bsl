...
//	ПараметрыСоставОбмена - Структура - влияет на объем выгружаемых данных.
//
...
//  ВыгружатьТолькоИзменения - Булево - Выгружать только изменения
// 
// Возвращаемое значение:
...
Функция ТекстФайлаОбмена(УзелОбмена, ВыгружатьТолькоИзменения = Истина) Экспорт
	...
	Если Параметры.ОбменТоварами Тогда 
		...
		Для Каждого СтрокаТаблицыКаталогов Из ТаблицаКаталогов Цикл
			...
			СтрокаТаблицыКаталогов.СтруктураРезультата = Новый Структура();
			СтрокаТаблицыКаталогов.СтруктураРезультата.Вставить("ВыгруженоТоваров", 0);
			СтрокаТаблицыКаталогов.СтруктураРезультата.Вставить("ВыгруженоКартинок", 0);
			СтрокаТаблицыКаталогов.СтруктураРезультата.Вставить("ВыгруженоФайлов", 0);
	...
	Если Параметры.ОбменЗаказами Тогда
		ВыполнятьПолнуюВыгрузкуПринудительно = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОбмена, "ВыполнятьПолнуюВыгрузкуПринудительно");
		МассивИзменений = Новый Массив;
		Если ВыгружатьТолькоИзменения
			И НЕ ВыполнятьПолнуюВыгрузкуПринудительно Тогда
		...
		Если НЕ ВыгружатьТолькоИзменения 
			...
			ИЛИ ВыполнятьПолнуюВыгрузкуПринудительно Тогда
...
//  КодГруппы - Строка
//  УзелОбмена - ПланОбменаСсылка
//  РежимЗапускаОбмена - Строка - Режим запуска обмена
// 
// Возвращаемое значение:
...
// 
// Возвращаемое значение:
//  ОбъектXDTO - Получить заказы веб сервис
Функция ПолучитьЗаказыВебСервис(ДатаИзменения, УзелОбмена, РежимЗапускаОбмена = "") Экспорт
	...
	МассивИзменений = Новый Массив;
	Если Параметры.ВыгружатьТолькоИзменения
		И НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОбмена, "ВыполнятьПолнуюВыгрузкуПринудительно") Тогда
...
Функция ЗагрузитьЗаказыВебСервис(УзелОбмена, ДанныеЗаказовXDTO, РежимЗапускаОбмена="") Экспорт
	...
	СтрокаТаблицыИнформации.ДатаНачала = ТекущаяДатаСеанса();
...
// Загрузить запись на услуги HTTPСервис.
// 
// Параметры:
//  ДанныеЗаказов - ОбъектXDTO
//  КодУзлаОбмена - Строка
// 
// Возвращаемое значение:
//  Структура - Загрузить запись на услуги HTTPСервис:
// * УспешноЗагружено - Булево - 
...
// Загрузить запись на услуги.
// 
// Параметры:
//  ДанныеЗаказов - Строка, Неопределено, ОбъектXDTO - Данные заказов
//  СтруктураСтатистики - Структура - Структура статистики:
// * ОбработаноНаЗагрузке - Число - 
// * Загружено - Массив из Число 
// * Пропущено - Массив из Число 
// * Обновлено - Массив из Число 
// * Создано - Массив из Число
// * Выгружено - Массив из Число 
// * ОбработаноНаЗагрузкеОплат - Число - 
// * ЗагруженоОплат - Массив из Число 
// * ПропущеноОплат - Массив из Число 
// * ОбновленоОплат - Массив из Число 
// * СозданоОплат - Массив из Число 
//  Параметры - Структура - Параметры:
// * ОрганизацияВладелецКаталога - Дата, Произвольный, Неопределено - 
// * РазрешенныеТипыНоменклатуры - Массив из СправочникСсылка 
// * РазрешенныеТипыКартинок - Массив из Строка 
// * РазрешенныеТипыФайлов - Массив из Строка
// * ДатаФормирования - Дата - 
// * ИспользоватьХарактеристики - Булево - 
// * ИспользоватьНаборы - Булево - 
// * УчетНоменклатурыПоставщиков - Булево - 
// * УчетВалютныхОпераций - Булево - 
// * НаименованиеНалога - Строка - 
// * ОписаниеОшибки - Строка - 
// * ИндексФайлаОбмена - Число - 
// * НомерВВыборке - Число - 
// * РежимВыгрузкиУслуг - Булево - 
// 
// Возвращаемое значение:
//  Булево - Загрузить запись на услуги
Функция ЗагрузитьЗаписьНаУслуги(ДанныеЗаказов, СтруктураСтатистики, Параметры) Экспорт
	...
	Для Каждого СсылкаНаДокумент Из СтруктураСтатистики.Загружено Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации("ОбменССайтом.ЗагрузитьЗаписьНаУслуги", УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
			);
		КонецПопытки;
...
//	ВладелецКартинок - СправочникСсылка.Номенклатура
//	
// Возвращаемое значение:
...
//
// Параметры:
//	КлассификаторXDTO - объектXDTO - типа Классификатор
//	ПакетCML - пакетXDTO,
//	ВыборкаСвойствНоменклатуры - ВыборкаИзРезультатаЗапроса - содержащая свойства Номенклатуры.
//	Параметры - Структура
//
...
// Добавить группы классификатора XDTO.
// 
// Параметры:
//  КлассификаторXDTO - ОбъектXDTO - Классификатор XDTO
//  ДеревоГрупп - Массив из Строка
//  СписокГрупп - Неопределено - Список групп
//  ПакетCML - ПакетXDTO - Пакет CML
//  Параметры - Структура - Параметры:
// * ОрганизацияВладелецКаталога - СправочникСсылка.Организации - 
// * РазрешенныеТипыНоменклатуры - Массив из СправочникСсылка
// * РазрешенныеТипыКартинок - Массив из Строка
// * РазрешенныеТипыФайлов - Массив из Строка
// * ДатаФормирования - Дата - 
...
// * НомерВВыборке - Число - 
// * РежимВыгрузкиУслуг - Булево - 
// * ОбменЧерезВебСервис - Булево - 
// * РежимЗапускаОбмена - Строка - 
// * ВыгружатьТолькоИзменения - Булево - 
// * СоответствиеФайлов - Соответствие из Строка 
// * ПлатформаWindows - Булево - 
// * ТаблицаИнформации - ТаблицаЗначений - :
// ** Описание - Строка - 
...
// ** ТипСайта - Строка
// * ФайлЗагрузки - Строка - 
// * КаталогНаДиске - Строка - 
// * ПодкаталогФайлов - Строка - 
...
// Сформировать заказы XDTO версия commerce m l205a.
// 
// Параметры:
//  МассивИзменений - Массив из ДокументСсылка
//  СтруктураСтатистики - Структура
//  Параметры - Структура
// 
// Возвращаемое значение:
//  ОбъектXDTO - Сформировать заказы XDTO версия commerce m l205a
...
//  ТоварXDTO - ОбъектXDTO, Произвольный - Товар XDTO
//  ЭтоУслуга - Булево - Это услуга
//  Номенклатура - Произвольный, СправочникСсылка.Номенклатура, Неопределено - Номенклатура
//  Параметры - Структура - Параметры:
// * ОрганизацияВладелецКаталога - Дата, Произвольный, Неопределено - 
// * РазрешенныеТипыНоменклатуры - Массив из СправочникСсылка 
// * РазрешенныеТипыКартинок - Массив из Строка
...
// Найти контрагента по полям поиска.
// 
// Параметры:
//  НастройкиПоиска - Соответствие из Строка 
//  ИнформацияПоКонтрагенту - ТаблицаЗначений - Информация по контрагенту:
// * Вид - Строка - 
// * Представление - Строка - 
// * Тип - Строка
//  ВариантПоиска - Число- Вариант поиска
//  СоответствиеЛогов - Неопределено - Соответствие логов
// 
// Возвращаемое значение:
//  Неопределено, СправочникСсылка.Контрагенты, СправочникСсылка.Лиды - Найти контрагента по полям поиска
...
// 
// Параметры:
//  МассивИзменений - Массив из СправочникСсылка
//  СтруктураСтатистики - Структура - Структура статистики:
// * ОбработаноНаЗагрузке - Число - 
// * Загружено - Массив из Число 
// * Пропущено - Массив из Число
...
// 
// Возвращаемое значение:
//  Неопределено - Получить данные заказов
...
Функция ЗагрузитьЗаказы(ДанныеЗаказов, СтруктураСтатистики, Параметры, ОписаниеОшибки) Экспорт
	...
	Для Каждого СсылкаНаДокумент Из СтруктураСтатистики.Загружено Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации("ОбменССайтом.ЗагрузитьЗаказы", УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
			);
		КонецПопытки;
...
// Изменить отбор компоновщика.
// 
// Параметры:
//  КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - Компоновщик настроек
//  НастройкиСтруктураКаталога - Неопределено, СправочникСсылка.Номенклатура, Структура - Настройки структура каталога:
// * РучнаяНастройкаКаталога - Число - 
// * ОтборНоменклатураРучнаяНастройкаКаталога - Неопределено - 
//  МассивИзмененийНоменклатуры - Массив из СправочникСсылка - Массив изменений номенклатуры
...
// 
// Возвращаемое значение:
//  Неопределено, HTTPСоединение - Установить соединение с сервером
Функция УстановитьСоединениеССервером(ПараметрыПодключения, ОписаниеОшибки) Экспорт
	...
	Попытка
		...
		Если ПараметрыПодключения.ЗащищенноеСоединение Тогда
			...
			ССЛ = ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение();
...
// Выполнить авторизацию для соединения.
// 
// Параметры:
//  Соединение - Неопределено - Соединение
//  ПараметрыПодключения - Структура - Параметры подключения:
// * ИмяПользователя - Строка
// * Пароль - Строка 
// * АдресСайта - Строка
// * ТипСайта - Строка
//  ОтветСервера - Строка - Ответ сервера
//  ОписаниеОшибки - Строка - Описание ошибки
//  ТипСоединения - Строка - Тип соединения
// 
// Возвращаемое значение:
//  Булево - Выполнить авторизацию для соединения
...
// Добавить единицы измерения в классификатор XDTO.
// 
// Параметры:
//  КлассификаторXDTO - ОбъектXDTO - Классификатор XDTO
//  ПакетCML - ПакетXDTO - Пакет CML
//  ЕдиницыИзмерения - ВыборкаИзРезультатаЗапроса
//  Параметры - Структура - Параметры:
// * ОрганизацияВладелецКаталога - СправочникСсылка.Организации - 
// * РазрешенныеТипыНоменклатуры - Массив из СправочникСсылка 
// * РазрешенныеТипыКартинок - Массив из Строка 
...
//
// Параметры:
//	УзелПланаОбмена - ПланОбменаСсылка
...
Функция ЗагрузитьНастройкиБитрикс(УзелОбмена, ОписаниеОшибки) Экспорт
	...
	СвойстваУзлаОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УзелОбмена, "ИмяПользователя, АдресСайта, ПротоколОбменаCMS");
	
	НастройкиПодключения = Новый Структура;
	НастройкиПодключения.Вставить("ИмяПользователя", СвойстваУзлаОбмена.ИмяПользователя);
	НастройкиПодключения.Вставить("АдресСайта", СвойстваУзлаОбмена.АдресСайта);
	НастройкиПодключения.Вставить("ТипСайта", СвойстваУзлаОбмена.ПротоколОбменаCMS);
	УстановитьПривилегированныйРежим(Истина);
	ПарольИзХранилища = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(УзелОбмена, "Пароль");
	УстановитьПривилегированныйРежим(Ложь);
...
// 
// Возвращаемое значение:
//  Строка - Загрузить настройки UMI
...
// Поля поиска контрагентов.
// 
// Параметры:
//  ИскатьПоНаименованию - Булево - Искать по наименованию
// 
// Возвращаемое значение:
//  Массив из Строка
...
// Поля поиска контрагентов по умолчанию.
// 
// Параметры:
//  ИскатьПоНаименованию - Булево - Искать по наименованию
// 
...
// Настройки выгрузки реквизитов по умолчанию.
// 
// Возвращаемое значение:
//  Соответствие из Строка
...
// 
// Возвращаемое значение:
//  Строка - Надпись нет группы
Функция НадписьНетГруппы() Экспорт
	Возврат НСтр("ru = '<Нет группы>'");
КонецФункции

// Добавить категории классификатора XDTO.
// 
// Параметры:
//  КлассификаторXDTO - ОбъектXDTO - Классификатор XDTO
//  Категории - ВыборкаИзРезультатаЗапроса
//  ПакетCML - ПакетXDTO - Пакет CML
//  Параметры - Структура - Параметры:
// * ОрганизацияВладелецКаталога - СправочникСсылка.Организации - 
// * РазрешенныеТипыНоменклатуры - Массив из Строка
// * РазрешенныеТипыКартинок - Массив из Строка 
// * РазрешенныеТипыФайлов - Массив из Строка 
// * ДатаФормирования - Дата - 
// * ИспользоватьХарактеристики - Булево - 
...
// Добавить ресурсы классификатора XDTO.
// 
// Параметры:
//  КлассификаторXDTO - ОбъектXDTO
//  УслугиРесурсы - ВыборкаИзРезультатаЗапроса
//  ПакетCML - ПакетXDTO
//  Параметры - Структура
...
// Получить путь тома для платформы.
// 
// Параметры:
//  ПлатформаWindows - Булево
//  Том - Строка
...
//
// Возвращаемое значение:
// Массив из Число - размерностью равный массиву коэффициентов, содержит
...
Функция ФорматВалютыДляXDTO(Валюта) Экспорт
	...
	Если ТипЗнч(Валюта) = Тип("СправочникСсылка.Валюты") Тогда
		ТекстВалюты =ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Валюта, "Наименование");
	КонецЕсли;
...
// Получить контрагента XDTO.
// 
// Параметры:
//  ДанныеКонтрагента - ВыборкаИзРезультатаЗапроса
//  ПакетCML - ПакетXDTO 
// 
// Возвращаемое значение:
//  ОбъектXDTO
...
// Подготовить путь для платформы.
// 
// Параметры:
//  ПлатформаWindows - Булево 
//  Путь - Строка
...
//
// Возвращаемое значение:
//      Соответствие из КлючИЗначение - Список сокращений.
//
...
//  Структура - Результат операции:
// * ОрганизацияВладелецКаталога - СправочникСсылка.Организации. 
// * РазрешенныеТипыНоменклатуры - Массив. 
// * РазрешенныеТипыКартинок - Массив. 
// * РазрешенныеТипыФайлов - Массив. 
// * ДатаФормирования - Дата. 
// * ИспользоватьХарактеристики - Булево.  
// * ИспользоватьНаборы - Булево.  
// * УчетНоменклатурыПоставщиков - Булево.  
...
Функция ПолучитьСтруктуруОсновныхПараметровОбмена() Экспорт
	...
	ОсновнаяОрганизация = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнаяОрганизация", , , ОрганизацияПоУмолчанию);
...
// Чтение JSONВСтруктуру.
// 
// Параметры:
//  СтруктураВСтроке - Структура
//  ВернутьСоответствие - Булево
// 
// Возвращаемое значение:
//  Произвольный
...
// Получить префикс для заказа с сайта.
// 
// Возвращаемое значение:
//  Строка
...
// Получить префикс по умолчанию для заказа с сайта.
// 
// Возвращаемое значение:
//  Строка
...
//
// Параметры:
//	ИмяПараметра			- Строка - имена параметров сеанса
...
//
// Возвращаемое значение:
//	Массив из ДокументСсылка- заказов покупателей.
//
...
Процедура ЗаполнитьГруппыКлассификатораXDTO(ГруппыКлассификатораXDTO, ДеревоГрупп, СписокГрупп, ПакетCML, Параметры, ТекущийРодитель = Неопределено)
	...
	Для Каждого СтрокаДерева Из ДеревоГрупп Цикл
		...
		Если СписокГрупп <> Неопределено И СписокГрупп.Количество() > 0 
			...
			Если СтрокаДерева.Строки.Количество() > 0 Тогда
				ЗаполнитьГруппыКлассификатораXDTO(ГруппыКлассификатораXDTO, СтрокаДерева.Строки, СписокГрупп, ПакетCML, Параметры, СтрокаДерева.Номенклатура);
			КонецЕсли;
		...
		Иначе
			...
			ГруппыXDTO = ПолучитьГруппыНоменклатурыXDTO(СтрокаДерева.Строки, СписокГрупп, ПакетCML, Параметры, СтрокаДерева.Номенклатура);
			Если ГруппыXDTO.Группа.Количество() > 0 Тогда
				ГруппаXDTO.Группы = ГруппыXDTO;
			КонецЕсли;
...
//	СписокГрупп - СписокЗначений из СправочникСсылка.Номенклатура - группы выгружаемого каталога
//	ПакетCML - ПакетXDTO
//	Параметры - Структура
...
//
Функция ПолучитьГруппыНоменклатурыXDTO(ДеревоГрупп, СписокГрупп, ПакетCML, Параметры, ТекущийРодитель = Неопределено)
...
//	Параметры - Структура - содержащая параметры обмена
//	ОписанияФайлов - массив из КлючИЗначение - ключом которого является пути к файлу, а значением описание файла.
//	ОписанияКартинок - Массив из КлючИЗначение
//
...
//
// Параметры:
//	Параметры - Структура - содержащая Выборкау файлов и Выборкау свойств
//
// Возвращаемое значение
//	Структура.
//
...
Процедура ДобавитьРеквизитыТовараДляПредложенияXDTO(ПредложениеXDTO, ПакетCML, Характеристика, Параметры)
	...
	Если Не Параметры.ОбменЧерезВебСервис Тогда
		Если ЗначениеЗаполнено(Характеристика) Тогда
			Наименование = Наименование + " (" + Строка(Характеристика) + ")";
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.ИспользоватьХарактеристики
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Иначе
				...
				ХарактеристикаТовараXDTO.Наименование = ФорматНаименованияДляXDTO(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Характеристика, "Наименование"));
...
Процедура СоздатьОбновитьДокументОплаты(ЗаказСсылка, СвойстваОплаты, Параметры, СтруктураСтатистики, ОписаниеОшибки)
		...
		ИначеЕсли СвойстваОплаты.Получить("Оплачено") = "false" Тогда
			...
			Если ОперацияПоПлатежнымКартам.Проведен Тогда
				...
				Исключение
					...
					ОписаниеИсключительнойОшибки(ПодробноеПредставлениеОшибки, НСтр("ru = 'Не удалось отменить проведение ОперацияПоПлатежнымКартам к заказу №'") + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаказСсылка, "Номер"));
					ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекстОшибки);
				КонецПопытки;
			КонецЕсли; 
	...
	СвойстваЗаказСсылка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаказСсылка, "СуммаДокумента, СтоимостьДоставки, ИнформацияПоКонтрагенту, Автор");
	...
	Если СвойстваОплаты.Получить("ПодчиненныйДокумент") = Истина Тогда
		...
		Если ЗначениеЗаполнено(СуммаОплатыЗначение) Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации("ОбменССайтом.СоздатьОбновитьДокументОплаты", УровеньЖурналаРегистрации.Ошибка,,,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
				);
			КонецПопытки;
		КонецЕсли;
	ИначеЕсли НЕ Параметры.ПодчиненныеДокументы И НРег(СвойстваОплаты.Получить("Заказ оплачен")) <> "true" Тогда
		...
		Возврат;
	ИначеЕсли СвойстваЗаказСсылка.СуммаДокумента + СвойстваЗаказСсылка.СтоимостьДоставки = 0 Тогда
	...
	ДокОперацияПоПлатежнымКартам.КассаККМ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокОперацияПоПлатежнымКартам.ЭквайринговыйТерминал, "Касса");
	ДокОперацияПоПлатежнымКартам.Статус = Перечисления.СтатусыЧековККМ.ЧекНеПробит;
	
	// Телефон и email для отправки чека
	СтрокаТЧТелефон = СвойстваЗаказСсылка.ИнформацияПоКонтрагенту.Найти("Телефон","Вид");
	Если СтрокаТЧТелефон=Неопределено Тогда
		СтрокаТЧТелефон = СвойстваЗаказСсылка.ИнформацияПоКонтрагенту.Найти(НРег("ТелефонМобильный"),"Вид");
	КонецЕсли;
	...
	СтрокаТЧEMail = СвойстваЗаказСсылка.ИнформацияПоКонтрагенту.Найти("email","Вид");
	Если СтрокаТЧEMail=Неопределено Тогда
		СтрокаТЧEMail = СвойстваЗаказСсылка.ИнформацияПоКонтрагенту.Найти("E-mail","Вид");
	КонецЕсли;
	Если СтрокаТЧEMail=Неопределено Тогда
		СтрокаТЧEMail = СвойстваЗаказСсылка.ИнформацияПоКонтрагенту.Найти("Адрес электронной почты","Вид");
	КонецЕсли;
	Если СтрокаТЧEMail=Неопределено Тогда
		СтрокаТЧEMail = СвойстваЗаказСсылка.ИнформацияПоКонтрагенту.Найти("АдресЭП","Вид");
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ДокОперацияПоПлатежнымКартам.ЭквайринговыйТерминал) Тогда
		...
		Если НастройкаПлатежныйТерминал<>Неопределено Тогда
			...
			ДокОперацияПоПлатежнымКартам.ЭквайринговыйТерминал = НастройкаПлатежныйТерминал.АналитикаОплаты;
			Если ЗначениеЗаполнено(ДокОперацияПоПлатежнымКартам.ЭквайринговыйТерминал) Тогда
				ДокОперацияПоПлатежнымКартам.КассаККМ = НастройкаПлатежныйТерминал.АналитикаОплаты.Касса;
			КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ДокОперацияПоПлатежнымКартам.Автор) Тогда
		Если ЗначениеЗаполнено(СвойстваЗаказСсылка.Автор) Тогда
			ДокОперацияПоПлатежнымКартам.Автор = СвойстваЗаказСсылка.Автор;
		Иначе
	...
	Исключение
		...
		ТекстОшибки = ОписаниеОшибки() + Символы.ПС 
		+ ОписаниеИсключительнойОшибки(ПодробноеПредставлениеОшибки, НСтр("ru = 'Не удалось записать ОперацияПоПлатежнымКартам к заказу №'") + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаказСсылка, "Номер"));
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекстОшибки);
	КонецПопытки;
...
//
// Параметры:
//	МассивНомеров - Массив из ДокументСсылка - номера загружаемых документов
//
// Возвращаемое значение
//	Результат запроса.
//
...
Функция СоздатьОбновитьЗаказы(ДокументыXDTO, РанееЗагруженныеЗаказы, СтруктураСтатистики, Параметры, ОписаниеОшибки)
	...
	ТаблицаПВХ = РезультатыЗапроса[0].Выгрузить();
	ТаблицаНаборы = РезультатыЗапроса[1].Выгрузить();
	ТаблицаВидовЗаказов = РезультатыЗапроса[2].Выгрузить();
	ТаблицаСостоянийЗаказов = РезультатыЗапроса[3].Выгрузить();	
	...
	Для каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Если НЕ Параметры.ОбменЧерезВебСервис Тогда
			УстановитьСостояниеЗаказа(ДокументОбъект, СвойстваЗаказа, Параметры.ТаблицаСоответствияСтатусовЗаказов, ТаблицаВидовЗаказов, ТаблицаСостоянийЗаказов, Параметры);
		КонецЕсли;
		...
		Если ЕстьСсылкиНаДокументыОплатыОтгрузки Тогда
			...
			Если ОбновитьСоздатьДополнительныеСведенияЗаказа(ДокументОбъект.Ссылка, СвойстваЗаказа, ОписаниеОшибки, ТаблицаПВХ, ТаблицаНаборы) Тогда
		...
		Иначе
			...
			ОбновитьСоздатьДополнительныеСведенияЗаказа(ДокументОбъект.Ссылка, СвойстваЗаказа, ОписаниеОшибки, ТаблицаПВХ, ТаблицаНаборы);
...
Функция СоздатьОбновитьЗаказНаряды(ДокументыXDTO, РанееЗагруженныеЗаказы, СтруктураСтатистики, Параметры)
	...
	Для каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Если ЕстьСсылкиНаДокументыОплатыОтгрузки Тогда
			...
			Если ОбновитьСоздатьДополнительныеСведенияЗаказа(ДокументОбъект.Ссылка, СвойстваСобытия, ОписаниеОшибки, ТаблицаПВХ, ТаблицаНаборы) Тогда
		...
		Иначе
			...
			Успешно = ЗаполнитьЗаказНарядДаннымиДокументаXDTO(ДокументОбъект, ДокументXDTO, СвойстваСобытия, Параметры, ТаблицаВидовЗаказов, ТаблицаСостоянийЗаказов);
			...
			ОбновитьСоздатьДополнительныеСведенияЗаказа(ДокументОбъект.Ссылка, СвойстваСобытия, ОписаниеОшибки, ТаблицаПВХ, ТаблицаНаборы);
			СтруктураСтатистики.Загружено.Добавить(ДокументОбъект.Ссылка);
...
Процедура ВыполнитьДействияПередЗаписьюЗаказа(ДокументОбъект, Параметры)
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Контрагент)
		И НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Контрагент, "ВестиРасчетыПоДоговорам")
...
Процедура УдалитьДвиженияУДокумента(ДокументОбъект)
	...
	Для Каждого СтрокаДвижения Из МассивОбрабатываемыхСтрокТаблицыДвижений Цикл		
		...
		Исключение
			...
			ВызватьИсключение "Операция не выполнена: " + СтрокаДвижения.Имя + Символы.ПС + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;
	КонецЦикла;
...
//	СвойстваЗаказа - Соответствие из КлючИЗначение - свойства и значения загружаемого заказа
//	ОписаниеОшибки - Строка - информация по ошибкам
//	ТаблицаПВХ - ТаблицаЗначений - таблица, содержащая элементы плана видов характеристик Дополнительные реквизиты
//	ТаблицаНаборы - ТаблицаЗначений - таблица, содержащая элементы плана видов характеристик Наборы дополнительных реквизитов
//
// Возвращаемого значение
//	Булево - Истина, если в процессе обновления/создания дополнительных сведений не возникло ошибок.
//
Функция ОбновитьСоздатьДополнительныеСведенияЗаказа(ЗаказПокупателя, СвойстваЗаказа, ОписаниеОшибки, ТаблицаПВХ, ТаблицаНаборы)
	...
	Для Каждого СвойствоЗаказа Из МассивСоздатьОбновить Цикл
		...
		СтруктураПоиска = Новый Структура("Наименование", СвойствоСтрокой);
		СтрокиПВХ = ТаблицаПВХ.НайтиСтроки(СтруктураПоиска);		
		
		Свойство = Неопределено;
		ТипЗначенияСвойства = Тип("Строка");

		Для Каждого Выборка Из СтрокиПВХ Цикл
		...
		Если Свойство = Неопределено Тогда
			...
			СтрокаПВХ = ТаблицаПВХ.Добавить();
			СтрокаПВХ.Наименование = СвойствоСтрокой; 
			СтрокаПВХ.ТипЗначения = СвойствоОбъект.ТипЗначения; 
			СтрокаПВХ.Ссылка = Свойство; 
			
		КонецЕсли;
		
		СтруктураПоиска = Новый Структура("Свойство", Свойство);
		СтрокиНаборы = ТаблицаНаборы.НайтиСтроки(СтруктураПоиска);	
		...
		Если СтрокиНаборы.Количество() = 0 Тогда
			...
			СтрокаНаборы = ТаблицаНаборы.Добавить();
			СтрокаНаборы.Свойство = СвойствоСтрокой; 
...
Функция ОбновитьСоздатьДополнительныеСведенияОперацияПоПлатежнымКартам(ОперацияПоПлатежнымКартам, СвойстваЗаказа, ОписаниеОшибки)
	...
	ТаблицаПВХ = ЗапросПВХ.Выполнить().Выгрузить();
	...
	ТаблицаНаборы = ЗапросНаборы.Выполнить().Выгрузить();
...
Функция ЗаполнитьЗаказДаннымиДокументаXDTO(ДокументОбъект, ДокументXDTO, СвойстваЗаказа, Параметры, ОписаниеОшибки)
	...
	Если ЗначениеЗаполнено(Склад) Тогда
		...
		Ответственный = ПолучитьМОСотрудника(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "МОЛ"));
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Организация) Тогда
		СвойстваОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОбъект.Организация, "КассаПоУмолчанию, БанковскийСчетПоУмолчанию");
		ДокументОбъект.БанковскийСчет =  СвойстваОрганизации.БанковскийСчетПоУмолчанию;
		ДокументОбъект.Касса = СвойстваОрганизации.КассаПоУмолчанию;
	Иначе
		ДокументОбъект.Касса =ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.ОрганизацияВладелецКаталога, "КассаПоУмолчанию");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Контрагент)
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Контрагент, "ВестиРасчетыПоДоговорам") Тогда
...
Функция ЗаполнитьЗаказНарядДаннымиДокументаXDTO(ДокументОбъект, ДокументXDTO, СвойстваЗаказа, Параметры, ТаблицаВидовЗаказов, ТаблицаСостоянийЗаказов)
	...
	ПорядокСостояний = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.ВидЗаказа, "ПорядокСостояний");
	...
	ИначеЕсли ПорядокСостояний.Количество() > 0 Тогда
		ДокументОбъект.СостояниеЗаказа =ПорядокСостояний[0].Состояние;
	КонецЕсли;
...
Функция ПроверитьИмяСвойстваКакРеквизит(ИмяРеквизита, РазрешенныеСимволыИмениРеквизита)
	...
	Счетчик = 1;
	Пока Счетчик <= СтрДлина(ИмяРеквизита) Цикл
		ПроверяемыйСимвол = Сред(ИмяРеквизита, Счетчик, 1);
...
Функция НайтиКонтрагентаПоИД(ИдКонтрагента)
	...
	Если ИдКонтрагента<>"" И СтрДлина(ИдКонтрагента)=СтрДлина(Новый УникальныйИдентификатор) Тогда
		Попытка
			...
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации("ОбменССайтом.НайтиКонтрагентаПоИД", УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
			);
		КонецПопытки;
	КонецЕсли;
...
Процедура СоздатьЗаполнитьКонтактноеЛицоКонтрагента(КонтрагентОбъект, КонтрагентXDTO, НаименованиеКонтрагента = "", КонтактноеЛицоСсылка = Неопределено)
...
Процедура ОбновитьКонтактнуюИнформациюКонтрагента(КонтрагентОбъект, КонтрагентXDTO, Замещать = Ложь)
	...
	Если НЕ АдресXDTO = Неопределено 
		...
		Если ЗамещатьАдрес = Ложь И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидКИ, "ХранитьИсториюИзменений") Тогда
...
//	СвойстваЗаказа - Соответствие из КлючИЗначение - свойства документа
//	Параметры - Структура - параметры обмена
//	ОписаниеОшибки - Строка - информация по ошибкам
//
// Возвращаемое значение:
...
//	СвойстваЗаказа - Соответствие из КлючИЗначение - свойства загружаемого заказа
...
//	ТаблицаВидовЗаказов - ТаблицаЗначений - таблица, содержащая виды заказов из справочника
//	ТаблицаСостоянийЗаказов -  ТаблицаЗначений - содержащая состояние заказов из справочника
//	Параметры - Структура - дополнительные параметры
//
Процедура УстановитьСостояниеЗаказа(ДокументОбъект, СвойстваЗаказа, ТаблицаСоответствияСтатусовЗаказов, ТаблицаВидовЗаказов, ТаблицаСостоянийЗаказов, Параметры)
	...
	Иначе
		...
		Если ВидЗаказа <> Неопределено
			...
			Выборка = ТаблицаВидовЗаказов.Найти(ВидЗаказа, "Наименование");
		...
		Если СтатусЗаказа <> Неопределено
			...
			Выборка = ТаблицаВидовЗаказов.Найти(СтатусЗаказа, "Наименование");
			Если Выборка <> Неопределено Тогда
...
//
// Параметры:
//	МассивЗаказов - Массив из ДокументСсылка.ЗаказПокупателя - содержащий ссылки на заказы.
//
// Возвращаемое значение
	...
	Пока Параметры.ВыполнятьОбменЗаказы Цикл
		...
		Если Параметры.ВыгружатьНаСайт Тогда
			...
			Для номерПопытки=1 По ВыгружатьРаз Цикл
				...
				Успешно = ВыгрузитьНаСайт(Параметры, Неопределено , ОписаниеОшибки, ВыгружатьПошагово, "sale");
...
//
// Параметры:
//	МассивИзменений - Массив из ДокументСсылка - заказы, зарегистрированными по узлу плана обмена
//	МассивЗагруженныхДокументов - Массив из ДокументСсылка - только что загруженных заказов
//	Параметры - Структура
//	
// Возвращаемое значение:
...
//	МассивОграничения - Массив из Строка - значения отбора
...
//	ВыборкаДокументов - ВыборкаИзРезультатаЗапроса - содержащая данные по заказам покупателей
...
// 	ПакетCML - ПакетXDTO
//	ТоварXDTO - объектXDTO - типа Товар
//	ТоварТип - объектXDTO - тип объекта Товар
//	Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры
//	ДеревоСвойствХарактеристик - ДеревоЗначений - содержащее свойства характеристик.
//	Параметры - Структура
//
...
//	ДокументXDTO - объектXDTO - типа Документ
//	ПакетCML - пакетXDTO - содержащий типы CML
//	ДанныеЗаказа - ВыборкаИзРезультатаЗапроса - содержащая данные заказа покупателя
//	Параметры - Структура - содержащая параметры обмена.
//
Процедура ДобавитьЗначенияРеквизитовЗаказаXDTO(ДокументXDTO, ПакетCML, ДанныеЗаказа, Параметры)
	...
	Если Параметры.Свойство("НастройкиВыгрузкиДопРеквизитовЗаказов") И НЕ ПустаяСтрока(Параметры.НастройкиВыгрузкиДопРеквизитовЗаказов) Тогда
		...
		Для каждого ДанныеРеквизита Из ДопРеквизитыЗаказа Цикл
			...
			КлючРеквизита = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеРеквизита.Свойство, "ИдентификаторДляФормул");
			ИмяВыгрузкиРеквизита = СоответствиеПолейДопРеквизитов.Получить(КлючРеквизита);
...
Процедура ЗаписатьКИ(КонтрагентОбъект, ПредставлениеКИ, ВидКИ, ТипКИ, Замещать)
	...
	Если Замещать Тогда
		...
		КонецЕсли;
	ИначеЕсли НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидКИ, "РазрешитьВводНесколькихЗначений") Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации("ОбменССайтом.ЗаписатьКИ", УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
			);			
		КонецПопытки;
	КонецЕсли;
...
Функция ПараметрыДляУслуг(Параметры)
	...
	СтруктураРезультата.Вставить("ВыгруженоФайлов", 0);
...
// Параметры:
//	Параметры								- Структура - основные параметры
//	МассивПодкаталогов						- Массив из Строка
//	ОписаниеОшибки							- Строка
...
//
Функция ОтправитьФайлНаСервер(ПолноеИмяФайла, Соединение, ОписаниеОшибки, ПараметрыЗапроса="", Заголовки="", Параметры = Неопределено)
...
//
// Параметры:
//	Объект		- ОбъектМетаданных - источник события
...
Процедура ЗарегистрироватьИзменения(Объект, Замещение = Ложь)
	...
	Если ТипОбъекта = Тип("РегистрНакопленияНаборЗаписей.ЗапасыНаСкладах")
		...
		Если Замещение = Истина Или Замещение = РежимЗамещения.Замещение Тогда
		...
		Если МассивУзловТовары.Количество() > 0 Тогда
			Для каждого Запись Из ТаблицаОбластиБлокировки Цикл
				Если ТипЗнч(Запись.Номенклатура) = ТипНоменклатура
...
Процедура УдалитьСписокВременныхФайлов(СписокФайлов)
	...
	Для каждого ТекФайл Из СписокФайлов Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации("ОбменССайтом.УдалитьСписокВременныхФайлов", УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
			);
		КонецПопытки;
...
Процедура ДобавитьНастройкиУзлаВПараметры(УзелОбмена, Параметры)
	...
	Если Параметры.ОбменЗаписьНаУслуги Тогда
		...
		Параметры.Вставить("ПроизводственныйКалендарь", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.ОрганизацияВладелецКаталога, "ПроизводственныйКалендарь"));
...
//
// Параметры:
//	ТипОбъектаXDTO - ТипОбъектаXDTO - от которого требуется получить тип свойства
...
Функция ТипЗначенияСправочник(Тип)
	...
	Исключение
		ЗаписьЖурналаРегистрации("ОбменССайтом.ТипЗначенияСправочник", УровеньЖурналаРегистрации.Ошибка,,,
		ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
		);
	КонецПопытки;
...
//
// Параметры: 
//	Документ - ДокументОбъект.ЗаказПокупателя.
//	Товары - Массив из Строка - Табличная часть
...
Функция ПолучитьЧислоИзСтроки(ЗначениеСтрока)
	...
	Исключение
		ЗаписьЖурналаРегистрации("ОбменССайтом.ПолучитьЧислоИзСтроки", УровеньЖурналаРегистрации.Ошибка,,,
		ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
		);	
	КонецПопытки;
...
Функция ПолучитьДатуВремяИзСтроки(ДатаСтрока, ВремяСтрока = "")
	...
	Исключение
		ЗаписьЖурналаРегистрации("ОбменССайтом.ПолучитьДатуВремяИзСтроки", УровеньЖурналаРегистрации.Ошибка,,,
		ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
		);	
	КонецПопытки;
...
Функция КодировкаФайла(ИмяФайла)
	...
	Исключение
		ЗаписьЖурналаРегистрации("ОбменССайтом.КодировкаФайла", УровеньЖурналаРегистрации.Ошибка,,,
		ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
		);
	КонецПопытки;
	ЧтениеXML.Закрыть();
...
Функция ПреобразоватьВСтрокуИзJSON(Строка)
	...
	Попытка
		...
		ЧтениеJSON.Закрыть();
	Исключение
		ЗаписьЖурналаРегистрации("ОбменССайтом.ПреобразоватьВСтрокуИзJSON", УровеньЖурналаРегистрации.Ошибка,,,
		ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
		);	
	КонецПопытки;
...
Функция ЗаполнитьИДСайтаДляОбъектаXDTO(ОбъектXDTO, ВыгружаемаяСсылка, Характеристика=Неопределено, Параметры=Неопределено, ИмяПоляИд="Ид")
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Контакты") И КонтрагентXDTO.Контакты <> Неопределено Тогда
		Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO.Контакты, "Контакт") 
			...
			Для каждого КонтактXDTO Из МассивКонтактовXDTO Цикл
				...
				ДобавитьСтрокуИнформацииПоКонтрагенту(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураКИ.Вид, "Наименование"), СтруктураКИ.Тип, КонтактXDTO.Значение, ИнформацияПоКонтрагенту);
...
//	ТипКИ - Строка - тип информации,
//	Представление - Строка - представление информации
//	ИнформацияПоКонтрагенту - ТабличнаяЧасть - документа ЗаказПокупателя.
//
