...
#Если Не ВнешнееСоединение Тогда
	Если ПравоДоступа("Изменение", Метаданные.Справочники.Контрагенты)
		...
		И Не ПерсональныеНастройкиРУНФ.Прочитать("ПредложениеИспользованияСистемыВзаимодействияДляТелефонииПоказано") Тогда
...
Функция АдресОбратногоВызоваОблачнаяТелефония(ИспользуемаяАТС = Неопределено) Экспорт
	...
	Если ЕстьНастройкиПубликации Тогда
		...
		Если Перечисления.ДоступныеАТС.ЭтоПлатформаItoolabs(ИспользуемаяАТС) Тогда
			Если Обработки.ОбщиеНастройкиТелефонии.ОбщиеНастройкиПубликацииСервисаАТСItoolabsХранятсяВУниверсальномВиджете(ИспользуемаяАТС) Тогда
				НастройкиПубликацииИнтеграции = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбщиеНастройкиТелефонии, "НастройкиПубликации", Новый Соответствие).Получить(Перечисления.ДоступныеАТС.УниверсальныйItoolabs);
			Иначе
...
//
Процедура ПроверитьАдресПубликацииСервисаТелефонии(HTTPОтвет = Неопределено, АдресПубликации = "") Экспорт
...
Функция ПолучитьСотрудникаПоПользователю(Пользователь)
	...
	Возврат ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойОтветственный", , Пользователь);
