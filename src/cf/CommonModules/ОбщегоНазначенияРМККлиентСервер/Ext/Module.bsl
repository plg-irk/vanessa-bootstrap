...
Процедура ПересчитатьСуммуВСтроке(Объект, ТекущаяСтрока, СкидкиРассчитаны = Ложь,
	...
	ФорматКорзиныПлиточногоИнтерфейса = Объект.ФорматКорзиныПлиточногоИнтерфейса;
	Если ФорматКорзиныПлиточногоИнтерфейса = 1 Или ФорматКорзиныПлиточногоИнтерфейса = 2 Тогда
		СимволРазделитель = ?(ФорматКорзиныПлиточногоИнтерфейса = 1, Символ(160), Символы.ПС);
		ТекущаяСтрока.ТекстКоличествоСумма = СтрШаблон("%1%2=%3",
				ТекущаяСтрока.Количество, СимволРазделитель, ТекущаяСтрока.Сумма);
	ИначеЕсли ФорматКорзиныПлиточногоИнтерфейса = 3 Или ФорматКорзиныПлиточногоИнтерфейса = 4 Тогда
		Если ТекущаяСтрока.СуммаСкидокОбщая = 0 Тогда
			ТекущаяСтрока.ТекстКоличествоСумма = СтрШаблон("%1 x %2 =%3",
				ТекущаяСтрока.Цена, ТекущаяСтрока.Количество, ТекущаяСтрока.Сумма);
		Иначе
			ЗначениеСкидки = ТекущаяСтрока.СуммаСкидокОбщая;
			ЗнакСкидки = ?(ЗначениеСкидки > 0, "-", "+");
			Если ФорматКорзиныПлиточногоИнтерфейса = 3 Тогда
				ЗначениеСкидки = "%";
			Иначе
				ЗначениеСкидки = ?(ЗначениеСкидки > 0, ЗначениеСкидки, -ЗначениеСкидки);
			КонецЕсли;
			
			ТекущаяСтрока.ТекстКоличествоСумма = СтрШаблон("%1 x %2= (%3%4) =%5",
				ТекущаяСтрока.Цена, ТекущаяСтрока.Количество, ЗнакСкидки, ЗначениеСкидки, ТекущаяСтрока.Сумма);
		КонецЕсли;
	Иначе
		ТекущаяСтрока.ТекстКоличествоСумма = "";
	КонецЕсли;
...
// Определяет является ли особенность учета при частичном выбытии указываемая мерным количеством, а не дрбным.
//
// Параметры:
//  ОсобенностьУчета - ОпределяемыйТип.ОсобенностиУчетаНоменклатурыРМК - особенность учета.
//
...
//
Функция ЧастичноеВыбытиеУказываетсяМернымКоличеством(ОсобенностьУчета) Экспорт
	
	Результат = Ложь;
	ОбщегоНазначенияРМККлиентСерверПереопределяемый.ОпределитьЧастичноеВыбытиеУказываетсяМернымКоличеством(ОсобенностьУчета, Результат);
