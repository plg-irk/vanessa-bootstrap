...
Функция ТекстНадписиЦеныИВалюта(Знач Объект, Знач ВалютаРасчетов, Знач КурсНациональнаяВалюта,
	...
	Если ЗначениеЗаполнено(ПоляНадписи.НалогообложениеНДС) Тогда
		Если ПоляНадписи.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС") 
		Тогда
			Если ПоляНадписи.СуммаВключаетНДС Тогда
				СоставНадписи.Добавить(НСтр("ru = 'НДС в сумме'"));
			Иначе		
				СоставНадписи.Добавить(НСтр("ru = 'НДС сверху'"));
			КонецЕсли;
		Иначе
