...
Процедура ОтправитьСобытиеGA(ПараметрыСобытия) Экспорт
	...
	Попытка
		...
		HTTPСоединение.ПолучитьАсинх(HTTPЗапрос);
	Исключение
		// Сбор статистики
		ПолноеОписание = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ОтправитьСобытие(ИмяСобытия) Экспорт
	...
	Попытка
		ИнтернетПрокси = ПолучениеФайловИзИнтернета.ПолучитьПрокси("HTTP");
		HTTPСоединение = Новый HTTPСоединение("unf-stat1c-gpt-msk.1c.ru", 80,,,ИнтернетПрокси, 5);
