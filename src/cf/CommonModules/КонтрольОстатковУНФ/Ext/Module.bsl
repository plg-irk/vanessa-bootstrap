...
Процедура ЗапасыНаСкладахСписком(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ШаблонСообщения = НСтр("ru = '%НомерСтроки% Номенклатура: %НоменклатураХарактеристикаПартия%,
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.ОтчетОРозничныхПродажах") Тогда
			НомерСтроки = ОпределитьНомерСтрокиПоВыборке(ВыборкаЗаписей, ДокОбъект);
			Если НомерСтроки <> Неопределено Тогда
				Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Запасы", НомерСтроки, "Номенклатура");
			Иначе
				Поле = "Запасы";
			КонецЕсли;
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", СтрШаблон(НСтр("ru = 'Строка №%1:'"), НомерСтроки));
			Если ВыборкаЗаписей.КонтрольОстатков Тогда
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокОбъект, Поле, "Объект", Отказ);
			Иначе
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокОбъект, Поле, "Объект");
			КонецЕсли;
		Иначе
...
// Сообщает об ошибках проведения по регистру НДС авансы по договорам комиссии.
//
// Параметры:
//  ДокОбъект - ДокументОбъект - проводимый документ,
//  ВыборкаЗаписей - ВыборкаИзРезультатаЗапроса - выборка из результата запроса контроля остатков,
//  Отказ - Булево - признак наличия ошибок.
Процедура НДСАвансыПоДоговорамКомиссии(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	
	ШаблонСообщения = НСтр("ru = 'Ошибка: Отрицательный остаток зачета аванса по договорам комиссии.
	|Остаток суммы без НДС: %ОстатокСуммы%. Покупатель: %Покупатель%. Дата события: %ДатаСобытия%'");
	
	Пока ВыборкаЗаписей.Следующий() Цикл
		
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%ОстатокСуммы%", ВыборкаЗаписей.СуммаБезНДСОстатокНДСАвансыПоДоговорамКомиссии); 
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Покупатель%", СокрЛП(ВыборкаЗаписей.ПокупательПредставление)); 
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДатаСобытия%", СокрЛП(ВыборкаЗаписей.ДатаСобытияПредставление));
...
Процедура ДенежныеСредстваСУчетомРезервов(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ПроведениеСверхРезерваРазрешено = Пользователи.РолиДоступны("ПроведениеДенежныхДокументовСверхРезерва", , Ложь);
...
Процедура ПодарочныеСертификаты(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ШаблонСообщения = НСтр("ru = '%НомерСтроки% Вид сертификата: %ПодарочныйСертификат%,
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.ОтчетОРозничныхПродажах") Тогда
			НомерСтроки = ОпределитьНомерСтрокиПоВыборке(ВыборкаЗаписей, ДокОбъект);
			Если НомерСтроки <> Неопределено Тогда
				Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("БезналичнаяОплата", НомерСтроки, "ВидОплаты");
			Иначе
				Поле = "БезналичнаяОплата";
			КонецЕсли;
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", СтрШаблон(НСтр("ru = 'Строка №%1:'"), НомерСтроки));
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокОбъект, Поле, "Объект", Отказ);
		Иначе
...
Процедура СерииНоменклатуры(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.ОтчетОРозничныхПродажах")
			И (Константы.КонтролироватьОстаткиПриПробитииЧековККМ.Получить() Или Не ВыборкаЗаписей.КонтрольОстатков) Тогда
			ЗаголовокОшибки = НСтр("ru = 'Внимание:'");
		Иначе
			ЗаголовокОшибки = НСтр("ru = 'Ошибка:'");
		КонецЕсли;
...
//                      * СерияПредставление - Строка.
//  Отказ			 - Булево - признак наличия ошибок.
Процедура СерииНоменклатурыКРасходуПоДокументам(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
...
//                      * СерияПредставление - Строка.
//  Отказ			 - Булево - признак наличия ошибок.
Процедура СерииНоменклатурыКПоступлению(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
...
Функция ОпределитьНомерСтрокиПоВыборке(ВыборкаЗаписей, ДокОбъект)
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаЗаписей, "СерияПредставление") Тогда
		
		Отбор = Новый Структура("Серия", ВыборкаЗаписей.СерияПредставление);
		МассивСтрок = ДокОбъект.СерииНоменклатуры.НайтиСтроки(Отбор);
		Если МассивСтрок.Количество() Тогда
			НомерСтроки = МассивСтрок[0].КлючСвязи;
		Иначе
			НомерСтроки = Неопределено;
		КонецЕсли;
		
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаЗаписей, "ПодарочныйСертификат") Тогда
		
		Отбор = Новый Структура;
		Отбор.Вставить("ПодарочныйСертификат", ВыборкаЗаписей.ПодарочныйСертификат);
		Отбор.Вставить("НомерСертификата", ВыборкаЗаписей.НомерСертификата);
		МассивСтрок = ДокОбъект.БезналичнаяОплата.НайтиСтроки(Отбор);
		Если МассивСтрок.Количество() Тогда
			НомерСтроки = МассивСтрок[0].НомерСтроки;
		Иначе
			НомерСтроки = Неопределено;
		КонецЕсли;
	...
	Иначе
		...
		Отбор = Новый Структура;
		Отбор.Вставить("Номенклатура", ВыборкаЗаписей.НоменклатураПредставление);
		Отбор.Вставить("Характеристика", ВыборкаЗаписей.ХарактеристикаПредставление);
		Отбор.Вставить("ЕдиницаИзмерения", ВыборкаЗаписей.ЕдиницаИзмеренияПредставление);
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаЗаписей, "СтруктурнаяЕдиницаПредставление") Тогда
			Отбор.Вставить("СтруктурнаяЕдиница", ВыборкаЗаписей.СтруктурнаяЕдиницаПредставление);
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаЗаписей, "ПартияПредставление") Тогда
			Отбор.Вставить("Партия", ВыборкаЗаписей.ПартияПредставление);
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаЗаписей, "ЯчейкаПредставление") Тогда
			Отбор.Вставить("Ячейка", ВыборкаЗаписей.ЯчейкаПредставление);
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаЗаписей, "ЗаказПредставление") Тогда
			Отбор.Вставить("Заказ", ВыборкаЗаписей.ЗаказПредставление);
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаЗаписей, "ЗаказПокупателяПредставление") Тогда
			Отбор.Вставить("Заказ", ВыборкаЗаписей.ЗаказПокупателяПредставление);
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаЗаписей, "НомерГТДПредставление") Тогда
			Отбор.Вставить("НомерГТД", ВыборкаЗаписей.НомерГТДПредставление);
		КонецЕсли;
		
		МассивСтрок = ДокОбъект.Запасы.НайтиСтроки(Отбор);
