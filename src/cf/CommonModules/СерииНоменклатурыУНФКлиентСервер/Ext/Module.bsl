...
//
Процедура ДобавитьСерияВСтроку(НоваяСтрока, Серия, ДокОбъект, 
	ИмяТЧСерииНоменклатуры = "СерииНоменклатуры", ИмяТЧЗапасы = "Запасы", ИмяПоляКлючСвязи = "КлючСвязи", ИспользоватьУникальныеСерии = Неопределено) Экспорт
	...
	Если ИспользоватьУникальныеСерии <> Неопределено Тогда 
		Если ИспользоватьУникальныеСерии Тогда
			СтрСерииНоменклатуры.Количество = НоваяСтрока.Количество;
		Иначе
			СтрСерииНоменклатуры.Количество = 1;
		КонецЕсли; 
	КонецЕсли;
