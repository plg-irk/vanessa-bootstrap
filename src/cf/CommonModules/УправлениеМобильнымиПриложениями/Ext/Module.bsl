...
Процедура ОбработатьПоведениеЭлементаПриИзменении(Форма, ИмяНастройки, НачальнаяНастройка = Ложь)
	...
	ИначеЕсли ИмяНастройки = "ВидЗаказаПокупателяДляСборки" Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
			Форма.СостояниеЗаказаДляСборки = РаботаСЗаявками.ПервоеСостояниеПослеЗаявки(Форма.ВидЗаказаПокупателяДляСборки);
		КонецЕсли;
	ИначеЕсли ИмяНастройки = "ВидЗаказаПокупателяСобран" Тогда 
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
			Форма.СостояниеЗаказаСобран = РаботаСЗаявками.ПервоеСостояниеПослеЗаявки(Форма.ВидЗаказаПокупателяСобран);
		КонецЕсли;
	ИначеЕсли ИмяНастройки = "ВидЗаказаПокупателяЗаданиеТорговомуПредставителю" Тогда 
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
			Форма.СостояниеЗаданиеТорговомуПредставителю = РаботаСЗаявками.ПервоеСостояниеПослеЗаявки(
			Форма.ВидЗаказаПокупателяЗаданиеТорговомуПредставителю);
		КонецЕсли;
