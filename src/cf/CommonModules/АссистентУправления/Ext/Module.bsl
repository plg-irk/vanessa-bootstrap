...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект) Экспорт
	...
	АссистентЗапущен = ВыполнитьТекущиеЗадачиСейчасВФоне(ТекущийОбъект.Ссылка);
	Если АссистентЗапущен Тогда
		Если ТекущийОбъект.ДополнительныеСвойства.Свойство("ДляОбсуждений") 
			И ТипЗнч(ТекущийОбъект.ДополнительныеСвойства.ДляОбсуждений) = Тип("Структура") Тогда
			ТекущийОбъект.ДополнительныеСвойства.ДляОбсуждений.Вставить("АссистентЗапущен", Истина);		
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьТекущиеЗадачиСейчас(Предмет = Неопределено) Экспорт
	...
	ПредметЭтоСсылка = ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Предмет));
	ОбсужденияУНФ.ЗаписатьОтложенныеСообщенияВСистемуВзаимодействияСейчас(
			?(ПредметЭтоСсылка = Истина, Предмет, Неопределено), ПользовательАссистент());	
...
//  Булево - запущена ли работа Ассистента
Функция ВыполнитьТекущиеЗадачиСейчасВФоне(Предмет = Неопределено) Экспорт
...
Функция ПредложениеВыполнитьЗадачуИнтерактивно(Предмет, Событие, Действие, ЗначенияЗаполнения, ДополнительныеПараметры) Экспорт
	...
	ДанныеСообщенияДанные.Вставить("ЗначенияЗаполнения", ЗначенияЗаполнения);
	ДанныеСообщенияДанные.Вставить("ДополнительныеПараметрыИсходные", ДополнительныеПараметры);	
...
Процедура РазыменоватьДанныеСообщенияСВ(ОписаниеСообщения) Экспорт
	...
	ИначеЕсли ТипЗнч(ОписаниеСообщения.Данные) = Тип("ХранилищеЗначения") Тогда 
		ОписаниеСообщения.РаспознанныеДанные = ОписаниеСообщения.Данные.Получить();	
	КонецЕсли;
...
Функция ОписаниеСобытия(Предмет, СобытиеИдентификатор)
	...
	Если НайденныеСобытия.Количество() = 0 Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='Событие по идентификатору не обнаружено: ""%1"", Предмет: ""%2""'"), СобытиеИдентификатор, Предмет);
	КонецЕсли;
