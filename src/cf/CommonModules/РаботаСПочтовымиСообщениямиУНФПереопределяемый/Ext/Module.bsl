...
Процедура ПриПолученииНастроекПочтовыхЯщиков(УчетнаяЗапись, ОписаниеПочтовыхЯщиков, ДополнительныеПараметры) Экспорт
	...
	ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог("Начало получения настроек почтовых ящиков", УчетнаяЗапись);
	...
	ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог("Конец получения настроек почтовых ящиков", УчетнаяЗапись);
...
Процедура ЗаполнитьПараметрыОтбораIMAP(УчетнаяЗапись, ОписаниеПочтовыхЯщиков, ДополнительныеПараметры)
	...
	Если Не ДополнительныеПараметры.ИспользуютсяПочтовыеЯщики Тогда
		...
		ЛогироватьПараметрыОтбораIMAP(УчетнаяЗапись, ОписаниеПочтовыхЯщиков);
		Возврат;
...
Функция ПослеДатыОтправленияПоПочтовымСерверам(ПослеДатыОтправления)
	...
	СамаяРанняяДата = '19700102';
	Если ПослеДатыОтправления < СамаяРанняяДата Тогда
		ПослеДатыОтправления = СамаяРанняяДата;
	КонецЕсли;
...
Процедура ЛогироватьПараметрыОтбораIMAP(УчетнаяЗапись, ОписаниеПочтовыхЯщиков)
	
	Если Не ОбменСВнешнимиСистемамиЛогированиеПовтИсп.ЛогированиеВключено(УчетнаяЗапись) Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого ОписаниеПочтовогоЯщика Из ОписаниеПочтовыхЯщиков Цикл
		
		Если ТипЗнч(ОписаниеПочтовогоЯщика.РанееПолученныеИдентификаторы) = Тип("Массив") Тогда
			КоличествоРанееПолученныхИдентфикаторов = ОписаниеПочтовогоЯщика.РанееПолученныеИдентификаторы.Количество();
		Иначе
			КоличествоРанееПолученныхИдентфикаторов = 0;
		КонецЕсли;
		
		Если ТипЗнч(ОписаниеПочтовогоЯщика.ПараметрыОтбораIMAP) = Тип("Структура")
			И ОписаниеПочтовогоЯщика.ПараметрыОтбораIMAP.Свойство("ПослеДатыОтправления") Тогда
			ПослеДатыОтправления = Строка(ОписаниеПочтовогоЯщика.ПараметрыОтбораIMAP.ПослеДатыОтправления);
		Иначе
			ПослеДатыОтправления = "";
		КонецЕсли;
		
		ТекстСообщения = СтрШаблон("Почтовый ящик ""%1"":
		|Количество ранее полученных идентификаторов - %2
		|Получать письма после даты - %3",
		ОписаниеПочтовогоЯщика.ПочтовыйЯщик,
		КоличествоРанееПолученныхИдентфикаторов,
		ПослеДатыОтправления);
