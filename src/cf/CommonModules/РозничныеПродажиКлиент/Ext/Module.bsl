...
Процедура РедактироватьОплату(Форма, ЭтоРМК_МК = Ложь) Экспорт
	...
	Если ЭлементыБезналичнаяОплата.ТекущиеДанные.ВидОплаты = ПредопределенноеЗначение(
		...
		СтруктураПараметров.Вставить("Товары", Форма.Объект.Запасы);
	...
	Если ШтрихкодированиеИСМПКлиент.РезультатПроверкиСредствамиККТТребуетФискализации(Результат)
		...
		Для Каждого ЭлементПроверки Из Результат.ЭлементыПроверки Цикл
			...
			Если Не (ЗначениеЗаполнено(ЭлементПроверки.РазрешительныйРежимИдентификаторЗапросаГИСМТ)
				И ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(ЭлементПроверки.ВидПродукции))
				И Не ЗначениеЗаполнено(ЭлементПроверки.РазрешительныйРежимИдентификаторЭкземпляраПО) Тогда
				Продолжить;
			КонецЕсли;
			...
			Для Каждого ПозицияЧека Из ДополнительныеПараметры.ОповещениеОЗавершении.ДополнительныеПараметры.ОбщиеПараметры.ПозицииЧека Цикл
				...
				Если РезультатРаспределенияШтрихкодов.ПолныйКодМаркировки = ЭлементПроверки.ПолныйКодМаркировки Тогда
					...
					Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПозицияЧека, "ЗапросПроверкиКода") Тогда
						...
						ПозицияЧека.ЗапросПроверкиКода.ИдентификаторЭкземпляра
							= ЭлементПроверки.РазрешительныйРежимИдентификаторЭкземпляраПО;
						ПозицияЧека.ЗапросПроверкиКода.ВерсияБазы
							= ЭлементПроверки.РазрешительныйРежимВерсияБазы;
					Иначе
						...
						ПозицияЧека.РезультатРаспределенияШтрихкодов.РазрешительныйРежимДатаЗапросаГИСМТ
							= ЭлементПроверки.РазрешительныйРежимДатаЗапросаГИСМТ;
						ПозицияЧека.РезультатРаспределенияШтрихкодов.РазрешительныйРежимИдентификаторЭкземпляраПО
							= ЭлементПроверки.РазрешительныйРежимИдентификаторЭкземпляраПО;
						ПозицияЧека.РезультатРаспределенияШтрихкодов.РазрешительныйРежимВерсияБазы
							= ЭлементПроверки.РазрешительныйРежимВерсияБазы;
					КонецЕсли;
