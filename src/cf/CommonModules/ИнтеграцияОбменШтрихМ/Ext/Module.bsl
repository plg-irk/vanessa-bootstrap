...
Функция ПолучитьРеквизитыКассыШтрихМ(КассаККМ) Экспорт
	...
	|ИЗ
	|	РегистрСведений.УдалитьНастройкиКассыШтрихМ КАК НастройкиКассыШтрихМ
	|ГДЕ
...
Функция ЗагрузитьДанныеОбменаИз1СКасса(ИмяФайла) Экспорт
	...
	Константы.УдалитьИспользоватьОбменСКассойMinikassir.Установить(Истина);
...
// Проверяет наличие в базе хотя бы одной подключенной кассы Штрих-MPAY
// и если ни одной не находит, то отключает опцию использования обмена с ними
//
Процедура АктуализироватьОпциюОбмена(Параметры) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьОбменСКассойMinikassir") Тогда
		Параметры.ОбработкаЗавершена = Истина;
		Возврат;
	КонецЕсли;
	
	Если ОбменСКассовымСерверомШтрихМ.ЕстьПодключенныеКассы() Тогда
		Параметры.ОбработкаЗавершена = Истина;
		Возврат;
	КонецЕсли;
