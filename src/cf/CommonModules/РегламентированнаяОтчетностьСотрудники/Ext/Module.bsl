...
Функция ВыполнитьРасчетВзносовИНалоговПоСотрудникам(Организация,ПериодРасчета, СобытиеКалендаря, ТринадцатыйПериод = Ложь) Экспорт
	...
	МассивСтруктурРасчетаНДФЛ = Новый Массив();
	СтруктураНДФЛ							= Новый Структура("КБК, Налог, Сумма", Неопределено,Неопределено, 0);
	СтруктураНДФЛСвышеПредельнойВеличины 	= Новый Структура("КБК, Налог, Сумма", Неопределено,Неопределено, 0);
	СтруктураНДФЛСвышеПредельнойВеличины18 	= Новый Структура("КБК, Налог, Сумма", Неопределено,Неопределено, 0);
	СтруктураНДФЛСвышеПредельнойВеличины20 	= Новый Структура("КБК, Налог, Сумма", Неопределено,Неопределено, 0);
	СтруктураНДФЛСвышеПредельнойВеличины22 	= Новый Структура("КБК, Налог, Сумма", Неопределено,Неопределено, 0);
	СтруктураНДФЛРКИСН 						= Новый Структура("КБК, Налог, Сумма", Неопределено,Неопределено, 0);
	СтруктураНДФЛРКИСНСвышеПредельнойВеличины 	= Новый Структура("КБК, Налог, Сумма", Неопределено,Неопределено, 0);
	...
	Для Каждого СтрокаТаблицы Из УдержаноНалогаЗаПериод Цикл
		...
		Если СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛСотрудники ИЛИ СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыРКИСН Тогда
			СтруктураРасчета.НДФЛСотрудники = СтруктураРасчета.НДФЛСотрудники + СтрокаТаблицы.Сумма;   
		ИначеЕсли СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины
			ИЛИ СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины18
			ИЛИ СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины20
			ИЛИ СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины22 
			ИЛИ СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыРКИСНСвышеПредельнойВеличины Тогда	
		...
		Если СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛСотрудники Тогда
			СтруктураНДФЛ.Налог = СтрокаТаблицы.Налог;
			СтруктураНДФЛ.КБК = СтрокаТаблицы.КБК;
			СтруктураНДФЛ.Сумма = СтруктураНДФЛ.Сумма + СтрокаТаблицы.Сумма; 
		ИначеЕсли СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины Тогда
			СтруктураНДФЛСвышеПредельнойВеличины.Налог = СтрокаТаблицы.Налог;
			СтруктураНДФЛСвышеПредельнойВеличины.КБК = СтрокаТаблицы.КБК;
			СтруктураНДФЛСвышеПредельнойВеличины.Сумма = СтруктураНДФЛСвышеПредельнойВеличины18.Сумма + СтрокаТаблицы.Сумма; 
		ИначеЕсли СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины18 Тогда
			СтруктураНДФЛСвышеПредельнойВеличины18.Налог = СтрокаТаблицы.Налог;
			СтруктураНДФЛСвышеПредельнойВеличины18.КБК = СтрокаТаблицы.КБК;
			СтруктураНДФЛСвышеПредельнойВеличины18.Сумма = СтруктураНДФЛСвышеПредельнойВеличины18.Сумма + СтрокаТаблицы.Сумма; 
		ИначеЕсли СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины20 Тогда	
			СтруктураНДФЛСвышеПредельнойВеличины20.Налог = СтрокаТаблицы.Налог;
			СтруктураНДФЛСвышеПредельнойВеличины20.КБК = СтрокаТаблицы.КБК;
			СтруктураНДФЛСвышеПредельнойВеличины20.Сумма = СтруктураНДФЛСвышеПредельнойВеличины20.Сумма + СтрокаТаблицы.Сумма;
		ИначеЕсли СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины22 Тогда	
			СтруктураНДФЛСвышеПредельнойВеличины22.Налог = СтрокаТаблицы.Налог;
			СтруктураНДФЛСвышеПредельнойВеличины22.КБК = СтрокаТаблицы.КБК;
			СтруктураНДФЛСвышеПредельнойВеличины22.Сумма = СтруктураНДФЛСвышеПредельнойВеличины22.Сумма + СтрокаТаблицы.Сумма;
		ИначеЕсли СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыРКИСН Тогда	
			СтруктураНДФЛРКИСН.Налог = СтрокаТаблицы.Налог;
			СтруктураНДФЛРКИСН.КБК = СтрокаТаблицы.КБК;
			СтруктураНДФЛРКИСН.Сумма = СтруктураНДФЛРКИСН.Сумма + СтрокаТаблицы.Сумма;
		ИначеЕсли СтрокаТаблицы.Налог = Справочники.ВидыНалогов.НДФЛДоходыРКИСНСвышеПредельнойВеличины Тогда	
			СтруктураНДФЛРКИСНСвышеПредельнойВеличины.Налог = СтрокаТаблицы.Налог;
			СтруктураНДФЛРКИСНСвышеПредельнойВеличины.КБК = СтрокаТаблицы.КБК;
			СтруктураНДФЛРКИСНСвышеПредельнойВеличины.Сумма = СтруктураНДФЛРКИСНСвышеПредельнойВеличины.Сумма + СтрокаТаблицы.Сумма;
		КонецЕсли;		
		
	КонецЦикла;
	
	Если ЗначениеЗаполнено(СтруктураНДФЛ.КБК) Тогда
		МассивСтруктурРасчетаНДФЛ.Добавить(СтруктураНДФЛ);
	КонецЕсли;

	Если ЗначениеЗаполнено(СтруктураНДФЛСвышеПредельнойВеличины.КБК) Тогда
		МассивСтруктурРасчетаНДФЛ.Добавить(СтруктураНДФЛСвышеПредельнойВеличины);
	КонецЕсли;
		
	Если ЗначениеЗаполнено(СтруктураНДФЛСвышеПредельнойВеличины18.КБК) Тогда
		МассивСтруктурРасчетаНДФЛ.Добавить(СтруктураНДФЛСвышеПредельнойВеличины18);
	КонецЕсли;
		
	Если ЗначениеЗаполнено(СтруктураНДФЛСвышеПредельнойВеличины20.КБК) Тогда
		МассивСтруктурРасчетаНДФЛ.Добавить(СтруктураНДФЛСвышеПредельнойВеличины20);
	КонецЕсли;
		
	Если ЗначениеЗаполнено(СтруктураНДФЛСвышеПредельнойВеличины22.КБК) Тогда
		МассивСтруктурРасчетаНДФЛ.Добавить(СтруктураНДФЛСвышеПредельнойВеличины22);
	КонецЕсли;  
	
	Если ЗначениеЗаполнено(СтруктураНДФЛРКИСН.КБК) Тогда
		МассивСтруктурРасчетаНДФЛ.Добавить(СтруктураНДФЛРКИСН);
	КонецЕсли;

	Если ЗначениеЗаполнено(СтруктураНДФЛРКИСНСвышеПредельнойВеличины.КБК) Тогда
		МассивСтруктурРасчетаНДФЛ.Добавить(СтруктураНДФЛРКИСНСвышеПредельнойВеличины);
	КонецЕсли;

	
	СтруктураРасчета.хзНДФЛМассивСтруктурРасчета = Новый ХранилищеЗначения(МассивСтруктурРасчетаНДФЛ);
