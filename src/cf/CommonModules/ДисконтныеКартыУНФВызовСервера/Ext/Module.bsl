...
//
// Параметры:
//  Штрихкод	 - Строка	 - код дисконтной карты для поиска.
//  BASE64		 - Булево	 - признак передачи кода в формате BASE64.
//  ПоискИзЧека	 - Булево	 - признак поиска из ЧекаККМ.
// 
// Возвращаемое значение:
//  Структура - результат поиска дисконтных карт по штрихкоду 
//  * Зарегистрированные дисконтные карты
//  * НеЗарегистрированныеДисконтныеКарты.
//
Функция НайтиДисконтныеКартыПоШтрихкоду(Штрихкод, BASE64 = Ложь, ПоискИзЧека = Ложь) Экспорт
	
	Если BASE64 Тогда
		Штрихкод = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.Base64ВШтрихкод(Штрихкод);
	КонецЕсли;
	
	Возврат ДисконтныеКартыУНФСервер.НайтиДисконтныеКарты(Штрихкод, Перечисления.ТипыКодовКарт.Штрихкод, Неопределено, ПоискИзЧека);
...
Процедура НачислитьБаллыПриРегистрациияКарты(ДисконтнаяКарта) Экспорт
	ДисконтныеКартыУНФСервер.НачислитьБаллыПриРегистрациияКарты(ДисконтнаяКарта);
КонецПроцедуры
#КонецОбласти
