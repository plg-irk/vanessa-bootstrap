...
Процедура ЗаполнениеДокументовЕГАИС(Источник, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ТТНИсходящаяЕГАИС") Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПеремещениеЗапасов") Тогда
			...
			ОсновнойВидЦенПродажи = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("ОсновнойВидЦенПродажи");
			Если ЗначениеЗаполнено(ОсновнойВидЦенПродажи) Тогда
				ПерезаполнитьЦеныПоВидуЦен(Источник, ОсновнойВидЦенПродажи);
			КонецЕсли;
...
Процедура ПерезаполнитьЦеныПоВидуЦен(ДокументОбъект, ВидЦен)
	...
	Для каждого СтрокаТовары Из ДокументОбъект.Товары Цикл
		...
		ДанныеСтроки.Вставить("ВидЦен",           ВидЦен);
		ДанныеСтроки.Вставить("ДатаОбработки",    ДокументОбъект.Дата);
		ДанныеСтроки.Вставить("СуммаВключаетНДС", Истина);
		ДанныеСтроки.Вставить("Номенклатура",     СтрокаТовары.Номенклатура);
		ДанныеСтроки.Вставить("Характеристика",   СтрокаТовары.Характеристика);
		ДанныеСтроки.Вставить("Цена",             СтрокаТовары.Цена);
		ДанныеСтроки.Вставить("ВалютаДокумента",  ВалютаУчета);
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТовары, "ЕдиницаИзмерения")
			И ТипЗнч(СтрокаТовары.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
			ДанныеСтроки.Вставить("Коэффициент", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеСтроки.ЕдиницаИзмерения, "Коэффициент"));
		Иначе
			ДанныеСтроки.Вставить("Коэффициент", 1);
		КонецЕсли;
		
		СтрокаТовары.Цена = ЦенообразованиеСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(ДанныеСтроки);
