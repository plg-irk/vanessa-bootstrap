...
Процедура ОбработкаОповещенияИС(Форма, ИмяСобытия, Параметр, Источник, ДополнительныеПараметры) Экспорт
	...
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокумента_РМК"
		...
		Если Источник = "ПодключаемоеОборудование" И Форма.ВводДоступен() Тогда
			Если ИмяСобытия = "ScanData" И Не ПараметрыПриложения["ПодключаемоеОборудованиеСобытиеОбработаноИС"] Тогда
	...
	Если Форма.ИмяФормы = "Документ.ЧекККМ.Форма.ФормаДокумента_РМК"
		...
		Если Элемент = ИмяТабличнойЧасти Тогда
			...
			Если НЕ ТекущаяСтрока.Номенклатура = КэшСтроки.Номенклатура
				...
				Или ТекущаяСтрока.ВидПродукцииИС = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ПивоВПотребительскихУпаковках")
				Или ТекущаяСтрока.ВидПродукцииИС = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БезалкогольноеПиво") Тогда
				ДополнительныеПараметры.Вставить("ТребуетсяСерверныйВызов");
...
Процедура ЗаполнитьКодТНВЭДПоСтрокеПослеВыбораИзСервиса(ВыбранноеЗначение, ДанныеТекущейСтроки) Экспорт
	...
	ДанныеТекущейСтроки.КодТНВЭД = ВыбранноеЗначение.Код;
