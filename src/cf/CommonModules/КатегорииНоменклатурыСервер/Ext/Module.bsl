...
//  ДеревоЗначений - дерево используемых категорий.
//
Функция ДеревоКатегорийХарактеристики(СортироватьКатегорииПоНаименованию = Ложь) Экспорт
	...
	Если СортироватьКатегорииПоНаименованию Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "КатегорииНоменклатуры.РеквизитДопУпорядочивания,", "");
	КонецЕсли;
...
Процедура ЗаполнитьСвойстваНоменклатурыПоКатегории(Номенклатура, ОбновлятьЗаписанныйОбъект = Ложь, ТолькоЗаполненныеСвойства = Ложь) Экспорт
	...
	Если Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас Тогда
		...
		Если Категория.ИспользоватьСерииНоменклатуры И Категория.ИспользоватьСерииИзКатегории Тогда
			ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", СтранаПроисхождения, ИспользоватьНомерПартии,
			| ИспользоватьСерииНоменклатуры, ИспользоватьСерииИзКатегории";
		Иначе
			ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", СтранаПроисхождения, ИспользоватьНомерПартии,
			| ИспользоватьСерииНоменклатуры, ИспользоватьСрокГодностиСерии, ИспользоватьДатуПроизводстваСерии,
			| ТочностьУказанияСрокаГодностиСерии, ИспользоватьУникальныеСерии, ПолитикаУчетаСерий, ИспользоватьСерииИзКатегории";
		КонецЕсли;
		
		// ИСМП.
		ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", ВидПродукцииИС, УдалитьТабачнаяПродукция, УдалитьОбувнаяПродукция";
	...
	ПереченьСвойств = ЗаполняемыеСвойства + ", " + СвойстваСПроверкойЗаполнения +
		...
		|, СчетУчетаАкцизаПоИмпорту, ВидПодакцизногоТовара, КодВидаПодакцизногоТовара, ЕдиницаИзмеренияДляРасчетаАкциза, КоэффициентПересчета";
	...
	Если Номенклатура.ТипНоменклатуры = ДанныеКатегории.ТипНоменклатурыПоУмолчанию Тогда
		Если ДанныеКатегории.ПодакцизныйТовар Тогда
			Номенклатура.СчетУчетаАкцизаПоИмпорту = ?(ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаАкцизаПоИмпорту), ДанныеКатегории.СчетУчетаАкцизаПоИмпорту, Номенклатура.СчетУчетаАкцизаПоИмпорту);
			
			Номенклатура.ВидПодакцизногоТовара = ?(ЗначениеЗаполнено(ДанныеКатегории.ВидПодакцизногоТовара), ДанныеКатегории.ВидПодакцизногоТовара, Номенклатура.ВидПодакцизногоТовара);
			Номенклатура.КодВидаПодакцизногоТовара = ?(ЗначениеЗаполнено(ДанныеКатегории.КодВидаПодакцизногоТовара), ДанныеКатегории.КодВидаПодакцизногоТовара, Номенклатура.КодВидаПодакцизногоТовара);
			Номенклатура.ЕдиницаИзмеренияДляРасчетаАкциза = ?(ЗначениеЗаполнено(ДанныеКатегории.ЕдиницаИзмеренияДляРасчетаАкциза), ДанныеКатегории.ЕдиницаИзмеренияДляРасчетаАкциза, Номенклатура.ЕдиницаИзмеренияДляРасчетаАкциза);
			Номенклатура.КоэффициентПересчета = ?(ЗначениеЗаполнено(ДанныеКатегории.КоэффициентПересчета), ДанныеКатегории.КоэффициентПересчета, Номенклатура.КоэффициентПересчета);
...
Процедура УстановитьНастройкуВидОтбораНоменклатуры(ВидОтбора = Неопределено) Экспорт
	...
	ТекущаяНастройка = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойВидОтбора");
	Если ТекущаяНастройка <> ВидОтбора Тогда
		ПерсональныеНастройкиРУНФ.Установить(ВидОтбора, "ОсновнойВидОтбора");
	КонецЕсли;
...
Функция ПолучитьНастройкуВидОтбораНоменклатуры() Экспорт
	...
	ВидОтбора = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойВидОтбора");
...
Функция ЗначениеЗаполненияКатегории() Экспорт
	...
	Категория = ПерсональныеНастройкиРУНФ.Прочитать("ЗначениеЗаполненияКатегорияНоменклатуры");
...
Процедура ЗаписатьЗначениеЗаполненияКатегории(Категория) Экспорт
	...
	КатегорияВНастройке = ПерсональныеНастройкиРУНФ.Прочитать("ЗначениеЗаполненияКатегорияНоменклатуры");
	Если КатегорияВНастройке = Категория
		ИЛИ НЕ ЗначениеЗаполнено(Категория) Тогда
		
		Возврат;
	КонецЕсли;
	ПерсональныеНастройкиРУНФ.Установить(Категория, "ЗначениеЗаполненияКатегорияНоменклатуры");
	
КонецПроцедуры

// Текст подсказки к элементу формы СтавкаНДС с расширенным текстом, если есть плательщики НДС на УСН.
// 
// Параметры:
//  ЕстьУСНсНДС - Булево - Есть плательщики НДС на УСН
// 
// Возвращаемое значение:
//  - ФорматированнаяСтрока - Текст подсказки ставка НДС
Функция ТекстПодсказкиСтавкаНДС(ЕстьУСНсНДС) Экспорт
	
	Если ЕстьУСНсНДС Тогда
		ТекстПодсказки = НСтр("ru = 'Используется для подстановки в документы при выборе номенклатуры
		|в зависимости от системы налогообложения.
		|Подробнее в статье <a href = ""https://its.1c.ru/db/metod81#content:8391:hdoc"">Учет НДС для организаций на УСН</a>.'");
	Иначе
		ТекстПодсказки = НСтр("ru = 'Используется для подстановки в документы при выборе номенклатуры.'");
	КонецЕсли;
...
Функция НастройкаВидОтбораНоменклатурыПоУмолчанию()
	...
	Результат = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойВидОтбора");
	Возврат Результат;
