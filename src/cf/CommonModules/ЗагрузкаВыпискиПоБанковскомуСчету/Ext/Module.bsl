	...
	Для Каждого КлючИЗначение Из Выписки Цикл
		...
		Для Каждого Операция Из Выписка.Операции Цикл
			...
			Если ХозяйственнаяОперация.Свойство("Контрагент") И ЗначениеЗаполнено(СтрокаСекции.Контрагент)
				И ТипЗнч(СтрокаСекции.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
				
				Если СтрокаСекции.Контрагент <> ХозяйственнаяОперация.Контрагент Тогда
...
Функция ИдентификаторыФизическогоЛицаИзНаименования(Наименование, ИНН, Счет, ВидОперацииЦР)
	...
	Для Каждого ЭлементНаименования Из ЭлементыНаименования Цикл
		ТолькоРазрешенныеСимволы.Добавить(СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(
...
Функция НовыйРаспознаннаяВыписка(Идентификатор, УчастникиОпераций)
	...
	РаспознаннаяВыписка.Вставить("НастройкиСчета", Новый Структура());
		   ...
		   Тогда
			...
			Если ЗначениеЗаполнено(ИсходныеДанные.ПлатежноеПоручение) Тогда
				...
				Если НЕ ЗначениеЗаполнено(СтрокаРасшифровки.СтавкаНДС) Тогда
					ЗначениеСтавкиНДС = 0;
					СуммаНДС = 0;
					ОпределитьСтавкуНДСПоНазначениюПлатежа(ХозяйственнаяОперация, СтрокаРасшифровки.СтавкаНДС,
						ЗначениеСтавкиНДС, СуммаНДС);
				Иначе
					ЗначениеСтавкиНДС = ИсходныеДанные.ПлатежноеПоручение.СтавкаНДС.Ставка;
					СуммаНДС = ИсходныеДанные.ПлатежноеПоручение.СуммаНДС;
				КонецЕсли;
			...
			Иначе
				Если ОбъектДокумента.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
					...
					СуммаНДС = 0;
					Если ОбъектДокумента.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
						ОпределитьСтавкуНДСПоНазначениюПлатежа(ХозяйственнаяОперация, СтавкаНДСВНазначенииПлатежа,
							ЗначениеСтавкиНДС, СуммаНДС);
					КонецЕсли;
...
Процедура ЗаполнитьРеквизитыПоступлениеНаСчет(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент,
	...
	ИначеЕсли ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам
		...
		ПоложениеЭквайринговогоТерминала = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеЭквайринговогоТерминалаВБанковскихДокументах");
		ПоложениеНастроекНалоговогоУчета = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеНастроекНалоговогоУчетаВБанковскихДокументах");
...
Процедура ОпределитьСтавкуНДСПоНазначениюПлатежа(ХозяйственнаяОперация, СтавкаНДСВНазначенииПлатежа, ЗначениеСтавкиНДС, СуммаНДС)
	
	Если ХозяйственнаяОперация <> Неопределено И ХозяйственнаяОперация.Свойство("СтавкаНДС") Тогда
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента, Документ, ПараметрыВыделенияКолонок, Объект,
	...
	РеквизитыСчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.БанковскийСчет,
		"ВалютаДенежныхСредств, НомерСчета, Банк, Владелец");
	БИК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыСчета.Банк, "Код");
	Исходящий = СтрокаДокумента.ПлательщикСчет = СокрЛП(РеквизитыСчета.НомерСчета)
				 И (СтрокаДокумента.ПлательщикБИК = СокрЛП(БИК)
	...
	Если ВсеРеквизитыПоискаЕсть Тогда
		...
		Если ДокументНайден Тогда
			...
			РасшифровкаПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "РасшифровкаПлатежа").Выгрузить();
			Если РасшифровкаПлатежа.Количество() = 1 Тогда
				СтрокаДокумента.Договор = РасшифровкаПлатежа[0].Договор;
				СтрокаДокумента.ПризнакАванса = РасшифровкаПлатежа[0].ПризнакАванса;
				СтрокаДокумента.Заказ = РасшифровкаПлатежа[0].Заказ;
				СтрокаДокумента.ДокументРасчетов = РасшифровкаПлатежа[0].Документ;
				СтрокаДокумента.СчетНаОплату = РасшифровкаПлатежа[0].СчетНаОплату;
				...
				Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
					СтрокаДокумента.ПлатежноеПоручение = ДокументОснование;
				КонецЕсли;
			КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.ВидОперации) Тогда
		...
		Если Не ЗначениеЗаполнено(ВидОперацииДокумента) Тогда
			...
			Если Исходящий Тогда
				...
				СчетПолучателя = Справочники.БанковскиеСчета.НайтиПоРеквизиту("НомерСчета",
						СтрокаДокумента.ПолучательСчет);

				Если ЗначениеЗаполнено(СчетПолучателя) Тогда
					//@skip-check bsl-legacy-check-string-literal
					ВладелецСчетаПолучателя =
						ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетПолучателя, "Владелец");
				Иначе
					ВладелецСчетаПолучателя = Неопределено;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтрокаДокумента.СтатусСоставителя) И СокрП(СтрокаДокумента.СтатусСоставителя) <> "0" Тогда
					...
					КонецЕсли;
					СтрокаДокумента.ВидОперацииНеНайденКартинка = -1;
				ИначеЕсли ВладелецСчетаПолучателя = РеквизитыСчета.Владелец Тогда // перевод на другой счет
				...
				ИначеЕсли ТипЗнч(ВладелецСчетаПолучателя) = Тип("СправочникСсылка.ФизическиеЛица") Тогда // Сотруднику
					ЭтоПлатежСотруднику = Истина;
			...
			ИначеЕсли КонтрагентПоБанковскомуСчету = РеквизитыСчета.Владелец Тогда // перевод на другой счет
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.СчетКонтрагента) Тогда
		...
		Иначе
			...
			Банк = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.БанковскийСчет, "Банк");
			СтрокаДокументаБИК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Банк, "Код");
			КоличествоЭлементовВВыборке = КоличествоЭлементовВВыборке(ВыборкаПоиска, ИННКонтрагента, СтрокаДокументаБИК);
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.Контрагент) Тогда
		...
		Иначе
			Если СтрокаДокумента.ВидОперации <> ВидОперацииПереводНаДругойСчет
				И СтрокаДокумента.ВидОперации <> ВидОперацииПереводСДругогоСчета Тогда
	...
	Если ТипЗнч(СтрокаДокумента.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
		Если НЕ ДанныеКонтрагентаПолучены И ЗначениеЗаполнено(СтрокаДокумента.Контрагент) Тогда
			РеквизитыКонтрагента =
				ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаДокумента.Контрагент, "ИНН, КПП, НаименованиеПолное");
			ИННКонтрагентаВБазе = РеквизитыКонтрагента.ИНН;
			КППКонтрагентаВБазе = РеквизитыКонтрагента.КПП;
			Если СокрЛП(КППКонтрагентаВБазе) = "0" Тогда
				КППКонтрагентаВБазе = "";
			КонецЕсли;
			НаименованиеПолноеКонтрагентаВБазе = РеквизитыКонтрагента.НаименованиеПолное;
		КонецЕсли;
	...
	Если Не ДокументНайден Тогда
		...
		Если Объект.АвтоматическиПодставлятьДокументы Тогда
			...
			Если НашлиПП Тогда
				...
				РасшифровкаПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					СтрокаДокумента.ПлатежноеПоручение, "РасшифровкаПлатежа").Выгрузить();
				Если РасшифровкаПлатежа.Количество() = 1 Тогда
					
					// Документ расчетов
					Если ЗначениеЗаполнено(РасшифровкаПлатежа[0].Документ) Тогда
						СтрокаДокумента.ДокументРасчетов = РасшифровкаПлатежа[0].Документ;
					КонецЕсли;
					
					// Заказ
					Если ЗначениеЗаполнено(РасшифровкаПлатежа[0].Заказ) Тогда
						СтрокаДокумента.Заказ = РасшифровкаПлатежа[0].Заказ;
					КонецЕсли;
					
					// Счет на оплату
					Если ЗначениеЗаполнено(РасшифровкаПлатежа[0].СчетНаОплату) Тогда
						СтрокаДокумента.СчетНаОплату = РасшифровкаПлатежа[0].СчетНаОплату;
					КонецЕсли;
...
Функция КоличествоЭлементовВВыборке(Знач ВыборкаПоиска, Знач ИННКонтрагента, Знач СтрокаДокументаБИК)
	...
	Если НЕ ПустаяСтрока(ИННКонтрагента) Тогда
		...
		Пока ВыборкаПоиска.Следующий() Цикл
			Если ВыборкаПоиска.ВладелецИНН = ИННКонтрагента Тогда
				Если  ВыборкаПоиска.БИК_из_Базы	= СтрокаДокументаБИК Тогда
...
Функция ОпределитьДокументРасчетовНаСервере(СтрокаДокумента, Объект, ДоговорВЗаказеСчете = Неопределено)
	...
	Если (ТипЗнч(СтрокаДокумента.Контрагент) = Тип("СправочникСсылка.Контрагенты")) 
		...
		ВестиРасчетыПоДокументам =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.Контрагент, "ВестиРасчетыПоДокументам");
	Иначе
...
Функция ОпределитьЗаказИСчетНаСервере(СтрокаДокумента, Объект)
	...
	Если (ТипЗнч(СтрокаДокумента.Контрагент) = Тип("СправочникСсылка.Контрагенты")) 
		...
		СтруктураКурсВалюта = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(
			СтрокаДокумента.ДатаДок, Новый Структура("Валюта", ВалютаСчета));
	...
	Если НашлиСчет И НЕ НашлиЗаказ И НЕ ЗначениеЗаполнено(СтрокаДокумента.Заказ) Тогда
		ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.СчетНаОплату, "ДокументОснование");
		Если (СтрокаДокумента.Исходящий И ТипЗнч(СтрокаДокумента.СчетНаОплату) = Тип("ДокументСсылка.СчетНаОплатуПоставщика")
			...
			СтрокаДокумента.Заказ = ДокументОснование;
...
Функция НайтиДоговорВВидеСтруктуры(ВладелецДоговора, ОрганизацияДоговора = Неопределено, Знач ВидОперации = Неопределено)
  ...
  +	"	И ДоговорыКонтрагентов.ПометкаУдаления = ЛОЖЬ";
 	...
 	Если ТипЗнч(ВладелецДоговора) = Тип("СправочникСсылка.Контрагенты") Тогда
	 	Если Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВладелецДоговора, "ВестиРасчетыПоДоговорам") = Истина Тогда
  			ТекстФильтра = СтрЗаменить(ТекстФильтра, "ДоговорыКонтрагентов.Организация = &ОрганизацияДоговора", "Истина");
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьСпособЗачетаПоДокументу(СтрокаДокумента)
	...
	РасшифровкаПлатежа =
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.Документ, "РасшифровкаПлатежа").Выгрузить();
	Если РасшифровкаПлатежа.Количество() > 0 Тогда
		СтрокаДокумента.СпособЗачетаТЧ = РасшифровкаПлатежа[0].СпособЗачета;
...
Функция ЭтоОперацияГдеВозможныАвтоРасчеты(ВидОперации)
...
Процедура ОчиститьИнформациюОДокументахНаСервере(Строка)
