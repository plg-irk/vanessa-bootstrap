...
// Процедура настройки параметров проверки документа.
//
// Параметры:
//  Настройки - Структура - Структура настроек проверки документа.
//
...
// Процедура проверки документа перед записью.
//
// Параметры:
//  Источник - ДокументОбъект - Документ, который необходимо проверить перед записью.
//
// Процедура проверяет необходимость использования ассистента управления, добавляет дополнительные свойства
// и проверяет изменение бонусов перед записью документа.
...
// Процедура проверяет изменение бонусов при записи.
//
// Параметры:
//  Источник - ДокументОбъект - Документ, который проверяется при записи.
//
...
// Выполняет действие, связанное с оповещением сотрудника об изменении бонусов.
//
// Параметры:
//  Действие - Строка - Имя действия, которое необходимо выполнить.
//  Контекст - Структура - Контекст выполнения действия.
//  ПараметрыЗадачи - Структура - Параметры задачи оповещения сотрудника.
//  ДополнительныеПараметры - Структура - Дополнительные параметры выполнения действия.
//  Результат - Произвольный - Результат выполнения действия.
//
// Допустимые значения параметра Действие:
//  "ОповеститьСотрудникаОбИзмененииБонусовВОбсуждении" - Оповещает сотрудника об изменении бонусов в обсуждении.
//  "ОповеститьСотрудникаОбИзмененииБонусовПоEmail" - Оповещает сотрудника об изменении бонусов по электронной почте.
//  "ОповеститьСотрудникаОбИзмененииБонусовПоSMS" - Оповещает сотрудника об изменении бонусов по SMS.
//
// Если параметр Действие не соответствует ни одному из допустимых значений, то вызывается исключение.
...
Процедура АвтоматическоеНачислениеИСписаниеБонусныхБаллов() Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Автоматическое начисление и списание бонусных баллов.'"),
...
Процедура УведомлениеОбАвтоматическомНачисленииСписанииБонусов() Экспорт
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстСообщения = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ЗаписьЖурналаРегистрации(НСтр("ru='Уведомление об автоматическом начислении и списании бонусных баллов.'"),
			УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
	КонецПопытки;
...
Процедура ПроверитьИзменениеБонусовПередЗаписью(Источник)
	...
	Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Источник, "ДисконтнаяКарта")
		...
		ИЛИ НЕ ЗначениеЗаполнено(ПолучитьБонуснуюПрограмму(Источник.ДисконтнаяКарта))
		ИЛИ НЕ ЗадачаОповещенияПокупателяОБонусахНайдена(Источник.Ссылка) Тогда
		Возврат;
	КонецЕсли;
...
Процедура АссистентУправленияПриСрабатыванииСобытия(ДокументОбъект) Экспорт
	...
	Если ЗадачаОповещенияПокупателяОБонусахНайдена(ДокументОбъект.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя")
		И НЕ РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ДокументОбъект.ВидОперации) Тогда
...
Функция ПолучитьДанныеБонуснойКарты(БонуснаяКарта, Период, Документ = Неопределено) Экспорт
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА БонусныеБаллыОстатки.НачисленоОстаток - БонусныеБаллыОстатки.КСписаниюОстаток - ЕСТЬNULL(БонусныеБаллыКСписаниюИНачислению.КСписанию, 0) + ЕСТЬNULL(БонусныеБаллыКСписаниюИНачислению.КНачислению, 0) < 0
	|			ТОГДА 0
	|		ИНАЧЕ БонусныеБаллыОстатки.НачисленоОстаток - БонусныеБаллыОстатки.КСписаниюОстаток - ЕСТЬNULL(БонусныеБаллыКСписаниюИНачислению.КСписанию, 0) + ЕСТЬNULL(БонусныеБаллыКСписаниюИНачислению.КНачислению, 0)
	|	КОНЕЦ КАК Остаток
	...
	|		ПО БонусныеБаллыОстатки.БонуснаяКарта = БонусныеБаллыКСписаниюИНачислению.ДисконтнаяКарта
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура РаспределитьОплатуБонусамиКакСкидку(Объект, ТаблицаОплат = Неопределено, ОтменаОплатыБонусами = Ложь) Экспорт
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		И РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(Объект.ВидОперации) Тогда
		Товары = Новый Массив();
	...
	Если ИзлишнеЗачтеннаяПредоплата > 0 Тогда
		...
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМ") ИЛИ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМВозврат") Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "БезналичнаяОплата") Тогда
				ТЧДокумента = Объект.БезналичнаяОплата;
		...
		Иначе
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Предоплата") Тогда
				ТЧДокумента = Объект.Предоплата;
				Отбор = Новый Структура("ОплатаБонусами", Истина);
			КонецЕсли;
		КонецЕсли;
		Если Не ТЧДокумента = Неопределено Тогда
			СтрокиОплатыБонусами = ТЧДокумента.НайтиСтроки(Отбор);
		КонецЕсли;
		...
		Для Каждого СтрокаУдалить Из СтрокиОчищеннымиБонусами Цикл
			ТЧДокумента.Удалить(СтрокаУдалить);
...
//  Массив из СправочникСсылка.ДисконтныеКарты
...
КонецФункции// Функция возвращает структуру с реквизитами бонусной программы для указанной дисконтной карты.

// Функция возвращает структуру с реквизитами бонусной программы для указанной дисконтной карты.
//
// Параметры:
//  ДисконтнаяКарта - СправочникСсылка.ДисконтныеКарты - Ссылка на дисконтную карту, для которой необходимо получить реквизиты бонусной программы.
//
// Возвращаемое значение:
//  см. ПустаяСтруктураПараметровБонуснойПрограммы
Функция РеквизитыБонуснойПрограммы(ДисконтнаяКарта) Экспорт
	
	СтруктураВозврата = ПустаяСтруктураПараметровБонуснойПрограммы();
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(БонусныеПрограммы.ИспользоватьОграничениеОстатка, ЛОЖЬ)
	|			ТОГДА ЕСТЬNULL(БонусныеПрограммы.КоличествоОграниченияОстатка, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоОграниченияОстатка
	|ИЗ
	|	ВидКарт КАК ВидКарт
...
// 
// Возвращаемое значение:
//  Структура - Реквизиты бонусной программы для указанной дисконтной карты:
// * МаксимальныйПроцентОплаты - Число - Максимальный процент оплаты бонусами.
// * ПериодичностьСписания - ПеречислениеСсылка.Периодичность - Периодичность списания бонусов.
// * КоличествоПериодовСписания - Число - Количество периодов списания бонусов.
// * ПериодичностьОтсрочки - ПеречислениеСсылка.Периодичность - Периодичность отсрочки списания бонусов.
// * КоличествоПериодовОтсрочки - Число - Количество периодов отсрочки списания бонусов.
// * НачислятьБонусыНаДеньРождения - Булево - Признак начисления бонусов на день рождения.
// * ДнейПередДнемРождения - Число - Количество дней перед днем рождения, в течение которых начисляются бонусы.
// * ДнейПослеДняРождения - Число - Количество дней после дня рождения, в течение которых начисляются бонусы.
// * КоличествоБонусовНаДеньРождения - Число - Количество бонусов, начисляемых на день рождения.
// * ОтсрочкаНачисления - Булево - Признак отсрочки начисления бонусов.
// * СписаниеНеиспользованных - Булево - Признак списания неиспользованных бонусов.
// * НачислятьПередДнемРождения - Булево - Признак начисления бонусов перед днем рождения.
// * СписыватьПослеДняРождения - Булево - Признак списания бонусов после дня рождения.
// * НеНачислятьПриОплатеБонусами - Булево - Признак запрета начисления бонусов при оплате бонусами.
// * НачислятьБонусыПриВозврате - Булево - Признак начисления бонусов при возврате.
// * НачислятьДатойЗаказа - Булево - Признак начисления бонусов датой заказа.
// * БонуснаяПрограмма - СправочникСсылка.БонусныеПрограммы - Ссылка на бонусную программу.
// * ЕстьУточненияОграниченийОплаты - Булево - Признак наличия уточнений ограничений оплаты.
// * ЕстьУточненияПоКатегориям - Булево - Признак наличия уточнений по категориям.
// * ЕстьУточненияПоНоменклатуре - Булево - Признак наличия уточнений по номенклатуре.
// * ЕстьУточненияПоСегментамНоменклатуры - Булево - Признак наличия уточнений по сегментам номенклатуры.
// * ЕстьУточненияПоЦеновымГруппам - Булево - Признак наличия уточнений по ценовым группам.
// * НачислятьБонусыДоСкидок - Булево - Признак начисления бонусов до скидок.
// * Назначение - ПеречислениеСсылка.НазначенияАвтоматическихСкидок - Назначение бонусной программы.
// * КурсБонусов - Число - Курс бонусов.
// * КоличествоОграниченияОстатка - Число - Количество ограничения остатка бонусов.
Функция ПустаяСтруктураПараметровБонуснойПрограммы()
	
	ОписаниеБонуснойПрограммы = Новый Структура();
	ОписаниеБонуснойПрограммы.Вставить("МаксимальныйПроцентОплаты", 0);
	ОписаниеБонуснойПрограммы.Вставить("ПериодичностьСписания", ПредопределенноеЗначение("Перечисление.Периодичность.ПустаяСсылка"));
	ОписаниеБонуснойПрограммы.Вставить("КоличествоПериодовСписания", 0);
	ОписаниеБонуснойПрограммы.Вставить("ПериодичностьОтсрочки", ПредопределенноеЗначение("Перечисление.Периодичность.ПустаяСсылка"));
	ОписаниеБонуснойПрограммы.Вставить("КоличествоПериодовОтсрочки", 0);
	ОписаниеБонуснойПрограммы.Вставить("НачислятьБонусыНаДеньРождения", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("ДнейПередДнемРождения", 0);
	ОписаниеБонуснойПрограммы.Вставить("ДнейПослеДняРождения", 0);
	ОписаниеБонуснойПрограммы.Вставить("КоличествоБонусовНаДеньРождения", 0);
	ОписаниеБонуснойПрограммы.Вставить("ОтсрочкаНачисления", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("СписаниеНеиспользованных", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("НачислятьПередДнемРождения", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("СписыватьПослеДняРождения", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("НеНачислятьПриОплатеБонусами", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("НачислятьБонусыПриВозврате", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("НачислятьДатойЗаказа", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("БонуснаяПрограмма", ПредопределенноеЗначение("Справочник.БонусныеПрограммы.ПустаяСсылка"));
	ОписаниеБонуснойПрограммы.Вставить("ЕстьУточненияОграниченийОплаты", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("ЕстьУточненияПоКатегориям", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("ЕстьУточненияПоНоменклатуре", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("ЕстьУточненияПоСегментамНоменклатуры", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("ЕстьУточненияПоЦеновымГруппам", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("НачислятьБонусыДоСкидок", Ложь);
	ОписаниеБонуснойПрограммы.Вставить("Назначение", ПредопределенноеЗначение("Перечисление.НазначенияАвтоматическихСкидок.Везде"));
	ОписаниеБонуснойПрограммы.Вставить("КурсБонусов", 1);
	ОписаниеБонуснойПрограммы.Вставить("КоличествоОграниченияОстатка", 0);
	
	Возврат ОписаниеБонуснойПрограммы;
...
Функция НастройкиБонуснойПрограммы(БонуснаяПрограмма) Экспорт
	...
	|	ВЫБОР
	|		КОГДА БонусныеПрограммы.ИспользоватьОграничениеОстатка
	|			ТОГДА БонусныеПрограммы.КоличествоОграниченияОстатка
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоОграниченияОстатка
	|ИЗ
	|	Справочник.БонусныеПрограммы КАК БонусныеПрограммы
...
Функция ПолучитьМаксимальнуюСуммуОплатыДокумента(ТабличнаяЧасть, ДисконтнаяКарта) Экспорт
	...
	Для Каждого СуммаОплаты Из МаксимальныеСуммыОплаты Цикл
		...
		СуммаТовараСоСкидками = СтрокаТЧ.Сумма + СтрокаТЧ.СуммаСкидкиОплатыБонусом;
		ДоступноОплатить = Окр(СуммаОплаты.Значение * СуммаТовараСоСкидками / 100, 2);
...
Функция ПолучитьДвиженияПоБонусам(Документ, ПоДвижениям = Истина) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(Документ.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя")
			И НЕ РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(Документ.ВидОперации) Тогда
