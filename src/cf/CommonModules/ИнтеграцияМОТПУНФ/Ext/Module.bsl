...
//
Процедура ПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыПоискаРМК, ПараметрыСканирования) Экспорт
	...
	Если Не ПараметрыПоискаРМК = Неопределено Тогда
		...
		УпаковкаРучногоСопоставления = Неопределено;
		Если ПараметрыПоискаРМК.Свойство("ДанныеРучногоСопоставления") Тогда
			...
			Если ТипЗнч(ДанныеРучногоСопоставления.Упаковка) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
				...
				УпаковкаРучногоСопоставления = ДанныеРучногоСопоставления.Упаковка;
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.ШтрихкодEAN) И МассивШтрихкодов.Количество() > 0 Тогда
				НоваяСтрока.ШтрихкодEAN = МассивШтрихкодов[0].Штрихкод;
			КонецЕсли;
		...
		Если Не ЭтоШтрихкодНоменклатуры Тогда
			Для Каждого ШтрихкодМассива Из МассивШтрихкодов Цикл
				Если Лев(ШтрихкодМассива.Штрихкод, 2) = "29" Или Лев(ШтрихкодМассива.Штрихкод, 3) = "029" Тогда
		...
		ДанныеОсновнойНоменклатуры = Неопределено;
		СверятьСОсновнойНоменклатуройЧастичногоВыбытия = Ложь;
		Если Не ЭтоШтрихкодНоменклатуры И ЭтоМаркаКТекущейСтроке Тогда
			...
			УпаковкаРучногоСопоставления = ПараметрыПоискаРМК.ПараметрыДляОбработкиШтрихкода.ТекущаяНоменклатураУпаковка;
			ДанныеОсновнойНоменклатуры
				= ОбщегоНазначенияРМКУНФ.ДанныеОсновнойНоменклатурыЧастичногоВыбытия(ТекущаяНоменклатура, ТекущаяХарактеристика);
			СверятьСОсновнойНоменклатуройЧастичногоВыбытия = ЗначениеЗаполнено(ДанныеОсновнойНоменклатуры.Номенклатура);
		КонецЕсли;
		...
		Если Не ЭтоШтрихкодНоменклатуры И НайденаНоменклатура И ЭтоМаркаКТекущейСтроке Тогда
			СтрокиДляУдаления = Новый Массив;
			Для Каждого СтрокаТовара Из ДанныеПоШтрихкодамРезультат Цикл
				НоменклатураСоответствует = Ложь;
				Если СтрокаТовара.Номенклатура = ТекущаяНоменклатура
					И СтрокаТовара.Характеристика = ТекущаяХарактеристика 
					И СтрокаТовара.Упаковка = УпаковкаРучногоСопоставления Тогда
						НоменклатураСоответствует = Истина;
				КонецЕсли;
				Если СверятьСОсновнойНоменклатуройЧастичногоВыбытия
					И СтрокаТовара.Номенклатура = ДанныеОсновнойНоменклатуры.Номенклатура
					И СтрокаТовара.Характеристика = ДанныеОсновнойНоменклатуры.Характеристика Тогда
						НоменклатураСоответствует = Истина;
				КонецЕсли;
					
				Если Не НоменклатураСоответствует Тогда
					СтрокиДляУдаления.Добавить(СтрокаТовара);
				КонецЕсли;
			КонецЦикла;
			
			Если ДанныеПоШтрихкодамРезультат.Количество() = СтрокиДляУдаления.Количество() Тогда
				СтрокиДляУдаления.Удалить(0);
			КонецЕсли;
			
			Для Каждого СтрокаДляУдаления Из СтрокиДляУдаления Цикл
				ДанныеПоШтрихкодамРезультат.Удалить(СтрокаДляУдаления);
			КонецЦикла;
		КонецЕсли;
		
		ЗапросОписанияНоменклатуры = Новый Запрос(
			"ВЫБРАТЬ
			...
			|	ОписаниеНоменклатурыИС.ПотребительскаяУпаковка КАК Упаковка
			|ИЗ
		...
		Если Не ЭтоШтрихкодНоменклатуры И Не ЭтоМаркаКТекущейСтроке Тогда
			...
			Если НЕ ДанныеРазбораКода = Неопределено Тогда
				ШтрихкодМарки = Справочники.ШтрихкодыУпаковокТоваров.ПолучитьПоЗначениюШтрихкода(ДанныеРазбораКода.НормализованныйКодМаркировки);
				
				ДанныеВскрытыхУпаковок = РегистрыСведений.ВскрытыеПотребительскиеУпаковкиИС.
					ВскрытыеПотребительскиеУпаковкиПоКодуМаркировки(ШтрихкодМарки, Истина);
					
				ДанныеВскрытия = ДанныеВскрытыхУпаковок[ШтрихкодМарки];
				Если ДанныеВскрытия <> Неопределено Тогда
					ЗапросОписанияНоменклатуры.УстановитьПараметр("Номенклатура", ШтрихкодМарки.Номенклатура);
					Если ЗапросОписанияНоменклатуры.Выполнить().Выбрать().Количество() > 0 Тогда
		...
		ПараметрыПоискаРМК.ФильтрПоискаПоШтрихкоду.Вставить("ПриоритетШтрихкодаУпаковок", Истина);
		...
		Если Не ЭтоШтрихкодНоменклатуры И ДанныеПоШтрихкодамРезультат.Количество() > 0 Тогда
			Если ДанныеШтрихкодаРМК.ЗначенияПоиска.Количество() > 0 Тогда
				ШтрихкодыЧастичногоВыбытияОднойКарточкой = Новый Массив;
				ШтрихкодыДляУдаления = Новый Массив;
				ИндексУдаления = 0;
				Для Каждого ЗначениеПоиска Из ДанныеШтрихкодаРМК.ЗначенияПоиска Цикл
					Если ЗначениеПоиска.ТипОбъекта = Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров")
						...
						Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗначениеПоиска.Владелец, "ТипУпаковки") = Перечисления.ТипыУпаковок.МонотоварнаяУпаковка Тогда
							ШтрихкодыЧастичногоВыбытияОднойКарточкой.Добавить(ЗначениеПоиска);
						Иначе
							Если УпаковкаРучногоСопоставления = Неопределено Тогда
								ШтрихкодыЧастичногоВыбытияОднойКарточкой.Добавить(ЗначениеПоиска);
							Иначе
								Если УпаковкаРучногоСопоставления = ЗначениеПоиска.Упаковка Тогда
									ШтрихкодыЧастичногоВыбытияОднойКарточкой.Добавить(ЗначениеПоиска);
								Иначе
									ШтрихкодыДляУдаления.Добавить(ИндексУдаления);
								КонецЕсли;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				...
				Для каждого ИндексУдаления Из ШтрихкодыДляУдаления Цикл
					ДанныеШтрихкодаРМК.ЗначенияПоиска.Удалить(ИндексУдаления);
				КонецЦикла;
				
				Если ДанныеШтрихкодаРМК.ЗначенияПоиска.Количество() = 1 И ДанныеШтрихкодаРМК.Свойство("ИспользоватьНовыйАлгоритмПоискаПоШтрихкоду") Тогда
					ДанныеШтрихкодаРМК.Удалить("ИспользоватьНовыйАлгоритмПоискаПоШтрихкоду");
				КонецЕсли;
				
				Если ШтрихкодыЧастичногоВыбытияОднойКарточкой.Количество() <> 0 Тогда
					ДанныеПоШтрихкодамРезультат.Очистить();
					
					Для каждого ЗначениеПоиска Из ШтрихкодыЧастичногоВыбытияОднойКарточкой Цикл
						СтрокаДанных = ДанныеПоШтрихкодамРезультат.Добавить();
						Если МассивШтрихкодов.Количество() > 0 Тогда
							СтрокаДанных.ШтрихкодEAN = МассивШтрихкодов[0].Штрихкод;
						КонецЕсли;
						...
						СтрокаДанных.Характеристика = ЗначениеПоиска.Характеристика;
						СтрокаДанных.Упаковка = ЗначениеПоиска.Упаковка;
						СтрокаДанных.Серия = ЗначениеПоиска.СерияНоменклатуры;
						НайденаНоменклатура = Истина;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		...
		Если Не ЭтоШтрихкодНоменклатуры И НайденаНоменклатура И ЭтоМаркаКТекущейСтроке Тогда
			...
			Для Каждого СтрокаТовара Из ДанныеПоШтрихкодамРезультат Цикл
				НоменклатураСоответствует = Ложь;
				Если СтрокаТовара.Номенклатура = ТекущаяНоменклатура
					И СтрокаТовара.Характеристика = ТекущаяХарактеристика Тогда
						НоменклатураСоответствует = Истина;
				КонецЕсли;
				Если СверятьСОсновнойНоменклатуройЧастичногоВыбытия
					И СтрокаТовара.Номенклатура = ДанныеОсновнойНоменклатуры.Номенклатура
			...
			Если ДанныеПоШтрихкодамРезультат.Количество() = 0 Тогда
				ДанныеШтрихкодаРМК.ПараметрыДляОбработкиШтрихкода.Вставить("НоменклатураПоМаркеОтличается", Истина);
				НайденаНоменклатура = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		//Корректировка для частичного выбытия
		Если НайденаНоменклатура И Не НайденоНесколькоПозиций И ЭтоМаркаКТекущейСтроке И Не ЭтоШтрихкодНоменклатуры 
			И ЗначениеЗаполнено(УпаковкаРучногоСопоставления) Тогда
			
			ЗначенияПоиска = ДанныеШтрихкодаРМК.ЗначенияПоиска;
			Если ЗначенияПоиска.Количество() = 1 Тогда
				Если ЗначенияПоиска[0].ТипОбъекта = Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров") Тогда
					ШтрихкодМарки = ЗначенияПоиска[0].Владелец;
					
					ДанныеВскрытыхУпаковок = РегистрыСведений.ВскрытыеПотребительскиеУпаковкиИС.
						ВскрытыеПотребительскиеУпаковкиПоКодуМаркировки(ШтрихкодМарки, Истина);
						
					ОстатокЧастичногоВыбытия = Неопределено;
					ДанныеВскрытия = ДанныеВскрытыхУпаковок[ШтрихкодМарки];
					Если ДанныеВскрытия <> Неопределено Тогда
						ПараметрыПоискаРМК.ПараметрыДляОбработкиШтрихкода.Вставить("МаркаРанееВыбывалаЧастично", Истина);
						ДанныеШтрихкодаРМК.ПараметрыДляОбработкиШтрихкода.Вставить("НоменклатураПоМаркеОтличается", Истина);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Если НайденаНоменклатура Тогда
			НоменклатураПоиска = ДанныеПоШтрихкодамРезультат.ВыгрузитьКолонку("Номенклатура");
			ЗапросОписанияНоменклатуры.УстановитьПараметр("Номенклатура", НоменклатураПоиска);
			Результат = ЗапросОписанияНоменклатуры.Выполнить().Выгрузить();
			Для каждого ПозицияПоиска Из Результат Цикл
				Отбор = Новый Структура();
				Отбор.Вставить("Номенклатура", ПозицияПоиска.Номенклатура);
				СтрокиТоваров = ДанныеПоШтрихкодамРезультат.НайтиСтроки(Отбор);
				Для каждого СтрокаТовара Из СтрокиТоваров Цикл
					Если СтрокаТовара.Упаковка = ПозицияПоиска.Упаковка Тогда
...
Процедура ЕстьТабачнаяПродукцияВКоллекции(Коллекция, ЕстьТабачнаяПродукция) Экспорт
	...
	|ГДЕ
	|	СправочникНоменклатура.УдалитьТабачнаяПродукция";
	Результат = Запрос.Выполнить();
...
Процедура ПриОпределенииПараметровИнтеграцииФормыПроверкиИПодбора(Форма, ПараметрыИнтеграции, ВидПродукции) Экспорт
	...
	Если СтрНайти(Форма.ИмяФормы, "ЕГАИС") = 0 И СтрНайти(Форма.ИмяФормы, "ИСМП") = 0 Тогда
...
Процедура СформироватьТаблицуТабачнойПродукцииДокумента(Документ, ТаблицаПродукции) Экспорт
	...
	|			И (СправочникНоменклатура.УдалитьТабачнаяПродукция)");
...
Функция ПроверитьМаркируемуюПродукциюДокумента(Ссылка) Экспорт
	...
	Если (ТипДокумента = Тип("ДокументСсылка.РасходнаяНакладная") И НЕ СуществуетОтгрузкаТоваровИСМПВведеннаяНаОснованииДокумента(Ссылка))
		...
		Результат = ЭлектронноеВзаимодействиеИСМП.ДанныеДокументаСоответствуютДаннымУпаковок(Ссылка);
	КонецЕсли;
...
Процедура ОтразитьРезультатыПроверкиИПодбораВДокументеПоступления(ПараметрыОкончанияСканирования)
	...
	Если Не ПараметрыОкончанияСканирования.СоздаватьАктОРасхождениях Тогда
		...
		ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерии(ДокументОбъект, ПараметрыОкончанияСканирования, ИмяТЧЗапасы,, Ложь);
...
//
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерии(ДокументОбъект, ПараметрыОкончанияСканирования, ИмяТЧЗапасы = "Запасы", ИмяТЧСерии = "СерииНоменклатуры", ЭтоИсходящийДокумент = Истина)
	
	ТаблицаПодобраннойПровереннойПродукции = ПараметрыОкончанияСканирования.ТаблицаПодобраннойПровереннойПродукции;
	РазрешенОборотНемаркируемыхОстатков = ОбщегоНазначенияИСМППовтИсп.РазрешенОборотНемаркируемыхОстатковПродукции(ПараметрыОкончанияСканирования.ВидПродукцииИС, ДокументОбъект.Дата);
	...
	Пока ВыборкаРасхождения.Следующий() Цикл
		...
		Если Недостача > 0 Тогда
			...
			// если разрешен оборот немаркируемой продукции, то не уменьшать количество в исходном документе
			Если РазрешенОборотНемаркируемыхОстатков И ЭтоИсходящийДокумент Тогда
				Продолжить;
			КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ИзмененныеСтроки Цикл
		...
		Иначе
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицыТовары, "Сумма") Тогда
