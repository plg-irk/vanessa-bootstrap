...
Процедура ПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыПоискаРМК = Неопределено, ПараметрыСканирования = Неопределено) Экспорт
	...
	ИнтеграцияМОТПУНФ.ПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыПоискаРМК, ПараметрыСканирования);
...
Процедура ПриПроверкеДанныхПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			...
			Если СтрокаВнешнихДанных.Свойство("ДанныеПО") И СтрокаВнешнихДанных.ДанныеПО <> Штрихкод Тогда
				...
				Для Каждого ЗначениеПоиска Из СтрокаВнешнихДанных.ЗначенияПоиска Цикл
					...
					Если ЗначениеПоиска.Свойство("СтрокаТовара")
						...
						Если ПараметрыДляОбработкиШтрихкода.Свойство("ДозаполнитьПоВосстановленнойМарке") Тогда
							СтрокаТовары = ЗначениеПоиска.СтрокаТовара;
							ШтрихкодированиеРМК.РазобратьМаркуИЗаполнитьДанные(СтрокаТовары, Штрихкод, СтрокаТовары.ОсобенностьУчета,
					...
					Если ЗначениеПоиска.Свойство("СтруктураНайденнойНоменклатуры")
						...
						Если ПараметрыДляОбработкиШтрихкода.Свойство("ДозаполнитьПоВосстановленнойМарке") Тогда
							ЗаполнитьЗначенияСвойств(ЗначениеПоиска.СтруктураНайденнойНоменклатуры, ЗначениеПоиска.СтрокаТовара,
		...
		СтрокаДанных.ТребуетВзвешивания = Ложь;
	...
	Если ДобавитьВнешниеДанные Тогда
		...
		СтатусАкцизнойМаркиНеопределен = Ложь;
		Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			Если СтрокаДанных.ВнешниеДанныеПоШтрихкодам.Количество() = 0 Тогда
				СтрокаДанных.ВнешниеДанныеПоШтрихкодам.Добавить(ДанныеШтрихкодаРМК);
			КонецЕсли;
			
			Если ЕстьОшибки И СтрокаДанных.Статус = Неопределено
				И СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Алкогольная
				И СтрокаДанных.ТекстОшибки = НСтр("ru = 'Статус: <Нет статуса>'") Тогда
					СтатусАкцизнойМаркиНеопределен = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если СтатусАкцизнойМаркиНеопределен Тогда
			ОпределятьСтатусАкцизнойМарки = Ложь;
			// ОрганизацияЕГАИС и ККТ товара
				...
				Если ЗначениеПоискаПоШК.Свойство("СтруктураНайденнойНоменклатуры") Тогда
					...
					Если ПараметрыПоискаРМК.Свойство("СоответствияККТОрганизаций") Тогда
						...
						Если ЗначениеЗаполнено(ККТОрганизации) Тогда
							...
							Если Не ПараметрыСканирования.ОрганизацияЕГАИС = ПараметрыКассы.ОрганизацияЕГАИС Тогда
								ОпределятьСтатусАкцизнойМарки = Истина;
								ПараметрыСканирования.ОрганизацияЕГАИС = ПараметрыКассы.ОрганизацияЕГАИС;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			...
			Если ОпределятьСтатусАкцизнойМарки Тогда
				ЕстьОшибки = Ложь;
				Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
