...
//
// Параметры:
//   Заказ                    - ДокументСсылка.ЗаказПокупателя - проверяемый заказ.
...
Функция РезультатПроверкиЗаказа(Заказ, РезультатПроверкиЗаказов) Экспорт
	...
	Если РезультатПроверкиЗаказа = Неопределено Тогда
		...
		РезультатПроверкиЗаказа.Вставить("ХозяйственнаяОперация",               ПредопределенноеЗначение( "Справочник.ХозяйственныеОперации.ПустаяСсылка" ));
...
Функция ШаблонНавигационнойСсылкиДокументаОтгрузки(Знач Документ, Знач Статус) Экспорт
	...
	ЭтоРеализация = (ТипЗнч(Документ) = Тип("ДокументСсылка.РасходнаяНакладная"));
...
Функция ОтчетОРеализацииТоваровПараметрыОтчетаНовый() Экспорт
	
	Результат = новый Структура;
	Результат.Вставить( "УчетнаяЗаписьМаркетплейса", Неопределено );
