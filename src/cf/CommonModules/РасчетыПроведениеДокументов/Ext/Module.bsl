...
// 	СоответствиеТабличныхЧастейИРеквизитаЗаказ - Соответствие из КлючИЗначение - .
// 	СоответствиеВременныхТаблицИРеквизитаЗаказ - Соответствие из КлючИЗначение - .
Процедура ИнициализироватьДополнительныеСвойстваДляПроведения(ДокументОбъект, СтруктураДополнительныеСвойства, Отказ,
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОбъект, "ВалютаДокумента") Тогда
		ВалютаДокумента = ДокументОбъект.ВалютаДокумента;
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОбъект, "ВалютаДенежныхСредств") Тогда
		ВалютаДокумента = ДокументОбъект.ВалютаДенежныхСредств;
	Иначе
	...
	Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.РасходИзКассы") Тогда
		Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
			Итог = ДокументОбъект.РасшифровкаПлатежа.Итог("СуммаПлатежа");
		Иначе
			Итог = 0;
		КонецЕсли;
		ОстатокСуммы = ДокументОбъект.СуммаДокумента - Итог;
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ОстатокСуммыДляПлатежногоКалендаря", ОстатокСуммы);
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ТЧВыплатаЗаработнойПлатыКоличествоСтрок",
			ДокументОбъект.ВыплатаЗаработнойПлаты.Количество());
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ТЧВыплатаРегламентированнойЗаработнойПлатыКоличествоСтрок",
			ДокументОбъект.ВыплатаРегламентированнойЗаработнойПлаты.Количество());
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ПроектВШапке",
			ДокументОбъект.ПоложениеПроекта = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ПодразделениеВШапке",
			ДокументОбъект.ПоложениеПодразделения = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("НаправлениеДеятельности",
			ДокументОбъект.НаправлениеДеятельности);
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ПериодРегистрации", ДокументОбъект.ПериодРегистрации);
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ВидНалога", ДокументОбъект.ВидНалога);
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("СрокУплаты", ДокументОбъект.СрокУплаты);
		Если ЗначениеЗаполнено(ДокументОбъект.ДокументОснование) Тогда
			ДатаДокументаОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.ДокументОснование, "Дата");
		Иначе
			ДатаДокументаОснования = Дата('00010101');
		КонецЕсли;
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ДатаДокументаОснования", ДатаДокументаОснования);
		Если ЗначениеЗаполнено(ДокументОбъект.ДоговорКредитаЗайма) Тогда
			ВалютаДоговораКредитаЗайма = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.ДоговорКредитаЗайма, "ВалютаРасчетов");
		Иначе
			ВалютаДоговораКредитаЗайма = Справочники.Валюты.ПустаяСсылка();
		КонецЕсли;
		СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ВалютаДоговораКредитаЗайма", ВалютаДоговораКредитаЗайма);
		
		ЭтоОплата = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику Или ДокументОбъект.ВидОперации
			= Перечисления.ВидыОперацийРасходИзКассы.НашейОрганизации;
		ЭтоВозвратОплаты = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Покупателю;
		ЭтоРасчетыСПоставщиком = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику
			ИЛИ ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.НашейОрганизации;
	...
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.РасходСоСчета") Тогда
		ЭтоОплата = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику Или ДокументОбъект.ВидОперации
			= Перечисления.ВидыОперацийРасходСоСчета.НашейОрганизации;
		ЭтоВозвратОплаты = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю;
		ЭтоРасчетыСПоставщиком = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику
		ИЛИ ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.НашейОрганизации;
		Если Не ЭтоОплата И Не ЭтоВозвратОплаты Тогда
			СтруктураДополнительныеСвойства.Вставить("ПараметрыДокумента", ПараметрыДокумента);
			Возврат;
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ПоступлениеВКассу") Тогда
		ЭтоОплата = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя
			Или ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтНашейОрганизации;
		ЭтоВозвратОплаты = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПоставщика;
		ЭтоРасчетыСПоставщиком = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПоставщика;
		Если Не ЭтоОплата И Не ЭтоВозвратОплаты Тогда
			СтруктураДополнительныеСвойства.Вставить("ПараметрыДокумента", ПараметрыДокумента);
			Возврат;
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ПоступлениеНаСчет") Тогда
		ЭтоОплата = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя
			Или ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтНашейОрганизации;
		ЭтоВозвратОплаты = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика;
		ЭтоРасчетыСПоставщиком = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика;
		Если ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты
			И ДокументОбъект.РасшифровкаПлатежа.Количество() Тогда
			РасшифровкаПлатежаДоговор = ДокументОбъект.РасшифровкаПлатежа[0].Договор;
			ПереноситьУдержанныеСуммыНаРасчетыСПоставщиком = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				РасшифровкаПлатежаДоговор, "ПереноситьУдержанныеСуммыНаРасчетыСПоставщиком");
			Если ПереноситьУдержанныеСуммыНаРасчетыСПоставщиком Тогда
				ЭтоДоставка = Истина;
			КонецЕсли;
		КонецЕсли;
		Если Не ЭтоОплата И Не ЭтоВозвратОплаты Тогда
			...
			Возврат;
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ОперацияПоПлатежнымКартам") Тогда
		ЭтоОплата =
			ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийЭквайринга.ПоступлениеОплатыОтПокупателя
				Или ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП;
		ЭтоВозвратОплаты = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю;
		Если Не ЭтоОплата И Не ЭтоВозвратОплаты Тогда
			СтруктураДополнительныеСвойства.Вставить("ПараметрыДокумента", ПараметрыДокумента);
			Возврат;
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		ЭтоЗаказ = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу;
		ЭтоЗаказНаряд = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд;
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		ЭтоКорректировка = Истина;
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ЧекККМ") Тогда
		Если ДокументОбъект.ОперацияСДенежнымиСредствами Тогда
			ЭтоОплата = Истина;
		КонецЕсли;
		ЭтоЧек = Истина;
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ЧекККМВозврат") Тогда
		Если ДокументОбъект.ОперацияСДенежнымиСредствами Тогда
			ЭтоВозвратОплаты = Истина;
		КонецЕсли;
		ЭтоЧек = Истина;
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") Тогда
	...
	Если ПараметрыДокумента.ЭтоОтгрузка Тогда
		...
		СтруктураДополнительныеСвойства.Вставить("КурсДокумента", ДокументОбъект.Курс);
		СтруктураДополнительныеСвойства.Вставить("КратностьДокумента", ДокументОбъект.Кратность);
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОбъект, "СпособЗачетаПредоплаты") Тогда
			СтруктураДополнительныеСвойства.Вставить("СпособЗачетаПредоплаты", ДокументОбъект.СпособЗачетаПредоплаты);
		Иначе
		...
		СтруктураДополнительныеСвойства.Вставить("Договор", ДокументОбъект.Договор);

	КонецЕсли;

	Если ЭтоЧек Тогда
		СтруктураДополнительныеСвойства.Вставить("Контрагент", ДокументОбъект.Контрагент);
		СтруктураДополнительныеСвойства.Вставить("Договор", ДокументОбъект.Договор);
	КонецЕсли;
...
//   * Отказ - Булево -
// 	ПараметрДата - Неопределено - Описание
Процедура ЗаполнитьКурсВалютыУчетаИРегВалюты(СтруктураДополнительныеСвойства, ПараметрДата = Неопределено) Экспорт
	...
	Иначе
		Запрос.УстановитьПараметр("Период", ПараметрДата);
	КонецЕсли;
...
// 
// Параметры:
//  СтрокаТабличнойЧасти - ДанныеФормыСтруктура - строка табличной части 
...
// Параметры:
//  ДокументОбъект - ДокументОбъект
Процедура ЗаполнитьСпособЗачетаВРасшифровкеПлатежа(ДокументОбъект) Экспорт
	
	Если ЗначениеЗаполнено(ДокументОбъект.ДокументОснование)
		И ТипЗнч(ДокументОбъект.ДокументОснование) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ТекущаяСтрока Из ДокументОбъект.РасшифровкаПлатежа Цикл
		...
		ИначеЕсли ТекущаяСтрока.СпособЗачета.Пустая() Тогда
			ТекущаяСтрока.СпособЗачета = РасчетыРаботаСФормамиВызовСервера.ПолучитьСпособЗачетаДляДоговора(
				ТекущаяСтрока.Договор, ДокументОбъект.Контрагент, Ложь);
		КонецЕсли;
	КонецЦикла;
...
// Параметры:
// 	ДокументОбъект - ДокументОбъект
Процедура ОбработатьРасшифровкуПлатежаПередЗаписью(ДокументОбъект) Экспорт
	
	ЗаполнятьРеквизитыВзаиморасчетов = ЭтоВидОперацииВзаиморасчетов(ДокументОбъект.ВидОперации);
	...
	Для Каждого ТекущаяСтрока Из ДокументОбъект.РасшифровкаПлатежа Цикл
		...
		Если ЗаполнятьРеквизитыВзаиморасчетов Тогда
			...
			ИначеЕсли ТекущаяСтрока.СпособЗачета = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Вручную
				...
				И ДокументОбъект.Контрагент.ВестиРасчетыПоДокументам Тогда
...
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.АктВыполненныхРабот, ДокументСсылка.ЗаказПокупателя -
//                 - ДокументСсылка.ПриходнаяНакладная, ДокументСсылка.РасходнаяНакладная - 
//  СтруктураДополнительныеСвойства - Структура - 
//  МассивДоступныхОпераций - Массив из ПеречислениеСсылка - 
Процедура СформироватьТаблицаОплатаСчетовИЗаказов(ДокументСсылка, СтруктураДополнительныеСвойства,
	МассивДоступныхОпераций) Экспорт
	
	Если МассивДоступныхОпераций <> Неопределено Тогда
		ВидОперации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ВидОперации");
		Если МассивДоступныхОпераций.Найти(ВидОперации) = Неопределено Тогда
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаОплатаСчетовИЗаказов",
...
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.АвансовыйОтчет, ДокументСсылка.ОперацияПоПлатежнымКартам - 
//                 - ДокументСсылка.ПоступлениеВКассу, ДокументСсылка.ПоступлениеНаСчет, ДокументСсылка.РасходИзКассы -
//                 - ДокументСсылка.РасходСоСчета, ДокументСсылка.ЧекККМ, ДокументСсылка.ЧекККМВозврат -
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства
//  МассивДоступныхОпераций - Массив из ПеречислениеСсылка
//  КонтрагентВДвижениях - Булево - Контрагент в движениях
Процедура СформироватьТаблицаОплатаДокументовДенежныеДокументы(ДокументСсылка, СтруктураДополнительныеСвойства,
	МассивДоступныхОпераций, КонтрагентВДвижениях = Ложь) Экспорт
	
	ВидОперацииДокумента = ВидОперацииДокумента(ДокументСсылка);
	
	Если МассивДоступныхОпераций <> Неопределено
		И МассивДоступныхОпераций.Найти(ВидОперацииДокумента) = Неопределено Тогда
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Контрагент", ДокументСсылка.Метаданные()) Тогда
		ВестиРасчетыПоДокументам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка,
			"Контрагент.ВестиРасчетыПоДокументам");
	КонецЕсли;
	
	Если НЕ КонтрагентВДвижениях И ВестиРасчетыПоДокументам <> Истина Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаОплатаДокументов", Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ВидОперацииДокумента = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтНашейОрганизации
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийПоступлениеВКассу.ОтНашейОрганизации
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийРасходИзКассы.Покупателю
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийРасходСоСчета.Покупателю
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийЭквайринга.ПоступлениеОплатыОтПокупателя
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП
		ИЛИ ВидОперацииДокумента = Перечисления.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю Тогда
	...
	Для Каждого ТекущаяСтрока Из ТекущаяТаблицаРасчетов Цикл
		...
		Если Коэффициент <> 0 Тогда
			...
			ТипПоляДокумент = ТипЗнч(ТекущаяСтрока.Документ);
			Если Не Метаданные.РегистрыНакопления.ОплатаДокументов.Измерения.Документ.Тип.СодержитТип(ТипПоляДокумент) Тогда
				Продолжить;
			КонецЕсли;
			
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВидОперации", ТекущаяСтрока.Документ.Метаданные()) Тогда
				//@skip-check bsl-legacy-check-string-literal
				ВидОперации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Документ, "ВидОперации");
				...
				// Возвраты авансов не должны делать записи в регистре Оплата документов	
			...
			Если ТипПоляДокумент = Тип("ДокументСсылка.Взаимозачет") Тогда
...
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства
//  МассивДоступныхОпераций - Массив из ПеречислениеСсылка
Процедура СформироватьТаблицаОплатаДокументовНакладные(ДокументСсылка, СтруктураДополнительныеСвойства,
	МассивДоступныхОпераций) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") Тогда
		КонтрагентПолучательВестиРасчетыПоДокументам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка,
			"КонтрагентПолучатель.ВестиРасчетыПоДокументам");
		Если КонтрагентПолучательВестиРасчетыПоДокументам <> Истина Тогда
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаОплатаДокументов",
				Новый ТаблицаЗначений);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(ДокументСсылка) <> Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") Тогда
		Если ВестиРасчетыПоДокументам <> Истина Тогда
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаОплатаДокументов",
				Новый ТаблицаЗначений);
			Возврат;
		КонецЕсли;
...
// Параметры:
//  ДокументСсылка - ДокументСсылка
//  СтруктураДополнительныеСвойства - Структура - Структура дополнительные свойства
...
// Сформировать движения по взаиморасчетам.
// 
// Параметры:
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства
//  ДокументСсылка - ДокументСсылка
//  ЗапросПараметры - Неопределено - Запрос параметры
//  ТребуемыеРеквизитыШапки - Строка - Требуемые реквизиты шапки
Процедура СформироватьДвиженияПоВзаиморасчетам(СтруктураДополнительныеСвойства, ДокументСсылка,
	ЗапросПараметры = Неопределено, ТребуемыеРеквизитыШапки = "") Экспорт
...
// Сформировать движения по взаиморасчетам оплата доставка.
// 
// Параметры:
//  ДокументСсылкаПоступлениеНаСчет - ДокументСсылка.ПоступлениеНаСчет
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства
//  ЗапросПараметры - Неопределено - Запрос параметры
//  ТребуемыеРеквизитыШапки - Строка - Требуемые реквизиты шапки
Процедура СформироватьДвиженияПоВзаиморасчетамОплатаДоставка(ДокументСсылкаПоступлениеНаСчет,
	СтруктураДополнительныеСвойства, ЗапросПараметры = Неопределено, ТребуемыеРеквизитыШапки = "") Экспорт
	
	Запрос = Новый Запрос;
	
	РеквизитыПоступленияНаСчет = Новый Массив();
	РеквизитыПоступленияНаСчет.Добавить("Дата");
	РеквизитыПоступленияНаСчет.Добавить("ДоговорКредитаЗайма");
	РеквизитыПоступленияНаСчет.Добавить("ВалютаДенежныхСредств");
	РеквизитыПоступленияНаСчет.Добавить("ХозяйственнаяОперация");
	
	СвойстваПоступленияНаСчет = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылкаПоступлениеНаСчет,
		РеквизитыПоступленияНаСчет);
	
	ВалютаДоговораКредитаЗайма = Справочники.Валюты.ПустаяСсылка();
	Если ЗначениеЗаполнено(СвойстваПоступленияНаСчет.ДоговорКредитаЗайма) Тогда
		ВалютаДоговораКредитаЗайма = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			СвойстваПоступленияНаСчет.ДоговорКредитаЗайма, "ВалютаРасчетов");
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ВалютаДС", СвойстваПоступленияНаСчет.ВалютаДенежныхСредств);
	Запрос.УстановитьПараметр("ВалютаДоговораКредитаЗайма", ВалютаДоговораКредитаЗайма);
	...
	Запрос.УстановитьПараметр("Период", СвойстваПоступленияНаСчет.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ХозяйственнаяОперация", СвойстваПоступленияНаСчет.ХозяйственнаяОперация);
	Запрос.УстановитьПараметр("ВалютаДокумента", СвойстваПоступленияНаСчет.ВалютаДенежныхСредств);
	Запрос.УстановитьПараметр("НациональнаяВалюта", СтруктураДополнительныеСвойства.НациональнаяВалюта);
	Запрос.УстановитьПараметр("ВалютаУчета", СтруктураДополнительныеСвойства.ВалютаУчета);
	Запрос.УстановитьПараметр("СодержаниеПроводки", НСтр(
		"ru='Увеличение долга поставщика перед нами на удержанную сумму'"));
		
	Результат = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ПараметрыДокумента.Вставить("ТаблицаДвиженийПоВзаиморасчетам", Результат.Выгрузить());
	
	СформироватьТаблицуРасчетовДляФормированияДвижений(СтруктураДополнительныеСвойства,
		ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства,
		СтруктураДополнительныеСвойства.ПараметрыДокумента.ТаблицаДвиженийПоВзаиморасчетам, ТребуемыеРеквизитыШапки,
		ЗапросПараметры);
	
КонецПроцедуры

// Взаимозачет добавить движения в таблицу расчетов.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства
Процедура ВзаимозачетДобавитьДвиженияВТаблицуРасчетов(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	СвойстваДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Контрагент, Организация");
	...
	Запрос.УстановитьПараметр("Контрагент", СвойстваДокумента.Контрагент);
	Запрос.УстановитьПараметр("Организация", Константы.УчетПоКомпании.Компания(СвойстваДокумента.Организация));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
...
// Подготовить таблицу взаиморасчетов с учетом авансов.
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ДополнительныеРасходы, ДокументСсылка.ОтчетКомиссионера -
//                 - ДокументСсылка.ОтчетКомитенту, ДокументСсылка.ОтчетОПереработке -
//                 - ДокументСсылка.ОтчетПереработчика, ДокументСсылка.ПередачаВА -
//                 - ДокументСсылка.ПриемИПередачаВРемонт -
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства
//  ЭтоРасчетыСПоставщиком - Булево 
Процедура ПодготовитьТаблицуВзаиморасчетовСУчетомАвансов(ДокументСсылка, СтруктураДополнительныеСвойства,
	ЭтоРасчетыСПоставщиком) Экспорт
	...
	ИмяВременнойТаблицыРасчетов = ?(ЭтоРасчетыСПоставщиком, "ВременнаяТаблицаРасчетыСПоставщиками",
		"ВременнаяТаблицаРасчетыСПокупателями");
	ТаблицаВзаиморасчетов = ПолучитьВременнуюТаблицуПоИмени(ИмяВременнойТаблицыРасчетов,
	...
	СвойстваДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка,
		"ВалютаДокумента, Договор, Контрагент");
	ДоговорВалютаРасчетов = Справочники.Валюты.ПустаяСсылка();
	Если ЗначениеЗаполнено(СвойстваДокумента.Договор) Тогда
		ДоговорВалютаРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СвойстваДокумента.Договор, "ВалютаРасчетов");
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Регистратор", ДокументСсылка);
	Запрос.УстановитьПараметр("Договор", СвойстваДокумента.Договор);
	Запрос.УстановитьПараметр("Контрагент", СвойстваДокумента.Контрагент);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	...
	Для каждого СтрокаРучногоАванса Из СтрокиАванса Цикл
		...
		Для каждого СтрокаОстатка Из МассивОстатков Цикл
			...
			Иначе
				...
				ИначеЕсли СтруктураДополнительныеСвойства.НациональнаяВалюта = ДоговорВалютаРасчетов Тогда
	...
	СтрокиЗакрытогоДолга = ТаблицаВзаиморасчетов.НайтиСтроки(Новый Структура("ТипРасчетов, ВидДвижения",
		Перечисления.ТипыРасчетов.Долг, ВидДвиженияНакопления.Расход));
	...
	СтрокиДолга = ТаблицаВзаиморасчетов.НайтиСтроки(Новый Структура("ТипРасчетов, ВидДвижения",
		Перечисления.ТипыРасчетов.Долг, ВидДвиженияНакопления.Приход));
	Если СвойстваДокумента.ВалютаДокумента <> УправлениеНебольшойФирмойПовтИсп.ПолучитьНациональнуюВалюту() Тогда
		Для каждого СтрокаДолга Из СтрокиДолга Цикл
			Если СтрокаДолга.СуммаВал >= АвансСуммаВал Тогда
				...
				ОстатокДолгаРег = ?(СтрокаДолга.СуммаРег = 0, 0, ОстатокДолгаВал * (СтрокаДолга.СуммаРег
					/ СтрокаДолга.СуммаВал));
	...
	ВременныеТаблицы.Вставить("ВременнаяТаблицаПредоплата", ПолучитьВременнуюТаблицуПоИмени(
		"ВременнаяТаблицаПредоплата", СтруктураДополнительныеСвойства));
...
// Обновить суммы в национальной валюте.
// 
// Параметры:
//  НаборЗаписей - РегистрНакопленияНаборЗаписей
Процедура ОбновитьСуммыВНациональнойВалюте(НаборЗаписей) Экспорт
	
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьВалютуУчета()
		<> УправлениеНебольшойФирмойПовтИсп.ПолучитьНациональнуюВалюту() Тогда
		Возврат;
	КонецЕсли;
...
// Перед записью накладной.
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект
//  МассивДоступныхОпераций - Массив из ПеречислениеСсылка, Неопределено
Процедура ПередЗаписьюНакладной(ДокументОбъект, МассивДоступныхОпераций = Неопределено) Экспорт
	
	Если ДокументОбъект.СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто Тогда
		ДокументОбъект.Предоплата.Очистить();
	КонецЕсли;
	...
	Если МассивДоступныхОпераций.Найти(ДокументОбъект.ВидОперации) = Неопределено Тогда
		ДокументОбъект.Предоплата.Очистить();
	КонецЕсли;
...
// Обработать таблицу документов для изменения при оплате.
// 
// Параметры:
...
// Обработать таблицу документов для изменения при отгрузке.
// 
// Параметры:
//  ДополнительныеСвойства - Структура
//  Отказ - Булево
...
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СтатусОплаты - статус оплаты заказа
//
Функция СтатусОплаты(Заказ) Экспорт
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтатусОплаты.НеУчитывается)
	|		КОГДА (ДокументЗаказПокупателя.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаявкаНаПродажу)
	|				ИЛИ ДокументЗаказПокупателя.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаявкаЗаказНаряд))
	|				И ЕСТЬNULL(ФактОплаты.СуммаОплаты, 0) + ЕСТЬNULL(ФактОплаты.СуммаАванса, 0) = 0
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтатусОплаты.НеУчитывается)
...
Процедура УстановитьКурсИКратностьДляКорректировок(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ЭтоКорректировкаРеализации Тогда
		...
		ИсправляемыйДокументРеализации =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ИсправляемыйДокументРеализации");
		Если ЗначениеЗаполнено(ИсправляемыйДокументРеализации) Тогда
			ИсправляемыйДокумент = ИсправляемыйДокументРеализации;	
		КонецЕсли;
	КонецЕсли;
	...
	Если ЭтоКорректировкаПоступления Тогда
		...
		ИсправляемыйДокументПоступления =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ИсправляемыйДокументПоступления");
		Если ЗначениеЗаполнено(ИсправляемыйДокументПоступления) Тогда
			ИсправляемыйДокумент = ИсправляемыйДокументПоступления;
		КонецЕсли;
	КонецЕсли;
...
// 
// Параметры:
//  ИсправляемыйДокумент - ДокументСсылка.КорректировкаПоступления, ДокументСсылка.КорректировкаРеализации - 
// 
// Возвращаемое значение:
//  Структура:
//  * ВыполненаУстановкаКурсаДляКорректировок - Булево
//  * Курс - Число
//  * Кратность - Число
Функция КурсИКратностьДляКорректировок(ИсправляемыйДокумент) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ВыполненаУстановкаКурсаДляКорректировок", Ложь);
	...
	Если ЗначениеЗаполнено(ИсправляемыйДокумент) Тогда
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаИсправляемогоДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсправляемыйДокумент,
				"СуммаДокумента");
			Результат.Курс = Выборка.СуммаРегл / СуммаИсправляемогоДокумента;
			Результат.Кратность = 1;
			Результат.ВыполненаУстановкаКурсаДляКорректировок = Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьТекстЗапросаОстаткиАвансов(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	|		И НЕ ВТ_Предоплата.ОплатаСертификатом
	|СГРУППИРОВАТЬ ПО
...
Процедура ДобавитьТекстЗапросаРеквизитовРасчетов(ТекстЗапроса, ПолучатьРеквизитыРасчетов, УстановитьНеопределено, ЭтоРасчетыСПоставщиками = Ложь, РеквизитыРасчетовСоответствие = Неопределено) Экспорт
	...
	Если УстановитьНеопределено Тогда
		...
		Если РеквизитыРасчетовСоответствие = Неопределено Тогда
			...
			|	НЕОПРЕДЕЛЕНО КАК Курс,
			|	НЕОПРЕДЕЛЕНО КАК Кратность";
...
Процедура СформироватьДвиженияПоВзаиморасчетамДляЗаказа(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	|	И НЕ РасчетыСПокупателями.ОплатаСертификатом
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
...
Процедура СформироватьТаблицуРасчетовДляФормированияДвижений(СтруктураДанныхДокумента, ДокументСсылка, СтруктураДополнительныеСвойства, ТаблицаДвиженийПоВзаиморасчетам, ТребуемыеРеквизитыШапки = "", ЗапросПараметры = Неопределено)
	...
	Если ОсвобождатьАванс Тогда
		...
		|Где НЕ ТаблицаДвиженийПоВзаиморасчетам.ОплатаСертификатом
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	Контрагент,
		...
		|		ИНАЧЕ ТаблицаДвиженийПоВзаиморасчетам.СуммаРег
		|	КОНЕЦ <> 0)
		|	И НЕ ТаблицаДвиженийПоВзаиморасчетам.ОплатаСертификатом
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Функция ПересчитатьВалютныеСуммыДляДокументовОплаты(СуммаДляПересчета, СтруктураДополнительныеСвойства, СтрокаРасшифровки)
	...
	ИначеЕсли СтруктураДополнительныеСвойства.ВалютаДокумента = СтруктураДополнительныеСвойства.ВалютаУчета Тогда
		СтруктураСумм.Сумма = СтрокаРасшифровки.СуммаПлатежа;
	Иначе
	...
	ИначеЕсли СтруктураДополнительныеСвойства.ВалютаДокумента = СтруктураДополнительныеСвойства.ВалютаУчета Тогда
		СтруктураСумм.СуммаРег = СтрокаРасшифровки.СуммаПлатежа;
	Иначе
...
Процедура РаспределитьАванс(ДокументОбъект, СтрокиТЗАвансовДляЗачета, СтрокаКЗачету, ДополнительныеСвойства) Экспорт
...
Процедура ДобавитьСтрокуЗадолженности(ДокументОбъект, СтрокаКЗачету, ДополнительныеСвойства) Экспорт
...
// 
// Параметры:
// 	ДокументСсылка - ДокументСсылка - ссылка на документ, который проводится
// 	Запрос - Запрос - запрос инициализации данных документа
// 	СтруктураДополнительныеСвойства - Структура - дополнительные свойства проведения документа
//
Процедура МодифицироватьЗапросДляПересчетаВВалюту(ДокументСсылка, Запрос, СтруктураДополнительныеСвойства) Экспорт
	...
	Если СтруктураДополнительныеСвойства.ВалютаУчета = СтруктураДополнительныеСвойства.НациональнаяВалюта
		И ВалютаДокумента = СтруктураДополнительныеСвойства.НациональнаяВалюта
...
// 
// Параметры:
// 	ТаблицаРаспределения - ТаблицаЗначений - временная таблица, в которой были распределены суммы в разных валютах 
// 	ИмяТаблицыВЗапросе - Строка - имя таблицы, в которую необходимо поместить данные
// 	ДополнительныеСвойства - Структура - дополнительные свойства проведения документа
//а
...
// 
// Параметры:
//  ИмяТаблицы - Строка - имя временной таблицы в менеджере временных таблиц
//  ДополнительныеСвойства - Структура - данные, которые формируются при проведении документов
// 
// Возвращаемое значение:
//  ТаблицаЗначений 
...
// 
// Параметры:
//  СтруктураДополнительныеСвойства - Структура - данные, которые формируются при проведении документов
// 
// Возвращаемое значение:
//  ТаблицаЗначений
	...
	Если НЕ СтруктураДополнительныеСвойства.Свойство("БезУчетаАвансов") Тогда  // По взаиморасчетам
		...
		Если СтруктураДополнительныеСвойства.Свойство("РаспределятьВсюСумму") Тогда
			...
			Если ВВалютеРегламентированногоУчета Тогда
				ИмяКолонкиБазы = "ВсегоВалютаДокумента";
			Иначе
				ИмяКолонкиБазы = "СуммаРег";
			КонецЕсли;
			РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяТаблица.Итог("Сумма"), 	  ТаблицаСуммДокумента, "Сумма", 	ИмяКолонкиБазы);
			РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяТаблица.Итог("СуммаВал"), ТаблицаСуммДокумента, "СуммаВал", ИмяКолонкиБазы);
			РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяТаблица.Итог("СуммаРег"), ТаблицаСуммДокумента, "СуммаРег", ИмяКолонкиБазы);
...
// Получить данные для заполнения ТЧПредоплата.
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект
//  ЭтоРасчетыСПоставщиком - Булево
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Получить данные для заполнения ТЧПредоплата
Функция ПолучитьДанныеДляЗаполненияТЧПредоплата(ДокументОбъект, ЭтоРасчетыСПоставщиком)
	...
	Запрос.УстановитьПараметр("ТипРасчетовАванс", Перечисления.ТипыРасчетов.Аванс);
	Запрос.УстановитьПараметр("ПериодСрезаПоследних", ДокументОбъект.Дата);
	...
	Для Каждого ТекущаяСтрока Из ТЗПредоплата Цикл
		...
		Если ДокументОбъект.ВалютаДокумента = ТекущаяСтрока.НациональнаяВалюта Тогда
			Если ТекущаяСтрока.СуммаРег = 0 Тогда
				ТекущаяСтрока.Курс = ?(ДокументОбъект.Курс = 0 , 1, ДокументОбъект.Курс);
				ТекущаяСтрока.Кратность = ?(ДокументОбъект.Кратность = 0, 1, ДокументОбъект.Кратность);
		...
		Иначе // Валюта документа и валюта расчетов равны.
			...
			Иначе
				ТекущаяСтрока.Кратность = ?(ДокументОбъект.Кратность = 0, 1, ДокументОбъект.Кратность);
				ТекущаяСтрока.Курс = Окр(ТекущаяСтрока.СуммаРег * ТекущаяСтрока.Кратность / ТекущаяСтрока.СуммаРасчетов, 4);
				ТекущаяСтрока.СуммаПлатежа = ТекущаяСтрока.СуммаРег / ?(ДокументОбъект.Курс = 0 , 1, ДокументОбъект.Курс) * ?(ДокументОбъект.Кратность = 0, 1, ДокументОбъект.Кратность);
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьДанныеДляЗаполненияТЧРасшифровкаПлатежа(ДокументОбъект, ЭтоРасчетыСПоставщиком, Договор)
	
	ТЗ_РасшифровкаПлатежа = ДокументОбъект.РасшифровкаПлатежа.Выгрузить();
	МассивСтрокДляУдаления = Новый Массив;
	
	Для Каждого ТекущаяСтрокаРасшифровки Из ТЗ_РасшифровкаПлатежа Цикл
		Если ТекущаяСтрокаРасшифровки.Договор = Договор
...
Функция ДобавитьУдалитьДанныеОбОплатеНакладной(ДокументОбъект, ДополнительныеСвойства, РаспределитьСуммуРасчетов, ТекущаяСтрокаПредоплата, ВариантИзмененияДанных)
	...
	Если ВариантИзмененияДанных = Перечисления.ВариантыИзмененияДанныхВДокументахИзПомощника.Удаление Тогда
	...
	Иначе
		...
		Если ВариантИзмененияДанных = Перечисления.ВариантыИзмененияДанныхВДокументахИзПомощника.Добавление Тогда
			Если ДополнительныеСвойства.ПараметрыДокумента.ЭтоЗаказ Тогда
			...
			КонецЕсли;
		ИначеЕсли ВариантИзмененияДанных = Перечисления.ВариантыИзмененияДанныхВДокументахИзПомощника.Удаление Тогда
			НоваяСтрокаПредоплата.Документ = Неопределено;
			НоваяСтрокаПредоплата.ПризнакАванса = Истина;
			Если ДополнительныеСвойства.ПараметрыДокумента.ЭтоЗаказ Тогда
		...
		СуммаПлатежа = (ТекущаяСтрокаПредоплата.СуммаПлатежа / ТекущаяСтрокаПредоплата.СуммаРасчетов
			* РаспределитьСуммуРасчетов);
		НоваяСтрокаПредоплата.СуммаПлатежа = СуммаПлатежа; //Окр(вСуммаПлатежа, "ЧДЦ=2");
...
Функция НужноОбрабатыватьСтрокуПриИзмененииУдалении(ТекущаяСтрока, ТекущаяСтрокаПредоплата, ДокументСсылка)
	...
	ТекущаяСтрокаПредоплатаЗаказ = ?(ЗначениеЗаполнено(ТекущаяСтрокаПредоплата.Заказ), ТекущаяСтрокаПредоплата.Заказ,
		Неопределено);
	ТекущаяСтрокаЗаказ = ?(ЗначениеЗаполнено(ТекущаяСтрока.Заказ), ТекущаяСтрока.Заказ, Неопределено);
	Если ЭтоЗаказПокупателя(ДокументСсылка) Тогда
		Возврат Не ЗначениеЗаполнено(ТекущаяСтрокаПредоплата.Документ)
			И ТекущаяСтрокаПредоплатаЗаказ = ТекущаяСтрокаЗаказ;
	Иначе
		Возврат ТекущаяСтрокаПредоплата.Документ = ДокументСсылка И ТекущаяСтрокаПредоплатаЗаказ = ТекущаяСтрокаЗаказ;
	КонецЕсли;
...
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.ЗаказПокупателя, ДокументСсылка -
// 
// Возвращаемое значение:
//  Булево - Это заказ покупателя
Функция ЭтоЗаказПокупателя(ДокументСсылка)
...
Процедура СвернутьТабличнуюЧасть(ДокументОбъект, ИмяТабличнойЧасти)
	
	СтрокаКолонок = "";
	
	Для Каждого ТекущаяКолонка Из ДокументОбъект.Метаданные().ТабличныеЧасти.РасшифровкаПлатежа.Реквизиты Цикл
	...
	Если СтрокаКолонок <> "" Тогда
		ДокументОбъект.РасшифровкаПлатежа.Свернуть(СтрокаКолонок, "СуммаПлатежа, СуммаРасчетов");
	КонецЕсли;
	
КонецПроцедуры

Процедура ВывестиРасшифровкуПлатежа(РасшифровкаПлатежа)
	
	Если РасшифровкаПлатежа.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Расшифровка платежа не заполнена.'"));
	Иначе
...
//
Функция ПодготовитьТаблицуСуммДокументаСФВыданный(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства,
...
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства
//  ВременнаяТаблицаЗапасы - Строка - Временная таблица запасы
//  ЭтоКорректировка - Булево - Это корректировка
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Подготовить таблицу документа расчеты в валюте руб
Функция ПодготовитьТаблицуДокументаРасчетыВВалютеРуб(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства,
	ВременнаяТаблицаЗапасы = "", ЭтоКорректировка = Ложь) Экспорт
...
//  ДокументСсылкаРасходнаяНакладная - ДокументСсылка.РасходнаяНакладная
//  СтруктураДополнительныеСвойства - Структура - дополнительные свойства
//  ТаблицаВзаиморасчеты - ТаблицаЗначений
//  ИмяТЧ - Строка - Имя ТЧ
//  СмотретьВДопСвойствах - Булево - Смотреть в доп свойствах
Процедура РаспределитьАвансыПоТаблицеРеализации(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства,
	ТаблицаВзаиморасчеты, ИмяТЧ = "Запасы", СмотретьВДопСвойствах = Ложь) Экспорт
...
//  ЭтоСФ - Булево - Это СФ
//  ЭтоКорректировка - Булево - Это корректировка
//  ДокументСсылка - ДокументСсылка, ДокументСсылка.РасходнаяНакладная - Документ ссылка
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Получить таблица предоплата
Функция ПолучитьТаблицаПредоплата(СтруктураДополнительныеСвойства, ЭтоСФ, ЭтоКорректировка, ДокументСсылка) Экспорт
	...
	СвойстваДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Контрагент, Договор");
	
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.УстановитьПараметр("ГраницаКонтроляИсключая",
	...
	Запрос.Параметры.Вставить("Контрагент", СвойстваДокумента.Контрагент);
	Запрос.Параметры.Вставить("Договор", СвойстваДокумента.Договор);
	...
	Если НЕ ЭтоСФ Тогда
		...
		КонецЕсли;
		Запрос.Параметры.Вставить("ТаблицаРасчетыСПокупателями",
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями);
		Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.Колонки.Найти(
			"СпособЗачета") = Неопределено Тогда
			СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.Колонки.Добавить(
				"СпособЗачета", Новый ОписаниеТипов("Булево"));
		КонецЕсли;
...
// 
// Параметры:
//  ТаблицаДокумента - ТаблицаЗначений, ДеревоЗначений - Таблица документа
//  ДокументыАвансов - ТаблицаЗначений - Документы авансов
//  Реквизиты - Структура
//  ТаблицаВзаиморасчетов - ТаблицаЗначений
//  ОстаткиАвансов - ТаблицаЗначений - Остатки авансов
//  ЭтоСФ - Булево - Это СФ
Процедура ЗаполнитьТаблицуВзаиморасчетовЗачетАвансов(ТаблицаДокумента, ДокументыАвансов, Реквизиты,
	ТаблицаВзаиморасчетов, ОстаткиАвансов, ЭтоСФ) Экспорт
...
// Данная процедура используется для совместимости и только для документов СчетФактура
//
Процедура ДобавитьСтрокиЗачтенныхАвансов(ТаблицаВзаиморасчетов, НераспределеннаяСумма, СтрокаОстатка, СтрокаДокумента,
	Реквизиты, ЭтоСФ)
