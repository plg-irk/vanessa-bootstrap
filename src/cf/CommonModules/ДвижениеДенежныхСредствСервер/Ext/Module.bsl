...
Процедура ОбработатьВидОперации(Форма) Экспорт
	...
	Если ВидОперацииСтрокой(Форма.Объект.ВидОперации) = "ЛичныеСредстваПредпринимателя" Тогда
		...
		Форма.Элементы.Организация.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрВыбора);
	Иначе
		...
		ПараметрВыбора.Добавить(Новый ПараметрВыбора("Отбор.ЮридическоеФизическоеЛицо",
			Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо));		
		Форма.Элементы.Организация.ПараметрыВыбора = Новый ФиксированныйМассив(Новый Массив);
		ПараметрВыбора.Добавить(Новый ПараметрВыбора("Отбор.ЮридическоеФизическоеЛицо",
...
Функция ПолучитьСтруктурыОперацийДенежныеСредства(ВидДокумента) Экспорт
	...
	Если ВидДокумента = "РасходСоСчета" Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВыплатыСамозанятым") Тогда
			ДобавитьОписаниеОперации(Операции, "РасходГруппаПрочееБанк",
				Перечисления.ВидыОперацийРасходСоСчета.ВыплатаСамозанятому, НСтр("ru = 'Выплата самозанятому'"));
			ДобавитьОписаниеОперации(Операции, "РасходГруппаПрочееБанк",
				Перечисления.ВидыОперацийРасходСоСчета.ВыплатыСамозанятым, НСтр("ru = 'Выплаты самозанятым по реестру'"));
		КонецЕсли;
	...
	ИначеЕсли ВидДокумента = "ПлатежноеПоручение" Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВыплатыСамозанятым") Тогда
			ДобавитьОписаниеОперации(Операции, "ППГруппаПрочееБанк",
				Перечисления.ВидыОперацийПлатежноеПоручение.ВыплатаСамозанятому, НСтр("ru = 'Выплата самозанятому'"));
			ДобавитьОписаниеОперации(Операции, "ППГруппаПрочееБанк",
				Перечисления.ВидыОперацийПлатежноеПоручение.ВыплатыСамозанятым, НСтр("ru = 'Выплаты самозанятым по реестру'"));
		КонецЕсли;
	
	ИначеЕсли ВидДокумента = "РасходДСПлан" Тогда
		
		ДобавитьОписаниеОперации(Операции, "РДСПГруппаПоставщикуНаРасходы",  Перечисления.ВидыОперацийРасходДСПлан.Поставщику);
		ДобавитьОписаниеОперации(Операции, "РДСПГруппаПоставщикуНаРасходы", Перечисления.ВидыОперацийРасходДСПлан.НаРасходы);
		
		Если Константы.ФункциональнаяОпцияИспользоватьПодсистемуЗарплата.Получить() Тогда
			ДобавитьОписаниеОперации(Операции,"РДСПГруппаЗарплата", Перечисления.ВидыОперацийРасходДСПлан.Зарплата, НСтр("ru = 'Зарплата по ведомости'"));
			ДобавитьОписаниеОперации(Операции,"РДСПГруппаЗарплата", Перечисления.ВидыОперацийРасходДСПлан.ЗарплатаСотруднику);
		КонецЕсли;
		
		ДобавитьОписаниеОперации(Операции, , Перечисления.ВидыОперацийРасходДСПлан.ПрочиеРасчеты, НСтр("ru = 'Прочему контрагенту (учредителю, таможне)'"));
		ДобавитьОписаниеОперации(Операции, , Перечисления.ВидыОперацийРасходДСПлан.Покупателю, НСтр("ru = 'Покупателю (возврат)'"));
		
		ДобавитьОписаниеОперации(Операции, "РДСПГруппаПрочее", Перечисления.ВидыОперацийРасходДСПлан.Прочее, НСтр("ru = 'Прочий расход'"));
		ДобавитьОписаниеОперации(Операции, "РДСПГруппаПрочее", Перечисления.ВидыОперацийРасходДСПлан.Подотчетнику);
		
		ДобавитьОписаниеОперации(Операции, "РДСПГруппаПрочее", Перечисления.ВидыОперацийРасходДСПлан.Налоги);
		
		Если ПолучитьФункциональнуюОпцию("КредитыИЗаймы") Тогда
			ДобавитьОписаниеОперации(Операции, "РДСПГруппаПрочее", Перечисления.ВидыОперацийРасходДСПлан.РасчетыПоКредитам, НСтр("ru = 'Возврат кредита'"));
		КонецЕсли;
		
		Если ОрганизацияИндивидуальныйПредпринимательСуществует() Тогда
			ДобавитьОписаниеОперации(Операции, "РДСПГруппаЛичныеСредства", Перечисления.ВидыОперацийРасходДСПлан.ЛичныеСредстваПредпринимателя, НСтр("ru = 'Личные средства'"));
		КонецЕсли;
