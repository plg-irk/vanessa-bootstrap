...
Функция ПолучитьПредставлениеЗаказа(ТекущаяСтрока, ФормироватьПоДвижениям, Шрифт, ВыводитьЗаголовок = Ложь, пЗаказ = Неопределено)
	...
	Если ТекущаяСтрока.СпособЗачета <> Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто ИЛИ ФормироватьПоДвижениям Тогда
		...
		Если ЗначениеЗаполнено(ТекущийЗаказ) Тогда
			...
			РеквизитЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийЗаказ, "Организация");		
			Префикс = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитЗаказа, "Префикс");
		
			НомерНаПечать = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(ТекущийЗаказ.Дата,
				ТекущийЗаказ.Номер, Префикс);
		...
		Если ЭтоИзменениеДокумента ИЛИ НЕ ЭтоОплата ИЛИ ТекущаяСтрока.ЭтоЗачет Или НЕ ТекущаяСтрока.ПризнакАванса Тогда
			...
			Если ЗначениеЗаполнено(ТекущаяСтрока.Документ) Тогда
				...
				Если ТипЗнч(ТекДокумент) = Тип("СправочникСсылка.Номенклатура") Тогда // Подарочный сертификат
					Наименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекДокумент, "Наименование");
					КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(СокрЛП(Наименование)));
					Если ЗначениеЗаполнено(ТекущаяСтрока.НомерСертификата) Тогда
						...
						КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(ТекстДокумент,
							ШрифтыСтиля.ШрифтПолужирныйУНФ));
					КонецЕсли;
				...
				Иначе
					Если ТипЗнч(ТекДокумент) = Тип("ДокументСсылка.ЗаказПокупателя")
						И РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(
							ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекДокумент, "ВидОперации")) Тогда
					...
					РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекДокумент, "Организация, Дата, Номер");
					
					ПрефиксОрганизации = ПолучитьПрефиксОрганизации(РеквизитыДокумента.Организация);
					Если ПрефиксОрганизации = Неопределено Тогда // Доступ ограничен на уровне записей.
						ТекстДокумент = "";
					Иначе
						ТекстДокумент = Символы.ПС
							+ ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(РеквизитыДокумента.Дата, РеквизитыДокумента.Номер, ПрефиксОрганизации)
							+ НСтр("ru = ' от '")
...
Функция ПолучитьПредставлениеСчетаНаОплату(ТекущаяСтрока, ФормироватьПоДвижениям, Шрифт, ВыводитьЗаголовок = Ложь)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.СчетНаОплату) Тогда
		...
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.СчетНаОплату, "Организация, Дата, Номер");
		Префикс = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыДокумента.Организация, "Префикс");
		
		НомерНаПечать = ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(РеквизитыДокумента.Дата,
			РеквизитыДокумента.Номер, Префикс);
		СтрокаНомерДата = ""+НомерНаПечать+НСтр("ru=' от '")+Формат(РеквизитыДокумента.Дата, "ДЛФ=D");
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(СтрокаНомерДата));
...
Функция ПолучитьСуммуПлатежаСтроки(ДокументОбъект, ТекущаяСтрока, ЭтоИзменениеДокумента)
	...
	Если НЕ ЭтоИзменениеДокумента И ТекущаяСтрока.ЭтоЗачет Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ТекущаяСтрока.Документ) Тогда
			РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Документ, "Курс, Кратность");		
			Если РеквизитыДокумента.Курс = Неопределено Тогда
				РеквизитыДокумента.Курс = 0;
			КонецЕсли;
			Если РеквизитыДокумента.Кратность = Неопределено Тогда
				РеквизитыДокумента.Кратность = 0;
			КонецЕсли;	
			...
			ТекущаяСуммаПлатежа = ЦенообразованиеСервер.ПересчитатьИзВалютыВВалюту(
				...
				?(РеквизитыДокумента.Курс = 0, 1, РеквизитыДокумента.Курс),
				?(КурсКратностьВалДокумента.Курс = 0, 1, КурсКратностьВалДокумента.Курс),
				?(РеквизитыДокумента.Кратность = 0, 1, РеквизитыДокумента.Кратность),
...
Функция ПолучитьВалютаНакладной(пДокумент) Экспорт
	...
	Если ЗначениеЗаполнено(пДокумент) Тогда
		...
		ИначеЕсли ТипЗнч(пДокумент) = Тип("ДокументСсылка.ПоступлениеВКассу")
			...
			Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(пДокумент, "ВалютаДенежныхСредств");
		Иначе
			Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(пДокумент, "ВалютаДокумента");
		КонецЕсли;
	Иначе
...
Процедура ВывестиСтрокуРасшифровкиПлатежаДляПросмотра(ЭтоОплата, ТабличныйДокументКарточка, ТекущаяСтрока, ФормироватьПоДвижениям,
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДанныеФормыСтруктура") Тогда
		РеквизитКонтрагент = ДокументОбъект.Контрагент;		
	Иначе
		//@skip-check bsl-legacy-check-string-literal
		РеквизитКонтрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект, "Контрагент");
	КонецЕсли;
	...
	Иначе
		РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитКонтрагент,
			"ВестиРасчетыПоДоговорам, ВестиРасчетыПоЗаказам, ВестиРасчетыПоДокументам, ВестиУчетОплатыПоСчетам");
		ВестиРасчетыПоДоговорам = (РеквизитыКонтрагента.ВестиРасчетыПоДоговорам = Истина);
		ВестиРасчетыПоЗаказам = (РеквизитыКонтрагента.ВестиРасчетыПоЗаказам = Истина);
		ВестиРасчетыПоДокументам = (РеквизитыКонтрагента.ВестиРасчетыПоДокументам = Истина);
		ВестиУчетОплатыПоСчетам = (РеквизитыКонтрагента.ВестиУчетОплатыПоСчетам = Истина);
	КонецЕсли;
	
	// Договор
	Если ЭтоОплата И ВестиРасчетыПоДоговорам Тогда
	...
	Если ВестиРасчетыПоЗаказам И НЕ ДокументЯвляетсяЗаказом(ТекущаяСтрока.Документ) Тогда
	...
	КонецЕсли;
	// Счет
	Если НЕ ЭтоИзменениеДокумента И НЕ ТекущаяСтрока.ЭтоЗачет И ЭтоОплата 
		И ВестиУчетОплатыПоСчетам И ПолучитьФункциональнуюОпцию("ИспользоватьСчетаНаОплату") Тогда
	...
	ВалютаРасчетовСП = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.ВалютаРасчетов, "СимвольноеПредставление");
	...
	Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
		...
		Если ТекущаяСтрока.Курс = 0 Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ТекущаяСтрока.Документ) Тогда
				РеквизитКурс = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Документ, "Курс");
				СтруктураОбластей.ОбластьДопСтрока3Значения.Параметры.Значение2 = Формат(РеквизитКурс, "ЧДЦ=4");
			Иначе
				КурсКратностьВалютыРасчетов = РегистрыСведений.КурсыВалют.Получить(ДокументОбъект.Дата, Новый Структура("Валюта", ТекущаяСтрока.ВалютаРасчетов));
		...
		НациональнаяВалютаСП = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Константы.НациональнаяВалюта.Получить(), "СимвольноеПредставление");
		
		СтруктураОбластей.ОбластьДопСтрока3Значения.Параметры.Значение3 = "" + НациональнаяВалютаСП
			+ "/" + ВалютаРасчетовСП;
		Если НЕ ЭтоИзменениеДокумента Тогда
	...
	СтруктураОбластей.ОбластьДопСтрока3Значения.Параметры.Значение3 = "" + ВалютаРасчетовСП;
	Если НЕ ЭтоИзменениеДокумента Тогда
	...
	Если НЕ ЭтоИзменениеДокумента И НЕ ТекущаяСтрока.ЭтоЗачет Тогда
		Если ДокументОбъект.НалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
			...
			Если ТипЗнч(ДокументОбъект) = Тип("ДанныеФормыСтруктура") Тогда
				РеквизитВалютаДенежныхСредств = ДокументОбъект.ВалютаДенежныхСредств;		
			Иначе
				//@skip-check bsl-legacy-check-string-literal
				РеквизитВалютаДенежныхСредств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект, "ВалютаДенежныхСредств");
			КонецЕсли;
			//@skip-check bsl-legacy-check-string-literal
			ВалютаДенежныхСредствСП = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитВалютаДенежныхСредств, "СимвольноеПредставление");
		
			СтруктураОбластей.ОбластьДопСтрока3Значения.Параметры.Значение3 = "" + ВалютаДенежныхСредствСП;
			Если НЕ ЭтоИзменениеДокумента Тогда
...
Процедура ВывестиСтрокуСИнформациейОбОплате(СтруктураВалютыКурсыИКратности, ЭтоОплата, ТабличныйДокументКарточка, ТекущаяСтрока, ФормироватьПоДвижениям,
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаДокумент, "ВалютаДокумента, Курс, Кратность");
	Если РеквизитыДокумента.Курс = Неопределено Тогда
		РеквизитыДокумента.Курс = 0;
	КонецЕсли;
	Если РеквизитыДокумента.Кратность = Неопределено Тогда
	...
	ТекущаяСуммаПлатежа = ТекущаяСтрока.СуммаПлатежа;
	Если ЭтоОплата Тогда
		Если РеквизитыДокумента.ВалютаДокумента = ТекущаяСтрока.ВалютаРасчетов Тогда
			ТекущаяСуммаПлатежа = ТекущаяСтрока.СуммаРасчетов;
		ИначеЕсли РеквизитыДокумента.Курс <> ТекущаяСтрока.Курс Тогда
			ТекущаяСуммаПлатежа = ТекущаяСтрока.СуммаРасчетов * РеквизитыДокумента.Курс / РеквизитыДокумента.Кратность;
		КонецЕсли;
	Иначе
		Если РеквизитыДокумента.ВалютаДокумента = ТекущаяСтрока.ВалютаРасчетов Тогда
			ТекущаяСуммаПлатежа = ТекущаяСтрока.СуммаРасчетов;
		Иначе
			ТекущаяСуммаПлатежа = ТекущаяСтрока.СуммаРасчетов * ?(РеквизитыДокумента.Курс = 0, 1, РеквизитыДокумента.Курс)
				/ ?(РеквизитыДокумента.Кратность = 0, 1, РеквизитыДокумента.Кратность);
		КонецЕсли;
	КонецЕсли;
	Если ТекущаяСуммаПлатежа = 0 Тогда
	...
	//@skip-check bsl-legacy-check-string-literal
	РеквизитКонтрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаДокумент, "Контрагент");
	Если РеквизитКонтрагент = Неопределено Тогда
		ВестиРасчетыПоЗаказам = Ложь;
		ВестиРасчетыПоДокументам = Ложь;
	Иначе
		РеквизитыКонтрагента =
			ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитКонтрагент, "ВестиРасчетыПоЗаказам, ВестиРасчетыПоДокументам");
		ВестиРасчетыПоЗаказам = (РеквизитыКонтрагента.ВестиРасчетыПоЗаказам = Истина);
		ВестиРасчетыПоДокументам = (РеквизитыКонтрагента.ВестиРасчетыПоДокументам = Истина);
	КонецЕсли;

	Если ТипЗнч(СсылкаНаДокумент) <> Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") И ВестиРасчетыПоЗаказам Тогда
		ТабличныйДокументКарточка.Присоединить(СтруктураОбластей.ОбластьСтрокаКолонкиЗаказ);
	КонецЕсли;
	Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями")	ИЛИ ВестиРасчетыПоДокументам Тогда
		ТабличныйДокументКарточка.Присоединить(СтруктураОбластей.ОбластьСтрокаКолонкиДокумент);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
		...
		Иначе // Зачет предоплаты
			...
			ИначеЕсли ТекущаяСтрока.Курс = 0 И ТекущаяСтрока.СуммаРег <> 0 И ТекущаяСтрока.СуммаРасчетов <> 0 Тогда
				...
				РеквизитыДокумента =
					ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаДокумент, "ВалютаДокумента, Курс, Кратность");		
				Если РеквизитыДокумента.Курс = Неопределено Тогда
					РеквизитыДокумента.Курс = 0;
				КонецЕсли;
				Если РеквизитыДокумента.Кратность = Неопределено Тогда
				...
				Если РеквизитыДокумента.ВалютаДокумента = СтруктураВалютыКурсыИКратности.НациональнаяВалюта Тогда
					ТекущаяСуммаПлатежа = ТекущаяСтрока.СуммаРег;
				// Валюта договора и документа <> национальной валюте.
				Иначе
					ТекущаяСуммаПлатежа = ТекущаяСтрока.СуммаРег / ?(РеквизитыДокумента.Курс = 0, 1, РеквизитыДокумента.Курс)
						* ?(РеквизитыДокумента.Кратность = 0, 1, РеквизитыДокумента.Кратность);
				КонецЕсли;
			...
			СимвольноеПредставление =
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыДокумента.ВалютаДокумента, "СимвольноеПредставление");
			СтруктураОбластей.ОбластьДопСтрока3Значения.Параметры.Значение2 = Формат(ТекущаяСуммаПлатежа, "ЧДЦ=2");
			СтруктураОбластей.ОбластьДопСтрока3Значения.Параметры.Значение3 = "" + СимвольноеПредставление;
			Если НЕ ЭтоОплата И ТекущаяСтрока.СпособЗачета = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Вручную Тогда
...
Процедура ВывестиШапку(ЭтоОплата, Контрагент, ТабличныйДокументКарточка, СтруктураОбластейШапки, ЭтоЗачет = Ложь, 
	...
	Если НЕ ЭтоЗачет И ЭтоОплата И Контрагент.ВестиУчетОплатыПоСчетам
		И ПолучитьФункциональнуюОпцию("ИспользоватьСчетаНаОплату") Тогда
...
Функция ПолучитьТабличныйДокументОплатаПоДокументуОплаты(Документ, ФормироватьПоДвижениям, Модифицированность, ТаблицаДокументовДляИзменения = Неопределено,
	...
	Для Каждого ТекущаяСтрока Из Объект.РасшифровкаПлатежа Цикл
		Если ТекущаяСтрока.СпособЗачета <> Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто
			...
			РеквизитОбъектаВалютаРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Договор, "ВалютаРасчетов");		
			НоваяСтрокаПредставления.ВалютаРасчетов = РеквизитОбъектаВалютаРасчетов;
	...
	Если ТипЗнч(Документ) = Тип("ДанныеФормыСтруктура") Тогда
		РеквизитВалютаДенежныхСредств = Объект.ВалютаДенежныхСредств;		
	Иначе
		//@skip-check bsl-legacy-check-string-literal
		РеквизитВалютаДенежныхСредств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект, "ВалютаДенежныхСредств");		
	КонецЕсли;
	//@skip-check bsl-legacy-check-string-literal
	СимвольноеПредставление = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитВалютаДенежныхСредств, "СимвольноеПредставление");
		
	ОбластьРазнесено.Параметры.Разнесено = НСтр("ru = 'Всего разнесено для оплаты долгов: '") + Разнесено + " " + СимвольноеПредставление
...
Функция ПолучитьТабличныйДокументОплатаПоДокументуОтгрузки(Документ, пФормироватьПоДвижениям, Модифицированность, ТаблицаДокументовДляИзменения = Неопределено,
	...
	ЭтоОтчетКомиссионера = (ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ОтчетКомиссионера"));
	
	ЭтоВозврат = (ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РасходнаяНакладная")
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		ЭтоЗаказНаряд = РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(Объект.ВидОперации);
		ЭтоЗаявка = РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(Объект.ВидОперации);
	Иначе
		ЭтоЗаказ = Ложь;
		ЭтоЗаказНаряд = Ложь;
		ЭтоЗаявка = Ложь;
	...
	Объект.Проведен
	И Объект.СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто
	И Не ЭтоЗаявка;
	...
	Если НЕ ФормироватьПоДвижениям Тогда
		Для Каждого ТекущаяСтрока Из Объект[ИмяТЧ] Цикл
			...
			Если ТипЗнч(Документ) = Тип("ДанныеФормыСтруктура") Тогда
				РеквизитОбъектаДоговор = Объект.Договор;		
			Иначе
				//@skip-check bsl-legacy-check-string-literal
				РеквизитОбъектаДоговор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект, "Договор");		
			КонецЕсли;
			
			Если ЗначениеЗаполнено(РеквизитОбъектаДоговор) Тогда
				//@skip-check bsl-legacy-check-string-literal
				РеквизитОбъектаВалютаРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитОбъектаДоговор, "ВалютаРасчетов");
			Иначе
				РеквизитОбъектаВалютаРасчетов = Справочники.Валюты.ПустаяСсылка();
			КонецЕсли;
	...
	Если ТипЗнч(Объект) = Тип("ДанныеФормыСтруктура") Тогда
		РеквизитыОбъектаВалютаДокумента = Объект.ВалютаДокумента;
		РеквизитыОбъектаДоговор = Объект.Договор;
	Иначе
		РеквизитыОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект, "ВалютаДокумента, Договор");
		РеквизитыОбъектаВалютаДокумента = РеквизитыОбъекта.ВалютаДокумента;
		РеквизитыОбъектаДоговор = РеквизитыОбъекта.Договор;
	КонецЕсли;
		
	//@skip-check bsl-legacy-check-string-literal
	ВалютаДокументаСимвольноеПредставление =
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыОбъектаВалютаДокумента, "СимвольноеПредставление");
	//@skip-check bsl-legacy-check-string-literal
	ВалютаРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыОбъектаДоговор, "ВалютаРасчетов");
	//@skip-check bsl-legacy-check-string-literal
	Если ЗначениеЗаполнено(РеквизитыОбъектаДоговор) Тогда
		//@skip-check bsl-legacy-check-string-literal
		ВалютаРасчетовСимвольноеПредставление =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВалютаРасчетов, "СимвольноеПредставление");
	Иначе
		ВалютаРасчетовСимвольноеПредставление = "";
	КонецЕсли;
	...
	Если Объект.ВалютаДокумента <> ВалютаРасчетов И ЗначениеЗаполнено(ВалютаРасчетов) Тогда
		...
		Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
			ПредставлениеСуммаВалРасчетов = СтрШаблон(НСтр("ru = '(%1 %2 по курсу %3)'"), СуммаДокументаВалРасчетов,
				ВалютаРасчетовСимвольноеПредставление, Формат(Объект.Курс, "ЧДЦ=4"));
	...
	Оплачено = 0;
	ВосстановленоАвансовВалДокумента = 0;
	ВосстановленоАвансов = 0;
	...
	Если ЭтоЗаказНаряд Тогда
		...
		КонецЦикла;
	ИначеЕсли ЭтоОтчетКомиссионера Тогда
		Для Каждого ТекущаяСтрока Из ПредставлениеРасшифровкиПлатежа Цикл
			Если ТекущаяСтрока.ЭтоВозврат И ТекущаяСтрока.Документ = Объект.Ссылка Тогда
				ВосстановленоАвансов = ВосстановленоАвансов + ТекущаяСтрока.СуммаРасчетов;
				УвеличитьОплаченоВалДокумента(ВосстановленоАвансовВалДокумента, ВосстановленоАвансов, ТекущаяСтрока, Документ,
					ВалютаРасчетов);
			Иначе
				Оплачено = Оплачено + ТекущаяСтрока.СуммаРасчетов;
				УвеличитьОплаченоВалДокумента(ОплаченоВалДокумента, Оплачено, ТекущаяСтрока, Документ, ВалютаРасчетов);
			КонецЕсли;
		КонецЦикла;
		Оплачено = Макс(Оплачено, ВосстановленоАвансов);
	...
	Если ЭтоВозврат Или ЭтоОтчетКомиссионера Тогда
	...
	Если Объект.ВалютаДокумента = ВалютаРасчетов Тогда
		ОбъектСуммаДокумента = Макс(-Объект.СуммаДокумента, Объект.СуммаДокумента);
		Если Оплачено = ВосстановленоАвансов Тогда
			ПроцентОплаты = "100";
		Иначе
			ПроцентОплаты =
				?(Документ.СуммаДокумента = 0, "0", Формат(Оплачено/ОбъектСуммаДокумента * 100, "ЧДЦ=2; ЧН=0"));
		КонецЕсли;
	...
	Если ТипЗнч(Документ.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		ДокументПроведен = РаботаСЗаявкамиКлиентСервер.ЗаказПроведен(Документ);
	Иначе
	...
	Если Объект.СпособЗачетаПредоплаты = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто Тогда
		Если ДокументПроведен И НЕ пФормироватьПоДвижениям И Модифицированность Тогда
