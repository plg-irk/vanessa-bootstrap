...
Процедура СформироватьТаблицаДоходыИРасходыКассовыйМетодЭквайринг(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	|					ИЛИ ВидОперацииЭквайринга = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП))
	...
	|		ИЛИ ДоходыИРасходыКассовыйМетодЭквайринг.ВидОперацииЭквайринга = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП))
...
Процедура ЗаполнитьЭквайринговыеОперации(Объект, ДатаНачала = '00010101', ДатаОкончания = '00010101', Договор = Неопределено) Экспорт
	...
	|			ИЛИ ВременнаяТаблицаРасчетыПоЭквайрингуОстатки.ВидОперацииЭквайринга = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП)
...
Процедура ЗаполнитьРасшифровкуПлатежаПоОперациямЭквайринга(Объект) Экспорт
	...
	Если ТипДокумента = Тип("ДокументСсылка.ПоступлениеНаСчет") Тогда
		КолонкиСвертки = "УчитыватьВНУ, НалогообложениеНДС, ВидОплатыДляСвертки";
	...
	Для Каждого ТекущаяСтрока Из ТаблицаЭквайринговыхОпераций Цикл
		...
		Иначе //РасходСоСчета 
			Если Объект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВозвратПродажиВКредит Тогда
				ТекущаяСуммаРасчетов = ТекущаяСтрока.СуммаРасчетовВозврата - ТекущаяСтрока.СуммаРасчетовКомиссииВозврата
									 - ТекущаяСтрока.СуммаРасчетов - ТекущаяСтрока.СуммаРасчетовКомиссии;
			 Иначе
				СуммаВозврата = ТекущаяСтрока.СуммаРасчетовВозврата - ТекущаяСтрока.СуммаРасчетовКомиссииВозврата;
				
				// Для случая, когда возвращается только комиссия - сумма должна быть положительной
				Если СуммаВозврата < 0 Тогда
					СуммаВозврата = -СуммаВозврата;
				КонецЕсли;
				
				СуммаРасчетов = ТекущаяСтрока.СуммаРасчетов - ТекущаяСтрока.СуммаРасчетовКомиссии;
				
				ТекущаяСуммаРасчетов = СуммаВозврата - СуммаРасчетов;
			КонецЕсли; 
		КонецЕсли; 
