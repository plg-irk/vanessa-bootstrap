...
//  Ответственный - СправочникСсылка.Сотрудники - Сотрудник, ответственный за источник.
//
...
//   * Ответственный   - Массив из СправочникСсылка.Сотрудники
//
...
Функция ПодключенТелеграм() Экспорт
	...
	Попытка
		...
		Для Каждого Интеграции Из ОбщегоНазначения.ВычислитьВБезопасномРежиме("СистемаВзаимодействия.ПолучитьИнтеграции()") Цикл
			Если Интеграции.ТипВнешнейСистемы <> ТипыВнешнихСистем.Telegram Тогда
				Продолжить;
			КонецЕсли;
...
// Очистка раздела Входящее Контакт-центра.
//
// Параметры:
//  Сотрудник - Массив из СправочникСсылка.Сотрудники - Сотрудник, ответственный за источник.
//
Процедура ОчиститьВходящееСотрудника(Сотрудник) Экспорт
	
	РегистрыСведений.КонтактЦентрВходящее.ОчиститьВходящееОтветственного(Сотрудник);
