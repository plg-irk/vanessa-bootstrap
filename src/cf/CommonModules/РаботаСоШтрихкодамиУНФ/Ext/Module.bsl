...
Процедура ЗаполнитьПрефиксыШтрихкодовНоменклатуры(СтруктураПрефиксов) Экспорт
	...
	ПрефиксыШтучныхШтрихкодов = РегистрыСведений.ШтрихкодыНоменклатуры.ПрефиксыШтучныхШтрихкодов();
	
	Для Каждого ПрефиксШтучногоШтрихкода Из ПрефиксыШтучныхШтрихкодов Цикл
		Доб1 = СтруктураПрефиксов.Добавить();
		Доб1.Ключ = "НоменклатураФасованный";
		Доб1.Значение = Префикс + ПрефиксШтучногоШтрихкода;
	КонецЦикла;
...
Процедура ДобавитьТекстЗапросаШтрихкодыУпаковокТоваров(ФильтрПоискаПоШтрихкоду, МассивЗапросов) Экспорт
	...
	Если ДобавитьЗапросПолногоВыбытия Тогда
		...
		|	Штрихкоды.Характеристика,
		|	ВЫБОР
		|		КОГДА ОписаниеНоменклатурыИС.ПотребительскаяУпаковка ЕСТЬ NULL
		|			ТОГДА ВЫБОР
		...
		|			КОНЕЦ
		|		ИНАЧЕ ОписаниеНоменклатурыИС.ПотребительскаяУпаковка
		|	КОНЕЦ,
		|	ВЫБОР
	...
	Если ДобавитьЗапросЧастичногоВыбытия Тогда
		...
		|			И (ОписаниеНоменклатурыИС.ВариантЧастичногоВыбытия = ЗНАЧЕНИЕ(Перечисление.ВариантыУчетаЧастичногоВыбытияИС.ТекущаяНоменклатура))
		|ГДЕ
		|	Штрихкоды.ЗначениеШтрихкода = &ШтрихкодМарки
		|	И НЕ Штрихкоды.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|	И НЕ ОписаниеНоменклатурыИС.Номенклатура ЕСТЬ NULL";
...
Процедура ЗаполнитьКодИКоличествоВесовогоТовара(Штрихкод, СтруктураВесовогоТовара) Экспорт
	...
	Если СтрДлина(Штрихкод) = 13 // EAN13
		И Лев(Штрихкод, 1) = "2" Тогда // Внутренний штрихкод
		
		// Штрихкод является весовым/фасованным, выполняем преобразование.
		// Формат штрихкода: 2 + П + ЧЧЧЧЧ + BBBBB + K
		// Где,
		//  П     - Префикс весового/фасованного товара
		//  ЧЧЧЧЧ - код товара
		//  BBBBB - Вес/Количество
		//  К     - Контрольное число
		
		Если ПрефиксыВесовыхШтрихкодов.Найти(Сред(Штрихкод, 2, 1)) <> Неопределено Тогда // Найден префикс весового товара
			Знаменатель = 1000;
		ИначеЕсли ПрефиксыШтучныхШтрихкодов.Найти(Сред(Штрихкод, 2, 1)) <> Неопределено Тогда // Найден префикс фасованного товара
			Знаменатель = 1;
		Иначе
			Возврат;
		КонецЕсли;
		
		КоличествоВес = Число(Сред(Штрихкод, 8, 5)) / Знаменатель;
		
		СтруктураВесовогоТовара = Новый Структура;
		СтруктураВесовогоТовара.Вставить("КодТовара", Число(Сред(Штрихкод, 3, 5)));
		СтруктураВесовогоТовара.Вставить("КоличествоТовара", КоличествоВес);
...
// См. РаботаСоШтрихкодамиПереопределяемый.СкорректироватьСтруктуруРезультата.
Процедура СкорректироватьСтруктуруРезультата(
	...
	ШтрихкодМарки = Неопределено;
	Если СтруктураПараметров.Свойство("ПараметрыШтрихкода")
		И Не СтруктураПараметров.ПараметрыШтрихкода = Неопределено
		И СтруктураПараметров.ПараметрыШтрихкода.Свойство("ФильтрПоискаПоШтрихкоду")
		И СтруктураПараметров.ПараметрыШтрихкода.ФильтрПоискаПоШтрихкоду.Свойство("ПриоритетШтрихкодаУпаковок") Тогда
		
		ШтрихкодМарки = СтруктураПараметров.ПараметрыШтрихкода.ШтрихкодМарки;
	Иначе
		Возврат;
	КонецЕсли;
	
	ЗначенияПоиска = СтруктураПараметров.ЗначенияПоиска;
	
	ИндексМассива = 0;
	ИндексШтрихкодаУпаковок = Новый Массив;
	ШтрихкодУпаковокДляДобавления = Неопределено;
	Для Каждого ЗначениеПоиска Из ЗначенияПоиска Цикл
		Если ЗначениеПоиска.ТипОбъекта = Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров") Тогда
			ЗначениеШтрихкода = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗначениеПоиска.Владелец, "ЗначениеШтрихкода");
			Если ШтрихкодУпаковокДляДобавления = Неопределено Тогда
				ШтрихкодУпаковокДляДобавления = ЗначениеПоиска.Владелец;
			КонецЕсли;
			Если ЗначениеШтрихкода = ШтрихкодМарки Тогда
				ШтрихкодУпаковокДляДобавления = ЗначениеПоиска.Владелец;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Если Не ШтрихкодУпаковокДляДобавления = Неопределено Тогда
		Для Каждого ЗначениеПоиска Из ЗначенияПоиска Цикл
			Если ЗначениеПоиска.ТипОбъекта = Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров") Тогда	
				Если ШтрихкодУпаковокДляДобавления = ЗначениеПоиска.Владелец Тогда
					ИндексШтрихкодаУпаковок.Добавить(ИндексМассива);
				КонецЕсли;
			КонецЕсли;
	...
	Если Не ИндексШтрихкодаУпаковок.Количество() = 0 Тогда
		ИндексМассива = 0;
		НовыеЗначенияПоиска = Новый Массив;
		Для Каждого ЗначениеПоиска Из ЗначенияПоиска Цикл
			Если (ЗначениеПоиска.ТипОбъекта = Тип("СправочникСсылка.Номенклатура")
				Или ЗначениеПоиска.ТипОбъекта = Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров"))
				И ИндексШтрихкодаУпаковок.Найти(ИндексМассива) = Неопределено Тогда
				ИндексМассива = ИндексМассива + 1;
				Продолжить;
			КонецЕсли;
			
			НовыеЗначенияПоиска.Добавить(ЗначениеПоиска);
			ИндексМассива = ИндексМассива + 1;
		КонецЦикла;
		
		СтруктураПараметров.ЗначенияПоиска = НовыеЗначенияПоиска;
	КонецЕсли;
