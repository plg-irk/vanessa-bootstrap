...
Процедура ИнициализацияЗапросаПубликуемыхТоваров(Запрос) Экспорт
	...
	|	ТорговыеПредложения.Организация КАК Организация
	|ПОМЕСТИТЬ ВР_СкидкиЗаОбъемПродажВидыЦен
	|ИЗ
	|	Товары КАК ВТТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СкидкиТорговыхПредложенийВидыЦен КАК СкидкиТорговыхПредложенийВидыЦен
	|		ПО ВТТовары.ПрайсЛист = СкидкиТорговыхПредложенийВидыЦен.ТорговоеПредложение
	|			И (СкидкиТорговыхПредложенийВидыЦен.Количество > 1)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ТорговыеПредложения КАК ТорговыеПредложения
	|		ПО ВТТовары.ПрайсЛист = ТорговыеПредложения.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВР_СкидкиЗаОбъемПродажВидыЦен.Номенклатура КАК Номенклатура,
	|	ВР_СкидкиЗаОбъемПродажВидыЦен.Количество КАК Количество,
	|	ВР_СкидкиЗаОбъемПродажВидыЦен.ВидыЦен КАК ВидыЦен,
	|	ВР_СкидкиЗаОбъемПродажВидыЦен.Характеристика КАК Характеристика,
	|	0 КАК Цена,
	|	0 КАК ЦенаДоСкидки,
	|	"""""""" КАК Комментарий
	|ПОМЕСТИТЬ СкидкиЗаОбъемПродажВидыЦен
	|ИЗ
	|	ВР_СкидкиЗаОбъемПродажВидыЦен КАК ВР_СкидкиЗаОбъемПродажВидыЦен
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТТовары.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ТорговыеПредложения.Организация КАК Организация
	|ПОМЕСТИТЬ ВР_СкидкиПроцентСуммаВидыЦен
	|ИЗ
	|	Товары КАК ВТТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СкидкиТорговыхПредложенийВидыЦен КАК СкидкиТорговыхПредложенийВидыЦен
	|		ПО ВТТовары.ПрайсЛист = СкидкиТорговыхПредложенийВидыЦен.ТорговоеПредложение
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВР_СкидкиПроцентСуммаВидыЦен.Номенклатура КАК Номенклатура,
	|	ВР_СкидкиПроцентСуммаВидыЦен.Количество КАК Количество,
	|	ВР_СкидкиПроцентСуммаВидыЦен.ВидыЦен КАК ВидыЦен,
	|	ВР_СкидкиПроцентСуммаВидыЦен.Характеристика КАК Характеристика,
	|	0 КАК Цена,
	|	0 КАК ЦенаДоСкидки,
	|	"""""""" КАК Комментарий
	|ПОМЕСТИТЬ СкидкиПроцентСуммаВидыЦен
	|ИЗ
	|	ВР_СкидкиПроцентСуммаВидыЦен КАК ВР_СкидкиПроцентСуммаВидыЦен";
		
	ТекстЗапроса = ТекстЗапроса + Разделитель + ТекстСвойства + Разделитель + ТекстСкидки;
