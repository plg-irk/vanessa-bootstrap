...
//  КоэффициентУпаковки - Число - коэффициент пересчета единицы измерения в упаковку
//  ДополнительныеПараметры - Структура - дополнительные парамтеры единиц измерения
//
Процедура ЗаполнитьПредставлениеЕдиницыИзмерения(ТекущаяСтрока, ПредставлениеЕИ, ПредставлениеЕИЧастичногоВыбытия,
		КоэффициентУпаковки, ДополнительныеПараметры) Экспорт
	
	Результат = НСтр("ru = ''");
	КодЕдиницыГрамм = "163";
	КоэффициентПереводаВКилограммы = 1;
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ЕдиницыИзмерения.Наименование КАК Наименование,
	|	ВЫБОР
	|		КОГДА ЕдиницыИзмерения.Код = ""163""
	|			ТОГДА 1000
	|		ИНАЧЕ 1
	...
	Если Выборка.Следующий() Тогда
		...
		КоэффициентПереводаВКилограммы = Выборка.КоэффициентПереводаВКилограммы;
	КонецЕсли;
	
	ПредставлениеЕИЧастичногоВыбытия = Результат;
	Если ЗначениеЗаполнено(ТекущаяСтрока.Упаковка) Тогда
		Если ТипЗнч(ТекущаяСтрока.Упаковка) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
			ДанныеУпаковки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Упаковка, "Наименование, Коэффициент");
			ПредставлениеЕИ = ДанныеУпаковки.Наименование;
			КоэффициентУпаковки = ДанныеУпаковки.Коэффициент;
		Иначе
			ДанныеУпаковки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Упаковка, "Наименование,Код");
			ПредставлениеЕИ = ДанныеУпаковки.Наименование;
			Если СокрЛП(ДанныеУпаковки.Код) = КодЕдиницыГрамм Тогда
				КоэффициентПереводаВКилограммы = 1000;
			КонецЕсли;
			ДополнительныеПараметры.КоэффициентПереводаВКилограммы = КоэффициентПереводаВКилограммы;
		КонецЕсли;
	Иначе
...
Процедура ЗаполнитьДанныеНайденнойНоменклатуры(СтрокаРезультата, Форма, ПараметрыДляОбработкиШтрихкода,
	...
	Если ТипЗнч(ВладелецШтрихкода) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		ЗаполнитьДанныеНоменклатуры(СтруктураВыбора, Форма);
		...
		Если Не СтрокаРезультата.Свойство("ВесИзШтрихкода") И ВладелецШтрихкода.Весовой Тогда
			Количество = 0;
		ИначеЕсли СтрокаРезультата.Свойство("ВесИзШтрихкода") И Не ШтрихкодУпаковки = Неопределено
			И ВладелецШтрихкода.Весовой Тогда
			Количество = ШтрихкодУпаковки.Количество;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(СтруктураВыбора.Упаковка) Тогда
			СтруктураВыбора.Упаковка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураВыбора.Номенклатура, "ЕдиницаИзмерения");
		КонецЕсли;
		...
		Если СтруктураВыбора.КоличествоВПотребительскойУпаковке = Коэффициент Тогда
			СтруктураВыбора.ПоштучноеВыбытие = Ложь;
			СтруктураВыбора.ЧастичноеВыбытиеВариантУчета = Неопределено;
			СтруктураВыбора.ЧастичноеВыбытиеКоличество = 0;
			СтруктураВыбора.ЧастичноеВыбытиеНоменклатура = Неопределено;
			СтруктураВыбора.ЧастичноеВыбытиеХарактеристика = Неопределено;
		КонецЕсли;
...
Процедура ЗаполнитьСтруктуруВыбора(СтруктураВыбора)
	...
	СтруктураВыбора.Вставить("РазрешительныйРежимИдентификаторЭкземпляраПО");
	СтруктураВыбора.Вставить("РазрешительныйРежимВерсияБазы");
	...
	СтруктураВыбора.Вставить("ПрепаратОтпускаетсяПоДокументу");
...
Процедура ЗаполнитьПризнакиПоштучногоВыбытия(Номенклатура, Характеристика, СтруктураВыбора, Форма)
	...
	|ВЫБРАТЬ
	|	ИСТИНА КАК ПоштучноеВыбытие,
	|	ОписаниеНоменклатурыИС.Номенклатура КАК ЧастичноеВыбытиеНоменклатура,
	|	&Характеристика КАК ЧастичноеВыбытиеХарактеристика,
	...
	|ПОМЕСТИТЬ ЧастичноеВыбытиеОднойКарточкой
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	ЧастичноеВыбытиеОднойКарточкой.КоличествоВПотребительскойУпаковке,
	|	ЧастичноеВыбытиеОднойКарточкой.ЧастичноеВыбытиеКоличество,
	|	ЧастичноеВыбытиеОднойКарточкой.Номенклатура,
	|	ЧастичноеВыбытиеОднойКарточкой.ЧастичноеВыбытиеВариантУчета,
	|	ЧастичноеВыбытиеОднойКарточкой.ПоштучноеВыбытие,
	|	ЧастичноеВыбытиеОднойКарточкой.ЧастичноеВыбытиеХарактеристика,
	|	ЧастичноеВыбытиеОднойКарточкой.ЧастичноеВыбытиеНоменклатура,
	|	ЧастичноеВыбытиеОднойКарточкой.Характеристика
	|ИЗ
	|	ЧастичноеВыбытиеОднойКарточкой КАК ЧастичноеВыбытиеОднойКарточкой";
...
Процедура ЗаполнитьДанныеНоменклатуры(СтруктураВыбора, ФормаРМК)
	...
	Если Выборка.Следующий() Тогда
		...
		ИсключаяСвойства = "";
		ИсключаяСвойства = ?(Не ФормаРМК.ИспользоватьВыборСкладаПриПродаже, "ТорговыйОбъект, Ячейка", ИсключаяСвойства);
		ЗаполнитьЗначенияСвойств(СтруктураВыбора, Выборка,, ИсключаяСвойства);
		
		ДополнительныеДанные = Новый Структура();
		ДополнительныеДанные.Вставить("ИспользоватьВыборСкладаПриПродаже", ФормаРМК.ИспользоватьВыборСкладаПриПродаже);
		ДополнительныеДанные.Вставить("ТорговыйОбъект", ФормаРМК.Объект.ТорговыйОбъект);
		
		ОбщегоНазначенияРМКУНФ.ПроверитьСкладДляРучногоВыбора(СтруктураВыбора, ДополнительныеДанные);
		ОбщегоНазначенияРМКУНФ.ПереопределитьПараметрыЗаполненияСерийИПартий(СтруктураВыбора, ФормаРМК);
	КонецЕсли;
...
Процедура ПроверитьЗаполненностьСерии(СтруктураВыбора, ШтрихкодУпаковки)
	...
	Иначе
		Если Не СтруктураВыбора.НеобходимостьВводаСерии = Неопределено И СтруктураВыбора.НеобходимостьВводаСерии Тогда
