...
Функция ПолучитьСтруктуруШаблонаЧека(ПараметрыШаблонаЧека, ДополнительныйТекст = "", ТипОборудования = "") Экспорт
	...
	Если НовыйТипШаблона Тогда
		...
		ДополнитьТаблицуДанных(ПараметрыШаблонаЧека, ТаблицаДанных);
...
Процедура ДополнитьТаблицуДанных(ПараметрыШаблонаЧека, ТаблицаДанных)
	
	Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыШаблонаЧека,
		"ДополнительныеПараметрыШаблонаЧека") Тогда
		Возврат;
	КонецЕсли;
	
	ДополнительныеПараметрыШаблонаЧека = ПараметрыШаблонаЧека.ДополнительныеПараметрыШаблонаЧека;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметрыШаблонаЧека,
		"СоставЧека") Тогда
		СтрокаСоставЧека = ТаблицаДанных.Найти("СоставЧека", "ИмяТаблицы");
		Если СтрокаСоставЧека <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(СтрокаСоставЧека, ДополнительныеПараметрыШаблонаЧека.СоставЧека);
		КонецЕсли;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметрыШаблонаЧека, "Товары") Тогда
		Для Каждого ДополнительныеПараметрыСтрокиТовары Из ДополнительныеПараметрыШаблонаЧека.Товары Цикл
			Если НЕ ЗначениеЗаполнено(ДополнительныеПараметрыСтрокиТовары.Значение) Тогда
				Продолжить;
			КонецЕсли;
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Вставить("ИмяТаблицы", "Товары");
			ПараметрыОтбора.Вставить("НомерСтрокиТовара", ДополнительныеПараметрыСтрокиТовары.Ключ);
			СтрокиТоваров = ТаблицаДанных.НайтиСтроки(ПараметрыОтбора);
			Если СтрокиТоваров.Количество() Тогда
				СтрокаТовары = СтрокиТоваров[0];
				ЗаполнитьЗначенияСвойств(СтрокаТовары, ДополнительныеПараметрыСтрокиТовары.Значение);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
