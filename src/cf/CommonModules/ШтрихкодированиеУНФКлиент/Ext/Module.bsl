...
//   * ЭтоПродукцияЕГАИС - Булево -  Истина, если штрихкод принадлежит к продукции ЕГАИС.
//   * ШтрихкодBase64 - Строка - закодированный штрихкод.
//
Функция ДополнительныеДанныеПоШтрихкоду(Штрихкод, ТипОбъекта) Экспорт
	...
	Если ОбщегоНазначенияРМККлиентСервер.ЕстьСвойство(ДанныеПоШтрихкоду, "ТипИдентификатораТовара") Тогда
		...
		ЭтоПродукцияЕГАИС = ТипыШтрихкодовМарок.Получить(ТипИдентификатораТовара);
		Если ЭтоПродукцияЕГАИС <> Неопределено Тогда
			ДополнительныеДанныеПоШтрихкоду.ЭтоКодМаркировки = Истина;
			ДополнительныеДанныеПоШтрихкоду.ЭтоПродукцияЕГАИС = ЭтоПродукцияЕГАИС;
		КонецЕсли;
	КонецЕсли;
	...
	ДополнительныеДанныеПоШтрихкоду.ШтрихкодBase64 = ДанныеПоШтрихкоду.ШтрихкодBase64;
...
Функция ПараметрыПоШтрихкоду()
	...
	Параметры.Вставить("ЭтоПродукцияЕГАИС"		, Ложь);
	Параметры.Вставить("ШтрихкодBase64"			, "");
...
Функция СформироватьСписокТиповШтрихкодовМарок()
	...
	ТипыШтрихкодовМарок = Новый Соответствие;
	...
	ТипыШтрихкодовМарок.Вставить(ПредопределенноеЗначение(ИмяМаркировкиМех), Ложь);
	ТипыШтрихкодовМарок.Вставить(ПредопределенноеЗначение(ИмяМаркировкиGS1), Ложь);
	ТипыШтрихкодовМарок.Вставить(ПредопределенноеЗначение(ИмяМаркировкиВФорматеЕГАИС2), Истина);
	ТипыШтрихкодовМарок.Вставить(ПредопределенноеЗначение(ИмяМаркировкиВФорматеЕГАИС3), Истина);
