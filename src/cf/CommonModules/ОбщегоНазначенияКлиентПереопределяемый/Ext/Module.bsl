...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	ПереданПараметрУстановкиБизнесПриложения = ВозможностиПриложенияКлиент.ПереданПараметрУстановкиБизнесПриложения();
	ДанныеНачалаРаботы = НачалоРаботыСПрограммойКлиент.ДанныеИзПараметровРаботыКлиента(ПараметрыРаботыКлиента);
	ДанныеНачалаРаботы.ПоказатьВыборДемоПриложений = 
	(ДанныеНачалаРаботы.ПоказатьВыборДемоПриложений И Не ПереданПараметрУстановкиБизнесПриложения);
	
	Если ДанныеНачалаРаботы.ПоказатьОкноНачалоРаботыСПрограммой Тогда
		Возврат;
	КонецЕсли;
	
	Если ДанныеНачалаРаботы.ПоказатьВыборДемоПриложений Тогда
		НачалоРаботыСПрограммойКлиент.УстановитьВыборДемоПриложений();
		ОбновитьИнтерфейс();
		Возврат;
	КонецЕсли;
	...
	Если ПереданПараметрУстановкиБизнесПриложения Тогда
		ТребуетсяОбновлениеИнтерфейса = Истина;
		РабочийСтолУНФВызовСервера.УстановитьНачальнуюСтраницуИИнтерфейс();
	ИначеЕсли ПараметрыРаботыКлиента.ТребуетсяУстановитьРабочийСтолПользователяУНФ И 
		ПараметрыРаботыКлиента.ТребуетсяУстановитьНастройкиСоставаИнтерфейсаПользователяУНФ Тогда
		РабочийСтолУНФВызовСервера.УстановитьНачальнуюСтраницуИИнтерфейс();
		ТребуетсяОбновлениеИнтерфейса = Истина;
	ИначеЕсли ПараметрыРаботыКлиента.ТребуетсяУстановитьРабочийСтолПользователяУНФ Тогда
		РабочийСтолУНФВызовСервера.УстановитьРабочийСтолПользователя(ТребуетсяОбновлениеИнтерфейса);
	ИначеЕсли ПараметрыРаботыКлиента.ТребуетсяУстановитьНастройкиСоставаИнтерфейсаПользователяУНФ  Тогда
		РабочийСтолУНФВызовСервера.УстановитьНастройкиСоставаИнтерфейса();
	Иначе
		ТребуетсяОбновлениеИнтерфейса = Ложь;
	КонецЕсли;
...
Процедура ПриОбработкеПараметровЗапуска(ПараметрыЗапуска, Отказ) Экспорт
	...
	ВозможностиПриложенияКлиент.УстановитьВидПриложенияПоПараметруЗапуска(ПараметрыЗапуска);
...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	Если ДанныеНачалаРаботы.ПоказатьОкноНачалоРаботыСПрограммой
		Или (ДанныеНачалаРаботы.ПоказатьВыборДемоПриложений И Не ПереданПараметрУстановкиБизнесПриложения) Тогда
		Возврат;
	КонецЕсли;
	...
	МессенджерыКлиент.ПослеНачалаРаботыСистемы();
	#КонецЕсли
...
Процедура ОбработкаПереходаПоНавигационнойСсылке(ДанныеПереходаПоНавигационнойСсылке,
	...
	// СовместноеИспользование
	СовместноеИспользованиеКлиент.ОбработкаПереходаПоНавигационнойСсылке(ДанныеПереходаПоНавигационнойСсылке,
