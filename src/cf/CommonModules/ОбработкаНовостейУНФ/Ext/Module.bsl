...
Процедура ЗначениеПредопределеннойКатегории(Категория, Значение) Экспорт
	...
	Если ТипЗнч(Категория) = Тип("ПланВидовХарактеристикСсылка.КатегорииНовостей") Тогда
		...
		КатегорияКод = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Категория, "Код");
		КатегорияКод = СокрЛП(КатегорияКод);
...
Функция ПолучитьДанныеНовостей() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ТаблицаНовостей", Неопределено);
	...
	ТаблицыНовостейЛенты = Новый Массив;
	СсылкиНаНовостиЛенты = Новый Массив;
	...
	Для Каждого ЛентаНовостей Из ЛентыНовостей Цикл
		...
		ТаблицыНовостейЛенты.Добавить(ТаблицаНовостейЛенты);
		
		Для Каждого НовостьЛенты Из ТаблицаНовостейЛенты Цикл
			СсылкиНаНовостиЛенты.Добавить(НовостьЛенты.Ссылка);
		КонецЦикла;
		
	КонецЦикла;
	
	СсылкиНаНовостиЛенты = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СсылкиНаНовостиЛенты);
	ДатыСбросаВажности = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СсылкиНаНовостиЛенты, "ДатаСбросаВажности");
	
	Если Не ТаблицыНовостейЛенты.Количество() Тогда
		Возврат Результат;
	КонецЕсли;
	
	РезультирующаяТаблицаНовостей = ТаблицыНовостейЛенты[0].СкопироватьКолонки();
	РезультирующаяТаблицаНовостей.Колонки.Добавить("КороткийТекстДляБаннера", Новый ОписаниеТипов("Строка"));
	
	Для Каждого ТаблицаНовостейЛенты Из ТаблицыНовостейЛенты Цикл
		Для Каждого НовостьЛенты Из ТаблицаНовостейЛенты Цикл
			
			ДатаСбросаВажности = ДатыСбросаВажности[НовостьЛенты.Ссылка];
			Если ДатаСбросаВажности = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Если ДатаСбросаВажности > НовостьЛенты.ДатаПубликации 
				И ДатаСбросаВажности < ТекущаяДатаСеанса() Тогда
				Продолжить;
			КонецЕсли;
			...
			СтрокаТЗ = РезультирующаяТаблицаНовостей.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТЗ, НовостьЛенты);
			СтрокаТЗ.КороткийТекстДляБаннера = КороткийТекстДляБаннера;
			
		КонецЦикла;
	КонецЦикла;
	
	Результат.Вставить("ТаблицаНовостей", РезультирующаяТаблицаНовостей);
	Возврат Результат;
