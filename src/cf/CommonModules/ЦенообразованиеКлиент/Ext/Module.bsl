...
// Выполняет пересчет Процента изменения цены в табличной части документа. 
//
// Параметры:
//  ФормаДокумента - Форма - форма документа.
//  ИмяТабличнойЧасти - Строка - содержит имя пересчитываемой табличной части.
//  СтрокаТЧ - СтрокаТабличнойЧасти - текущая строка табличной части объекта.
//
Процедура РассчитатьОтклонениеЦенВСтроке(ФормаДокумента, ИмяТабличнойЧасти, СтрокаТЧ = Неопределено) Экспорт
	
	ЕстьВидОперацииПомощника = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		ФормаДокумента, "ВидОперацииПомощника");
	
	Если НЕ ЕстьВидОперацииПомощника Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ФормаДокумента.ВидОперацииПомощника) Тогда  
		Возврат;
	КонецЕсли;       
	
	ПолноеИмяПоступлениеОтПоставщика = "Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика";
	ПолноеИмяПриемНаКомиссию = "Перечисление.ВидыОперацийПриходнаяНакладная.ПриемНаКомиссию";	
	
	ПоступлениеОтПоставщика = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент(ПолноеИмяПоступлениеОтПоставщика);  
	ПриемНаКомиссию = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент(ПолноеИмяПриемНаКомиссию); 
	
	Если (ПоступлениеОтПоставщика <> Неопределено ИЛИ ПриемНаКомиссию <> Неопределено) 
		И (ФормаДокумента.ВидОперацииПомощника = ПоступлениеОтПоставщика 
			ИЛИ ФормаДокумента.ВидОперацииПомощника = ПриемНаКомиссию)  Тогда 
		
		ИмяПроверяемыйРеквизитФормы = "ПоказыватьКолонкуЦенаПрошлойЗакупки";
		ИмяПоказательКолонкаТаблицы = "ЦенаПрошлойЗакупки";
		
	Иначе
		Возврат;
	КонецЕсли;
	
	ПоказыватьДополнительнуюКолонку = ФормаДокумента[ИмяПроверяемыйРеквизитФормы];	
	Если НЕ ПоказыватьДополнительнуюКолонку Тогда
		Возврат;
	КонецЕсли;
	...
	Иначе
		ТекущаяСтрока = ФормаДокумента.Элементы[ИмяТабличнойЧасти].ТекущиеДанные; 
	КонецЕсли;
	
	Если ТекущаяСтрока <> Неопределено 
		И ТекущаяСтрока[ИмяПоказательКолонкаТаблицы] > 0 И ТекущаяСтрока.Цена > 0 Тогда 
		
   		ТекущаяСтрока.ПроцентИзмененияЦен = (ТекущаяСтрока.Цена / ТекущаяСтрока[ИмяПоказательКолонкаТаблицы] - 1) * 100;
		
	Иначе
		
		ТекущаяСтрока.ПроцентИзмененияЦен = 0;
	 
 	КонецЕсли;
 
КонецПроцедуры   

// Дополняет структуру данных для получения цены прошлой закупки
//
// Параметры:
//  Объект - ДанныеФормыСтруктура - Документ объект.
//  СтруктураДанные - Структура - Структура которую необходимо дополнить
//
Процедура ДополнитьСтруктуруДаннымиЦенаПрошлойЗакупки(Объект, СтруктураДанные) Экспорт 
	
	СтруктураДанные.Вставить("Ссылка"					, Объект.Ссылка); 
	СтруктураДанные.Вставить("ДатаДокументаПоступления"	, Объект.Дата);
	СтруктураДанные.Вставить("ЦенаПрошлойЗакупки"		, 0); 
	...
	Если НЕ СтруктураДанные.Свойство("ВалютаДокумента") Тогда 
		СтруктураДанные.Вставить("ВалютаДокумента", Объект.ВалютаДокумента);
	КонецЕсли; 
		
	СтруктураДанные.Вставить("ПоказыватьКолонкуЦенаПрошлойЗакупки", Истина);
	
КонецПроцедуры   

// Выполняет пересчет данных в строке
//
// Параметры:
//  СтрокаТЧ - СтрокаТабличнойЧасти - текущая строка табличной части объекта.
//  ПараметрыЗаполнения - Структура - Структура с данными о цене прошлой закупки.
//
Процедура ЗаполнитьЦенуПрошлойЗакупкиВСтроке(СтрокаТЧ, ПараметрыЗаполнения) Экспорт
	
	СтрокаТЧ.ЦенаПрошлойЗакупки = ПараметрыЗаполнения.ЦенаПрошлойЗакупки;
...
Процедура ПерезаполнитьЦеныТабличнойЧастиПоВидуЦенКонтрагента(ФормаДокумента, ИмяТабличнойЧасти) Экспорт
	...
	Для каждого СтрокаТЧ Из ТабличнаяЧастьДокумента Цикл
		...
		Для каждого СтрокаРезультат Из РезультатПоиска Цикл
			...
			РассчитатьСуммуСтрокиТабличнойЧасти(ФормаДокумента, СтрокаРезультат);
			РассчитатьОтклонениеЦенВСтроке(ФормаДокумента, ИмяТабличнойЧасти, СтрокаРезультат);
...
// Дополняет структуру параметров вызова формы Цены и валюта параметрами авторегистрации цен
//
// Параметры:
//  ПараметрыФормыЦеныИВалюта - см. ЦеныИВалютаКлиентСервер.НовыеПараметрыФормыЦеныИВалюта
//  АвтоРегистрацияЦенПродажи - Булево - зависит от ФО "УчетЦенПродажи" 
//
Процедура ДополнитьСтруктуруВызоваФормыЦеныИВалютыПараметрамиАвторегистрацииЦен(ПараметрыФормыЦеныИВалюта,
	АвтоРегистрацияЦенПродажи, РегистрироватьЦеныПродажи) Экспорт
	 
	ПараметрыФормыЦеныИВалюта.РегистрироватьЦеныПродажиЕстьРеквизит = АвтоРегистрацияЦенПродажи;
	ПараметрыФормыЦеныИВалюта.РегистрироватьЦеныПродажи 			= РегистрироватьЦеныПродажи;
...
// Параметры:
//  ПараметрыФормыЦеныИВалюта - см. ЦеныИВалютаКлиентСервер.НовыеПараметрыФормыЦеныИВалюта
//  АвтоРегистрацияЦенПоставщика - Булево - зависит от ФО "УчетЦенКонтрагентов" 
//
Процедура ДополнитьСтруктуруВызоваФормыЦеныИВалютыПараметрамиАвторегистрацииЦенПоставщика(ПараметрыФормыЦеныИВалюта,
	РегистрироватьЦеныПоставщикаЕстьРеквизит, РегистрироватьЦеныПоставщика) Экспорт
	
	ПараметрыФормыЦеныИВалюта.РегистрироватьЦеныПоставщикаЕстьРеквизит 	= РегистрироватьЦеныПоставщикаЕстьРеквизит;
	ПараметрыФормыЦеныИВалюта.РегистрироватьЦеныПоставщика 				= РегистрироватьЦеныПоставщика;
