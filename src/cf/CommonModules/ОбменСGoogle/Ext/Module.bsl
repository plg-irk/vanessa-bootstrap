...
Процедура ОбновитьЗаписьКалендаря(КалендарьСотрудника, ЗаписьКалендаря, Пользователь)
	...
	Иначе
		// Проверка периода на "Весь день"
		Если НЕ ЗаписьКалендаря.Получить("start") = Неопределено
			И НЕ ЗаписьКалендаря.Получить("end") = Неопределено
			И НЕ ЗаписьКалендаря["start"].Получить("date") = Неопределено
			И НЕ ЗаписьКалендаря["end"].Получить("date") = Неопределено Тогда
			
			ДанныеЗаполнения.Вставить("ВесьДень", Ложь);
			НачалоСобытия = ЗаписьКалендаря["start"]["date"];
			ОкончаниеСобытия = ЗаписьКалендаря["end"]["date"];
			Если НачалоСобытия = НачалоДня(НачалоСобытия)
				И ОкончаниеСобытия = НачалоДня(ОкончаниеСобытия)
				И НачалоСобытия < ОкончаниеСобытия Тогда
...
Процедура УстановитьКалендарьПоУмолчанию(КалендарьСотрудника, ДанныеЗаполнения)
	...
	ОсновнойКалендарь = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойКалендарь", , ДанныеЗаполнения.Пользователь);
	
	Если ЗначениеЗаполнено(ОсновнойКалендарь) Тогда
		Возврат;
	КонецЕсли;
	
	ПерсональныеНастройкиРУНФ.Установить(КалендарьСотрудника, "ОсновнойКалендарь", ДанныеЗаполнения.Пользователь);
...
Функция ДанныеСообщений(Идентификаторы, Знач СеансовыеДанные, Знач ВариантЗагрузки)
	...
	ИнтернетПрокси = ПолучениеФайловИзИнтернета.ПолучитьПрокси(Сервер);
	
	HTTPСоединение = Новый HTTPСоединение(Сервер,,,,
		ИнтернетПрокси, 20, ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение());
...
Функция ЗаписатьСобытиеПоДаннымGMail(ДанныеСообщения, УчетнаяЗапись, СопоставленныеКонтакты, Пользователь, ПараметрыСинхронизации)
	...
	Попытка
		...
		ЗначенияЗаполнения.Вставить("ЗагрузитьПриОткрытии", ЭлектроннаяПочтаУНФ.ЗагрузитьПриОткрытии(
			ПараметрыСинхронизации.ВариантЗагрузки));
		...
		Иначе
			НовоеСобытие.Ответственный = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойОтветственный", , Пользователь);
		КонецЕсли;
