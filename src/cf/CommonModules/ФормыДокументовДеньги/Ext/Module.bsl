...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(Форма, ПриСозданииНаСервере = Ложь) Экспорт
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты Тогда
		...
		Если Объект.ПоложениеЭквайринговогоТерминала = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти
			И Объект.ЭквайринговыеОперации.Количество() > 0 Тогда
			Терминал = Объект.ЭквайринговыеОперации[0].ЭквайринговыйТерминал; // СправочникСсылка.СпособыОплаты
		Иначе
...
Процедура УстановитьВидимостьДокументыПланирования(Форма) Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийЭквайринга.ПоступлениеОплатыОтПокупателя
			ИЛИ ВидОперации = Перечисления.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП
...
Функция ПолучитьСписокДляВыбораДокументаОснования(ИмяФормы, ВидОперации, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ФормыДокументовДеньгиКлиентСервер.ЭтоРасходСоСчета(ИмяФормы) Тогда
		...
		СписокОснований.Добавить("Документ.ПоступлениеНаСчет.ФормаВыбора", НСтр("ru = 'Поступление на счет'"));
	...
	Если ФормыДокументовДеньгиКлиентСервер.ЭтоРасходИзКассы(ИмяФормы) Тогда
		...
		СписокОснований.Добавить("Документ.ПоступлениеВКассу.ФормаВыбора", НСтр("ru = 'Поступление в кассу'"));
...
Процедура УстановитьСтатьиДДСВРасшифровкеПлатежа(Форма) Экспорт
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.РасчетыПоКредитам
		...
		ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперацииПСПК(Форма, ПриСозданииНаСервере = Ложь)
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.РасчетыПоКредитам") Тогда
		...
		Элементы.СуммаРасчетов.Видимость = Ложь;
		Элементы.СуммаНДС.Видимость = Ложь;
		Элементы.ДокументыПланированияРасшифровкаПлатежаСуммаПлатежа.Видимость = ПолучитьФункциональнуюОпцию(
			"ПлатежныйКалендарь");
...
Процедура ЗаполнитьРеквизитыВидовОпераций(Форма)
	...
	Если ФормыДокументовДеньгиКлиентСервер.ЭтоОперацияПоПлатежнымКартам(Форма) Тогда
		...
		Форма.ВидОперацииОплатаСБП = Перечисления.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП;
...
Процедура ИзменитьВидГруппыДополнительно(Форма)
	...
	Если ФормыДокументовДеньгиКлиентСервер.ЭтоДокументПоБанку(Форма) Тогда
		Элементы.Подразделение.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.Проект.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.НалогообложениеНДС.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.СчетКонтрагента.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.ПодписьКассира.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.НазначениеПлатежа.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
	КонецЕсли;
	...
	Если ФормыДокументовДеньгиКлиентСервер.ЭтоПоступлениеВКассу(Форма) Тогда
		...
		Элементы.Проект.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.НалогообложениеНДС.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.ПринятоОт.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.Основание.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.Приложение.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
	КонецЕсли;

	Если ФормыДокументовДеньгиКлиентСервер.ЭтоРасходИзКассы(Форма) Тогда
		Элементы.Подразделение.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.Проект.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.ПоДокументу.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.Приложение.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.Основание.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.Выдать.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
		Элементы.НалогообложениеНДС.РастягиватьПоГоризонтали = КоличествоВидимых > 1;
	КонецЕсли;
