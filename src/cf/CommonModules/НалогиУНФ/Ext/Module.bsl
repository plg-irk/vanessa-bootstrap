...
//	Склад - СправочникСсылка.СтруктурныеЕдиницы - Розничный склад, для которой определяется система налогообложения
...
Функция НалогообложениеНДС(Организация, Склад = Неопределено, Дата = '00010101') Экспорт
	...
	Запрос.УстановитьПараметр("ДатаОтменыЕНВД", РегламентированнаяОтчетностьУСНКлиентСервер.ДатаОтменыЕНВД());

	Запрос.Текст =
	"ВЫБРАТЬ
	|	Налогообложение.Организация КАК Организация,
	|	Налогообложение.СистемаНалогообложения КАК СистемаНалогообложения,
	|	Налогообложение.ПрименяетсяПатент КАК ПрименяетсяПатент,
	|	Налогообложение.ПлательщикНДСПрименяющийУСН КАК ПлательщикНДСПрименяющийУСН
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПримененияПатенты.Организация КАК Организация,
	|	ПримененияПатенты.Патент КАК Патент,
	|	ПримененияПатенты.РозничнаяТорговляОблагаетсяПатентом КАК РозничнаяТорговляОблагаетсяПатентом
	|ПОМЕСТИТЬ ПримененияПатенты
	|ИЗ
	...
	|ИЗ
	|	ПримененияПатенты КАК ПримененияПатенты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Налогообложение
	|		ПО	ПримененияПатенты.Организация = Налогообложение.Организация
	|			И ПримененияПатенты.РозничнаяТорговляОблагаетсяПатентом
	|			И Налогообложение.ПрименяетсяПатент
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
//		* ВидСтавкиНДС - ПеречислениеСсылка.ВидыСтавокНДС
//					   - Неопределено
//
...
// Параметры:
//	Организация - СправочникСсылка.Организации
//	Склад		- СправочникСсылка.СтруктурныеЕдиницы - склад
//				- Неопределено - склад не будет учитываться при определении налогообложения
//	Дата		- Дата
//
...
Функция НастройкиНДСОрганизации(Организация = Неопределено, Склад = Неопределено, Дата = '00010101') Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
					...
					|ВЫБРАТЬ
	               	...
	               	|	10
	               	|ИЗ
	               	|	Налогообложение КАК СистемыНалогообложенияОрганизацийСрезПоследних
