...
Процедура ОткрытьСчетФактуру(ФормаДокумента, Полученный = Ложь) Экспорт
	...
	Иначе
		...
		Если Полученный И Не Корректировочный Тогда
			...
			Если ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
				СчетФактураНомерИсправления = ФормаДокумента.СчетФактураИсправление;
			Иначе
				СчетФактураНомерИсправления = 0;
			КонецЕсли;
			
			ПараметрыСчетФактуры = СчетаФактурыУНФВызовСервера.СоздатьСчетФактуру(СсылкаНаОбъект, СчетФактураДата,
				ФормаДокумента.СчетФактураНомер, СчетФактураНомерИсправления);
		...
		КонецЕсли;
		ПредставлениеСчетФактуры = СчетаФактурыУНФКлиентСервер.ПредставлениеСчетаФактуры(ПредставлениеДата, ПредставлениеНомер, Истина);
