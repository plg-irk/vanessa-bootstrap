...
Процедура ДобавитьИнформациюОНабореПриПолученииДанныхНоменклатуры(СтруктураДанные) Экспорт
	...
	Если СтруктураДанные.ЭтоНабор Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			НастройкиЗаполненияНДС.Номенклатура = Выборка.Номенклатура;
			Если ЗначениеЗаполнено(Выборка.Партия) Тогда
				НастройкиЗаполненияНДС.Партия = Выборка.Партия;
			КонецЕсли;
			...
			СтруктураПолей.Вставить("СтавкаНДС",
				?(ЗначениеЗаполнено(ДанныеНДС.СтавкаНДС), ДанныеНДС.СтавкаНДС, Выборка.СтавкаНДС));
...
Процедура ОбновитьСоставНабораВДокументе(СтруктураДанных, Объект) Экспорт
	...
	ЭтоЗаказ = (ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		И Не РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(Объект.ВидОперации));
...
Процедура КомпоноватьТабличнуюЧастьПоНаборам(ДанныеОбъектовПечати, ИмяТабличнойЧасти, Ошибки = Неопределено, ПроверятьРазделители = Ложь) Экспорт
	...
	Для каждого ОбъектПечати Из ДанныеОбъектовПечати Цикл
		...
		Для каждого СтрокаНабора Из ДобавленныеНаборы Цикл
			...
			СтруктураПараметровОтгрузочногоДокумента = Новый Структура;
			СтруктураПараметровОтгрузочногоДокумента.Вставить("ДатаОтгрузочногоДокумента", 			'00010101');
			СтруктураПараметровОтгрузочногоДокумента.Вставить("НомерОтгрузочногоДокумента", 		"");
			СтруктураПараметровОтгрузочногоДокумента.Вставить("НаименованиеОтгрузочногоДокумента", 	"");
			СтавкаНДСНабора = Неопределено;
			Для каждого СтрокаСоставаНабора Из СоставНабора Цикл
				
				ЗаполнитьЗначенияСвойств(СтруктураПараметровОтгрузочногоДокумента, СтрокаСоставаНабора);
			...
			ЗаполнитьЗначенияСвойств(ДобавляемаяСтрока, СтруктураПараметровОтгрузочногоДокумента);
