...
// Загрузить оплаты по Юкаssа. Для вызова из обработчика командной панели списков
// 
// Параметры:
//  ОснованиеПлатежа - ДокументСсылка - Основание платежа
//  ПараметрыВыполненияКоманды - Структура, Неопределено - Параметры выполнения команды
Процедура ЗагрузитьОплатыПоЮкаssа(ОснованиеПлатежа, ПараметрыВыполненияКоманды = Неопределено) Экспорт

	НачатьЗагрузкуОпераций(Истина);
