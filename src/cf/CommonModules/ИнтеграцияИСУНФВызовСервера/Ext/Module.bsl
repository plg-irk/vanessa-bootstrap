...
Процедура ПерезаполнитьЦеныПоВидуЦен(ДанныеТоваров, ВидЦен, ДатаОбработки) Экспорт
	
	ВалютаУчета = Константы.ВалютаУчета.Получить();
	Для Каждого ДанныеСтроки Из ДанныеТоваров Цикл
		
		Если ЗначениеЗаполнено(ДанныеСтроки.ЕдиницаИзмерения) Тогда
			ДанныеСтроки.Коэффициент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеСтроки.ЕдиницаИзмерения, "Коэффициент");
		КонецЕсли;
		
		ДанныеСтроки.Вставить("ВалютаДокумента", ВалютаУчета);
		ДанныеСтроки.Цена = ЦенообразованиеСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(ДанныеСтроки);
