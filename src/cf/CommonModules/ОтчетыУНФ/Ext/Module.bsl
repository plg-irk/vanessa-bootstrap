...
Процедура ФормаОтчетаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ЭлементФормыТаблица <> Неопределено Тогда
		ЭлементФормыТаблица.РежимМасштабированияПросмотра = РежимМасштабированияПросмотра.Авто;
	КонецЕсли;
...
Процедура СформироватьКонтактыДляЭкспорта(ОтчетДанныеРасшифровки, КонтактыОтчета) Экспорт
	...
	КонтактыОтчета = СформироватьМассивДанныхПоРезультатамОтчета(ОтчетДанныеРасшифровки,МассивТипов);

КонецПроцедуры

// Формирование списка номенклатуры для экспорта
//
// Параметры:
//  ОтчетДанныеРасшифровки	 - 	Строка - Адрес данных расшифровки отчета
//  НоменклатураОтчета		 - 	Массив - Номенклатура отчета
//
Процедура СформироватьНоменклатуруДляЭкспорта(ОтчетДанныеРасшифровки, НоменклатураОтчета) Экспорт
	...
	НоменклатураОтчета = СформироватьМассивДанныхПоРезультатамОтчета(ОтчетДанныеРасшифровки,МассивТипов);
...
Процедура ДобавитьРеквизитыИКомандыОтправкиКонтактов(Форма) Экспорт
	...
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() = Ложь Тогда 
		Возврат;
	КонецЕсли;
...
// Добавление реквизитов и команд расширения сегментов на форму отчета
//
// Параметры:
//  Форма	 - 	ФормаКлиентскогоПриложения - форма отчета
//
Процедура ДобавитьРеквизитыИКомандыРасширитьСегменты(Форма) Экспорт
	
	Если Форма.Элементы.Найти("РасширитьСегментыКонтрагентовПоРезультатамОтчета") <> Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма,"КонтактыОтчета") <> Истина Тогда 
		Возврат;
	КонецЕсли;

	Команда = Форма.Команды.Добавить("РасширитьСегментыКонтрагентовПоРезультатамОтчета");
	Команда.Действие  = "Подключаемый_Команда";
	Команда.Заголовок = НСтр("ru = 'Добавить в сегмент контрагентов из отчета'");
	
	ГруппаКнопокСегмента = Форма.Элементы.Добавить("ГруппаКомандРасширениеСегмента", 
		Тип("ГруппаФормы"), 
		Форма.Элементы.ГруппаОтправить);
    ГруппаКнопокСегмента.Вид = ВидГруппыФормы.ГруппаКнопок;

	Кнопка = Форма.Элементы.Добавить(Команда.Имя, Тип("КнопкаФормы"), ГруппаКнопокСегмента);
	Кнопка.ИмяКоманды = Команда.Имя;
	
	Команда = Форма.Команды.Добавить("РасширитьСегментыНоменклатурыПоРезультатамОтчета");
	Команда.Действие  = "Подключаемый_Команда";
	Команда.Заголовок = НСтр("ru = 'Добавить в сегмент номенклатуру из отчета'");

	Кнопка = Форма.Элементы.Добавить(Команда.Имя, Тип("КнопкаФормы"), ГруппаКнопокСегмента);
	Кнопка.ИмяКоманды = Команда.Имя;
	
	МассивДобавляемыхРеквизитов = Новый Массив;
 	ИмяТаблицы = "НоменклатураОтчета";
	МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(ИмяТаблицы, Новый ОписаниеТипов("ТаблицаЗначений"),,, Истина));
	МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("Номенклатура",
...
Функция СформироватьМассивДанныхПоРезультатамОтчета(ОтчетДанныеРасшифровки, МассивТипов)

	МассивВыходной = Новый Массив;
	...
	Для Каждого ЭлементРасшифровки Из ИспользуемыеДанныеРасшифровки.Элементы Цикл
		...
		Если ТипЗнч(ЭлементРасшифровки) = Тип("ЭлементРасшифровкиКомпоновкиДанныхПоля") Тогда
			...
			Для Каждого ЗначениеПоля Из ЗначенияПолей Цикл
				...
				Если НЕ ЗначениеЗаполнено(ЗначениеПоля.Значение) Тогда 
					Продолжить; 
				КонецЕсли;    
				
				Для Каждого Тип Из МассивТипов Цикл 
					
					Если ТипЗнч(ЗначениеПоля.Значение) = Тип Тогда
						ЗначениеУжеЕсть = МассивВыходной.Найти(ЗначениеПоля.Значение);
						Если ЗначениеУжеЕсть <> Неопределено Тогда
							Продолжить;
						КонецЕсли;
						
						МассивВыходной.Добавить(ЗначениеПоля.Значение);
					КонецЕсли;
...
Процедура ДобавитьКомандыОтправкиКонтактов(Форма)
	...
	Команда.Действие  = "Подключаемый_Команда";
	Команда.Заголовок = НСтр("ru = 'Создать задачи по контрагентам из отчета'");
	...
	ГруппаКнопокРассылки.Заголовок 	= НСтр("ru = 'Добавить в массовую рассылку контрагентов из отчета'");
	
	ГруппаКнопокРассылкиЕще.Вид		= ВидГруппыФормы.Подменю;
	ГруппаКнопокРассылкиЕще.Заголовок 	= НСтр("ru = 'Добавить в массовую рассылку контрагентов из отчета'");
