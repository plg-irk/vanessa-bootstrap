...
Функция ПроверяемыеРеквизитыОбсуждения() Экспорт
	...
	|Организация";
	СтруктураРеквизитов.Вставить(ДокументКлюч, СтрокаРеквизитов);
	ДокументКлюч = "ВыплатыСамозанятым";
	СтрокаРеквизитов =
	"Комментарий,
	|Организация,
	|СуммаДокумента";
	СтруктураРеквизитов.Вставить(ДокументКлюч, СтрокаРеквизитов);
	ДокументКлюч = "ЗадачаСотрудника";
	СтрокаРеквизитов =
	"Календарь,
	|Автор,
	|Описание,
...
Функция ПолучитьПараметрыКассыККМ(КассаККМ) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ПодключаемоеОборудованиеКассыККМ.ТипОборудования = ЗНАЧЕНИЕ(Перечисление.ТипыПодключаемогоОборудования.ОблачнаяККТ)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоОблачнаяККТ
	|ИЗ
	|	Справочник.КассыККМ КАК КассыККМ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПодключаемоеОборудование КАК ПодключаемоеОборудованиеКассыККМ
	|		ПО КассыККМ.ПодключаемоеОборудование = ПодключаемоеОборудованиеКассыККМ.Ссылка
	|ГДЕ
...
Функция ТребуетсяКонтрольДоговоровКонтрагентов() Экспорт
	...
	Если Константы.ТребуетсяКонтрольДоговоровКонтрагентовВДенежныхДокументах.Получить() Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если (Не РаботаВМоделиСервиса.РазделениеВключено()
			И Не ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных")) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтавкаНДС) И Не СтавкаНДС.НеОблагается Тогда
		...
		Если ЭтоАванс Тогда
			...
			ИначеЕсли СтавкаНДС.Ставка = 20 Тогда
				Возврат 120;
			ИначеЕсли СтавкаНДС.Ставка = 22 Тогда
				Возврат 122;
			ИначеЕсли СтавкаНДС.Ставка = 5 Тогда
				Возврат 105;
...
// Устарела. См. ПерсональныеНастройкиРУНФ.Прочитать
// Функция возвращает значение по умолчанию для передаваемого пользователя и настройки
//
// Параметры:
//  Настройка - Строка - признак, для которого возвращается значение по умолчанию
//  Организация - СправочникСсылка.Организации - организация, в разрезе которой нужна настройки
//
// Возвращаемое значение:
//  Произвольный - значение настройки
Функция ПолучитьЗначениеНастройки(Настройка, Знач Организация = Неопределено) Экспорт
	
	Результат = ПерсональныеНастройкиРУНФ.Прочитать(Настройка, Организация);
	Возврат Результат;
...
//  Настройка - Строка - признак, для которого возвращается значение по умолчанию
//  ПустоеЗначение - Произвольный - значение по-умолчанию 
//
// Возвращаемое значение:
...
Функция ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, Настройка, ПустоеЗначение = Неопределено) Экспорт
	...
	Результат = ПерсональныеНастройкиРУНФ.Прочитать(Настройка, , Пользователь, ПустоеЗначение);
	Возврат Результат;

КонецФункции

// Устарела. См. ПерсональныеНастройкиРУНФ.ЗачетАвансов
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ДаНет
Функция ПолучитьЗначениеНастройкиЗачетаАвансов() Экспорт
	Результат = ПерсональныеНастройкиРУНФ.ЗачетАвансов();
	Возврат Результат;
КонецФункции

// Устарела. См. ПерсональныеНастройкиРУНФ.РеквизитВШапке
// 
// Параметры:
//  Настройки - Строка - имя настройки
// Возвращаемое значение:
//  Булево
Функция РеквизитВШапке(Настройка) Экспорт

	Если ПолучитьФункциональнуюОпцию("МиграцияПерсональныхНастроекЗавершена") Тогда
		Возврат ПерсональныеНастройкиРУНФ.РеквизитВШапке(Настройка);
	КонецЕсли;
