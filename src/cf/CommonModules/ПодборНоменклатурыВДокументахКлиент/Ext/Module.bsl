...
Процедура ОткрытьФормуПодбораНоменклатуры(ФормаВладелец, ИмяТабличнойЧасти = "Запасы", ПараметрыПодбора = Неопределено, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	ВидСтавкиНДСПоУмолчанию = Неопределено;
	...
	Иначе
		...
		ЦенаНаДатуСенанса = Ложь;
		ПолностьюОбновить = Ложь;
		
		// Добавляет в параметры открытия формы поддбора переданные данные с формы
		Если ТипЗнч(ПараметрыПодбора) = Тип("Структура") Тогда
			
			ЦенаНаДатуСенанса = ?(ПараметрыПодбора.Свойство("ЦенаНаДатуСенанса") = Истина, ПараметрыПодбора.ЦенаНаДатуСенанса, Ложь);
			
			Если ПараметрыПодбора.Свойство("ПолностьюОбновить") Тогда
				...
				Если ПараметрыПодбора.Свойство("ВидСтавкиНДСПоУмолчанию") Тогда
					ВидСтавкиНДСПоУмолчанию = ПараметрыПодбора.ВидСтавкиНДСПоУмолчанию;
				КонецЕсли;
				
				ПолностьюОбновить = Истина;
		...
		Если ПолностьюОбновить = Истина Тогда
			ЗаполнитьЗначенияПараметровПодбораНоменклатуры(ФормаВладелец, ИмяТабличнойЧасти,
				"Справочники.Номенклатура.ФормаСписка", ПараметрыПодбора);
		КонецЕсли;
		
		ЗаполнитьЗначенияПараметровНачальнаяНастройка(ФормаВладелец, ИмяТабличнойЧасти, ПараметрыПодбора);
		
		Если ЦенаНаДатуСенанса = Истина Тогда
			ПараметрыПодбора.ПериодЦен = Дата(1,1,1);
		КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаВладелец, "Объект")
		...
		Если НЕ ЗначениеЗаполнено(ВидСтавкиНДСПоУмолчанию)
			И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаВладелец, "ВидСтавкиНДСПоУмолчанию") Тогда
			ВидСтавкиНДСПоУмолчанию = ФормаВладелец.ВидСтавкиНДСПоУмолчанию;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидСтавкиНДСПоУмолчанию) Тогда
		ПараметрыПодбора.Вставить("ВидСтавкиНДСПоУмолчанию", ВидСтавкиНДСПоУмолчанию);
	КонецЕсли;
...
Процедура ЗаполнитьЗначенияПараметровНачальнаяНастройка(ФормаВладелец, ИмяТабличнойЧасти, ПараметрыПодбора)
	...
	Если ФормаВладелец.ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаЗаказНаряда"  Тогда
		Если Не ИмяТабличнойЧасти = "Запасы" И Не ИмяТабличнойЧасти = "ЗНМатериалы" Тогда
	...
	Если ФормаВладелец.ИмяФормы = "Документ.ЗаказНаПроизводство.Форма.ФормаДокумента" Тогда
		...
		ЭтоСборка = ФормаВладелец.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказНаПроизводство.Сборка");
		
		Если Не ПараметрыПодбора.Свойство("ПоказыватьКолонкуРезерв") Тогда
			ПараметрыПодбора.Вставить("ПоказыватьКолонкуРезерв", ИмяТабличнойЧасти = "Запасы" И ЭтоСборка);
		Иначе
			ПараметрыПодбора.ПоказыватьКолонкуРезерв = ИмяТабличнойЧасти = "Запасы" И ЭтоСборка;
		КонецЕсли;
	КонецЕсли;
	...
	Если ФормаВладелец.ИмяФормы = "Документ.СчетНаОплату.Форма.ФормаДокумента"
		ИЛИ ФормаВладелец.ИмяФормы = "Документ.СписаниеЗапасов.Форма.ФормаДокумента"
		ИЛИ ФормаВладелец.ИмяФормы = "Документ.ПриходнаяНакладная.Форма.ФормаДокумента" Тогда
...
Процедура ЗаполнитьПараметрСтруктурнаяЕдиница(ФормаВладелец, ПараметрыПодбора)
	...
	ИначеЕсли ФормаВладелец.ИмяФормы = "Документ.КомплектацияЗапасов.Форма.ФормаДокумента" Тогда 	
		ФормаВладелец.ЭтотОбъект.Объект.Свойство("СтруктурнаяЕдиницаМатериалы", СтруктурнаяЕдиница);
		ПараметрыПодбора.Вставить("СтруктурнаяЕдиница", СтруктурнаяЕдиница);
	Иначе
