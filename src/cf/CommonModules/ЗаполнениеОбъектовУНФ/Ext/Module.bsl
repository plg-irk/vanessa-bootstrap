...
//  СтратегияЗаполнения	 - Строка - имя процедуры обработчика заполнения в модуле объекта;
//                       - Соответствие из КлючИЗначение:
//                        * Ключ - Тип - тип параметра "ДанныеЗаполнения"
//                        * Значение - Строка - имя обработчика заполнения в модуле объекта
...
Процедура ЗаполнитьДокумент(ДокументОбъект, Знач ДанныеЗаполнения, Знач СтратегияЗаполнения = Неопределено, ИсключаяСвойства = "") Экспорт
	...
	РазыменоватьПоляСобытия(ДокументОбъект, ДанныеЗаполнения);
	...
	Если ДанныеЗаполнения.Свойство("Основание") 
		И СтратегияЗаполнения = "ЗаполнитьЗаказНарядПоСчетуНаОплату" Тогда
		
		ДанныеЗаполнения.Вставить("СуммаВключаетНДС", ДанныеЗаполнения.Основание.СуммаВключаетНДС);
	КонецЕсли;
	...
	Справочники.РекламныеБлокиПечатныхФорм.ОбработкаЗаполненияРекламногоБлокаВДокументе(ДокументОбъект);
...
// 
// Возвращаемое значение:
//  Произвольный - Наиболее часто встречающиеся в табличной части значение поля.
//
...
// 
// Возвращаемое значение:
//  Структура - Структурная единица и ячейка для шапки:
// * СтруктурнаяЕдиница - СправочникСсылка.СтруктурныеЕдиницы, Неопределено -
// * Ячейка - СправочникСсылка.Ячейки, Неопределено -
...
//
// Параметры:
//  ПоложениеСклада		 - ПеречислениеСсылка.ПоложениеРеквизитаНаФорме
//	Организация			 - СправочникСсылка.Организации - Организация, для которой определяется система налогообложения
//	СтруктурнаяЕдиница	 - СправочникСсылка.СтруктурныеЕдиницы - Розничный склад, для которой определяется система налогообложения
...
Функция ПолучитьКалендарьСотрудника(Сотрудник = Неопределено) Экспорт
	...
	Календарь = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойКалендарь", , Пользователь);
...
//  ДляЗаявок - Булево
//
Функция ПолучитьВидЗаказаПокупателя(ДляЗаявок = Ложь) Экспорт
	...
	Если ДляЗаявок Тогда
		Возврат РаботаСЗаявками.ВидЗаказаСЗаявками(Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу);
	Иначе
		Возврат Справочники.ВидыЗаказовПокупателей.Основной;
	КонецЕсли;
...
//  ДляЗаявок - Булево
//
Функция ПолучитьВидЗаказНаряда(ДляЗаявок = Ложь) Экспорт
	...
	Если ДляЗаявок Тогда
		Возврат РаботаСЗаявками.ВидЗаказаСЗаявками(Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд);
	Иначе
		Возврат Справочники.ВидыЗаказНарядов.Основной;
	КонецЕсли;
...
Процедура ОбработатьЗаполнениеПоШаблону(ДанныеЗаполнения, ДокументОбъект)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ШаблоныДокументовУНФЗначенияРеквизитов.Реквизит КАК Реквизит,
	|	ШаблоныДокументовУНФЗначенияРеквизитов.Значение КАК Значение
	|ИЗ
	|	Справочник.ШаблоныДокументовУНФ.ЗначенияРеквизитов КАК ШаблоныДокументовУНФЗначенияРеквизитов
	|ГДЕ
	|	ШаблоныДокументовУНФЗначенияРеквизитов.Ссылка = &ШаблонДокумента
	|	И ШаблоныДокументовУНФЗначенияРеквизитов.Использование";
	Запрос.УстановитьПараметр("ШаблонДокумента", ДанныеЗаполнения.ШаблонДокумента);
	РезультатЗапроса = Запрос.Выполнить();
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеЗаполнения.Вставить(Выборка.Реквизит, Выборка.Значение);

	КонецЦикла;

КонецПроцедуры

// Выбирает обработчик заполнения на основе переданного параметра СтратегияЗаполнения,
// затем вызывает этот обработчик из модуля объекта ДокументОбъект.
//
// Параметры:
//  СтратегияЗаполнения	 - Строка - имя процедуры обработчика заполнения в модуле объекта
//                       - Соответствие из КлючИЗначение:
...
Процедура ДополнитьЗначениямиИзНастроек(ДанныеЗаполнения, ДокументОбъект)
	...
	ДополнитьПоложениеЯчейки(ЗначенияИзНастроек, ДокументОбъект);
	ДополнитьПоложениеПатентаВЧеках(ЗначенияИзНастроек, ДокументОбъект);
	...
	ДополнитьПоложениеМестаРезерва(ЗначенияИзНастроек, ДокументОбъект);
...
Процедура ДополнитьОрганизация(ЗначенияИзНастроек, ДокументОбъект)
	...
	ОрганизацияПоУмолчанию = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнаяОрганизация");
...
Процедура ДополнитьПодразделение(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПодразделениеПоУмолчанию = ПерсональныеНастройкиРУНФ.Прочитать("ОсновноеПодразделение");
...
Процедура ДополнитьСтруктурнуюЕдиницу(ЗначенияИзНастроек, ДокументОбъект, ИмяНастройки)
	...
	СтруктурнаяЕдиница = ПерсональныеНастройкиРУНФ.Прочитать(ИмяНастройки); // СправочникСсылка.СтруктурныеЕдиницы
...
Процедура ДополнитьВидЦен(ЗначенияИзНастроек, ДокументОбъект)
	...
	ВидЦенПоУмолчанию = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойВидЦенПродажи");
...
Процедура ДополнитьПоложениеОтветственный(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеОтветственный = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеОтветственный");
...
Процедура ДополнитьПоложениеДатыПоступленияВЗаказеПоставщику(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеДатыПоступленияПоУмолчанию = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьНастройкиЗаказНаряда(ЗначенияИзНастроек, ДокументОбъект, ДанныеЗаполнения)
	...
	Если Не РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ВидОперации) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из РеквизитыЗаказНаряда Цикл
		...
		ИмяНастройки = СтрШаблон("%1ВЗаказНаряде", КлючИЗначение.Ключ);
		ЗначениеПоУмолчанию = КлючИЗначение.Значение;
		ЗначениеНастройки = ПерсональныеНастройкиРУНФ.Прочитать(ИмяНастройки, , , ЗначениеПоУмолчанию);
		
		Если ЗначениеЗаполнено(ЗначениеНастройки) Тогда
			ЗначенияИзНастроек.Вставить(КлючИЗначение.Ключ, ЗначениеНастройки);
		Иначе
			ЗначенияИзНастроек.Вставить(КлючИЗначение.Ключ, ЗначениеПоУмолчанию);
		КонецЕсли;
...
Процедура ДополнитьПоложениеВидаРаботВЗаданииНаРаботу(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеВидаРаботПоУмолчанию = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеВидаРаботВЗаданииНаРаботу");
...
Процедура ДополнитьПоложениеДатыОтгрузкиВЗаказеПокупателя(ЗначенияИзНастроек, ДокументОбъект)
	...
	Если РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(ДокументОбъект.ВидОперации) Тогда
		Возврат;
	КонецЕсли;
	...
	ПоложениеДатыОтгрузкиПоУмолчанию = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеДатыОтгрузкиВЗаказеПокупателя");
...
Процедура ДополнитьПоложениеЗаказаПокупателяВДокументахОтгрузки(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеЗаказаПокупателяВДокументахОтгрузки = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеЗаказаПоставщикуВДокументахПоступления(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеЗаказаПоставщикуВДокументахПоступления = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеЗаказаПокупателяВПеремещенииЗапасов(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеЗаказаПокупателяВПеремещенииЗапасов = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеЗаказаПокупателяВДокументахПроизводства(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеЗаказаПокупателяВДокументахПроизводства = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеЗаказаПокупателяВДокументахПоступления(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеЗаказаПокупателяВДокументахПоступления = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеЗаказаНаПроизводствоВДокументахОтгрузки(ЗначенияИзНастроек, ДокументОбъект)
	...
	Если ТипЗнч(ДокументОбъект.Ссылка) <> Тип("ДокументСсылка.АктВыполненныхРабот")
		И ТипЗнч(ДокументОбъект.Ссылка) <> Тип("ДокументСсылка.СдельныйНаряд") Тогда
		Возврат;
	КонецЕсли;
	...
	ПоложениеЗаказаНаПроизводствоВДокументахОтгрузки = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьИспользоватьРесурсыПредприятияВЗаказеНаПроизводство(ЗначенияИзНастроек, ДокументОбъект)
	...
	ИспользоватьРесурсыПредприятияВЗаказеНаПроизводство = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьНоменклатуройДоставки(ЗначенияИзНастроек, ДокументОбъект)
	...
	НоменклатураДоставки = ПерсональныеНастройкиРУНФ.Прочитать("НоменклатураДоставки"); // СправочникСсылка.Номенклатура
...
Процедура ДополнитьПоложениеСкладаВДокументахПоступления(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеСкладаВДокументахПоступления = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеСкладаВДокументахПоступления");
...
Процедура ДополнитьПоложениеСкладаВДокументахОтгрузки(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеСкладаВДокументахОтгрузки = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеСкладаВДокументахОтгрузки");
...
Процедура ДополнитьПоложениеИсполнителяВСдельномНаряде(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеИсполнителяВСдельномНаряде = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеИсполнителяВСдельномНаряде");
...
Процедура ДополнитьПоложениеСтруктурнойЕдиницыВСдельномНаряде(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеСтруктурнойЕдиницыВСдельномНаряде = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеСкладаВДокументахПроизводства(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеСкладаВДокументахПроизводства = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеЯчейки(ЗначенияИзНастроек, ДокументОбъект)
	
	Если ТипЗнч(ДокументОбъект.Ссылка) <> Тип("ДокументСсылка.ПриходныйОрдер")
		И ТипЗнч(ДокументОбъект.Ссылка) <> Тип("ДокументСсылка.РасходныйОрдер") Тогда
		Возврат;
	КонецЕсли;
	...
	ПоложениеЯчейки = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеЯчейки");
	
	Если ЗначениеЗаполнено(ПоложениеЯчейки) Тогда
		ЗначенияИзНастроек.Вставить("ПоложениеЯчейки", ПоложениеЯчейки);
	Иначе
		ЗначенияИзНастроек.Вставить("ПоложениеЯчейки", Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
	КонецЕсли;
	
КонецПроцедуры

Процедура ДополнитьПоложениеПатентаВЧеках(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеПатентаВЧеках = ПерсональныеНастройкиРУНФ.Прочитать(
		"ПоложениеПатентаВЧеках");
	
	Если ЗначениеЗаполнено(ПоложениеПатентаВЧеках) Тогда
		ЗначенияИзНастроек.Вставить("ПоложениеПатента", ПоложениеПатентаВЧеках);
	Иначе
		ЗначенияИзНастроек.Вставить("ПоложениеПатента", Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
	КонецЕсли;
...
Процедура ДополнитьПоложениеЭквайринговогоТерминалаВБанковскихДокументах(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеЭквайринговогоТерминала = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеСтатьиВБанковскихДокументах(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеСтатьи = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеСтатьи");
...
Процедура ДополнитьПоложениеПодразделенияВБанковскихДокументах(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеПодразделения = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеПодразделенияВДенежныхДокументах");
...
Процедура ДополнитьПоложениеРасходовВОтчетеПереработчика(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеРасходовВОтчетеПереработчика = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеРасходовВОтчетеПереработчика");
...
Процедура ДополнитьПоложениеНастроекНалоговогоУчета(ЗначенияИзНастроек, ДокументОбъект)
	...
	ПоложениеНастроекНалоговогоУчета = ПерсональныеНастройкиРУНФ.Прочитать(
...
Процедура ДополнитьПоложениеМестаРезерва(ЗначенияИзНастроек, ДокументОбъект)
	
	Если ТипЗнч(ДокументОбъект.Ссылка) <> Тип("ДокументСсылка.РезервированиеЗапасов") Тогда
		Возврат;
	КонецЕсли;
	
	Если ОтсутствуетНезаполненныйРеквизит(
		"ПоложениеМестаРезерва",
		ДокументОбъект) Тогда
		Возврат;
	КонецЕсли;
	
	ПоложениеМестаРезерва = ПерсональныеНастройкиРУНФ.Прочитать("ПоложениеМестаРезерва");
	
	Если ЗначениеЗаполнено(ПоложениеМестаРезерва) Тогда
		ЗначенияИзНастроек.Вставить("ПоложениеМестаРезерва", ПоложениеМестаРезерва);
	Иначе
		ЗначенияИзНастроек.Вставить("ПоложениеМестаРезерва", Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
	КонецЕсли;
...
Процедура ДополнитьПредопределеннымиСтруктурнымиЕдиницами(ПредопределенныеЭлементыСправочников, ДокументОбъект)
	...
	Если Не ОтсутствуетНезаполненныйРеквизит("СтруктурнаяЕдиница", ДокументОбъект) Тогда
		...
		ПредопределеннаяСтруктурнаяЕдиница = ПравилаЗаполненияДляСтруктурнойЕдиницы[ТипЗнч(ДокументОбъект)]; // СправочникСсылка.СтруктурныеЕдиницы
		...
		Если ЗначениеЗаполнено(ПредопределеннаяСтруктурнаяЕдиница)
			...
			Если ТипСтруктурнойЕдиницыЕстьВПараметрахВыбора(Реквизит.ПараметрыВыбора) Тогда
				ТипСтруктурнойЕдиницы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПредопределеннаяСтруктурнаяЕдиница,
					"ТипСтруктурнойЕдиницы", Истина);
...
Процедура РазыменоватьПоляСобытия(ДокументОбъект, РазыменованныеПоля)
	
	Если Не РазыменованныеПоля.Свойство("Событие") Тогда
		Возврат;
	КонецЕсли;
	
	РазыменоватьПоляСобытияКонтрагент(ДокументОбъект, РазыменованныеПоля);
	РазыменоватьПоляСобытияПроект(РазыменованныеПоля);
	
КонецПроцедуры

Процедура РазыменоватьПоляСобытияКонтрагент(ДокументОбъект, РазыменованныеПоля)
	...
	ИначеЕсли ТипЗнч(КонтактСобытия) = Тип("СправочникСсылка.Лиды") Тогда
		...
		Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда
			...
			ЗаявкиИспользуются = РаботаСЗаявками.ЗаявкиИспользуются(ДокументОбъект.ВидЗаказа, ДокументОбъект.ВидОперации);
			Если ЗаявкиИспользуются Тогда
				Лид = КонтактСобытия;
			Иначе
	...
	Если Лид <> Неопределено Тогда
		РазыменованныеПоля.Вставить("Лид", Лид);
	КонецЕсли;
...
Процедура РазыменоватьПоляОрганизацииНалогообложениеНДС(РазыменованныеПоля, ДокументОбъект, Организация)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОбъект, "Дата")
		И ЗначениеЗаполнено(ДокументОбъект.Дата) Тогда
	...
	РазыменованныеПоля.Вставить("НалогообложениеНДС",
		НалогиУНФ.НалогообложениеНДС(Организация, , ДатаДокумента));
...
// 
// Возвращаемое значение:
//  Массив из Строка - массив строк - имен реквизитов документа.
//
...
Функция ТребуетсяЗаполнятьЗаказВ_ТЧ_Предоплата(ДокументОбъект)
	...
	Возврат МассивТипов.Найти(ТипЗнч(ДокументОбъект)) = Неопределено;
