...
// Устанавливает признак использования системы быстрых платежей
//
// Параметры:
//  Использовать - Булево - признак включения опции. Если задано значение неопределено опция определяется запросом из настроек подключения
//
Процедура УстановитьФункциональнуюОпциюИспользованиеСистемыБыстрыхПлатежей(Знач Использовать = Неопределено) Экспорт
	
	Если Использовать = Неопределено Тогда
		Использовать = ИнтеграцияСПлатежнымиСистемамиУНФПовтИсп.ИнтеграцияДоступна();
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	Константы.ИспользоватьСистемуБыстрыхПлатежей.Установить(Использовать);
...
Процедура ПриЗаписиНастроекИнтеграции(ИмяРегистра, ПараметрыОплаты, Отказ, СообщениеОбОшибке) Экспорт
	...
	Попытка
		...
		СрокДействияQRКода	= ПараметрыОплаты.Получить("СрокДействияQRКода");
		...
		Запись.СрокДействияQRКода				= СрокДействияQRКода;
		
		Если ИмяРегистра = "СоответствиеНастроекИнтеграцииУНФ" Тогда
			
			ВидПоступления = ПараметрыОплаты.Получить("ВидПоступленияОплаты");
			Если ЗначениеЗаполнено(ВидПоступления) Тогда
				Запись.ВидПоступленияОплаты = ВидПоступления;
			ИначеЕсли Интеграция.ВариантНастройки = Перечисления.ВариантыНастройкиСБП.b2b Тогда
...
Процедура ПриНастройкеЭлементовФормыИнтеграции(ИмяРегистра, НастройкиФормы, ОбщиеЭлементы, ДополнительныеПараметры) Экспорт
	...
	НастройкиФормы.ЭлементыНастроекОплаты.СрокДействияQRКода.Видимость = Ложь;

	Если ИмяРегистра = "СоответствиеНастроекИнтеграцииУНФ" Тогда

		Если ЗначениеЗаполнено(ДополнительныеПараметры.ИдентификаторУчастника) Тогда
				
			НастройкиУчастникаСБП  = СистемаБыстрыхПлатежейСлужебный.НастройкиУчастникаСБП(
				ДополнительныеПараметры.ИдентификаторУчастника);
		...
		НастройкиФормы.ЭлементыНастроекОплаты.ВидПоступленияОплаты.Подсказка
			= НСтр("ru = 'Если в выписке банка, в операциях оплаты через СБП отражаются реквизиты контрагента-плательщика - укажите вид операции ""Оплата от покупателя"".
		                 |Если отражаются реквизиты банка - укажите вид операции ""Поступление по платежным картам"".'");
			
		НастройкиФормы.ЭлементыНастроекОплаты.ВидПоступленияОплаты.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
				
	КонецЕсли;
	
	ИнтеграцияСПлатежнымиСистемамиУНФКлиентСервер.НастроитьДополнительнуюИнформациюНастройкиПодключенияСБП(
		ОбщиеЭлементы.ДекорацияДополнительнаяИнформация)
...
//
Процедура ПриЗаполненииФормыИнтеграции(НастройкиОплаты, ОбщиеНастройки, ДополнительныеПараметры, ДополнительныеНастройки = Неопределено) Экспорт
	
	Если ДополнительныеПараметры <> Неопределено 
		И ДополнительныеПараметры.Свойство("ТорговыйОбъект") Тогда
	...
	Если ДополнительныеНастройки <> Неопределено
		И ДополнительныеНастройки.Свойство("b2b")
		И Не ЗначениеЗаполнено(ДополнительныеНастройки.b2b.ВидПоступления) Тогда

		Если ДополнительныеНастройки.b2b.ВидПоступления = "ACQUIRING" Тогда
			НастройкиОплаты.ВидПоступленияОплаты = Перечисления.ВидыПоступленияОплатыСБПb2b.Эквайринг;
		ИначеЕсли ДополнительныеНастройки.b2b.ВидПоступления = "PAYMENT_FROM_THE_BUYER" Тогда
			НастройкиОплаты.ВидПоступленияОплаты = Перечисления.ВидыПоступленияОплатыСБПb2b.ОплатаОтПокупателя;
		Иначе
			НастройкиОплаты.ВидПоступленияОплаты = Перечисления.ВидыПоступленияОплатыСБПb2b.Эквайринг;
		КонецЕсли;
...
Процедура ПриФормированииЗаказаНаОплатуСБП(ДокументОплаты, 
	...
	Иначе
		...
		ТаблицаНастроек = СоответствиеНастроекИнтеграции(ТорговаяТочка);
		...
		СрокЖизниQRКода		= ?(ЗначениеЗаполнено(ПараметрыОплаты.СрокДействияQRКода), 
			ПараметрыОплаты.СрокДействияQRКода, 
			24 * 10); // 10 дней в часах
		...
		РеквизитыОплаты.Вставить("СрокЖизниQRКода",				Окр(СрокЖизниQRКода * 60)); // в минутах
		...
		Если ТипЗнч(ДокументОплаты) = Тип("ДокументСсылка.ПлатежнаяСсылкаСБП") Тогда
			...
			ОснованиеПлатежа  = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОплаты, "ОснованиеПлатежа");			
			НазначениеПлатежа = ПолучитьНазначениеПлатежа(ЗаказНаОплату.ШаблоныНазначений, ОснованиеПлатежа, РеквизитыОплаты);
...
Процедура ПриЗагрузкеСтатусаОперации(
		...
		РезультатОбработки,
		Обработан,
		ТребуетФискализацииНаОблачнойКассе = Истина,
		ПараметрыФискализации = Неопределено) Экспорт
	...
	Если ТипЗнч(НастройкаПодключения) = Тип("СправочникСсылка.НастройкиПодключенияКСистемеБыстрыхПлатежей") Тогда
		// обработка вида поступления оплаты
		...
		Если ВариантНастройки = Перечисления.ВариантыНастройкиСБП.b2b Тогда
			...
			Настройки 		  	 = СоответствиеНастроекИнтеграции(НастройкаПодключения);
			ВидПоступленияОплаты = ?(Настройки.Количество(), Настройки[0].ВидПоступленияОплаты, Неопределено);
			
		Иначе
			ВидПоступленияОплаты = Перечисления.ВидыПоступленияОплатыСБПb2b.Эквайринг;
		КонецЕсли;
	Иначе
		ВидПоступленияОплаты = Неопределено;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДокументОперации)
		И РезультатОбработки.СтатусОперации = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполнена() Тогда
		...
		Попытка
			...
			Если РезультатЗапроса.Пустой() Тогда
				...
				ДокументОплаты = СоздатьДокументОперацияПоПлатежнойСистеме(
					...
					ВидПоступленияОплаты); 
					
				Если ЗначениеЗаполнено(ПараметрыФискализации) Тогда
					ПараметрыФискализации.ДокументОперации = ДокументОплаты.Ссылка;
					ДокументОплаты.ФискализируетсяЗаказомВОблачнойКассе = Истина;
				КонецЕсли;
			...
			Иначе
				...
				Пока ВыборкаЗапроса.Следующий() Цикл
					Если ЗначениеЗаполнено(ПараметрыФискализации) Тогда
						ДокументОплаты = ВыборкаЗапроса.Ссылка;
						ПараметрыФискализации.ДокументОперации = ВыборкаЗапроса.Ссылка;
						ДокументОплаты.ФискализируетсяЗаказомВОблачнойКассе = Истина;
					КонецЕсли;
				КонецЦикла;
	...
	ИначеЕсли РезультатОбработки.СтатусОперации = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииОшибка() Тогда
...
#Область НастройкиОплатыИнтернетЭквайринг
...
//                        необходимо скрыть на форме настройки интеграции. 
//
Процедура ПриОпределенииНастроекИнтеграцииИнтернетЭквайринга(ИмяРегистра, Настройки) Экспорт
...
//
Процедура ПриНастройкеЭлементовФормыПодключенияИнтернетЭквайринг(
	НастройкиФормы,
	НастройкиЭлементов,
	...
	Если ИспользоватьНесколькоВидовОплаты() Тогда
		...
		ЭлементФормы 		  = НастройкиЭлементов.ЭлементыНастроекОплаты.СпособОплаты;
		...
		Если ИспользоватьНесколькоОрганизаций() Тогда
			...
			ИмяРеквизитаОрганизация = НастройкиЭлементов.ЭлементыНастроекОплаты["Организация"].ПутьКДанным;
			Связь = Новый СвязьПараметраВыбора("Отбор.Организация", ИмяРеквизитаОрганизация);
			СвязиПараметровВыбора.Добавить(Связь);
		...
		НовыйПараметр = Новый ПараметрВыбора("Отбор.Тип", Перечисления.ТипыСпособовОплат.Эквайринг);
		ПараметрыВыбора.Добавить(НовыйПараметр);
	...
	// настройка подключения кассовых ссылок         
	Если ИспользоватьНесколькоОрганизаций() Тогда
		Организация = НастройкиЭлементов.ЗначенияНастроекОплаты.Организация;
		НастройкиЭлементов.ЗначенияНастроекОплаты.Организация = Организация;
	Иначе
		Организация = ОрганизацияПоУмолчанию();
	КонецЕсли;
	
	Если ИспользоватьСтруктурныеЕдиницы() Тогда
		НастройкиЭлементов.ЗначенияНастроекОплаты.ТорговыйОбъект = НастройкиЭлементов.ЗначенияНастроекОплаты.ТорговыйОбъект;
	Иначе
		НастройкиЭлементов.ЗначенияНастроекОплаты.ТорговыйОбъект = СтруктурнаяЕдиницаПоУмолчанию();
	КонецЕсли;
	
	Если ИспользоватьНесколькоВидовОплаты() Тогда
		НастройкиЭлементов.ЗначенияНастроекОплаты.СпособОплаты = НастройкиЭлементов.ЗначенияНастроекОплаты.СпособОплаты;
	Иначе
		НастройкиЭлементов.ЗначенияНастроекОплаты.СпособОплаты = ВидОплатыПоУмолчанию(Организация);
	КонецЕсли;
...
//    Отказ установлено значение Истина.
//
Процедура ПриЗаписиНастроекПодключенияИнтернетЭквайринга(
		ПараметрыОплаты,
		Отказ,
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СоответствиеНастроекИнтеграцииУНФИнтернетЭквайринг");
		ЭлементБлокировки.УстановитьЗначение("Организация",  					Организация);
		ЭлементБлокировки.УстановитьЗначение("СпособОплаты",  					СпособОплаты);
		ЭлементБлокировки.УстановитьЗначение("ТорговыйОбъект",  				ТорговыйОбъект);
		...
		Если ЗначениеЗаполнено(Интеграция) Тогда
			...
			|	СоответствиеНастроекИнтеграции.ТорговыйОбъект КАК ТорговыйОбъект
			|ИЗ
			|	РегистрСведений.СоответствиеНастроекИнтеграцииУНФИнтернетЭквайринг КАК СоответствиеНастроекИнтеграции
			|ГДЕ
			|	СоответствиеНастроекИнтеграции.Интеграция = &Интеграция";
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				Набор = РегистрыСведений.СоответствиеНастроекИнтеграцииУНФИнтернетЭквайринг.СоздатьНаборЗаписей();
				Набор.Отбор.Организация.Установить(ВыборкаДетальныеЗаписи.Организация);
				Набор.Отбор.СпособОплаты.Установить(ВыборкаДетальныеЗаписи.СпособОплаты);
				Набор.Отбор.ТорговыйОбъект.Установить(ВыборкаДетальныеЗаписи.ТорговыйОбъект);
				Набор.Записать();
		...
		Запись = РегистрыСведений["СоответствиеНастроекИнтеграцииУНФИнтернетЭквайринг"].СоздатьМенеджерЗаписи();
...
//
Процедура ПриФормированииЗаказаНаВозвратИнтернетЭквайринга(
		ДокументВозврата,
		ЗаказНаВозврат,
		НастройкаПодключения,
		ДокументВозвратаСтруктура) Экспорт
...
//  ТребуетФискализацииНаОблачнойКассе - Булево - признак провести фискализацию на облачной кассе.
//
Процедура ПриЗагрузкеСтатусаОперацииИнтернетЭквайринг(
		ДокументОперации,
		НастройкаПодключения,
		РезультатОбработки,
		Обработан,
	...
	|	ОперацияПоПлатежнымКартам.ПометкаУдаления = ЛОЖЬ
	|	И ОперацияПоПлатежнымКартам.СсылочныйНомер = &ИдентификаторПлатежа
	|	И ОперацияПоПлатежнымКартам.СпособОплаты = ЗНАЧЕНИЕ(Перечисление.СпособыОплатыЭквайринг.ОплатаКартой)";
	#КонецОбласти
	...
	Если ЗначениеЗаполнено(ДокументОперации)
		...
		Попытка
			...
			ИдентификаторПлатежа 	= РезультатОбработки.ПараметрыОперации.ИдентификаторОперации;
...
Процедура ВозвратОплаты(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	Если ТипЗнч(ПараметрыПроцедуры.Интеграция) = Тип("СправочникСсылка.НастройкиПодключенияКСистемеБыстрыхПлатежей") Тогда
...
Процедура СтатусВозвратОплаты(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	Иначе
		РезультатОперации = ИнтернетЭквайринг.СтатусВозврата(ПараметрыПроцедуры.ДокументВозврата);
	КонецЕсли;
...
Функция ОтменитьОперацию(ДокументОплаты, 
	...
	Если РезультатОперации.СтатусОперации = НастройкиПлатежныхСистемКлиентСервер.СтатусОперацииВыполняется() Тогда
		...
		Иначе
			...
			Иначе
				ИнтернетЭквайринг.УдалитьИдентификаторыОперации(ДокументОплаты);
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьПараметрыДокументаОплаты(ДокументОплаты) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументОплаты) = Тип("ДокументСсылка.ОперацияПоПлатежнымКартам") Тогда
		...
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА Т.СпособОплаты = ЗНАЧЕНИЕ(Перечисление.СпособыОплатыЭквайринг.ОплатаСБП)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.СБП)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.БанковскаяКарта)
		|	КОНЕЦ КАК ВидОплаты,
		|	Т.СсылочныйНомер КАК СсылочныйНомер,
		|	Т.Ссылка.Организация КАК Организация,
		|	Т.ИдентификаторПлатежнойСистемы КАК ИдентификаторПлатежнойСистемы,
		...
		|			ИЛИ Т.СпособОплаты = ЗНАЧЕНИЕ(Перечисление.СпособыОплатыЭквайринг.ОплатаОнлайн))";
...
Функция ТекстЗапросаНастройкиИнтеграции(ИдентификаторПлатежнойСистемы = Неопределено, ИмяРегистра = "СоответствиеНастроекИнтеграцииРМК") Экспорт
	...
	|	ВЫБОР
	|		КОГДА СоответствиеНастроекИнтеграции.СпособОплаты.Договор.РасчетКомиссииВОтчетеЭквайера
	|			ТОГДА 0
	...
	Если ИмяРегистра = "СоответствиеНастроекИнтеграцииУНФИнтернетЭквайринг" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"СоответствиеНастроекИнтеграции.ИдентификаторПлатежнойСистемы",
			"НЕОПРЕДЕЛЕНО"); 
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.СБП",
			"ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.БанковскаяКарта");
	КонецЕсли;
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	СистемаБыстрыхПлатежейСобытия.ПриОпределенииДоступностиПереводов(ПереводыДоступны);
	
	Если ПереводыДоступны Тогда
		...
		Если ЗначениеЗаполнено(ТипыОбъектов) Тогда
...
Процедура ПолучитьСтатусыОперацияC2B(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	ИнтернетЭквайринг.СтатусыОпераций();
...
Функция НайтиИлиСоздатьЭквайринговыйТерминалСБП(Организация, КассаККМ = Неопределено) Экспорт
	...
	Касса = ?(ЗначениеЗаполнено(КассаККМ), 
		КассаККМ, 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "КассаПоУмолчанию"));
	...
	Если НЕ ЗначениеЗаполнено(Эквайрер) Тогда
		...
		ОсновнойОтветственный = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойОтветственный");
...
Процедура ПриПолученииОблачныхКассОрганизации(Организация, Кассы) Экспорт
	...
	|	ПодключаемоеОборудование.Ссылка КАК ОблачнаяКасса
	|ИЗ
	|	Справочник.ПодключаемоеОборудование КАК ПодключаемоеОборудование
	|ГДЕ
	|	ПодключаемоеОборудование.ТипОборудования = ЗНАЧЕНИЕ(Перечисление.ТипыПодключаемогоОборудования.ОблачнаяККТ)
	|	И ПодключаемоеОборудование.Организация = &Организация";
	
	Запрос.УстановитьПараметр("Организация", Организация);
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаПоРезультатамЗапроса = РезультатЗапроса.Выбрать();
	
	Пока ВыборкаПоРезультатамЗапроса.Следующий() Цикл
		Кассы.Добавить(ВыборкаПоРезультатамЗапроса.ОблачнаяКасса);
	КонецЦикла;
...
//
Процедура ПриЗаписиНастройкиПодключенияОблачнойКассы(Организация, Наименование, Касса) Экспорт
	
	Если ЗначениеЗаполнено(Организация) Тогда
		Касса = ОборудованиеЧекопечатающиеОблачныеККТ.НовоеОборудованиеОблачнойККТ(Организация, Наименование);
	КонецЕсли;
...
Функция СоздатьДокументОперацияПоПлатежнойСистеме(Интеграция, ОснованиеПлатежа, ИдентификаторПлатежа, СуммаОплаты, ВидПоступленияОплаты)
	
	Если ТипЗнч(Интеграция) = Тип("СправочникСсылка.НастройкиПодключенияКСистемеБыстрыхПлатежей") Тогда
		НастройкиИнтеграции		= НастройкиТорговойТочки(Интеграция);
		СпособОплаты = Перечисления.СпособыОплатыЭквайринг.ОплатаСБП; 
		ВидПлатежнойКарты		= ИнтеграцияСПлатежнымиСистемамиУНФКлиентСервер.ВидПлатежнойКартыСБП();
	Иначе
		НастройкиИнтеграции		= Новый Структура("Идентификатор", Неопределено);
		СпособОплаты = Перечисления.СпособыОплатыЭквайринг.ОплатаОнлайн;
		ВидПлатежнойКарты		= НСтр("ru = 'Картой'");
	КонецЕсли;
	
	ДатаОплаты 				= ТекущаяДатаСеанса();
	СтруктураРеквизитов		= Новый Структура();
	
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("БанковскийСчет", ОснованиеПлатежа.Метаданные()) Тогда
		БанковскийСчет		= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОснованиеПлатежа, "БанковскийСчет");
	Иначе
		
		БанковскийСчет		= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			ОснованиеПлатежа, 
			"Организация.БанковскийСчетПоУмолчанию");
			
	КонецЕсли;
	
	РеквизитыОснования		= Новый Структура("Касса, Организация", "Организация.КассаПоУмолчанию", "Организация");
	РеквизитыОснования		= ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОснованиеПлатежа, РеквизитыОснования);
		
	Касса					= РеквизитыОснования.Касса;
	...
	ВидОперации				= ?(ВидПоступленияОплаты = Перечисления.ВидыПоступленияОплатыСБПb2b.ОплатаОтПокупателя,
		Перечисления.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП,
		Перечисления.ВидыОперацийЭквайринга.ПоступлениеОплатыОтПокупателя);
	Если ТипЗнч(ЭквайринговыйТерминал.Касса) = Тип("СправочникСсылка.КассыККМ") Тогда
		КассаККМ = ЭквайринговыйТерминал.Касса;
	Иначе
	...
	АдресСайта = ?(ЗначениеЗаполнено(ОснованиеПлатежа.АдресСайта), ОснованиеПлатежа.АдресСайта, "");
	...
	ДанныеЗаполнения.Вставить("СпособОплаты", 					СпособОплаты);
	...
	ДанныеЗаполнения.Вставить("РасчетВИнтернете",				Истина);
	ДанныеЗаполнения.Вставить("АдресСайта",						АдресСайта);
...
Процедура СоздатьВТДокументовОплаты(Запрос)
	...
	|			ИЛИ ОперацияПоПлатежнымКартам.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЭквайринга.ОплатаСчетаЧерезСБП)
...
Процедура ОбновитьИспользованиеСистемыБыстрыхПлатежейПриЗаписи(Источник, Отказ, Замещение) Экспорт
	
	Если Источник.Количество() > 0 
		И Не ПолучитьФункциональнуюОпцию("ИспользоватьСистемуБыстрыхПлатежей") Тогда
		
		УстановитьФункциональнуюОпциюИспользованиеСистемыБыстрыхПлатежей(Истина);
		
	КонецЕсли;
	
КонецПроцедуры

Функция СоответствиеНастроекИнтеграции(Интеграция)
	...
	|	СоответствиеНастроекИнтеграции.СпособОплаты КАК СпособОплаты,
	|	СоответствиеНастроекИнтеграции.ТорговыйОбъект КАК ТорговыйОбъект,
	|	СоответствиеНастроекИнтеграции.СпособОплаты.БанковскийСчетЭквайринг.НомерСчета КАК РасчетныйСчет,
	|	СоответствиеНастроекИнтеграции.СрокДействияQRКода КАК СрокДействияQRКода,
	|	NULL КАК ВидПоступленияОплаты
	|ИЗ
	|	РегистрСведений.СоответствиеНастроекИнтеграцииРМК КАК СоответствиеНастроекИнтеграции
	|ГДЕ
	|	СоответствиеНастроекИнтеграции.Интеграция = &Интеграция
	...
	|	СоответствиеНастроекИнтеграции.СпособОплаты,
	|	СоответствиеНастроекИнтеграции.ТорговыйОбъект,
	|	СоответствиеНастроекИнтеграции.СпособОплаты.БанковскийСчетЭквайринг.НомерСчета,
	|	СоответствиеНастроекИнтеграции.СрокДействияQRКода,
	|	СоответствиеНастроекИнтеграции.ВидПоступленияОплаты
	|ИЗ
	|	РегистрСведений.СоответствиеНастроекИнтеграцииУНФ КАК СоответствиеНастроекИнтеграции
	|ГДЕ
	|	СоответствиеНастроекИнтеграции.Интеграция = &Интеграция";
