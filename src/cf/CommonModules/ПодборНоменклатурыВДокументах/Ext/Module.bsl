...
Процедура НачальноеЗаполнениеНастроекПодбора(Пользователь = Неопределено, СтандартнаяОбработка = Истина) Экспорт
	...
	Если СтандартнаяОбработка = Истина Тогда
		...
		ПерсональныеНастройкиРУНФ.Установить(Истина, "ИспользоватьНовыйМеханизмПодбора", Пользователь);
...
//
// Параметры:
//  ТекущийПользователь - СправочникСсылка.Пользователи - пользователь, для которого устанавливаются настройки подбора
//
Процедура УстановитьСтандартныеНастройкиПодбора(ТекущийПользователь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекущийПользователь) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр(
			"ru= 'Не указан пользователь, для которого устанавливаются настройки подбора по умолчанию.'"));
		Возврат;
	КонецЕсли;
	...
	Для Каждого Настройка Из НастройкиПодбораПоУмолчанию Цикл
		ПерсональныеНастройкиРУНФ.Установить(Настройка.Значение, Настройка.Ключ, ТекущийПользователь);
	КонецЦикла;
