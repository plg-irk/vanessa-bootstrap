...
Функция СписокНастроек() Экспорт  
	Возврат "СобиратьСтатистикуПоПоказателям, НижняяГраницаСчитыванияСтатистики, НеДобавлятьИдентификаторВСтатистику, ПозицияИдентификатораВСтатистике, ИдентификаторИнформационнойБазы";
КонецФункции 
...
Функция НастройкиСтатистикиПоПоказателям() Экспорт
	...
	ЗаполнитьЗначенияСвойств(СтруктураНастроек, НаборКонстант, СписокНастроек);    
	
	ЗапросТекущейОбласти = Новый Запрос;
	ЗапросТекущейОбласти.Текст = "ВЫБРАТЬ
	                             |	ОбластиДанных.Статус КАК Статус,
	                             |	ОбластиДанных.ОбластьДанныхВспомогательныеДанные КАК ОбластьДанныхВспомогательныеДанные
	                             |ИЗ
	...
	Выборка = ЗапросТекущейОбласти.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		НомерТекущейОбласти = Выборка.ОбластьДанныхВспомогательныеДанные;
	Иначе
		НомерТекущейОбласти = 0;
	КонецЕсли;  
	МассивДанных = Новый Массив;
	МассивДанных.Добавить(НомерТекущейОбласти);
	МассивДанных.Добавить(СтруктураНастроек.ИдентификаторИнформационнойБазы);
	СтруктураНастроек.Вставить("ИдентификаторБазы", ИдентификаторБазыЧерезХешированиеДанных(МассивДанных));
	
	ИнтерфейсВыставленияСчетов = ПолучитьФункциональнуюОпцию("ИнтерфейсВыставлениеСчетов");
	Если ИнтерфейсВыставленияСчетов Тогда
		СтруктураНастроек.Вставить("ОписательФункциональности", "ВыставлениеСчетов");
	Иначе
		СтруктураНастроек.Вставить("ОписательФункциональности", "СтандартнаяФункциональность");
	КонецЕсли;
	
	ЭтоБизнесПриложение = ВозможностиПриложения.ЭтоБизнесПриложение();
	СтруктураНастроек.Вставить("ЭтоБизнесПриложение", ЭтоБизнесПриложение);
...
Функция ИдентификаторБазыЧерезХешированиеДанных(МассивИдентифицирующихДанных) Экспорт
	
	ХешированиеДанныхОбъект = Новый ХешированиеДанных(ХешФункция.CRC32);
	Для Каждого Данные из МассивИдентифицирующихДанных Цикл
		ХешированиеДанныхОбъект.Добавить(Строка(Данные));
	КонецЦикла;
	
	Возврат Формат(ХешированиеДанныхОбъект.ХешСумма,"ЧГ=0;");
