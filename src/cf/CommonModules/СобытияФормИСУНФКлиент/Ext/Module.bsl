...
Процедура ПриЗакрытииФормыВыбораВидаЦен(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтрокаТовары Из ДокументОбъект.Товары Цикл
		...
		ДанныеСтроки.Вставить("ВидЦен",           Результат);
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТовары, "ЕдиницаИзмерения")
			...
			ДанныеСтроки.Вставить("ЕдиницаИзмерения", СтрокаТовары.ЕдиницаИзмерения);
		Иначе
			ДанныеСтроки.Вставить("ЕдиницаИзмерения", Неопределено);
		КонецЕсли;
	...
	ИнтеграцияИСУНФВызовСервера.ПерезаполнитьЦеныПоВидуЦен(ДанныеТоваров,
		Результат,
		ДокументОбъект.Дата);
		
	ПараметрыПоиска = Новый Структура("Номенклатура, Характеристика");
	Для Каждого СтрокаДанных Из ДанныеТоваров Цикл
		
		ЗаполнитьЗначенияСвойств(ПараметрыПоиска, СтрокаТовары);
		СтрокиТоваров = ДокументОбъект.Товары.НайтиСтроки(ПараметрыПоиска);
		Для Каждого СтрокаТовары Из СтрокиТоваров Цикл
			СтрокаТовары.Цена = СтрокаДанных.Цена;
			СобытияФормЕГАИСКлиент.ПриИзмененииЦены(СтрокаТовары);
		КонецЦикла;
