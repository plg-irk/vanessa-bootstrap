...
Процедура ДобавитьИзменитьКодМаркировкиКегаНаОборудованииРозлива(Форма, Элемент, ДополнительныеПараметры, ИдентификаторСтроки)
	...
	Если Не ИдентификаторСтроки = Неопределено Тогда
		СтрокаТЧ = Форма.Объект[Элемент].НайтиПоИдентификатору(ИдентификаторСтроки);
		Если Не СтрокаТЧ.ВидПродукцииИС = Перечисления.ВидыПродукцииИС.Пиво
			И Не СтрокаТЧ.ВидПродукцииИС = Перечисления.ВидыПродукцииИС.БезалкогольноеПиво Тогда
			Возврат;
		КонецЕсли;
	...
	ВидыПивнойПродукции = Новый Массив;
	ВидыПивнойПродукции.Добавить(Перечисления.ВидыПродукцииИС.Пиво);
	ВидыПивнойПродукции.Добавить(Перечисления.ВидыПродукцииИС.БезалкогольноеПиво);
	...
	Запрос.УстановитьПараметр("ВидыПродукцииИСМП", ВидыПивнойПродукции);
