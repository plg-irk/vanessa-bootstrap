...
Процедура ДанныеПанелиКонтактнойИнформацииВыбор(Форма, Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка) Экспорт
	...
	// При нажатии на пиктограмму включаем возможность выбора из списка
	Если СтрНайти(Поле.Имя,"Пиктограмма") <> 0
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыКИ") = Истина
		И ТипЗнч(Форма.ПараметрыКИ) = Тип("Структура")
		И Форма.ПараметрыКИ.Свойство("ОписаниеКоманд")Тогда 

		ПараметрыКИ = ОбщегоНазначенияСлужебныйКлиент.СкопироватьСтруктуру(Форма.ПараметрыКИ, Ложь); 
		ПараметрыКИ.Вставить("СтрокаКИ", СтрокаКИ);

		ОписаниеКоманд = ПараметрыКИ.ОписаниеКоманд;
		КомандыДляВывода = ОписаниеКоманд[Элемент.ТекущиеДанные.ТипКИ];	
			
		КоличествоКоманд = КомандыДляВывода.Количество();
		// Если команда одна, то выполняем существующее действие
		Если КоличествоКоманд > 1 Тогда
			Список = Новый СписокЗначений;
			Для Каждого Команда Из КомандыДляВывода Цикл 
				// Ввод комментариев из формы списка отключаем
				Если Команда.Ключ = "ВвестиКомментарийКАдресу" ИЛИ Команда.Ключ = "ПоказатьИсториюИзменений" Тогда
					Продолжить;	
				КонецЕсли;  
				Если Команда.Ключ = "ОтправитьSMS" И ПараметрыКИ.МожноОтправлятьSMS <> Истина Тогда
					Продолжить;	
				КонецЕсли;  
				Если НЕ ЗначениеЗаполнено(Команда.Значение.Действие) Тогда
					Продолжить;	
				КонецЕсли;
			...
			Если Список.Количество() > 1 Тогда 
				ОповещениеМеню = Новый ОписаниеОповещения("ПослеВыбораИзМеню", ЭтотОбъект, ПараметрыКИ);
...
Функция ОбрабатыватьОповещения(Форма, ИмяСобытия, Параметр) Экспорт
	...
	Результат = (ИмяСобытия = "Запись_Контрагент" и Параметр.ИзменилиКонтактнуюИнформацию)
...
// Процедура описания оповещения выбора из меню контактной информации 
//
// Параметры:
//  ВыбранныйЭлемент - ЭлементСпискаЗначений - Выбранный элемент меню
//  ДополнительныеПараметры - Структура - Дополнительные параметры
Процедура ПослеВыбораИзМеню(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;    

	СтрокаКИ = ДополнительныеПараметры.СтрокаКИ;  
	//ДополнительныеПараметры.Удалить("СтрокаКИ");
	
	ПараметрыКоманды = Новый Структура;   
	
	// Заполняем вспомогательную структуру дополнительных параметров 
	// см. УправлениеКонтактнойИнформациейКлиент.ДополнительныеПараметрыДляВыполненияКоманд   
	ДополнительныеПараметры = Новый Структура;         
	ДополнительныеПараметры.Вставить("Форма", ЭтотОбъект);
	ДополнительныеПараметры.Вставить("ИмяРеквизита", "");
	...
	Если (СтрокаКИ.ТипКИ = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон")
			И ВыбранныйЭлемент.Значение <> "УправлениеКонтактнойИнформациейКлиент.ПередСозданиемSMS" 
			// см. ТелефонияКлиент.ПозвонитьПоНомеру
			И НЕ (ТелефонияКлиент.ПоддерживаютсяВызовы() И (Действие = "ОткрытьСобытие" ИЛИ Действие = "ОткрытьСобытиеИЗвонок"))) Тогда
		// Для телефонных звонков ВладелецКИ должен быть структурой, иначе подтягиваются
		// все номера, включая подчиненные контактные лица
		ВладелецКонтактнойИнформации = Новый Структура;
		ВладелецКонтактнойИнформации.Вставить("ЗначениеКИ", СтрокаКИ.ПредставлениеКИ);
		ВладелецКонтактнойИнформации.Вставить("Кому", СтрокаКИ.ПредставлениеКИ);
		ВладелецКонтактнойИнформации.Вставить("Контакт", СтрокаКИ.ВладелецКИ);  
		ВладелецКонтактнойИнформации.Вставить("КонтактРодитель", Неопределено); 	
		ДополнительныеПараметры.Вставить("ВладелецКонтактнойИнформации", ВладелецКонтактнойИнформации);		
	Иначе	
		ДополнительныеПараметры.Вставить("ВладелецКонтактнойИнформации", СтрокаКИ.ВладелецКИ);
	КонецЕсли;

	ПараметрыКоманды.Вставить("ДополнительныеПараметры", ДополнительныеПараметры);
	
	// Итоговая передаваемая структура
	КонтактнаяИнформация = Новый Структура;
	КонтактнаяИнформация.Вставить("Вид", Неопределено);
	КонтактнаяИнформация.Вставить("Тип", СтрокаКИ.ТипКИ);
	КонтактнаяИнформация.Вставить("Значение", СтрокаКИ.ПредставлениеКИ);
	КонтактнаяИнформация.Вставить("Представление", СтрокаКИ.ПредставлениеКИ);
	ПараметрыКоманды.Вставить("КонтактнаяИнформация", КонтактнаяИнформация);
	
	//УправлениеКонтактнойИнформациейКлиент.ПослеВыбораИзМеню(ВыбранныйЭлемент, ПараметрыКоманды);  
	Если ВыбранныйЭлемент <> Неопределено Тогда
		
		ИмяОбработчика = ВыбранныйЭлемент.Значение;
		...
		ВыполнитьОбработкуОповещения(Новый ОписаниеОповещения(ИмяПроцедуры, ОбщегоНазначенияКлиент.ОбщийМодуль(ИмяМодуля),
			ПараметрыКоманды.ДополнительныеПараметры), ПараметрыКоманды.КонтактнаяИнформация);	
