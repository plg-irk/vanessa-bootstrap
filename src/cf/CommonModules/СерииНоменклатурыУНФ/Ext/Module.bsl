...
Процедура ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, ИмяТабличнойЧасти = "Запасы", 
	...
	Если ЭтоЗаказПокупателя И РаботаСЗаявкамиКлиентСервер.ЭтоТипЗаказНаряда(Объект.ВидОперации) Тогда
	...
	|		ПО ВЫБОР
	|			КОГДА ВыборкаИзТабличнойЧастиЗапасы.Ссылка.ИспользоватьСерииИзКатегории
	|				ТОГДА ВыборкаИзТабличнойЧастиЗапасы.Ссылка.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|			ИНАЧЕ ВыборкаИзТабличнойЧастиЗапасы.Ссылка = ПолитикиУчетаСерий.Владелец
	|		КОНЕЦ
	...
	|	И ЕСТЬNULL(ПолитикиУчетаСерий.ПолитикаУчетаСерий, ВЫБОР
	|		КОГДА ВыборкаИзТабличнойЧастиЗапасы.Ссылка.ИспользоватьСерииИзКатегории
	|			ТОГДА ВыборкаИзТабличнойЧастиЗапасы.Ссылка.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|		ИНАЧЕ ВыборкаИзТабличнойЧастиЗапасы.Ссылка.ПолитикаУчетаСерий
	|	КОНЕЦ) В (&МассивДоступныхПолитик)
	|СГРУППИРОВАТЬ ПО
...
Процедура ОбновитьСтатусыСерийНоменклатурыВТабличнойЧастиВводНачальныхОстатков(Объект, ИмяТабличнойЧасти = "Запасы", ДополнительныеСвойства = Неопределено, ИмяТабличнойЧастиСерии = "СерииНоменклатуры") Экспорт
	...
	ТипДокумента = ОбщегоНазначения.СтроковоеПредставлениеТипа(Тип("ДокументСсылка.ВводНачальныхОстатков"));
	ВидОперации = Неопределено;
...
Функция СтатусСерийНоменклатурыПриИзменении(СтруктураДанных) Экспорт
	...
	|			ТОГДА Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПолитикиУчетаСерий КАК ПолитикиУчетаСерий
	|		ПО ВЫБОР
	|			КОГДА Номенклатура.КатегорияНоменклатуры.ИспользоватьСерииИзКатегории
...
Процедура ЗаполнитьСерииНоменклатурыВНаличии(ДокОбъект, ИмяТЧЗапасы="Запасы", ИмяТЧСерииНоменклатуры="СерииНоменклатуры") Экспорт
	...
	|	РасходнаяНакладнаяЗапасы.Количество КАК КоличествоНеобходимо,
	|	СерииНоменклатурыОстатки.КоличествоОстаток КАК Остаток,
	|	ЛОЖЬ КАК СерийныеНомера
	|ПОМЕСТИТЬ ВТОстаткиКоличественныеСерии
	|ИЗ
	...
	|ГДЕ
	|	НЕ СерииНоменклатурыОстатки.Серия ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	спрСерииНоменклатуры.Ссылка КАК Серия,
	|	РасходнаяНакладнаяЗапасы.НомерСтроки КАК НомерСтроки,
	|	РасходнаяНакладнаяЗапасы.КлючСвязи КАК КлючСвязи,
	|	ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	|	ИСТИНА КАК СерийныеНомера,
	|	РасходнаяНакладнаяЗапасы.Количество КАК КоличествоНеобходимо,
	|	1 КАК Остаток
	|ПОМЕСТИТЬ ВТОстаткиСерийныеНомера
	|ИЗ
	...
	|	И КонтрольОстатковСерийНоменклатуры.Значение = ЛОЖЬ
	|	И РасходнаяНакладнаяЗапасы.Номенклатура.ИспользоватьУникальныеСерии
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТОстаткиСерийныеНомера.Серия КАК Серия,
	|	ВТОстаткиСерийныеНомера.НомерСтроки КАК НомерСтроки,
	|	ВТОстаткиСерийныеНомера.КлючСвязи КАК КлючСвязи,
	|	ВТОстаткиСерийныеНомера.Номенклатура КАК Номенклатура,
	|	ВТОстаткиСерийныеНомера.КоличествоНеобходимо КАК КоличествоНеобходимо,
	|	ВТОстаткиСерийныеНомера.СерийныеНомера КАК СерийныеНомера,
	|	ВТОстаткиСерийныеНомера.Остаток КАК Остаток
	|ПОМЕСТИТЬ ИтоговаяВыборка
	|ИЗ
	|	ВТОстаткиСерийныеНомера КАК ВТОстаткиСерийныеНомера
	|ГДЕ
	|	НЕ ВТОстаткиСерийныеНомера.Серия В
	|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|					ВТОстаткиКоличественныеСерии.Серия КАК Серия
	|				ИЗ
	|					ВТОстаткиКоличественныеСерии КАК ВТОстаткиКоличественныеСерии)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВТОстаткиКоличественныеСерии.Серия,
	|	ВТОстаткиКоличественныеСерии.НомерСтроки,
	|	ВТОстаткиКоличественныеСерии.КлючСвязи,
	|	ВТОстаткиКоличественныеСерии.Номенклатура,
	|	ВТОстаткиКоличественныеСерии.КоличествоНеобходимо,
	|	ВТОстаткиКоличественныеСерии.СерийныеНомера,
	|	ВТОстаткиКоличественныеСерии.Остаток
	|ИЗ
	|	ВТОстаткиКоличественныеСерии КАК ВТОстаткиКоличественныеСерии
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИтоговаяВыборка.Серия.ГоденДо КАК СерияГоденДо,
	|	ИтоговаяВыборка.Серия КАК Серия,
	|	ИтоговаяВыборка.НомерСтроки КАК НомерСтроки,
	|	ИтоговаяВыборка.КлючСвязи КАК КлючСвязи,
	|	ИтоговаяВыборка.Номенклатура КАК Номенклатура,
	|	ИтоговаяВыборка.КоличествоНеобходимо КАК КоличествоНеобходимо,
	|	ИтоговаяВыборка.СерийныеНомера КАК СерийныеНомера,
	|	ИтоговаяВыборка.Остаток КАК Остаток
	|ИЗ
	...
	Пока ВыборкаНомерСтроки.Следующий() Цикл
		...
		КоличествоКЗаполнению = ВыборкаНомерСтроки.КоличествоНеобходимо;
		
		ВыборкаСерииНоменклатуры = ВыборкаНомерСтроки.Выбрать();
		Пока ВыборкаСерииНоменклатуры.Следующий() Цикл
			
			Если Не ВыборкаСерииНоменклатуры.СерийныеНомера Тогда
				...
				Если КоличествоКЗаполнению <= ВыборкаСерииНоменклатуры.Остаток Тогда
					НовСтр.Количество = КоличествоКЗаполнению;
					Прервать
				КонецЕсли;
				
				НовСтр.Количество = ВыборкаСерииНоменклатуры.Остаток;
				КоличествоКЗаполнению = КоличествоКЗаполнению - ВыборкаСерииНоменклатуры.Остаток;
			...
			НовСтр.Количество = 1;
			КоличествоКЗаполнению = КоличествоКЗаполнению - 1;
...
Функция МассивКонтроляПоСериям(ТаблицаОбъекта, ВходящийОбъект, ИмяПоляНоменклатура, Знач СтруктурнаяЕдиница = Неопределено) 
	...
	|			И (&Организация = ПолитикиУчетаСерий.Организация)
	|			И (ВЫБОР
	|				КОГДА ВыборкаИзТабличнойЧастиЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|					ТОГДА ВыборкаИзТабличнойЧастиЗапасы.Номенклатура.КатегорияНоменклатуры = ПолитикиУчетаСерий.Владелец
	|				ИНАЧЕ ВыборкаИзТабличнойЧастиЗапасы.Номенклатура = ПолитикиУчетаСерий.Владелец
	|			КОНЕЦ)
	|ГДЕ
	|	ЕСТЬNULL(ПолитикиУчетаСерий.ПолитикаУчетаСерий, ВЫБОР
	|				КОГДА ВыборкаИзТабличнойЧастиЗапасы.Номенклатура.ИспользоватьСерииИзКатегории
	|					ТОГДА ВыборкаИзТабличнойЧастиЗапасы.Номенклатура.КатегорияНоменклатуры.ПолитикаУчетаСерий
	|				ИНАЧЕ ВыборкаИзТабличнойЧастиЗапасы.Номенклатура.ПолитикаУчетаСерий
	|			КОНЕЦ) В (&МассивДоступныхПолитик)";
