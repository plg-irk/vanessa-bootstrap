...
Функция ОперацияДокументаПодходитПодНастройкиВыгрузкиУзла(Узел, Объект)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда
		Если РаботаСЗаявкамиКлиентСервер.ЭтоЗаявка(Объект.ВидОперации) Тогда
			Возврат Ложь;
		ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
			Возврат Истина;
		Иначе
