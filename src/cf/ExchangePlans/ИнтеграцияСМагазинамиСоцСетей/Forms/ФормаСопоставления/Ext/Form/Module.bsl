...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ОбязательноЗаполнять Тогда
		...
		Для Каждого СтрРодитель Из ДеревоКатегорийТоваров.ПолучитьЭлементы() Цикл
			Для Каждого СтрЭлемент Из СтрРодитель.ПолучитьЭлементы() Цикл
				...
				Если СтрЭлемент.Используется И НЕ (ЗначениеЗаполнено(СтрЭлемент.КатегорияНоменклатуры)
				Или ЗначениеЗаполнено(СтрЭлемент.ГруппаНоменклатуры)) Тогда
		...
		Если МассивНаименований.Количество() Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю("Заполните используемые категории:" + Символы.ПС + СтрСоединить(МассивНаименований,", "),,,,Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Отказ И ОткрытьУзелОбменаПослеЗакрытия Тогда
		...
		ОткрытьФорму("ПланОбмена.ИнтеграцияСМагазинамиСоцСетей.Форма.ФормаУзлаВКонтакте", ПараметрыФормы, 
		ВладелецФормы,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДеревоКатегорийТоваров
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаНоменклатур
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаДополнительныхРеквизитов
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаЗначенийДополнительныхРеквизитов
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаВиртуальныеКаталоги
...
Процедура КнопкаПоказатьИдентификаторыВКонтактеПереключить(Команда)
	...
	ПоказатьИдентификаторыВКонтакте = Не ПоказатьИдентификаторыВКонтакте;
	Элементы.КнопкаПоказатьИдентификаторыВКонтактеПереключить.Пометка = ПоказатьИдентификаторыВКонтакте;
...
Процедура ЗагрузитьНоменклатуру(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗагрузитьНоменклатуруЗавершение", ЭтотОбъект);
	
	ОписаниеУзлаОбменаВК = Новый Структура;
	ОписаниеУзлаОбменаВК.Вставить("СсылкаНаУзелОбмена" , СсылкаНаУзелОбмена);
	
	ОткрытьФорму("ПланОбмена.ИнтеграцияСМагазинамиСоцСетей.Форма.ФормаЗагрузки", ОписаниеУзлаОбменаВК,
...
Процедура ЗаписатьФормуНаСервере()
	...
	НаборЗаписей = РегистрыСведений.СопоставлениеКаталоговДляМагазинаСоцСетей.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.УзелИнформационнойБазы.Установить(СсылкаНаУзелОбмена);
	
	ЗаписатьФормуНаСервереРекурсивно(ДеревоКатегорийТоваров, НаборЗаписей);
...
Процедура ЗаписатьВиртуальныеКаталогиНаСервере()
	...
	НаборЗаписей.Отбор.УзелИнформационнойБазы.Установить(СсылкаНаУзелОбмена);
	НаборЗаписей.Отбор.ТипИдентификатора.Установить("album");
...
Процедура ЭлементФормыИдентификаторыВКонтактеОбновить()
	Элементы.ДеревоКатегорийТоваровИдентификатор.Видимость = ПоказатьИдентификаторыВКонтакте;
	Элементы.ТаблицаНоменклатурИдентификаторТовара.Видимость = ПоказатьИдентификаторыВКонтакте;
	Элементы.ТаблицаНоменклатурИдентификаторКатегории.Видимость = ПоказатьИдентификаторыВКонтакте;
	Элементы.ТаблицаХарактеристикИдентификатор.Видимость = ПоказатьИдентификаторыВКонтакте;
КонецПроцедуры
