...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТолькоПросмотр = НЕ ПравоДоступа("Редактирование", Метаданные.Справочники.Номенклатура);
	...
	Если Параметры.Свойство("ПереданПараметрИспользоватьХарактеристики") И ИспользоватьХарактеристики
		<> Параметры.ИспользоватьХарактеристики Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр(
			"ru = 'После изменения признака использования характеристик следует сохранить карточку номенклатуры'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если НЕ Параметры.СпособРасчетаЦеныНабора = Перечисления.СпособыРасчетаЦеныНабора.ПустаяСсылка() Тогда
	...
	Цена = 0;
	Если НЕ Параметры.СоставНабора = Неопределено Тогда
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыНаборы
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Шрифт", ШрифтыСтиля.ШрифтВыделенныйСервисДоставки);
...
Процедура УстановитьВидимостьЭлементовДляМобильногоКлиента()
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.ЗаголовокХарактеристики.Шрифт = ШрифтыСтиля.УвеличенныйШрифтТекста;
		Элементы.ЗаголовокСостав.Шрифт = ШрифтыСтиля.УвеличенныйШрифтТекста;
	КонецЕсли;
...
Процедура СохранитьИзменения()
	...
	Иначе
		...
		Для каждого Стр Из СоставНабора Цикл
			...
			СтруктураСтроки.Вставить("ДоляСтоимости");
			СтруктураСтроки.Вставить("ЕдиницаИзмерения");
			СтруктураСтроки.Вставить("ФиксированнаяСтоимость");
			СтруктураСтроки.Вставить("ТипНоменклатурыЗапас");
			СтруктураСтроки.Вставить("ТипНоменклатурыУслуга");
...
//
// Параметры:
//  Направление	 - Число - [-1;1] Задает направление перемещения строк.
...
Процедура ИзменитьДоступностьЭлементовФормы(ПризнакДоступности = Истина)
	...
	Элементы.ЭтоНабор.Доступность = ?(Элементы.ЭтоНабор.Доступность = Истина, Не ТолькоПросмотр, Элементы.ЭтоНабор.Доступность);
...
Процедура ПроверкаИспользованияНаборов()
	...
	Иначе 
		Элементы.ЗаголовокСостав.Заголовок = НСтр("ru = 'Состав набора:'"); 
		АвтоЗаголовок = Ложь;
