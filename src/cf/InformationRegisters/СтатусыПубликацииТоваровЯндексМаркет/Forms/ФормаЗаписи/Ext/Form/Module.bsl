...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	Оповестить( "ПриЗакрытии", ИмяФормы, ЭтотОбъект );
КонецПроцедуры
...
Процедура ИнициироватьПодсказки()
	...
	НСтр("ru = 'Если не знаете SKU товара на Маркете, получите информацию о рекомендованных связях - проверьте информацию о товаре на закладке ""Данные товара"""".
		 |При необходимости дозаполните недостающую информацию и нажмите кнопку ""Получить рекомендацию"". 
		 |Все поля на закладке """"Данные товара"""" необязательны, но рекомендованы к заполнению.
		 |Подробное описание свойств товара увеличит вероятность подбора правильной рекомендации.
		 |Дождитесь результатного статуса, после смены статуса переходите к проверке и утверждению рекомендованной связи.'");
...
Процедура ОбновитьПредставлениеТовара()
	...
	КатегорииНоменклатурыЗначение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
	Запись.Номенклатура, "КатегорияНоменклатуры" );
	 
	Запись.ПредставлениеТовара = "" + Запись.Номенклатура + ХарактеристикаСтрока;
	Запись.КатегорияНоменклатуры = КатегорииНоменклатурыЗначение;
...
Процедура ОбновитьДопИнфо()
	...
	НоменклатураЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта( Запись.Номенклатура, 
	"Артикул, Производитель, СтранаПроисхождения" );
	
	Если НоменклатураЗначенияРеквизитов.Артикул <> "" Тогда 
		ШаблонСообщения = НСтр("ru = 'Артикул: %1'") + Символы.ПС;
		Артикул = СтрШаблон(ШаблонСообщения, НоменклатураЗначенияРеквизитов.Артикул);
	Иначе	
		Артикул  = НСтр("ru = 'Артикул: не указан'") + Символы.ПС;
	КонецЕсли;
	
	Если НоменклатураЗначенияРеквизитов.Производитель <> Справочники.Контрагенты.ПустаяСсылка() Тогда 
		ШаблонСообщения = НСтр("ru = 'Производитель: %1'") + Символы.ПС;
		Производитель = СтрШаблон(ШаблонСообщения, НоменклатураЗначенияРеквизитов.Производитель);					
	Иначе	
		Производитель  = НСтр("ru = 'Производитель: не указан'") + Символы.ПС;
	КонецЕсли;
	
	Если НоменклатураЗначенияРеквизитов.СтранаПроисхождения <> Справочники.СтраныМира.ПустаяСсылка() Тогда 		
		ШаблонСообщения = НСтр("ru = 'Страна происхождения: %1'") + Символы.ПС;
		Страна = СтрШаблон(ШаблонСообщения, НоменклатураЗначенияРеквизитов.СтранаПроисхождения);		
	Иначе	
