...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если (Запись.ИсходныйКлючЗаписи.Пустой() Или ИзмененыПараметрыТовара)
		...
		ДанныеЗаписи = Новый Структура("УчетнаяЗаписьМаркетплейса, Номенклатура, Характеристика");
...
Процедура ВыгружатьПоНастройкамУчетнойЗаписиПриИзменении(Элемент)
	...
	Запись.ВыгружатьПоНастройкамУчетнойЗаписи = ВыгружатьПоНастройкамУчетнойЗаписи;
...
Процедура ВыгружатьОписаниеТовараПриИзменении(Элемент)
	...
	Запись.ВыгружатьОписаниеТовара = ВыгружатьОписаниеТовара;
...
Процедура ВыгружатьЦеныПриИзменении(Элемент)
	...
	Запись.ВыгружатьЦены = ВыгружатьЦены;
...
Процедура ВыгружатьОстаткиПриИзменении(Элемент)
	...
	Запись.ВыгружатьОстатки = ВыгружатьОстатки;
...
Процедура ТаблицаФайловОбновить()
	...
	Элементы.ТаблицаФайловВладелец.Видимость = ЗначениеЗаполнено( Запись.Характеристика );
...
Процедура КнопкаФормаЗаписиСлужебнаяОткрыть(Команда)
	...
	СтруктураЗаписи = Новый Структура( "УчетнаяЗаписьМаркетплейса, ВидОбъектаМаркетплейса, Номенклатура" );
	СтруктураЗаписи.Вставить( "Характеристика" );
	СтруктураЗаписи.Вставить( "Упаковка" );
	СтруктураЗаписи.Вставить( "ОбъектПубликации" );
...
Процедура ПроверитьНаличиеОшибок(Команда)
	...
	ПроверитьЗаполнениеКарточкиТовара();
...
Процедура ЭлементыФормыЗаполнитьИзДанныхЗаписи()
	...
	ВыгружатьПоНастройкамУчетнойЗаписи = Запись.ВыгружатьПоНастройкамУчетнойЗаписи;
	ВыгружатьОписаниеТовара = Запись.ВыгружатьОписаниеТовара;
	ВыгружатьЦены = Запись.ВыгружатьЦены;
	ВыгружатьОстатки = Запись.ВыгружатьОстатки;
...
Функция ТаблицаФайловСтрокаДозаполнить( ТаблицаФайловСтрока )
	...
	Если ПустаяСтрока( ТаблицаФайловТекущаяСтрока.КартинкаАдрес ) Тогда
		ТаблицаФайловТекущаяСтрока.КартинкаАдрес =
		ИнтеграцияСМаркетплейсамиСервер.ПрисоединенныйФайлВоВременноеХранилище( ТаблицаФайловТекущаяСтрока.Файл, 
...
Процедура ТаблицаФайловОбновитьНаСервере()
	...
	ТаблицаТоваровВладелецФайлов = ?(ЗначениеЗаполнено(Запись.Характеристика), Запись.Характеристика, Запись.Номенклатура );
	...
	Изображения = ИнтеграцияСМаркетплейсомOzonСервер.НоменклатураИзображенияПолучить( ТаблицаТоваровВладелецФайлов, Истина, ИзображенияПараметры );
...
Функция ПроверитьЗаполнениеКарточкиТовара()
	...
	ПараметрыФормы.Вставить( "ИсточникКатегорииМаркетплейса" );
	ПараметрыФормы.Вставить( "Валюта" );
	ПараметрыФормы.Вставить( "Цены" );
	ПараметрыФормы.Вставить( "СтавкаНДС" );
	ПараметрыФормы.Вставить( "Штрихкод" );
	ПараметрыФормы.Вставить( "ИзмененыПараметрыТовара");
...
Процедура УстановитьДоступностьИЗаполнениеЭлементовТоварнойПозиции()
	...
	Элементы.Номенклатура.ТолькоПросмотр = ТоварСозданВOzon;
	Элементы.ИмпортироватьДанныеТовараСТорговойПлощадки.Доступность = ТоварСозданВOzon;
	...
	Если ЗначениеЗаполнено( Запись.Номенклатура ) Тогда
		...
		Элементы.Характеристика.Доступность = НоменклатураРеквизитыЗначения.ИспользоватьХарактеристики;
...
Функция ЗаполнитьДанныеТовараНаСервере(ПараметрыДействий)
	...
	Если ЗначениеЗаполнено(Запись.Номенклатура)
		...
		ПараметрыФормы.ИсточникКатегорииМаркетплейса = Запись.ИсточникКатегорииМаркетплейса;
