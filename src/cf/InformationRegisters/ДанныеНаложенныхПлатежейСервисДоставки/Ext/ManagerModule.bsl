...
Процедура УдалитьЗаписиПоОтбору(ОтборДляУдаления) Экспорт
	...
	Если ОтборДляУдаления.УдалитьОбработанные = Истина Тогда
		МассивЗамен.Добавить("ДанныеДляОбработкиПоНаложеннымПлатежам.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОбработкиПроцедурСинхронизацииСервисДоставки.Удалить)");
	КонецЕсли;
