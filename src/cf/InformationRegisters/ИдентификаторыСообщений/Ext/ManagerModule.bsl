...
Функция ДанныеЗапросом(УчетнаяЗапись, НачинаяСДаты) Экспорт
	...
	"ВЫБРАТЬ
	|	ИдентификаторыСообщений.ИдентификаторСообщения КАК ИдентификаторСообщения,
	|	ИдентификаторыСообщений.Событие КАК Событие,
	|	ИдентификаторыСообщений.Ключ КАК Ключ
	|ИЗ
	|	РегистрСведений.ИдентификаторыСообщений КАК ИдентификаторыСообщений
	|ГДЕ
	|	ИдентификаторыСообщений.УчетнаяЗапись = &УчетнаяЗапись
	|	И ИдентификаторыСообщений.Событие.НачалоСобытия >= &НачинаяСДаты");
	Запрос.УстановитьПараметр("УчетнаяЗапись", УчетнаяЗапись);
	Если ЗначениеЗаполнено(НачинаяСДаты) Тогда
		Запрос.УстановитьПараметр("НачинаяСДаты", НачинаяСДаты);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ИдентификаторыСообщений.Событие.НачалоСобытия >= &НачинаяСДаты", "");
	КонецЕсли;
