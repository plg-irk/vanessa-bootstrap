...
// Возвращает значения ресурса регистра системы налогообложения за заданный период
//
// Параметры:
...
//  - Массив - значения запрошенного ресурса
//
Функция ЗначенияРесурсаУчетнойПолитикиЗаПериод(ИмяРесурса, Организация, НачалоПериода, ОкончаниеПериода) Экспорт
	...
	Выбор = ОсновнойЗапрос.Операторы[0];
	ТаблицаСрезПоследних = Выбор.Источники.Добавить("РегистрСведений.СистемыНалогообложенияОрганизаций.СрезПоследних",
		"СистемыНалогообложенияОрганизаций");
	ТаблицаСрезПоследних.Источник.Параметры[0].Выражение = Новый ВыражениеСхемыЗапроса("&НачалоПериода");
	ТаблицаСрезПоследних.Источник.Параметры[1].Выражение = Новый ВыражениеСхемыЗапроса("Организация = &Организация");
	Выбор.ВыбираемыеПоля.Добавить(ИмяРесурса);
	Если СледующийМесяц < КонецМесяца(ОкончаниеПериода) Тогда
		ВыборПериод = ОсновнойЗапрос.Операторы.Добавить();
		ВыборПериод.ТипОбъединения = ТипОбъединенияСхемыЗапроса.ОбъединитьВсе;
		ВыборПериод.Источники.Добавить("РегистрСведений.СистемыНалогообложенияОрганизаций", "СистемыНалогообложенияОрганизаций");
		ВыборПериод.ВыбираемыеПоля.Добавить(ИмяРесурса);
		ВыборПериод.Отбор.Добавить("СистемыНалогообложенияОрганизаций.Организация = &Организация");
		ВыборПериод.Отбор.ДОбавить("СистемыНалогообложенияОрганизаций.Период МЕЖДУ &СледующийМесяц И &ОкончаниеПериода");
	КонецЕсли;
	ОсновнойЗапрос.Колонки[0].Псевдоним = "Значение";
