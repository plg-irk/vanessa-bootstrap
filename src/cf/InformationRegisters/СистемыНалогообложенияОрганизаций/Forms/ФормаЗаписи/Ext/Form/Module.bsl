...
Процедура ПериодПриИзменении(Элемент)
	Если Запись.ПлательщикНДСПрименяющийУСН
		И НЕ РегламентированнаяОтчетностьУСНКлиентСервер.Применяется176ФЗ(Запись.Период) Тогда
		
		Запись.ПлательщикНДСПрименяющийУСН = Ложь;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.Организация, "ЮридическоеФизическоеЛицо")
	...
	Элементы.ПлательщикНДСПрименяющийУСН.Видимость = Запись.ПлательщикУСН
		И РегламентированнаяОтчетностьУСНКлиентСервер.Применяется176ФЗ(Запись.Период);
