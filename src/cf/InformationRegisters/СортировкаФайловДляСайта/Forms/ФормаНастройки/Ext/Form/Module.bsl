...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТаблицаФайловВладелец.Видимость = ЗначениеЗаполнено( ХарактеристикаНоменклатуры );
	Элементы.ХарактеристикаНоменклатуры.Видимость = ЗначениеЗаполнено( ХарактеристикаНоменклатуры );
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч( ВладелецФормы ) = Тип( "ФормаКлиентскогоПриложения" ) Тогда
		ВладелецФормаИмя = ВладелецФормы.ИмяФормы;
		Если СтрНайти( ВладелецФормаИмя, "Справочник.Номенклатура" ) > 0 Тогда
	...
	Если ИмяСобытия = "Запись_Файл" Тогда
		Если ТипЗнч( Параметр ) = Тип( "Структура" ) Тогда
			Если Параметр.Свойство( "Файл" ) И Параметр.Свойство( "ЭтоНовый" ) И Параметр.ЭтоНовый = Истина Тогда
				Если Параметр.Свойство( "ВладелецФайла" ) И Параметр.ВладелецФайла = ВладелецФайлов Тогда
                    ...
                    Элементы.ТаблицаФайлов.Обновить();
...
Процедура ТаблицаФайловДобавитьФайлСДиска(Команда)
	РаботаСФайламиСлужебныйКлиент.ДобавитьФайлИзФайловойСистемы(ВладелецФайлов, ЭтотОбъект);
КонецПроцедуры
...
Функция ЗаписатьИзмененияНаСервере()
	...
	Модифицированность = Не РезультатЗаписи;
...
Функция ОповеститьФормыОЗаписи()
	...
	Оповестить( "РегистрСведений.СортировкаФайловДляСайта.Запись", ПараметрыЗаписи, ИмяФормы );
...
Процедура ОбновитьТаблицуФайлов()
	...
	Для Каждого Стр Из ТаблицаФайлов Цикл
		Стр.Картинка = ИнтеграцияСМаркетплейсамиСервер.ПрисоединенныйФайлВоВременноеХранилище(Стр.Файл, УникальныйИдентификатор);
	КонецЦикла;
...
Процедура ТаблицаФайловДобавитьСтроку(Параметр)
	...
	ТаблицаФайловСтрока.Картинка = ИнтеграцияСМаркетплейсамиВызовСервера.ПрисоединенныйФайлВоВременноеХранилище( ТаблицаФайловСтрока.Файл, УникальныйИдентификатор );
