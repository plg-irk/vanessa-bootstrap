...
// Возвращает фамилию и инициалы переданного ФИО. Если подключена Интернет-поддержка, перед сокращением
// производится поиск возможных перепутанных частей ФИО при помощи подсистемы ПодборФИО.
//
// Параметры:
//  ФИО	 - Строка - Полное ФИО в формате "Фамилия Имя Отчество"
// 
// Возвращаемое значение:
//  Строка
//
Функция ФамилияИнициалыФИО(ФИО) Экспорт
	...
	Если ИнтернетПоддержкаПодключена Тогда
		ЗаполнитьПолноеИмяПоКлассификатору(ПолноеИмя);
	КонецЕсли;
	
	Возврат ФизическиеЛицаКлиентСервер.ФамилияИнициалы(ТРег(ПолноеИмя));
...
//
// Параметры:
//  Владелец - ЛюбаяСсылка - ссылка на элемент-владелец
//  Наименование - Строка - наименование элемента
//
// Возвращаемое значение:
//  - СправочникСсылка.КонтактныеЛица - ссылка на элемент справочника
//
...
//  * КакСвязаться - Строка - как связаться
...
Процедура ДобавитьКИ(ЭлементСправочника, ПредставлениеКИ, ВидКИ)
	...
	Иначе
		ТипКИ = Перечисления.ТипыКонтактнойИнформации.ПустаяСсылка();
	КонецЕсли;
...
Процедура ДополнитьДанныеВыбораСвязямиСКонтрагентами(ДанныеВыбора, Параметры)
	...
	Контакты = КонтактыИзДанныхВыбора(ДанныеВыбора);
	СоответствиеКонтрагентовИКонтактов = СоответствиеКонтрагентовИКонтактов(Контакты);
...
Функция КонтактыИзДанныхВыбора(ДанныеВыбора)
	
	Значения = ДанныеВыбора.ВыгрузитьЗначения();
	Для Индекс = 0 По Значения.ВГраница() Цикл
		
		Элемент = Значения[Индекс];
		
		Если ТипЗнч(Элемент) = Тип("Структура") Тогда
			Значения[Индекс] = Элемент.Значение;
		КонецЕсли;
...
Процедура ЗаполнитьПолноеИмяПоКлассификатору(ПолноеИмя)
	
	ЧастиИмени = СтрРазделить(ПолноеИмя, " ");
	ПодобранныеФИО = ПодборФИО.НайтиФИО(ЧастиИмени, Ложь);

	Если ПодобранныеФИО.Количество() Тогда
		
		ЧастиИмени = ФизическиеЛицаКлиентСервер.ЧастиИмени(ПолноеИмя);
		
		ПредполагаемыеФамилии  = ПодобранныеФИО.Фамилии;
		ПредполагаемыеИмена    = ПодобранныеФИО.Имена;
		ПредполагаемыеОтчества = ПодобранныеФИО.Отчества;
		
		ФамилияНайдена = ПредполагаемыеФамилии.Количество() > 0;
		Фамилия = ?(ФамилияНайдена, ПредполагаемыеФамилии[0].Значение, ЧастиИмени.Фамилия);

		ИмяНайдено = ПредполагаемыеИмена.Количество() > 0;
		Имя = ?(ИмяНайдено, ПредполагаемыеИмена[0].Значение, ЧастиИмени.Имя);
		
		ОтчествоНайдено = ПредполагаемыеОтчества.Количество() > 0;
		Отчество = ?(ОтчествоНайдено, ПредполагаемыеОтчества[0].Значение, ЧастиИмени.Отчество);
		
		ПолноеИмя = СтрШаблон("%1 %2 %3", Фамилия, Имя, Отчество);
