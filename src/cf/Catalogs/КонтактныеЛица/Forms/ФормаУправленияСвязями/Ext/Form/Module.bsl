...
Процедура ОбновитьЭлементыКонтрагентов()
	...
	Для Каждого Контрагент Из ДанныеКонтрагентов Цикл
		...
		Если ИндексКонтрагента > 0 Тогда
			...
			КнопкаУстановитьСвязь.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанелиИВДополнительномПодменю;
...
Процедура ЗаписатьДанныеПоСвязямВРегистр()
	...
	Для Каждого Контрагент Из НедействительныеСвязиСКонтрагентами Цикл
		...
		ДанныеКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент.Контрагент, "КонтактноеЛицо, КонтактноеЛицоПодписант");
		Если ИзменяемыйКонтрагент.Количество() = 0 Тогда
			...
			Если ДанныеКонтрагента.КонтактноеЛицо = Контрагент.Контакт И Контрагент.ЭтоОсновноеКЛ Тогда
				НоваяСтрока.ИзменитьОсновной = Контрагент.ЭтоОсновноеКЛ;
			КонецЕсли;
			Если ДанныеКонтрагента.КонтактноеЛицоПодписант = Контрагент.Контакт И Контрагент.ЭтоПодписант Тогда
				НоваяСтрока.ИзменитьПодписанта = Контрагент.ЭтоПодписант;
			КонецЕсли;
		Иначе
			НоваяСтрока = ИзменяемыйКонтрагент[0];
			Если НЕ НоваяСтрока.ИзменитьОсновной И ДанныеКонтрагента.КонтактноеЛицо = Контрагент.Контакт Тогда
				НоваяСтрока.ИзменитьОсновной = Контрагент.ЭтоОсновноеКЛ;
			КонецЕсли;
			Если НЕ НоваяСтрока.ИзменитьПодписанта И ДанныеКонтрагента.КонтактноеЛицоПодписант = Контрагент.Контакт Тогда
				НоваяСтрока.ИзменитьПодписанта = Контрагент.ЭтоПодписант;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьКонтактНаОсновнойИПодписанта(ДанныеКонтрагента,Оповещение)
	...
	Если ЗначениеЗаполнено(КонтактОсновноеКЛ) И ЗначениеЗаполнено(КонтактПодписант) Тогда
		Если ВыделенныеКонтакты.Количество() > 1 Тогда
			ТекстВопроса = СтрШаблон(НСтр("ru = 'Среди выделенных контактов есть контакты, которые используются
			|как основной/подписант для контрагента %1
	...
	ИначеЕсли ЗначениеЗаполнено(КонтактОсновноеКЛ) Тогда
		ТекстВопроса = СтрШаблон(НСтр("ru = 'Контакт %1 используется как основной для контрагента %2
		|При записи основной контакт контрагента станет незаполненным'"),КонтактОсновноеКЛ,ДанныеКонтрагента.Контрагент);
	Иначе
		ТекстВопроса = СтрШаблон(НСтр("ru = 'Контакт %1 используется как подписант для контрагента %2
