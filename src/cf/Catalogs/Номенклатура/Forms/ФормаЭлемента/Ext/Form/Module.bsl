...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Параметры.ПодконтрольнаяПродукцияВЕТИС Тогда
	...
	ЕстьУСНсНДС = ПолучитьФункциональнуюОпцию("ПлательщикНДСПрименяющийУСН");								
	Справочники.СтавкиНДС.ЗаполнитьСписокВыбораСтавокНДС(ЭтотОбъект, Элементы.ВидСтавкиНДС.Имя, ЕстьУСНсНДС = Истина);
	ЧастиЗаголовка = Новый Массив;
	ЧастиЗаголовка.Добавить(КатегорииНоменклатурыСервер.ТекстПодсказкиСтавкаНДС(ЕстьУСНсНДС));
	ЧастиЗаголовка.Добавить(Символы.ПС);
	ЧастиЗаголовка.Добавить(Символы.ПС);
	ЧастиЗаголовка.Добавить(Элементы.ВидСтавкиНДС.РасширеннаяПодсказка.Заголовок);
	Если ЕстьУСНсНДС Тогда
		Элементы.ВидСтавкиНДС.МаксимальнаяШирина = Элементы.ПризнакПредметаРасчета.МаксимальнаяШирина;
	КонецЕсли;
	Элементы.ВидСтавкиНДС.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(ЧастиЗаголовка);
	...
	Если Параметры.Свойство("РеквизитПозиционирования") Тогда
		...
		Если РеквизитПозиционирования.ДопРеквизит = Ложь Тогда
			...
			Если Не НайденоеСвойствоФормы = Неопределено И НайденоеСвойствоФормы.Видимость Тогда 
				ТекущийЭлемент = НайденоеСвойствоФормы
			КонецЕсли;
		Иначе
			ПараметрыПоиска = Новый Структура("Наименование", РеквизитПозиционирования.ИмяРеквизитаОбъекта);
			НайденныйРеквизит = ЭтотОбъект.Свойства_ОписаниеДополнительныхРеквизитов.НайтиСтроки(ПараметрыПоиска);
	...
	Если ЗначениеЗаполнено(Параметры.ШтрихКодДляУстановкиПослеЗаписи) Тогда
	...
	ЕстьКлассификацияНоменклатурПоОперациям0 = УчетНДС.ЕстьКлассификацияНоменклатурПоОперациям0();
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Операции0", "Видимость", ЕстьКлассификацияНоменклатурПоОперациям0);
	
	Если ЕстьКлассификацияНоменклатурПоОперациям0 Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеЗначенияИспользоватьСерии"
		Или ИмяСобытия = "ИзменениеЗначенияПолитикаУчетаСерий" Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.Ссылка.Пустая() Тогда
		СтатистикаПоПоказателямКлиентСервер.ДобавитьСобытие("Данные.Номенклатура.ЗаписьНового");
	Иначе
		СтатистикаПоПоказателямКлиентСервер.ДобавитьСобытие("Данные.Номенклатура.Запись");
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ШтрихКодДляУстановкиПослеЗаписи) Тогда
		...
		Если СтрНайти(ШтрихКодДляУстановкиПослеЗаписи, ",") > 0 Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Отказ
		...
		ОткрытьФорму("Справочник.Номенклатура.Форма.ФормаНастройкиКопированияНоменклатуры", ПараметрыФормы, 
...
Процедура КатегорияНоменклатурыПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияВопросаПерезаполнитьПоКатегории", ЭтотОбъект);
...
Процедура ПродолжитьЗаполнениеПоКатегории(ОбновитьРеквизиты)
	...
	Если Не КатегорияДоИзменения = Объект.КатегорияНоменклатуры Тогда
		Если Объект.ИспользоватьСерииНоменклатуры И ЗначениеЗаполнено(Объект.Ссылка) Тогда
			
			Если Объект.ИспользоватьСерииНоменклатуры И Не Объект.ИспользоватьСерииИзКатегории Тогда
				НастройкаИспользованияСерий = "Индивидуальные";
			Иначе
				НастройкаИспользованияСерий = "ИзКатегории";
			КонецЕсли;
	
			СтруктураВозврата = СерииНоменклатурыУНФВызовСервера.ИзменениеИспользованияСерийПоНоменклатуреВозможно(Объект.Ссылка, НастройкаИспользованияСерий, Объект.КатегорияНоменклатуры);
			
			Если СтруктураВозврата.ОтказПоНастройкамСерий Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'В номенклатуре установлена настройка учета серий - Индивидуальные.
				|Запрещено изменение на категорию с настройками серий - Общие для категории.'"));
				Объект.КатегорияНоменклатуры = КатегорияДоИзменения;
				Возврат;
			КонецЕсли;
			
			Если Объект.ИспользоватьСерииИзКатегории И СтруктураВозврата.ОтказПоДвижениямПоСериям Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Для номенклатуры используются общие серии из категории по которым есть движения. Изменение категории невозможно.'"));
				Объект.КатегорияНоменклатуры = КатегорияДоИзменения;
				Возврат;
			КонецЕсли;
...
Процедура ЕдиницаДляОтчетовПриИзмененииСервер()
	...
	Иначе
		Объект.КоэффициентЕдиницыДляОтчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЕдиницаДляОтчетов, "Коэффициент");
	КонецЕсли;
...
Функция ДанныеВыбораТНВЭД(ПараметрыПоиска)
	...
	Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыПоиска.СтрокаПоиска) Тогда
...
Процедура ВидСтавкиНДСРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "http") Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура НадписьСерииНеИспользуютсяОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОткрытьНастройкиПрограммыСерии();
КонецПроцедуры
...
&НаКлиенте
Процедура НадписьСерииНеВведеныОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстСообщения = НСтр("ru = 'Для включения использования серий номенклатуры требуетсяя сохранить элемент.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
	Иначе
		ПараметрыФормы = Новый Структура("Отбор, ТолькоПросмотрФормы", Новый Структура("Владелец", Объект.Ссылка), ТолькоПросмотр);
		ОткрытьФорму("Справочник.СерииНоменклатуры.Форма.ФормаСписка", ПараметрыФормы, ЭтотОбъект,, Окно);
	КонецЕсли;
...
&НаКлиенте
Процедура ПодарочныйСертификатПриниматьКОплатеТолькоОрганизациейЭмитентомПриИзменении(Элемент)
	
	Если Не МожноИзменитьПараметрСертификата("ПодарочныйСертификатПринимаетКОплатеТолькоОрганизациеяЭмитент") Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ГруппаХарактеристикаЦенРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ВыделитьКолонкуПорядок", Истина);
	
	ОткрытьФорму("Справочник.ВидыЦен.ФормаСписка", ПараметрыОткрытия);
...
Процедура КнопкаСортировкаФайловНаСайте(Команда)
	...
	ОткрытьФорму("РегистрСведений.СортировкаФайловДляСайта.Форма.ФормаНастройки" , ПараметрыОткрытияФормы , ЭтотОбъект);
КонецПроцедуры
...
Процедура ИсторияИзменений(Команда)
	...
	ОткрытьФорму("РегистрСведений.ИсторияИзмененийЕдиницИзмерения.ФормаСписка", ПараметрыОтбора, ЭтотОбъект,,,,, 
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура КомандыИзображения(Команда)
	...
	Если Не ЭтоОсновноеИзображение И ЕстьИзображения Тогда
		СписокДоступныхКоманд.Добавить("УстановитьИзображениеОсновным", НСтр("ru='Сделать основным        '"),,
			БиблиотекаКартинок.ИзображениеОсновное);
	КонецЕсли;
	СписокДоступныхКоманд.Добавить("ДобавитьИзображение",    НСтр("ru='Добавить           '"),, 
	...
	Если ОтображатьКнопкуСортировкиНаФорме Тогда
		СписокДоступныхКоманд.Добавить("КнопкаСортировкаФайловНаСайте", 
			НСтр("ru='Настроить состав и сортировку для сайта        '"),, 
			БиблиотекаКартинок.ДоступВИнтернет32);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЕстьДвиженияПодарочынхСертификатов(Номенклатура)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Номенклатура", Номенклатура);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	|	Движения.ПодарочныйСертификат КАК Номенклатура
	|ИЗ
	|	РегистрНакопления.ПодарочныеСертификаты КАК Движения
	|ГДЕ
	|	Движения.ПодарочныйСертификат = &Номенклатура
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	Движения.ПодарочныйСертификат КАК Номенклатура
	|ИЗ
	|	РегистрСведений.ПодарочныеСертификатыКСписаниюНачислениюСерверЛояльности КАК Движения
	|ГДЕ
	|	Движения.ПодарочныйСертификат = &Номенклатура";
	Возврат НЕ Запрос.Выполнить().Пустой();
...
Функция МожноИзменитьПараметрСертификата(ИмяРеквизита = "")
	...
	Если ЕстьДвиженияЗапасов(Объект.Ссылка) 
		ИЛИ ЕстьДвиженияПодарочынхСертификатов(Объект.Ссылка) Тогда
...
Процедура УправлениеФормой(ОткрытиеФормы = Ложь)
	...
	// Изображения
	Элементы.ПереключениеИзображений.Видимость = ЭтоЗапас() ИЛИ ЭтоУслуга() ИЛИ ЭтоРабота() ИЛИ ЭтоСертификат();
	...
	Если ФОИспользуютсяСертификаты Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаХарактеристикаЦен",		"Видимость", (Не ЭтоСертификат()) И РазрешенПросмотрЦен);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДинамикаЦен",					"Видимость", (Не ЭтоСертификат()) И РазрешенПросмотрЦен);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПодарочныйСертификатПриниматьКОплатеТолькоОрганизациейЭмитентом",
			"Доступность", Объект.ИспользоватьСерииНоменклатуры И  НЕ УчетПоКомпанииВЦелом);
...
// Параметры:
// 	ВыполнитьВФоне - Булево - признак выполнения в фоне
...
Процедура ДиаграммаЦеныКоманднаяПанельСоздатьКнопкиВыбораЕдиницыИзмерения()
	...
	Если ВыборкаЕдиницыИзмерения.Количество() <> 0 Тогда
		...
		КнопкаФормы.Заголовок = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЕдиницаИзмерения, "Наименование");
...
&НаКлиенте
Процедура ВопросСоздатьНоменклатуруСНовымШтрихКодом(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Нет Тогда
		Возврат
	КонецЕсли;

	ШтрихКодДляУстановкиПослеЗаписи = ДополнительныеПараметры.ШтрихКод;
	Модифицированность = Истина;
...
Процедура НастроитьВидимостьКоэффициентаПересчетаПоАкцизам()
	...
	Если ЗначениеЗаполнено(Объект.КодВидаПодакцизногоТовара) Тогда
		Если Объект.ВидПодакцизногоТовара = ПредопределенноеЗначение("Перечисление.ВидыПодакцизныхТоваров.АвтомобилиИМотоциклы") Тогда
			...
			Элементы.КоэффициентПересчета.Заголовок = НСтр("ru = 'Количество'") + Символы.НПП + Объект.ЕдиницаИзмеренияДляРасчетаАкциза;
		...
		Иначе
			...
			Если (ЗначениеЗаполнено(Объект.КоэффициентПересчета)
				...
				Элементы.ГруппаКоэффициентаАкциза.Видимость = Истина;
				Элементы.КоэффициентПересчета.Заголовок = НСтр("ru = 'В 1'") + Символы.НПП + Объект.ЕдиницаИзмерения + ".";
...
Процедура ПолученШтрихкод(ДанныеШтрихкода)
	...
	ДополнительныеДанныеПоШтрихкоду = ШтрихкодированиеУНФКлиент.ДополнительныеДанныеПоШтрихкоду(
		ДанныеШтрихкода.Штрихкод, Неопределено);
		...
		Если ТипЗнч(ГруппаВторогоУровня) = Тип("ГруппаФормы") Тогда
			Для Каждого ПодчиненныйЭлемент Из ГруппаВторогоУровня.ПодчиненныеЭлементы Цикл
				...
				Если ТипЗнч(ПодчиненныйЭлемент) = Тип("ПолеФормы") 
					И ТипЗнч(ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(ЭтотОбъект, ПодчиненныйЭлемент.ПутьКДанным)) = Тип("Строка") Тогда
...
Процедура ОткрытьКатегориюИВыполнитьПозиционирование(ИмяЭлемента)
	...
	Если Не ЗначениеЗаполнено(Объект.КатегорияНоменклатуры) Тогда
		ТекущийЭлемент = Элементы.КатегорияНоменклатуры;
		Возврат;
	КонецЕсли;
	...
	ОткрытьФорму("Справочник.КатегорииНоменклатуры.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
...
Процедура УстановитьНадписьНастроекСерийПриРаботеССертификатами()
	...
	НастройкиВыполнены = Объект.ИспользоватьСерииНоменклатуры 
		И ЗначениеЗаполнено(Объект.ПолитикаУчетаСерий)
		И ФОИспользоватьСерииНоменклатуры;	
...
Процедура ЗаполнитьОбъектПродукцияВЕТИС()
	...
	Если НЕ Параметры.ПодконтрольнаяПродукцияВЕТИС Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьНастройкиФормы()
	...
	ПоказыватьГрафикиПродажИЦен = НЕ ПерсональныеНастройкиРУНФ.Прочитать("НеВыводитьОтчетыПоПродажамИЦенамВКарточкеНоменклатуры");
	ИспользоватьБыструюФормуНоменклатурыВДокументах = ПерсональныеНастройкиРУНФ.Прочитать("ИспользоватьБыструюФормуНоменклатурыВДокументах");
	Элементы.ФормаПоказыватьГрафикиПродажИЦен.Пометка = ПоказыватьГрафикиПродажИЦен;
	Элементы.ДиаграммаПродажиКонтекстноеМенюПоказыватьГрафикиПродажИЦен.Пометка = ПоказыватьГрафикиПродажИЦен;
	Элементы.ДиаграммаЦеныКонтекстноеМенюПоказыватьГрафикиПродажИЦен.Пометка = ПоказыватьГрафикиПродажИЦен;
...
Функция ПараметрыСозданияДопРеквизита()
	...
	Если ЗначениеЗаполнено(Объект.КатегорияНоменклатуры) Тогда	
		ТекущийНаборСвойств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.КатегорияНоменклатуры, "НаборСвойств");
	Иначе
...
Процедура ЗаполнитьЗначенияПоУмолчаниюСервер(НовыйЭлемент = Ложь)
	...
	Если Элементы.СпособПополнения.Видимость И Не ЗначениеЗаполнено(Объект.СпособПополнения) Тогда
		...
		КатегорияСпособПополнения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.КатегорияНоменклатуры, "СпособПополнения");
		
		Если ЗначениеЗаполнено(КатегорияСпособПополнения) Тогда
			Объект.СпособПополнения = КатегорияСпособПополнения;
		Иначе
	...
	Если ЗначениеЗаполнено(Объект.КатегорияНоменклатуры) Тогда
		ПараметрыКатегории = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.КатегорияНоменклатуры, 
			"ПризнакПредметаРасчета, ТипНоменклатурыПоУмолчанию");
