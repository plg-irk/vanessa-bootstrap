...
// Параметры:
//   Ограничение - см. УправлениеДоступомПереопределяемый.ПриЗаполненииВидовДоступа
...
Функция КоличествоСтруктурныхЕдиниц(ТипСтруктурнойЕдиницы, ВключаяОсновное = Истина) Экспорт
	...
	Если Не ВключаяОсновное Тогда
		...
		Запрос.УстановитьПараметр("ОсновноеЗначение", ?(ТипСтруктурнойЕдиницы
			= Перечисления.ТипыСтруктурныхЕдиниц.Подразделение, ОсновноеПодразделение, ОсновнойСклад));
...
Функция ОсновнойСклад() Экспорт
	...
	Результат = ?(ЗначениеЗаполнено(ЗначениеНастройки), ЗначениеНастройки, ОсновнойСклад);
	Возврат Результат;
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	НаименованиеРодителя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Данные.Родитель, "Наименование", Истина);
	#Иначе
	НаименованиеРодителя = "";
	#КонецЕсли
	
	Представление = СтрШаблон("%1 (%2)", Данные.Наименование, НаименованиеРодителя);
