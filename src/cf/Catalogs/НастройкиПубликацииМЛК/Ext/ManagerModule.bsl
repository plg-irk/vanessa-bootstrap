...
Функция СинхронизироватьУзелОбмена(УзелОбмена, РазрешитьДлительнуюОперацию) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ТекущиеНастройкиКабинетКлиента = НастройкиИнтеграцииКабинетКлиента(ПолучитьНастройкиПубликацииМЛК());
	УстановитьПривилегированныйРежим(Ложь);
...
Функция СоздатьСлужебногоПользователяИнтеграцииРежимСервиса(НомерОбласти)
	...
	ПрограммныйИнтерфейсСервиса.СоздатьПользователяАбонента(ПараметрыСоздания);
...
Процедура АктивироватьПромокодЕслиНеобходимоРежимСервиса()
	...
	ПрефиксПромокодаАктивации = ПромокодАктивацииПриложенияКабинетКлиента();
	ПромокодКабинетКлиента = ПрефиксПромокодаАктивации;
	...
	Если КоличествоАктивированныхПромокодов > 0 Тогда
		ПромокодКабинетКлиента = ПрефиксПромокодаАктивации + "_" + Строка(КоличествоАктивированныхПромокодов);
	КонецЕсли;
...
Процедура ЗарегистрироватьЦеныНоменклатуры(УзелОбмена, НастройкаПубликацииМЛК, Номенклатура, ВидыЦен)
	...
	|				И ДокументУстановки ССЫЛКА Документ.УстановкаЦенНоменклатуры
	|				И ДокументУстановки <> ЗНАЧЕНИЕ(Документ.УстановкаЦенНоменклатуры.ПустаяСсылка)) КАК ЦеныНоменклатурыСрезПоследних";
	...
	|				И (&ИспользоватьХарактеристики
	|					ИЛИ Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))
	|				И (ДокументУстановки = ЗНАЧЕНИЕ(Документ.УстановкаЦенНоменклатуры.ПустаяСсылка)
	|					ИЛИ НЕ ДокументУстановки ССЫЛКА Документ.УстановкаЦенНоменклатуры)) КАК ЦеныНоменклатурыСрезПоследних";
