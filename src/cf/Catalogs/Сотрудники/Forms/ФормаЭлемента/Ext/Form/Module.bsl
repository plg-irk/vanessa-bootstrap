...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Физлицо) Тогда
		ФизлицоБанковскийСчетПоУмолчанию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Физлицо,
			"БанковскийСчетПоУмолчанию");
		ПрочитатьБанковскийСчетПоУмолчанию(ФизлицоБанковскийСчетПоУмолчанию);
	КонецЕсли;
	...
	ФизическоеЛицоИНН = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ФизическоеЛицо, "ИНН", Истина);
	СформироватьПредставлениеПроверкиИНННаСервере(ФизическоеЛицоИНН);
	
	Если НЕ Пользователи.РолиДоступны("ДобавлениеИзменениеДанныхСотрудников, ПолныеПрава") Тогда
		Элементы.НастройкиИдентификаторов.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ВычетыПоНДФЛНаДетейНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ПроверитьСотрудникаНаНовый() Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполненияВычетаНаДетей", ЭтотОбъект);
		ОткрытьФорму("РегистрСведений.НДФЛСтандартныеВычетыНаДетей.Форма.ФормаЗаполненияСтандартныхВычетовДляДетей",
			Новый Структура("Сотрудник", Объект.Ссылка), ЭтотОбъект, , , , ОписаниеОповещения,
...
Процедура ИмущественныйВычетНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Если ПроверитьСотрудникаНаНовый() Тогда
		Возврат;
	КонецЕсли;
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполненияИмущественного", ЭтотОбъект);
	ОткрытьФорму("РегистрСведений.НДФЛИмущественныйВычет.Форма.ФормаЗаполненияИмущественныхВычетов",
		Новый Структура("Сотрудник", Объект.Ссылка), , , , , ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура СоциальныйВычетНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ПроверитьСотрудникаНаНовый() Тогда
		Возврат;
	КонецЕсли;
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполненияСоциального", ЭтотОбъект);
...
Процедура СтандартныйЛичныйВычетНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ПроверитьСотрудникаНаНовый() Тогда
		Возврат;
	КонецЕсли;
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполненияЛичного", ЭтотОбъект);
...
Процедура СтандартныеВычетыРеглНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ПроверитьСотрудникаНаНовый() Тогда
		Возврат;
	КонецЕсли;
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполненияВычетовНаДетейРегл", ЭтотОбъект);
	...
	Иначе
		...
		Если ДанныеВычетовНДФЛ.ВычетНаДетейУпр.Сумма > 0 Тогда
			...
			Для Каждого СтрВычет Из ДанныеВычетовНДФЛ.ВычетНаДетейУпр.КодыВычетов Цикл
				...
				СтрокиВычетов.Добавить(Новый Структура("КодВычета, ДействуетДо", СтрВычет.КодВычета,
					СтрВычет.ПериодЗавершения));
			КонецЦикла;
...
Процедура СоциальныйВычетРеглНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ПроверитьСотрудникаНаНовый() Тогда
		Возврат;
	КонецЕсли;
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполненияИмущественныхВычетовРегл", ЭтотОбъект);
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ИзФормыСотрудника", Истина);
	...
	Иначе
		...
		Если ДанныеВычетовНДФЛ.ИмущественныйВычетУпр.Сумма > 0 Тогда
			ДанныеЗаполнения.Вставить("РасходыНаСтроительствоПриобретение",
				ДанныеВычетовНДФЛ.ИмущественныйВычетУпр.РасходыПриобретение);
		...
		Если ДанныеВычетовНДФЛ.СоциальныйВычетУпр.Сумма > 0
			...
			ДанныеЗаполнения.Вставить("" + ДанныеВычетовНДФЛ.СоциальныйВычетУпр.ВидРасходов,
				ДанныеВычетовНДФЛ.СоциальныйВычетУпр.Сумма);
		КонецЕсли;
		ПараметрыФормы.Вставить("ЗначенияЗаполнения", ДанныеЗаполнения);
	КонецЕсли;
...
Процедура ДоходыПредыдущееМестоРаботыНажатие(Элемент, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполненияДоходыСПредыдущегоМестаРаботыРегл", ЭтотОбъект);
...
Процедура ИННПриИзменении(Элемент)
	ТекстРедактированияИНН = ОбщегоНазначенияКлиентСервер.ЗначениеСвойстваЭлементаФормы(Элементы, "ИНН",
		"ТекстРедактирования");
	СформироватьПредставлениеПроверкиИНННаСервере(ТекстРедактированияИНН);
КонецПроцедуры 
...
Процедура ПрекратитьСтандартныеВычеты(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗакрытиеПрекращениеСтандартныхВычетовРегл", ЭтотОбъект);
	ОткрытьФорму("Документ.ПрекращениеСтандартныхВычетовНДФЛ.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект, , , ,
		ОписаниеОповещения);
КонецПроцедуры
...
Функция ЭлементОсновной()
	...
	Если ПолучитьФункциональнуюОпцию("МиграцияПерсональныхНастроекЗавершена") Тогда
		ОсновнойОтветственный = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойОтветственный");
		Возврат ОсновнойОтветственный = Объект.Ссылка;
	КонецЕсли;
...
Функция ДанныеВычетовДляЗаголовка(СтруктураВычетов, ТипВычетов, ТекстВычетовПолный)
	...
	ТекстВычетовКраткий = НовыйТекстВычетов();
...
Процедура УстановитьЗаголовокВычетовНДФЛ(Форма)
	...
	ПолныеЗаголовкиУпр = НовыйТекстВычетов();
	ПолныеЗаголовкиУпр.ВычетНаДетей = Форма.ВычетыПоНДФЛНаДетей;
	ПолныеЗаголовкиУпр.ЛичныйВычет = Форма.СтандартныйЛичныйВычет;
	ПолныеЗаголовкиУпр.ИмущественныйВычет = Форма.ИмущественныйВычет;
	ПолныеЗаголовкиУпр.СоциальныйВычет = Форма.СоциальныйВычет;
	ДанныеЗаголовкаУпр = ДанныеВычетовДляЗаголовка(Форма.ДанныеВычетовНДФЛ, "Упр", ПолныеЗаголовкиУпр);
	
	ПолныеЗаголовкиРегл = НовыйТекстВычетов();
	ПолныеЗаголовкиРегл.ВычетНаДетей = Форма.СтандартныеВычетыРегл;
	ПолныеЗаголовкиРегл.ЛичныйВычет = Форма.СтандартныеВычетыРегл;
	ПолныеЗаголовкиРегл.ИмущественныйВычет = Форма.СоциальныйВычетРегл;
	ПолныеЗаголовкиРегл.СоциальныйВычет = Форма.СоциальныйВычетРегл;
...
&НаКлиентеНаСервереБезКонтекста
Функция НовыйТекстВычетов()
	Результат = Новый Структура;
	Результат.Вставить("ВычетНаДетей", НСтр("ru = 'на детей'"));
	Результат.Вставить("ЛичныйВычет", НСтр("ru = 'личный'"));
	Результат.Вставить("ИмущественныйВычет", НСтр("ru = 'имущественные'"));
	Результат.Вставить("СоциальныйВычет", НСтр("ru = 'социальные'"));
	Возврат Результат;
КонецФункции
...
&НаСервере
Процедура СформироватьПредставлениеПроверкиИНННаСервере(ПараметрИНН)
	
	ОписаниеОшибки = "";
	ТекущийИНН = СокрЛП(ПараметрИНН);
	Если ПустаяСтрока(ТекущийИНН) Тогда
		Объект.ИННВведенКорректно = Истина;
	Иначе
		Объект.ИННВведенКорректно = РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ТекущийИНН, Ложь,
			ОписаниеОшибки);
	КонецЕсли;
	ПредставлениеПроверкиИНН = Новый ФорматированнаяСтрока(ОписаниеОшибки, , ЦветТекстаНекорректногоИНН);
	
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		ЦветТекстаИНН = ЦветТекстаНекорректногоИНН;
	Иначе
		ЦветТекстаИНН = ЦветТекстаФормы;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИНН", "ЦветТекста", ЦветТекстаИНН);
