...
Функция ПолучитьТиповыеУсловия(Настройка = Неопределено, Организация = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("МиграцияПерсональныхНастроекЗавершена") Тогда
		...
		Результат = ПерсональныеНастройкиРУНФ.Прочитать(ИмяНастройки, Организация, , ТиповыеУсловия);
		Возврат Результат;
	КонецЕсли;
...
Процедура ЗаполнениеТиповыхДополнительныхУсловий() Экспорт
	...
	Для Каждого КлючИЗначение Из ТиповыеМакеты Цикл
		...
		Попытка
			...
			ДополнительныеУсловияОбъект = СоздатьЭлемент();
			...
			Макет = ПолучитьМакет(КлючИЗначение.Ключ);
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Предупреждение, Метаданные.Справочники.ДополнительныеУсловия, ТекстСообщения);
