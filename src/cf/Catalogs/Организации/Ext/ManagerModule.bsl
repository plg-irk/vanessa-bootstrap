...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Пользователи.ЭтоПолноправныйПользователь() Тогда
			...
			Дело.Форма			= "Обработка.ПанельАдминистрированияБЭД.Форма.ОбщиеНастройки";
			Дело.ПараметрыФормы = Новый Структура;
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.НаименованиеПолное = НСтр("ru = 'ООО ""Наша фирма""'", ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.Префикс = НСтр("ru = 'НФ-'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОрганизацияПоУмолчанию() Экспорт
	...
	ПользовательскаяНастройкаОсновнойОрганизации = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнаяОрганизация");
...
// Сокращенное наименование на дату
//
// Параметры:
//  Организация	 - СправочникСсылка.Организации	 - Организация, для которой нужно получить наименование
//  ДатаСведений - Дата - дата, на которую нужно получить значение наименования
...
//   Строка - значение наименования на дату
//
Функция СокращенноеНаименованиеНаДату(Организация, ДатаСведений) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаСведений) Тогда
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			Возврат Выборка.НаименованиеСокращенное;
		КонецЕсли;
	КонецЕсли;
...
// Наименование для печати на дату
//
// Параметры:
//  Организация	 - СправочникСсылка.Организации	 - Организация, для которой нужно получить наименование
//  ДатаСведений - Дата - дата, на которую нужно получить значение наименования
...
//   Строка - значение наименования на дату
//
Функция НаименованиеДляПечатиНаДату(Организация, ДатаСведений) Экспорт
	...
	Если ВидНаименования = Перечисления.ВидыНаименованийДляПечатиДокументов.Сокращенное Тогда
		Результат = СокращенноеНаименованиеНаДату(Организация, ДатаСведений);
	Иначе
		Результат = ПолноеНаименованиеНаДату(Организация, ДатаСведений);
	КонецЕсли;
...
// Проверяет, является ли организация юрлицом.
//
// Параметры:
//     Организация - СправочникСсылка.Организация - Организация.
//
// Возвращаемое значение:
//     Булево - признак того, что организация является юрлицом.
//
...
Процедура ДополнитьВалютуИКассуПриОтсутствииНастройкиПользователя(ЗначенияЗаполнения, Объект, ЗначениеРеквизитаВалюта,
	...
	ПользовательскаяНастройкаОсновнойКассы = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнаяКасса");
