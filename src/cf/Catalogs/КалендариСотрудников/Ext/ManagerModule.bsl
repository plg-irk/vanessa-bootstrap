...
//  ОтборПоДействительным - Булево - отбирает календари с отключенным признаком "Недействителен"
// 
...
// 
// Возвращаемое значение:
//  СправочникСсылка.КалендариСотрудников - Ссылка по идентификатору
...
// Проверить создать календарь сотрудника.
// 
// Параметры:
...
Процедура ПроверитьСоздатьКалендарьСотрудника(Сотрудник, Пользователь = Неопределено) Экспорт
	...
	НовыйКалендарь = СоздатьЭлемент();
	НовыйКалендарь.УстановитьНовыйКод();
	НовыйКалендарь.Заполнить(ДанныеЗаполнения);
	НовыйКалендарь.Записать();
	
	ПерсональныеНастройкиРУНФ.Установить(НовыйКалендарь.Ссылка, "ОсновнойКалендарь", Пользователь);
	
КонецПроцедуры

// Есть настроенные календари.
// 
// Параметры:
//  ДоступныеКалендари - Массив из СправочникСсылка.КалендариСотрудников - доступные календари
// 
// Возвращаемое значение:
//  Булево - Есть настроенные календари
...
Функция КалендариСотрудниковОтмеченныеКакОсновнойУПользователя()
	...
	ВсеКалендариСотрудников = Выбрать();
	
	УстановитьПривилегированныйРежим(Истина);
	
	Пока ВсеКалендариСотрудников.Следующий() Цикл
		ОсновнойКалендарьПользователя = ПерсональныеНастройкиРУНФ.Прочитать("ОсновнойКалендарь", ,
			ВсеКалендариСотрудников.Пользователь);
		Если ВсеКалендариСотрудников.Ссылка = ОсновнойКалендарьПользователя Тогда
			ОсновныеКалендариСотрудников.Вставить(ВсеКалендариСотрудников.ВладелецКалендаря,
				ВсеКалендариСотрудников.Ссылка);
		КонецЕсли;
	КонецЦикла;
