...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Контрагент" 
		...
		УстановитьВидимостьДоговора();
	ИначеЕсли ИмяСобытия = "Запись_ДоговорЭквайринга"
		И ЗначениеЗаполнено(Параметр)
		И Объект.Договор = Параметр Тогда
		УстановитьВидимостьКомиссии();
	КонецЕсли;
...
Функция ПолучитьПараметрыФормыВыбора(Ссылка, Организация, Контрагент, Договор)
	...
	Возврат Справочники.ДоговорыКонтрагентов.ПолучитьПараметрыФормыВыбора(Ссылка, Организация, Контрагент, Договор, Ложь);
...
Процедура УстановитьВидимостьКомиссии()
	...
	РасчетКомиссииВОтчетеЭквайера = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор,
	...
	Элементы.ГруппаПояснениеНастроекКомиссии1.Видимость = РасчетКомиссииВОтчетеЭквайера;
	Элементы.ПроцентКомиссии.Видимость = НЕ РасчетКомиссииВОтчетеЭквайера;
	Элементы.ПроцентКомиссииПриВозврате.Видимость = НЕ РасчетКомиссииВОтчетеЭквайера;
	
	Если Объект.Тип = Перечисления.ТипыСпособовОплат.Кредит Тогда
		Элементы.ДекорацияНадписьУчетКомиссии1.Заголовок =
			НСтр("ru = 'Комиссия вводится вручную в поступлении ДС от банка'");
	Иначе
		Элементы.ДекорацияНадписьУчетКомиссии1.Заголовок =
			НСтр("ru = 'Комиссия вводится вручную в отчете эквайера'");
	КонецЕсли;
