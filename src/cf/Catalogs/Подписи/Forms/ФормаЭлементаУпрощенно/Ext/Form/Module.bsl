...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("БыстроеСоздание") Тогда
		//@skip-check unknown-form-parameter-access
		Объект.Наименование = Параметры.Наименование;
		Объект.РасшифровкаПодписи = Объект.Наименование;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ФизическоеЛицо)
		ИЛИ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ФизическоеЛицо, "Наименование") <> Объект.Наименование Тогда
		СоздатьОбновитьФизЛицо(ТекущийОбъект);
	КонецЕсли;
	
	ТекущийОбъект.РасшифровкаПодписи = Объект.Наименование;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_Подписи", Объект.Ссылка, Объект.Ссылка);
...
Процедура ФаксимилеПодписиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Наименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Сначала необходимо заполнить ФИО'"),, "Наименование");
		Возврат;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.ФизическоеЛицо) Тогда
		Объект.ФизическоеЛицо = СоздатьФизЛицо(Объект.Наименование);
		Модифицированность = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура ДекорацияПредыдущиеКартинкиОтсутствуютНажатие(Элемент)
...
&НаКлиенте
Процедура ДекорацияСледующиеКартинкиОтсутствуютНажатие(Элемент)
...
Процедура УдалитьФаксимилеПодписиИзПлажки(Команда)
	...
	АдресВыбранногоИзображения = ЭтотОбъект[ЭтотОбъект.ТекущийЭлемент.Имя];
...
&НаСервере
Процедура СоздатьОбновитьФизЛицо(ТекущийОбъект)
	
	Если ЗначениеЗаполнено(ТекущийОбъект.ФизическоеЛицо) Тогда
		ФизЛицоОбъект = ТекущийОбъект.ФизическоеЛицо.ПолучитьОбъект();
	Иначе
		ФизЛицоОбъект = Справочники.ФизическиеЛица.СоздатьЭлемент();
	КонецЕсли;
	
	ФизЛицоОбъект.Наименование = ТекущийОбъект.Наименование;
	ФизЛицоОбъект.УстановитьНовыйКод();
	ФизЛицоОбъект.Записать();
	
	ТекущийОбъект.ФизическоеЛицо = ФизЛицоОбъект.Ссылка;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция СоздатьФизЛицо(Наименование)
...
Процедура ОбработатьНажатиеПлашки(Элемент)
	...
	СтрокаИзображенияТекущиеПлашки = НайтиТекущуюСтрокуВТаблицеИзображенийНаКлиенте(Элемент.Имя);
	//@skip-check module-self-reference
	Если Не ЗначениеЗаполнено(ЭтотОбъект[ЭтотОбъект.ТекущийЭлемент.Имя]) Тогда
...
Процедура ПодставитьИзображенияТройкиВПлашки(Тройка)
	...
	Элементы.ДекорацияСледующиеКартинки.Видимость = ЗначениеЗаполнено(Плашка3);
...
Функция РазрешеноИзменениеИзображения(Изображение)
	...
	ПараметрыИзображения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Изображение,"Зашифрован, ПодписанЭП, Редактирует");	
	Если ПараметрыИзображения.Зашифрован
		ИЛИ ПараметрыИзображения.ПодписанЭП
		ИЛИ ЗначениеЗаполнено(ПараметрыИзображения.Редактирует) Тогда
