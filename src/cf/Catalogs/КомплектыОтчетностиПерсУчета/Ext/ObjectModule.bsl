...
// Создание и заполнение пачек отчетности перс. учета.
// 
// Параметры:
//  РСВКомплекта - ДокументСсылка.РегламентированныйОтчет
...
Процедура СформироватьСведенияКвартальнойОтчетности(РСВКомплекта, УчитыватьИсправления = Истина) Экспорт
	...
	Если ОтчетныйПериод >= '20140101' Тогда
		...
		Если СписокПачекСЗВ.Количество() > 0
			И ТипЗнч(СписокПачекСЗВ[0].ПачкаДокументов) = Тип("ДокументСсылка.ПачкаРазделов6РасчетаРСВ_1") Тогда
			ПачкаРазделов6 = СписокПачекСЗВ[0].ПачкаДокументов;
		Иначе
			ПачкаРазделов6 = НовыйПачкаРазделов6(Организация, ОтчетныйПериод);
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(ПачкаРазделов6,
			СправкиПоНДФЛ.ДанныеПодписывающихЛиц(Организация, ОтчетныйПериод, ,Истина));
	...
	Если ОписьПачекСЗВОбъект <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ОписьПачекСЗВОбъект,
			СправкиПоНДФЛ.ДанныеПодписывающихЛиц(Организация, ОтчетныйПериод, ,Истина));
	КонецЕсли;
	...
	ТаблицаИсходныхСЗВ = УчетСтраховыхВзносовУНФ.СформироватьПачкиДокументовСЗВ(
		?(ОписьПачекСЗВОбъект = Неопределено, ПачкаРазделов6, ОписьПачекСЗВОбъект),
		'00010101', УчитыватьИсправления, Истина, МассивДокументовКомплекта, , ОтчетныйПериод); 
...
// Сформировать корректирующие сведения.
// 
// Параметры:
...
//  - ТаблицаЗначений
// 		* ПачкаДокументов - ДокументСсылка.ПачкаРазделов6РасчетаРСВ_1
//		* НомерПачки - Число
Функция СформироватьКорректирующиеСведения(КорректируемыйПериод, МассивФизЛиц = Неопределено) Экспорт
	...
	Если МассивФизЛиц = Неопределено Тогда
		СписокПачек = Справочники.КомплектыОтчетностиПерсучета.СкопироватьПачкиПрошлогоПериода(Организация,
			КорректируемыйПериод, МассивФизЛиц);
		ЗаменятьСуществующиеПачки = Истина;
	Иначе
		СписокПачек = Справочники.КомплектыОтчетностиПерсучета.МассивПачекПрошлогоПериода(Организация,
			КорректируемыйПериод, МассивФизЛиц);
	...
	МассивКорректирующихПачекПериода = МассивСсылокСЗВ(Перечисления.ТипыСведенийСЗВ.КОРРЕКТИРУЮЩАЯ,
		МассивУчитываемыхПериодов);
	...
	ТаблицаСЗВ = СформироватьКорректирующиеПачки(ОписьОбъект, КорректируемыйПериод, СписокПачек, МассивФизЛиц,
		УчитыватьИсправления, ЗаменятьСуществующиеПачки);		
...
// Перераспределить взносы между сотрудниками в комплекте отчетности.
// 
// Параметры:
...
// Создание временных таблиц ВТССотрудникамиКомплектаПоКатегориям и ВТДублиСотрудниковПоКатегориям.
// 
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
//  СписокПачек - Массив из ДокументСсылка.ПачкаДокументовСЗВ_6_1 - Список пачек
//  МассивФизЛиц - Неопределено
//				 - Массив из СправочникСсылка.ФизическиеЛица
Процедура СформироватьВТССотрудникамиКомплектаПоКатегориям(МенеджерВременныхТаблиц, СписокПачек,
		МассивФизЛиц = Неопределено) Экспорт
	...
	|	ДанныеДокументов.КатегорияЗастрахованныхЛиц КАК КатегорияЗастрахованныхЛиц,
...
// Зафиксировать строки стажа по сотруднику в документах отчетности перс. учета.
// 
// Параметры:
...
Функция СформироватьСведенияКорректирующихПериодов(ОписьПачекСЗВОбъект, УчитыватьИсправления)
	...
	Для Каждого КорректируемыйПериод Из МассивПериодов Цикл	
		...
		ТаблицаКорректирующихСЗВ = СформироватьКорректирующиеПачки(ОписьПачекСЗВОбъект, КорректируемыйПериод,
			МассивКорректирующихПачекКомплекта, МассивФизЛиц, УчитыватьИсправления);
...
Функция СформироватьКорректирующиеПачки(ОписьПачекСЗВОбъект, КорректируемыйПериод, МассивПачек, МассивФизЛиц,
	УчитыватьИсправления = Ложь, ЗаменятьСуществующиеПачки = Истина)
	
	Возврат  УчетСтраховыхВзносовУНФ.СформироватьПачкиДокументовСЗВ(ОписьПачекСЗВОбъект, КорректируемыйПериод,
		УчитыватьИсправления, ЗаменятьСуществующиеПачки, МассивПачек, МассивФизЛиц, КорректируемыйПериод);
...
Функция СписокКорректируемыхПериодов()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РеестрСЗВ_6_2.КорректируемыйПериод КАК КорректируемыйПериод
	|ИЗ
...
Процедура ПерераспределитьВзносыЗаПериод(КорректирующиеСведения, ПериодРасчета, СравниватьСТекущимиДанными = Ложь,
	ДанныеИзменены = Ложь)
	...
	УчетСтраховыхВзносовУНФ.СформироватьВТСДаннымиОВзносахПоКатегориям(МенеджерВТ, Организация, ПериодРасчета,
		ДатаАктуальности, КорректирующиеСведения, МассивПачек, Истина);
