...
Процедура ТорговыйОбъектПриИзменении(Элемент)
	...
	Для Каждого СтрокаКассы Из Объект.КассыККМ Цикл
		СтрокаКассы.КассаККМ = Неопределено;
	КонецЦикла;
	
	Для Каждого СтрокаРаспределения Из Объект.РаспределениеНоменклатуры Цикл
		СтрокаРаспределения.КассаККМ = Неопределено;
	КонецЦикла;
	
	ТорговыйОбъектПриИзмененииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ТорговыйОбъектПриИзмененииНаСервере()
