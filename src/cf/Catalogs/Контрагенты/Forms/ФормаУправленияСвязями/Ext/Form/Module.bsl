...
Процедура ЗаписатьДанныеПоСвязямВРегистр()
	...
	Для Каждого Контакт Из НедействительныеСвязиКонтактныеЛица Цикл
		...
		Если ИзменяемыйКонтрагент.Количество() = 0 Тогда
			...
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контакт.Контрагент, "КонтактноеЛицо")
				= Контакт.КонтактноеЛицо И Контакт.ЭтоОсновноеКЛ Тогда
				НоваяСтрока.ИзменитьОсновной = Контакт.ЭтоОсновноеКЛ;
			КонецЕсли;
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контакт.Контрагент, "КонтактноеЛицоПодписант")
				= Контакт.КонтактноеЛицо И Контакт.ЭтоПодписант Тогда
				НоваяСтрока.ИзменитьПодписанта = Контакт.ЭтоПодписант;
			КонецЕсли;
		Иначе
			НоваяСтрока = ИзменяемыйКонтрагент[0];
			Если Не НоваяСтрока.ИзменитьОсновной И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контакт.Контрагент,
				"КонтактноеЛицо") = Контакт.КонтактноеЛицо Тогда
				НоваяСтрока.ИзменитьОсновной = Контакт.ЭтоОсновноеКЛ;
			КонецЕсли;
			Если Не НоваяСтрока.ИзменитьПодписанта И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контакт.Контрагент,
				"КонтактноеЛицоПодписант") = Контакт.КонтактноеЛицо Тогда
				НоваяСтрока.ИзменитьПодписанта = Контакт.ЭтоПодписант;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьКонтактНаОсновнойИПодписанта(ДанныеКонтактногоЛица,Оповещение)
	...
	Если КонтактОсновноеКЛ И КонтактПодписант Тогда
		...
		Если ВыделенныеКонтрагенты.Количество() > 1 Тогда
			ТекстВопроса = НСтр("ru = 'Контакт используется как основной и подписант для некоторых контрагентов
			|При записи данные признаки контрагента станут незаполненными'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Контакт используется как основной/подписант для контрагента
			|При записи данные признаки станут незаполненными'");
		КонецЕсли;
	ИначеЕсли КонтактОсновноеКЛ Тогда
		
		Если ВыделенныеКонтрагенты.Количество() > 1 Тогда
			ТекстВопроса = НСтр("ru = 'Контакт используется как основной для некоторых контрагентов
	...
	ИначеЕсли КонтактПодписант Тогда
		Если ВыделенныеКонтрагенты.Количество() > 1 Тогда
			ТекстВопроса = НСтр("ru = 'Контакт используется как подписант для некоторых контрагентов
				|При записи данный признак станет незаполненным'");
		Иначе
