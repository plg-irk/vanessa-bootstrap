...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытиеДляПоказаПослеЗакрытия = Параметры.СобытиеДляПоказаПослеЗакрытия;
	...
	ИспользоватьПодсистемуМобильныйЛичныйКабинет = ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуМобильныйЛичныйКабинет");
	...
	Иначе
		...
		Если ВестиУчетПрослеживаемыхТоваров И ЗначениеЗаполнено(Объект.СтранаРегистрации) Тогда
			УчастникЕАЭС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СтранаРегистрации, "УчастникЕАЭС");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПривязанныйКонтакт) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.Наименование) Тогда
			Объект.Наименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПривязанныйКонтакт, "Наименование");
		КонецЕсли;
	...
	Если Параметры.Свойство("ТекущийКонтакт", ТекущийКонтакт) Тогда
		...
		Если ТекущиеКонтакты.Количество() <> 0 Тогда 
			ЭлементКонтакта = Элементы.Найти("НаименованиеКонтакт_" + ДанныеКонтактныхЛиц.Индекс(ТекущиеКонтакты[0]));
			Если ЭлементКонтакта <> Неопределено Тогда
				ТекущийЭлемент = ЭлементКонтакта;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.Ссылка.Пустая() Тогда
		СтатистикаПоПоказателямКлиентСервер.ДобавитьСобытие("Данные.Контрагенты.ЗаписьНового");
	Иначе
		СтатистикаПоПоказателямКлиентСервер.ДобавитьСобытие("Данные.Контрагенты.Запись");
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ПривязанныйКонтакт = Неопределено;
	ЗаполнитьИОбновитьКонтактныеЛица();
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	УстановитьЗаголовокФормы(ЭтотОбъект);
	СтруктураПараметров = Новый Структура("Ссылка, ИзменилиКонтактнуюИнформацию", Объект.Ссылка, ИзменилиКонтактнуюИнформацию);
	Оповестить("Запись_Контрагент", СтруктураПараметров, ЭтотОбъект);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВыбранДубльКонтакта = Истина Тогда			
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выбран дубль контакта! Дублирование контактов в карточке контрагента не допускается'"),,);
		ВыбранДубльКонтакта = Ложь;	
	КонецЕсли;
...
Процедура ОстатокВзаиморасчетовОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ВариантыОтчетовКлиент.ОткрытьФормуОтчета(ЭтотОбъект, Вариант, ПараметрыОткрытия);
...
Процедура НаименованиеКонтактногоЛица_ПриИзменении(Элемент)
	...
	ЗаголовкиГрупп = ЗаголовкиГруппКонтакта(ДанныеКонтактныхЛиц[ИндексКЛ], ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов, ИндексКЛ);
...
Процедура НаименованиеКонтактногоЛица_ОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.КонтактныеЛица") Тогда
		...
		СтандартнаяОбработка = Ложь;
		ИндексВыбранногоКонтактногоЛица = Число(Сред(Элемент.Имя, СтрДлина("НаименованиеКонтакт_")+1));
		ОбновитьЭлементТаблицыКонтактныхЛиц(ИндексВыбранногоКонтактногоЛица, ВыбранноеЗначение, Истина);
		ТекущийЭлемент = Элементы.НаименованиеПолное;
...
&НаКлиенте
Процедура КППРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "ВидКИЮрАдрес" Тогда
		
		СтандартнаяОбработка = Ложь;
		ОткрытьЗначениеАсинх(ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ЮрАдресКонтрагента"));
...
Процедура ИспользоватьКакОсновной(Команда)
	...
	Если ОсновнойКонтакт.Количество() > 0 Тогда
		...
		ИндексБывшегоОсновного = Число(Сред(ОсновнойКонтакт[0].ИмяЭлементаДляРазмещения,СтрНайти(ОсновнойКонтакт[0].ИмяЭлементаДляРазмещения,"_") + 1));
		ДанныеКонтактныхЛиц[ИндексБывшегоОсновного].Изменен = Истина;  
		ЗаголовкиГрупп = ЗаголовкиГруппКонтакта(ДанныеКонтактныхЛиц[ИндексБывшегоОсновного], ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов, ИндексБывшегоОсновного);
	
		Элементы["Контакт_"+ИндексБывшегоОсновного].Заголовок = ЗаголовкиГрупп.ЗаголовокГруппы;
		Элементы["Контакт_"+ИндексБывшегоОсновного].ЗаголовокСвернутогоОтображения = ЗаголовкиГрупп.ЗаголовокСвернутойГруппы;
	...
	ЗаголовкиГрупп = ЗаголовкиГруппКонтакта(ДанныеКонтактныхЛиц[ИндексКЛ], ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов, ИндексКЛ);
	Объект.КонтактноеЛицо = ДанныеКонтактныхЛиц[ИндексКЛ].КонтактноеЛицо;
...
Процедура ОбновитьКонтактнуюИнформацию(Результат)
	ИзменилиКонтактнуюИнформацию = Истина;
...
Функция ЯвляетсяБанкомРФ(Банк)
	...
	Если ТипЗнч(Банк) = Тип("СправочникСсылка.КлассификаторБанков") Тогда
		КодСтраныРоссия = "643";
		Результат =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Банк, "КодСтраны") = КодСтраныРоссия;
	КонецЕсли;
	
	Если ТипЗнч(Банк) = Тип("СправочникСсылка.Банки") Тогда
		Результат =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Банк, "Страна") = Справочники.СтраныМира.Россия;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Если ЭтоЮридическоеЛицо Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "УчастникЕАЭС", "Видимость", ВестиУчетПрослеживаемыхТоваров);
	...
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ГосударственныйОрган") Тогда
		...
		Элементы.Пол.Видимость								= Ложь;
		Элементы.ДатаРождения.Видимость						= Ложь;
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "УчастникЕАЭС", "Видимость", Ложь);
	...
	Элементы.ОстатокВзаиморасчетов.Видимость	= ПросмотрВзаиморасчетов;
	Элементы.СуммаПродаж.Видимость				= ПросмотрПродаж И Объект.Покупатель;
	Элементы.ПоследняяПродажа.Видимость			= ПросмотрПродаж И Объект.Покупатель;
...
Процедура ЗаполнитьКонтактКакСвязаться()
	...
	Если КонтактКакСвязаться.ВидКонтакта = "Контрагент" Тогда
		...
		СтруктураКИ.Вставить("Представление", КонтактКакСвязаться.КакСвязаться);
		СтруктураКИ.Вставить("Комментарий");
		СтруктураКИ.Вставить("Значение");
	...
	Если КонтактКакСвязаться.ВидКонтакта = "КонтактноеЛицо" Тогда
		...
		Если ТипЗнч(КонтактКакСвязаться.Контакт) = Тип("СправочникСсылка.КонтактныеЛица") Тогда
			КонтактСсылка = КонтактКакСвязаться.Контакт; // см. СправочникСсылка.КонтактныеЛица
			ДанныеКЛ.Наименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтактСсылка, "Наименование");
...
Процедура ЗаполнитьКонтактнуюИнформацию()
	...
	Если Параметры.ДобавитьКонтактнуюИнформацию = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого ОписаниеКИ Из ДобавляемаяКИ Цикл
		ДанныеКИ = Новый Структура();
		ДанныеКИ.Вставить("ИмяЭлементаДляРазмещения");
		ДанныеКИ.Вставить("Вид");
		ДанныеКИ.Вставить("Представление");
		ДанныеКИ.Вставить("Значение");
...
&НаСервере
Процедура ЗаполнитьПоДаннымКонтактнойФормы(ДополнительныеПараметры)
	
	Если ДополнительныеПараметры.Свойство("Связь") Тогда
		ДанныеКЛ = ДанныеКонтактныхЛиц.Добавить();
		ИндексКЛ = ДанныеКонтактныхЛиц.Индекс(ДанныеКЛ);
		ДанныеКЛ.Изменен = Истина;
		ОбновитьЭлементыКонтактныхЛиц();
	...
	Иначе
		...
		Объект.Наименование = ДополнительныеПараметры.Контакт;
	КонецЕсли;
	
	ДанныеКФ = ДополнительныеПараметры.ДанныеИзКонтактнойФормы;
...
Процедура УстановитьВидимостьИДоступностьМобильноеПриложениеИРМК()
	...
	Если МобильноеПриложениеВызовСервера.НужноОграничитьФункциональность() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаСправочникКонтрагентыДосьеКонтрагента", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПроверитьКонтрагента", "Видимость", Ложь);
...
Процедура УстановитьВидимостьИДоступность()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "УчастникЕАЭС", "Видимость", ДоступностьВыбораУчастникаЕАЭС);
	
	Если Не ВестиУчетПрослеживаемыхТоваров Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтранаРегистрации", "МаксимальнаяШирина", 34);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаИннГоловной", "ГоризонтальныйИнтервал", ИнтервалМеждуЭлементамиФормы.Одинарный);
	КонецЕсли;
...
Процедура РазорватьСвязьНаСервере()
	...
	Если ЗначениеЗаполнено(ДанныеКонтактногоЛица.КонтактноеЛицо) Тогда
		...
		ОсновнойКонтакт = ДанныеКонтактныхЛиц.НайтиСтроки(Новый Структура("Основной", Истина));
		Если ОсновнойКонтакт.Количество() > 0 И ОсновнойКонтакт[0].КонтактноеЛицо = ДанныеКонтактногоЛица.КонтактноеЛицо Тогда
			ОсновнойКонтакт[0].Основной = Ложь;
			Объект.КонтактноеЛицо = Неопределено;
		КонецЕсли; 
	
		Подписант = ДанныеКонтактныхЛиц.НайтиСтроки(Новый Структура("Подписант", Истина));
		Если Подписант.Количество() > 0 И Подписант[0].КонтактноеЛицо = ДанныеКонтактногоЛица.КонтактноеЛицо Тогда
			Подписант[0].Подписант = Ложь;
			Объект.КонтактноеЛицоПодписант = Неопределено;
		КонецЕсли; 
...
// Установить ИННОбособленного подразделения.
// 
// Параметры:
...
&НаКлиенте
Процедура СозданиеНовогоКонтакта(Результат, ДополнительныеПараметры) Экспорт
	
	ЭлементКИ = Элементы.Найти("ПредставлениеКонтакт_"+ИндексВыбранногоКонтактногоЛица+"_КИ_0");
	
	ДанныеКонтактныхЛиц[ИндексВыбранногоКонтактногоЛица].Наименование = ДополнительныеПараметры.Текст;
	ДанныеКонтактныхЛиц[ИндексВыбранногоКонтактногоЛица].Изменен = Истина;
	ДанныеКонтактныхЛиц[ИндексВыбранногоКонтактногоЛица].ФамилияИнициалы = ФамилияИнициалыФИО(
...
&НаСервереБезКонтекста
Функция ФамилияИнициалыФИО(Знач Наименование)
	
	Возврат Справочники.КонтактныеЛица.ФамилияИнициалыФИО(Наименование);
...
Процедура ЗаполнитьТаблицуКонтактнойИнформацииКонтактныхЛиц()
	...
	Если ЗначениеЗаполнено(ПривязанныйКонтакт) Тогда
		...
		ДанныеКЛ = ДанныеКонтактныхЛиц.Добавить();
		ДанныеКЛ.Изменен = Истина;
		ДанныеПривязанногоКонтакта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПривязанныйКонтакт,
			"Наименование, ФамилияИнициалы");
		
		КонтактноеЛицо = Новый Структура;
		КонтактноеЛицо.Вставить("КонтактноеЛицо",  ПривязанныйКонтакт);
		КонтактноеЛицо.Вставить("Ссылка",          ПривязанныйКонтакт);       				
		КонтактноеЛицо.Вставить("Наименование",    ДанныеПривязанногоКонтакта.Наименование);
		КонтактноеЛицо.Вставить("ФамилияИнициалы", ДанныеПривязанногоКонтакта.ФамилияИнициалы);
		КонтактноеЛицо.Вставить("Должность",       "");
		
		ЗаполнитьЗначенияСвойств(ДанныеКЛ, КонтактноеЛицо,
			"КонтактноеЛицо,Наименование,Должность,Ссылка,ФамилияИнициалы"); 
		Возврат;
	...
	|	КонтактныеЛица.ФамилияИнициалы КАК ФамилияИнициалы,
	...
	|	втКонтакты.ФамилияИнициалы КАК ФамилияИнициалы,
	...
	Пока ВыборкаКонтакты.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(ДанныеКЛ, ВыборкаКонтакты,
			"КонтактноеЛицо,Наименование,Должность,ФамилияИнициалы,Порядок");
...
Процедура ОбновитьЭлементыКонтактныхЛиц(Знач ИндексОбновляемогоКонтакта = Неопределено)
	...
	Для ИндексКонтакта = 0 По ПоследнийИндексКонтакта Цикл
		...
		Если ИндексКонтакта > 0 Тогда
			...
			КомандаУстановитьСвязь = Команды.Добавить("КомандаУстановитьСвязь_" + ИндексКонтакта);
			...
			КомандаРазорватьСвязь = Команды.Добавить("КомандаРазорватьСвязь_" + ИндексКонтакта);
		...
		ЗаголовкиГрупп = ЗаголовкиГруппКонтакта(ДанныеКонтактныхЛиц[ИндексКонтакта],
			ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов, ИндексКонтакта);
		Если ИндексКонтакта = 0 Тогда     
...
Процедура ДобавитьКоманднуюПанельКонтакта(ИндексКонтакта, ГруппаДолжность)
	...
	КомандаИспользоватьКакОсновной = Команды.Добавить("ИспользоватьКакОсновной_" + ИндексКонтакта);
	...
	КомандаИспользоватьКакПодписанта = Команды.Добавить("ИспользоватьКакПодписанта_" + ИндексКонтакта);
	...
	КомандаПереместитьВверх = Команды.Добавить("ПереместитьВверх_" + ИндексКонтакта);
	...
	КомандаПереместитьВниз = Команды.Добавить("ПереместитьВниз_" + ИндексКонтакта);
	...
	КомандаПоказатьВСписке = Команды.Добавить("ПоказатьВСписке_" + ИндексКонтакта);
...
&НаКлиентеНаСервереБезКонтекста
Функция ЗаголовкиГруппКонтакта(ДанныеКЛ, ТаблицаКИДопРеквизиты = Неопределено, ИндексКЛ = Неопределено)
	...
	Если ЗначениеЗаполнено(ДанныеКЛ.КонтактноеЛицо) Тогда
		...
		Если ЗначениеЗаполнено(ТаблицаКИДопРеквизиты) И ИндексКЛ <> Неопределено Тогда  
			ОтборПоТелефону = Новый Структура("Тип, ИмяЭлементаДляРазмещения", 
				ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон"),"КонтактнаяИнформацияКонтакт_" + ИндексКЛ);
			ТипКИТелефон = ТаблицаКИДопРеквизиты.НайтиСтроки(ОтборПоТелефону);     
		КонецЕсли;		
		...
		Если ЗначениеЗаполнено(ТаблицаКИДопРеквизиты) И ИндексКЛ <> Неопределено Тогда  
			ОтборПоПочте = Новый Структура("Тип, ИмяЭлементаДляРазмещения", 
				ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты"),"КонтактнаяИнформацияКонтакт_" + ИндексКЛ);
			ТипКИПочта = ТаблицаКИДопРеквизиты.НайтиСтроки(ОтборПоПочте);     
		КонецЕсли;
...
// Добавить данные основной подписант.
// 
// Параметры:
//  Заголовки - Структура из КлючИЗначение
//  ДанныеКЛ - ТаблицаЗначений
//@skip-check module-unused-method
&НаКлиентеНаСервереБезКонтекста
...
&НаСервере
Процедура ОбновитьЭлементТаблицыКонтактныхЛиц(Индекс, Контакт, ОтправлятьСообщение = Ложь)
	
	СтрокиДубли = ДанныеКонтактныхЛиц.НайтиСтроки(Новый Структура("КонтактноеЛицо", Контакт));
	Если СтрокиДубли.Количество() > 0 Тогда
		Если ОтправлятьСообщение = Истина Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр(
				"ru = 'Выбран дубль контакта! Дублирование контактов в карточке контрагента не допускается'"), , );
		Иначе
			ВыбранДубльКонтакта = Истина;
		КонецЕсли;
	...
	ДанныеКонтактныхЛиц[Индекс].ФамилияИнициалы = Контакт.ФамилияИнициалы;
	...
	Для Каждого ДанныеКЛ Из ДанныеКонтактныхЛиц Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеКЛ.КонтактноеЛицо) Или КонтактноеЛицоОбъект = Неопределено Тогда
			...
			Если ТекущийОбъект.ДополнительныеСвойства.Свойство("НовоеОсновноеКонтактноеЛицо") Тогда
				КонтактноеЛицоОбъект.УстановитьСсылкуНового(
					ТекущийОбъект.ДополнительныеСвойства.НовоеОсновноеКонтактноеЛицо);
			...
			Если ТекущийОбъект.ДополнительныеСвойства.Свойство("НовоеКонтактноеЛицоПодписант") Тогда
				КонтактноеЛицоОбъект.УстановитьСсылкуНового(
					ТекущийОбъект.ДополнительныеСвойства.НовоеКонтактноеЛицоПодписант);
		...
		// Внесение изменений
		ЗаполнитьЗначенияСвойств(КонтактноеЛицоОбъект, ДанныеКЛ, "Наименование, ФамилияИнициалы");
		
		Если ЗначениеЗаполнено(Объект.ГруппаДоступа)
			И НЕ ЗначениеЗаполнено(КонтактноеЛицоОбъект.ГруппаДоступа) Тогда
			КонтактноеЛицоОбъект.ГруппаДоступа = Объект.ГруппаДоступа;
		КонецЕсли;
		
		УправлениеКонтактнойИнформацией.ПриЗаписиНаСервереСвязанныхВладельцевКИ(ЭтотОбъект, КонтактноеЛицоОбъект,
			ДанныеКЛ, Отказ);
...
Процедура ПрочитатьДанныеПанелиДопИнформации()
	...
	КрупныйШрифт = ШрифтыСтиля.ШрифтВспомогательнойНадписи11;
	МелкийШрифт  = ШрифтыСтиля.ШрифтМеток;
	...
	|ГДЕ
	|	(СобытиеУчастники.Контакт В (&КонтактыНаПоиск)
	|			ИЛИ СвязиАдресатКонтактCRM.КонтактCRM В (&КонтактыНаПоиск))
	|	И СобытиеУчастники.Ссылка.ПометкаУдаления = ЛОЖЬ
	|
	|УПОРЯДОЧИТЬ ПО
	|	СобытиеУчастники.Ссылка.НачалоСобытия УБЫВ";
	
	КонтактыНаПоиск = Новый Массив;
	КонтактыНаПоиск.Добавить(Объект.Ссылка);
	СвязанныеКонтакты = Справочники.Контрагенты.СвязанныеКонтакты(Объект.Ссылка);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(КонтактыНаПоиск, СвязанныеКонтакты);

	Если РаботаСЛидами.ЛидыДоступны() Тогда
		ЛидОснование = Справочники.Контрагенты.ЛидОснование(Объект.Ссылка);
		КонтактыНаПоиск.Добавить(ЛидОснование);
	КонецЕсли;
	
	Запрос.УстановитьПараметр("КонтактыНаПоиск", КонтактыНаПоиск);
...
Процедура ПроверитьНаДубли(Реквизит)
	...
	Если Реквизит = "НаименованиеПолное" Тогда
		Если НЕ ЗначениеЗаполнено(Объект.НаименованиеПолное) Тогда
			Возврат;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ОрганизационноПравоваяФорма.НаименованиеБезОПФ) Тогда
			Возврат;
		КонецЕсли;
		НаименованиеПолноеДляПоискаДублей = НаименованиеДляПоискаДублей(ОрганизационноПравоваяФорма.НаименованиеБезОПФ);
	КонецЕсли;
...
Функция ТекстПредупрежденияПоДублям(СообщениеПриЗаписи = Ложь)
	...
	Если СообщениеПриЗаписи Тогда
		Возврат СтрШаблон(НСтр("ru = 'Проигнорировано сообщение о дублировании информации по %1'"), СтрСоединить(МассивСтрок,", "));
	КонецЕсли;
	
	ТекстПредупреждения = СтрШаблон(НСтр("ru = 'Найдены дубли по %1'"), СтрСоединить(МассивСтрок,", "));
...
Процедура УстановитьОтображениеКабинетаКлиента()
	...
	Если ИспользоватьПодсистемуМобильныйЛичныйКабинет = Ложь Тогда 
	...
	НастройкиПубликацииМЛК = Справочники.НастройкиПубликацииМЛК.ПолучитьНастройкиПубликацииМЛК(); // см. СправочникСсылка.НастройкиПубликацииМЛК
