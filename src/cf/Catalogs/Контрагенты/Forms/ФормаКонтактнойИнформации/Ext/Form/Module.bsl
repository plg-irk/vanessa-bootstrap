...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.КонтрагентыНачальноеЗначение) = Тип("Массив") Тогда
		...
		Для Каждого ЭлементСписка Из СписокКонтрагентов Цикл
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементСписка.Значение, "ЭтоГруппа") Тогда
	...
	Если ПустаяСтрока(Параметры.ВариантПечатиНачальноеЗначение) Тогда
		Если СписокКонтрагентов.Количество() > 1 Или (СписокКонтрагентов.Количество() = 1 И 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СписокКонтрагентов[0].Значение, "ЭтоГруппа")) Тогда
...
//
// Параметры:
//  Контрагенты	 - Массив из см. ОбновитьКонтактнуюИнформациюНаСервере - контрагенты для которых распечатывается контактная информация
	...
	Для Каждого Контрагент Из Контрагенты Цикл
		...
		Если ВыборкаКонтрагенты.НайтиСледующий(ПоискПоКонтрагенту) Тогда
			...
			Если НомерЗаписиКонтрагента % 2 = 0 Тогда
				...
				Область.ЦветФона =  ЦветаСтиля.ЦветИзмененногоПоля;
			КонецЕсли;
...
//
// Параметры:
//  СписокКонтрагентов - СписокЗначений из см. ОбновитьКонтактнуюИнформациюНаСервере - Список контрагентов может содержать элементы и группы справочника.
// Возвращаемое значение:
//  СписокЗначений из см. ОбновитьКонтактнуюИнформациюНаСервере - Список контрагентов, содержит только элементы. Имеющиеся группы разворачиваются до элементов.
&НаСервереБезКонтекста
Функция ПолучитьКонтрагентовКПечати(СписокКонтрагентов)
	...
	КонтрагентыСсылки = Новый Массив;
	КонтрагентыГруппы = Новый Массив;
	
	Для Каждого Контрагент Из Контрагенты Цикл
		Если Контрагент.ЭтоГруппа Тогда
			КонтрагентыГруппы.Добавить(Контрагент);
		Иначе
			КонтрагентыСсылки.Добавить(Контрагент);
		КонецЕсли;
	КонецЦикла;
	...
	Запрос.Текст = 
		...
		|	Справочник.Контрагенты КАК Контрагенты
		|ГДЕ
		|	Контрагенты.Ссылка В ИЕРАРХИИ (&СсылкаГруппыКонтрагентов)
		|	И Контрагенты.ПометкаУдаления = ЛОЖЬ
		|	И Контрагенты.ЭтоГруппа = ЛОЖЬ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	врКонтрагенты.Контрагент КАК Контрагент
		|ИЗ
	...
	Запрос.УстановитьПараметр("Ссылка", КонтрагентыСсылки);
	Запрос.УстановитьПараметр("СсылкаГруппыКонтрагентов", КонтрагентыГруппы);
