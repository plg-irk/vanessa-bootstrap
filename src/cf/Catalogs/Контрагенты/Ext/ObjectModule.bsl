...
Процедура СформироватьОсновныеСведения() Экспорт
	...
	КонтактнаяИнф = КонтактнаяИнформация.Выгрузить();
	КонтактнаяИнф.Сортировать("Вид");
	
	ВидыКонтактнойИнформации = КонтактнаяИнф.ВыгрузитьКолонку("Вид"); // Массив из СправочникСсылка.ВидыКонтактнойИнформации 
	РеквизитыВид = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ВидыКонтактнойИнформации, "ХранитьИсториюИзменений");
	
	ВидКИ = Неопределено;
	ДатаКИ = Дата(1,1,1,0,0,0);
	СтрокаКИДляДобавления = "";
	
	Для Каждого СтрокаКИ Из КонтактнаяИнф Цикл
		...
		Если РеквизитыВид[СтрокаКИ.Вид] = Истина Тогда
			Если СтрокаКИ.Вид = ВидКИ Тогда
				Продолжить;
			КонецЕсли;
			ОбработатьЗаписьСИсториейКИ(КонтактнаяИнф, СтрокаКИ, ДатаКИ, СтрокаКИДляДобавления);
...
// 
//Параметры:
// Таблица - см. УправлениеДоступом.ЗаполнитьНаборыЗначенийДоступа
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.НужноЗаписыватьВРегистрПриЗаписи Тогда
		Справочники.Контрагенты.ВыполнитьДвиженияПоРегиструДублей(Ссылка, Ложь, ИНН, КПП);
	КонецЕсли;
	...
	Если Не ЭтоГруппа И ДополнительныеСвойства.Свойство("НовыйОсновнойДоговор") Тогда
		...
		Если ДополнительныеСвойства.Свойство("ЗаписьИзФормыЭлементаКонтрагента") Тогда
			...
			СтруктураЗаполнения.Вставить("ЗаписьИзФормыЭлементаКонтрагента", Истина);
			СтруктураЗаполнения.Вставить("ОснованиеЗаполнения", Ссылка);
			СтруктураЗаполнения.Вставить("ВидОперацииКомиссия", ДополнительныеСвойства.ВидОперацииКомиссия);
			СтруктураЗаполнения.Вставить("ВидОперации", ДополнительныеСвойства.ВидОперации);
		...
		Если (ИзменилсяИНН Или ИзменилсяКПП)
			...
			Если МассивДублей.Количество() > 0 Тогда
				...
				Для Каждого ЭлементМассива Из МассивДублей Цикл
					Справочники.Контрагенты.ВыполнитьДвиженияПоРегиструДублей(ЭлементМассива, Ложь, ИНН, КПП);
				КонецЦикла;
			...
			Если ПрежнийМассивДублей.Количество() > 0 Тогда
				...
				Справочники.Контрагенты.ВыполнитьДвиженияПоРегиструДублей(Ссылка, Истина, СтруктураПрежнихЗначений.ИНН, СтруктураПрежнихЗначений.КПП);
				
				Если ПрежнийМассивДублей.Количество() = 1 Тогда
					Справочники.Контрагенты.ВыполнитьДвиженияПоРегиструДублей(ПрежнийМассивДублей[0], Истина, СтруктураПрежнихЗначений.ИНН, СтруктураПрежнихЗначений.КПП);
				КонецЕсли;
...
Процедура УдалитьРегистрациюДублейПередУдалением()
	...
	Если МассивДублей.Количество() = 1 Тогда
		...
		Справочники.Контрагенты.ВыполнитьДвиженияПоРегиструДублей(МассивДублей[0], Истина, ИНН, КПП);
