...
Процедура ЗадачиНаВесьДеньКалендарьПриАктивизацииЯчейки(Элемент)
	...
	СостоянияДокументовКлиент.ПриАктивизацииЯчейкиЗадачиНаВесьДень(ЗадачиНаВесьДеньКалендарь,
		Элементы.ЗадачиНаВесьДеньКалендарь, 
...
Процедура Подключаемый_ИзменитьСостояние(Команда)
	...
	Иначе
		ОбрабатываемыеОбъекты = СостоянияДокументовКлиент.ДокументыВыбранныхСобытийПланировщика(
			Элементы.Планировщик);
	КонецЕсли;
	
	ОбработанныеТипыДокументов = Новый Массив;
	СостоянияДокументовКлиент.УстановитьСостояние(Команда.Имя, ДоступныеСостояния,
		ОбрабатываемыеОбъекты, ОбработанныеТипыДокументов);	
...
Процедура ОткрытьНастройкуСинхронизации(Элемент)
	Индекс = Число(СтрЗаменить(ТекущийЭлемент.Имя,"КнопкаВнешнийКалендарь",""));
	ОчиститьСообщения();
...
Процедура НачатьАвторизацию()
	...
	ОткрытьФорму("РегистрСведений.СеансовыеДанныеGoogle.Форма.ЗапросТокена", ПараметрыФормы, ЭтотОбъект, , , ,
		ОписаниеОповещения);
...
Функция СинхронизацияСGoogle()
	...
	НастроенаСинхронизацияСGoogle = ИдентификацияПриложенияGoogle И
		СинхронизацияКалендаряGoogle И ЗаполненТокенДоступа;
