...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.СпособРаспределенияПоЗаказам) Тогда
		ЗначениеМодифицированностиФормы = ЭтотОбъект.Модифицированность;
		Объект.СпособРаспределенияПоЗаказам = Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто;
		ЭтотОбъект.Модифицированность = ЗначениеМодифицированностиФормы;
	КонецЕсли;
	...
	//НДС
	ЭтоПлательщикНДС = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Объект.Организация, 
	ТекущаяДатаСеанса());
	...
	// АвторегистрацияЦен 
	АвтоматическиРегистрироватьЦеныФО = ПолучитьФункциональнуюОпцию("УчетЦенКонтрагентов") 
			И ПолучитьФункциональнуюОпцию("РегистрироватьЦеныКонтрагентов");

	Если Параметры.Ключ.Пустая() Тогда  
		УстановитьЗначениеАвтоматическиРегистрироватьЦены();
	КонецЕсли;  
	
	УстановитьВидимостьЭлементаФормыАвтоматическиРегистрироватьЦены();
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Элементы.Переместить(Элементы.ГруппаНалог, Элементы.ГруппаНДС, Элементы.ГруппаНДС);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Контрагент" Тогда
		...
		Если ВестиРасчетыПоЗаказам Тогда
			...
			УстановитьЗаголовокКомиссия(ЭтотОбъект);
		КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	КонецЕсли;
	Если ИзмененаНастройкаРасчетКомиссииВОтчетеЭквайера Тогда
		Оповестить("Запись_ДоговорЭквайринга", Объект.Ссылка);
	КонецЕсли;
...
Процедура ВидДоговораПриИзменении(Элемент)
	...
	Если Объект.ВидДоговора = ПредопределенноеЗначение("Перечисление.ВидыДоговоров.СПоставщиком") Тогда
		Объект.АвтоматическиРегистрироватьЦены = Истина;  
	Иначе
...
Процедура РасчетКомиссииВОтчетеЭквайераПриИзменении(Элемент)
	...
	ИзмененаНастройкаРасчетКомиссииВОтчетеЭквайера = Истина;
	Элементы.ПроцентКомиссииЗаКредит.Видимость = НЕ Объект.РасчетКомиссииВОтчетеЭквайера;
	ПроверитьВозможностьСнятияОпцииЭквайринг();
...
&НаСервере
Процедура УстановитьВидимостьЭлементаФормыАвтоматическиРегистрироватьЦены() 
	
	ВидДоговораСПоставщиком = (Объект.ВидДоговора = Перечисления.ВидыДоговоров.СПоставщиком);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АвтоматическиРегистрироватьЦены", 
			"Видимость", АвтоматическиРегистрироватьЦеныФО И ВидДоговораСПоставщиком); 

КонецПроцедуры   

 &НаСервере
Процедура УстановитьЗначениеАвтоматическиРегистрироватьЦены() 
				
	ВидДоговораСПоставщиком = (Объект.ВидДоговора = Перечисления.ВидыДоговоров.СПоставщиком); 
	
	Объект.АвтоматическиРегистрироватьЦены = АвтоматическиРегистрироватьЦеныФО И ВидДоговораСПоставщиком;
...
Процедура ПерейтиКНастройкеВзаиморасчетов(Вариант)
	...
	Иначе
		...
		ПараметрыОткрытия.Вставить("Ключ", Объект.Владелец);
		ПараметрыОткрытия.Вставить("АктивизироватьГруппуВзаиморасчеты", Истина);
		Оповестить("ПовторноеОткрытиеФормыИзПодсказкиПоВзаиморасчетам");
...
Процедура УправлениеФормой()
	...
	ИначеЕсли ЭтоПлательщикНДС Тогда
		Если ЗначениеЗаполнено(Объект.ПорядокРегистрацииСчетовФактурНаАвансПоДоговору) Тогда
			ТекстЗаголовкаНДС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'НДС: %1'"), Строка(
				Объект.ПорядокРегистрацииСчетовФактурНаАвансПоДоговору));
		Иначе
			ТекстЗаголовкаНДС = НСтр(
				"ru = 'НДС: Регистрировать счета-фактуры на аванс в порядке, соответствующем учетной политике'");
		КонецЕсли;
	Иначе
		ТекстЗаголовкаНДС = НСтр("ru = 'НДС'");
	КонецЕсли;

	Если Объект.СпособЗаполненияСтавкиНДС = ПредопределенноеЗначение(
		"Перечисление.СпособыЗаполненияСтавкиНДС.ИзКарточкиНоменклатуры") Тогда
		ТекстЗаголовкаНДС = НСтр("ru = 'НДС: ставка в документах из карточки номенклатуры'");
	ИначеЕсли Объект.СпособЗаполненияСтавкиНДС = ПредопределенноеЗначение(
		"Перечисление.СпособыЗаполненияСтавкиНДС.БезНДС") Тогда
		ТекстЗаголовкаНДС = НСтр("ru = 'НДС: ставка в документах ""без НДС""'");
	ИначеЕсли Объект.СпособЗаполненияСтавкиНДС = ПредопределенноеЗначение(
		"Перечисление.СпособыЗаполненияСтавкиНДС.Автоматически") Тогда
		ТекстЗаголовкаНДС = НСтр("ru = 'НДС: ставка в документах определяется автоматически'");
	ИначеЕсли Объект.СпособЗаполненияСтавкиНДС = ПредопределенноеЗначение(
        "Перечисление.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН") Тогда
		ТекстЗаголовкаНДС = НСтр("ru = 'НДС: ставка в документах ""УСН 5%""'");
	ИначеЕсли Объект.СпособЗаполненияСтавкиНДС = ПредопределенноеЗначение(
        "Перечисление.СпособыЗаполненияСтавкиНДС.ОбщаяУСН") Тогда
		ТекстЗаголовкаНДС = НСтр("ru = 'НДС: ставка в документах ""УСН 7%""'");
	Иначе
		ТекстЗаголовкаНДС = НСтр("ru = 'НДС'");
	КонецЕсли;
	
	Элементы.ГруппаНДС.ЗаголовокСвернутогоОтображения = ТекстЗаголовкаНДС;
...
Процедура УстановитьВидимостьИДоступность()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПроцентКомиссииЗаКредит",		"Видимость",
		Объект.ЭтоДоговорКредита И НЕ Объект.РасчетКомиссииВОтчетеЭквайера);
	...
	// НДС с авансов в документах покупателя
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНДССАвансов", "Видимость",
		Объект.ВидДоговора = Перечисления.ВидыДоговоров.СПокупателем И ЭтоПлательщикНДС);
...
Процедура ОбновитьРеквизитыКредита()
	...
	Если Объект.ЭтоДоговорКредита Тогда
		...
		УстановитьБанковскийСчетПоУмолчанию = Ложь;
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			...
			Если МассивТерминалов.Количество() > 0 Тогда
				...
				ДанныеЭквайринговогоТерминала = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭквайринговыйТерминал,
					"БанковскийСчетЭквайринг, ВидыПлатежныхКарт");
				БанковскийСчет = ДанныеЭквайринговогоТерминала.БанковскийСчетЭквайринг;
				ВидыПлатежныхКарт = ДанныеЭквайринговогоТерминала.ВидыПлатежныхКарт.Выгрузить();
				ПроцентКомиссииЗаКредит = ?(ВидыПлатежныхКарт.Количество() > 0,
					ВидыПлатежныхКарт[0].ПроцентКомиссии, 0);
			Иначе
				УстановитьБанковскийСчетПоУмолчанию = Истина;
			КонецЕсли;
		Иначе
			УстановитьБанковскийСчетПоУмолчанию = Истина;
		КонецЕсли;
		
		Если УстановитьБанковскийСчетПоУмолчанию Тогда
			БанковскийСчет = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Организация,
...
Процедура СоздатьТерминалНаСервере()
	...
	НовыйТерминал.Касса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НовыйТерминал.Организация,
		"КассаПоУмолчанию");
	...
	СтрокаНовыйВидПлатежнойКарты.ПроцентКомиссии = ?(Объект.РасчетКомиссииВОтчетеЭквайера, 0, ПроцентКомиссииЗаКредит);
	
	НовыйТерминал.Наименование = СтрШаблон("%1 (%2%3); ""%4""",
		Объект.Владелец, СтрокаНовыйВидПлатежнойКарты.ПроцентКомиссии, "%", Объект.Наименование);
