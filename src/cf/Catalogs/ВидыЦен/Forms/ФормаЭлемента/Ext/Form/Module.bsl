...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И Параметры.Свойство("РассчитыватьАвтоматически")
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "РассчитыватьАвтоматически", "Доступность", Ложь);
	...
	Если ИспользоватьУстановкуЦен Тогда
		...
		ВидимостьКнопкиРасчетаЦен = НЕ Объект.РассчитыватьАвтоматически 
			И (Объект.ТипВидаЦен = КэшЗначений.ДинамическийПроцент
			ИЛИ Объект.ТипВидаЦен = КэшЗначений.ДинамическийФормула);
   
	КонецЕсли;

	// Видимость кнопок подменю "Установить цены".
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаРасчитатьЦеныСправочникНоменклатура", "Видимость",
		ВидимостьКнопкиРасчетаЦен);  
	...
	ЗаполнитьПризнакПропуститьРегистрациюОчередиЦен();
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ЕстьОшибкаВНаименование = ПроверитьИдентификаторФормул();
	Если ЕстьОшибкаВНаименование Тогда 		
		ТекстСообщения = НСтр("ru = 'Наименование вида цен пересекается с наименованием колонок документа установка цен.'");
...
Процедура ТипВидаЦенПриИзменении(Элемент)
	...
	Объект.РассчитыватьАвтоматически = Ложь;
...
Процедура РассчитатьЦены(Команда)
	...
	// расчет позиций номенклатуры из регистра "очередь расчета".
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ЗаполнитьЗапасыПоОчередиРасчета", Истина);
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru ='Расчет данных может занять длительное время.
							|Продолжить?'");

		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаОтветаОСозданиеУстановкиЦен", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
&НаКлиенте
Процедура РасчитатьЦеныСправочникНоменклатура(Команда)
	
	// расчет позиций номенклатуры для которых есть цена в регистре "цены номенклатуры" для базового вида цен.
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("ЗаполнитьЗапасыПоВсейНоменклатуре", Истина);
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru ='Расчет измененных цен может занять длительное время.
							|Продолжить?'");
...
&НаКлиенте
Функция ПроверитьИдентификаторФормул()
	
	ЕстьОшибки = Ложь;
	
	МассивЗапрещенныхНаименований = Новый Массив; 	
	МассивЗапрещенныхНаименований.Добавить("НОМЕНКЛАТУРА"); 
	МассивЗапрещенныхНаименований.Добавить("ХАРАКТЕРИСТИКА");
	МассивЗапрещенныхНаименований.Добавить("ЕДИНИЦАИЗМЕРЕНИЯ");
	МассивЗапрещенныхНаименований.Добавить("ВИДЦЕНЫ");
	МассивЗапрещенныхНаименований.Добавить("ЦЕНА");
	МассивЗапрещенныхНаименований.Добавить("ЦЕНАСТАРАЯ");
	МассивЗапрещенныхНаименований.Добавить("ВАЛЮТАСТАРАЯ"); 
	
	Для Каждого ТекНаименование Из МассивЗапрещенныхНаименований Цикл 
		
		Если ВРег(Объект.ИдентификаторФормул) = ТекНаименование Тогда	
			ЕстьОшибки = Истина; 
			Прервать;
		КонецЕсли;
...
&НаКлиенте
Процедура УстановитьВидимостьФормаРассчитатьЦены(ЭтоДинамическийПроцент, ЭтоДинамическийФормула)
	
	ВидимостьФормаРассчитатьЦены = НЕ Объект.РассчитыватьАвтоматически 
		И (ЭтоДинамическийПроцент ИЛИ ЭтоДинамическийФормула);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаРассчитатьЦены", "Видимость",
		ВидимостьФормаРассчитатьЦены);

КонецПроцедуры
	
&НаКлиенте
Процедура УстановитьВидимостьФормаРассчитатьЦеныСправочник(ЭтоДинамическийПроцент, ЭтоДинамическийФормула)
	
	ВидимостьКомандыФормаИзмененияЦен = НЕ Объект.РассчитыватьАвтоматически 
		И (ЭтоДинамическийПроцент ИЛИ ЭтоДинамическийФормула); 
        		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаРасчитатьЦеныСправочникНоменклатура", "Видимость", 
		ВидимостьКомандыФормаИзмененияЦен); 
		
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПризнакПропуститьРегистрациюОчередиЦен()   
	
	ЭтоСтатический = (Объект.ТипВидаЦен = КэшЗначений.Статический);

	Если НЕ Объект.РассчитыватьАвтоматически ИЛИ ЭтоСтатический Тогда
		ПропуститьРегистрациюОчередиЦен = Истина; 
	Иначе
		ПропуститьРегистрациюОчередиЦен = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработкаОтветаОСозданиеУстановкиЦен(Результат, ДополнительныеПараметры) Экспорт  
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		
		ВыполнитьРасчетЦенНаСервере(ДополнительныеПараметры); 
...
Процедура УстановитьВидимостьЭлементов()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НесколькоВалютДляЦен", "Видимость",
 	...
 	ВидимостьГруппыПредупреждениеРегистрацииЦен = (ЭтоДинамическийПроцент ИЛИ ЭтоДинамическийФормула)  
		И КэшЗначений.ПоказыватьИнформациюПользователюПоРегистрацииЦен И Объект.РассчитыватьАвтоматически; 
	...
	УстановитьВидимостьФормаРассчитатьЦены(ЭтоДинамическийПроцент, ЭтоДинамическийФормула);
	УстановитьВидимостьФормаРассчитатьЦеныСправочник(ЭтоДинамическийПроцент, ЭтоДинамическийФормула);
...
Функция ПолучитьДанныеБазовойЦены(БазовыйВидЦен)
	...
	СтруктураРеквизитовБазовыйВидЦен = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(БазовыйВидЦен, "ВалютаЦены, ЦенаВключаетНДС");
	
	Возврат Новый Структура("ВалютаЦены, ЦенаВключаетНДС", ?(ЗначениеЗаполнено(БазовыйВидЦен),
		СтруктураРеквизитовБазовыйВидЦен.ВалютаЦены, Справочники.Валюты.ПустаяСсылка()), ?(ЗначениеЗаполнено(БазовыйВидЦен),
		СтруктураРеквизитовБазовыйВидЦен.ЦенаВключаетНДС, Ложь));
...
&НаКлиенте
Процедура ВыполнитьРасчетЦенНаСервере(ДополнительныеПараметры = Неопределено)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ДополнительныеПараметры <> Неопределено Тогда   
			ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыОткрытия, ДополнительныеПараметры, Ложь)	
		КонецЕсли;
	
		Если ПараметрыОткрытия.Свойство("ЗаполнитьЗапасыПоВсейНоменклатуре") Тогда        
			
			ИмяМетода = "Документы.УстановкаЦенНоменклатуры.ЗаполнитьЗапасыПоВидуЦен";
						
		ИначеЕсли ПараметрыОткрытия.Свойство("ЗаполнитьЗапасыПоОчередиРасчета") Тогда
			
			ИмяМетода = "Документы.УстановкаЦенНоменклатуры.ЗаполнитьЗапасыПоОчередиРасчета";
			  		
		КонецЕсли;  
		
		ДлительнаяОперация = НачатьЗаполнениеЦенНаСервере(ИмяМетода);
		
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПриЗавершенииРасчетаЦен", ЭтотОбъект, ПараметрыОткрытия); 
		
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Расчет цен ...'");
...
&НаКлиенте
Процедура ПриЗавершенииРасчетаЦен(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено И ЭтоАдресВременногоХранилища(Результат.АдресРезультата) Тогда  
		...
		ОбработатьРезультатЗавершенияРасчетаЦенНаСервере(Результат.АдресРезультата, ДополнительныеПараметры); 		
		
		Если ДополнительныеПараметры <> Неопределено Тогда 
			
			ПараметрыОткрытия = ДополнительныеПараметры;
			ПараметрыОткрытия.Вставить("АдресВХ", АдресВХ); 
			ПараметрыОткрытия.Вставить("ДатаРасчетаЦен", ОбщегоНазначенияКлиент.ДатаСеанса());
...
&НаСервере
Процедура ОбработатьРезультатЗавершенияРасчетаЦенНаСервере(АдресРезультата, ДополнительныеПараметры) 
	
	АдресВХ = АдресРезультата;  
	
КонецПроцедуры

&НаСервере
Функция НачатьЗаполнениеЦенНаСервере(ИмяМетода)
	
	НаименованиеФоновогоЗадания = НСтр("ru = 'Расчет цен из карточки вида цен'");
	...
	ПоказыватьХарактеристики = ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики"); 
	Период = ТекущаяДатаСеанса();
	
	Если ИмяМетода = "Документы.УстановкаЦенНоменклатуры.ЗаполнитьЗапасыПоВидуЦен" Тогда
		...
		СтруктураПараметров.Вставить("ВидыЦенВыбраныПриЗаполнении"				, Истина);
		...
		// "МассивВидовЦенВыбранныхПриЗаполнение" - будет содержать нужные виды цен
		Документы.УстановкаЦенНоменклатуры.ПолучитьДеревоВидовЦен(СтруктураПараметров); 
		
		МассивВидовЦенДляРасчета = СтруктураПараметров.МассивВидовЦенВыбранныхПриЗаполнение;  		
		МассивУникальныхВидовЦен = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивВидовЦенДляРасчета); 
		
		Если МассивУникальныхВидовЦен.Количество() > 2 Тогда 
			
			ОбработатьВыбранныеВидыЦен(МассивУникальныхВидовЦен, Объект.Ссылка); 
			
		КонецЕсли;

		Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, ИмяМетода, 
			МассивУникальныхВидовЦен, ПоказыватьХарактеристики, Период); 
			
	ИначеЕсли ИмяМетода = "Документы.УстановкаЦенНоменклатуры.ЗаполнитьЗапасыПоОчередиРасчета" Тогда
		
		Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, ИмяМетода, 
			Объект.Ссылка, ПоказыватьХарактеристики, Период); 
...
Процедура ОбработатьВыбранныеВидыЦен(МассивУникальныхВидовЦен, ВидЦенИсточник) 
	...
	Пока ИндексЭлемента >= 0 Цикл 
		...
		// вид цен из которого инициализируем создание нужно оставить.
