...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Настройки.Вставить("ОтборПродажПоНоменклатуре");
	ЗагрузкаДанныхИзВнешнегоИсточника.ПриСозданииНаСервере(Метаданные.Справочники.УсловияПредоставленияСкидокНаценок.ТабличныеЧасти.ОтборПродажПоНоменклатуре, 
		Настройки.ОтборПродажПоНоменклатуре, ЭтотОбъект, Ложь);
...
&НаКлиенте
Процедура ПодобратьОтборНоменклатуры(Команда)
	ИмяТабличнойЧасти = "ОтборПродажПоНоменклатуре";
	...
	ОбработчикПодбора = Новый ОписаниеОповещения("ОбработкаПодбораНоменклатуры", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ЗагрузитьДанныеИзВнешнегоИсточникаОтборПродаж(Команда)

	ТекущаяНастройкаЗагрузкиДанных = НастройкиЗагрузкиДанных.ОтборПродажПоНоменклатуре;
	ТекущаяНастройкаЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Справочник.УсловияПредоставленияСкидокНаценок.ТабличнаяЧасть.ОтборПродажПоНоменклатуре";
...
Процедура ВариантПредоставленияСкидкиПриИзменении()
	...
	Если Объект.ВариантПредоставленияСкидкиПоНоменклатуре = Перечисления.ВариантПредоставленияСкидкиПоНоменклатуре.ПоСегментуНоменклатуры Тогда
		...
		Элементы.ОтборПродажПоНоменклатуреПодобратьОтборНоменклатуры.Видимость = Ложь;
	Иначе
		...
		Элементы.ОтборПродажПоНоменклатуреПодобратьОтборНоменклатуры.Видимость = Истина;
	КонецЕсли;
...
Функция СформироватьАвтоНаименованиеНаКлиенте()
	...
	ИначеЕсли Объект.УсловиеПредоставления = ПредопределенноеЗначение("Перечисление.УсловияПредоставленияСкидокНаценок.ЗаНакопленныйОбъемПродаж") Тогда
		СтрокаНаименования = ""+?(Объект.КритерийОграниченияПримененияЗаОбъемПродаж = ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Количество"), "Кол-во", Объект.КритерийОграниченияПримененияЗаОбъемПродаж) + " " + 
		Объект.ВидПериода + " " + ?(Объект.ВидПериода = ПредопределенноеЗначение("Перечисление.ВидыПериодовДляНакопительныхСкидок.ВесьПериод"), "", НРег(Объект.Периодичность)) + " " + Объект.ТипСравнения + " "+Объект.ЗначениеУсловияОграничения + 
			?(Объект.КритерийОграниченияПримененияЗаОбъемПродаж = ПредопределенноеЗначение("Перечисление.КритерииОграниченияПримененияСкидкиНаценкиЗаОбъемПродаж.Количество"), НСтр("ru = ' ед.'"), " "+Объект.ВалютаОграничения);
		Если Объект.ОтборПродажПоНоменклатуре.Количество() > 0 Тогда
			СтрокаНаименования = СтрокаНаименования + ": " + ОписаниеТабличнойЧастиКлиент("ОтборПродажПоНоменклатуре", "Номенклатура");
		КонецЕсли;
...
Процедура ЗагрузкаДанныхИзВнешнегоИсточникаОбработкаРезультата(РезультатЗагрузки, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатЗагрузки) = Тип("Структура") Тогда
		...
		ИначеЕсли РезультатЗагрузки.ОписаниеДействия = "ОбработатьПодготовленныеДанные" Тогда
			...
			Если ПолноеИмяОбъектаЗаполнения = "Справочник.УсловияПредоставленияСкидокНаценок.ТабличнаяЧасть.ОтборПродажПоНоменклатуре" Тогда
