...
&НаКлиенте
Процедура ТолькоДействительныеПриИзменении(Элемент)
...
Процедура ОтборИерархияПриАктивизацииСтроки(Элемент)
	...
	Для каждого элОтбора Из Номенклатура.КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы Цикл
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Иначе
		ТекущаяСтрокаИдентификатор = Элементы.Дерево.ТекущиеДанные.Идентификатор;
	КонецЕсли;
...
Процедура ЗаполнитьДеревоИзСохраненныхДанных(ИдентификаторКаталога, АдресНастроекСтруктурыКаталога)
	...
	Иначе	
		...
		НаборЗаписей.Отбор.ИдентификаторКаталога.Установить(ИдентификаторКаталога);
		НаборЗаписей.Прочитать();
		СохраненныеДанные = НаборЗаписей.Выгрузить();
...
&НаСервере
Процедура ПреобразоватьВДеревоРекурсия(Дерево, Таблица, Идентификатор, Исключения)
	...
	Поиск = Новый Структура("РодительИдентификатор", Идентификатор);
	РезультатПоиска = Таблица.НайтиСтроки(Поиск);
	...
	Для Каждого Стр Из РезультатПоиска Цикл
		...
		Если ЗначениеЗаполнено(Стр.Идентификатор)
			...
			ПреобразоватьВДеревоРекурсия(НоваяСтрока, Таблица, Стр.Идентификатор, Исключения);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ПреобразоватьВТаблицуРекурсия(Дерево, СохраненныеДанные, РодительИдентификатор = "")
...
Функция ЗаписатьИЗакрытьСервер()
	...
	Результат.Вставить( "ЕстьНастройки", ЕстьНастройки );
	Результат.Вставить( "ТекущаяСтрокаИдентификатор", ТекущаяСтрокаИдентификатор );
	Результат.Вставить( "ДетализацияОтбора", ?(ЕстьНастройки, ДетализацияОтбора, 0) );
...
&НаСервере
Функция ДеревоКатегорий(КатегорииСписок)
...
Процедура ДобавитьСтрокиНаСервере(ИДПриемник, ИсточникМассив)
	...
	Если ДетализацияОтбора = 2 Тогда
		...
		Для Каждого Стр Из ИсточникМассив Цикл
			Если ТипЗнч(Стр) = Тип("СправочникСсылка.Номенклатура") Тогда
				СтрЭтоГруппа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта( Стр, "ЭтоГруппа" );
				Если НЕ СтрЭтоГруппа Тогда
					Возврат;
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбновитьВыбранныеТовары(Показывать)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Номенклатура, "Действителен", ТолькоДействительные
	, ВидСравненияКомпоновкиДанных.Равно, , ТолькоДействительные);
