...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВкладкаОбслуживаниеВниманиеЗначения = Новый Структура;
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Параметры.Свойство("УзелОбменаССайтом") Тогда
			...
			ОбъектУзелОбменаССайтом = Объект.УзелОбменаССайтом.ПолучитьОбъект();
			
			Объект.СоответствиеСтатусовЗаказов.Загрузить(ОбъектУзелОбменаССайтом.СоответствиеСтатусовЗаказов.Выгрузить());
			Объект.ВидыЦен.Загрузить(ОбъектУзелОбменаССайтом.ВидыЦен.Выгрузить());
			Объект.СоответствиеВидовОплат.Загрузить(ОбъектУзелОбменаССайтом.СоответствиеВидовОплат.Выгрузить());
			Объект.СоответствиеСлужбДоставки.Загрузить(ОбъектУзелОбменаССайтом.СоответствиеСлужбДоставки.Выгрузить());
			Объект.СтруктураКаталога.Загрузить(ОбъектУзелОбменаССайтом.СтруктураКаталога.Выгрузить());
			Объект.СоответствиеПолейНоменклатуры.Загрузить(ОбъектУзелОбменаССайтом.СоответствиеПолейНоменклатуры.Выгрузить());
			Объект.ТипСайта = ОбъектУзелОбменаССайтом.ПротоколОбменаCMS;
			Объект.Логин = ОбъектУзелОбменаССайтом.ИмяПользователя;
...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ДиагностикаРегистрСведенийЦеныНоменклатуры" Тогда
		...
		ОткрытьФорму( "РегистрСведений.ЦеныНоменклатуры.ФормаСписка", ПараметрыФормы, ЭтотОбъект );
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ДиагностикаПрисоединенныеФайлы" Тогда
		...
		Если ЗначениеЗаполнено( ДиагностикаХарактеристика ) Тогда
			...
			ОткрытьФорму( "Справочник.НоменклатураПрисоединенныеФайлы.ФормаСписка", ПараметрыФормы, ЭтотОбъект );
		Иначе
			ПараметрыФормы = Новый Структура( "ВладелецФайла", ДиагностикаНоменклатура );
			ОткрытьФорму( "Справочник.ХарактеристикиНоменклатурыПрисоединенныеФайлы.ФормаСписка", 
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы И Объект.НастройкаЗавершена = Истина Тогда
		Если ПоказатьВопросОбОтключенииЛогаПередЗакрытиемФормы = Истина Тогда
...
Процедура ДекорацияТекстПодсказки11ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОткрытьФорму( "Обработка.СоздатьСайт.Форма.Форма",, ЭтотОбъект );
КонецПроцедуры
...
&НаКлиенте
Процедура ТехническомуСпециалистуНадписьНажатие2(Элемент)
	ПоказатьСтраницуТехническомуСпециалисту();
КонецПроцедуры
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСоответствиеСлужбДоставки
...
Процедура ТаблицаКаталоговПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ОбработатьВыбранныеГруппыСерверБезКонтекста(Элемент.ТекущиеДанные.Группы, НадписьВсеЭлементыСписка, 
	Элементы.ТаблицаКаталоговГруппыКатегории.ВыборГруппИЭлементов);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыНастройкиЗаписьНаУслугиИнтернетМагазина
...
Процедура КнопкаСоответствияОбъектовИнтернетМагазинаОткрыть(Команда)
	КлючИнтеграции = ПолучитьКлючПоНастройке( Объект.Ссылка );
	Отбор = Новый Структура( "КлючИнтеграции", КлючИнтеграции );
	ПараметрыФормы = Новый Структура( "Отбор", Отбор );
	ОткрытьФорму( "РегистрСведений.СоответствияОбъектовИнтернетМагазина.ФормаСписка",
...
Процедура КнопкаРегистрСведенийВыгруженныеТоварыИнтернетМагазинаОткрыть(Команда)
	...
	ПараметрыФормы = Новый Структура( "Отбор", Отбор );
	ОткрытьФорму( "РегистрСведений.ВыгруженныеТоварыИнтернетМагазина.ФормаСписка",
	ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор );
КонецПроцедуры
...
Процедура КнопкаОплатыССайтаОткрыть(Команда)
	...
	ОткрытьФорму( "РегистрСведений.ОплатыПокупателейССайта.ФормаСписка", ПараметрыФормы, ЭтотОбъект,
...
Процедура КнопкаЛогВыключить(Команда)
	...
	ФормаЛогВидимостьОбновитьСервер();
	Оповестить( "ЛогНастройкиИзменены", , ЭтотОбъект );
КонецПроцедуры
...
Процедура КнопкаЗаказыССайтаОткрыть(Команда)
	ПараметрыФормы = Новый Структура( "УчетнаяЗапись", Объект.Ссылка );
	ОткрытьФорму( "РегистрСведений.ЗаказыПокупателейССайта.ФормаСписка", ПараметрыФормы, 
...
Процедура ВыбратьВидыЦенУслуг(Команда)
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения( "ВыбратьВидыЦенУслугЗавершение", ЭтотОбъект ); 
	ОткрытьФорму( "Справочник.ВидыЦен.ФормаВыбора", ПараметрыФормы,
...
Процедура ВыбратьВидыЦенНоменклатуры(Команда)
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ВыбратьВидыЦенНоменклатурыЗавершение", ЭтотОбъект );
		
	ОткрытьФорму("Справочник.НастройкиИнтеграцииСИнтернетМагазином.Форма.ФормаВыбораВидовЦен",
	ПараметрыФормы, ЭтотОбъект,,,, 
...
Процедура УдалитьЗаписьНаУслугу(Команда)
	...
	ПоказатьВопрос(ОповещениеОЗакрытии, СтрШаблон(НСтр("ru = 'Удалить настройки для услуги ""%1""?'"),ТекУслуга),
	РежимДиалогаВопрос.ОКОтмена);
...
Процедура ОткрытьФормуРеквизитов(Команда)
	...
	ОткрытьФорму("Справочник.НастройкиИнтеграцииСИнтернетМагазином.Форма.ФормаВыбораРеквизитов", ПараметрыФормы,
	ЭтотОбъект,,,,ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура КнопкаЛогОткрыть(Команда)
	ПараметрыФормы = Новый Структура( "УчетнаяЗапись", Объект.Ссылка );
	ОткрытьФорму( "РегистрСведений.ОбменСВнешнимиСистемамиЛогирование.ФормаСписка", ПараметрыФормы, , УникальныйИдентификатор );
КонецПроцедуры
...
Процедура ЗаполнитьГиперссылкиЦенСервер()
	...
	Для Каждого СтрокаВидовЦен Из Объект.ВидыЦен Цикл 
		...
		ВидЦенНаименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта( СтрокаВидовЦен.ВидЦен, "Наименование" );
...
Функция ДиагностикаТовараРезультатНаФормеОбновить( ДиагностикаРезультат, ПараметрыКонтекст )
	...
	ДиагностикаТовараРезультатПодробноИзображенияКонтейнерыКоличество = 1;
...
Процедура ДиагностикаТовараРезультатНаФормеИзображенияОбновить( ДиагностикаРезультат )
	...
	Для Каждого ФайлыСтрока Из ДиагностикаРезультат.Файлы Цикл
		...
		ДиагностикаТовараРезультатКартинкуИзПрисоединенногоФайлаНаФормуДобавить( Файл, ЭтотОбъект, ЭлементИмя, ЭлементРодительИмя );
...
Функция ДиагностикаТовараВыполнитьИОбновитьНаФорме()
	...
	КаталогИдентификатор = ДиагностикаИдентификаторКаталога;
	...
	ДиагностикаРезультатАдрес = ПоместитьВоВременноеХранилище( ДиагностикаРезультат, УникальныйИдентификатор );
...
Функция ИзображенияНаФормеКонтейнерыИмяПолучить()
	...
	Для Номер = 1 По ДиагностикаТовараРезультатПодробноИзображенияКонтейнерыКоличество Цикл
...
Процедура ДиагностикаТоварПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено( ДиагностикаНоменклатура ) Тогда
		ИспользоватьХарактеристики = НоменклатураИспользоватьХарактеристики( ДиагностикаНоменклатура );
		Элементы.ДиагностикаХарактеристика.Видимость = ИспользоватьХарактеристики;
	Иначе
		Элементы.ДиагностикаХарактеристика.Видимость = Ложь;
	КонецЕсли;
...
Процедура ДиагностикаТовараИзображенияПоказатьВсе()
	...
	Если ПустаяСтрока( ДиагностикаРезультатАдрес ) Тогда
		Возврат;
	КонецЕсли;
	
	ДиагностикаРезультат = ПолучитьИзВременногоХранилища( ДиагностикаРезультатАдрес );
	...
	ДиагностикаТовараРезультатПодробноИзображенияКонтейнерыКоличество = 10;
...
Процедура ФормаЛогВидимостьОбновитьСервер()
	...
	ПоказатьВопросОбОтключенииЛогаПередЗакрытиемФормы = ЛогВключен;
	
	Элементы.ГруппаЛогВыключить.Видимость = ЛогВключен;

	ВкладкаОбслуживаниеВниманиеЗначения.Вставить( "ЛогВключен", ЛогВключен );
...
Процедура ПередЗакрытиемФормыВопросОтключениеЛогаЗавершение( Результат, Параметры ) Экспорт
	...
	ПоказатьВопросОбОтключенииЛогаПередЗакрытиемФормы = Ложь;
...
Процедура ПоказатьВопросОбОтключенииЛогаПередЗакрытиемФормы() Экспорт
	...
	ОповещениеВопросЗавершение = Новый ОписаниеОповещения( "ПередЗакрытиемФормыВопросОтключениеЛогаЗавершение", 
	ЭтотОбъект );
	...
	ПоказатьВопрос( ОповещениеВопросЗавершение, ВопросТекст, РежимДиалогаВопрос.ДаНет, , , 
	НСтр( "ru = 'Запись в Лог включена'" ) );
...
&НаКлиенте
Процедура ВыборФайлаЗавершить(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	ОбщегоНазначенияКлиент.СообщитьПользователю( НСтр( "ru = 'Выбранные файлы'" ) );
КонецПроцедуры
...
Процедура ОбновитьОтображениеВремениЗапускаИСтатистикуОбмена()
	...
	ВкладкаОбслуживаниеВниманиеЗначения.Вставить( "ЕстьОшибкиОбмена", ЕстьОшибкиОбмена );
	ФормаВкладкаОбслуживаниеВниманиеУстановитьСервер();
...
Процедура ОбновитьОтображениеНаСервере()
	...
	Заголовок = НСтр( "ru = 'Интеграция с интернет-магазином'" ) + ": "
...
Процедура ЗагрузитьТоварыИнтернетМагазинаНаКлиенте()
	...
	ОткрытьФорму("Обработка.ЗагрузитьДанныеСайта.Форма.Форма", ПараметрыФормы, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗапуститьОперациюВФоне(ПолныйОбмен = Ложь)
	...
	ЭтотОбъект, Объект.Ссылка,,,ОписаниеОповещения);
...
Процедура УдалитьУслугуНаСервереБезКонтекста(Услуга, НастройкаИнтеграции)
	...
	НаборЗаписей = РегистрыСведений.НастройкиЗаписьНаУслугиИнтернетМагазина.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ИсточникЗаписи.Установить(Перечисления.ЗаписьНаУслугиИсточник.Сайт);
...
Процедура ОбновитьНастройкиСтруктурыКаталога(СтрокаТаблицыКаталогов)
	...
	НаборЗаписей = РегистрыСведений.СтруктураКаталогаИнтернетМагазина.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.НастройкаИнтеграции.Установить(Объект.Ссылка);
	НаборЗаписей.Отбор.ИдентификаторКаталога.Установить(СтрокаТаблицыКаталогов.ИдентификаторКаталога);
	...
	НаборЗаписей.Загрузить(СохраненныеДанные);
...
Процедура ЗаполнитьТаблицуКаталоговСервер()
	...
	Если ЭтоПеренос И Объект.Ссылка.Пустая() Тогда
		...
		УзелОбменаССайтомСохраненнаяТаблицаКаталогов =
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта( Объект.УзелОбменаССайтом, "СохраненнаяТаблицаКаталогов" );
		
		СохраненнаяТаблицаКаталогов = УзелОбменаССайтомСохраненнаяТаблицаКаталогов.Получить(); 
	Иначе
...
Процедура ОткрытьФормуОтбораКаталога(АдресНастроекКомпоновки)
	...
	ОткрытьФорму("Справочник.НастройкиИнтеграцииСИнтернетМагазином.Форма.ФормаНастройкиОтбора", ПараметрыФормы, 
	ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытия);
...
Процедура ФормаВкладкаОбслуживаниеВниманиеУстановитьСервер()
	...
	Для Каждого Элемент Из ВкладкаОбслуживаниеВниманиеЗначения Цикл
...
Процедура ОбновитьОтображениеНаФормеНеобходимоВыполнитьПолныйОбмен()
	...
	ВкладкаОбслуживаниеВниманиеЗначения.Вставить( "НеобходимоВыполнитьПолныйОбмен", Объект.НеобходимоВыполнитьПолныйОбмен );
	ФормаВкладкаОбслуживаниеВниманиеУстановитьСервер();
...
Процедура ТаблицаКаталоговСохранить(ТекущийОбъект)
	...
	ТаблицаКаталоговТЗ = РеквизитФормыВЗначение("ТаблицаКаталогов", Тип("ТаблицаЗначений"));
