...
Процедура ПередЗаписью(Отказ)
	...
	Если ВыгружатьНаСайт Тогда
		...
		Если НЕ ЗначениеЗаполнено(АдресСайта) Тогда
			ОбщегоНазначения.СообщитьПользователю("Заполните поле ""Адрес сайта""",, "АдресСайта", "Объект", Отказ);
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Логин) Тогда
			ОбщегоНазначения.СообщитьПользователю("Заполните поле ""Логин""",, "Логин", "Объект", Отказ);
		КонецЕсли;
		
	Иначе
		
		Если НЕ ЗначениеЗаполнено(КаталогВыгрузки) Тогда
			ОбщегоНазначения.СообщитьПользователю("Заполните поле ""Каталог выгрузки""",,"КаталогВыгрузки","Объект", Отказ);
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ФайлЗагрузки) Тогда
			ОбщегоНазначения.СообщитьПользователю("Заполните поле ""Файл загрузки заказов""",,"ФайлЗагрузки","Объект", Отказ);
		КонецЕсли;
	...
	Если ОбменЗаписьНаУслуги И Не (ЭтоНовый() И ЗначениеЗаполнено(УзелОбменаССайтом)) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВидЦенУслуг) Тогда
			ОбщегоНазначения.СообщитьПользователю("Заполните поле ""Вид цен услуг""",, "ВидЦенУслуг", "Объект", Отказ);
		КонецЕсли;
	...
	Если СоответствиеСлужбДоставки.Количество() Тогда
		...
		Для Каждого Стр Из СоответствиеСлужбДоставки Цикл
			Если Стр.СпособДоставки <> ПредопределенноеЗначение("Перечисление.СпособыДоставки.Самовывоз")
				...
				ОбщегоНазначения.СообщитьПользователю("Заполните колонку ""Служба доставки"" в таблице доставки.",,"СоответствиеСлужбДоставки","Объект", Отказ);
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	СостояниеЗаказа = РаботаСЗаявками.ПервоеСостояниеПослеЗаявки(ВидЗаказа);
