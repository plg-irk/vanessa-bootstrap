...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ОбработатьОповещениеКлиентскаяЗаписьФайла(ИмяСобытия, Параметр, Источник);
...
Процедура ФайлШаблонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ЗначениеОтбора = Новый Структура("Расширение", Новый ФиксированныйМассив(ПодходящиеРасширения()));
...
Процедура УправлениеФормой()
	...
	ПоказатьПолеВводаФайл = ЗначениеЗаполнено(Объект.ФайлШаблон) Или ЕстьПрисоединенныеФайлыШаблона();
	Элементы.ДекорацияВыборФайла.Видимость = Не ПоказатьПолеВводаФайл;
	Элементы.ГруппаФайлШаблон.Видимость = ПоказатьПолеВводаФайл;
	
	РасширениеВыбранногоФайла = РасширениеПрисоединенногоФайла(Объект.ФайлШаблон);
	...
	Элементы.РедактированиеФайла.Доступность = ЗначениеЗаполнено(Объект.ФайлШаблон);
...
&НаКлиенте
Функция ФайлПодходящегоФормата(ДанныеФайла)
	
	РасширениеФайла = "";

	Если ДанныеФайла.ТипФайла = Тип("СправочникСсылка.ШаблоныПечатиОфисныхДокументовПрисоединенныеФайлы") Тогда
		РасширениеФайла = РасширениеПрисоединенногоФайла(ДанныеФайла.СсылкаНаФайл);
	Иначе
	...
	ФайлПодходящегоФормата = ПодходящиеРасширения().Найти(РасширениеФайла) <> Неопределено;
...
Функция ПроверитьФайлПодходящегоФормата(Файл)
	...
	ДанныеФайлаШаблона = ДанныеФайлаШаблона(Файл);
	
	ЭтоФайлПодходящегоФормата = ФайлПодходящегоФормата(ДанныеФайлаШаблона);
	Если Не ЭтоФайлПодходящегоФормата Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Выбран файл неподходящего формата.'"));
	КонецЕсли;	
	
	Возврат ЭтоФайлПодходящегоФормата;
	
КонецФункции

&НаКлиенте
Функция ДанныеФайлаШаблона(Файл)

	ТипФайла = ТипЗнч(Файл);
	
	СсылкаНаФайл = Неопределено;
	РасширениеФайла = Неопределено;
	
	Если ТипФайла = Тип("СправочникСсылка.ШаблоныПечатиОфисныхДокументовПрисоединенныеФайлы") Тогда
		СсылкаНаФайл = Файл;
	Иначе
		РасширениеФайла = Файл.Расширение;
	КонецЕсли;
	
	ДанныеФайлаШаблона = Новый Структура;
	ДанныеФайлаШаблона.Вставить("ТипФайла",     ТипФайла);
	ДанныеФайлаШаблона.Вставить("Расширение",   РасширениеФайла);
	ДанныеФайлаШаблона.Вставить("СсылкаНаФайл", СсылкаНаФайл);

	Возврат ДанныеФайлаШаблона;
			
КонецФункции

&НаСервере
Процедура НайтиДобавляемыеПараметры(ДобавляемыеПараметрыДокумента, ДобавляемыеПараметрыТаблицыДокумента,
	ДобавляемыеКартинкиДокумента, СписокПараметров = Неопределено)
	...
	Для каждого Параметр Из СписокПараметров Цикл
		...
		НайтиДобавляемыеПараметры(ДобавляемыеПараметрыДокумента, ДобавляемыеПараметрыТаблицыДокумента,
			ДобавляемыеКартинкиДокумента, Параметр.ПолучитьЭлементы());
...
&НаСервереБезКонтекста
Функция РасширениеПрисоединенногоФайла(Знач ПрисоединенныйФайл)
...
&НаСервере
Функция ЕстьПрисоединенныеФайлыШаблона()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ШаблоныПечатиОфисныхДокументовПрисоединенныеФайлы.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ШаблоныПечатиОфисныхДокументовПрисоединенныеФайлы КАК ШаблоныПечатиОфисныхДокументовПрисоединенныеФайлы
	|ГДЕ
	|	ШаблоныПечатиОфисныхДокументовПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайла
	|	И ШаблоныПечатиОфисныхДокументовПрисоединенныеФайлы.Расширение В(&ПодходящиеРасширения)";
	
	Запрос.УстановитьПараметр("ВладелецФайла", Объект.Ссылка);
	Запрос.УстановитьПараметр("ПодходящиеРасширения", ПодходящиеРасширения());
...
Функция ПодходящиеРасширения()
	
	ПодходящиеРасширения = Новый Массив;
	ПодходящиеРасширения.Добавить("docx");
	ПодходящиеРасширения.Добавить("odt");
	Возврат ПодходящиеРасширения;
	
КонецФункции

&НаКлиенте
Процедура ОбработатьОповещениеКлиентскаяЗаписьФайла(ИмяСобытия, Параметр, Источник)
	...
	Если Параметр.Свойство("ЭтоНовый") И Параметр.ЭтоНовый
		...
		ДанныеФайлаШаблона = ДанныеФайлаШаблона(ФайлШаблон);
		
		ЭтоФайлПодходящегоФормата =  ФайлПодходящегоФормата(ДанныеФайлаШаблона);
		Если ЭтоФайлПодходящегоФормата Тогда
