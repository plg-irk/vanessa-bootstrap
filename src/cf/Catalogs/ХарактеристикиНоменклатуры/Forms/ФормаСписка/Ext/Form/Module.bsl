...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Отбор") И Параметры.Отбор.Свойство("Владелец") Тогда
		...
		Если ТипЗнч(ОбъектВладелец) = Тип("СправочникСсылка.Номенклатура") Тогда
			...
			Номенклатура = ОбъектВладелец;
			
			Если ЗначениеЗаполнено(Номенклатура) Тогда
				
				ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				Номенклатура, "ИспользоватьХарактеристики, ЭтоНабор, ПроверятьЗаполнениеХарактеристики, ТипНоменклатуры, КатегорияНоменклатуры");
			...
			Иначе
				...
				ЗначенияРеквизитов.Вставить("ИспользоватьХарактеристики", Ложь);
				ЗначенияРеквизитов.Вставить("ЭтоНабор", Ложь);
				ЗначенияРеквизитов.Вставить("ПроверятьЗаполнениеХарактеристики", Ложь);
				ЗначенияРеквизитов.Вставить("ТипНоменклатуры", Перечисления.ТипыНоменклатуры.ПустаяСсылка());
				ЗначенияРеквизитов.Вставить("КатегорияНоменклатуры", Справочники.КатегорииНоменклатуры.ПустаяСсылка());
				
			КонецЕсли;
			
			НастройкиЗагрузкиДанных.Вставить("ОбщееЗначение", Номенклатура);
			Если ЗначениеЗаполнено(ЗначенияРеквизитов.КатегорияНоменклатуры) Тогда
				НаборДопРеквизитов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗначенияРеквизитов.КатегорияНоменклатуры, "НаборСвойствХарактеристики");
			Иначе
				НаборДопРеквизитов = Справочники.НаборыДополнительныхРеквизитовИСведений.ПустаяСсылка();
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ОбъектВладелец) = Тип("СправочникСсылка.КатегорииНоменклатуры") Тогда
			
			КатегорияНоменклатуры = ОбъектВладелец;
			...
			НастройкиЗагрузкиДанных.Вставить("ОбщееЗначение", КатегорияНоменклатуры);
			Если ЗначениеЗаполнено(КатегорияНоменклатуры) Тогда
				НаборДопРеквизитов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КатегорияНоменклатуры, "НаборСвойствХарактеристики");
			Иначе
				НаборДопРеквизитов = Справочники.НаборыДополнительныхРеквизитовИСведений.ПустаяСсылка();
			КонецЕсли;
...
&НаКлиенте
Процедура РазвернутьИзображение(Элемент)
	
	СвернутьРазвернутьИзображение();
	
КонецПроцедуры

&НаКлиенте
Процедура СвернутьИзображение(Элемент)
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("СозданиеФормы" + РаботаСФормойДокументаКлиентСервер.ПолучитьИмяФормыСтрокой(ИмяФормы));
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("ОткрытиеФормы" + РаботаСФормойДокументаКлиентСервер.ПолучитьИмяФормыСтрокой(ИмяФормы));
...
Процедура СписокПередУдалением(Элемент, Отказ)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("УстановитьПометкуНаУдалениеКлиент", ЭтотОбъект, ВыделенныеСтроки);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Номенклатура) И Не Элементы.Список.ТекущиеДанные.ЭтоКатегория
		...
		ОткрытьФорму("Справочник.ХарактеристикиНоменклатуры.ФормаОбъекта",ПараметрыОткрытия,ЭтотОбъект);
...
Процедура ДобавитьПросмотрИзображений()
	...
	Если ЗначениеЗаполнено(ТекущаяХарактеристика) Тогда
		ФайлКартинки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяХарактеристика, "ФайлКартинки");
	Иначе
		ФайлКартинки = Справочники.ХарактеристикиНоменклатурыПрисоединенныеФайлы.ПустаяСсылка();
	КонецЕсли;
	
	Если НЕ ФайлКартинки.Пустая() Тогда
		
		ДвоичныеДанныеКартинки = РаботаСФайламиУНФ.СсылкаНаДвоичныеДанныеФайла(ФайлКартинки, УникальныйИдентификатор);
		
		Если ДвоичныеДанныеКартинки <> Неопределено Тогда
			НоваяСтрока = Изображения.Вставить(0);
			НоваяСтрока.Ссылка = ФайлКартинки;
...
&НаКлиенте
Процедура СвернутьРазвернутьИзображение()
...
Функция ПолучитьЗначениеПоУмолчаниюДляКатегорииНоменклатуры() 
	...
	Категория = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "КатегорияНоменклатуры");
	Если Не ЗначениеЗаполнено(Категория) Тогда
		Возврат Неопределено
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("КатегорияНоменклатуры", Категория);
...
Процедура ИспользоватьКакОсновнуюПервуюХарактеристикуСервер(СсылкаНаНовуюХарактеристику)
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		РеквзизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "ПроверятьЗаполнениеХарактеристики,КатегорияНоменклатуры");
	ИначеЕсли ЗначениеЗаполнено(КатегорияНоменклатуры) Тогда
		ПроверятьХарактеристикуНаЗаполнение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КатегорияНоменклатуры, "ПроверятьЗаполнениеХарактеристики");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Номенклатура)
		И НЕ РеквзизитыНоменклатуры.ПроверятьЗаполнениеХарактеристики
		И ПроверятьЗаполнениеХарактеристики Тогда
		
		Если КоличествоХарактеристик(Номенклатура, РеквзизитыНоменклатуры.КатегорияНоменклатуры) = 1
			И НоменклатураВДокументахСервер.ЗначенияНоменклатурыПоУмолчанию(Номенклатура) = Неопределено Тогда
			ИспользоватьКакОсновнуюСервер(Номенклатура,  СсылкаНаНовуюХарактеристику);
		КонецЕсли;
		
	ИначеЕсли ЗначениеЗаполнено(КатегорияНоменклатуры)
		И Не ПроверятьХарактеристикуНаЗаполнение
		И ПроверятьЗаполнениеХарактеристики Тогда
		
		Если КоличествоХарактеристик(КатегорияНоменклатуры) = 1 Тогда
