...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ВладелецФормы) = Тип("ТаблицаФормы") И НЕ ВладелецФормы.ТекущиеДанные = Неопределено
		Тогда
		ТолькоПросмотр = ?(ВладелецФормы.ТекущиеДанные.Свойство("ЭтоКатегория") И Не ЭтоНовый, ВладелецФормы.ТекущиеДанные.ЭтоКатегория, Ложь);
		Элементы.СвойстваИЗначения.ТолькоПросмотр = ТолькоПросмотр;
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АдресКартинкиКонтекстноеМенюДобавитьИзображение", "Доступность", Не ТолькоПросмотр);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АдресКартинкиКонтекстноеМенюУстановитьИзображениеОсновным", "Доступность", Не ТолькоПросмотр);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АдресКартинкиКонтекстноеМенюУдалитьИзображение", "Доступность", Не ТолькоПросмотр);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаСкопировать", "Доступность", Не ТолькоПросмотр);
		Если ЭтоКопия И ТолькоПросмотр Тогда
			Отказ = Истина;
		КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.Ссылка) Тогда
		ФайлКартинки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийОбъект.Ссылка, "ФайлКартинки");
	Иначе
		ФайлКартинки = Справочники.ХарактеристикиНоменклатурыПрисоединенныеФайлы.ПустаяСсылка();
	КонецЕсли;
	
	ИзмененаОсновнаяКартинка = ТекущийОбъект.ФайлКартинки <> ФайлКартинки;
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	ОбработкаНаименованияПриИзменении();
...
&НаКлиенте
Процедура ДекорацияИндикацияОсновнойКартинкиНажатие(Элемент)
	УстановкаИзображенияОсновным();
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияИндикацияНеОсновнойКартинкиНажатие(Элемент)
	УстановкаИзображенияОсновным();
КонецПроцедуры

#КонецОбласти

#Область ОбработчикиСобытийЭлементовТаблицыФормыСвойства_ТаблицаСвойстваИЗначения
...
Процедура КнопкаСортировкаФайловНаСайте(Команда)
	ОбработкаКнопкиСортировкиФайловНаСайте();
КонецПроцедуры
...
Процедура КомандыИзображения(Команда)
	...
	Если Не ЭтоОсновноеИзображение И ЕстьИзображения Тогда
		ПредставлениеКоманды = НСтр("ru='Сделать основным'")
		+ Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП;
		СписокДоступныхКоманд.Добавить("УстановитьИзображениеОсновным", ПредставлениеКоманды, , БиблиотекаКартинок.ИзображениеОсновное);
	КонецЕсли;
	
	ПредставлениеКоманды = НСтр("ru='Добавить'")
	+ Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП
	+ Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП;
	СписокДоступныхКоманд.Добавить("ДобавитьИзображение", ПредставлениеКоманды, , БиблиотекаКартинок.СоздатьЭлементСписка);
	...
	Если ОтображатьКнопкуСортировкиНаФорме Тогда
		ПредставлениеКоманды = НСтр("ru='Настроить состав и сортировку для сайта'")
		+ Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП + Символы.НПП;
		СписокДоступныхКоманд.Добавить("КнопкаСортировкаФайловНаСайте", ПредставлениеКоманды,,БиблиотекаКартинок.ДоступВИнтернет32);
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработкаКнопкиСортировкиФайловНаСайте()
...
&НаКлиенте
Процедура УстановкаИзображенияОсновным()
...
&НаКлиенте
Процедура ОбработкаНаименованияПриИзменении()
...
Функция ПолучитьНоменклатурнуюГруппуВладельца(НоменклатураВладелец)
	...
	Если ЗначениеЗаполнено(НоменклатураВладелец) Тогда
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоменклатураВладелец, "КатегорияНоменклатуры");
	Иначе
