...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Отбор.Свойство("Владелец") Тогда
		Если ТипЗнч(Параметры.Отбор.Владелец) = Тип("СправочникСсылка.Номенклатура") Тогда
			...
			Если ЗначениеЗаполнено(Номенклатура) Тогда
				...
				Номенклатура, "КатегорияНоменклатуры,ИспользоватьХарактеристики,ТипНоменклатуры");
			Иначе
				ДанныеНоменклатуры = Новый Структура("КатегорияНоменклатуры,ИспользоватьХарактеристики,ТипНоменклатуры",
				Справочники.КатегорииНоменклатуры.ПустаяСсылка(), Ложь, Перечисления.ТипыНоменклатуры.ПустаяСсылка());
			КонецЕсли;
			
			КатегорияНоменклатуры = ДанныеНоменклатуры.КатегорияНоменклатуры;
			ИспользоватьХарактеристики = ДанныеНоменклатуры.ИспользоватьХарактеристики;
			ТипНоменклатуры = ДанныеНоменклатуры.ТипНоменклатуры;
			
			ТекстСообщения = "";
			Если НЕ ЗначениеЗаполнено(Номенклатура) Тогда
				ТекстСообщения = НСтр("ru = 'Не заполнена номенклатура!'");
			ИначеЕсли Параметры.Свойство("ЭтоПриходныйДокумент") И ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
	...
	Если ЗначениеЗаполнено(КатегорияНоменклатуры) Тогда
		...
		ПорядокСортировки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КатегорияНоменклатуры, "ПорядокСортировки").Выгрузить();
		
		Если ПорядокСортировки.Количество() Тогда
			
			Список.Порядок.Элементы.Очистить();
			
			СвойстваПорядка = Новый Массив;
			Для Каждого СтрокаПорядокСортировки Из ПорядокСортировки Цикл
				СвойстваПорядка.Добавить(СтрокаПорядокСортировки.Свойство);
			КонецЦикла;
			
			//@skip-check bsl-legacy-check-string-literal
			НаименованияСвойств = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СвойстваПорядка, "Наименование");
			
			Для Каждого СтрокаПорядокСортировки Из ПорядокСортировки Цикл
				
				НаименованиеСвойства = НаименованияСвойств[СтрокаПорядокСортировки.Свойство];
				Если НаименованиеСвойства = Неопределено Тогда
				...
				ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных("Ссылка." + НаименованияСвойств[СтрокаПорядокСортировки.Свойство]);
...
Процедура УстановитьОтборПоЗначениямХарактеристики(Характеристика)
	...
	Если ЗначениеЗаполнено(Характеристика) Тогда
		ДополнительныеРеквизиты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Характеристика, "ДополнительныеРеквизиты").Выгрузить();
	Иначе
	...
	Для каждого Эл Из Свойства_ТаблицаСвойстваИЗначения Цикл
		...
		ЗначениеХарактеристики = ДополнительныеРеквизиты.Найти(Эл.Свойство, "Свойство");
