...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ГрупповоеИзменениеСтрокСервер.ПриСозданииНаСервере(НаборЭлементовГрупповогоИзмененияСтрокСостав(ЭтотОбъект),
		СоставИзменениеСтрокДействие);
	ГрупповоеИзменениеСтрокСервер.ПриСозданииНаСервере(НаборЭлементовГрупповогоИзмененияСтрокОперации(ЭтотОбъект),
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ПриСозданииНаСервере(Метаданные.Справочники.Спецификации.ТабличныеЧасти.Состав,
...
Процедура НедействителенПриИзменении(Элемент)
	...
	Если ЭлементОсновной() Тогда
		...
		ПоказатьПредупреждение(ОповещениеОЗакрытии, НСтр(
			"ru = 'Для установки ""Недействителен"" необходимо снять свойство ""Основной"".'"), , НСтр(
			"ru = 'Элемент выбран основным'"));
	КонецЕсли;
...
Процедура ОперацииПометкаПриИзменении(Элемент)
	...
	ГрупповоеИзменениеСтрокКлиент.СтрокаПометкаПриИзменении(Объект.Операции, Элементы.ОперацииУстановитьФлажки,
		Элементы.ОперацииСнятьФлажки);
...
Процедура ПодборСостав(Команда)
	...
	КонецЕсли; 
	ОткрытьФорму("Справочник.Номенклатура.ФормаВыбора", СтруктураОткрытия, Элементы.Состав, , , , ,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПодборОперации(Команда)
	...
	КонецЕсли; 
	ОткрытьФорму("Справочник.Номенклатура.ФормаВыбора", СтруктураОткрытия, Элементы.Операции, , , , ,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗаполнитьПоБазовой(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ЗаполнитьПоБазовойЗавершение", ЭтотОбъект);
	ПоказатьВопрос(Оповещение, НСтр(
		"ru = 'Спецификация будет полностью перезаполнена по базовой. Продолжить выполнение операции?'"),
...
Процедура ЗагрузкаДанныхИзВнешнегоИсточника(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗагрузкаДанныхИзВнешнегоИсточникаОбработкаРезультата", ЭтотОбъект, НастройкиЗагрузкиДанных);
		...
		Для каждого Идентификатор Из Элементы.Состав.ВыделенныеСтроки Цикл
			...
			Если СтрокаТабличнойЧасти.ИспользоватьФормулуНоменклатура
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр(
					"ru='Копирование строк с формулами не поддерживается.'"));
				Возврат;
			КонецЕсли; 
		КонецЦикла; 
	КонецЕсли; 
...
Процедура УправлениеФормой(Форма)
	...
	// Этапы производства
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоставЭтап", "Видимость",
		ЗначениеЗаполнено(Объект.ВидПроизводства));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОперацииЭтап", "Видимость",
		ЗначениеЗаполнено(Объект.ВидПроизводства));
	
	// Параметрические спецификации
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоставИспользоватьФормулуНоменклатура",
		"Видимость", КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоставФормулаНоменклатура", "Видимость",
		КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоставФормулаХарактеристика", "Видимость",
		КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоставИспользоватьФормулуКоличество",
		"Видимость", КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоставФормулаКоличество", "Видимость",
		КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОперацииИспользоватьФормулуОперация",
		"Видимость", КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОперацииФормулаОперация", "Видимость",
		КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОперацииИспользоватьФормулуКоличество",
		"Видимость", КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОперацииФормулаНормаВремени", "Видимость",
		КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОперацииФормулаКоличество", "Видимость",
		КэшЗначений.ИспользоватьПараметрическиеСпецификации И Не ЗначениеЗаполнено(Объект.Заказ));
	
КонецПроцедуры

&НаСервере
Функция ОбработатьПодготовленныеДанные(Знач РезультатЗагрузки)
	
	ПараметрыВызоваСервера = Новый Структура;
	ПараметрыВызоваСервера.Вставить("НастройкиЗагрузкиДанных"	, РезультатЗагрузки.НастройкиЗагрузкиДанных);
	ТаблицаСопоставления = ДанныеФормыВЗначение(РезультатЗагрузки.ТаблицаСопоставленияДанных, Тип("ТаблицаЗначений"));
	ПараметрыВызоваСервера.Вставить("ТаблицаСопоставленияДанных", ТаблицаСопоставления);
	...
	ИмяМетода = "Справочники.Спецификации.ОбработатьПодготовленныеДанные";
...
Функция СоответствуетТипСтрокиТипНоменклатуры(СтруктураДанные)
	...
	ТипНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураДанные.Номенклатура, "ТипНоменклатуры");
	
	Если (СтруктураДанные.ТипСтрокиСостава = ПредопределенноеЗначение("Перечисление.ТипыСтрокСоставаСпецификации.Расход")
		И ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Запас"))
		ИЛИ (СтруктураДанные.ТипСтрокиСостава <> ПредопределенноеЗначение("Перечисление.ТипыСтрокСоставаСпецификации.Расход")
		И ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга")) Тогда
...
Функция ПараметрыСозданияДопРеквизита()
	...
	Если ЗначениеЗаполнено(КэшЗначений.КатегорияНоменклатуры) Тогда
		КэшЗначенийКатегорияНоменклатуры = КэшЗначений.КатегорияНоменклатуры; // СправочникСсылка.КатегорииНоменклатуры
		ТекущийНаборСвойств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КэшЗначенийКатегорияНоменклатуры,
...
Процедура ОткрытьКонструкторФормул(ПараметрыОткрытия, Оповещение)
	...
	ОткрытьФорму("Справочник.Спецификации.Форма.КонструкторФормул", ПараметрыОткрытия, , , , , Оповещение,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗаполнитьСлужебныеДанныеПослеЧтенияОбъекта(Объект, КэшЗначений)
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.Состав Цикл
		ИспользоватьХарактеристикиНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			СтрокаТабличнойЧасти.Номенклатура, "ИспользоватьХарактеристики");
		...
		СтрокаТабличнойЧасти.ИспользоватьХарактеристики = ИспользоватьХарактеристикиНоменклатуры;
	КонецЦикла; 
	
	Для каждого СтрокаТабличнойЧасти Из Объект.Операции Цикл
		СтрокаТабличнойЧасти.ФиксированнаяСтоимость = КэшЗначений.ОперацияФиксированнаяСтоимость.Получить(
			СтрокаТабличнойЧасти.Операция);
		СтрокаТабличнойЧасти.ИспользоватьФормулуОперация = Не ПустаяСтрока(СтрокаТабличнойЧасти.ФормулаОперация)
			И Не ЗначениеЗаполнено(Объект.Заказ);
		СтрокаТабличнойЧасти.ИспользоватьФормулуКоличество = (Не ПустаяСтрока(СтрокаТабличнойЧасти.ФормулаКоличество)
			Или Не ПустаяСтрока(СтрокаТабличнойЧасти.ФормулаНормаВремени)) И Не ЗначениеЗаполнено(Объект.Заказ);
	КонецЦикла; 	
...
Процедура ОбновитьСопоставление(ТекущаяСтрока, ДанныеКонструктора, ИмяТабличнойЧасти)
	...
	Если ТекущаяСтрока.ИспользуетсяСопоставление Тогда
		...
		КонецЕсли; 
	ИначеЕсли ТекущаяСтрока.Свойство("ИспользуетсяРасширение") И ТекущаяСтрока.ИспользуетсяРасширение Тогда
		Если ИмяТабличнойЧасти="СопоставлениеОпераций" Тогда
			ИмяПоляФормула = "ФормулаОперация";
		Иначе
			ИмяПоляФормула = "ФормулаНоменклатура";
		...
		Если НЕ ПустаяСтрока(ДанныеКонструктора.ИдентификаторСпецификации) Тогда
			ТекущаяСтрока[ИмяПоляФормула] = СтрШаблон(НСтр("ru = '<Расширение: %1>'"),
				ДанныеКонструктора.ИдентификаторСпецификации);
		Иначе
		...
		Иначе
			...
			Для каждого Свойство Из КэшЗначений.ПроверяемыеДопРеквизиты Цикл
				...
				Для каждого СтрокаТабличнойЧасти Из Строки Цикл
					...
					Элемент = Форма.Элементы.Найти(СтрокаТабличнойЧасти.ИмяРеквизитаЗначение);
					Если Элемент <> Неопределено И ТипЗнч(Элемент) = Тип("ПолеФормы") И Элемент.Вид
...
Процедура ЗагрузкаДанныхИзВнешнегоИсточникаОбработкаРезультата(РезультатЗагрузки, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатЗагрузки) = Тип("Структура") Тогда
		...
		ИначеЕсли РезультатЗагрузки.ОписаниеДействия = "ОбработатьПодготовленныеДанные" Тогда
			...
			СтруктураПараметров = Новый Структура;   
			СтруктураПараметров.Вставить("РезультатЗагрузки", РезультатЗагрузки);
...
&НаСервере
Процедура ОбработатьРезультатЗагрузкиИзФайлаНаСервере(РезультатЗагрузки) 
	...
	Если РезультатЗагрузки <> Неопределено Тогда
		...
		Если РезультатЗагрузки.Свойство("МассивСопоставленияДанных") Тогда
			
			МассивСопоставленияДанных = РезультатЗагрузки.МассивСопоставленияДанных; 	
			Если МассивСопоставленияДанных <> Неопределено Тогда 
				
				Для Каждого ЭлементМассива Из МассивСопоставленияДанных Цикл
					
					Если ЭлементМассива[ЗагрузкаДанныхИзВнешнегоИсточника.ИмяСлужебногоПоляЗагрузкаВПриложениеВозможна()] Тогда
						
						ЗаполнитьЗначенияСвойств(Объект.Состав.Добавить(), ЭлементМассива);
...
&НаКлиенте
Процедура ПослеВыполненияФоновогоЗадания(РезультатЗагрузки, ДополнительныеПараметры) Экспорт
	
	Если РезультатЗагрузки.Статус = "Выполнено" Тогда  
		
		ОбработатьРезультатЗагрузкиИзФайлаНаСервере(РезультатЗагрузки);    
		ЗаполнитьПризнакиИспользованияХарактеристик();
