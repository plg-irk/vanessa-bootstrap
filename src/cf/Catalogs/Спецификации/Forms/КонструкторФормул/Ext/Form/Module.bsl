...
Процедура ОК(Команда)
	...
	// для формулы проверка
	Если Режим = 0 Тогда  
		
		ОчиститьСообщения();
		ПроверитьФормулуНаСервере(Ошибки);
		
		// если пройдена основная проверка формулы продолжим логику.
		Если Ошибки = Неопределено Тогда
			
			ОбработатьДанныеНаКлиенте();			
...
&НаКлиенте
Процедура ОбработатьДанныеНаКлиенте() 
...
Процедура ЗаполнитьДеревоОперандов()
	...
	ИспользоватьПереработку = ПолучитьФункциональнуюОпцию("ПередачаСырьяВПереработку");
	...
	Для каждого Поле Из ДоступныеПоля Цикл
		...
		Если ИмяПоля = "ЗаказПоставщику" И Не ИспользоватьПереработку Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ИмяПоля = "ЗаказПокупателя" ИЛИ ИмяПоля = "ЗаказНаПроизводство" ИЛИ ИмяПоля = "ЗаказПоставщику" Тогда
		...
		Для каждого Реквизит Из Поле.Элементы Цикл
			...
			ЭтоСтрокаПродукции = (ИмяРеквизита = "ЗаказПокупателя.Запасы" ИЛИ ИмяРеквизита = "ЗаказНаПроизводство.Продукция" ИЛИ ИмяРеквизита = "ЗаказПоставщику.Запасы");
