...
Процедура ОбработатьПодготовленныеДанные(СтруктураДанных, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	Если ТаблицаСопоставленияДанных.Количество() > 0 Тогда 
		...
		ПроверяемыеКолонкиТаблицыСопоставления = Справочники.Номенклатура.ЗаполнитьСтруктуруПроверяемыеКолонкиТаблицыСопоставления(ПерваяСтрокаТаблицы);
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ПараметрыСопоставления, АдресРезультата) Экспорт
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		ЗначениеПоУмолчанию = Справочники.Номенклатура.ПроверитьИСопоставитьКатегориюПоУмолчанию(ФиксированныйШаблон);  // +++ Добавил признак Экспорт в цпроцку!
		...
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьКатегориюНоменклатуры(
			СтрокаТаблицыФормы.КатегорияНоменклатуры, 
