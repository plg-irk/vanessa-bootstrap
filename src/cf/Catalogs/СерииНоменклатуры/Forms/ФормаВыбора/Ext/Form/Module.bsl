...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Отбор.Свойство("Владелец") Тогда
		...
		Если СсылкаВладелец.ИспользоватьСерииИзКатегории Тогда
			Параметры.Отбор.Владелец = СсылкаВладелец;
		КонецЕсли;
		
		ЭтоКатегория = ТипЗнч(СсылкаВладелец) = Тип("СправочникСсылка.КатегорииНоменклатуры");
		ТипВладельца = ?(ЭтоКатегория, НСтр("ru = 'категории'"), НСтр("ru = 'номенклатуры'"));
		
		Заголовок = СтрШаблон(НСтр("ru = 'Серии %1 %2'"), ТипВладельца, СсылкаВладелец);
