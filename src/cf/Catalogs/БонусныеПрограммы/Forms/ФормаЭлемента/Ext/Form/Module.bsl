...
Процедура УправлениеВидимостьюИДоступностью()
	...
	НастроитьЭлементыНачислятьПриРегистрации();
	УстановитьНастройкиЭлементовОграничения();
КонецПроцедуры
...
&НаКлиенте
Процедура НачислятьБонусыРегистрацияКартыПриИзменении(Элемент)
	НастроитьЭлементыНачислятьПриРегистрации();
	Если НЕ Объект.НачислятьБонусыРегистрацияКарты
		И Объект.КоличествоБонусовРегистрацияКарты > 1 Тогда
		Объект.КоличествоБонусовРегистрацияКарты = 1;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПерейтиКНастройкеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЕстьДействующиеПравила = ЕстьДействующиеПравила();
	
	НовоеЗначение = ЕстьДействующиеПравила.Начисления;
	Если Не ЕстьДействующиеПравилаНачисления = НовоеЗначение Тогда
		...
		АвтоматическиеОкругления.Параметры.УстановитьЗначениеПараметра("БонуснаяПрограмма", Объект.Ссылка);
	КонецЕсли;
	НовоеЗначение = ЕстьДействующиеПравила.Округления;
...
&НаСервере
Функция ЕстьДействующиеПравила()
	
	Результат = Новый Структура;
	
	Результат.Вставить("Начисления", РаботаСБонусами.ЕстьДействующиеПравилаНачисления(Объект.Ссылка));
	Результат.Вставить("Округления", РаботаСБонусами.ЕстьДействующиеПравилаОкругления(Объект.Ссылка));
...
#Область НачислятьПриРегистрации
&НаКлиенте
Процедура НастроитьЭлементыНачислятьПриРегистрации()
	Элементы.ГруппаНачислятьПриРегистрацииНастройка.Доступность = Объект.НачислятьБонусыРегистрацияКарты;
	Элементы.ГруппаНачислениеОтсрочка.Доступность = Объект.НачислятьБонусыРегистрацияКарты;
	Элементы.ГруппаСписаниеПослеНачисления.Доступность = Объект.НачислятьБонусыРегистрацияКарты;
	Элементы.ГруппаНачислениеНастройка.Доступность = Объект.ИспользоватьОтсрочкуРегистрацияКарты;
	Элементы.ГруппаСписаниеНастройка.Доступность = Объект.СписыватьБаллыРегистрацииКарты;
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьОтсрочкуРегистрацияКартыПриИзменении(Элемент)
	НастроитьЭлементыНачислятьПриРегистрации();
КонецПроцедуры

&НаКлиенте
Процедура СписыватьБаллыРегистрацииКартыПриИзменении(Элемент)
	НастроитьЭлементыНачислятьПриРегистрации();
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьОграничениеОстаткаПриИзменении(Элемент)
	УстановитьНастройкиЭлементовОграничения();
КонецПроцедуры

&НаКлиенте
Процедура УстановитьНастройкиЭлементовОграничения()
	Элементы.ГруппаНастройкаКоличестваОстатка.Доступность = Объект.ИспользоватьОграничениеОстатка;
КонецПроцедуры
#КонецОбласти
#КонецОбласти